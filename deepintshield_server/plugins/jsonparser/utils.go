...
func (p *JsonParserPlugin) getRequestID(ctx *schemas.DeepIntShieldContext, result *schemas.DeepIntShieldResponse) string {
	...
	if ctx != nil {
		if requestID, ok := ctx.Value(schemas.DeepIntShieldContextKeyRequestID).(string); ok && requestID != "" {
			return requestID
		}
	}
...
func (p *JsonParserPlugin) shouldRun(ctx *schemas.DeepIntShieldContext, requestType schemas.RequestType) bool {
...
// deepCopyDeepIntShieldResponse creates a deep copy of DeepIntShieldResponse to avoid modifying the original
func (p *JsonParserPlugin) deepCopyDeepIntShieldResponse(original *schemas.DeepIntShieldResponse) *schemas.DeepIntShieldResponse {
	if original == nil {
		return nil
	}

	// Create a new DeepIntShieldResponse
	...
	if original.ChatResponse != nil {
		result.ChatResponse = p.deepCopyDeepIntShieldChatResponse(original.ChatResponse)
	}
...
// deepCopyDeepIntShieldChatResponse creates a deep copy of DeepIntShieldChatResponse
func (p *JsonParserPlugin) deepCopyDeepIntShieldChatResponse(original *schemas.DeepIntShieldChatResponse) *schemas.DeepIntShieldChatResponse {
	if original == nil {
		return nil
	}

	result := &schemas.DeepIntShieldChatResponse{
		ID:                original.ID,
	...
	if original.Choices != nil {
		result.Choices = make([]schemas.DeepIntShieldResponseChoice, len(original.Choices))
		for i, choice := range original.Choices {
			result.Choices[i] = p.deepCopyDeepIntShieldResponseChoice(choice)
		}
	}
...
// deepCopyDeepIntShieldResponseChoice creates a deep copy of DeepIntShieldResponseChoice
func (p *JsonParserPlugin) deepCopyDeepIntShieldResponseChoice(original schemas.DeepIntShieldResponseChoice) schemas.DeepIntShieldResponseChoice {
	result := schemas.DeepIntShieldResponseChoice{
