...
// TestJsonParserPluginEndToEnd tests the integration of the JSON parser plugin with DeepIntShield.
// It performs the following steps:
// 1. Initializes the JSON parser plugin with AllRequests usage
// 2. Sets up a test DeepIntShield instance with the plugin
...
func TestJsonParserPluginEndToEnd(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("Error initializing DeepIntShield: %v", err)
	}
		...
		Input: []schemas.ChatMessage{
			{
				...
				Content: &schemas.ChatMessageContent{
					ContentStr: deepintshield.Ptr("Return a JSON object with name, age, and city fields. Example: {\"name\": \"John\", \"age\": 30, \"city\": \"New York\"}"),
				},
			},
		},
	...
	responseChan, deepintshieldErr := client.ChatCompletionStreamRequest(ctx, request)

	if deepintshieldErr != nil {
		t.Fatalf("Error in DeepIntShield request: %v", deepintshieldErr)
	}
	...
	if responseChan != nil {
		...
		for streamResponse := range responseChan {
			...
			if streamResponse.DeepIntShieldError != nil {
				t.Logf("Streaming response error: %v", streamResponse.DeepIntShieldError)
			}

			if streamResponse.DeepIntShieldChatResponse != nil {
				if streamResponse.DeepIntShieldChatResponse.Choices != nil {
					for _, outputMsg := range streamResponse.DeepIntShieldChatResponse.Choices {
...
func TestJsonParserPluginPerRequest(t *testing.T) {
		...
		Input: []schemas.ChatMessage{
			{
				Role: schemas.ChatMessageRoleUser,
				Content: &schemas.ChatMessageContent{
					ContentStr: deepintshield.Ptr("Return a JSON object with name and age fields."),
				},
			},
		},
		Params: &schemas.ChatParameters{
	...
	newContext := schemas.NewDeepIntShieldContext(context.Background(), schemas.NoDeadline).WithValue(EnableStreamingJSONParser, true)

	// Make the streaming request
	responseChan, deepintshieldErr := client.ChatCompletionStreamRequest(newContext, request)

	if deepintshieldErr != nil {
		t.Logf("Error in DeepIntShield request: %v", deepintshieldErr)
	}
	...
	if responseChan != nil {
		...
		for streamResponse := range responseChan {
			...
			if streamResponse.DeepIntShieldChatResponse != nil {
				for _, choice := range streamResponse.DeepIntShieldChatResponse.Choices {
