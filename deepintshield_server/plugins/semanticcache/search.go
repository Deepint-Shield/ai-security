...
func (plugin *Plugin) performDirectSearch(ctx *schemas.DeepIntShieldContext, req *schemas.DeepIntShieldRequest, cacheKey string) (*schemas.LLMPluginShortCircuit, error) {
...
func (plugin *Plugin) generateEmbeddingsForStorage(ctx *schemas.DeepIntShieldContext, req *schemas.DeepIntShieldRequest) error {
...
func (plugin *Plugin) performSemanticSearch(ctx *schemas.DeepIntShieldContext, req *schemas.DeepIntShieldRequest, cacheKey string) (*schemas.LLMPluginShortCircuit, error) {
	// Extract text and metadata for embedding
	text, paramsHash, err := plugin.extractTextForEmbedding(req)
	if err != nil {
	...
	strictFilters = plugin.appendTenantFilter(ctx, strictFilters)
...
func (plugin *Plugin) buildResponseFromResult(ctx *schemas.DeepIntShieldContext, req *schemas.DeepIntShieldRequest, result vectorstore.SearchResult, cacheType CacheType, threshold float64, inputTokens int) (*schemas.LLMPluginShortCircuit, error) {
	...
	if expiresAtRaw, exists := properties["expires_at"]; exists && expiresAtRaw != nil {
		...
		if validType {
			...
			if expiresAt < currentTime {
				...
				go func() {
					defer safegoroutine.Recover(plugin.logger, "semanticcache.expired-entry-delete")
...
func (plugin *Plugin) buildSingleResponseFromResult(ctx *schemas.DeepIntShieldContext, req *schemas.DeepIntShieldRequest, result vectorstore.SearchResult, responseData interface{}, cacheType CacheType, threshold float64, similarity float64, inputTokens int) (*schemas.LLMPluginShortCircuit, error) {
	provider, _, _ := req.GetRequestFields()

	responseStr, ok := semanticCacheString(responseData)
	if !ok {
		return nil, fmt.Errorf("cached response is not a string")
	}

	// Unmarshal the cached response
	var cachedResponse schemas.DeepIntShieldResponse
	if err := json.Unmarshal([]byte(responseStr), &cachedResponse); err != nil {
		return nil, fmt.Errorf("failed to unmarshal cached response: %w", err)
	}

	extraFields := cachedResponse.GetExtraFields()
	if latency := cacheHitLatency(ctx); latency > 0 {
	...
	extraFields.CacheDebug.HitType = deepintshield.Ptr(string(cacheType))
	extraFields.CacheDebug.CacheID = deepintshield.Ptr(result.ID)
	plugin.populateCacheDebugScope(extraFields.CacheDebug, ctx, cacheResolution{})
	plugin.populateGuardrailReuseDebug(extraFields.CacheDebug, ctx)
...
func (plugin *Plugin) buildStreamingResponseFromResult(ctx *schemas.DeepIntShieldContext, req *schemas.DeepIntShieldRequest, result vectorstore.SearchResult, streamData interface{}, cacheType CacheType, threshold float64, similarity float64, inputTokens int) (*schemas.LLMPluginShortCircuit, error) {
	provider, _, _ := req.GetRequestFields()
	...
	streamChan := make(chan *schemas.DeepIntShieldStreamChunk)

	go func() {
		defer close(streamChan)
		defer safegoroutine.Recover(plugin.logger, "semanticcache.cache-hit-stream")
		...
		for i, chunkData := range streamArray {
			...
			if !ok {
				if chunkBytes, ok := chunkData.([]byte); ok {
			...
			// Unmarshal the chunk as DeepIntShieldResponse
			var cachedResponse schemas.DeepIntShieldResponse
			...
			if i == len(streamArray)-1 {
				...
				cacheDebug := schemas.DeepIntShieldCacheDebug{
					CacheHit: true,
					HitType:  deepintshield.Ptr(string(cacheType)),
					CacheID:  deepintshield.Ptr(result.ID),
				}
				plugin.populateCacheDebugScope(&cacheDebug, ctx, cacheResolution{})
				plugin.populateGuardrailReuseDebug(&cacheDebug, ctx)
				if cacheType == CacheTypeSemantic {
					cacheDebug.ProviderUsed = deepintshield.Ptr(string(plugin.config.Provider))
					cacheDebug.ModelUsed = deepintshield.Ptr(plugin.config.EmbeddingModel)
			...
			streamChan <- &schemas.DeepIntShieldStreamChunk{
				DeepIntShieldTextCompletionResponse:        cachedResponse.TextCompletionResponse,
				DeepIntShieldChatResponse:                  cachedResponse.ChatResponse,
				DeepIntShieldResponsesStreamResponse:       cachedResponse.ResponsesStreamResponse,
				DeepIntShieldSpeechStreamResponse:          cachedResponse.SpeechStreamResponse,
				DeepIntShieldTranscriptionStreamResponse:   cachedResponse.TranscriptionStreamResponse,
				DeepIntShieldImageGenerationStreamResponse: cachedResponse.ImageGenerationStreamResponse,
			}
		}
	}()
...
func cacheHitLatency(ctx context.Context) int64 {
...
func (plugin *Plugin) parseStreamChunks(streamData interface{}) ([]interface{}, error) {
	...
	case []byte:
		var stringArray []string
		if err := json.Unmarshal(v, &stringArray); err != nil {
...
func semanticCacheString(value interface{}) (string, bool) {
	switch v := value.(type) {
	case string:
		return v, true
	case []byte:
