...
func setVirtualKeyGuardrailContext(ctx *schemas.DeepIntShieldContext, vk *configstoreTables.TableVirtualKey) {
	if ctx == nil || vk == nil {
		return
	}
	policyIDs := make([]string, 0, len(vk.GuardrailPolicies))
	seen := make(map[string]struct{}, len(vk.GuardrailPolicies))
	for _, policy := range vk.GuardrailPolicies {
	...
	if len(policyIDs) > 0 {
		ctx.SetValue(schemas.DeepIntShieldContextKeyGovernanceGuardrailPolicyIDs, policyIDs)
	}
}
...
func (r *BudgetResolver) EvaluateModelAndProviderRequest(ctx *schemas.DeepIntShieldContext, provider schemas.ModelProvider, model string) *EvaluationResult {
...
func (r *BudgetResolver) EvaluateUserRequest(ctx *schemas.DeepIntShieldContext, userID string, request *EvaluationRequest) *EvaluationResult {
...
func (r *BudgetResolver) EvaluateVirtualKeyRequest(ctx *schemas.DeepIntShieldContext, virtualKeyValue string, provider schemas.ModelProvider, model string, requestType schemas.RequestType) *EvaluationResult {
	...
	ctx.SetValue(schemas.DeepIntShieldContextKeyGovernanceVirtualKeyID, vk.ID)
	ctx.SetValue(schemas.DeepIntShieldContextKeyGovernanceVirtualKeyName, vk.Name)
	if strings.TrimSpace(vk.CacheKey) != "" {
		ctx.SetValue(schemas.DeepIntShieldContextKeyGovernanceVirtualKeyCacheKey, strings.TrimSpace(vk.CacheKey))
	}
	if vk.CacheEnabled != nil {
		ctx.SetValue(schemas.DeepIntShieldContextKeyGovernanceVirtualKeyCacheEnabled, *vk.CacheEnabled)
	}
	if vk.SemanticCacheEnabled != nil {
		ctx.SetValue(schemas.DeepIntShieldContextKeyGovernanceVirtualKeySemanticCacheEnabled, *vk.SemanticCacheEnabled)
	}
	if vk.CacheScopeMode != nil && *vk.CacheScopeMode != "" {
		ctx.SetValue(schemas.DeepIntShieldContextKeyGovernanceVirtualKeyCacheScopeMode, *vk.CacheScopeMode)
	}
	if len(vk.CacheMetadataScopeKeys) > 0 {
		ctx.SetValue(schemas.DeepIntShieldContextKeyGovernanceVirtualKeyCacheMetadataScopeKeys, vk.CacheMetadataScopeKeys)
	}
	if vk.CacheAllowSemanticWhenUnscoped != nil {
		ctx.SetValue(schemas.DeepIntShieldContextKeyGovernanceVirtualKeyCacheAllowSemanticWhenUnscoped, *vk.CacheAllowSemanticWhenUnscoped)
	}
	setVirtualKeyGuardrailContext(ctx, vk)
	if vk.Team != nil {
		ctx.SetValue(schemas.DeepIntShieldContextKeyGovernanceTeamID, vk.Team.ID)
		ctx.SetValue(schemas.DeepIntShieldContextKeyGovernanceTeamName, vk.Team.Name)
		if vk.Team.Customer != nil {
			ctx.SetValue(schemas.DeepIntShieldContextKeyGovernanceCustomerID, vk.Team.Customer.ID)
			ctx.SetValue(schemas.DeepIntShieldContextKeyGovernanceCustomerName, vk.Team.Customer.Name)
		}
	}
	if vk.Customer != nil {
		ctx.SetValue(schemas.DeepIntShieldContextKeyGovernanceCustomerID, vk.Customer.ID)
		ctx.SetValue(schemas.DeepIntShieldContextKeyGovernanceCustomerName, vk.Customer.Name)
	}
	if !vk.IsActive {
		return &EvaluationResult{
	...
	for _, pc := range vk.ProviderConfigs {
		if schemas.ModelProvider(pc.Provider) == provider {
			if len(pc.Keys) > 0 {
				...
				ctx.SetValue(schemas.DeepIntShieldContextKeyGovernanceIncludeOnlyKeys, includeOnlyKeys)
			}
			// Propagate key selection strategy for load balancer (no-op when feature is disabled
			// since the StrategyAwareKeySelector won't be installed)
			if pc.KeySelectionStrategy != nil && *pc.KeySelectionStrategy != "" {
				ctx.SetValue(schemas.DeepIntShieldContextKeyKeySelectionStrategy, schemas.KeySelectionStrategy(*pc.KeySelectionStrategy))
			}
			break
		}
	}
...
func (r *BudgetResolver) EvaluateVirtualKeyFiltering(ctx *schemas.DeepIntShieldContext, virtualKeyValue string, provider schemas.ModelProvider, model string, requestType schemas.RequestType) *EvaluationResult {
	// 1. Validate virtual key exists and is active
	vk, exists := r.store.GetVirtualKey(ctx, virtualKeyValue)
	if !exists {
		return &EvaluationResult{
	...
	// Set include-only keys and key selection strategy for provider config routing
	for _, pc := range vk.ProviderConfigs {
		if schemas.ModelProvider(pc.Provider) == provider {
			if len(pc.Keys) > 0 {
				includeOnlyKeys := make([]string, 0, len(pc.Keys))
