...
func TestPreLLMHook_ProviderBudgetExceeded_NoVirtualKey(t *testing.T) {
	...
	req := &schemas.DeepIntShieldRequest{
		...
		ChatRequest: &schemas.DeepIntShieldChatRequest{
			Provider: schemas.OpenAI,
			Model:    "gpt-4",
		},
	}
...
func TestPreLLMHook_ModelProviderPass_VirtualKeyBudgetExceeded(t *testing.T) {
	...
	parentCtx := context.WithValue(context.Background(), schemas.DeepIntShieldContextKeyVirtualKey, "sk-bf-test")
	parentCtx = context.WithValue(parentCtx, schemas.DeepIntShieldContextKeyRequestID, "req-1")
	ctx := schemas.NewDeepIntShieldContext(parentCtx, schemas.NoDeadline)
	req := &schemas.DeepIntShieldRequest{
		RequestType: schemas.ChatCompletionRequest,
		ChatRequest: &schemas.DeepIntShieldChatRequest{
			Provider: schemas.OpenAI,
...
func TestPreLLMHook_ModelProviderPass_VirtualKeyNotFound(t *testing.T) {
	...
	parentCtx := context.WithValue(context.Background(), schemas.DeepIntShieldContextKeyVirtualKey, "sk-bf-nonexistent")
	parentCtx = context.WithValue(parentCtx, schemas.DeepIntShieldContextKeyRequestID, "req-1")
	ctx := schemas.NewDeepIntShieldContext(parentCtx, schemas.NoDeadline)
	req := &schemas.DeepIntShieldRequest{
		RequestType: schemas.ChatCompletionRequest,
...
func TestPostHook_UpdatesProviderBudgetUsage_NoVirtualKey(t *testing.T) {
	...
	parentCtx1 := context.WithValue(context.Background(), schemas.DeepIntShieldContextKeyRequestID, "req-1")
	ctx1 := schemas.NewDeepIntShieldContext(parentCtx1, schemas.NoDeadline)
	req1 := &schemas.DeepIntShieldRequest{
		RequestType: schemas.ChatCompletionRequest,
		ChatRequest: &schemas.DeepIntShieldChatRequest{
			Provider: schemas.OpenAI,
			Model:    "gpt-4",
	...
	result1 := &schemas.DeepIntShieldResponse{
	...
	parentCtx2 := context.WithValue(context.Background(), schemas.DeepIntShieldContextKeyRequestID, "req-2")
	ctx2 := schemas.NewDeepIntShieldContext(parentCtx2, schemas.NoDeadline)
	req2 := &schemas.DeepIntShieldRequest{
		RequestType: schemas.ChatCompletionRequest,
		ChatRequest: &schemas.DeepIntShieldChatRequest{
			Provider: schemas.OpenAI,
			Model:    "gpt-4",
...
func TestPostHook_UpdatesProviderRateLimitUsage_TenantScopedVirtualKey(t *testing.T) {
	logger := NewMockLogger()
	rateLimit := buildRateLimit("rl1", 10, 1000)
	rateLimit.TenantID = "tenant-a"
	provider := buildProviderWithGovernance("openai", nil, rateLimit)
	provider.TenantID = "tenant-a"
	vk := buildVirtualKey("vk1", "sk-bf-tenant-a", "Tenant A VK", true)
	vk.TenantID = "tenant-a"
	...
	parentCtx1 := context.WithValue(context.Background(), schemas.DeepIntShieldContextKeyRequestID, "req-tenant-1")
	ctx1 := schemas.NewDeepIntShieldContext(parentCtx1, schemas.NoDeadline)
	ctx1.SetValue(schemas.DeepIntShieldContextKeyVirtualKey, "sk-bf-tenant-a")
	req1 := &schemas.DeepIntShieldRequest{
		RequestType: schemas.ChatCompletionRequest,
		ChatRequest: &schemas.DeepIntShieldChatRequest{
			Provider: schemas.OpenAI,
	...
	_, _, err = plugin.PostLLMHook(ctx1, result1, nil)
	assert.NoError(t, err, "Should successfully process PostHook for tenant-scoped provider rate limit usage update")

	time.Sleep(200 * time.Millisecond)

	parentCtx2 := context.WithValue(context.Background(), schemas.DeepIntShieldContextKeyRequestID, "req-tenant-2")
	ctx2 := schemas.NewDeepIntShieldContext(parentCtx2, schemas.NoDeadline)
	ctx2.SetValue(schemas.DeepIntShieldContextKeyVirtualKey, "sk-bf-tenant-a")
	req2 := &schemas.DeepIntShieldRequest{
		RequestType: schemas.ChatCompletionRequest,
		ChatRequest: &schemas.DeepIntShieldChatRequest{
			Provider: schemas.OpenAI,
	...
	_, shortCircuit2, _ := plugin.PreLLMHook(ctx2, req2)
	assert.NotNil(t, shortCircuit2, "Second request should fail PreLLMHook due to tenant-scoped token limit exceeded")
	assert.Contains(t, shortCircuit2.Error.Error.Message, "token limit exceeded", "Error should indicate token limit exceeded")
}
...
func TestStore_CheckModelRateLimit_ProviderSpecificPrefixedModelMatch(t *testing.T) {
	logger := NewMockLogger()
	rateLimit := buildRateLimitWithUsage("rl1", 50, 50, 1000, 0)
	providerStr := "anthropic"
	modelConfig := buildModelConfig("mc1", "claude-sonnet-4-5", &providerStr, nil, rateLimit)
	...
	errResult, decision := store.CheckModelRateLimit(context.Background(), &EvaluationRequest{Model: "anthropic/claude-sonnet-4-5", Provider: schemas.Anthropic}, nil, nil)
	assert.Error(t, errResult, "provider-specific config should match prefixed Anthropic model names")
	assert.Contains(t, errResult.Error(), "token limit exceeded")
	assert.NotEqual(t, DecisionAllow, decision)
}

func TestStore_UpdateModelRateLimitUsage_ProviderSpecificPrefixedModelMatch(t *testing.T) {
	logger := NewMockLogger()
	rateLimit := buildRateLimitWithUsage("rl1", 50, 0, 1000, 0)
	providerStr := "anthropic"
	modelConfig := buildModelConfig("mc1", "claude-sonnet-4-5", &providerStr, nil, rateLimit)
	...
	err = store.UpdateProviderAndModelRateLimitUsageInMemory(context.Background(), "anthropic/claude-sonnet-4-5", schemas.Anthropic, 50, true, false)
	assert.NoError(t, err)

	errResult, decision := store.CheckModelRateLimit(context.Background(), &EvaluationRequest{Model: "anthropic/claude-sonnet-4-5", Provider: schemas.Anthropic}, nil, nil)
	assert.Error(t, errResult, "usage updates should accrue against the provider-specific model config")
	assert.Contains(t, errResult.Error(), "token limit exceeded")
	assert.NotEqual(t, DecisionAllow, decision)
}
