...
func (re *RoutingEngine) EvaluateRoutingRules(ctx *schemas.DeepIntShieldContext, routingCtx *RoutingContext) (*RoutingDecision, error) {
	if routingCtx == nil {
	...
	for _, scope := range scopeChain {
		...
		for _, rule := range rules {
			...
			if matched {
				...
				ctx.SetValue(schemas.DeepIntShieldContextKeyGovernanceRoutingRuleID, rule.ID)
				ctx.SetValue(schemas.DeepIntShieldContextKeyGovernanceRoutingRuleName, rule.Name)
