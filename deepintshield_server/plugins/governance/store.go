...
func (gs *LocalGovernanceStore) GetGovernanceData(ctx context.Context) *GovernanceData {
	...
	gs.virtualKeys.Range(func(_, value interface{}) bool {
		...
		if !ok || vk == nil || !governanceMatchesTenant(ctx, vk.TenantID) {
		...
		virtualKeys[clone.ID] = &clone
		return true // continue iteration
	})
	teams := make(map[string]*configstoreTables.TableTeam)
	gs.teams.Range(func(_, value interface{}) bool {
		team, ok := value.(*configstoreTables.TableTeam)
		if !ok || team == nil || !governanceMatchesTenant(ctx, team.TenantID) {
			return true // continue
		}
		// Cross-reference live budget/rate limit from standalone maps
		clone := *team
		...
		teams[clone.ID] = &clone
		return true // continue iteration
	})
	customers := make(map[string]*configstoreTables.TableCustomer)
	gs.customers.Range(func(_, value interface{}) bool {
		customer, ok := value.(*configstoreTables.TableCustomer)
		if !ok || customer == nil || !governanceMatchesTenant(ctx, customer.TenantID) {
			return true // continue
		}
		// Cross-reference live budget/rate limit from standalone maps
		clone := *customer
	...
	gs.budgets.Range(func(_, value interface{}) bool {
		...
		if !ok || budget == nil || !governanceMatchesTenant(ctx, budget.TenantID) {
			return true // continue
		}
	...
	gs.rateLimits.Range(func(_, value interface{}) bool {
		...
		if !ok || rateLimit == nil || !governanceMatchesTenant(ctx, rateLimit.TenantID) {
			return true // continue
		}
		rateLimits[rateLimit.ID] = rateLimit
		return true // continue iteration
	})
	...
	gs.routingRules.Range(func(_, value interface{}) bool {
		...
		for _, rule := range rules {
			if rule != nil && governanceMatchesTenant(ctx, rule.TenantID) {
	...
	gs.modelConfigs.Range(func(_, value interface{}) bool {
		...
		if !ok || mc == nil || !governanceMatchesTenant(ctx, mc.TenantID) {
			return true // continue
		}
		// Cross-reference live budget/rate limit from standalone maps
		// (usage updates clone into budgets/rateLimits maps, so embedded pointers go stale)
	...
	gs.providers.Range(func(_, value interface{}) bool {
		...
		if !ok || p == nil || !governanceMatchesTenant(ctx, p.TenantID) {
			return true // continue
		}
		// Cross-reference live budget/rate limit from standalone maps
		clone := *p
	...
	gs.users.Range(func(_, value interface{}) bool {
...
func (gs *LocalGovernanceStore) GetVirtualKey(ctx context.Context, vkValue string) (*configstoreTables.TableVirtualKey, bool) {
	if tenantID := governanceTenantIDFromContext(ctx); tenantID != "" {
		value, exists := gs.virtualKeys.Load(governanceVirtualKeyStoreKey(tenantID, vkValue))
		if !exists || value == nil {
			return nil, false
		}
		vk, ok := value.(*configstoreTables.TableVirtualKey)
	...
	var matched *configstoreTables.TableVirtualKey
	gs.virtualKeys.Range(func(_, value interface{}) bool {
		vk, ok := value.(*configstoreTables.TableVirtualKey)
		if !ok || vk == nil {
			return true
...
func (gs *LocalGovernanceStore) getTeamByTenant(tenantID string, teamID string) (*configstoreTables.TableTeam, bool) {
	value, exists := gs.teams.Load(governanceTeamStoreKey(tenantID, teamID))
	if !exists || value == nil {
		return nil, false
	}
	team, ok := value.(*configstoreTables.TableTeam)
	return team, ok && team != nil
}

func (gs *LocalGovernanceStore) getCustomerByTenant(tenantID string, customerID string) (*configstoreTables.TableCustomer, bool) {
	value, exists := gs.customers.Load(governanceCustomerStoreKey(tenantID, customerID))
	if !exists || value == nil {
		return nil, false
	}
	customer, ok := value.(*configstoreTables.TableCustomer)
	return customer, ok && customer != nil
}

func (gs *LocalGovernanceStore) getProviderByContext(ctx context.Context, provider schemas.ModelProvider) (*configstoreTables.TableProvider, bool) {
	value, exists := gs.providers.Load(governanceProviderStoreKey(governanceTenantIDFromContext(ctx), string(provider)))
	if !exists || value == nil {
		return nil, false
	}
	providerTable, ok := value.(*configstoreTables.TableProvider)
	return providerTable, ok && providerTable != nil
}
...
func (gs *LocalGovernanceStore) CheckProviderBudget(ctx context.Context, request *EvaluationRequest, baselines map[string]float64) error {
	...
	// Get provider config
	providerTable, exists := gs.getProviderByContext(ctx, provider)
	if !exists || providerTable == nil {
...
func (gs *LocalGovernanceStore) findModelOnlyConfig(ctx context.Context, model string) (*configstoreTables.TableModelConfig, string) {
	tenantID := governanceTenantIDFromContext(ctx)
	...
	if gs.modelCatalog != nil {
		...
		if baseName != model {
			if value, exists := gs.modelConfigs.Load(governanceModelStoreKey(tenantID, baseName, nil)); exists && value != nil {
	...
	if value, exists := gs.modelConfigs.Load(governanceModelStoreKey(tenantID, model, nil)); exists && value != nil {
		if mc, ok := value.(*configstoreTables.TableModelConfig); ok && mc != nil {
			return mc, model
		}
	}
...
// findProviderSpecificModelConfig looks up a provider-specific config using the exact model string
// first, then falls back to the normalized base model name for the same provider.
func (gs *LocalGovernanceStore) findProviderSpecificModelConfig(ctx context.Context, model string, provider schemas.ModelProvider) (*configstoreTables.TableModelConfig, string) {
	if provider == "" {
		return nil, ""
	}
	...
	if value, exists := gs.modelConfigs.Load(governanceModelStoreKey(tenantID, model, &providerName)); exists && value != nil {
		if mc, ok := value.(*configstoreTables.TableModelConfig); ok && mc != nil {
			return mc, model
		}
	}
	...
	if gs.modelCatalog != nil {
		baseName := gs.modelCatalog.GetBaseModelName(model)
		if baseName != model {
			if value, exists := gs.modelConfigs.Load(governanceModelStoreKey(tenantID, baseName, &providerName)); exists && value != nil {
				if mc, ok := value.(*configstoreTables.TableModelConfig); ok && mc != nil {
					return mc, baseName
				}
			}
...
func (gs *LocalGovernanceStore) CheckModelBudget(ctx context.Context, request *EvaluationRequest, baselines map[string]float64) error {
	...
	if provider != nil {
		if mc, configKey := gs.findProviderSpecificModelConfig(ctx, model, *provider); mc != nil && mc.Budget != nil {
			modelConfigsToCheck = append(modelConfigsToCheck, mc)
			budgetNames = append(budgetNames, fmt.Sprintf("Model:%s:Provider:%s", configKey, string(*provider)))
		}
	}
	...
	if mc, configKey := gs.findModelOnlyConfig(ctx, model); mc != nil && mc.Budget != nil {
		modelConfigsToCheck = append(modelConfigsToCheck, mc)
...
func (gs *LocalGovernanceStore) CheckModelRateLimit(ctx context.Context, request *EvaluationRequest, tokensBaselines map[string]int64, requestsBaselines map[string]int64) (error, Decision) {
	...
	// Check model+provider config first (more specific) - if provider is provided
	if provider != nil {
		if mc, configKey := gs.findProviderSpecificModelConfig(ctx, model, *provider); mc != nil && mc.RateLimitID != nil {
			modelConfigsToCheck = append(modelConfigsToCheck, mc)
			rateLimitNames = append(rateLimitNames, fmt.Sprintf("Model:%s:Provider:%s", configKey, string(*provider)))
		}
	}
...
func (gs *LocalGovernanceStore) UpdateProviderAndModelBudgetUsageInMemory(ctx context.Context, model string, provider schemas.ModelProvider, cost float64) error {
	...
	if provider != "" {
		if providerTable, exists := gs.getProviderByContext(ctx, provider); exists && providerTable != nil && providerTable.BudgetID != nil {
	...
	// Check model+provider config first (more specific) - if provider is provided
	if provider != "" {
		if mc, _ := gs.findProviderSpecificModelConfig(ctx, model, provider); mc != nil && mc.BudgetID != nil {
...
func (gs *LocalGovernanceStore) UpdateProviderAndModelRateLimitUsageInMemory(ctx context.Context, model string, provider schemas.ModelProvider, tokensUsed int64, shouldUpdateTokens bool, shouldUpdateRequests bool) error {
	...
	if provider != "" {
		if providerTable, exists := gs.getProviderByContext(ctx, provider); exists && providerTable != nil && providerTable.RateLimitID != nil {
	...
	// Check model+provider config first (more specific) - if provider is provided
	if provider != "" {
		if mc, _ := gs.findProviderSpecificModelConfig(ctx, model, provider); mc != nil && mc.RateLimitID != nil {
...
func (gs *LocalGovernanceStore) rebuildInMemoryStructures(ctx context.Context, customers []configstoreTables.TableCustomer, teams []configstoreTables.TableTeam, virtualKeys []configstoreTables.TableVirtualKey, budgets []configstoreTables.TableBudget, rateLimits []configstoreTables.TableRateLimit, modelConfigs []configstoreTables.TableModelConfig, providers []configstoreTables.TableProvider, routingRules []configstoreTables.TableRoutingRule) {
	...
	for i := range customers {
		...
		gs.customers.Store(governanceCustomerStoreKey(customer.TenantID, customer.ID), customer)
	}

	// Build teams map
	for i := range teams {
		team := &teams[i]
		gs.teams.Store(governanceTeamStoreKey(team.TenantID, team.ID), team)
	}
	...
	for i := range virtualKeys {
		vk := &virtualKeys[i]
		gs.virtualKeys.Store(governanceVirtualKeyStoreKey(vk.TenantID, vk.Value), vk)
	}
	...
	for i := range modelConfigs {
		...
		if mc.Provider != nil {
			...
			key := governanceModelStoreKey(mc.TenantID, mc.ModelName, mc.Provider)
			...
			gs.modelConfigs.Store(governanceModelStoreKey(mc.TenantID, key, nil), mc)
		}
	}
	...
	for i := range providers {
		provider := &providers[i]
		gs.providers.Store(governanceProviderStoreKey(provider.TenantID, provider.Name), provider)
	}
	...
	for i := range routingRules {
		...
		key := governanceRoutingScopeStoreKey(rule.TenantID, rule.Scope, scopeID)
...
func (gs *LocalGovernanceStore) collectRateLimitsFromHierarchy(vk *configstoreTables.TableVirtualKey, requestedProvider schemas.ModelProvider) ([]*configstoreTables.TableRateLimit, []string) {
	...
	if vk.TeamID != nil {
		if team, exists := gs.getTeamByTenant(vk.TenantID, *vk.TeamID); exists && team != nil {
			if team.RateLimitID != nil {
			...
			if team.CustomerID != nil {
				...
				if customer, exists := gs.getCustomerByTenant(vk.TenantID, *team.CustomerID); exists && customer != nil {
					if customer.RateLimitID != nil {
	...
	if vk.CustomerID != nil && (teamCustomerID == "" || *vk.CustomerID != teamCustomerID) {
		if customer, exists := gs.getCustomerByTenant(vk.TenantID, *vk.CustomerID); exists && customer != nil {
			if customer.RateLimitID != nil {
				if rateLimitValue, exists := gs.rateLimits.Load(*customer.RateLimitID); exists && rateLimitValue != nil {
					if rateLimit, ok := rateLimitValue.(*configstoreTables.TableRateLimit); ok && rateLimit != nil {
						rateLimits = append(rateLimits, rateLimit)
...
func (gs *LocalGovernanceStore) UpdateVirtualKeyInMemory(vk *configstoreTables.TableVirtualKey, budgetBaselines map[string]float64, rateLimitTokensBaselines map[string]int64, rateLimitRequestsBaselines map[string]int64) {
	...
	if existingVKValue, exists := gs.virtualKeys.Load(governanceVirtualKeyStoreKey(vk.TenantID, vk.Value)); exists && existingVKValue != nil {
		...
		gs.virtualKeys.Store(governanceVirtualKeyStoreKey(vk.TenantID, vk.Value), &clone)
	} else {
...
func (gs *LocalGovernanceStore) UpdateTeamInMemory(team *configstoreTables.TableTeam, budgetBaselines map[string]float64) {
	...
	if existingTeamValue, exists := gs.teams.Load(governanceTeamStoreKey(team.TenantID, team.ID)); exists && existingTeamValue != nil {
		...
		gs.teams.Store(governanceTeamStoreKey(team.TenantID, team.ID), &clone)
	} else {
...
func (gs *LocalGovernanceStore) UpdateCustomerInMemory(customer *configstoreTables.TableCustomer, budgetBaselines map[string]float64) {
	...
	if existingCustomerValue, exists := gs.customers.Load(governanceCustomerStoreKey(customer.TenantID, customer.ID)); exists && existingCustomerValue != nil {
		...
		gs.customers.Store(governanceCustomerStoreKey(customer.TenantID, customer.ID), &clone)
	} else {
...
func (gs *LocalGovernanceStore) DeleteCustomerInMemory(customerID string) {
	...
	gs.customers.Range(func(key, value interface{}) bool {
		customer, ok := value.(*configstoreTables.TableCustomer)
		if !ok || customer == nil || customer.ID != customerID {
			return true
		}
		// Delete associated budget if exists
		if customer.BudgetID != nil {
	...
	gs.teams.Range(func(key, value interface{}) bool {
		...
		isOwner := team.CustomerID != nil && *team.CustomerID == customerID
		hasMembership := slices.ContainsFunc(team.CustomerMembers, func(member configstoreTables.TableCustomer) bool {
			return member.ID == customerID
		})
		if isOwner || hasMembership {
			...
			if hasMembership {
				clone.CustomerMembers = slices.DeleteFunc(clone.CustomerMembers, func(member configstoreTables.TableCustomer) bool {
					return member.ID == customerID
				})
			}
...
func (gs *LocalGovernanceStore) UpdateModelConfigInMemory(mc *configstoreTables.TableModelConfig) *configstoreTables.TableModelConfig {
	...
	if clone.Provider != nil {
		key := governanceModelStoreKey(clone.TenantID, clone.ModelName, clone.Provider)
		...
		gs.modelConfigs.Store(governanceModelStoreKey(clone.TenantID, key, nil), &clone)
	}
...
func (gs *LocalGovernanceStore) UpdateProviderInMemory(provider *configstoreTables.TableProvider) *configstoreTables.TableProvider {
	...
	gs.providers.Store(governanceProviderStoreKey(clone.TenantID, clone.Name), &clone)
...
func (gs *LocalGovernanceStore) DeleteProviderInMemory(providerName string) {
	...
	gs.providers.Range(func(key, value interface{}) bool {
		provider, ok := value.(*configstoreTables.TableProvider)
		if !ok || provider == nil || provider.Name != providerName {
			return true
		}
		// Delete associated budget if exists
		if provider.BudgetID != nil {
...
func (gs *LocalGovernanceStore) GetScopedRoutingRules(ctx context.Context, scope string, scopeID string) []*configstoreTables.TableRoutingRule {
	key := governanceRoutingScopeStoreKey(governanceTenantIDFromContext(ctx), scope, scopeID)
...
func (gs *LocalGovernanceStore) GetBudgetAndRateLimitStatus(ctx context.Context, model string, provider schemas.ModelProvider, vk *configstoreTables.TableVirtualKey, budgetBaselines map[string]float64, tokenBaselines map[string]int64, requestBaselines map[string]int64) *BudgetAndRateLimitStatus {
	...
	if model != "" {
		...
		if modelConfig, _ := gs.findProviderSpecificModelConfig(ctx, model, provider); modelConfig != nil {
	...
	if providerTable, ok := gs.getProviderByContext(ctx, provider); ok && providerTable != nil {
		// Get rate limit status
		if providerTable.RateLimitID != nil {
...
func (gs *LocalGovernanceStore) UpdateRoutingRuleInMemory(rule *configstoreTables.TableRoutingRule) error {
	...
	// First, remove the rule from all scopes for the same tenant (in case it was moved)
	gs.routingRules.Range(func(key, value interface{}) bool {
		rules, ok := value.([]*configstoreTables.TableRoutingRule)
		if !ok {
			return true
