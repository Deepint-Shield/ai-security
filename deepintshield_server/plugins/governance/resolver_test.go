...
func TestBudgetResolver_ContextPopulation(t *testing.T) {
	...
	vkID, _ := ctx.Value(schemas.DeepIntShieldContextKeyGovernanceVirtualKeyID).(string)
	teamID, _ := ctx.Value(schemas.DeepIntShieldContextKeyGovernanceTeamID).(string)
	customerID, _ := ctx.Value(schemas.DeepIntShieldContextKeyGovernanceCustomerID).(string)
