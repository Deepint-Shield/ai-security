...
func TestGovernanceStore_TenantScopedReads(t *testing.T) {
	logger := NewMockLogger()

	tenantAVK := *buildVirtualKey("vk-tenant-a", "sk-bf-tenant-a", "Tenant A VK", true)
	tenantAVK.TenantID = "tenant-a"
	tenantBVK := *buildVirtualKey("vk-tenant-b", "sk-bf-tenant-b", "Tenant B VK", true)
	tenantBVK.TenantID = "tenant-b"

	tenantAProvider := buildProviderWithGovernance("openai", nil, nil)
	tenantAProvider.TenantID = "tenant-a"
	tenantBProvider := buildProviderWithGovernance("openai", nil, nil)
	tenantBProvider.TenantID = "tenant-b"

	tenantARule := configstoreTables.TableRoutingRule{
		ID:            "rule-tenant-a",
		TenantID:      "tenant-a",
		Name:          "Tenant A Rule",
		Scope:         "global",
	...
	tenantBRule := configstoreTables.TableRoutingRule{
		ID:            "rule-tenant-b",
		TenantID:      "tenant-b",
		Name:          "Tenant B Rule",
		Scope:         "global",
	...
	store, err := NewLocalGovernanceStore(context.Background(), logger, nil, &configstore.GovernanceConfig{
		VirtualKeys:  []configstoreTables.TableVirtualKey{tenantAVK, tenantBVK},
		Providers:    []configstoreTables.TableProvider{*tenantAProvider, *tenantBProvider},
		RoutingRules: []configstoreTables.TableRoutingRule{tenantARule, tenantBRule},
	}, nil)
	require.NoError(t, err)

	tenantACtx := context.WithValue(context.Background(), schemas.DeepIntShieldContextKeyTenantID, "tenant-a")
	tenantBCtx := context.WithValue(context.Background(), schemas.DeepIntShieldContextKeyTenantID, "tenant-b")

	dataA := store.GetGovernanceData(tenantACtx)
	require.Len(t, dataA.VirtualKeys, 1)
	require.Len(t, dataA.Providers, 1)
	require.Len(t, dataA.RoutingRules, 1)
	assert.Equal(t, "vk-tenant-a", dataA.VirtualKeys["vk-tenant-a"].ID)
	assert.Equal(t, "tenant-a", dataA.Providers[0].TenantID)

	dataB := store.GetGovernanceData(tenantBCtx)
	require.Len(t, dataB.VirtualKeys, 1)
	require.Len(t, dataB.Providers, 1)
	require.Len(t, dataB.RoutingRules, 1)
	assert.Equal(t, "vk-tenant-b", dataB.VirtualKeys["vk-tenant-b"].ID)
	assert.Equal(t, "tenant-b", dataB.Providers[0].TenantID)

	vkA, ok := store.GetVirtualKey(tenantACtx, "sk-bf-tenant-a")
	require.True(t, ok)
	assert.Equal(t, "tenant-a", vkA.TenantID)

	vkB, ok := store.GetVirtualKey(tenantBCtx, "sk-bf-tenant-b")
	require.True(t, ok)
	assert.Equal(t, "tenant-b", vkB.TenantID)

	assert.Len(t, store.GetScopedRoutingRules(tenantACtx, "global", ""), 1)
	assert.Equal(t, "rule-tenant-a", store.GetScopedRoutingRules(tenantACtx, "global", "")[0].ID)
	assert.Len(t, store.GetScopedRoutingRules(tenantBCtx, "global", ""), 1)
	assert.Equal(t, "rule-tenant-b", store.GetScopedRoutingRules(tenantBCtx, "global", "")[0].ID)
}
...
func TestGovernanceStore_RoutingRules_CreateAndRetrieve(t *testing.T) {
	...
	rule1 := &configstoreTables.TableRoutingRule{
		...
		Targets: []configstoreTables.TableRoutingTarget{
			{Provider: deepintshield.Ptr("openai"), Model: deepintshield.Ptr("gpt-4"), Weight: 1.0},
		},
		Fallbacks:       nil,
	...
	rule2 := &configstoreTables.TableRoutingRule{
		...
		Targets: []configstoreTables.TableRoutingTarget{
			{Provider: deepintshield.Ptr("azure"), Weight: 1.0},
		},
		Fallbacks:       nil,
...
func TestGovernanceStore_RateLimitStatus(t *testing.T) {
	...
	store.providers.Store(governanceProviderStoreKey("", "openai"), providerConfig)
...
func TestCompileAndCacheProgram(t *testing.T) {
	...
	rule := &configstoreTables.TableRoutingRule{
		...
		Targets: []configstoreTables.TableRoutingTarget{
			{Provider: deepintshield.Ptr("openai")},
		},
		Enabled: true,
	}
