...
func TestEvaluateRoutingRules_NilContext(t *testing.T) {
	...
	_, err = engine.EvaluateRoutingRules(schemas.NewDeepIntShieldContext(context.Background(), time.Now()), nil)
	assert.Error(t, err)
...
func TestEvaluateRoutingRules_NoRulesMatch(t *testing.T) {
	...
	decision, err := engine.EvaluateRoutingRules(schemas.NewDeepIntShieldContext(context.Background(), time.Now()), ctx)
	assert.NoError(t, err)
	assert.Nil(t, decision)
}
...
func TestEvaluateRoutingRules_GlobalRuleMatches(t *testing.T) {
	store, err := NewLocalGovernanceStore(context.Background(), NewMockLogger(), nil, &configstore.GovernanceConfig{}, nil)
	require.NoError(t, err)
	bgCtx := schemas.NewDeepIntShieldContext(context.Background(), time.Now())
	...
	rule := &configstoreTables.TableRoutingRule{
		...
		Targets: []configstoreTables.TableRoutingTarget{
			{Provider: deepintshield.Ptr("azure"), Model: deepintshield.Ptr("gpt-4-turbo"), Weight: 1.0},
		},
		Enabled:  true,
		Scope:    "global",
		Priority: 0,
...
// DeepIntShieldContext under DeepIntShieldContextKeyAPIKeyID.
func TestEvaluateRoutingRules_MultiTargetDeterministicWithPinnedKey(t *testing.T) {
	...
	rule := &configstoreTables.TableRoutingRule{
		...
		Targets: []configstoreTables.TableRoutingTarget{
			{
				Provider: deepintshield.Ptr("azure"),
				Model:    deepintshield.Ptr("gpt-4-turbo"),
				KeyID:    deepintshield.Ptr(pinnedKeyID),
				Weight:   1.0,
			},
			{
				Provider: deepintshield.Ptr("openai"),
				Model:    deepintshield.Ptr("gpt-3.5"),
				Weight:   0.0,
			},
		},
		Enabled:  true,
	...
	// assert the pinned key_id is visible in the DeepIntShieldContext.
	if decision.KeyID != "" {
		bgCtx.SetValue(schemas.DeepIntShieldContextKeyAPIKeyID, decision.KeyID)
	}
	ctxKeyID, _ := bgCtx.Value(schemas.DeepIntShieldContextKeyAPIKeyID).(string)
...
func TestEvaluateRoutingRules_ScopePrecedence(t *testing.T) {
	...
	globalRule := &configstoreTables.TableRoutingRule{
		...
		CelExpression: "model == 'gpt-4o'",
		Targets: []configstoreTables.TableRoutingTarget{
			{Provider: deepintshield.Ptr("openai"), Model: deepintshield.Ptr("gpt-4o"), Weight: 1.0},
		},
		Enabled:  true,
		Scope:    "global",
		Priority: 0,
	...
	vkRule := &configstoreTables.TableRoutingRule{
		...
		Enabled:  true,
		Scope:    "virtual_key",
		ScopeID:  deepintshield.Ptr("vk-123"),
		Priority: 10,
	}
...
func TestResolveRoutingWithFallback_NoMatch(t *testing.T) {
	...
	decision, err := resolveRoutingWithFallback(schemas.NewDeepIntShieldContext(context.Background(), time.Now()), ctx, engine)
	require.NoError(t, err)
	require.NotNil(t, decision)
...
func TestCompileAndCacheProgram_ValidExpression_Routing(t *testing.T) {
	...
	rule := &configstoreTables.TableRoutingRule{
		...
		CelExpression: "model == 'gpt-4o'",
		Targets: []configstoreTables.TableRoutingTarget{
			{Provider: deepintshield.Ptr("openai"), Weight: 1.0},
		},
		Enabled: true,
	}
