...
import (
	...
	"github.com/deepint-shield/ai-security/cli/internal/app"
	...
	if args := flag.Args(); len(args) > 0 {
		...
		case "update":
			if err := update.RunSelfUpdate(version); err != nil {
				fmt.Fprintf(os.Stderr, "deepintshield: %v\n", err)
				os.Exit(1)
			}
			return
		case "version":
			fmt.Printf("deepintshield %s (%s)\n", version, commit)
			return
		default:
			fmt.Fprintf(os.Stderr, "deepintshield: unknown command %q\n", args[0])
