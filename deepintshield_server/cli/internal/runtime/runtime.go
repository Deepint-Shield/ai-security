...
func BuildEnv(spec LaunchSpec) ([]string, error) {
	...
	if model != "" {
		env = append(env, "DEEPINTSHIELD_MODEL="+model, "DEEPINTSHIELD_MODEL="+model)
		if spec.Harness.ModelEnv != "" {
			env = append(env, spec.Harness.ModelEnv+"="+model)
		}
	}

	// Mark session as running inside deepintshield
	env = append(env,
		"DEEPINTSHIELD_SESSION=1",
		"DEEPINTSHIELD_SESSION=1",
		"DEEPINTSHIELD_BASE_URL="+spec.BaseURL,
		"DEEPINTSHIELD_BASE_URL="+spec.BaseURL,
	)
	return env, nil
}
...
// TTY access. It prints a deepintshield banner before launch and a summary after exit.
func RunInteractive(ctx context.Context, stdout, stderr io.Writer, spec LaunchSpec) error {
	...
	if err := runWithPTY(ctx, stdout, p.Cmd); err != nil {
		fmt.Fprintf(stdout, "\n\033[36mdeepintshield>\033[0m session ended with error: %v\n", err)
		return fmt.Errorf("run harness: %w", err)
	}
	fmt.Fprintf(stdout, "\n\033[36mdeepintshield>\033[0m session ended\n")
	return nil
}
...
func renderBanner(spec LaunchSpec) string {
	...
	b.WriteString(cyan + "deepintshield>" + reset + " " + bold + spec.Harness.Label + reset + "  " + dim + spec.Model + reset + "\n")
