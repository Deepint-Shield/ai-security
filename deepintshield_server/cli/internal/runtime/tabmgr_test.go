...
func TestBuildTabBarStringUsesCLIBrandAndRightSideVersion(t *testing.T) {
	...
	if !strings.Contains(got, "DeepIntShield CLI") {
		t.Fatalf("expected tab bar to contain DeepIntShield CLI branding, got %q", got)
	}
	if strings.Contains(got, " DeepIntShield ") {
		t.Fatalf("did not expect legacy DeepIntShield branding, got %q", got)
	}
