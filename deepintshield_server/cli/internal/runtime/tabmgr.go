...
const pendingTabLabel = "DeepIntShield"
...
// prefix is the deepintshield prefix key: Ctrl+B (0x02).
// When pressed, PTY output is frozen and deepintshield enters tab mode.
const prefix = 0x02
...
//	1…9             - jump to tab N
//	U               - update deepintshield (if available)
...
func tabBarBrandString(bg string) string {
	...
	b.WriteString(bg)
	b.WriteString(" ")
	b.WriteString("\x1b[1;36mDeepIntShield CLI ")
	return b.String()
}
...
func tabBarBrandWidth() int {
	return 1 + len("DeepIntShield CLI ")
}

func (tm *TabManager) initCursorTrace() {
	path := strings.TrimSpace(os.Getenv("DEEPINTSHIELD_CLAUDE_CURSOR_TRACE"))
	if path == "" {
		path = strings.TrimSpace(os.Getenv("DEEPINTSHIELD_CLAUDE_CURSOR_TRACE"))
	}
	if path == "" {
	...
	if err != nil {
		if tm.stderr != nil {
			fmt.Fprintf(tm.stderr, "deepintshield: failed to open Claude cursor trace %q: %v\n", path, err)
		}
		return
	}
	tm.cursorTrace = f
