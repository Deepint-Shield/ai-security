...
// Harness defines a coding assistant CLI that DeepIntShield can launch and manage.
type Harness struct {
	ID               string
	Label            string
	Binary           string
	...
	// WriteNativeConfig persists the deepintshield connection settings into the
	// harness CLI's own config file so the same configuration is available
	// when users launch the CLI directly outside deepintshield.
...
// opencodePreLaunch writes temporary OpenCode config files when DeepIntShield needs
...
func opencodePreLaunch(baseURL, apiKey, model string) ([]string, func(), error) {
  ...
  "provider": {
	...
	f, err := os.CreateTemp("", "deepintshield-opencode-*.json")
	if err != nil {
...
func opencodeModelRef(model string) string {
	return "deepintshield/" + strings.TrimSpace(model)
}
...
func opencodeTUIPreLaunch() ([]string, func(), error) {
	...
	f, err := os.CreateTemp("", "deepintshield-opencode-tui-*.json")
	if err != nil {
