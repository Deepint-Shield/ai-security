...
const issuesURL = "https://github.com/deepint-shield/ai-security/issues/new"
const repoURL = "https://github.com/deepint-shield/ai-security"
const docsURL = "https://deepintshield.com/quickstart/cli/getting-started"
	...
	case tea.KeyMsg:
		...
		case phaseVirtualKey:
			...
			if s == "f1" {
				...
				if baseURL != "" {
					openBrowser(baseURL)
					m.notify("opened deepintshield dashboard", false)
				}
				return m, nil
			}
			var cmd tea.Cmd
...
func (m chooserModel) View() string {
	...
	if updateVersion := strings.TrimSpace(m.cfg.UpdateVersion); updateVersion != "" {
		...
		body.WriteString(hint.Render("deepintshield " + updateVersion + "  "))
	...
	switch m.phase {
	case phaseBaseURL:
		title = accent.Render("Base URL (DeepIntShield Base URL)")
...
func renderPlainChooserView(title, body, footer string) string {
	...
	out.WriteString("DEEPINTSHIELD CLI\n\n")
	if title != "" {
...
// from the DeepIntShield API and sends the result as a modelsMsg.
