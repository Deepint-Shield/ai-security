...
func TestRenderPlainChooserView(t *testing.T) {
	...
	for _, want := range []string{"DEEPINTSHIELD CLI", "Ready", "base url", "model", "enter launch"} {
...
func TestChooserViewShowsUpdatePrompt(t *testing.T) {
	...
	for _, want := range []string{"Update available:", "deepintshield v1.2.3", "press y to update now"} {
