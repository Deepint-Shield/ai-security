...
func TestResolveManagedBinaryTargetFallsBackToCLIInstallLocation(t *testing.T) {
	...
	binaryName := "deepintshield"
	if runtime.GOOS == "windows" {
		binaryName = "deepintshield.exe"
	}

	got, err := deepintshieldCLIManagedBinaryPath(binaryName)
	if err != nil {
		t.Fatalf("deepintshieldCLIManagedBinaryPath() error = %v", err)
	}

	want := filepath.Join(home, ".deepintshield", "bin", binaryName)
	if got != want {
		t.Fatalf("deepintshieldCLIManagedBinaryPath() = %q, want %q", got, want)
	}
}
...
func TestIsWrapperManagedBinaryPathMatchesCLIInstallLocation(t *testing.T) {
	...
	path := filepath.Join(home, ".deepintshield", "bin", binaryName)
...
func TestIsWrapperManagedBinaryPathMatchesCacheInstallLocation(t *testing.T) {
	if runtime.GOOS == "windows" {
		...
		path := filepath.Join(localAppData, "deepintshield", "v1.2.3", "bin", "deepintshield.exe-0")
		if !isWrapperManagedBinaryPath(path, "deepintshield.exe") {
	...
	if runtime.GOOS == "linux" {
		...
		path := filepath.Join(xdg, "deepintshield", "v1.2.3", "bin", "deepintshield-0")
		if !isWrapperManagedBinaryPath(path, "deepintshield") {
			t.Fatalf("expected %q to be recognized as wrapper-managed cache path", path)
		}
		return
	}

	path := filepath.Join(home, "Library", "Caches", "deepintshield", "v1.2.3", "bin", "deepintshield-0")
	if !isWrapperManagedBinaryPath(path, "deepintshield") {
		t.Fatalf("expected %q to be recognized as wrapper-managed cache path", path)
	}
}
...
func TestStageUpdateBinaryCopiesIntoTargetDirectory(t *testing.T) {
	...
	root := t.TempDir()
	targetDir := filepath.Join(root, ".deepintshield", "bin")
	if err := os.MkdirAll(targetDir, 0o755); err != nil {
		t.Fatalf("mkdir target dir: %v", err)
	}

	downloadPath := filepath.Join(t.TempDir(), "downloaded-deepintshield")
	if err := os.WriteFile(downloadPath, []byte("new-binary"), 0o600); err != nil {
		t.Fatalf("write download: %v", err)
	}

	targetPath := filepath.Join(targetDir, "deepintshield")
