...
func RunSelfUpdate(currentVersion string) error {
	fmt.Println("Checking latest deepintshield version...")
	...
	fmt.Printf("Updating deepintshield from %s to %s...\n", currentVersion, latest)

	binaryName := "deepintshield"
	if runtime.GOOS == "windows" {
		binaryName = "deepintshield.exe"
	}
	downloadURL := fmt.Sprintf("%s/deepintshield-cli/%s/%s/%s/%s", baseURL, latest, runtime.GOOS, runtime.GOARCH, binaryName)
	...
	tmpFile, err := os.CreateTemp("", ".deepintshield-update-*")
	if err != nil {
	...
	fmt.Printf("Updated deepintshield from %s to %s. Please restart.\n", currentVersion, latest)
	return nil
}
...
func resolveManagedBinaryTarget(binaryName string) (string, error) {
	...
	return deepintshieldCLIManagedBinaryPath(binaryName)
}

func deepintshieldCLIManagedBinaryPath(binaryName string) (string, error) {
	home, err := os.UserHomeDir()
	if err != nil {
		return "", err
	}
	return filepath.Join(home, ".deepintshield", "bin", binaryName), nil
}
...
func isWrapperManagedBinaryPath(path, binaryName string) bool {
	...
	homePath, err := deepintshieldCLIManagedBinaryPath(binaryName)
	...
	cachePrefix := filepath.Clean(filepath.Join(cacheRoot, "deepintshield")) + string(os.PathSeparator)
...
func stageUpdateBinary(downloadPath, targetPath string, mode os.FileMode) (string, error) {
	stageFile, err := os.CreateTemp(filepath.Dir(targetPath), ".deepintshield-stage-*")
	if err != nil {
		return "", err
	}
