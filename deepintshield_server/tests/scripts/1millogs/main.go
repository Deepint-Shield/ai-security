...
var (
	...
	dbName       = flag.String("dbname", "deepintshield_logs", "PostgreSQL database name")
...
func main() {
	...
	fmt.Println("🚀 DeepIntShield Logs Population Script")
	fmt.Println("==================================")
...
func generateLog(index int, targetSize int) logstore.Log {
	...
	if status == "error" {
		errorDetails := schemas.DeepIntShieldError{
			...
			Type: deepintshield.Ptr("rate_limit_error"),
		}
...
func generateMCPLog(index int) logstore.MCPToolLog {
	...
	if status == "error" {
		errorDetails := schemas.DeepIntShieldError{
			...
			Type: deepintshield.Ptr("tool_execution_error"),
		}
		errorJSON, _ := json.Marshal(errorDetails)
		log.ErrorDetails = string(errorJSON)
	}
