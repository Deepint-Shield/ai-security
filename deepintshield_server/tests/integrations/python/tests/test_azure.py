...
This test suite uses the AzureOpenAI SDK to test Azure-routed requests through DeepIntShield.
...
- Model names are passed RAW (e.g., "gpt-4o") without format_provider_model() prefix
  because DeepIntShield's AzureEndpointPreHook automatically adds the "azure/" prefix
...
def get_provider_azure_client(provider="azure", vk_enabled=False):
    ...
    It places the model/deployment-id in the URL path, and DeepIntShield's
...
class TestAzureIntegration:
    """Test suite for Azure OpenAI SDK integration through DeepIntShield"""
    ...
    def test_41_file_upload(self, test_config, provider, model, vk_enabled):
        ...
        s3_prefix = integration_settings.get("output_s3_prefix", "deepintshield-batch-output")
