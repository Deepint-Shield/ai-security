...
Tests LangChain standard interface compliance and DeepIntShield integration:
...
def setup_langchain():
    """Setup LangChain with DeepIntShield configuration and dummy credentials"""
    # Set dummy credentials since DeepIntShield handles actual authentication
    os.environ["OPENAI_API_KEY"] = "dummy-openai-key-deepintshield-handles-auth"
    os.environ["ANTHROPIC_API_KEY"] = "dummy-anthropic-key-deepintshield-handles-auth"
    os.environ["GOOGLE_API_KEY"] = "dummy-google-api-key-deepintshield-handles-auth"
    os.environ["VERTEX_PROJECT"] = "dummy-vertex-project"
    os.environ["VERTEX_LOCATION"] = "us-central1"

    # Get DeepIntShield URL for LangChain
    base_url = get_integration_url("langchain")
    config = get_config()
    integration_settings = config.get_integration_settings("langchain")

    # Store original base URLs and set DeepIntShield URLs
    original_openai_base = os.environ.get("OPENAI_BASE_URL")
    original_anthropic_base = os.environ.get("ANTHROPIC_BASE_URL")

    if base_url:
        # Configure provider base URLs to route through DeepIntShield
...
class TestLangChainChatOpenAI(ChatModelIntegrationTests):
    """Standard LangChain tests for ChatOpenAI through DeepIntShield"""
...
class TestLangChainOpenAIEmbeddings(EmbeddingsIntegrationTests):
    """Standard LangChain tests for OpenAI Embeddings through DeepIntShield"""
...
class TestLangChainIntegration:
    """Comprehensive LangChain integration tests through DeepIntShield"""
    ...
    def test_14_gemini_chat_integration(self, test_config):
        ...
        try:
            # Use ChatGoogleGenerativeAI with DeepIntShield routing
            llm = ChatGoogleGenerativeAI(
                model="gemini-2.5-flash",
                google_api_key="dummy-google-api-key-deepintshield-handles-auth",
                temperature=0.7,
    ...
    def test_15_mistral_chat_integration(self, test_config):
        ...
        try:
            # Mistral is OpenAI-compatible, so it can route through DeepIntShield easily
            base_url = get_integration_url("langchain")
            if base_url:
                chat = ChatMistralAI(
                    model="mistral/mistral-small-2506",
                    mistral_api_key="dummy-mistral-api-key-deepintshield-handles-auth",
                    endpoint=f"{base_url}/v1",  # Route through DeepIntShield
                    temperature=0.7,
                    max_tokens=100,
                )
                ...
                pytest.skip("DeepIntShield URL not configured for LangChain integration")
    ...
    def test_19_structured_outputs(self, test_config, provider, model):
        ...
        try:
            # Create LangChain ChatOpenAI instance with DeepIntShield routing
            llm = ChatOpenAI(
                model=format_provider_model(provider, model),
                base_url=(
                    get_integration_url("langchain")
                ...
                api_key="dummy-key",  # Keys managed by DeepIntShield
            )
    ...
    def test_24_thinking_azure(self, test_config):
        ...
        try:
            default_headers = {}
            # Azure routing requires specific headers for DeepIntShield
