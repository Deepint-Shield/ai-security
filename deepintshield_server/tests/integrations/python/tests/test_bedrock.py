...
This test suite uses the AWS SDK (boto3) to test against multiple AI providers through DeepIntShield.
...
Tests core scenarios using AWS SDK (boto3) directly against DeepIntShield:
...
@pytest.fixture
def s3_client():
    """Create S3 client for file operations via DeepIntShield's S3-compatible API"""
    base_url = get_integration_url("bedrock")
    config = get_config()
    integration_settings = config.get_integration_settings("bedrock")
    region = integration_settings.get("region", "us-west-2")

    # Point boto3 S3 client to DeepIntShield's S3-compatible endpoint
    client = boto3.client(
        "s3",
        region_name=region,
        endpoint_url=f"{base_url}/files",
...
class TestBedrockIntegration:
    ...
    def test_11_file_upload(self, test_config, provider, model):
        ...
        # Upload to S3
        s3_key = f"deepintshield-test-files/batch_input_{int(time.time())}.jsonl"
    ...
    def test_12_file_list(self, test_config, provider, model):
        ...
        s3_key = f"deepintshield-test-files/test_list_{int(time.time())}.jsonl"
        ...
        try:
            # List files
            response = s3_client.list_objects_v2(Bucket=s3_bucket, Prefix="deepintshield-test-files/")
    ...
    def test_13_file_retrieve(self, test_config, provider, model):
        ...
        s3_key = f"deepintshield-test-files/test_retrieve_{int(time.time())}.jsonl"
    ...
    def test_14_file_delete(self, test_config, provider, model):
        ...
        s3_key = f"deepintshield-test-files/test_delete_{int(time.time())}.jsonl"
    ...
    def test_15_file_content(self, test_config, provider, model):
        ...
        s3_key = f"deepintshield-test-files/test_content_{int(time.time())}.jsonl"
    ...
    def test_16_batch_create(self, test_config, provider, model):
        ...
        s3_bucket = "deepintshield-batch-api-file-upload-testing"
        s3_key = f"deepintshield-batch-input/batch_input_{int(time.time())}.jsonl"
        upload_response = s3_client.put_object(
            Bucket=s3_bucket,
            Key=s3_key,
            Body=jsonl_content.encode(),
        ...
        try:
            ...
            response = bedrock_client.create_model_invocation_job(
                jobName=f"deepintshield-test-batch-{int(time.time())}",
                modelId=model,
                roleArn="",
                inputDataConfig={
                ...
                outputDataConfig={
                    "s3OutputDataConfig": {"s3Uri": f"s3://{s3_bucket}/deepintshield-batch-output/"}
                },
                tags=[
    ...
    def test_19_batch_cancel(self, test_config, provider, model):
        ...
        s3_key = f"deepintshield-batch-cancel/batch_cancel_{int(time.time())}.jsonl"
        upload_response = s3_client.put_object(
            Bucket=s3_bucket,
            Key=s3_key,
            Body=jsonl_content.encode(),
        ...
        try:
            ...
            create_response = bedrock_client.create_model_invocation_job(
                jobName=f"deepintshield-cancel-test-{int(time.time())}",
                modelId=model,
                roleArn=role_arn,
                inputDataConfig={
                    "s3InputDataConfig": {"s3Uri": input_uri, "s3InputFormat": "JSONL"}
                ...
                outputDataConfig={
                    "s3OutputDataConfig": {
                        "s3Uri": f"s3://{s3_bucket}/deepintshield-batch-cancel-output/"
                    }
                },
                tags=[
                    {"key": "endpoint", "value": "/v1/chat/completions"},
    ...
    def test_20_batch_e2e(self, test_config, provider, model):
        ...
        s3_key = f"deepintshield-batch-e2e/batch_e2e_{int(time.time())}.jsonl"
        upload_response = s3_client.put_object(
            Bucket=s3_bucket,
            Key=s3_key,
            Body=jsonl_content.encode(),
        ...
        try:
            ...
            create_response = bedrock_client.create_model_invocation_job(
                jobName=f"deepintshield-e2e-{int(time.time())}",
                modelId=model,
                roleArn=role_arn,
                inputDataConfig={
                    "s3InputDataConfig": {"s3Uri": input_uri, "s3InputFormat": "JSONL"}
                ...
                outputDataConfig={
                    "s3OutputDataConfig": {"s3Uri": f"s3://{s3_bucket}/deepintshield-batch-e2e-output/"}
                },
                tags=[
                    {"key": "endpoint", "value": "/v1/chat/completions"},
                    {"key": "file_id", "value": file_id},
