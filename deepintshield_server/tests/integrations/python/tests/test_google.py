...
This test suite uses the Google GenAI SDK to test against multiple AI providers through DeepIntShield.
...
class TestGoogleIntegration:
    ...
    def test_pdf_file_input(self, test_config):
        ...
        # Create direct Google client (without DeepIntShield) for file upload
        direct_client = genai.Client(
            api_key=os.getenv("GEMINI_API_KEY")
        )
        
        # Get DeepIntShield client for generate_content
        deepintshield_client = get_provider_google_client("gemini")
        ...
        try:
            # Upload the PDF file using direct Google client (not through DeepIntShield)
            uploaded_file = direct_client.files.upload(
                file=temp_pdf_path,
                config=types.UploadFileConfig(display_name='test_pdf_gemini')
            )
            
            # Use the uploaded file in a generate_content request through DeepIntShield
            response = deepintshield_client.models.generate_content(
...
Joe: Pretty good, thanks for asking."""
    ...
    def test_41a_image_generation_simple(self, test_config, provider, model):
        """Test Case 41a: Simple image generation with Gemini model via DeepIntShield"""
        if provider == "_no_providers_" or model == "_no_model_":
    ...
    def test_41b_imagen_predict(self, test_config, provider, model):
        """Test Case 41b: Image generation using Imagen model via DeepIntShield"""
        if provider == "_no_providers_" or model == "_no_model_":
        ...
        # DeepIntShield will automatically route to the :predict endpoint for Imagen models
        # Use provider/model format so DeepIntShield can parse the provider and route correctly
        try:
            response = client.models.generate_content(
                model=format_provider_model(provider, model),
                contents=IMAGE_GENERATION_SIMPLE_PROMPT,
    ...
    def test_42a_image_edit_simple(self, test_config, provider, model):
        """Test Case 42a: Simple image edit with Gemini/Imagen via DeepIntShield"""
        if provider == "_no_providers_" or model == "_no_model_":
    ...
    def test_42b_imagen_edit(self, test_config, provider, model):
        """Test Case 42b: Image editing using Imagen model via DeepIntShield"""
        if provider == "_no_providers_" or model == "_no_model_":
        ...
        # For Imagen edit models, DeepIntShield routes to :editImage endpoint
        try:
            response = client.models.generate_content(
                model=format_provider_model(provider, model),
                contents=[
    ...
    # =========================================================================
    # CONTEXT CACHING TEST CASES (Gemini Caches API - pass-through via DeepIntShield)
    # =========================================================================
