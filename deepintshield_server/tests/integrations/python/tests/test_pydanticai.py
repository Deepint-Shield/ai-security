...
This test suite uses Pydantic AI to test against multiple AI providers through DeepIntShield.
...
Tests Pydantic AI standard interface compliance and DeepIntShield integration:
...
def setup_pydanticai():
    """Setup Pydantic AI with DeepIntShield configuration and dummy credentials"""
    ...
    os.environ["GEMINI_API_KEY"] = "dummy-gemini-api-key-deepintshield-handles-auth"
    os.environ["CO_API_KEY"] = "dummy-cohere-key-deepintshield-handles-auth"
...
def get_openai_model(model_name: str | None = None) -> OpenAIChatModel:
    """Create an OpenAI model configured for DeepIntShield"""
    ...
    provider = OpenAIProvider(
        ...
        api_key="dummy-openai-key-deepintshield-handles-auth"
    )
    return OpenAIChatModel(model_name, provider=provider)


def get_anthropic_model(model_name: str = "claude-3-haiku-20240307") -> Optional[Any]:
    """Create an Anthropic model configured for DeepIntShield"""
    ...
    provider = AnthropicProvider(
        base_url=base_url,
        api_key="dummy-anthropic-key-deepintshield-handles-auth"
    )
    return AnthropicModel(model_name, provider=provider)


def get_google_model(model_name: str = "gemini-2.0-flash") -> Optional[Any]:
    """Create a Google model configured for DeepIntShield"""
    if not GOOGLE_AVAILABLE:
        return None

    base_url = get_integration_url("pydanticai")

    # Configure GoogleProvider with DeepIntShield endpoint
    provider = GoogleProvider(
        api_key="dummy-google-api-key-deepintshield-handles-auth",
        base_url=base_url
    )
...
def get_cohere_model(model_name: str = "command-r7b-12-2024") -> Optional[Any]:
    """Create a Cohere model configured for DeepIntShield"""
    ...
    # We create a custom client pointing to DeepIntShield and pass it to CohereProvider
    cohere_client = CohereAsyncClient(
        api_key="dummy-cohere-key-deepintshield-handles-auth",
        base_url=base_url
    )
...
class TestPydanticAIIntegration:
    """Comprehensive Pydantic AI integration tests through DeepIntShield"""
