...
# GenAI-specific invalid role content that passes SDK validation but fails at DeepIntShield
GENAI_INVALID_ROLE_CONTENT = [
    {
        "role": "tester",  # Invalid role that should be caught by DeepIntShield
...
def assert_error_propagation(error_response: Any, integration: str):
    """
    Assert that error is properly propagated through DeepIntShield to the integration.
...
def get_bedrock_s3_config() -> Dict[str, Optional[str]]:
    ...
    return {
        ...
        "output_s3_prefix": os.environ.get("AWS_OUTPUT_S3_PREFIX", "deepintshield-batch-output/"),
...
def create_bedrock_batch_s3_uri(
    bucket: str, prefix: str = "deepintshield-batch-input/", filename: str | None = None
) -> str:
    """
...
def get_ws_base_url():
    ...
    config = get_config()
    base_url = config._config["deepintshield"]["base_url"]
