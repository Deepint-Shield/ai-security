"""
Configuration loader for DeepIntShield integration tests.

This module loads configuration from config.yml and provides utilities
for constructing integration URLs through the DeepIntShield gateway.
"""
...
@dataclass
class DeepIntShieldConfig:
    """DeepIntShield gateway configuration"""
...
class TestConfig:
    ...
    deepintshield: DeepIntShieldConfig
    api: Dict[str, Any]
...
class ConfigLoader:
    ...
    def get_integration_url(self, integration: str) -> str:
        ...
        """
        deepintshield_config = self._config["deepintshield"]
        base_url = deepintshield_config["base_url"]
        endpoint = deepintshield_config["endpoints"].get(integration, "")
    ...
    def get_deepintshield_config(self) -> DeepIntShieldConfig:
        """Get DeepIntShield configuration"""
        deepintshield_data = self._config["deepintshield"]
        return DeepIntShieldConfig(
            base_url=deepintshield_data["base_url"], endpoints=deepintshield_data["endpoints"]
        )
    ...
    def validate_config(self) -> bool:
        ...
        required_sections = ["deepintshield", "providers", "api", "test_settings"]

        for section in required_sections:
            if section not in self._config:
                raise ValueError(f"Missing required configuration section: {section}")

        # Validate DeepIntShield configuration
        deepintshield = self._config["deepintshield"]
        if "base_url" not in deepintshield or "endpoints" not in deepintshield:
            raise ValueError("DeepIntShield configuration missing base_url or endpoints")
    ...
    def print_config_summary(self):
        ...
        print("🔧 DEEPINTSHIELD INTEGRATION TEST CONFIGURATION")
        print("=" * 80)

        # DeepIntShield configuration
        deepintshield = self.get_deepintshield_config()
        print("\n🌉 DEEPINTSHIELD GATEWAY:")
        print(f"  Base URL: {deepintshield.base_url}")
        print("  Endpoints:")
        for integration, endpoint in deepintshield.endpoints.items():
            full_url = f"{deepintshield.base_url.rstrip('/')}/{endpoint}"
