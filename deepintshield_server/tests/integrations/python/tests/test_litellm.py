...
# that DeepIntShield's LiteLLM integration doesn't support
...
def setup_litellm(monkeypatch):
    """Setup LiteLLM with DeepIntShield configuration and dummy credentials"""
    import os
    ...
    os.environ["MISTRAL_API_KEY"] = "dummy-mistral-key-deepintshield-handles-auth"
    ...
    # LiteLLM will load these dummy credentials but all actual requests go through DeepIntShield
    ...
    # Since DeepIntShield handles auth, we don't need LiteLLM to authenticate
    def mock_refresh(self, request):
        """Mock refresh that sets a dummy token - DeepIntShield handles real auth"""
        import datetime

        self.token = "dummy-access-token-deepintshield-handles-auth"
    ...
    # Get DeepIntShield URL for LiteLLM
