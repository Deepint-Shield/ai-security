# DeepIntShield Integration Tests

Production-ready end-to-end test suite for testing AI integrations through DeepIntShield proxy. This test suite provides uniform testing across multiple AI integrations with comprehensive coverage of chat, tool calling, image processing, embeddings, speech synthesis, and multimodal workflows.
...
cd deepintshield/tests/integrations
uv sync
...
The DeepIntShield integration tests use a centralized configuration system that routes all AI integration requests through DeepIntShield as a gateway/proxy:

```text
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│   Test Client   │───▶│  DeepIntShield Gateway │───▶│  AI Integration    │
...
- **Base URL**: `http://localhost:8080` (configurable via `DEEPINTSHIELD_BASE_URL`)
...
- **🌉 DeepIntShield Gateway Integration**: All integrations route through DeepIntShield proxy
...
```bash
# Required: DeepIntShield gateway
export DEEPINTSHIELD_BASE_URL="http://localhost:8080"
...
- DeepIntShield gateway settings and endpoints
...
```yaml
deepintshield:
  base_url: "${DEEPINTSHIELD_BASE_URL:-http://localhost:8080}"
  endpoints:
    openai: "openai"
    anthropic: "anthropic"
    google: "genai"
...
# Get DeepIntShield URL for OpenAI
openai_url = get_integration_url("openai")
# Returns: http://localhost:8080/openai

# Get integration URL through DeepIntShield
openai_url = get_integration_url("openai")
# Returns: http://localhost:8080/openai
```
...
3. **DeepIntShield Configuration**
   ```bash
   # Ensure DeepIntShield is running and accessible
...
# Check DeepIntShield logs for audio endpoint requests
# (Check your DeepIntShield instance logs)
```
...
- ✅ **Full DeepIntShield Integration**: Complete base URL support
...
- ✅ **Full DeepIntShield Integration**: Complete custom transport implementation
...
- ✅ **Full DeepIntShield Integration**: Global base URL configuration
...
```bash
# DeepIntShield gateway (required)
export DEEPINTSHIELD_BASE_URL="http://localhost:8080"
...
```yaml
# Add to deepintshield endpoints
deepintshield:
  endpoints:
...
- [ ] **DeepIntShield Integration**: Base URL properly configured and tested
...
```bash
# Error: Connection refused to DeepIntShield
ConnectionError: Connection refused to localhost:8080

# Solutions:
# 1. Check if DeepIntShield is running
curl http://localhost:8080/health

# 2. Ensure DEEPINTSHIELD_BASE_URL is set correctly
echo $DEEPINTSHIELD_BASE_URL
```
...
**Note**: This test suite is designed for testing AI integrations through DeepIntShield proxy. Ensure your DeepIntShield instance is properly configured and running before executing tests. The configuration system provides DeepIntShield routing for maximum flexibility.
