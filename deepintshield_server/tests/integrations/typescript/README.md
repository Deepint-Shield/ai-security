# DeepIntShield TypeScript Integration Tests

TypeScript/JavaScript integration test suite for testing AI providers through DeepIntShield proxy. This test suite uses Vitest and provides comprehensive coverage across multiple AI SDKs.

## Quick Start

```bash
# 1. Install dependencies
cd deepintshield/tests/integrations/typescript
npm install
...
The TypeScript integration tests use the same centralized configuration as the Python tests, routing all AI requests through DeepIntShield:

```text
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│   Test Client   │───▶│  DeepIntShield Gateway │───▶│  AI Provider    │
...
The OpenAI test file supports cross-provider testing through DeepIntShield's model name routing. By formatting the model name as `provider/model`, DeepIntShield routes the request to the appropriate provider:
...
This allows testing any provider using the OpenAI SDK format while DeepIntShield handles the routing based on the model name prefix.
...
```
Solution: Ensure DeepIntShield is running on the expected port.
...
```
Solution: Check network connectivity and DeepIntShield logs.
...
# Check DeepIntShield logs
tail -f /tmp/deepintshield-test.log
```
