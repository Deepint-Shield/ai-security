...
class ConfigLoader {
  ...
  getIntegrationUrl(integration: string): string {
    ...
    const deepintshieldConfig = this.config.deepintshield
    const baseUrl = deepintshieldConfig.base_url
    const endpoint = deepintshieldConfig.endpoints[integration]
  ...
  getDeepIntShieldConfig(): DeepIntShieldConfig {
    if (!this.config) throw new Error('Config not loaded')
    return this.config.deepintshield
  }
  ...
  validateConfig(): boolean {
    ...
    const requiredSections = ['deepintshield', 'providers', 'api', 'test_settings']
    ...
    const deepintshield = this.config.deepintshield
    if (!deepintshield.base_url || !deepintshield.endpoints) {
      throw new Error('DeepIntShield configuration missing base_url or endpoints')
    }
  ...
  printConfigSummary(): void {
    ...
    console.log('🔧 DEEPINTSHIELD INTEGRATION TEST CONFIGURATION (TypeScript)')
    console.log('='.repeat(80))

    // DeepIntShield configuration
    const deepintshield = this.getDeepIntShieldConfig()
    console.log('\n🌉 DEEPINTSHIELD GATEWAY:')
    console.log(`  Base URL: ${deepintshield.base_url}`)
    console.log('  Endpoints:')
    for (const [integration, endpoint] of Object.entries(deepintshield.endpoints)) {
      const fullUrl = `${deepintshield.base_url.replace(/\/$/, '')}/${endpoint}`
