...
test.describe('Placeholder and Enterprise Pages', () => {
  ...
  test('should load alert-channels page', async ({ page }) => {
    ...
    await expect(popup).toHaveURL(/^https:\/\/docs\.getdeepintshield\.ai\/enterprise\/alert-channels(\?|$)/)
    await popup.close()
  })
  ...
  test('should load adaptive-routing page', async ({ page }) => {
    ...
    await expect(readMore).toBeVisible()
    const [popup] = await Promise.all([page.waitForEvent('popup'), readMore.click()])
    await expect(popup).toHaveURL(/^https:\/\/docs\.getdeepintshield\.ai\/enterprise\/adaptive-load-balancing(\?|$)/)
    await popup.close()
  })
