 ...
 *   2. DEEPINTSHIELD_DB_URL env var       (explicit DSN)
 *   3. --reporter-dbverify-config   (path to DeepIntShield config.json; auto-detects type + DSN)
 ...
 *   2. DEEPINTSHIELD_LOGS_DB_URL env var
...
// ─── DeepIntShield config.json reader ───────────────────────────────────────────────

/**
 * Resolve an EnvVar field from DeepIntShield config.
...
function postgresUrlFromConfig(c) {
  ...
  const user     = resolveEnvVar(c.user)     || 'deepintshield';
  const password = resolveEnvVar(c.password) || '';
  const dbName   = resolveEnvVar(c.db_name)  || 'deepintshield';
...
/**
 * Read a DeepIntShield config.json and return a DB connection URL for the main
 * config_store, or null if not enabled / unreadable.
 */
function dbUrlFromDeepIntShieldConfig(configPath) {
  let raw;
...
/**
 * Read a DeepIntShield config.json and return a DB connection URL for the logs_store,
 * or null if not enabled / unreadable.
 */
function logsDbUrlFromDeepIntShieldConfig(configPath) {
  let raw;
  try { raw = fs.readFileSync(configPath, 'utf8'); }
  catch (_) { return null; }
...
module.exports = function (newman, options) {
  ...
  const configPath = (options && options['config'])
    || process.env.DEEPINTSHIELD_CONFIG_PATH
    || path.resolve(process.cwd(), 'config.json');

  // Main DB (config_store)
  let dbUrl = (options && options['db-url']) || process.env.DEEPINTSHIELD_DB_URL || null;
  if (!dbUrl) {
    dbUrl = dbUrlFromDeepIntShieldConfig(configPath);
    if (dbUrl && !silent) console.log(`[dbverify] Auto-detected main DB from config: ${configPath}`);
  }
  if (!dbUrl) {
    console.warn('[dbverify] No main DB URL found. Provide --reporter-dbverify-db-url, DEEPINTSHIELD_DB_URL, or --reporter-dbverify-config. Skipping DB checks.');
  }

  // Logs DB (logs_store)
  let logsDbUrl = (options && options['logs-db-url']) || process.env.DEEPINTSHIELD_LOGS_DB_URL || null;
  if (!logsDbUrl) {
    logsDbUrl = logsDbUrlFromDeepIntShieldConfig(configPath);
