...
# DeepIntShield API Management & Health Tests
...
# Configuration
COLLECTION="$SCRIPT_DIR/deepintshield-api-management.postman_collection.json"
...
# Print banner
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}DeepIntShield API Management & Health Tests${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""
...
BAIL=""
DB_VERIFY=""
DB_URL="${DEEPINTSHIELD_DB_URL:-}"
LOGS_DB_URL="${DEEPINTSHIELD_LOGS_DB_URL:-}"
DB_CONFIG_PATH=""
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "  --logs-db-url <dsn> Explicit logs DB url (also reads DEEPINTSHIELD_LOGS_DB_URL; auto-detected)"
            echo "                      PostgreSQL: postgresql://user:pass@host:port/db"
            echo "                      SQLite:     sqlite:///path/to/file.db"
            echo "  --config-path <p>   Path to DeepIntShield config.json for auto DB detection"
            echo "                      (default: ./config.json; also reads DEEPINTSHIELD_CONFIG_PATH env)"
...
DEEPINTSHIELD_ROOT="$(cd "$SCRIPT_DIR/../../.." && pwd)"
PLUGIN_DIR="$DEEPINTSHIELD_ROOT/examples/plugins/hello-world"
...
HTTP_SERVER_DIR="$DEEPINTSHIELD_ROOT/examples/mcps/http-no-ping-server"
