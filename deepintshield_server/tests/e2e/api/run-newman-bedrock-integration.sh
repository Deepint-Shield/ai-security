...
# DeepIntShield Bedrock Integration API Newman Test Runner
# This script runs the Bedrock integration API test suite using Newman

set -e

# Configuration
COLLECTION="deepintshield-bedrock-integration.postman_collection.json"
ENVIRONMENT="deepintshield-v1.postman_environment.json"
...
if [ "$ci_normalized" = "1" ] || [ "$ci_normalized" = "true" ]; then
    echo -e "${GREEN}DeepIntShield Bedrock Integration API Test Runner with retries: 10${NC}"
else
    echo -e "${GREEN}DeepIntShield Bedrock Integration API Test Runner${NC}"
fi
echo -e "${GREEN}========================================${NC}"
echo ""
...
if [ -z "$PROVIDER_ENV_FILE" ] && [ -d "$PROVIDER_CONFIG_DIR" ]; then
    ...
    while IFS= read -r -d '' f; do
        # basename: deepintshield-v1-openai.postman_environment.json -> openai
        name="${f##*/}"
        name="${name#deepintshield-v1-}"
    ...
    done < <(find "$PROVIDER_CONFIG_DIR" -maxdepth 1 -name "deepintshield-v1-*.postman_environment.json" -print0 2>/dev/null | sort -z)
fi
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "  --env <path>        Postman env .json path or provider name (e.g. provider_config/deepintshield-v1-openai.postman_environment.json or openai)"
            echo "  --help              Show this help message"
            echo ""
            echo "Environment Variables:"
            echo "  DEEPINTSHIELD_BASE_URL    Override base URL (default: http://localhost:8080)"
            echo "  DEEPINTSHIELD_PROVIDER    Override provider (default: openai)"
            echo "  DEEPINTSHIELD_MODEL       Override model name (default: gpt-4o)"
            echo "  DEEPINTSHIELD_MODEL_INVOKE Override model for Bedrock invoke/invoke-stream (default: amazon.titan-text-express-v1)"
            echo "  DEEPINTSHIELD_EMBEDDING_MODEL    Override embedding model (default: text-embedding-3-small)"
            echo "  DEEPINTSHIELD_SPEECH_MODEL       Override speech model (default: tts-1)"
            echo "  DEEPINTSHIELD_TRANSCRIPTION_MODEL  Override transcription model (default: whisper-1)"
            echo "  DEEPINTSHIELD_IMAGE_MODEL        Override image model (default: dall-e-3)"
            echo "  DEEPINTSHIELD_BEDROCK_API_KEY    Bedrock API key (when --env bedrock)"
            echo "  DEEPINTSHIELD_BEDROCK_ACCESS_KEY Bedrock AWS access key (when --env bedrock)"
            echo "  DEEPINTSHIELD_BEDROCK_SECRET_KEY Bedrock AWS secret key (when --env bedrock)"
            echo "  DEEPINTSHIELD_BEDROCK_REGION     Bedrock region (default: us-east-1)"
            echo ""
            echo "Examples:"
            echo "  $0                                    # Run collection for all providers (each provider_config/deepintshield-v1-*.postman_environment.json)"
            ...
            echo "  DEEPINTSHIELD_BASE_URL=http://api:8080 $0  # Custom base URL"
            exit 0
            ;;
        *)
...
# Optional second arg: path to Postman env .json file (e.g. provider_config/deepintshield-v1-openai.postman_environment.json).
# When given, uses only that env file; otherwise uses default env and DEEPINTSHIELD_* overrides.
run_newman() {
    ...
    if [ -n "${2:-}" ] && [ -f "${2}" ]; then
        ...
        if [[ "${1:-}" == "bedrock" ]]; then
            [ -n "${DEEPINTSHIELD_BEDROCK_API_KEY:-}" ] && cmd+=(--env-var "bedrock_api_key=$DEEPINTSHIELD_BEDROCK_API_KEY")
            [ -n "${DEEPINTSHIELD_BEDROCK_ACCESS_KEY:-}" ] && cmd+=(--env-var "bedrock_access_key=$DEEPINTSHIELD_BEDROCK_ACCESS_KEY")
            [ -n "${DEEPINTSHIELD_BEDROCK_SECRET_KEY:-}" ] && cmd+=(--env-var "bedrock_secret_key=$DEEPINTSHIELD_BEDROCK_SECRET_KEY")
            [ -n "${DEEPINTSHIELD_BEDROCK_REGION:-}" ] && cmd+=(--env-var "bedrock_region=$DEEPINTSHIELD_BEDROCK_REGION")
            [ -n "${DEEPINTSHIELD_BEDROCK_SESSION_TOKEN:-}" ] && cmd+=(--env-var "bedrock_session_token=$DEEPINTSHIELD_BEDROCK_SESSION_TOKEN")
        fi
    else
        local base_url="${DEEPINTSHIELD_BASE_URL:-http://localhost:8080}"
        local provider="${DEEPINTSHIELD_PROVIDER:-openai}"
        local model="${DEEPINTSHIELD_MODEL:-gpt-4o}"
        local model_invoke="${DEEPINTSHIELD_MODEL_INVOKE:-amazon.titan-text-express-v1}"
        local embedding_model="${DEEPINTSHIELD_EMBEDDING_MODEL:-text-embedding-3-small}"
        local speech_model="${DEEPINTSHIELD_SPEECH_MODEL:-tts-1}"
        local transcription_model="${DEEPINTSHIELD_TRANSCRIPTION_MODEL:-whisper-1}"
        local image_model="${DEEPINTSHIELD_IMAGE_MODEL:-dall-e-3}"
...
if [ -n "$PROVIDER_ENV_FILE" ]; then
    ...
    elif [ -f "$PROVIDER_CONFIG_DIR/deepintshield-v1-${PROVIDER_ENV_FILE}.postman_environment.json" ]; then
        SINGLE_JSON_ENV="$PROVIDER_CONFIG_DIR/deepintshield-v1-${PROVIDER_ENV_FILE}.postman_environment.json"
    fi
    if [ -z "$SINGLE_JSON_ENV" ]; then
        echo -e "${RED}Error: Env file not found: $PROVIDER_ENV_FILE${NC}"
        echo "Use a path to a .json env (e.g. provider_config/deepintshield-v1-openai.postman_environment.json) or provider name (e.g. openai)"
        exit 1
    fi
    SINGLE_PROVIDER_NAME="${SINGLE_JSON_ENV##*/}"
    SINGLE_PROVIDER_NAME="${SINGLE_PROVIDER_NAME#deepintshield-v1-}"
...
if [ -z "${PROVIDER_JSON_FILES+x}" ] || [ ${#PROVIDER_JSON_FILES[@]} -eq 0 ]; then
    ...
    echo -e "  Base URL: ${YELLOW}${DEEPINTSHIELD_BASE_URL:-http://localhost:8080}${NC}"
    echo -e "  Provider: ${YELLOW}${DEEPINTSHIELD_PROVIDER:-openai}${NC}"
    echo -e "  Reports:  ${YELLOW}$REPORT_DIR${NC}"
    echo ""
    echo -e "${GREEN}Running tests...${NC}"
    echo ""
