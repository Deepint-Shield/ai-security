...
# DeepIntShield Anthropic Integration API Newman Test Runner
# This script runs the Anthropic integration API test suite using Newman

set -e

# Configuration
COLLECTION="deepintshield-anthropic-integration.postman_collection.json"
...
if [ "$ci_normalized" = "1" ] || [ "$ci_normalized" = "true" ]; then
    echo -e "${GREEN}DeepIntShield Anthropic Integration API Test Runner with retries: 10${NC}"
else
    echo -e "${GREEN}DeepIntShield Anthropic Integration API Test Runner${NC}"
fi
echo -e "${GREEN}========================================${NC}"
echo ""
