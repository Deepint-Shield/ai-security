...
# DeepIntShield V1 API Newman Test Runner
# This script runs the complete DeepIntShield V1 API test suite using Newman
...
# Configuration
COLLECTION="deepintshield-v1-complete.postman_collection.json"
...
if [ "$ci_normalized" = "1" ] || [ "$ci_normalized" = "true" ]; then
    echo -e "${GREEN}DeepIntShield V1 API Test Runner with retries: 10${NC}"
else
    echo -e "${GREEN}DeepIntShield V1 API Test Runner${NC}"
fi
echo -e "${GREEN}==============================================${NC}"
echo ""
...
jq -n --rawfile cap "$PROVIDER_CAPABILITIES_JSON" '{id: "deepintshield-provider-capabilities", name: "Provider capabilities", values: [{key: "provider_capabilities", value: $cap, type: "default", enabled: true}]}' > "$GLOBALS_TMP"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "  DEEPINTSHIELD_CHAT_MODEL  Override chat completions model (default: DEEPINTSHIELD_MODEL)"
            echo "  DEEPINTSHIELD_TEXT_COMPLETION_MODEL  Override text completions model (default: DEEPINTSHIELD_MODEL)"
            echo "  DEEPINTSHIELD_RESPONSES_MODEL  Override Responses API model (default: DEEPINTSHIELD_MODEL)"
...
run_newman() {
    ...
    if [ -n "${2:-}" ] && [ -f "${2}" ]; then
        ...
        local chat_model="${DEEPINTSHIELD_CHAT_MODEL:-$model}"
        local text_completion_model="${DEEPINTSHIELD_TEXT_COMPLETION_MODEL:-$model}"
        local responses_model="${DEEPINTSHIELD_RESPONSES_MODEL:-$model}"
