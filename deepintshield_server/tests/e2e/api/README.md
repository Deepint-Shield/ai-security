...
End-to-end API tests for the DeepIntShield API using Postman collections and [Newman](https://www.npmjs.com/package/newman) (CLI).
...
| Path | Description |
|------|-------------|
| `deepintshield-v1-complete.postman_collection.json` | Postman collection: all `/v1` endpoints (models, chat, completions, responses, embeddings, audio, images, count tokens, batches, files, containers, MCP) |
| `deepintshield-v1.postman_environment.json` | Optional/legacy Postman environment (OpenAI). `run-newman-tests.sh` uses **DEEPINTSHIELD_*** environment variables as the fallback when no provider-specific env file is passed (see script and `--help`). |
...
| Path | Description |
|------|-------------|
| `deepintshield-openai-integration.postman_collection.json` | OpenAI integration endpoints: `/openai/v1/*`, `/openai/*`, `/openai/deployments/*` (38 requests) |
| `deepintshield-anthropic-integration.postman_collection.json` | Anthropic integration endpoints: `/anthropic/v1/*` (13 requests) |
| `deepintshield-bedrock-integration.postman_collection.json` | Bedrock integration endpoints: `/bedrock/model/*`, `/bedrock/files/*`, `/bedrock/model-invocation-*` (13 requests, including List Objects S3 ListObjectsV2). Auth via request headers set from collection/environment variables. |
| `deepintshield-composite-integrations.postman_collection.json` | Composite integrations: GenAI, Cohere, LiteLLM, LangChain, PydanticAI, Health (21 requests) |
...
| Path | Description |
|------|-------------|
| `provider_config/` | Per-provider Postman env `.json` files (`deepintshield-v1-openai.postman_environment.json`, etc.). Reused across all collections. |
...
- DeepIntShield server running (e.g. `http://localhost:8080`) with at least one provider configured (API keys, etc.)
...
```bash
# Run for all providers in parallel (each provider_config/deepintshield-v1-*.postman_environment.json except sgl and ollama)
./run-newman-tests.sh
...
- **Batch API config (500)** – **"no batch-enabled keys found"** / **"no config found for batch APIs"** when batch endpoints are not configured for that provider.
  ...
  **To fix:** In DeepIntShield's config (or provider config), enable "Use for Batch APIs" on at least one API key for the provider, e.g. in config JSON:
  ```json
  {
    "providers": [
      {
...
- **Bedrock** – Model (converse/invoke) or S3 file operations may fail with 403/500 if AWS keys or S3 are not configured. The Bedrock **integration** collection (`deepintshield-bedrock-integration.postman_collection.json`) tests `/bedrock/*` and supports auth via **request headers** (set from collection or environment variables by the collection’s pre-request script). Credentials can be provided via env vars (e.g. `DEEPINTSHIELD_BEDROCK_API_KEY`, `DEEPINTSHIELD_BEDROCK_ACCESS_KEY`, `DEEPINTSHIELD_BEDROCK_SECRET_KEY`, `DEEPINTSHIELD_BEDROCK_REGION`) when using the runner with `--env bedrock`; the runner passes these into Postman variables, which the pre-request script forwards as `x-bf-bedrock-*` headers. Set authentication in `provider_config/deepintshield-v1-bedrock.postman_environment.json` or collection variables:
...
3. **Re-inject variables into a collection**: From this directory run:
   ```bash
   node scripts/update-collection-capabilities.js deepintshield-openai-integration.postman_collection.json --inject
   ```
...
Request bodies match core (e.g. batch inline with `custom_id`/`body`/`Say hello`, container create with `name: "deepintshield-test-container"`).

## Syncing from OpenAPI docs

The collection and supporting files are maintained under `docs/openapi/`. To refresh this e2e copy:

- `deepintshield-v1-complete.postman_collection.json` ← `docs/openapi/deepintshield-v1-complete.postman_collection.json`
- `deepintshield-v1.postman_environment.json` ← `docs/openapi/deepintshield-v1.postman_environment.json`
