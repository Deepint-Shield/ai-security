...
const DEEPINTSHIELD_BASE_URL = process.env.DEEPINTSHIELD_BASE_URL ?? 'http://localhost:8080'

const REPO_ROOT = resolve(__dirname, '../..')
const TEST_PLUGIN_PATH = join(REPO_ROOT, 'tmp', 'deepintshield-test-plugin.so')
...
async function waitForDeepIntShieldAPI(baseUrl: string, maxAttempts = 30): Promise<void> {
  for (let i = 0; i < maxAttempts; i++) {
    try {
  ...
  throw new Error(`DeepIntShield API at ${baseUrl} did not become ready after ${maxAttempts} attempts`)
}
...
async function ensureTestClient001AndSendResponses(baseUrl: string): Promise<void> {
  ...
  if (clientAfter.state !== 'connected') {
    ...
    if (reconnectRes.statusCode < 200 || reconnectRes.statusCode >= 300) {
      throw new Error(
        `POST /api/mcp/client/.../reconnect failed: ${reconnectRes.statusCode} ${reconnectRes.body}. Ensure MCP server is running and reload DeepIntShield if needed.`
      )
    }
  }
...
/**
 * Seed LLM logs by sending a few chat completion requests through DeepIntShield.
...
async function runDeepIntShieldMCPAndResponsesSetup(): Promise<void> {
  if (!process.env.DEEPINTSHIELD_BASE_URL) {
    console.log('Skipping DeepIntShield MCP client and /v1/responses (DEEPINTSHIELD_BASE_URL not set)')
    return
  }
  console.log(`Waiting for DeepIntShield API at ${DEEPINTSHIELD_BASE_URL}...`)
  await waitForDeepIntShieldAPI(DEEPINTSHIELD_BASE_URL)
  console.log(`✓ DeepIntShield API ready`)
  await ensureTestClient001AndSendResponses(DEEPINTSHIELD_BASE_URL)
  await seedLLMLogs(DEEPINTSHIELD_BASE_URL, 30)
}
...
async function globalSetup(): Promise<() => Promise<void>> {
  ...
  try {
    await runDeepIntShieldMCPAndResponsesSetup()
  } catch (error: unknown) {
    const err = error as Error
    console.error(`\n❌ DeepIntShield MCP client / v1/responses setup failed: ${err?.message || String(error)}`)
    console.error(`   Ensure DeepIntShield is running at ${DEEPINTSHIELD_BASE_URL} and OPENAI_API_KEY is set for /v1/responses.`)
    runMCPTeardown()
    throw error
  }
  return async () => {
