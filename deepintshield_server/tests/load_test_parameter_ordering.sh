#!/bin/bash
# Load test: detect JSON key ordering mutations in DeepIntShield's request proxying.
# Sends randomized payloads with different schema shapes and compares the input
# key order against what DeepIntShield actually sent to the provider (via
...
#
# Prerequisites:
#   - DeepIntShield running with send_back_raw_request: true on the openai provider
...
DEEPINTSHIELD_URL="http://localhost:8080/litellm/v1/chat/completions"
RPS="${1:-20}"
DURATION="${2:-10}"
...
export DEEPINTSHIELD_URL TMPDIR NUM_PAYLOADS
