...
func HTTPTransportPreHook(ctx *schemas.DeepIntShieldContext, req *schemas.HTTPRequest) (*schemas.HTTPResponse, error) {
	...
	ctx.SetValue(schemas.DeepIntShieldContextKey("multi-http-request-time"), time.Now())
	ctx.SetValue(schemas.DeepIntShieldContextKey("multi-http-path"), req.Path)
...
func HTTPTransportPostHook(ctx *schemas.DeepIntShieldContext, req *schemas.HTTPRequest, resp *schemas.HTTPResponse) error {
	...
	if startTime, ok := ctx.Value(schemas.DeepIntShieldContextKey("multi-http-request-time")).(time.Time); ok {
...
func PreLLMHook(ctx *schemas.DeepIntShieldContext, req *schemas.DeepIntShieldRequest) (*schemas.DeepIntShieldRequest, *schemas.LLMPluginShortCircuit, error) {
	...
	if pluginConfig.EnableLogging {
		...
		httpPath := ctx.Value(schemas.DeepIntShieldContextKey("multi-http-path"))
		fmt.Printf("[Multi-Interface Plugin] Processing LLM request from path: %v\n", httpPath)
	}

	// Store LLM metadata
	ctx.SetValue(schemas.DeepIntShieldContextKey("multi-llm-start-time"), time.Now())
...
func PostLLMHook(ctx *schemas.DeepIntShieldContext, resp *schemas.DeepIntShieldResponse, deepintshieldErr *schemas.DeepIntShieldError) (*schemas.DeepIntShieldResponse, *schemas.DeepIntShieldError, error) {
	...
	if startTime, ok := ctx.Value(schemas.DeepIntShieldContextKey("multi-llm-start-time")).(time.Time); ok {
		duration := time.Since(startTime)
		if pluginConfig.EnableLogging {
		...
		ctx.SetValue(schemas.DeepIntShieldContextKey("multi-llm-duration"), duration)
	}
...
func PreMCPHook(ctx *schemas.DeepIntShieldContext, req *schemas.DeepIntShieldMCPRequest) (*schemas.DeepIntShieldMCPRequest, *schemas.MCPPluginShortCircuit, error) {
	if !pluginConfig.EnableMCPHooks {
		return req, nil, nil
	}
	...
	// Store MCP metadata
	ctx.SetValue(schemas.DeepIntShieldContextKey("multi-mcp-start-time"), time.Now())
	ctx.SetValue(schemas.DeepIntShieldContextKey("multi-mcp-type"), req.RequestType)
...
func PostMCPHook(ctx *schemas.DeepIntShieldContext, resp *schemas.DeepIntShieldMCPResponse, deepintshieldErr *schemas.DeepIntShieldError) (*schemas.DeepIntShieldMCPResponse, *schemas.DeepIntShieldError, error) {
	if !pluginConfig.EnableMCPHooks {
		return resp, deepintshieldErr, nil
	}
	...
	if startTime, ok := ctx.Value(schemas.DeepIntShieldContextKey("multi-mcp-start-time")).(time.Time); ok {
		duration := time.Since(startTime)
		if pluginConfig.EnableLogging {
		...
		// Store for observability
		ctx.SetValue(schemas.DeepIntShieldContextKey("multi-mcp-duration"), duration)
	}
