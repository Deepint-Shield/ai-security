...
// HTTPTransportPreHook is called at the HTTP layer before requests enter DeepIntShield core
...
func HTTPTransportPreHook(ctx *schemas.DeepIntShieldContext, req *schemas.HTTPRequest) (*schemas.HTTPResponse, error) {
	...
	ctx.SetValue(schemas.DeepIntShieldContextKey("http-plugin-start-time"), time.Now())

	// Return nil to continue processing
	return nil, nil
}

// HTTPTransportPostHook is called at the HTTP layer after DeepIntShield core processes the request
...
func HTTPTransportPostHook(ctx *schemas.DeepIntShieldContext, req *schemas.HTTPRequest, resp *schemas.HTTPResponse) error {
	...
	startTime := ctx.Value(schemas.DeepIntShieldContextKey("http-plugin-start-time"))
	...
	if resp.StatusCode >= 400 {
		...
		if err := json.Unmarshal(resp.Body, &errorBody); err == nil {
			...
			errorBody["request_id"] = ctx.Value(schemas.DeepIntShieldContextKey("request_id"))
