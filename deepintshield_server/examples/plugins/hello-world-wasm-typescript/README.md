# DeepIntShield WASM Plugin (TypeScript/AssemblyScript)

A comprehensive example of a DeepIntShield plugin written in TypeScript and compiled to WebAssembly using AssemblyScript. This plugin demonstrates proper structure definitions, JSON parsing, context handling, and request/response modification patterns.
...
@json
class DeepIntShieldChatRequest {
...
@json
class DeepIntShieldChatResponse {
  id: string = ''
  model: string = ''
...
class LLMPluginShortCircuit {
  response: DeepIntShieldResponse | null = null  // Success short-circuit
  error: DeepIntShieldError | null = null        // Error short-circuit
}
```
...
export function pre_hook(inputPtr: u32, inputLen: u32): u64 {
  ...
  if (model === 'mock-model') {
    ...
    // Build mock response
    const mockResponse = new DeepIntShieldResponse()
    mockResponse.chat_response = new DeepIntShieldChatResponse()
...
export function pre_hook(inputPtr: u32, inputLen: u32): u64 {
  ...
  if (shouldRateLimit(input.context.request_id)) {
    ...
    const error = new DeepIntShieldError()
