...
func PreMCPHook(ctx *schemas.DeepIntShieldContext, req *schemas.DeepIntShieldMCPRequest) (*schemas.DeepIntShieldMCPRequest, *schemas.MCPPluginShortCircuit, error) {
	...
	if pluginConfig.EnableAudit {
		auditMsg := fmt.Sprintf("MCP request processed at %v", ctx.Value(schemas.DeepIntShieldContextKey("request_id")))
		ctx.SetValue(schemas.DeepIntShieldContextKey("mcp-audit-trail"), auditMsg)
		if pluginConfig.EnableLogging {
...
func PostMCPHook(ctx *schemas.DeepIntShieldContext, resp *schemas.DeepIntShieldMCPResponse, deepintshieldErr *schemas.DeepIntShieldError) (*schemas.DeepIntShieldMCPResponse, *schemas.DeepIntShieldError, error) {
	...
	if pluginConfig.EnableAudit {
		auditTrail := ctx.Value(schemas.DeepIntShieldContextKey("mcp-audit-trail"))
		if pluginConfig.EnableLogging {
			fmt.Printf("[MCP-Only Plugin] Audit trail: %v\n", auditTrail)
		}
	}
	...
	if deepintshieldErr != nil && deepintshieldErr.Error != nil {
		fmt.Printf("[MCP-Only Plugin] Error occurred: %v\n", deepintshieldErr.Error.Message)
	}

	// Example: Transform error responses (configurable)
	if pluginConfig.TransformErrors && deepintshieldErr != nil && deepintshieldErr.StatusCode != nil && *deepintshieldErr.StatusCode == 404 {
		// Convert 404 to a more user-friendly error
		if deepintshieldErr.Error != nil {
			deepintshieldErr.Error.Message = "The requested MCP resource was not found. Please check your request."
			if pluginConfig.EnableLogging {
