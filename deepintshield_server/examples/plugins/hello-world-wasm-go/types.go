...
type PreHookInput struct {
	Context map[string]interface{}  `json:"context"`
	Request *schemas.DeepIntShieldRequest `json:"request,omitempty"` // Keep raw for pass-through
}

// PreHookOutput is the output for pre_hook
type PreHookOutput struct {
	Context         map[string]interface{}      `json:"context"`
	Request         *schemas.DeepIntShieldRequest     `json:"request,omitempty"`
...
type PostHookInput struct {
	Context  map[string]interface{}   `json:"context"`
	Response *schemas.DeepIntShieldResponse `json:"response,omitempty"`
	Error    *schemas.DeepIntShieldError    `json:"error,omitempty"`
...
type HTTPStreamChunkHookInput struct {
	Context map[string]interface{}     `json:"context"`
	Request *schemas.HTTPRequest       `json:"request,omitempty"`
	Chunk   *schemas.DeepIntShieldStreamChunk `json:"chunk,omitempty"`
}
