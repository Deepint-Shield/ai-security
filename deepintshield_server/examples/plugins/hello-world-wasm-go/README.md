...
A minimal example of a DeepIntShield plugin written in Go and compiled to WebAssembly using TinyGo.
...
**PreLLMHook Input:**
- `ctx`: `{"request_id": "..."}` (context info)
- `req`: DeepIntShield request JSON
...
**PostLLMHook Input:**
- `ctx`: Context JSON
- `resp`: DeepIntShield response JSON
- `err`: DeepIntShield error JSON (or null)

**PostLLMHook Output:**
```json
{
  "response": { ... },
  "deepintshield_error": null,
  "error": ""
}
```
