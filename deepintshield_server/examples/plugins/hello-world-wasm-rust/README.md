# DeepIntShield WASM Plugin (Rust)

A comprehensive example of a DeepIntShield plugin written in Rust and compiled to WebAssembly. This plugin demonstrates proper structure definitions with serde, JSON parsing, context handling, and request/response modification patterns.
...
pub struct DeepIntShieldContext {
...
pub extern "C" fn pre_hook(input_ptr: u32, input_len: u32) -> u64 {
    ...
    if model == "mock-model" {
        ...
        // Build mock response
        let mock_response = DeepIntShieldResponse {
            chat_response: Some(DeepIntShieldChatResponse {
...
pub extern "C" fn pre_hook(input_ptr: u32, input_len: u32) -> u64 {
    ...
    if should_rate_limit(&input.context) {
        ...
        output.short_circuit = Some(LLMPluginShortCircuit {
            response: None,
            error: Some(
                DeepIntShieldError::new("Rate limit exceeded")
...
## Usage with DeepIntShield

Configure the plugin in your DeepIntShield config:
