...
This is a sample MCP server implementation that runs over HTTP but **does not support the optional `ping` method**. This demonstrates how to configure DeepIntShield to use the `listTools` health check method instead of ping.
...
2. **How to configure DeepIntShield** to work with such servers using `is_ping_available: false`
3. **Why this matters**: When `is_ping_available` is `false`, DeepIntShield will use `listTools` for health checks instead of the lightweight `ping` method
...
Note: This server does NOT support ping. Use is_ping_available=false in DeepIntShield config.
```

## Connecting via DeepIntShield
...
When you add this server to DeepIntShield with `is_ping_available: false`:

1. DeepIntShield will **NOT** send `ping` requests (since the server doesn't support them)
2. Instead, DeepIntShield will use `listTools` every 10 seconds to check server health
...
- ✅ Demonstrate the `is_ping_available=false` behavior in DeepIntShield
...
- [DeepintShield documentation](https://deepintshield.com)
