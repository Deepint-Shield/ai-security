...
import { DeepIntShieldConfig, GlobalProxyConfig, LatestReleaseResponse } from "@/lib/types/config";
...
export const configApi = baseApi.injectEndpoints({
	endpoints: (builder) => ({
		...
		getCoreConfig: builder.query<DeepIntShieldConfig, { fromDB?: boolean }>({
			query: ({ fromDB = false } = {}) => ({
				url: "/config",
		...
		getLatestRelease: builder.query<LatestReleaseResponse, void>({
			queryFn: async (_arg, { signal }) => {
				try {
					const response = await axios.get("https://deepintshield.com/latest-release", {
		...
		updateCoreConfig: builder.mutation<null, DeepIntShieldConfig>({
			query: (data) => ({
				url: "/config",
				method: "PUT",
