...
function buildFilterParams(filters: LogFilters): Record<string, string | number> {
	...
	if (filters.guardrail_statuses && filters.guardrail_statuses.length > 0) {
		params.guardrail_statuses = filters.guardrail_statuses.join(",");
	}
	if (filters.guardrail_sources && filters.guardrail_sources.length > 0) {
		params.guardrail_sources = filters.guardrail_sources.join(",");
	}
...
export const logsApi = baseApi.injectEndpoints({
	endpoints: (builder) => ({
		...
		// Get semantic cache histogram with hit/miss breakdown
		getLogsCacheHistogram: builder.query<
			CacheHistogramResponse,
			{
				filters: LogFilters;
			}
		...
		// Get per-source cost-optimization savings + categorical aggregates.
		// Feeds the Cost Optimization analytics tab; same filter shape as the
		// cost histogram above.
		getLogsOptimizationHistogram: builder.query<
			OptimizationHistogramResponse,
			{
				filters: LogFilters;
			}
		>({
			query: ({ filters }) => ({
				url: "/logs/histogram/optimization",
				params: buildFilterParams(filters),
			}),
			providesTags: ["Logs"],
		}),
		...
		getAvailableFilterData: builder.query<
			{
				...
				guardrail_statuses: string[];
				metadata_keys: Record<string, string[]>;
			},
			{ scope?: "llm" | "agentic" | "all" } | void
		>({
			// Scope drives which metadata keys the server includes - see
			// /api/logs/filterdata handler for the LLM/agentic split. When
			// no scope is passed the server defaults to MetadataScopeLLM
			// so the AI Logs route doesn't surface agentic decision
			// keys as permanently-empty dynamic columns.
			query: (arg) => {
				const s = arg && "scope" in (arg as object) ? (arg as { scope?: string }).scope : undefined;
				return s ? `/logs/filterdata?scope=${encodeURIComponent(s)}` : "/logs/filterdata";
			},
			providesTags: ["Logs"],
		}),
		...
		refreshLogsDashboard: builder.mutation<{ refreshed: boolean }, void>({
			query: () => ({
				url: "/logs/refresh-dashboard",
				method: "POST",
			}),
			invalidatesTags: ["Logs"],
		}),
...
export const {
	...
	useGetLogsCacheHistogramQuery,
	useGetLogsCostHistogramQuery,
	useGetLogsOptimizationHistogramQuery,
	...
	useLazyGetLogsOptimizationHistogramQuery,
