...
export const promptsApi = baseApi.injectEndpoints({
  ...
  endpoints: (builder) => ({
    ...
    // Get all prompts (optionally filtered by folder + workspace).
    // workspaceId, when set, narrows the result to org-wide prompts and
    // prompts scoped to that workspace; empty preserves the legacy
    // tenant-wide listing.
    getPrompts: builder.query<GetPromptsResponse, { folderId?: string; workspaceId?: string } | void>({
      query: (params) => {
        ...
        return `/prompt-repo/prompts${qs ? `?${qs}` : ''}`
      },
