import {
	...
	GetGovernanceUsersParams,
	GetGovernanceUsersResponse,
	...
	HealthCheckResponse,
	InviteGovernanceUserRequest,
	InviteGovernanceUserResponse,
	KeyHealthResponse,
	ModelConfig,
	ProviderGovernance,
	RateLimit,
	...
	Team,
	UpdateGovernanceUserRoleRequest,
	UpdateGovernanceUserRoleResponse,
	UpdateBudgetRequest,
...
type ApiCollection<T> = T[] | Record<string, T> | undefined;

function normalizeCollection<T>(collection: ApiCollection<T>): T[] {
...
export const governanceApi = baseApi.injectEndpoints({
	endpoints: (builder) => ({
		...
		getVirtualKeys: builder.query<GetVirtualKeysResponse, GetVirtualKeysParams | void>({
			...
			transformResponse: (response: Omit<GetVirtualKeysResponse, "virtual_keys"> & {
				virtual_keys?: VirtualKey[] | Record<string, VirtualKey>;
			}): GetVirtualKeysResponse => ({
				...response,
				virtual_keys: normalizeCollection(response.virtual_keys),
			}),
		...
		rotateVirtualKey: builder.mutation<{ message: string; virtual_key: VirtualKey }, { vkId: string; data?: RotateVirtualKeyRequest }>({
			query: ({ vkId, data }) => ({
		...
		// across selected VKs or every agent-bound VK in the active workspace.
		bulkUpdateVirtualKeyAgentAttributes: builder.mutation<
			{ updated: number; matched: number; workspace_id: string },
			BulkAgentAttributesRequest
		>({
			query: (data) => ({
				url: "/governance/virtual-keys/bulk-agent-attributes",
				method: "POST",
				body: data,
			}),
			invalidatesTags: ["VirtualKeys"],
		...
		getTeams: builder.query<GetTeamsResponse, GetTeamsParams | void>({
			...
			transformResponse: (response: Omit<GetTeamsResponse, "teams"> & {
				teams?: Team[] | Record<string, Team>;
			}): GetTeamsResponse => ({
				...response,
				teams: normalizeCollection(response.teams),
			}),
			providesTags: ["Teams"],
		}),
		...
		// User provisioning
		getGovernanceUsers: builder.query<GetGovernanceUsersResponse, GetGovernanceUsersParams | void>({
			query: (params) => ({
		...
		inviteGovernanceUser: builder.mutation<InviteGovernanceUserResponse, InviteGovernanceUserRequest>({
			query: (data) => ({
				url: "/governance/users/invitations",
				method: "POST",
				body: data,
			}),
		...
		resendGovernanceInvitation: builder.mutation<InviteGovernanceUserResponse, string>({
			query: (invitationId) => ({
				url: `/governance/users/invitations/${invitationId}/resend`,
				method: "POST",
			}),
			invalidatesTags: ["Users"],
		}),

		updateGovernanceUserRole: builder.mutation<UpdateGovernanceUserRoleResponse, { userId: string; data: UpdateGovernanceUserRoleRequest }>({
			query: ({ userId, data }) => ({
				url: `/governance/users/${userId}/role`,
				method: "PUT",
				body: data,
			}),
			invalidatesTags: ["Users"],
		...
		updateGovernanceInvitationRole: builder.mutation<UpdateGovernanceUserRoleResponse, { invitationId: string; data: UpdateGovernanceUserRoleRequest }>({
			query: ({ invitationId, data }) => ({
				url: `/governance/users/invitations/${invitationId}/role`,
				method: "PUT",
				body: data,
			}),
			invalidatesTags: ["Users"],
		...
		deleteGovernanceUser: builder.mutation<GovernanceActionResponse, string>({
			query: (userId) => ({
				url: `/governance/users/${userId}`,
				method: "DELETE",
			}),
			invalidatesTags: ["Users"],
		}),

		revokeGovernanceInvitation: builder.mutation<GovernanceActionResponse, string>({
			query: (invitationId) => ({
				url: `/governance/users/invitations/${invitationId}`,
				method: "DELETE",
			}),
			invalidatesTags: ["Users"],
		}),
		...
		updateModelConfig: builder.mutation<{ message: string; model_config: ModelConfig }, { id: string; data: UpdateModelConfigRequest }>({
			...
			invalidatesTags: (_result, _error, { id }) => ["ModelConfigs", { type: "ModelConfigs", id }],
		}),
		...
		// Key Health (Load Balancer)
		getKeyHealth: builder.query<KeyHealthResponse, void>({
			query: () => "/governance/key-health",
		}),
	}),
});
...
export const {
	...
	useBulkUpdateVirtualKeyAgentAttributesMutation,
	...
	// User provisioning
	useGetGovernanceUsersQuery,
	useInviteGovernanceUserMutation,
	useResendGovernanceInvitationMutation,
	useUpdateGovernanceUserRoleMutation,
	useUpdateGovernanceInvitationRoleMutation,
	useDeleteGovernanceUserMutation,
	useRevokeGovernanceInvitationMutation,
