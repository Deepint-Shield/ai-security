...
// DeepIntShieldErrorResponse matching Go's schemas.DeepIntShieldError
export interface DeepIntShieldErrorResponse {
	event_id?: string;
	type?: string;
...
export interface DeepIntShieldConfig {
...
// Core DeepIntShield configuration types
...
export type CacheAutoScopeMode = "conservative" | "balanced" | "aggressive";
export type CacheSharedVKPolicy = "exact_only_when_unscoped" | "allow_semantic_when_unscoped";
export type PromptCacheAnthropicTTL = "5m" | "1h";
export type PromptCacheGoogleTTL = "5m" | "1h" | "6h" | "24h";
...
export interface CacheConfig {
	...
	auto_scope_enabled?: boolean;
	auto_scope_mode?: CacheAutoScopeMode;
	shared_vk_policy?: CacheSharedVKPolicy;
	scope_signal_order?: string[];
	metadata_scope_keys?: string[];
	// Provider-native prompt caching (workspace-level switch). Defaults applied
	// server-side when omitted, so all fields are optional on the wire.
	prompt_cache_enabled?: boolean;
	prompt_cache_providers?: string[];
	prompt_cache_anthropic_ttl?: PromptCacheAnthropicTTL;
	prompt_cache_google_ttl?: PromptCacheGoogleTTL;
	prompt_cache_min_static_tokens?: number;
	prompt_cache_breakpoints?: string[];

	// Request coalescing - in-flight deduplication of identical requests.
	coalescing_enabled?: boolean;
	coalescing_max_in_flight?: number;
	coalescing_wait_timeout_ms?: number;
	coalescing_vk_scope?: string[]; // empty = all VKs; non-empty = only these VK IDs
	// Per-tool MCP TTL overrides - map of "<server>-<tool>" → duration string.
	mcp_tool_ttls?: Record<string, string>;
	// Guardrail evaluation cache.
	guardrail_cache_enabled?: boolean;
	guardrail_cache_ttl_seconds?: number;
	guardrail_cache_max_entries?: number;
	guardrail_cache_vk_scope?: string[];
	// Top-level feature VK scopes - empty/undefined = applies to all virtual
	// keys; non-empty = only those VK IDs get the feature.
	semantic_cache_vk_scope?: string[];
	prompt_cache_vk_scope?: string[];
	// Embedding via workspace VK - when enabled, semantic cache stops calling
	// the local huggingface sidecar and instead routes embedding generation
	// through the operator-chosen VK (e.g. OpenAI text-embedding-3-small).
	// Lets fresh workspaces get a working semantic cache without standing
	// up the embedding container; trade off ~$0.02 / 1M tokens vs sidecar
	// hosting overhead.
	embedding_via_vk_enabled?: boolean;
	embedding_vk_id?: string;
	embedding_via_vk_model?: string;
	created_at?: string;
	updated_at?: string;
}
