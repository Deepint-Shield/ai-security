// Types for the logs interface based on DeepIntShieldResponse schema
...
export interface DeepIntShieldSpeech {
	usage?: AudioLLMUsage;
	audio: string; // base64 encoded audio data
}

export interface DeepIntShieldTranscribe {
	text: string;
...
export interface DeepIntShieldEmbedding {
	index: number;
	object: string;
...
export interface DeepIntShieldImageGenerationData {
	url?: string;
	b64_json?: string;
...
export interface DeepIntShieldImageGenerationOutput {
	id?: string;
	created?: number;
	model?: string;
	data: DeepIntShieldImageGenerationData[];
	background?: string;
	output_format?: string;
	quality?: string;
	size?: string;
...
export interface DeepIntShieldVideoGenerationOutput {
	videos: VideoOutput[];
	id?: string;
	completed_at?: number;
	created_at?: number;
...
export interface DeepIntShieldVideoDownloadOutput {
	video_id: string;
	content_type?: string;
}

export interface DeepIntShieldVideoDeleteOutput {
	id: string;
	deleted: boolean;
	object?: string;
}

export interface DeepIntShieldVideoListOutput {
	object: string;
	data: VideoObject[];
	first_id?: string;
	has_more?: boolean;
...
export interface CacheDebug {
	...
	cache_id?: string;
	hit_type?: string;
	effective_cache_key?: string;
	scope_type?: string;
	scope_value_hash?: string;
	scope_source?: string;
	semantic_suppressed_reason?: string;
	provider_used?: string;
	model_used?: string;
	input_tokens?: number;
	threshold?: number;
	similarity?: number;
	guardrail_reused?: boolean;
	guardrail_cache_source?: string;
	guardrail_fingerprint_match?: boolean;
}
...
export interface DeepIntShieldError {
	event_id?: string;
	type?: string;
	is_deepintshield_error: boolean;
	status_code?: number;
	error: ErrorField;
}
...
export interface LogEntry {
	...
	embedding_output?: DeepIntShieldEmbedding[];
	rerank_output?: RerankResult[];
	image_generation_output?: DeepIntShieldImageGenerationOutput;
	video_generation_output?: DeepIntShieldVideoGenerationOutput;
	video_retrieve_output?: DeepIntShieldVideoGenerationOutput;
	video_download_output?: DeepIntShieldVideoDownloadOutput;
	video_list_output?: DeepIntShieldVideoListOutput;
	video_delete_output?: DeepIntShieldVideoDeleteOutput;
	params?: ModelParameters;
	speech_input?: SpeechInput;
	transcription_input?: TranscriptionInput;
	image_generation_input?: { prompt: string };
	video_generation_input?: { prompt: string };
	speech_output?: DeepIntShieldSpeech;
	transcription_output?: DeepIntShieldTranscribe;
	...
	guardrail_source?: string; // "ai_model" | "policy" | "mixed" | "" - which engine produced findings
	cache_savings?: number;
	prompt_cache_savings?: number; // Cost avoided by provider-native prompt caching on this request
	cost?: number; // Cost in dollars (total cost of the request - includes cache lookup cost)
	status: string; // "success" or "error"
	error_details?: DeepIntShieldError;
...
export interface LogFilters {
	...
	guardrail_statuses?: string[]; // For filtering by guardrail decision (allow, deny, warn, etc.)
	guardrail_sources?: string[];  // For filtering by guardrail engine kind: "ai_model" | "policy" | "mixed"
	status?: string[];
...
export interface LogStats {
	...
	total_tokens: number;
	total_cost: number;
	total_cache_savings?: number; // Semantic-cache savings (request short-circuited)
	total_prompt_cache_savings?: number; // Provider-native prompt cache savings (cached_read discount net of cached_write premium)
}
...
export interface TokenHistogramBucket {
	...
	total_tokens: number;
	cached_read_tokens: number;
	effective_cached_prompt_tokens: number;
}
...
export interface CacheHistogramBucket {
	timestamp: string;
	...
	user_scope_hits: number;
	use_case_scope_hits: number;
	session_scope_hits: number;
	virtual_key_scope_hits: number;
	custom_metadata_scope_hits: number;
	tenant_scope_hits: number;
	auto_scoped_hits: number;
	explicit_override_hits: number;
	default_scope_hits: number;
	request_fallback_hits: number;
	semantic_suppressions: number;
}

export interface CacheHistogramResponse {
	buckets: CacheHistogramBucket[];
	bucket_size_seconds: number;
}
...
export interface CostHistogramBucket {
	...
	by_model_cache_savings: Record<string, number>;
}
...
// Per-source savings + categorical aggregates surfaced by
// GET /logs/histogram/optimization. Mirrors the Go OptimizationHistogramBucket
// in framework/logstore/tables.go.
export interface OptimizationHistogramBucket {
	timestamp: string;
	semantic_savings: number;
	prompt_cache_savings: number;
	reasoning_savings: number;
	compression_savings: number;
	rag_savings: number;
	consistency_savings: number;
	total_savings: number;

	compression_applied: number;
	compression_cache_hits: number;
	compression_original_tokens: number;
	compression_compressed_tokens: number;
	compression_avg_latency_ms: number;
	compression_avg_ratio: number;

	reasoning_applied: number;
	reasoning_sampled: number;
	reasoning_effort_mix: Record<string, number>;
	reasoning_tokens: number;

	cascade_avg_score: number;
	cascade_escalations: number;
	cascade_source_mix: Record<string, number>;

	batch_eligible: number;
	batch_provider_mix: Record<string, number>;

	rag_applied: number;
	rag_chunks_detected: number;
	rag_chunks_kept: number;
	rag_trimmed_tokens: number;
	rag_original_tokens: number;
	rag_avg_rerank_latency_ms: number;
	rag_cache_hits: number;

	summarization_savings: number;
	summarization_applied: number;
	summarization_cache_hits: number;
	summarization_async_kickoffs: number;
	summarization_turns_summarized: number;
	summarization_original_tokens: number;
	summarization_summary_tokens: number;
	summarization_saved_tokens: number;

	ttft_applied: number;
	ttft_messages_reordered: number;
	ttft_stable_prefix_tokens: number;

	parallel_tools_applied: number;
	parallel_tools_total: number;
	parallel_tools_parallel_count: number;
	parallel_tools_sequential_count: number;
	parallel_tools_wall_clock_ms: number;
	parallel_tools_serial_estimate_ms: number;
	parallel_tools_latency_saved_ms: number;

	// Hallucination eval aggregates - count of rows scored + AVG of each
	// 0..1 quality metric (0 when no rows scored in the bucket).
	hallucination_scored: number;
	hallucination_avg_faithfulness: number;
	hallucination_avg_answer_relevance: number;
	hallucination_avg_coherence: number;
	hallucination_avg_helpfulness: number;
	hallucination_avg_citation_precision: number;
	hallucination_avg_score: number;

	// Hallucination Control - proactive mitigation telemetry.
	hallucination_control_applied: number;
	hallucination_control_avg_improvement: number;
}

export interface OptimizationHistogramResponse {
...
export interface ProviderCostHistogramBucket {
	...
	by_provider_cache_savings: Record<string, number>;
}
...
export interface MCPToolLogEntry {
	...
	cache_hit?: boolean; // True when the response was served from the MCP cache plugin
	guardrail_status?: string;
	guardrail_source?: string; // "ai_model" | "policy" | "mixed" | "" - which engine produced findings // Latest guardrail decision joined from /guardrails/traces at list time
