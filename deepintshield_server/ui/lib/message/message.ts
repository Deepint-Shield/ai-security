...
export class Message {
  ...
  static toAPIMessages(messages: Message[]): APIMessage[] {
    ...
    for (let i = 0; i < messages.length; i++) {
      if (messages[i].type !== MessageType.CompletionError) continue
      droppedIndices.add(i)
      for (let j = i - 1; j >= 0; j--) {
        if (droppedIndices.has(j)) continue
        const prev = messages[j]
        if (prev.type === MessageType.CompletionError) continue
        if (prev.role === 'user' || prev.type === MessageType.ToolResult) {
    ...
    return messages
      .filter((_, idx) => !droppedIndices.has(idx))
      .map((m): APIMessage => {
