...
export default function SecurityView() {
	...
	useEffect(() => {
		...
		if (deepintshieldConfig?.auth_config) {
			setAuthConfig(deepintshieldConfig.auth_config);
		}
	...
	const hasChanges = useMemo(() => {
		...
		const usernameChanged =
			authConfig.admin_username?.value !== deepintshieldConfig?.auth_config?.admin_username?.value ||
			authConfig.admin_username?.env_var !== deepintshieldConfig?.auth_config?.admin_username?.env_var ||
			authConfig.admin_username?.from_env !== deepintshieldConfig?.auth_config?.admin_username?.from_env;
		const passwordChanged =
			authConfig.admin_password?.value !== deepintshieldConfig?.auth_config?.admin_password?.value ||
			authConfig.admin_password?.env_var !== deepintshieldConfig?.auth_config?.admin_password?.env_var ||
			authConfig.admin_password?.from_env !== deepintshieldConfig?.auth_config?.admin_password?.from_env;
		const authChanged =
			authConfig.is_enabled !== deepintshieldConfig?.auth_config?.is_enabled ||
			usernameChanged ||
			passwordChanged ||
			authConfig.disable_auth_on_inference !== deepintshieldConfig?.auth_config?.disable_auth_on_inference;
	...
	}, [config, localConfig, authConfig, deepintshieldConfig]);
	...
	}, [deepintshieldConfig, localConfig, authConfig, updateCoreConfig]);
		...
		<div className="workspace-page-shell space-y-5">
			<header className="space-y-1.5">
				...
				<div className="flex items-center gap-2.5">
					...
					<div>
						<h1 className="text-2xl font-semibold leading-none tracking-tight">Security Settings</h1>
						<p className="text-muted-foreground mt-1 text-sm">Configure security and access control settings.</p>
					</div>
				</div>
			</header>
							...
							<div className="flex items-center justify-between">
								<div className="space-y-0.5">
									...
									<p className="text-muted-foreground text-sm">
										Set up authentication credentials to protect your DeepIntShield dashboard. Once configured, use the generated token for
										all admin API calls.
									</p>
								</div>
				...
				<div className="flex items-center justify-between space-x-2 rounded-lg border p-4">
					<div className="space-y-0.5">
						...
						<p className="text-muted-foreground text-sm">
							...
							DeepIntShield will directly use the key.
						</p>
					</div>
					<Switch
...
const RestartWarning = () => {
	return (
		<Alert variant="destructive" className="mt-2">
			...
			<AlertDescription>Need to restart DeepIntShield to apply changes.</AlertDescription>
		</Alert>
	);
};
