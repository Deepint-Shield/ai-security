...
export default function ProxyView() {
	...
	const proxyConfig = deepintshieldConfig?.proxy_config;
				...
				<form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
					<header className="space-y-1.5">
						...
						<div className="flex items-center gap-2.5">
							...
							<div>
								<h1 className="text-2xl font-semibold leading-none tracking-tight">Proxy Settings</h1>
								<p className="text-muted-foreground mt-1 text-sm">Configure global proxy settings for outbound requests.</p>
							</div>
						</div>
					</header>
						...
						<div className={cn("space-y-4 rounded-lg border p-4 transition-opacity", !watchedEnabled && "pointer-events-none opacity-50")}>
							...
							{!IS_ENTERPRISE && (
								<Alert>
									...
									<AlertDescription>SCIM proxy support is available in DeepIntShield advanced controls.</AlertDescription>
								</Alert>
							)}
						</div>
					</fieldset>
