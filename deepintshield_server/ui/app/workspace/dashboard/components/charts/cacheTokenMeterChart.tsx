...
import { CHART_COLORS, CHART_NEEDLE_COLOR } from "../../utils/chartUtils";
...
const METER_COLORS = {
	cached: CHART_COLORS.cachedReadTokens,
	input: CHART_COLORS.promptTokens,
};
...
function Needle({ percentage, geometry }: { percentage: number; geometry: GaugeGeometry }) {
	...
	return (
		<g>
			<path d={`M ${bx1} ${by1} L ${tipX} ${tipY} L ${bx2} ${by2} Z`} fill={CHART_NEEDLE_COLOR} />
			<circle cx={cx} cy={cy} r={5} fill={CHART_NEEDLE_COLOR} />
		</g>
	);
}
...
export default function CacheTokenMeterChart({ data }: CacheTokenMeterChartProps) {
	...
	const { percentage, totalCachedRead, totalPromptTokens } = useMemo(() => {
		...
		for (const bucket of data.buckets) {
			cachedRead += bucket.effective_cached_prompt_tokens ?? bucket.cached_read_tokens;
						...
						<Tooltip>
							<TooltipTrigger asChild>
								<button
									...
									className="dashboard-chart-tooltip-title hover:text-foreground transition-colors"
									aria-label="More information about cache hit rate"
								>
									<Info className="h-3 w-3" />
								</button>
							</TooltipTrigger>
							<TooltipContent side="top">This reflects prompt coverage from both provider prompt caching and DeepIntShield gateway cache hits.</TooltipContent>
						</Tooltip>
					</div>
				</div>
