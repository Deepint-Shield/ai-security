...
export interface OverviewTabProps {
	...
	guardrailLatencyData: LatencyHistogramResponse | null;
	...
	loadingGuardrailLatency: boolean;
	...
	guardrailLatencyChartType: ChartType;
	...
	onGuardrailLatencyChartToggle: (type: ChartType) => void;
...
export function OverviewTab({
	...
	onGuardrailLatencyChartToggle,
	onCostModelChange,
	onUsageModelChange,
}: OverviewTabProps) {
	// Canonical platform savings - on the "all" view this folds in every cost-
	// saving feature (gateway cache_savings incl. Response Consistency, plus the
	// agentic-cache overlay) so the Overview "Savings" pill reconciles with the
	// Cost-Opt + Cache tabs. A specific model selection is dimension-scoped, so
	// the hook auto-omits the flat agentic-cache overlay (RCE still counts, since
	// it's per-row model-stamped).
	const costTotals = usePlatformSavings({ costData, startTime, endTime, selectedModel: costModel });
		...
		<>
			...
			<div className="grid grid-cols-1 gap-2 lg:grid-cols-2 2xl:grid-cols-3">
				...
				<ChartCard
					...
					loading={loadingGuardrailLatency}
					testId="chart-guardrail-latency"
					headerActions={
						<div className={CHART_HEADER_ACTIONS_CLASS}>
							<div className={CHART_HEADER_LEGEND_CLASS}>
								<span className="flex items-center gap-1">
							...
							<div className={CHART_HEADER_CONTROLS_CLASS}>
								<ChartTypeToggle
									chartType={guardrailLatencyChartType}
									onToggle={onGuardrailLatencyChartToggle}
									data-testid="dashboard-guardrail-latency-chart-toggle"
								/>
							</div>
						</div>
					}
				>
					<LatencyChart data={guardrailLatencyData} chartType={guardrailLatencyChartType} startTime={startTime} endTime={endTime} />
				</ChartCard>
				{/* Cost Chart */}
				<ChartCard
					title="Cost"
						...
						<div className={CHART_HEADER_ACTIONS_CLASS}>
							...
							<div className={CHART_HEADER_CONTROLS_CLASS}>
								<CostHeaderSummary
									...
									testIdPrefix="dashboard-cost-summary"
								/>
								<ModelFilterSelect
