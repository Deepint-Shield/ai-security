...
function TrendBadge({ value, positiveIsGood = true, isNew = false }: { value: number; positiveIsGood?: boolean; isNew?: boolean }) {
	...
	return (
		<span
			...
			style={{ color: isGood ? CHART_COLORS.success : CHART_COLORS.error }}
		>
...
function UsageShareTooltip({ active, payload, models }: any) {
	...
	return (
		<div className="dashboard-chart-tooltip">
			<div className="dashboard-chart-tooltip-title">{formatFullTimestamp(data.timestamp)}</div>
					...
					return (
						<div key={model} className="flex items-center justify-between gap-4">
							<span className="flex items-center gap-1.5">
								...
								<span className="dashboard-chart-tooltip-meta max-w-[140px] truncate">{model}</span>
							</span>
							<span className="dashboard-chart-tooltip-value">{val.toLocaleString()}</span>
						</div>
					);
				})}
			</div>
		...
		<div className="flex flex-col gap-4">
			...
			{loading ? (
				...
				<Card className="rounded-sm p-2 shadow-none" data-testid="dashboard-model-rankings-table">
					<span className="text-primary pl-2 text-sm font-medium">AI Model Rankings</span>
					<Table>
						<TableHeader>
							<TableRow>
