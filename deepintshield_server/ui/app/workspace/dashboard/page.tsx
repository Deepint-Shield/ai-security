...
import {
	...
	useRefreshLogsDashboardMutation,
} from "@/lib/store";
import type {
...
import { Activity, BarChart3, Bot, Cpu, Database, GitBranch, RefreshCw, ScanSearch, Shield, Sparkles } from "lucide-react";
...
export default function DashboardPage() {
	...
	const [guardrailLatencyData, setGuardrailLatencyData] = useState<LatencyHistogramResponse | null>(null);
	...
	const [loadingGuardrailLatency, setLoadingGuardrailLatency] = useState(true);
	...
	const [loadBalancerRefreshKey, setLoadBalancerRefreshKey] = useState(0);
	...
	const [triggerGuardrailLatency] = useLazyGetGuardrailLatencyHistogramQuery();
	...
	const [refreshLogsDashboard] = useRefreshLogsDashboardMutation();
	...
	const [urlState, setUrlState] = useQueryStates(
		{
			...
			cache_requests_chart: parseAsString.withDefault("bar"),
			cache_savings_chart: parseAsString.withDefault("bar"),
			cache_hit_rate_chart: parseAsString.withDefault("bar"),
			cache_hit_types_chart: parseAsString.withDefault("bar"),
			model_chart: parseAsString.withDefault("bar"),
			latency_chart: parseAsString.withDefault("bar"),
			guardrail_latency_chart: parseAsString.withDefault("bar"),
			...
			governance_team: parseAsString.withDefault("all"),
			governance_request_chart: parseAsString.withDefault("bar"),
			governance_token_chart: parseAsString.withDefault("bar"),
			governance_cost_chart: parseAsString.withDefault("bar"),
			governance_latency_chart: parseAsString.withDefault("bar"),
			governance_provider_cost_chart: parseAsString.withDefault("bar"),
			governance_provider_token_chart: parseAsString.withDefault("bar"),
			governance_provider_latency_chart: parseAsString.withDefault("bar"),
			governance_error_chart: parseAsString.withDefault("bar"),
			governance_cache_chart: parseAsString.withDefault("bar"),
			governance_member_chart: parseAsString.withDefault("bar"),
			governance_role_chart: parseAsString.withDefault("bar"),
			governance_login_chart: parseAsString.withDefault("bar"),
			governance_cost_model: parseAsString.withDefault("all"),
			governance_provider_cost_provider: parseAsString.withDefault("all"),
			governance_provider_token_provider: parseAsString.withDefault("all"),
			governance_provider_latency_provider: parseAsString.withDefault("all"),
			...
			lb_volume_chart: parseAsString.withDefault("bar"),
			lb_cost_chart: parseAsString.withDefault("bar"),
			lb_latency_chart: parseAsString.withDefault("bar"),
			lb_volume_engine: parseAsString.withDefault("all"),
			lb_cost_engine: parseAsString.withDefault("all"),
			lb_latency_engine: parseAsString.withDefault("all"),
		},
		{
			history: "push",
			shallow: false,
	...
	// Teams (governance), Governance Insights, and Response Consistency are
	// Cloud/Enterprise only and are not part of the OSS dashboard; fold any stale
	// URL for them back to Summary.
	const NON_OSS_TABS = ["governance", "governance-insights", "response-consistency"];
	const activeTab = useMemo(() => {
		const t = urlState.tab || "overview";
		return NON_OSS_TABS.includes(t) ? "overview" : t;
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [urlState.tab]);

	useEffect(() => {
		if (NON_OSS_TABS.includes(urlState.tab || "")) {
	...
	const fetchOverviewData = useCallback(async () => {
		...
		setLoadingGuardrailLatency(true);

		const fetchFilters = { filters };
		const guardrailFetchFilters = {
			start_time: dateUtils.toISOString(urlState.start_time),
			end_time: dateUtils.toISOString(urlState.end_time),
		};

		const [histogramResult, cacheResult, tokenResult, costResult, modelResult, latencyResult, guardrailLatencyResult] = await Promise.all([
			triggerHistogram(fetchFilters, false),
			triggerCache(fetchFilters, false),
			...
			triggerGuardrailLatency(guardrailFetchFilters, false),
		]);
		...
		setGuardrailLatencyData(guardrailLatencyResult.data ?? null);
		setLoadingGuardrailLatency(false);
	}, [
		filters,
		urlState.end_time,
		urlState.start_time,
	...
	useEffect(() => {
		const tab = activeTab;
		if (tab === "overview" || tab === "cache" || tab === "cost-optimization") void ensureOverviewDataLoaded();
	...
	}, [activeTab, ensureOverviewDataLoaded, ensureProviderDataLoaded, ensureRankingsDataLoaded, ensureMcpDataLoaded]);
	...
	}, [activeTab, ensureOverviewDataLoaded, ensureProviderDataLoaded, ensureMcpDataLoaded, ensureRankingsDataLoaded]);
	...
	const handleTabChange = useCallback(
		(tab: string) => {
			setUrlState({ tab: tab === "governance-insights" ? "governance" : tab });
		},
		[setUrlState],
	);
	...
	const handleCacheRequestsChartToggle = useCallback((type: ChartType) => setUrlState({ cache_requests_chart: type }), [setUrlState]);
	const handleCacheSavingsChartToggle = useCallback((type: ChartType) => setUrlState({ cache_savings_chart: type }), [setUrlState]);
	const handleCacheHitRateChartToggle = useCallback((type: ChartType) => setUrlState({ cache_hit_rate_chart: type }), [setUrlState]);
	const handleCacheHitTypesChartToggle = useCallback((type: ChartType) => setUrlState({ cache_hit_types_chart: type }), [setUrlState]);
	...
	const handleGuardrailLatencyChartToggle = useCallback((type: ChartType) => setUrlState({ guardrail_latency_chart: type }), [setUrlState]);
	...
	// Load Balancer chart type toggles
	const handleLbVolumeChartToggle = useCallback((type: ChartType) => setUrlState({ lb_volume_chart: type }), [setUrlState]);
	const handleLbCostChartToggle = useCallback((type: ChartType) => setUrlState({ lb_cost_chart: type }), [setUrlState]);
	const handleLbLatencyChartToggle = useCallback((type: ChartType) => setUrlState({ lb_latency_chart: type }), [setUrlState]);

	// Load Balancer engine filter changes
	const handleLbVolumeEngineChange = useCallback((engine: string) => setUrlState({ lb_volume_engine: engine }), [setUrlState]);
	const handleLbCostEngineChange = useCallback((engine: string) => setUrlState({ lb_cost_engine: engine }), [setUrlState]);
	const handleLbLatencyEngineChange = useCallback((engine: string) => setUrlState({ lb_latency_engine: engine }), [setUrlState]);
	...
	const handleGovernanceTeamChange = useCallback((teamId: string) => setUrlState({ governance_team: teamId }), [setUrlState]);
	const handleGovernanceRequestChartToggle = useCallback(
		(type: ChartType) => setUrlState({ governance_request_chart: type }),
		[setUrlState],
	);
	const handleGovernanceTokenChartToggle = useCallback((type: ChartType) => setUrlState({ governance_token_chart: type }), [setUrlState]);
	const handleGovernanceCostChartToggle = useCallback((type: ChartType) => setUrlState({ governance_cost_chart: type }), [setUrlState]);
	const handleGovernanceLatencyChartToggle = useCallback(
		(type: ChartType) => setUrlState({ governance_latency_chart: type }),
		[setUrlState],
	);
	const handleGovernanceProviderCostChartToggle = useCallback(
		(type: ChartType) => setUrlState({ governance_provider_cost_chart: type }),
		[setUrlState],
	);
	const handleGovernanceProviderTokenChartToggle = useCallback(
		(type: ChartType) => setUrlState({ governance_provider_token_chart: type }),
		[setUrlState],
	);
	const handleGovernanceProviderLatencyChartToggle = useCallback(
		(type: ChartType) => setUrlState({ governance_provider_latency_chart: type }),
		[setUrlState],
	);
	const handleGovernanceErrorChartToggle = useCallback((type: ChartType) => setUrlState({ governance_error_chart: type }), [setUrlState]);
	const handleGovernanceCacheChartToggle = useCallback((type: ChartType) => setUrlState({ governance_cache_chart: type }), [setUrlState]);
	const handleGovernanceCostModelChange = useCallback((model: string) => setUrlState({ governance_cost_model: model }), [setUrlState]);
	const handleGovernanceProviderCostProviderChange = useCallback(
		(provider: string) => setUrlState({ governance_provider_cost_provider: provider }),
		[setUrlState],
	);
	const handleGovernanceProviderTokenProviderChange = useCallback(
		(provider: string) => setUrlState({ governance_provider_token_provider: provider }),
		[setUrlState],
	);
	const handleGovernanceProviderLatencyProviderChange = useCallback(
		(provider: string) => setUrlState({ governance_provider_latency_provider: provider }),
		[setUrlState],
	);
	...
	const handleRefreshDashboard = useCallback(async () => {
		...
		rankingsFetchedRef.current = false;
		rankingsLoadingRef.current = false;
		setGovernanceRefreshKey((current) => current + 1);
		setLoadBalancerRefreshKey((current) => current + 1);

		try {
			await refreshLogsDashboard()
				.unwrap()
				.catch(() => undefined);
			await Promise.allSettled([fetchOverviewData(), fetchProviderData(), fetchMcpData(), fetchRankingsData()]);
	...
	}, [fetchMcpData, fetchOverviewData, fetchProviderData, fetchRankingsData, isRefreshingDashboard, refreshLogsDashboard]);

	return (
		<div className="workspace-page-shell page-fit">
			{/* Header with time filter */}
			<div className="flex flex-wrap items-end justify-between gap-3">
				<div className="space-y-1">
					<div className="text-muted-foreground text-[11px] font-semibold tracking-[0.18em] uppercase">Analytics</div>
					<div className="flex items-center gap-2.5">
						...
						<div>
							<h1 className="text-2xl leading-none font-semibold tracking-tight">Overview</h1>
							{/* <p className="mt-1 text-muted-foreground text-xs">
								Live request, latency, cache, cost, and model telemetry across the active workspace.
							</p> */}
						</div>
					</div>
				</div>
				<div className="border-border/60 bg-card/70 flex flex-wrap items-center gap-2 rounded-xl border p-1.5 shadow-[0_1px_2px_rgba(11,42,49,0.04),0_8px_18px_-12px_rgba(11,42,49,0.10)] backdrop-blur-md">
					{(activeTab === "overview" ||
						activeTab === "cache" ||
						...
						activeTab === "load-balancer") && <FilterPopover filters={filters} onFilterChange={handleFilterChange} />}
			...
			{/* Tabs */}
			<Tabs value={activeTab} onValueChange={handleTabChange} className="page-fit-scroll flex flex-col gap-3">
				<TabsList className="border-border/60 bg-card/70 grid h-auto w-full shrink-0 grid-cols-2 gap-1 rounded-2xl border p-1.5 backdrop-blur-md md:grid-cols-6 xl:grid-cols-12">
					<TabsTrigger
						value="overview"
						data-testid="dashboard-tab-overview"
						className="data-[state=active]:bg-primary/12 data-[state=active]:text-primary gap-1.5 rounded-xl text-xs font-medium data-[state=active]:shadow-[inset_0_1px_0_rgba(255,255,255,0.16),0_2px_8px_-4px_rgba(34,211,196,0.35)]"
					>
					...
					<TabsTrigger
						value="cache"
						data-testid="dashboard-tab-cache"
						className="data-[state=active]:bg-primary/12 data-[state=active]:text-primary gap-1.5 rounded-xl text-xs font-medium data-[state=active]:shadow-[inset_0_1px_0_rgba(255,255,255,0.16),0_2px_8px_-4px_rgba(34,211,196,0.35)]"
					>
					...
					<TabsTrigger
						...
						data-testid="dashboard-tab-cost-optimization"
						className="data-[state=active]:bg-primary/12 data-[state=active]:text-primary gap-1.5 rounded-xl text-xs font-medium data-[state=active]:shadow-[inset_0_1px_0_rgba(255,255,255,0.16),0_2px_8px_-4px_rgba(34,211,196,0.35)]"
					>
					...
					<TabsTrigger
						value="guardrails"
						data-testid="dashboard-tab-guardrails"
						className="data-[state=active]:bg-primary/12 data-[state=active]:text-primary gap-1.5 rounded-xl text-xs font-medium data-[state=active]:shadow-[inset_0_1px_0_rgba(255,255,255,0.16),0_2px_8px_-4px_rgba(34,211,196,0.35)]"
					>
					...
					<TabsTrigger
						value="agentic"
						data-testid="dashboard-tab-agentic"
						className="data-[state=active]:bg-primary/12 data-[state=active]:text-primary gap-1.5 rounded-xl text-xs font-medium data-[state=active]:shadow-[inset_0_1px_0_rgba(255,255,255,0.16),0_2px_8px_-4px_rgba(34,211,196,0.35)]"
					>
					...
					<TabsTrigger
						value="hallucination"
						data-testid="dashboard-tab-hallucination"
						className="data-[state=active]:bg-primary/12 data-[state=active]:text-primary gap-1.5 rounded-xl text-xs font-medium data-[state=active]:shadow-[inset_0_1px_0_rgba(255,255,255,0.16),0_2px_8px_-4px_rgba(34,211,196,0.35)]"
					>
					...
					<TabsTrigger
						...
						data-testid="dashboard-tab-provider-usage"
						className="data-[state=active]:bg-primary/12 data-[state=active]:text-primary gap-1.5 rounded-xl text-xs font-medium data-[state=active]:shadow-[inset_0_1px_0_rgba(255,255,255,0.16),0_2px_8px_-4px_rgba(34,211,196,0.35)]"
					>
					...
					<TabsTrigger
						value="rankings"
						data-testid="dashboard-tab-rankings"
						className="data-[state=active]:bg-primary/12 data-[state=active]:text-primary gap-1.5 rounded-xl text-xs font-medium data-[state=active]:shadow-[inset_0_1px_0_rgba(255,255,255,0.16),0_2px_8px_-4px_rgba(34,211,196,0.35)]"
					>
						<Bot className="h-3.5 w-3.5" />
						Models
					...
					<TabsTrigger
						value="load-balancer"
						data-testid="dashboard-tab-load-balancer"
						className="data-[state=active]:bg-primary/12 data-[state=active]:text-primary gap-1.5 rounded-xl text-xs font-medium data-[state=active]:shadow-[inset_0_1px_0_rgba(255,255,255,0.16),0_2px_8px_-4px_rgba(34,211,196,0.35)]"
					>
				...
				<TabsContent value="overview">
					<OverviewTab
						...
						guardrailLatencyData={guardrailLatencyData}
						loadingHistogram={loadingHistogram}
						loadingTokens={loadingTokens}
						loadingCost={loadingCost}
						loadingModels={loadingModels}
						loadingLatency={loadingLatency}
						loadingGuardrailLatency={loadingGuardrailLatency}
						...
						guardrailLatencyChartType={toChartType(urlState.guardrail_latency_chart)}
						...
						onGuardrailLatencyChartToggle={handleGuardrailLatencyChartToggle}
				...
				<TabsContent value="cache">
					<CacheTab
						...
						startTime={urlState.start_time}
						endTime={urlState.end_time}
						cacheRequestsChartType={toChartType(urlState.cache_requests_chart)}
						cacheSavingsChartType={toChartType(urlState.cache_savings_chart)}
						cacheHitRateChartType={toChartType(urlState.cache_hit_rate_chart)}
						cacheHitTypesChartType={toChartType(urlState.cache_hit_types_chart)}
						onCacheRequestsChartToggle={handleCacheRequestsChartToggle}
						onCacheSavingsChartToggle={handleCacheSavingsChartToggle}
						onCacheHitRateChartToggle={handleCacheHitRateChartToggle}
						onCacheHitTypesChartToggle={handleCacheHitTypesChartToggle}
					/>
				</TabsContent>
				...
				{/* LLM Load Balancer Tab */}
				<TabsContent value="load-balancer">
					<LoadBalancerTab
						filters={filters}
						isActive={activeTab === "load-balancer"}
						refreshKey={loadBalancerRefreshKey}
						startTime={urlState.start_time}
						endTime={urlState.end_time}
						volumeChartType={toChartType(urlState.lb_volume_chart)}
						costChartType={toChartType(urlState.lb_cost_chart)}
						latencyChartType={toChartType(urlState.lb_latency_chart)}
						volumeEngine={urlState.lb_volume_engine}
						costEngine={urlState.lb_cost_engine}
						latencyEngine={urlState.lb_latency_engine}
						onVolumeChartToggle={handleLbVolumeChartToggle}
						onCostChartToggle={handleLbCostChartToggle}
						onLatencyChartToggle={handleLbLatencyChartToggle}
						onVolumeEngineChange={handleLbVolumeEngineChange}
						onCostEngineChange={handleLbCostEngineChange}
						onLatencyEngineChange={handleLbLatencyEngineChange}
					/>
				</TabsContent>
				...
				<TabsContent value="guardrails">
					<GuardrailsTab startTime={urlState.start_time} endTime={urlState.end_time} />
				</TabsContent>

				<TabsContent value="agentic">
					<AgenticTab startTime={urlState.start_time} endTime={urlState.end_time} />
				</TabsContent>

				<TabsContent value="hallucination">
					<HallucinationTab startTime={urlState.start_time} endTime={urlState.end_time} />
				</TabsContent>
			</Tabs>
		</div>
	);
