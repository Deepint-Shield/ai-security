...
export default function CollapsibleBox({ title, children, collapsedHeight = 60, expandedMaxHeight = 450, onCopy }: CollapsibleBoxProps) {
	...
	return (
		<div className="w-full rounded-sm border">
			<div className="flex items-center justify-between border-b py-2 pl-6">
				...
				{onCopy && (
					<Button
						variant="ghost"
						size="sm"
						className="text-muted-foreground mx-2 h-6 py-1 hover:bg-transparent hover:text-black dark:hover:text-foreground"
						onClick={handleCopy}
					>
			...
			<div
				className={`${isExpanded ? "custom-scrollbar overflow-y-auto" : "overflow-hidden"} transition-all duration-200`}
