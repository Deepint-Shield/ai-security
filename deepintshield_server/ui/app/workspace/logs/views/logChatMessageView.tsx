	...
	if (block.text) {
		...
		return (
			<CollapsibleBox title={blockType} onCopy={() => block.text || ""} collapsedHeight={100}>
				<div className="custom-scrollbar max-h-[400px] overflow-y-auto px-6 py-3">
				...
				<>
					{isJson(message.reasoning) ? (
						...
						<CollapsibleBox title="Reasoning" onCopy={() => message.reasoning || ""} collapsedHeight={100}>
							<div className="custom-scrollbar max-h-[400px] overflow-y-auto px-6 py-3 italic">
								<MarkdownText content={message.reasoning} className="text-muted-foreground" />
							</div>
						</CollapsibleBox>
					)}
				</>
						...
						<>
							{isJson(message.content) ? (
								...
								<CollapsibleBox title="Content" onCopy={() => (message.content as string) || ""} collapsedHeight={100}>
									<div className="custom-scrollbar max-h-[400px] overflow-y-auto px-6 py-3">
										<MarkdownText content={message.content as string} />
									</div>
								</CollapsibleBox>
							)}
						</>
