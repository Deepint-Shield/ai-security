...
function isCacheHit(log: LogEntry) {
	return log.cache_debug?.cache_hit === true || (log.cache_savings ?? 0) > 0;
}

function getActualTotalTokens(log: LogEntry) {
	return isCacheHit(log) ? 0 : (log.token_usage?.total_tokens ?? 0);
}

export default function LogsPage() {
	// This component backs two routes: /workspace/logs (AI Logs - LLM requests)
	// and /workspace/agentic-logs (Agentic Logs - agent/tool PDP decisions,
	// object_type "agentic.*"). The route decides the scope; the agentic view
	// pins the objects filter so its list + KPIs cover only agentic activity.
	const pathname = usePathname();
	const isAgenticLogs = (pathname ?? "").includes("/workspace/agentic-logs");
	...
	const [urlState, setUrlState] = useQueryStates(
		{
			...
			guardrail_statuses: parseAsArrayOf(parseAsString).withDefault([]),
			guardrail_sources: parseAsArrayOf(parseAsString).withDefault([]),
	...
	const filters: LogFilters = useMemo(
		() => ({
			...
			// Agentic Logs route: scope to agentic activity. Respect a narrower
			// agentic Type filter (e.g. just agentic.tool) if the user set one,
			// else default to both agentic types. AI Logs route leaves objects
			// untouched (backend default-excludes agentic).
			objects: isAgenticLogs
				? urlState.objects.some((o) => o.startsWith("agentic."))
					? urlState.objects
					: ["agentic.tool", "agentic.agent"]
				: urlState.objects,
			...
			guardrail_statuses: urlState.guardrail_statuses,
			guardrail_sources: urlState.guardrail_sources,
		...
		[
			...
			urlState.guardrail_statuses,
			urlState.guardrail_sources,
	...
	const setFilters = useCallback(
		(newFilters: LogFilters) => {
			...
			setUrlState({
				...
				guardrail_statuses: newFilters.guardrail_statuses || [],
				guardrail_sources: newFilters.guardrail_sources || [],
	...
	// The active workspace ID drives the WS payload filter below - every log
	// broadcast is tenant-scoped on the server (NOT workspace-scoped) so an
	// API call in sibling-workspace "dev-ws" would otherwise flash into the
	// "prod-ws" live feed for a frame before the next refetch cleared it.
	...
	const handleLogMessage = useCallback((log: LogEntry, operation: "create" | "update") => {
		...
		// Workspace gate - the server broadcasts log events tenant-wide, so
		// drop any payload that belongs to a different workspace than the
		// sidebar's active selection. `workspace_id` on LogEntry may be
		// missing on legacy/pre-workspace rows; treat those as belonging to
		// the active workspace so historical data stays visible.
		const logWs = (log as LogEntry & { workspace_id?: string | null }).workspace_id;
			...
			if (!logExists) {
				...
				if (log.status == "success" || log.status == "error") {
					setStats((prevStats) => {
						...
						// Update total tokens
						newStats.total_tokens += getActualTotalTokens(log);
	...
	const fetchLogs = useCallback(async () => {
		...
		try {
			...
			setError("Cannot fetch logs. Please check if logs are enabled in your DeepIntShield configuration.");
			setLogs([]);
			setTotalItems(0);
	...
	// When the sidebar workspace switcher fires, clear local state IMMEDIATELY
	// - otherwise the previous workspace's rows linger in the table until the
	// next fetch returns. The fetch hooks below will re-populate from the
	// fresh /api/logs response (which is now correctly workspace-scoped at
	// the server). The active workspace is included in fetchLogs/fetchStats
	// dependency arrays so the queries re-run on switch.
	...
	useEffect(() => {
		...
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [filters, pagination, initialLoading, activeWorkspaceId]);
	...
	useEffect(() => {
		...
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [filters, initialLoading, activeWorkspaceId]);
	...
	const matchesFilters = (log: LogEntry, filters: LogFilters, applyTimeFilters = true): boolean => {
		...
		const actualTotalTokens = getActualTotalTokens(log);
		if (filters.min_tokens && actualTotalTokens < filters.min_tokens) {
			return false;
		}
		if (filters.max_tokens && actualTotalTokens > filters.max_tokens) {
			return false;
		}
			...
			},
			{
				...
				value: fetchingStats ? (
					...
					<div className="flex flex-col gap-0.5">
						...
						{(stats.total_prompt_cache_savings ?? 0) > 0 ? (
							<span className="text-emerald-600 dark:text-emerald-400 text-[10px] font-medium normal-case tracking-normal">
								saved ${(stats.total_prompt_cache_savings ?? 0).toFixed(4)} via prompt cache
							</span>
						) : null}
					</div>
				) : (
	...
	// Scope keeps the two routes' dynamic columns disjoint - see the
	// /api/logs/filterdata server handler for the LLM/agentic split.
	// Without the scope, AI Logs renders PDP-decision keys (decision_id,
	// identity_type, verdict, …) as permanently-empty columns and the
	// Agentic Logs page over-renders LLM-only metadata.
	const { data: filterData } = useGetAvailableFilterDataQuery({
		scope: isAgenticLogs ? "agentic" : "llm",
	});
	const metadataKeys = useMemo(() => {
		if (!filterData?.metadata_keys) return [];
		return Object.keys(filterData.metadata_keys)
			.filter((key) => key !== "latency_breakdown_ms")
			.sort();
	}, [filterData?.metadata_keys]);

	const virtualKeysById = useMemo<Record<string, VirtualKey>>(() => {
		return Object.fromEntries((filterData?.virtual_keys ?? []).map((virtualKey) => [virtualKey.id, virtualKey]));
	}, [filterData?.virtual_keys]);

	const columns = useMemo(
		() => createColumns(handleDelete, hasDeleteAccess, metadataKeys, virtualKeysById, isAgenticLogs),
		[handleDelete, hasDeleteAccess, metadataKeys, virtualKeysById, isAgenticLogs],
	);
