...
export default function VirtualKeyDetailSheet({ virtualKey, onClose }: VirtualKeyDetailSheetProps) {
	const getEntityInfo = () => {
		...
		if (virtualKey.customer) {
			return { type: "Member", name: virtualKey.customer.name };
		}
					...
					<div className="space-y-4">
						...
						<div className="grid gap-4">
							...
							<div className="grid grid-cols-3 items-start gap-4">
								<span className="text-muted-foreground text-sm">Guardrail Policies</span>
								<div className="col-span-2 flex flex-wrap gap-2">
									{virtualKey.guardrail_policies && virtualKey.guardrail_policies.length > 0 ? (
										virtualKey.guardrail_policies.map((policy) => (
											<Badge key={policy.id} variant="outline">
												{policy.name}
											</Badge>
										))
									) : (
										<span className="text-muted-foreground text-sm">No explicit guardrail policies attached to this key.</span>
									)}
								</div>
							</div>
						</div>
