...
import { CheckCircle2, ChevronLeft, ChevronRight, CircleSlash, Edit, Search, Trash2 } from "lucide-react";
				...
				<Table>
					<TableHeader className="bg-muted/30">
						<TableRow className="hover:bg-transparent">
							...
							<TableHead className="text-muted-foreground text-[10px] font-semibold uppercase tracking-[0.16em]">Targets</TableHead>
							<TableHead className="text-muted-foreground text-[10px] font-semibold uppercase tracking-[0.16em]">Scope</TableHead>
							<TableHead className="text-muted-foreground text-right text-[10px] font-semibold uppercase tracking-[0.16em]">Priority</TableHead>
							<TableHead className="text-muted-foreground text-[10px] font-semibold uppercase tracking-[0.16em]">Expression</TableHead>
							<TableHead className="text-muted-foreground text-[10px] font-semibold uppercase tracking-[0.16em]">Status</TableHead>
							<TableHead className="text-muted-foreground text-right text-[10px] font-semibold uppercase tracking-[0.16em]">Actions</TableHead>
						</TableRow>
					</TableHeader>
					<TableBody>
							...
							<TableRow className="hover:bg-transparent">
								<TableCell colSpan={7} className="py-12 text-center">
									<div className="flex flex-col items-center gap-1.5">
										...
										<p className="text-foreground text-sm font-semibold">No matching routing rules</p>
						...
						sortedRules.map((rule) => (
							<TableRow key={rule.id} className="border-border/40 hover:bg-primary/5 cursor-pointer transition-colors">
								<TableCell className="font-medium">
									<div className="flex flex-col gap-0.5">
										<span className="truncate max-w-xs">{rule.name}</span>
										{rule.description && (
											<span data-testid="routing-rule-description" className="text-muted-foreground truncate max-w-xs text-xs">{rule.description}</span>
										)}
									</div>
								</TableCell>
								<TableCell>
								...
								<TableCell>
									<Badge variant="outline" className="text-[10px] font-medium uppercase tracking-wider">{getScopeLabel(rule.scope)}</Badge>
								</TableCell>
								<TableCell className="text-right">
									<div className={`inline-flex items-center justify-center rounded-md px-2.5 py-1 font-mono text-xs font-semibold tabular-nums ${getPriorityBadgeClass(rule.priority)}`}>
										{rule.priority}
									</div>
								</TableCell>
								<TableCell>
									<span className="bg-muted/40 text-muted-foreground inline-block max-w-xs truncate rounded px-2 py-0.5 font-mono text-[11px]" title={rule.cel_expression}>
										{truncateCELExpression(rule.cel_expression) || "-"}
									</span>
								</TableCell>
								<TableCell>
									{rule.enabled ? (
										<span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-500/15 px-2 py-0.5 text-[10px] font-semibold text-emerald-700 dark:text-emerald-400">
											<CheckCircle2 className="h-3 w-3" />
											Enabled
										</span>
									) : (
										<span className="bg-muted text-muted-foreground inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-[10px] font-semibold">
	...
	return (
		<div className="flex flex-col gap-0.5">
			<div className="flex items-center gap-2">
				{first.provider ? (
					<span className="bg-[linear-gradient(135deg,rgba(34,211,196,0.18),rgba(96,169,255,0.14))] inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-lg shadow-[inset_0_1px_0_rgba(255,255,255,0.18)]">
						<RenderProviderIcon
					...
					<span className="bg-muted/60 text-muted-foreground inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-[10px] font-semibold uppercase tracking-wider">
						Any
					</span>
				)}
				<span className="text-sm truncate max-w-[160px]">{label}</span>
				{targets.length === 1 && (
					<span className="bg-muted text-muted-foreground inline-flex shrink-0 rounded-full px-1.5 py-0 font-mono text-[10px] font-medium tabular-nums">
						{first.weight}
					</span>
				)}
			</div>
			{targets.length > 1 && (
				<span className="text-muted-foreground ml-9 text-xs">+{targets.length - 1} more target{targets.length > 2 ? "s" : ""}</span>
			)}
		</div>
	);
}
