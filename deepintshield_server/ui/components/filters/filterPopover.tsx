...
import { GuardrailStatusLabels, RequestTypeLabels, RequestTypes, RoutingEngineUsedLabels, Statuses } from "@/lib/constants/logs";
...
export function FilterPopover({ filters, onFilterChange, onMetadataFilterChange, showMissingCost }: FilterPopoverProps) {
	...
	// Scope the Type options to the route: the Agentic Logs view offers only
	// agentic types (Tool/Agent); AI Logs offers only LLM request types.
	const isAgenticLogs = (usePathname() ?? "").includes("/workspace/agentic-logs");
	...
	const availableGuardrailStatuses = filterData?.guardrail_statuses || [];
	...
	const FILTER_OPTIONS: Record<string, string[]> = {
		...
		Type: isAgenticLogs
			? RequestTypes.filter((t) => t.startsWith("agentic."))
			: RequestTypes.filter((t) => !t.startsWith("agentic.")),
		...
		Guardrail: filterDataLoading ? [] : availableGuardrailStatuses,
		// "Guardrail Engine" surfaces the same partition the new column shows -
		// AI Model (deepintshield_models wrapper) vs Policy (regex/card) vs
		// Mixed (both fired). Hard-coded since the set is closed; no need to
		// fetch from the API like statuses.
		"Guardrail Engine": ["ai_model", "policy", "mixed"],
	};

	// Add dynamic metadata categories. High-cardinality / per-row identifier
	// keys (e.g. agentic decision_id, args_digest) are useless as filter facets
	// - one value per row - so they're excluded from the filter list (they
	// remain available as columns and in the log detail view).
	const HIDDEN_METADATA_FILTER_KEYS = new Set(["decision_id", "args_digest"]);
	for (const [metadataKey, values] of Object.entries(availableMetadataKeys)) {
		if (HIDDEN_METADATA_FILTER_KEYS.has(metadataKey)) continue;
		FILTER_OPTIONS[`Metadata: ${metadataKey}`] = values;
	}

	const isCategoryLoading = (category: string) =>
		(category === "Providers" && providersLoading) ||
		(category !== "Status" && category !== "Type" && category !== "Providers" && category !== "Guardrail Engine" && !category.startsWith("Metadata: ") && filterDataLoading);
	...
	const filterKeyMap: Record<string, keyof LogFiltersType> = {
		...
		Guardrail: "guardrail_statuses",
		"Guardrail Engine": "guardrail_sources",
	};
												...
												>
													...
													<span className={cn(category === "Status" && "lowercase")}>
														{category === "Type" ? RequestTypeLabels[value as keyof typeof RequestTypeLabels] :
															category === "Routing Engines" ? (RoutingEngineUsedLabels[value as keyof typeof RoutingEngineUsedLabels] ?? value) :
															category === "Guardrail" ? (GuardrailStatusLabels[value] ?? value) :
															category === "Guardrail Engine" ? (({ai_model: "AI Model", policy: "Policy", mixed: "Mixed"} as Record<string, string>)[value] ?? value) : value}
													</span>
												</CommandItem>
											);
										})
