...
const AUTO_API_KEY_ID = "__auto__";

function normalizePromptRepositoryApiKeyId(apiKeyId?: string): string {
	if (!apiKeyId || apiKeyId.startsWith("sk-bf-")) {
		return AUTO_API_KEY_ID;
	}
	return apiKeyId;
}
...
export function PromptProvider({ children }: { children: ReactNode }) {
	...
	// API queries - prompts are scoped to the active workspace (org-wide
	// prompts plus prompts owned by the active workspace) so a workspace
	// switch in the sidebar narrows the prompt list automatically.
	...
	const [apiKeyId, setApiKeyId] = useState(AUTO_API_KEY_ID);
	const [isStreaming, setIsStreaming] = useState(false);
	const activeRunRef = useRef<symbol | null>(null);
	const [variables, setVariables] = useState<VariableMap>({});

	// Resolve an active virtual key to authorize Playground runs - governance
	// requires a VK (x-bf-vk header) on every inference call. Revalidate on mount
	// so a hard load doesn't serve a stale/empty cross-session cache snapshot.
	const { data: vkData } = useGetVirtualKeysQuery({}, { refetchOnMountOrArgChange: true });
	const activeVirtualKey = useMemo(() => {
		const vks = vkData?.virtual_keys ?? [];
		return (vks.find((k) => k.is_active) ?? vks[0])?.value ?? "";
	}, [vkData]);
	...
	useEffect(() => {
		...
		const loadFromParams = (params: ModelParams, prov: string, mod: string) => {
			...
			setApiKeyId(normalizePromptRepositoryApiKeyId(api_key_id));
			setProvider(prov || "");
			setModel(mod || "");
		};
	...
	const diffAgainst = useCallback(
		(ref: { messages?: any[]; model_params?: ModelParams; provider?: string; model?: string } | undefined) => {
			...
			const currentApiKeyId = normalizePromptRepositoryApiKeyId(apiKeyId);
			const normalizedRefApiKeyId = normalizePromptRepositoryApiKeyId(refApiKeyId);
			if ((currentApiKeyId !== AUTO_API_KEY_ID ? currentApiKeyId : undefined) !== (normalizedRefApiKeyId !== AUTO_API_KEY_ID ? normalizedRefApiKeyId : undefined)) return true;
	...
	const handleSendMessage = useCallback(
		async (pendingMessage?: Message) => {
			...
			setIsStreaming(true);
			await executePrompt(messages, pendingMessage, { provider, model, modelParams, apiKeyId, variables, virtualKey: activeVirtualKey }, {
		...
		[messages, provider, model, modelParams, apiKeyId, variables, activeVirtualKey],
	);
	...
	const handleSubmitToolResult = useCallback(
		async (afterIndex: number, toolCallId: string, content: string) => {
			...
			setIsStreaming(true);
			await executePrompt(newMessages, undefined, { provider, model, modelParams, apiKeyId, variables, virtualKey: activeVirtualKey }, {
				onStreamingStart: (allMessages, placeholder) => {
					if (!isActive()) return;
					setMessages([...allMessages, placeholder]);
				},
