		...
		<div ref={containerRef}>
			<AsyncCreatableSelect
				...
				classNames={{
					...
					control: ({ isFocused }) =>
						cn(
							"multiselect-control !border-border/70 !bg-background/70 flex flex-wrap items-start justify-between rounded-xl border shadow-[0_14px_28px_-24px_rgba(7,24,30,0.34)] backdrop-blur-md transition-colors",
							isFocused && "!border-primary/55 ring-primary/15 ring-2",
							props.triggerClassName,
						),
					placeholder: () => "truncate p-0 text-sm text-ellipsis text-muted-foreground/85",
					group: () => cn(props.groupClassName),
					input: () => "m-0 border-none p-0 text-sm !text-foreground",
					menu: () =>
						cn(
							"border-border/70 !bg-card/92 overflow-hidden rounded-[1.1rem] border p-0 shadow-[0_22px_44px_-24px_rgba(7,24,30,0.38)] backdrop-blur-xl",
							props.menuClassName,
						),
					...
					option: ({ isFocused }) =>
						cn(
							"multiselect-option text-foreground flex w-full justify-between rounded-lg border border-transparent px-2.5 py-2 text-sm transition-colors",
							isFocused && "border-primary/20 bg-primary/10",
						),
					singleValue: () => "text-sm text-foreground",
					multiValue: () =>
						"flex cursor-pointer items-center gap-1 rounded-full border border-primary/20 bg-primary/10 px-2 py-1 text-sm text-primary",
					multiValueLabel: () => "text-current",
					multiValueRemove: () => "h-inherit flex items-center text-primary/70 opacity-75 hover:cursor-pointer hover:opacity-100",
