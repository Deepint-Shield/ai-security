			...
			<div className={cn("relative h-full", className)} ref={containerRef}>
				...
				<pre
					...
					className={cn(
						...
						"no-scrollbar text-muted-foreground pointer-events-none absolute inset-0 m-0 block w-full bg-transparent font-[inherit]",
					)}
	...
	return createPortal(
		...
		>
			<CustomDropdown
				...
				className={cn(
					"custom-scrollbar border-border/70 bg-card/92 max-h-full min-w-[220px] rounded-[1.1rem] border p-1 shadow-[0_20px_42px_-24px_rgba(7,24,30,0.38)] backdrop-blur-xl",
					suggestionDropdownClassName,
				)}
