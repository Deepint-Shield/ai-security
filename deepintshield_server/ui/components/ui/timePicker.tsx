...
export const TimePicker = React.forwardRef<HTMLDivElement, TimePickerProps>((props, forwardedRef) => {
	...
	// Trigger styling: tighter horizontal padding (px-2.5 instead of the
	// default px-4) plus a gap reduction so the value + chevron actually
	// fit in a narrow time-cell. Default SelectTrigger reserves ~32px
	// for padding + 16px chevron = 48px overhead, which clipped "09"
	// and rendered "AM" as "Al" inside the previous 70px width. Each
	// trigger is now wide enough for its real content with a hairline
	// of visual breathing room.
	const triggerClass =
		"!px-2.5 !gap-1.5 h-9 [&_svg]:size-3.5 font-medium tabular-nums justify-center";

	return (
		<div
			ref={forwardedRef}
			className={cn(
				"inline-flex h-9 w-full items-center gap-1.5",
				className,
			)}
		>
			<Select value={hour12.toString()} onValueChange={handleHourChange}>
				<SelectTrigger size="sm" className={cn(triggerClass, "w-[68px]")} aria-label="Hour">
			...
			<Select value={minute.toString()} onValueChange={handleMinuteChange}>
				<SelectTrigger size="sm" className={cn(triggerClass, "w-[68px]")} aria-label="Minute">
			...
			<Select value={period} onValueChange={(v) => handlePeriodChange(v as "AM" | "PM")}>
				<SelectTrigger size="sm" className={cn(triggerClass, "w-[72px]")} aria-label="AM/PM">
