...
export function LoggingDisabledView() {
  ...
  const handleEnable = useCallback(async () => {
    if (!deepintshieldConfig?.client_config) {
    ...
    try {
      await updateCoreConfig({
        ...deepintshieldConfig,
        client_config: { ...deepintshieldConfig.client_config, enable_logging: true },
      }).unwrap()
      toast.success("Logging enabled.")
    } catch (error) {
      toast.error(getErrorMessage(error))
    }
  }, [deepintshieldConfig, updateCoreConfig])

  return (
    <div
      className={cn(
        "workspace-page-shell flex min-h-[80vh] flex-col items-center justify-center gap-4 text-center"
      )}
    >
