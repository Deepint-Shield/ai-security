...
func validateRequest(req *schemas.DeepIntShieldRequest) *schemas.DeepIntShieldError {
	if req == nil {
		return newDeepIntShieldErrorFromMsg("deepintshield request cannot be nil")
	}
	provider, model, _ := req.GetRequestFields()
	if provider == "" {
		return newDeepIntShieldErrorFromMsg("provider is required")
	}
	if isModelRequired(req.RequestType) && model == "" {
		return newDeepIntShieldErrorFromMsg("model is required")
	}
	return nil
}
...
func newDeepIntShieldError(err error) *schemas.DeepIntShieldError {
...
// newDeepIntShieldErrorFromMsg creates a DeepIntShieldError with a custom message.
// This helper function is used for static error messages.
func newDeepIntShieldErrorFromMsg(message string) *schemas.DeepIntShieldError {
	return &schemas.DeepIntShieldError{
		IsDeepIntShieldError: false,
		Error: &schemas.ErrorField{
			Message: message,
...
// newDeepIntShieldMessageChan creates a channel that sends a deepintshield response.
// It is used to send a deepintshield response to the client.
func newDeepIntShieldMessageChan(message *schemas.DeepIntShieldResponse) chan *schemas.DeepIntShieldStreamChunk {
	ch := make(chan *schemas.DeepIntShieldStreamChunk)

	go func() {
		defer close(ch)
		ch <- &schemas.DeepIntShieldStreamChunk{
			DeepIntShieldTextCompletionResponse:      message.TextCompletionResponse,
			DeepIntShieldChatResponse:                message.ChatResponse,
			DeepIntShieldResponsesStreamResponse:     message.ResponsesStreamResponse,
			DeepIntShieldSpeechStreamResponse:        message.SpeechStreamResponse,
			DeepIntShieldTranscriptionStreamResponse: message.TranscriptionStreamResponse,
		}
	}()
...
func clearCtxForFallback(ctx *schemas.DeepIntShieldContext) {
	ctx.ClearValue(schemas.DeepIntShieldContextKeyAPIKeyID)
	ctx.ClearValue(schemas.DeepIntShieldContextKeyAPIKeyName)
}
...
func GetTracerFromContext(ctx *schemas.DeepIntShieldContext) (schemas.Tracer, string, error) {
...
func IsFinalChunk(ctx *schemas.DeepIntShieldContext) bool {
	if ctx == nil {
		return false
	}

	isStreamEndIndicator := ctx.Value(schemas.DeepIntShieldContextKeyStreamEndIndicator)
...
func GetResponseFields(result *schemas.DeepIntShieldResponse, err *schemas.DeepIntShieldError) (requestType schemas.RequestType, provider schemas.ModelProvider, model string) {
	if result != nil {
