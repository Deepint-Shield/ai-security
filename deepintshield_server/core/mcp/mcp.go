...
const (
	...
	DeepIntShieldMCPVersion             = "1.0.0"               // Version identifier for DeepIntShield
	DeepIntShieldMCPClientName          = "DeepIntShieldClient" // Name for internal DeepIntShield MCP client
	DeepIntShieldMCPClientKey           = "deepintshieldInternal"     // Key for internal DeepIntShield client in clientMap
	MCPLogPrefix                        = "[DeepIntShield MCP]" // Consistent logging prefix
	...
	MCPContextKeyIncludeClients schemas.DeepIntShieldContextKey = "mcp-include-clients" // Context key for whitelist client filtering
	MCPContextKeyIncludeTools   schemas.DeepIntShieldContextKey = "mcp-include-tools"   // Context key for whitelist tool filtering (Note: toolName should be in "clientName-toolName" format for individual tools, or "clientName-*" for wildcard)
)
...
// MCPManager manages MCP integration for DeepIntShield core.
// It provides a bridge between DeepIntShield and various MCP servers, supporting
...
//   - req: The DeepIntShield request to add tools to
//
// Returns:
//   - *schemas.DeepIntShieldRequest: The request with tools added
func (m *MCPManager) AddToolsToRequest(ctx context.Context, req *schemas.DeepIntShieldRequest) *schemas.DeepIntShieldRequest {
	// Phase 22.6 early-exit: when the resolving virtual key carries
	// no MCP configs, attaching tools is guaranteed to be a no-op -
	// the toolsManager would walk an empty client list and return
	// the request unchanged. Short-circuiting here saves a slice
	// allocation + a context-value lookup + a manager mutex acquire
	// on every request from VKs that don't use MCP. The hint is
	// stamped by the VK auth middleware on dashboard / inference
	// requests; fall through (false) when absent so SDK / CLI
	// callers without the hint still get the full path.
	if hint, ok := ctx.Value(schemas.DeepIntShieldContextKey("__bf_vk_has_mcp")).(bool); ok && !hint {
		return req
	}
...
//
// Returns:
//   - *schemas.DeepIntShieldMCPResponse: The result response containing tool execution output (ChatMessage or ResponsesMessage)
//   - error: Any error that occurred during tool execution
func (m *MCPManager) ExecuteToolCall(ctx *schemas.DeepIntShieldContext, request *schemas.DeepIntShieldMCPRequest) (*schemas.DeepIntShieldMCPResponse, error) {
...
//
// Returns:
//   - *schemas.DeepIntShieldChatResponse: The final response after agent execution (or original if no tool calls)
...
//
// Returns:
//   - *schemas.DeepIntShieldResponsesResponse: The final response after agent execution (or original if no tool calls)
//   - *schemas.DeepIntShieldError: Any error that occurred during agent execution
