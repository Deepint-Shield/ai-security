...
import (
	...
	codemcp "github.com/deepint-shield/ai-security/core/mcp"
...
func (s *StarlarkCodeMode) callMCPTool(ctx context.Context, clientName, toolName string, args map[string]interface{}, appendLog func(string)) (interface{}, error) {
	...
	// Get DeepIntShieldContext for plugin pipeline
	var deepintshieldCtx *schemas.DeepIntShieldContext
	var ok bool
	if deepintshieldCtx, ok = ctx.(*schemas.DeepIntShieldContext); !ok {
		return s.callMCPToolDirect(ctx, client, originalToolName, clientName, toolName, args, appendLog)
	}

	originalRequestID, _ := deepintshieldCtx.Value(schemas.DeepIntShieldContextKeyRequestID).(string)

	// Generate new request ID for this nested tool call
	var newRequestID string
	if s.fetchNewRequestIDFunc != nil {
		newRequestID = s.fetchNewRequestIDFunc(deepintshieldCtx)
	} else {
		newRequestID = fmt.Sprintf("exec_%d_%s", time.Now().UnixNano(), toolName)
	}

	// Create new child context
	deadline, hasDeadline := deepintshieldCtx.Deadline()
	if !hasDeadline {
		deadline = schemas.NoDeadline
	}
	nestedCtx := schemas.NewDeepIntShieldContext(deepintshieldCtx, deadline)
	nestedCtx.SetValue(schemas.DeepIntShieldContextKeyRequestID, newRequestID)
	if originalRequestID != "" {
		nestedCtx.SetValue(schemas.DeepIntShieldContextKeyParentMCPRequestID, originalRequestID)
	}
	...
	// Create DeepIntShieldMCPRequest
	...
	if callErr != nil {
		...
		if after, ok := strings.CutPrefix(rawResult, "Error: "); ok {
			...
			mcpResp = &schemas.DeepIntShieldMCPResponse{
	...
	// Run post-hooks
	finalResp, finalErr := pipeline.RunMCPPostHooks(nestedCtx, mcpResp, deepintshieldErr, preCount)
