...
func retrieveExternalTools(ctx context.Context, client *client.Client, clientName string, logger schemas.Logger) (map[string]schemas.ChatTool, map[string]string, error) {
	...
	for _, mcpTool := range toolsResponse.Tools {
		...
		// Convert MCP tool schema to DeepIntShield format
		deepintshieldTool := convertMCPToolToDeepIntShieldSchema(&mcpTool, logger)
		...
		if deepintshieldTool.Function != nil {
			deepintshieldTool.Function.Name = prefixedToolName
		}
		// Store the tool with the prefixed name
		tools[prefixedToolName] = deepintshieldTool
...
// convertMCPToolToDeepIntShieldSchema converts an MCP tool definition to DeepIntShield format.
func convertMCPToolToDeepIntShieldSchema(mcpTool *mcp.Tool, logger schemas.Logger) schemas.ChatTool {
...
func hasToolCallsForChatResponse(response *schemas.DeepIntShieldChatResponse) bool {
...
func hasToolCallsForResponsesResponse(response *schemas.DeepIntShieldResponsesResponse) bool {
