...
func (m *MCPManager) RegisterTool(name, description string, toolFunction MCPToolFunction[any], toolSchema schemas.ChatTool) error {
	...
	internalClient, ok := m.clientMap[DeepIntShieldMCPClientKey]
	if !ok {
		return fmt.Errorf("deepintshield client not found")
	}

	// Create prefixed tool name for consistency with external tools
	// Format: deepintshieldInternal-toolName
	prefixedToolName := fmt.Sprintf("%s-%s", DeepIntShieldMCPClientKey, name)
	...
	m.logger.Debug("%s Registering typed tool: %s -> prefixed as %s (client: %s)", MCPLogPrefix, name, prefixedToolName, DeepIntShieldMCPClientKey)
...
func (m *MCPManager) connectToMCPClient(config *schemas.MCPClientConfig) error {
	...
	extInitRequest := mcp.InitializeRequest{
		Params: mcp.InitializeParams{
			...
			ClientInfo: mcp.Implementation{
				Name:    fmt.Sprintf("DeepIntShield-%s", config.Name),
				Version: "1.0.0",
			},
		},
	}
	...
	// Start tool syncing for the client (skip for internal deepintshield client)
	if config.ID != DeepIntShieldMCPClientKey {
...
func (m *MCPManager) setupLocalHost() error {
	...
	m.server = server
	m.clientMap[DeepIntShieldMCPClientKey] = client
	m.mu.Unlock()
...
func (m *MCPManager) createLocalMCPClient() (*schemas.MCPClientState, error) {
	...
	return &schemas.MCPClientState{
		ExecutionConfig: &schemas.MCPClientConfig{
			ID:             DeepIntShieldMCPClientKey,
			Name:           DeepIntShieldMCPClientKey, // Use same value as ID for consistent prefixing
...
func (m *MCPManager) startLocalMCPServer() error {
	...
	clientEntry, ok := m.clientMap[DeepIntShieldMCPClientKey]
	if !ok {
		return fmt.Errorf("deepintshield client not found")
	}
	...
	initRequest := mcp.InitializeRequest{
		Params: mcp.InitializeParams{
			...
			Capabilities:    mcp.ClientCapabilities{},
			ClientInfo: mcp.Implementation{
				Name:    DeepIntShieldMCPClientName,
				Version: DeepIntShieldMCPVersion,
			},
		},
	}
