...
// This allows ToolsManager to run plugin hooks without direct dependency on DeepIntShield
type PluginPipeline interface {
	RunMCPPreHooks(ctx *schemas.DeepIntShieldContext, req *schemas.DeepIntShieldMCPRequest) (*schemas.DeepIntShieldMCPRequest, *schemas.MCPPluginShortCircuit, int)
	RunMCPPostHooks(ctx *schemas.DeepIntShieldContext, mcpResp *schemas.DeepIntShieldMCPResponse, deepintshieldErr *schemas.DeepIntShieldError, runFrom int) (*schemas.DeepIntShieldMCPResponse, *schemas.DeepIntShieldError)
}
...
type ToolsManager struct {
	...
	// This id is attached to ctx.Value(schemas.DeepIntShieldContextKeyRequestID) in the agent mode.
...
func buildIntegrationDuplicateCheckMap(existingTools []schemas.ChatTool, integrationUserAgent string) map[string]bool {
	...
	case "claude-cli":
		...
		// Examples:
		//   mcp__deepintshield__executeToolCode -> executeToolCode
		//   mcp__deepintshield__listToolFiles -> listToolFiles
		//   mcp__deepintshield__readToolFile -> readToolFile
...
// ParseAndAddToolsToRequest parses the available tools per client and adds them to the DeepIntShield request.
//
// Parameters:
//   - ctx: Execution context
//   - req: DeepIntShield request
//   - availableToolsPerClient: Map of client name to its available tools
//
// Returns:
//   - *schemas.DeepIntShieldRequest: DeepIntShield request with MCP tools added
func (m *ToolsManager) ParseAndAddToolsToRequest(ctx context.Context, req *schemas.DeepIntShieldRequest) *schemas.DeepIntShieldRequest {
	...
	integrationUserAgent := ctx.Value(schemas.DeepIntShieldContextKeyUserAgent)
...
//
// Returns:
//   - *schemas.DeepIntShieldMCPResponse: Tool execution result (Chat or Responses format)
//   - error: Any execution error
func (m *ToolsManager) ExecuteTool(ctx *schemas.DeepIntShieldContext, request *schemas.DeepIntShieldMCPRequest) (*schemas.DeepIntShieldMCPResponse, error) {
	...
	extraFields := schemas.DeepIntShieldMCPResponseExtraFields{
	...
	switch request.RequestType {
	case schemas.MCPRequestTypeChatToolCall:
		return &schemas.DeepIntShieldMCPResponse{
...
func (m *ToolsManager) executeToolInternal(ctx *schemas.DeepIntShieldContext, toolCall *schemas.ChatAssistantMessageToolCall) (*schemas.ChatMessage, string, string, error) {
	...
	if !isToolPermittedForExecution(ctx, client, toolName, m.logger) {
...
func isToolPermittedForExecution(ctx context.Context, client *schemas.MCPClientState, toolName string, logger schemas.Logger) bool {
	...
	if !shouldIncludeClient(client.ExecutionConfig.Name, includeClients, logger) {
		return false
	}
	if shouldSkipToolForConfig(toolName, client.ExecutionConfig) {
		return false
	}
	if shouldSkipToolForRequest(ctx, client.ExecutionConfig.Name, toolName) {
		return false
	}
...
func (m *ToolsManager) ExecuteAgentForChatRequest(
	...
	resp *schemas.DeepIntShieldChatResponse,
...
func (m *ToolsManager) ExecuteAgentForResponsesRequest(
	...
	resp *schemas.DeepIntShieldResponsesResponse,
