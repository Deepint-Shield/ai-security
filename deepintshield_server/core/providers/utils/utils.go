...
func MakeRequestWithContext(ctx context.Context, client *fasthttp.Client, req *fasthttp.Request, resp *fasthttp.Response) (time.Duration, *schemas.DeepIntShieldError, func()) {
	startTime := time.Now()
	errChan := make(chan error, 1)

	// Per-tenant network config (PER_TENANT_NETWORK_CONFIG): when the worker has
	// stamped a per-(tenant,provider) client and/or the tenant's request timeout
	// on the context, honor them here so the change applies per request without
	// rebuilding the shared provider. Both are strict no-ops when absent (flag
	// off, or no tenant override) - client.Do on the shared client, as before.
	client = ClientFromContext(ctx, client)
	reqTimeout, hasReqTimeout := RequestTimeoutFromContext(ctx)

	go func() {
		// client.Do/DoTimeout is a blocking call.
		// It will send an error (or nil for success) to errChan when it completes.
		if hasReqTimeout {
			errChan <- client.DoTimeout(req, resp, reqTimeout)
		} else {
	...
	case <-ctx.Done():
		...
		return latency, &schemas.DeepIntShieldError{
	...
	case err := <-errChan:
		...
		if err != nil {
			...
			if errors.Is(err, fasthttp.ErrTimeout) || errors.Is(err, context.DeadlineExceeded) {
				return latency, NewDeepIntShieldOperationError(schemas.ErrProviderRequestTimedOut, err, ""), noop
			}
...
func GetPathFromContext(ctx context.Context, defaultPath string) string {
	if pathInContext, ok := ctx.Value(schemas.DeepIntShieldContextKeyURLPath).(string); ok {
...
func CheckAndGetRawRequestBody(ctx context.Context, request RequestBodyGetter) ([]byte, bool) {
	if rawBody, ok := ctx.Value(schemas.DeepIntShieldContextKeyUseRawRequestBody).(bool); ok && rawBody {
...
func IsLargePayloadPassthroughEnabled(ctx context.Context) bool {
	...
	isLargePayload, ok := ctx.Value(schemas.DeepIntShieldContextKeyLargePayloadMode).(bool)
	if !ok || !isLargePayload {
		return false
	}
	reader, ok := ctx.Value(schemas.DeepIntShieldContextKeyLargePayloadReader).(io.Reader)
...
func ApplyLargePayloadRequestBodyWithModelNormalization(
	...
	bodyReader, _ := ctx.Value(schemas.DeepIntShieldContextKeyLargePayloadReader).(io.Reader)
	bodySize := -1
	if contentLength, ok := ctx.Value(schemas.DeepIntShieldContextKeyLargePayloadContentLength).(int); ok {
		bodySize = contentLength
	}

	if contentType, ok := ctx.Value(schemas.DeepIntShieldContextKeyLargePayloadContentType).(string); ok && contentType != "" {
		ctLower := strings.ToLower(contentType)
		if metadata, ok := ctx.Value(schemas.DeepIntShieldContextKeyLargePayloadMetadata).(*schemas.LargePayloadMetadata); ok && metadata != nil {
...
func CheckContextAndGetRequestBody(ctx context.Context, request RequestBodyGetter, requestConverter RequestBodyConverter, providerType schemas.ModelProvider) ([]byte, *schemas.DeepIntShieldError) {
	if IsLargePayloadPassthroughEnabled(ctx) {
		return nil, nil
	}
	...
	if !ok {
		...
		if err != nil {
			return nil, NewDeepIntShieldOperationError(schemas.ErrRequestBodyConversion, err, providerType)
		}
		if convertedBody == nil {
			return nil, NewDeepIntShieldOperationError("request body is not provided", nil, providerType)
		}

		jsonBody, err := sonic.MarshalIndent(convertedBody, "", "  ")
		if err != nil {
			return nil, NewDeepIntShieldOperationError(schemas.ErrProviderRequestMarshal, err, providerType)
		}
		// Merge ExtraParams into the JSON if passthrough is enabled
		if ctx.Value(schemas.DeepIntShieldContextKeyPassthroughExtraParams) != nil && ctx.Value(schemas.DeepIntShieldContextKeyPassthroughExtraParams) == true {
...
// It attempts to unmarshal the error response and returns a DeepIntShieldError
...
func HandleProviderAPIError(resp *fasthttp.Response, errorResp any) *schemas.DeepIntShieldError {
...
// EnrichError attaches the raw request and response to a DeepIntShieldError.
// Returns the request and response from provider embedded in DeepIntShieldError.ExtraFields.
func EnrichError(
	...
	if ShouldSendBackRawRequest(ctx, sendBackRawRequest) && len(requestBody) > 0 {
		...
		deepintshieldErr.ExtraFields.RawRequest = compactRawJSON(requestBody)
	} else {
		deepintshieldErr.ExtraFields.RawRequest = nil
	}

	if ShouldSendBackRawResponse(ctx, sendBackRawResponse) {
		if len(responseBody) > 0 {
			deepintshieldErr.ExtraFields.RawResponse = compactRawJSON(responseBody)
		}
	} else {
		deepintshieldErr.ExtraFields.RawResponse = nil
	}
...
// and returns either the parsed response or a DeepIntShieldError if parsing fails.
...
func HandleProviderResponse[T any](responseBody []byte, response *T, requestBody []byte, sendBackRawRequest bool, sendBackRawResponse bool) (rawRequest interface{}, rawResponse interface{}, deepintshieldErr *schemas.DeepIntShieldError) {
	// Check for empty response
	trimmed := strings.TrimSpace(string(responseBody))
	if len(trimmed) == 0 {
		return nil, nil, &schemas.DeepIntShieldError{
			IsDeepIntShieldError: true,
			Error: &schemas.ErrorField{
...
func ParseAndSetRawRequest(extraFields *schemas.DeepIntShieldResponseExtraFields, jsonBody []byte) {
	if len(jsonBody) > 0 {
...
func ParseAndSetRawRequestIfJSON(fasthttpReq *fasthttp.Request, extraFields *schemas.DeepIntShieldResponseExtraFields) {
...
func NewUnsupportedOperationError(requestType schemas.RequestType, providerName schemas.ModelProvider) *schemas.DeepIntShieldError {
	return &schemas.DeepIntShieldError{
		IsDeepIntShieldError: false,
		Error: &schemas.ErrorField{
...
func CheckOperationAllowed(defaultProvider schemas.ModelProvider, config *schemas.CustomProviderConfig, operation schemas.RequestType) *schemas.DeepIntShieldError {
...
func NewConfigurationError(message string, providerType schemas.ModelProvider) *schemas.DeepIntShieldError {
	return &schemas.DeepIntShieldError{
		IsDeepIntShieldError: false,
		Error: &schemas.ErrorField{
			Message: message,
...
// NewDeepIntShieldOperationError creates a standardized error for deepintshield operation errors.
// This helper reduces code duplication across providers that have deepintshield operation errors.
func NewDeepIntShieldOperationError(message string, err error, providerType schemas.ModelProvider) *schemas.DeepIntShieldError {
	return &schemas.DeepIntShieldError{
		IsDeepIntShieldError: true,
		Error: &schemas.ErrorField{
			Message: message,
...
func NewProviderAPIError(message string, err error, statusCode int, providerType schemas.ModelProvider, errorType *string, eventID *string) *schemas.DeepIntShieldError {
	return &schemas.DeepIntShieldError{
		IsDeepIntShieldError: false,
		StatusCode:           &statusCode,
		Type:                 errorType,
...
// DeepIntShieldContextKeySendBackRawRequest=true in the request context so a single flag is checked here.
...
func ShouldSendBackRawRequest(ctx context.Context, defaultSendBackRawRequest bool) bool {
	if sendBackRawRequest, ok := ctx.Value(schemas.DeepIntShieldContextKeySendBackRawRequest).(bool); ok && sendBackRawRequest {
...
// promote false→true and will not override a true config to false, avoiding accidental suppression.
// Both full send-back mode and logging-only mode (store_raw_request_response) set
// DeepIntShieldContextKeySendBackRawResponse=true in the request context so a single flag is checked here.
// In logging-only mode the payload is stripped before the response reaches the client.
func ShouldSendBackRawResponse(ctx context.Context, defaultSendBackRawResponse bool) bool {
	if sendBackRawResponse, ok := ctx.Value(schemas.DeepIntShieldContextKeySendBackRawResponse).(bool); ok && sendBackRawResponse {
...
func SendCreatedEventResponsesChunk(ctx *schemas.DeepIntShieldContext, postHookRunner schemas.PostHookRunner, provider schemas.ModelProvider, model string, startTime time.Time, responseChan chan *schemas.DeepIntShieldStreamChunk) {
	firstChunk := &schemas.DeepIntShieldResponsesStreamResponse{
	...
	//TODO add deepintshield response pooling here
	deepintshieldResponse := &schemas.DeepIntShieldResponse{
		ResponsesStreamResponse: firstChunk,
	}
	ProcessAndSendResponse(ctx, postHookRunner, deepintshieldResponse, responseChan)
}

// SendInProgressEventResponsesChunk sends a ResponsesStreamResponseTypeInProgress event
func SendInProgressEventResponsesChunk(ctx *schemas.DeepIntShieldContext, postHookRunner schemas.PostHookRunner, provider schemas.ModelProvider, model string, startTime time.Time, responseChan chan *schemas.DeepIntShieldStreamChunk) {
	chunk := &schemas.DeepIntShieldResponsesStreamResponse{
...
// BuildClientStreamChunk constructs a DeepIntShieldStreamChunk from post-hook results.
// It never mutates the shared processedResponse or processedError objects - when in
// logging-only mode (DeepIntShieldContextKeyRawRequestResponseForLogging) it shallow-copies
// each inner response struct and the DeepIntShieldError, nils only the raw fields on those
...
func BuildClientStreamChunk(ctx context.Context, processedResponse *schemas.DeepIntShieldResponse, processedError *schemas.DeepIntShieldError) *schemas.DeepIntShieldStreamChunk {
	dropRaw, _ := ctx.Value(schemas.DeepIntShieldContextKeyRawRequestResponseForLogging).(bool)
	streamResponse := &schemas.DeepIntShieldStreamChunk{}
	if processedResponse != nil {
		streamResponse.DeepIntShieldTextCompletionResponse = processedResponse.TextCompletionResponse
		streamResponse.DeepIntShieldChatResponse = processedResponse.ChatResponse
		streamResponse.DeepIntShieldResponsesStreamResponse = processedResponse.ResponsesStreamResponse
		streamResponse.DeepIntShieldSpeechStreamResponse = processedResponse.SpeechStreamResponse
		streamResponse.DeepIntShieldTranscriptionStreamResponse = processedResponse.TranscriptionStreamResponse
		streamResponse.DeepIntShieldImageGenerationStreamResponse = processedResponse.ImageGenerationStreamResponse
		...
		if dropRaw {
			if streamResponse.DeepIntShieldTextCompletionResponse != nil {
				cp := *streamResponse.DeepIntShieldTextCompletionResponse
				cp.ExtraFields.RawRequest = nil
				cp.ExtraFields.RawResponse = nil
				streamResponse.DeepIntShieldTextCompletionResponse = &cp
			}
			if streamResponse.DeepIntShieldChatResponse != nil {
				cp := *streamResponse.DeepIntShieldChatResponse
				cp.ExtraFields.RawRequest = nil
				cp.ExtraFields.RawResponse = nil
				streamResponse.DeepIntShieldChatResponse = &cp
			}
			if streamResponse.DeepIntShieldResponsesStreamResponse != nil {
				cp := *streamResponse.DeepIntShieldResponsesStreamResponse
				cp.ExtraFields.RawRequest = nil
				cp.ExtraFields.RawResponse = nil
				streamResponse.DeepIntShieldResponsesStreamResponse = &cp
			}
			if streamResponse.DeepIntShieldSpeechStreamResponse != nil {
				cp := *streamResponse.DeepIntShieldSpeechStreamResponse
				cp.ExtraFields.RawRequest = nil
				cp.ExtraFields.RawResponse = nil
				streamResponse.DeepIntShieldSpeechStreamResponse = &cp
			}
			if streamResponse.DeepIntShieldTranscriptionStreamResponse != nil {
				cp := *streamResponse.DeepIntShieldTranscriptionStreamResponse
				cp.ExtraFields.RawRequest = nil
				cp.ExtraFields.RawResponse = nil
				streamResponse.DeepIntShieldTranscriptionStreamResponse = &cp
			}
			if streamResponse.DeepIntShieldImageGenerationStreamResponse != nil {
				cp := *streamResponse.DeepIntShieldImageGenerationStreamResponse
				cp.ExtraFields.RawRequest = nil
				cp.ExtraFields.RawResponse = nil
				streamResponse.DeepIntShieldImageGenerationStreamResponse = &cp
			}
		}
	}
	if processedError != nil {
		if dropRaw {
			...
			streamResponse.DeepIntShieldError = &errCopy
		} else {
			streamResponse.DeepIntShieldError = processedError
		}
	}
...
func ProcessAndSendResponse(
	...
	if tracer, ok := ctx.Value(schemas.DeepIntShieldContextKeyTracer).(schemas.Tracer); ok && tracer != nil {
	...
	if HandleStreamControlSkip(processedError) {
		...
		if isFinalChunk := ctx.Value(schemas.DeepIntShieldContextKeyStreamEndIndicator); isFinalChunk != nil {
...
// ProcessAndSendDeepIntShieldError handles post-hook processing and sends the deepintshield error to the channel.
// This utility reduces code duplication across streaming implementations by encapsulating
// the common pattern of running post hooks, handling errors, and sending responses with
// proper context cancellation handling.
// It also completes the deferred LLM span when the final chunk is sent (StreamEndIndicator is true).
func ProcessAndSendDeepIntShieldError(
	ctx *schemas.DeepIntShieldContext,
	postHookRunner schemas.PostHookRunner,
	deepintshieldErr *schemas.DeepIntShieldError,
	responseChan chan *schemas.DeepIntShieldStreamChunk,
	...
	processedResponse, processedError := postHookRunner(ctx, nil, deepintshieldErr)
...
// before deepintshield considers the connection stalled and closes it. This protects
...
func SetStreamIdleTimeoutIfEmpty(ctx *schemas.DeepIntShieldContext, configSeconds int) {
	if existing, ok := ctx.Value(schemas.DeepIntShieldContextKeyStreamIdleTimeout).(time.Duration); ok && existing > 0 {
		return // already set from upstream (transport/header), respect it
	}
	if configSeconds > 0 {
		ctx.SetValue(schemas.DeepIntShieldContextKeyStreamIdleTimeout, time.Duration(configSeconds)*time.Second)
	}
}

// GetStreamIdleTimeout reads the per-chunk idle timeout from context,
// falling back to DefaultStreamIdleTimeout if not set.
func GetStreamIdleTimeout(ctx *schemas.DeepIntShieldContext) time.Duration {
	if timeout, ok := ctx.Value(schemas.DeepIntShieldContextKeyStreamIdleTimeout).(time.Duration); ok && timeout > 0 {
		return timeout
	}
	return DefaultStreamIdleTimeout
}

// --- Per-tenant network config request-path helpers (PER_TENANT_NETWORK_CONFIG) ---
//
// These let a per-tenant provider timeout and a per-(tenant,provider) HTTP client
// be applied per request without rebuilding the shared, per-type provider. The
// request worker stamps the values on the context (only when the feature flag is
// on and the tenant has resolved config); every helper falls back to prior
// behavior when the values are absent, so existing single-tenant/SDK flows and
// the flag-off path are unaffected.

// SetRequestTimeoutIfEmpty stamps the per-request provider timeout (seconds) on
// the context, unless an upstream layer already set one. Mirrors
// SetStreamIdleTimeoutIfEmpty. A non-positive value is ignored (keeps the
// shared client's baked timeout).
func SetRequestTimeoutIfEmpty(ctx *schemas.DeepIntShieldContext, configSeconds int) {
	if existing, ok := ctx.Value(schemas.DeepIntShieldContextKeyRequestTimeout).(time.Duration); ok && existing > 0 {
		return
	}
	if configSeconds > 0 {
		ctx.SetValue(schemas.DeepIntShieldContextKeyRequestTimeout, time.Duration(configSeconds)*time.Second)
	}
}

// RequestTimeoutFromContext returns the per-tenant provider request timeout
// stamped on the context. The bool is false when none is set, in which case
// callers fall back to the shared client's baked ReadTimeout (prior behavior).
func RequestTimeoutFromContext(ctx context.Context) (time.Duration, bool) {
	if ctx == nil {
		return 0, false
	}
	if t, ok := ctx.Value(schemas.DeepIntShieldContextKeyRequestTimeout).(time.Duration); ok && t > 0 {
		return t, true
	}
	return 0, false
}

// ClientFromContext returns the per-(tenant,provider) HTTP client stamped on the
// context for transport-bound fields (proxy/TLS/conns), falling back to the
// provided shared client when none is set.
func ClientFromContext(ctx context.Context, fallback *fasthttp.Client) *fasthttp.Client {
	if ctx != nil {
		if c, ok := ctx.Value(schemas.DeepIntShieldContextKeyHTTPClient).(*fasthttp.Client); ok && c != nil {
			return c
		}
	}
	return fallback
...
// ResolveBaseURL returns the per-tenant provider base URL stamped on the context
// (PER_TENANT_NETWORK_CONFIG), falling back to the provider's configured base URL
// when none is set. Lets a tenant point a base-URL-aware provider at a different
// endpoint without rebuilding the shared provider.
func ResolveBaseURL(ctx context.Context, providerBaseURL string) string {
	if ctx != nil {
		if u, ok := ctx.Value(schemas.DeepIntShieldContextKeyTenantBaseURL).(string); ok && u != "" {
			return u
		}
	}
...
// StreamingDo executes a streaming provider request honoring the per-tenant
// request timeout (a time-to-first-byte bound) and the per-(tenant,provider)
// client. Per-chunk stalls remain governed by the stream idle timeout. Falls
// back to client.Do on the shared client when no per-tenant values are present,
// preserving existing streaming behavior.
func StreamingDo(ctx context.Context, client *fasthttp.Client, req *fasthttp.Request, resp *fasthttp.Response) error {
	client = ClientFromContext(ctx, client)
	if t, ok := RequestTimeoutFromContext(ctx); ok {
		return client.DoTimeout(req, resp, t)
	}
...
func HandleStreamCancellation(
	...
	if indicator := ctx.GetAndSetValue(schemas.DeepIntShieldContextKeyStreamEndIndicator, true); indicator != nil {
		if set, ok := indicator.(bool); ok && set {
			return // Already handled
		}
	}
	// Create cancellation error
	cancelErr := &schemas.DeepIntShieldError{
	...
	ProcessAndSendDeepIntShieldError(ctx, postHookRunner, cancelErr, responseChan, logger)
}
...
func HandleStreamTimeout(
	...
	timeoutErr := &schemas.DeepIntShieldError{
	...
	// Send through PostHook chain - this updates the log to "error" status
	ProcessAndSendDeepIntShieldError(ctx, postHookRunner, timeoutErr, responseChan, logger)
}
...
func ProcessAndSendError(
	...
	processedResponse, processedError := postHookRunner(ctx, nil, deepintshieldError)
...
// CreateDeepIntShieldTextCompletionChunkResponse creates a deepintshield text completion chunk response.
func CreateDeepIntShieldTextCompletionChunkResponse(
	id string,
	...
	providerName schemas.ModelProvider,
	model string,
) *schemas.DeepIntShieldTextCompletionResponse {
...
// CreateDeepIntShieldChatCompletionChunkResponse creates a deepintshield chat completion chunk response.
func CreateDeepIntShieldChatCompletionChunkResponse(
	id string,
	usage *schemas.DeepIntShieldLLMUsage,
	finishReason *string,
	currentChunkIndex int,
	...
	providerName schemas.ModelProvider,
	model string,
) *schemas.DeepIntShieldChatResponse {
...
func HandleStreamControlSkip(deepintshieldErr *schemas.DeepIntShieldError) bool {
	if deepintshieldErr == nil || deepintshieldErr.StreamControl == nil {
		return false
	}
	if deepintshieldErr.StreamControl.SkipStream != nil && *deepintshieldErr.StreamControl.SkipStream {
		if deepintshieldErr.StreamControl.LogError != nil && *deepintshieldErr.StreamControl.LogError {
			getLogger().Warn("Error in stream: " + deepintshieldErr.Error.Message)
		}
		return true
	}
	return false
...
// Note: CustomProviderKey is internally set by DeepIntShield and should always match the provider name.
...
// GetDeepIntShieldResponseForStreamResponse converts the provided responses to a deepintshield response.
func GetDeepIntShieldResponseForStreamResponse(
	textCompletionResponse *schemas.DeepIntShieldTextCompletionResponse,
	chatResponse *schemas.DeepIntShieldChatResponse,
	responsesStreamResponse *schemas.DeepIntShieldResponsesStreamResponse,
	speechStreamResponse *schemas.DeepIntShieldSpeechStreamResponse,
	transcriptionStreamResponse *schemas.DeepIntShieldTranscriptionStreamResponse,
	imageGenerationStreamResponse *schemas.DeepIntShieldImageGenerationStreamResponse,
) *schemas.DeepIntShieldResponse {
	//TODO add deepintshield response pooling here
	deepintshieldResponse := &schemas.DeepIntShieldResponse{}

	switch {
	case textCompletionResponse != nil:
		deepintshieldResponse.TextCompletionResponse = textCompletionResponse
		return deepintshieldResponse
	case chatResponse != nil:
		deepintshieldResponse.ChatResponse = chatResponse
		return deepintshieldResponse
	case responsesStreamResponse != nil:
		deepintshieldResponse.ResponsesStreamResponse = responsesStreamResponse
		return deepintshieldResponse
	case speechStreamResponse != nil:
		deepintshieldResponse.SpeechStreamResponse = speechStreamResponse
		return deepintshieldResponse
	case transcriptionStreamResponse != nil:
		deepintshieldResponse.TranscriptionStreamResponse = transcriptionStreamResponse
		return deepintshieldResponse
	case imageGenerationStreamResponse != nil:
		deepintshieldResponse.ImageGenerationStreamResponse = imageGenerationStreamResponse
		return deepintshieldResponse
	}
	return nil
}

// aggregateListModelsResponses merges multiple DeepIntShieldListModelsResponse objects into a single response.
...
func aggregateListModelsResponses(responses []*schemas.DeepIntShieldListModelsResponse) *schemas.DeepIntShieldListModelsResponse {
	if len(responses) == 0 {
		return &schemas.DeepIntShieldListModelsResponse{
			Data: []schemas.Model{},
		}
	}
	...
	aggregated := &schemas.DeepIntShieldListModelsResponse{
...
func extractSuccessfulListModelsResponses(
	...
	providerName schemas.ModelProvider,
) ([]*schemas.DeepIntShieldListModelsResponse, []schemas.KeyStatus, *schemas.DeepIntShieldError) {
	var successfulResponses []*schemas.DeepIntShieldListModelsResponse
	...
	if len(successfulResponses) == 0 {
		...
		return nil, keyStatuses, &schemas.DeepIntShieldError{
			IsDeepIntShieldError: false,
			Error: &schemas.ErrorField{
...
func HandleKeylessListModelsRequest(
	provider schemas.ModelProvider,
	listFunc func() (*schemas.DeepIntShieldListModelsResponse, *schemas.DeepIntShieldError),
) (*schemas.DeepIntShieldListModelsResponse, *schemas.DeepIntShieldError) {
	resp, deepintshieldErr := listFunc()
	...
	if deepintshieldErr != nil {
		...
		keyStatus.Error = deepintshieldErr
		deepintshieldErr.ExtraFields.KeyStatuses = []schemas.KeyStatus{keyStatus}
...
func HandleMultipleListModelsRequests(
	...
	listModelsByKey func(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldListModelsRequest) (*schemas.DeepIntShieldListModelsResponse, *schemas.DeepIntShieldError),
) (*schemas.DeepIntShieldListModelsResponse, *schemas.DeepIntShieldError) {
	startTime := time.Now()
	...
	for _, key := range keys {
		...
		go func(k schemas.Key) {
			defer wg.Done()
			resp, deepintshieldErr := listModelsByKey(ctx, k, request)
			results <- schemas.ListModelsByKeyResult{Response: resp, Err: deepintshieldErr, KeyID: k.ID}
		}(key)
	}
...
func completeDeferredSpan(ctx *schemas.DeepIntShieldContext, result *schemas.DeepIntShieldResponse, err *schemas.DeepIntShieldError) {
	if ctx == nil {
		return
	}

	// Get the trace ID from context (this IS available in the provider's goroutine)
	traceID, ok := ctx.Value(schemas.DeepIntShieldContextKeyTraceID).(string)
	if !ok || traceID == "" {
		return
	}

	// Get the tracer from context
	tracerVal := ctx.Value(schemas.DeepIntShieldContextKeyTracer)
	if tracerVal == nil {
		return
	}
	...
	// This builds a complete DeepIntShieldResponse from all the streaming chunks
	...
	if finalizer, ok := ctx.Value(schemas.DeepIntShieldContextKeyPostHookSpanFinalizer).(func(context.Context)); ok && finalizer != nil {
		...
		if spanID != "" {
			finalizerCtx := context.WithValue(ctx, schemas.DeepIntShieldContextKeySpanID, spanID)
...
func CheckAndSetDefaultProvider(ctx *schemas.DeepIntShieldContext, defaultProvider schemas.ModelProvider) schemas.ModelProvider {
	if ctx != nil {
		if ctx.Value(schemas.DeepIntShieldContextKeyDirectKey) != nil || ctx.Value(schemas.DeepIntShieldContextKeySkipKeySelection) != nil {
			return defaultProvider
		}
		if ctx.Value(schemas.DeepIntShieldContextKeyAvailableProviders) != nil {
			availableProviders, ok := ctx.Value(schemas.DeepIntShieldContextKeyAvailableProviders).([]schemas.ModelProvider)
