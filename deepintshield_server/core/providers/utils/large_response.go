...
func PrepareResponseStreaming(ctx *schemas.DeepIntShieldContext, client *fasthttp.Client, resp *fasthttp.Response) *fasthttp.Client {
	// Per-tenant transport client (PER_TENANT_NETWORK_CONFIG): when the worker
	// stamped a per-(tenant,provider) client (proxy/TLS/max-conns overrides),
	// substitute it here so every streaming path that routes through this helper
	// honors the tenant's transport config. No-op otherwise (returns the shared
	// client unchanged), so the common path and flag-off behavior are unchanged.
...
func MaterializeStreamErrorBody(ctx *schemas.DeepIntShieldContext, resp *fasthttp.Response) {
	responseThreshold, _ := ctx.Value(schemas.DeepIntShieldContextKeyLargeResponseThreshold).(int64)
	if responseThreshold <= 0 {
		return
	}
...
func FinalizeResponseWithLargeDetection(
...
) ([]byte, bool, *schemas.DeepIntShieldError) {
	responseThreshold, _ := ctx.Value(schemas.DeepIntShieldContextKeyLargeResponseThreshold).(int64)
	...
	if responseThreshold <= 0 {
		...
		if err != nil {
			return nil, false, NewDeepIntShieldOperationError(schemas.ErrProviderResponseDecode, err, providerName)
		}
	...
	if contentLength > 0 && int64(contentLength) <= responseThreshold {
		if bodyStream := resp.BodyStream(); bodyStream != nil {
			...
			if readErr != nil {
				return nil, false, NewDeepIntShieldOperationError(schemas.ErrProviderResponseDecode, readErr, providerName)
			}
	...
	if contentLength <= 0 {
		if bodyStream := resp.BodyStream(); bodyStream != nil {
			...
			ctx.SetValue(schemas.DeepIntShieldContextKeyLargeResponseMode, true)
			ctx.SetValue(schemas.DeepIntShieldContextKeyLargeResponseReader, closableReader)
			ctx.SetValue(schemas.DeepIntShieldContextKeyLargeResponseContentLength, contentLength)
			if ct := string(resp.Header.ContentType()); ct != "" {
				ctx.SetValue(schemas.DeepIntShieldContextKeyLargeResponseContentType, ct)
			}
			previewLen := min(len(bodyBytes), 1048576)
			ctx.SetValue(schemas.DeepIntShieldContextKeyLargePayloadResponsePreview, string(bodyBytes[:previewLen]))
			return nil, true, nil
		}
		// No stream - buffered fallback
		body, err := CheckAndDecodeBody(resp)
	...
	if ps, ok := ctx.Value(schemas.DeepIntShieldContextKeyLargePayloadPrefetchSize).(int); ok && ps > 0 {
		prefetchSize = ps
	}
	...
	ctx.SetValue(schemas.DeepIntShieldContextKeyLargePayloadResponsePreview, string(prefetchBuf[:previewLen]))

	return nil, true, nil
}

// ParseOpenAIUsageFromBytes parses OpenAI-format usage from raw JSON bytes into DeepIntShieldLLMUsage.
...
func ParseOpenAIUsageFromBytes(data []byte) *schemas.DeepIntShieldLLMUsage {
	var usage struct {
	...
	result := &schemas.DeepIntShieldLLMUsage{}
...
func SetupStreamingPassthrough(ctx *schemas.DeepIntShieldContext, resp *fasthttp.Response) bool {
