...
func TestApplyLargePayloadRequestBodyWithModelNormalization(t *testing.T) {
	...
	ctx.SetValue(schemas.DeepIntShieldContextKeyLargePayloadMode, true)
	ctx.SetValue(
		schemas.DeepIntShieldContextKeyLargePayloadReader,
		strings.NewReader(payload),
	)
	ctx.SetValue(schemas.DeepIntShieldContextKeyLargePayloadContentLength, len(payload))
	ctx.SetValue(schemas.DeepIntShieldContextKeyLargePayloadContentType, "application/json")
	ctx.SetValue(schemas.DeepIntShieldContextKeyLargePayloadMetadata, &schemas.LargePayloadMetadata{
		Model: "openai/gpt-5",
	})
...
// always includes the raw response body in DeepIntShieldError.ExtraFields.RawResponse
func TestHandleProviderAPIError_RawResponseIncluded(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			deepintshieldErr := HandleProviderAPIError(resp, &errorResp)
			...
			if tt.name != "Decode failure" {
				rawResponseBytes, err := sonic.Marshal(deepintshieldErr.ExtraFields.RawResponse)
				if err != nil {
...
func TestEnrichError_PreservesExistingRawResponse(t *testing.T) {
	...
	enrichedErr := EnrichError(ctx, deepintshieldErr, requestBody, nil, true, true)
...
func TestEnrichError_OverwritesWithProvidedResponse(t *testing.T) {
	...
	enrichedErr := EnrichError(ctx, deepintshieldErr, requestBody, responseBody, true, true)
...
func TestEnrichError_RespectsFlags(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			enrichedErr := EnrichError(ctx, deepintshieldErr, requestBody, responseBody, tt.sendBackRawRequest, tt.sendBackRawResponse)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.contextPath != nil {
				ctx = context.WithValue(ctx, schemas.DeepIntShieldContextKeyURLPath, *tt.contextPath)
			}
...
func TestParseAndSetRawRequest_CompactsJSON(t *testing.T) {
  ...
  "temperature": 0.7
	...
	var extraFields schemas.DeepIntShieldResponseExtraFields
...
func TestParseAndSetRawRequest_SSEStreamingChunks(t *testing.T) {
  ...
  "temperature": 0.7
	...
	// Uses DeepIntShieldChatResponse which is the actual type marshaled in the streaming path.
	chunk := schemas.DeepIntShieldChatResponse{
		ID:     "chatcmpl-test",
		Model:  "gpt-4",
	...
	var parsed schemas.DeepIntShieldChatResponse
...
// BuildClientStreamChunk correctly handles DeepIntShieldImageGenerationStreamResponse:
...
func TestBuildClientStreamChunk_ImageGenerationStripping(t *testing.T) {
	...
	imgResp := &schemas.DeepIntShieldImageGenerationStreamResponse{
	...
	response := &schemas.DeepIntShieldResponse{ImageGenerationStreamResponse: imgResp}

	t.Run("logging-only: raw fields stripped from image gen chunk, original preserved", func(t *testing.T) {
		ctx := schemas.NewDeepIntShieldContext(context.Background(), schemas.NoDeadline)
		ctx.SetValue(schemas.DeepIntShieldContextKeyRawRequestResponseForLogging, true)

		chunk := BuildClientStreamChunk(ctx, response, nil)
		if chunk.DeepIntShieldImageGenerationStreamResponse == nil {
			t.Fatal("expected DeepIntShieldImageGenerationStreamResponse in chunk")
		}
		if chunk.DeepIntShieldImageGenerationStreamResponse.ExtraFields.RawRequest != nil {
			t.Error("expected RawRequest stripped from chunk, but it was present")
		}
		if chunk.DeepIntShieldImageGenerationStreamResponse.ExtraFields.RawResponse != nil {
			t.Error("expected RawResponse stripped from chunk, but it was present")
		}
		// Original must not be mutated.
		if imgResp.ExtraFields.RawRequest == nil {
			t.Error("original DeepIntShieldImageGenerationStreamResponse.ExtraFields.RawRequest was mutated")
		}
		if imgResp.ExtraFields.RawResponse == nil {
			t.Error("original DeepIntShieldImageGenerationStreamResponse.ExtraFields.RawResponse was mutated")
		}
		if chunk.DeepIntShieldImageGenerationStreamResponse == imgResp {
...
// that when DeepIntShieldContextKeyRawRequestResponseForLogging is set, ProcessAndSendResponse
// strips RawRequest and RawResponse from the outgoing stream chunk, while leaving other
// ExtraFields intact. It also verifies that the original DeepIntShieldResponse is not mutated
...
func TestProcessAndSendResponse_StoreRawLoggingOnly_StripsRawDataFromResponseChunk(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			passThrough := func(ctx *schemas.DeepIntShieldContext, resp *schemas.DeepIntShieldResponse, err *schemas.DeepIntShieldError) (*schemas.DeepIntShieldResponse, *schemas.DeepIntShieldError) {
				return resp, err
			}

			responseChan := make(chan *schemas.DeepIntShieldStreamChunk, 1)
			ProcessAndSendResponse(ctx, passThrough, response, responseChan)

			chunk := <-responseChan
			if chunk.DeepIntShieldChatResponse == nil {
				t.Fatal("expected non-nil DeepIntShieldChatResponse in stream chunk")
			}

			hasRawReq := chunk.DeepIntShieldChatResponse.ExtraFields.RawRequest != nil
			hasRawResp := chunk.DeepIntShieldChatResponse.ExtraFields.RawResponse != nil
			...
			if tt.expectStripped {
				...
				if response.ChatResponse.ExtraFields.RawRequest == nil {
					t.Error("original DeepIntShieldResponse.ChatResponse.ExtraFields.RawRequest was mutated (nil); shared object must be preserved")
				}
				if response.ChatResponse.ExtraFields.RawResponse == nil {
					t.Error("original DeepIntShieldResponse.ChatResponse.ExtraFields.RawResponse was mutated (nil); shared object must be preserved")
				}
				// The chunk must be a copy, not the same pointer as the original.
				if chunk.DeepIntShieldChatResponse == response.ChatResponse {
					t.Error("chunk.DeepIntShieldChatResponse is the same pointer as the original; it must be a copy to avoid data races")
				}
			} else {
				if !hasRawReq {
...
// that when DeepIntShieldContextKeyRawRequestResponseForLogging is set, raw data is stripped
// from DeepIntShieldError payloads embedded in stream chunks, without mutating the shared
// DeepIntShieldError object (shared object safety for PostLLMHook goroutines).
func TestProcessAndSendResponse_StoreRawLoggingOnly_StripsRawDataFromErrorChunk(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Use a postHookRunner that converts the response to a DeepIntShieldError with raw data
			deepintshieldErr := &schemas.DeepIntShieldError{
				IsDeepIntShieldError: false,
			...
			errorRunner := func(ctx *schemas.DeepIntShieldContext, resp *schemas.DeepIntShieldResponse, err *schemas.DeepIntShieldError) (*schemas.DeepIntShieldResponse, *schemas.DeepIntShieldError) {
				return nil, deepintshieldErr
			}

			responseChan := make(chan *schemas.DeepIntShieldStreamChunk, 1)
			ProcessAndSendResponse(ctx, errorRunner, &schemas.DeepIntShieldResponse{
				ChatResponse: &schemas.DeepIntShieldChatResponse{ID: "chatcmpl-001"},
			}, responseChan)

			chunk := <-responseChan
			if chunk.DeepIntShieldError == nil {
				t.Fatal("expected non-nil DeepIntShieldError in stream chunk")
			}

			hasRawReq := chunk.DeepIntShieldError.ExtraFields.RawRequest != nil
			hasRawResp := chunk.DeepIntShieldError.ExtraFields.RawResponse != nil
			...
			if tt.expectStripped {
				...
				// Critical: the original shared DeepIntShieldError must NOT have been mutated.
				if deepintshieldErr.ExtraFields.RawRequest == nil {
					t.Error("original DeepIntShieldError.ExtraFields.RawRequest was mutated (nil); shared object must be preserved")
				}
				if deepintshieldErr.ExtraFields.RawResponse == nil {
					t.Error("original DeepIntShieldError.ExtraFields.RawResponse was mutated (nil); shared object must be preserved")
				}
				// The chunk must hold a copy, not the same pointer as the original.
				if chunk.DeepIntShieldError == deepintshieldErr {
					t.Error("chunk.DeepIntShieldError is the same pointer as the original; it must be a copy to avoid data races")
				}
			} else {
				if !hasRawReq {
...
//   - DeepIntShieldContextKeySendBackRawRequest=true in context: always returns true
//   - Logging-only mode: requestWorker sets DeepIntShieldContextKeySendBackRawRequest=true,
...
func TestShouldSendBackRawRequest(t *testing.T) {
	tests := []struct {
		...
		{
			// requestWorker sets DeepIntShieldContextKeySendBackRawRequest=true in logging-only
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.contextSendBack {
				ctx.SetValue(schemas.DeepIntShieldContextKeySendBackRawRequest, true)
			}
...
func TestShouldSendBackRawResponse(t *testing.T) {
	tests := []struct {
		...
		{
			// requestWorker sets DeepIntShieldContextKeySendBackRawResponse=true in logging-only
			// mode so a single flag covers both full send-back and logging-only cases.
			name:            "logging-only: context SendBack=true set by requestWorker",
			contextSendBack: true,
			providerDefault: false,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			ctx := schemas.NewDeepIntShieldContext(context.Background(), schemas.NoDeadline)
			if tt.contextSendBack {
				ctx.SetValue(schemas.DeepIntShieldContextKeySendBackRawResponse, true)
			}
