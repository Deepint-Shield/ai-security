...
func TestMakeRequestWithContext_SuccessReturnsNoopWait(t *testing.T) {
	...
	latency, deepintshieldErr, wait := MakeRequestWithContext(context.Background(), client, req, resp)
	defer wait()

	if deepintshieldErr != nil {
		t.Fatalf("expected no error, got: %v", deepintshieldErr.Error.Message)
	}
	if latency <= 0 {
...
func TestMakeRequestWithContext_ContextCancelReturnsBlockingWait(t *testing.T) {
	...
	_, deepintshieldErr, wait := MakeRequestWithContext(ctx, client, req, resp)

	// Should get a cancellation error
	if deepintshieldErr == nil {
		t.Fatal("expected cancellation error")
	}
	if deepintshieldErr.Error.Type == nil || *deepintshieldErr.Error.Type != schemas.RequestCancelled {
		t.Fatalf("expected RequestCancelled error type, got: %v", deepintshieldErr.Error.Type)
	}
...
func TestMakeRequestWithContext_ClientError(t *testing.T) {
	...
	_, deepintshieldErr, wait := MakeRequestWithContext(context.Background(), client, req, resp)
	defer wait()
