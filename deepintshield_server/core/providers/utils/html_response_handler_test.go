...
func TestHandleProviderAPIErrorWithHTML(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if deepintshieldErr.StatusCode == nil || *deepintshieldErr.StatusCode != tt.statusCode {
				t.Errorf("HandleProviderAPIError() status code = %v, want %v", deepintshieldErr.StatusCode, tt.statusCode)
			}
			...
			if tt.expectedInMessage != "" && !strings.Contains(deepintshieldErr.Error.Message, tt.expectedInMessage) {
				t.Errorf("Expected message to contain %q, got %q", tt.expectedInMessage, deepintshieldErr.Error.Message)
			}

			t.Logf("Handled %s: status=%d, message=%q", tt.name, *deepintshieldErr.StatusCode, deepintshieldErr.Error.Message)
		})
	}
}
