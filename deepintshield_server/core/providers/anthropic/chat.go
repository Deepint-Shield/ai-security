...
// ToAnthropicChatRequest converts a DeepIntShield request to Anthropic format
// This is the reverse of ConvertChatRequestToDeepIntShield for provider-side usage
func ToAnthropicChatRequest(ctx *schemas.DeepIntShieldContext, deepintshieldReq *schemas.DeepIntShieldChatRequest) (*AnthropicMessageRequest, error) {
	...
	if deepintshieldReq.Params != nil {
		...
		if deepintshieldReq.Params.MaxCompletionTokens != nil {
			anthropicReq.MaxTokens = *deepintshieldReq.Params.MaxCompletionTokens
		}
		...
		if deepintshieldReq.Params.ResponseFormat != nil {
			...
			if deepintshieldReq.Provider == schemas.Vertex {
				responseFormatTool := convertChatResponseFormatToTool(ctx, deepintshieldReq.Params)
				...
				outputFormat := convertChatResponseFormatToAnthropicOutputFormat(deepintshieldReq.Params.ResponseFormat)
				if outputFormat != nil {
		...
		if deepintshieldReq.Params.Tools != nil {
			tools := make([]AnthropicTool, 0, len(deepintshieldReq.Params.Tools))
		...
		if deepintshieldReq.Params.ToolChoice != nil {
			...
			if deepintshieldReq.Params.ToolChoice.ChatToolChoiceStr != nil {
				switch schemas.ChatToolChoiceType(*deepintshieldReq.Params.ToolChoice.ChatToolChoiceStr) {
			...
			} else if deepintshieldReq.Params.ToolChoice.ChatToolChoiceStruct != nil {
				switch deepintshieldReq.Params.ToolChoice.ChatToolChoiceStruct.Type {
				case schemas.ChatToolChoiceTypeFunction:
					toolChoice.Type = "tool"
					if deepintshieldReq.Params.ToolChoice.ChatToolChoiceStruct.Function != nil {
						toolChoice.Name = deepintshieldReq.Params.ToolChoice.ChatToolChoiceStruct.Function.Name
					}
		...
		if deepintshieldReq.Params.Reasoning != nil {
			if deepintshieldReq.Params.Reasoning.MaxTokens != nil {
			...
			} else if deepintshieldReq.Params.Reasoning.Effort != nil && *deepintshieldReq.Params.Reasoning.Effort != "none" {
				...
				if SupportsAdaptiveThinking(deepintshieldReq.Model) {
					...
					budgetTokens, err := providerUtils.GetBudgetTokensFromReasoningEffort(*deepintshieldReq.Params.Reasoning.Effort, MinimumReasoningMaxTokens, anthropicReq.MaxTokens)
					if err != nil {
						return nil, err
					}
					anthropicReq.Thinking = &AnthropicThinking{
		...
		default:
			...
			if msg.ChatAssistantMessage != nil && msg.ChatAssistantMessage.ReasoningDetails != nil {
				for _, rd := range msg.ChatAssistantMessage.ReasoningDetails {
					if rd.Type == schemas.DeepIntShieldReasoningDetailsTypeContentBlocks {
					...
					if rd.Type == schemas.DeepIntShieldReasoningDetailsTypeText {
...
// ToDeepIntShieldChatResponse converts an Anthropic message response to DeepIntShield format
func (response *AnthropicMessageResponse) ToDeepIntShieldChatResponse(ctx *schemas.DeepIntShieldContext) *schemas.DeepIntShieldChatResponse {
	if response == nil {
		return nil
	}
		...
		if allText {
			...
			if !needsCombine {
				...
				for _, rd := range reasoningDetails {
					if rd.Type == schemas.DeepIntShieldReasoningDetailsTypeText && rd.Text != nil {
				...
				if len(blockMeta) > 1 {
					if metaJSON, err := providerUtils.MarshalSorted(blockMeta); err == nil {
						...
						reasoningDetails = append(reasoningDetails, schemas.ChatReasoningDetails{
							Index: len(reasoningDetails),
							Type:  schemas.DeepIntShieldReasoningDetailsTypeContentBlocks,
							Text:  &metaStr,
						})
					}
				}
				...
				for i := range reasoningDetails {
					if reasoningDetails[i].Type == schemas.DeepIntShieldReasoningDetailsTypeText {
	...
	choice := schemas.DeepIntShieldResponseChoice{
		...
		FinishReason: func() *string {
			if response.StopReason != "" {
				mapped := ConvertAnthropicFinishReasonToDeepIntShield(response.StopReason)
				return &mapped
			}
			return nil
		}(),
	...
	if response.Usage != nil {
		...
		if response.Usage.ServiceTier != nil {
			deepintshieldResponse.ServiceTier = response.Usage.ServiceTier
		}
	}
...
// ToAnthropicChatResponse converts a DeepIntShield response to Anthropic format
func ToAnthropicChatResponse(deepintshieldResp *schemas.DeepIntShieldChatResponse) *AnthropicMessageResponse {
	...
	if deepintshieldResp.Usage != nil {
		anthropicResp.Usage = &AnthropicUsage{
			InputTokens:  deepintshieldResp.Usage.PromptTokens,
			OutputTokens: deepintshieldResp.Usage.CompletionTokens,
		}
		...
		if deepintshieldResp.Usage.PromptTokensDetails != nil && deepintshieldResp.Usage.PromptTokensDetails.CachedReadTokens > 0 {
			anthropicResp.Usage.CacheReadInputTokens = deepintshieldResp.Usage.PromptTokensDetails.CachedReadTokens
			anthropicResp.Usage.InputTokens = anthropicResp.Usage.InputTokens - deepintshieldResp.Usage.PromptTokensDetails.CachedReadTokens
		}
		if deepintshieldResp.Usage.PromptTokensDetails != nil && deepintshieldResp.Usage.PromptTokensDetails.CachedWriteTokens > 0 {
			anthropicResp.Usage.CacheCreationInputTokens = deepintshieldResp.Usage.PromptTokensDetails.CachedWriteTokens
			anthropicResp.Usage.InputTokens = anthropicResp.Usage.InputTokens - deepintshieldResp.Usage.PromptTokensDetails.CachedWriteTokens
		}
		// Forward service tier
		if deepintshieldResp.ServiceTier != nil {
			anthropicResp.Usage.ServiceTier = deepintshieldResp.ServiceTier
		}
	}
	...
	if len(deepintshieldResp.Choices) > 0 {
		choice := deepintshieldResp.Choices[0] // Anthropic typically returns one choice

		if choice.FinishReason != nil {
			anthropicResp.StopReason = ConvertDeepIntShieldFinishReasonToAnthropic(*choice.FinishReason)
		}
		...
		if choice.ChatNonStreamResponseChoice != nil && choice.Message != nil && choice.Message.ChatAssistantMessage != nil && choice.Message.ChatAssistantMessage.ReasoningDetails != nil {
			for _, reasoningDetail := range choice.Message.ChatAssistantMessage.ReasoningDetails {
				if reasoningDetail.Type == schemas.DeepIntShieldReasoningDetailsTypeText && reasoningDetail.Text != nil &&
...
// ToDeepIntShieldChatCompletionStream converts an Anthropic stream event to a DeepIntShield Chat Completion Stream response
func (chunk *AnthropicStreamEvent) ToDeepIntShieldChatCompletionStream(ctx *schemas.DeepIntShieldContext, structuredOutputToolName string, state *AnthropicStreamState) (*schemas.DeepIntShieldChatResponse, *schemas.DeepIntShieldError, bool) {
	if state == nil {
	...
	case AnthropicStreamEventTypeError:
		if chunk.Error != nil {
			...
			return nil, deepintshieldErr, true
		}
	}
...
// ToAnthropicChatStreamResponse converts a DeepIntShield streaming response to Anthropic SSE string format
func ToAnthropicChatStreamResponse(deepintshieldResp *schemas.DeepIntShieldChatResponse) string {
	if deepintshieldResp == nil {
		return ""
	}
	...
	if len(deepintshieldResp.Choices) > 0 {
		...
		if choice.ChatStreamResponseChoice != nil && choice.ChatStreamResponseChoice.Delta != nil {
			...
			if delta.Content != nil {
				...
				stopReason := ConvertDeepIntShieldFinishReasonToAnthropic(*choice.FinishReason)
	...
	// Set common fields
	if deepintshieldResp.ID != "" {
		streamResp.ID = &deepintshieldResp.ID
	}
	if deepintshieldResp.Model != "" {
		if streamResp.Message == nil {
			streamResp.Message = &AnthropicMessageResponse{}
		}
		streamResp.Message.Model = deepintshieldResp.Model
	}
...
// ToAnthropicChatStreamError converts a DeepIntShieldError to Anthropic streaming error in SSE format
func ToAnthropicChatStreamError(deepintshieldErr *schemas.DeepIntShieldError) string {
	errorResp := ToAnthropicChatCompletionError(deepintshieldErr)
	if errorResp == nil {
		return ""
	}
	// Marshal to JSON
