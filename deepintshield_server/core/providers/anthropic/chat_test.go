...
func TestToAnthropicChatRequest_PreservesPropertyOrder(t *testing.T) {
	...
	defer cancel()
	result, err := ToAnthropicChatRequest(ctx, deepintshieldReq)
	if err != nil {
...
func TestToAnthropicChatRequest_CachingDeterminism(t *testing.T) {
	makeReq := func(props *schemas.OrderedMap) *schemas.DeepIntShieldChatRequest {
...
func TestToDeepIntShieldChatResponse_MultipleTextBlocksWithThinking(t *testing.T) {
	...
	ctx, cancel := schemas.NewDeepIntShieldContextWithCancel(nil)
	defer cancel()
	result := response.ToDeepIntShieldChatResponse(ctx)
	...
	if rd[0].Type != schemas.DeepIntShieldReasoningDetailsTypeText {
		t.Errorf("expected first reasoning detail type %s, got %s", schemas.DeepIntShieldReasoningDetailsTypeText, rd[0].Type)
	}
	...
	lastRD := rd[len(rd)-1]
	if lastRD.Type != schemas.DeepIntShieldReasoningDetailsTypeContentBlocks {
		t.Errorf("expected last reasoning detail type %s, got %s", schemas.DeepIntShieldReasoningDetailsTypeContentBlocks, lastRD.Type)
	}
	if lastRD.Text == nil {
...
func TestToDeepIntShieldChatResponse_SingleTextBlockNoThinking(t *testing.T) {
	...
	deepintshieldReq := &schemas.DeepIntShieldChatRequest{
		...
		Input: []schemas.ChatMessage{
			...
			},
			{
				...
				ChatAssistantMessage: &schemas.ChatAssistantMessage{
					ReasoningDetails: []schemas.ChatReasoningDetails{
						{Index: 0, Type: schemas.DeepIntShieldReasoningDetailsTypeText, Signature: &signature},
						{Index: 1, Type: schemas.DeepIntShieldReasoningDetailsTypeContentBlocks, Text: &metaStr},
					},
				},
			},
			{
...
func TestToAnthropicChatRequest_NormalFlowUnchanged(t *testing.T) {
	...
	// no deepintshield.content_blocks) produces the same output as before.
