...
// --- Streaming: Anthropic → DeepIntShield (inbound) ---

func TestToDeepIntShieldResponsesStream_CompactionContentBlockStart(t *testing.T) {
	t.Parallel()
	...
	responses, err, isLast := chunk.ToDeepIntShieldResponsesStream(context.Background(), 0, state)
	if err != nil {
...
func TestToDeepIntShieldResponsesStream_CompactionDelta(t *testing.T) {
	t.Parallel()
...
func TestToDeepIntShieldResponsesStream_CompactionContentBlockStop(t *testing.T) {
	t.Parallel()
...
// --- Streaming: DeepIntShield → Anthropic (outbound, non-passthrough) ---
...
func TestToAnthropicResponsesStreamResponse_CompactionOutputItemAdded(t *testing.T) {
	...
	deepintshieldResp := &schemas.DeepIntShieldResponsesStreamResponse{
	...
	events := ToAnthropicResponsesStreamResponse(ctx, deepintshieldResp)
...
func TestToDeepIntShieldResponsesResponse_PreservesStopReason(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			deepintshieldResp := resp.ToDeepIntShieldResponsesResponse(ctx)

			if deepintshieldResp.StopReason == nil {
				t.Fatal("expected StopReason to be non-nil")
			}
			if *deepintshieldResp.StopReason != tt.expectedStopReason {
				t.Errorf("StopReason = %q, want %q", *deepintshieldResp.StopReason, tt.expectedStopReason)
			}
		})
	}
}

func TestToDeepIntShieldResponsesResponse_EmptyStopReason(t *testing.T) {
	t.Parallel()
	...
	if deepintshieldResp.StopReason != nil {
		t.Errorf("expected nil StopReason for empty stop_reason, got %q", *deepintshieldResp.StopReason)
	}
}

func TestToAnthropicResponsesResponse_StopReasonFromDeepIntShield(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name:           "compaction stop reason from deepintshield",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result := ToAnthropicResponsesResponse(ctx, deepintshieldResp)
...
func TestCompactionContentBlock_NonStreamingRoundTrip(t *testing.T) {
	...
	// Step 1: Anthropic → DeepIntShield
	deepintshieldResp := anthropicResp.ToDeepIntShieldResponsesResponse(ctx)

	if deepintshieldResp.StopReason == nil || *deepintshieldResp.StopReason != "compaction" {
		t.Fatalf("expected stop_reason='compaction', got %v", deepintshieldResp.StopReason)
	}
	...
	for _, msg := range deepintshieldResp.Output {
		if msg.Content != nil {
	...
	if !foundCompaction {
		t.Error("compaction block not found in DeepIntShield output")
	}

	// Step 2: DeepIntShield → Anthropic
	result := ToAnthropicResponsesResponse(ctx, deepintshieldResp)
