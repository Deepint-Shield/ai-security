...
var (
	...
	anthropicFinishReasonToDeepIntShield = map[AnthropicStopReason]string{
	...
	deepintshieldToAnthropicFinishReason = map[string]AnthropicStopReason{
...
// MapDeepIntShieldEffortToAnthropic maps a DeepIntShield effort level to an Anthropic effort level.
// Anthropic supports "low", "medium", "high", "max"; DeepIntShield also has "minimal" which maps to "low".
func MapDeepIntShieldEffortToAnthropic(effort string) string {
	if effort == "minimal" {
		return "low"
	}
	return effort
...
// MapAnthropicEffortToDeepIntShield maps an Anthropic effort level to a DeepIntShield effort level.
// Anthropic supports "max" (Opus 4.6+) which is not in DeepIntShield's enum; it maps to "high".
func MapAnthropicEffortToDeepIntShield(effort string) string {
	if effort == "max" {
		return "high"
	}
	return effort
...
func getRequestBodyForResponses(ctx *schemas.DeepIntShieldContext, request *schemas.DeepIntShieldResponsesRequest, providerName schemas.ModelProvider, isStreaming bool, excludeFields []string) ([]byte, *schemas.DeepIntShieldError) {
	...
	if useRawBody, ok := ctx.Value(schemas.DeepIntShieldContextKeyUseRawRequestBody).(bool); ok && useRawBody {
		...
		if convErr != nil {
			return nil, providerUtils.NewDeepIntShieldOperationError(schemas.ErrRequestBodyConversion, convErr, providerName)
		}
		if reqBody == nil {
			return nil, providerUtils.NewDeepIntShieldOperationError("request body is not provided", nil, providerName)
		}
		...
		if err != nil {
			return nil, providerUtils.NewDeepIntShieldOperationError(schemas.ErrProviderRequestMarshal, fmt.Errorf("failed to marshal request body: %w", err), providerName)
		}
...
func AddMissingBetaHeadersToContext(ctx *schemas.DeepIntShieldContext, req *AnthropicMessageRequest, provider schemas.ModelProvider) error {
	...
	if ctx.Value(schemas.DeepIntShieldContextKeyExtraHeaders) == nil {
		extraHeaders = map[string][]string{}
	} else {
		if ctxExtraHeaders, ok := ctx.Value(schemas.DeepIntShieldContextKeyExtraHeaders).(map[string][]string); ok {
	...
	ctx.SetValue(schemas.DeepIntShieldContextKeyExtraHeaders, extraHeaders)
	return nil
}
...
func convertChatResponseFormatToTool(ctx *schemas.DeepIntShieldContext, params *schemas.ChatParameters) *AnthropicTool {
	...
	// Set deepintshield context key structured output tool name
	toolName = fmt.Sprintf("bf_so_%s", toolName)
	ctx.SetValue(schemas.DeepIntShieldContextKeyStructuredOutputToolName, toolName)
...
// This is used when the provider is Vertex, which doesn't support native structured outputs
func convertResponsesTextFormatToTool(ctx *schemas.DeepIntShieldContext, textConfig *schemas.ResponsesTextConfig) *AnthropicTool {
...
// ConvertAnthropicFinishReasonToDeepIntShield converts provider finish reasons to DeepIntShield format
func ConvertAnthropicFinishReasonToDeepIntShield(providerReason AnthropicStopReason) string {
	if deepintshieldReason, ok := anthropicFinishReasonToDeepIntShield[providerReason]; ok {
		return deepintshieldReason
	}
	return string(providerReason)
}

// ConvertDeepIntShieldFinishReasonToAnthropic converts DeepIntShield finish reasons to provider format
func ConvertDeepIntShieldFinishReasonToAnthropic(deepintshieldReason string) AnthropicStopReason {
	if providerReason, ok := deepintshieldToAnthropicFinishReason[deepintshieldReason]; ok {
		return providerReason
	}
	return AnthropicStopReason(deepintshieldReason)
}

// ConvertToAnthropicImageBlock converts a DeepIntShield image block to Anthropic format
...
// ConvertToAnthropicDocumentBlock converts a DeepIntShield file block to Anthropic document format
...
func (block AnthropicContentBlock) ToDeepIntShieldContentImageBlock() schemas.ChatContentBlock {
...
// It searches backwards through deepintshieldMessages to find the matching call and updates its action.
func attachWebSearchSourcesToCall(deepintshieldMessages []schemas.ResponsesMessage, toolUseID string, resultBlock AnthropicContentBlock, includeExtendedFields bool) {
...
func IsClaudeCodeMaxMode(ctx *schemas.DeepIntShieldContext) bool {
	userAgent, _ := ctx.Value(schemas.DeepIntShieldContextKeyUserAgent).(string)
	skipKeySelection, _ := ctx.Value(schemas.DeepIntShieldContextKeySkipKeySelection).(bool)
...
func IsClaudeCodeRequest(ctx *schemas.DeepIntShieldContext) bool {
	if userAgent, ok := ctx.Value(schemas.DeepIntShieldContextKeyUserAgent).(string); ok {
