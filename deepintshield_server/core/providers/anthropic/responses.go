...
// ToDeepIntShieldResponsesStream converts an Anthropic stream event to a DeepIntShield Responses Stream response
...
func (chunk *AnthropicStreamEvent) ToDeepIntShieldResponsesStream(ctx context.Context, sequenceNumber int, state *AnthropicResponsesStreamState) ([]*schemas.DeepIntShieldResponsesStreamResponse, *schemas.DeepIntShieldError, bool) {
	switch chunk.Type {
		...
		if chunk.ContentBlock != nil && chunk.Index != nil {
			...
			if chunk.ContentBlock.Type == AnthropicContentBlockTypeToolUse &&
				...
				*chunk.ContentBlock.Name == string(AnthropicToolNameComputer) &&
				...
				return []*schemas.DeepIntShieldResponsesStreamResponse{{
			...
			default:
				...
				if ctx.Value(schemas.DeepIntShieldContextKeyIntegrationType) == "anthropic" {
					return []*schemas.DeepIntShieldResponsesStreamResponse{{
						Type:           "",
						SequenceNumber: sequenceNumber,
					}}, nil, false
		...
		if chunk.Index != nil {
			...
			if chunk.Index != nil {
				if state.TextContentIndices[*chunk.Index] {
					...
					emptyText := ""
					textDoneResponse := &schemas.DeepIntShieldResponsesStreamResponse{
	...
	case AnthropicStreamEventTypeMessageDelta:
		if chunk.Delta.StopReason != nil {
			state.StopReason = schemas.Ptr(ConvertAnthropicFinishReasonToDeepIntShield(*chunk.Delta.StopReason))
		}
		// Check if integration type in ctx is anthropic
		if ctx.Value(schemas.DeepIntShieldContextKeyIntegrationType) == "anthropic" {
			// Convert usage from Anthropic format to DeepIntShield
			deepintshieldUsage := ConvertAnthropicUsageToDeepIntShieldUsage(chunk.Usage)

			// Convert stop reason if present
			var stopReason *string
			if chunk.Delta != nil && chunk.Delta.StopReason != nil {
				converted := ConvertAnthropicFinishReasonToDeepIntShield(*chunk.Delta.StopReason)
			...
			if deepintshieldUsage != nil {
				response.Usage = deepintshieldUsage
			}
...
// ToAnthropicResponsesStreamResponse converts a DeepIntShield Responses stream response to Anthropic SSE string format
func ToAnthropicResponsesStreamResponse(ctx *schemas.DeepIntShieldContext, deepintshieldResp *schemas.DeepIntShieldResponsesStreamResponse) []*AnthropicStreamEvent {
	...
	case schemas.ResponsesStreamResponseTypeCreated:
		...
		if deepintshieldResp.Response != nil {
			...
			if deepintshieldResp.Response.ID != nil {
				streamMessage.ID = *deepintshieldResp.Response.ID
			}
			// Preserve model from Response if available, otherwise use ExtraFields
			if deepintshieldResp.ExtraFields.ModelRequested != "" {
				if deepintshieldResp.Response != nil && deepintshieldResp.Response.Model != "" {
					streamMessage.Model = deepintshieldResp.Response.Model
				} else {
					streamMessage.Model = deepintshieldResp.ExtraFields.ModelRequested
				}
			}
	...
	case schemas.ResponsesStreamResponseTypeOutputItemAdded:
		...
		if deepintshieldResp.Item != nil &&
			deepintshieldResp.Item.Type != nil &&
			*deepintshieldResp.Item.Type == schemas.ResponsesMessageTypeComputerCall {

			// Computer tool - emit content_block_start
			streamResp.Type = AnthropicStreamEventTypeContentBlockStart

			if deepintshieldResp.OutputIndex != nil {
				streamResp.Index = deepintshieldResp.OutputIndex
			} else if deepintshieldResp.ContentIndex != nil {
				streamResp.Index = deepintshieldResp.ContentIndex
			}
			...
			contentBlock := &AnthropicContentBlock{
				...
				ID:   deepintshieldResp.Item.ID,                            // The tool use ID
			...
			deepintshieldResp.Item.Type != nil &&
			*deepintshieldResp.Item.Type == schemas.ResponsesMessageTypeWebSearchCall {
			...
			if deepintshieldResp.ContentIndex != nil {
				streamResp.Index = deepintshieldResp.ContentIndex
			} else if deepintshieldResp.OutputIndex != nil {
				streamResp.Index = deepintshieldResp.OutputIndex
			}
			...
			if deepintshieldResp.Item != nil {
				...
				if isCompactionItem(deepintshieldResp.Item) {
					contentBlock.Type = AnthropicContentBlockTypeCompaction
					contentBlock.Content = &AnthropicContent{ContentStr: schemas.Ptr("")}
					if deepintshieldResp.Item.Content.ContentBlocks[0].CacheControl != nil {
						contentBlock.CacheControl = deepintshieldResp.Item.Content.ContentBlocks[0].CacheControl
					}
				} else if deepintshieldResp.Item.Type != nil {
					switch *deepintshieldResp.Item.Type {
					...
					case schemas.ResponsesMessageTypeReasoning:
						...
						if deepintshieldResp.Item.ResponsesReasoning != nil && deepintshieldResp.Item.ResponsesReasoning.EncryptedContent != nil && *deepintshieldResp.Item.ResponsesReasoning.EncryptedContent != "" {
							contentBlock.Data = deepintshieldResp.Item.ResponsesReasoning.EncryptedContent
					...
					case schemas.ResponsesMessageTypeFunctionCall:
						...
						if deepintshieldResp.Item.ResponsesReasoning != nil {
							// This is actually reasoning content, not a function call
							contentBlock.Type = AnthropicContentBlockTypeThinking
							contentBlock.Thinking = schemas.Ptr("")
							contentBlock.Signature = schemas.Ptr("")
							// Check if there's encrypted content for redacted_thinking
							if deepintshieldResp.Item.ResponsesReasoning.EncryptedContent != nil && *deepintshieldResp.Item.ResponsesReasoning.EncryptedContent != "" {
								contentBlock.Type = AnthropicContentBlockTypeRedactedThinking
								contentBlock.Data = deepintshieldResp.Item.ResponsesReasoning.EncryptedContent
							}
						} else {
							...
							contentIndex := 0
							if deepintshieldResp.ContentIndex != nil {
								contentIndex = *deepintshieldResp.ContentIndex
							}
							...
							if deepintshieldResp.Response != nil && deepintshieldResp.Response.Output != nil {
								for _, msg := range deepintshieldResp.Response.Output {
							...
							if isFirstBlock && hasReasoningInResponse {
								...
								if deepintshieldResp.Item.ResponsesToolMessage != nil {
									contentBlock.ID = deepintshieldResp.Item.ResponsesToolMessage.CallID
									contentBlock.Name = deepintshieldResp.Item.ResponsesToolMessage.Name
									// Always start with empty input for streaming compatibility
									contentBlock.Input = json.RawMessage("{}")

									// Track WebSearch tools so we can skip their argument deltas
									if deepintshieldResp.Item.ResponsesToolMessage.Name != nil &&
										*deepintshieldResp.Item.ResponsesToolMessage.Name == "WebSearch" &&
										deepintshieldResp.Item.ID != nil {
										webSearchItemIDs.Store(*deepintshieldResp.Item.ID, true)
									}
								}
							}
						}
					case schemas.ResponsesMessageTypeMCPCall:
						...
						if deepintshieldResp.Item.ResponsesToolMessage != nil {
							contentBlock.ID = deepintshieldResp.Item.ID
							contentBlock.Name = deepintshieldResp.Item.ResponsesToolMessage.Name
							if deepintshieldResp.Item.ResponsesToolMessage.ResponsesMCPToolCall != nil {
								contentBlock.ServerName = &deepintshieldResp.Item.ResponsesToolMessage.ResponsesMCPToolCall.ServerLabel
							}
							// Always start with empty input for streaming compatibility
							contentBlock.Input = json.RawMessage("{}")
						}
		...
		if isCompactionItem(deepintshieldResp.Item) {
			block := deepintshieldResp.Item.Content.ContentBlocks[0]
			if block.ResponsesOutputMessageContentCompaction != nil {
				var indexToUse *int
				if deepintshieldResp.OutputIndex != nil {
					indexToUse = deepintshieldResp.OutputIndex
				} else if deepintshieldResp.ContentIndex != nil {
					indexToUse = deepintshieldResp.ContentIndex
				}
		...
		if deepintshieldResp.Item != nil && deepintshieldResp.Item.ResponsesToolMessage != nil {
			...
			switch *deepintshieldResp.Item.Type {
			case schemas.ResponsesMessageTypeFunctionCall:
				if deepintshieldResp.Item.ResponsesToolMessage.Arguments != nil && *deepintshieldResp.Item.ResponsesToolMessage.Arguments != "" {
					argumentsJSON = *deepintshieldResp.Item.ResponsesToolMessage.Arguments
			...
			case schemas.ResponsesMessageTypeComputerCall:
				if deepintshieldResp.Item.ResponsesToolMessage.Action != nil && deepintshieldResp.Item.ResponsesToolMessage.Action.ResponsesComputerToolCallAction != nil {
					actionInput := convertResponsesToAnthropicComputerAction(deepintshieldResp.Item.ResponsesToolMessage.Action.ResponsesComputerToolCallAction)
	...
	case schemas.ResponsesStreamResponseTypeFunctionCallArgumentsDelta:
		...
		if deepintshieldResp.ItemID != nil {
			if _, isWebSearch := webSearchItemIDs.Load(*deepintshieldResp.ItemID); isWebSearch {
				return nil
			}
			if _, isWebFetch := webFetchItemIDs.Load(*deepintshieldResp.ItemID); isWebFetch {
				return nil
			}
		}
		...
		if deepintshieldResp.Arguments != nil {
			streamResp.Delta = &AnthropicStreamDelta{
				Type:        AnthropicStreamDeltaTypeInputJSON,
				PartialJSON: deepintshieldResp.Arguments,
			}
		} else if deepintshieldResp.Delta != nil {
			// Handle cases where Delta field is used instead of Arguments
			streamResp.Delta = &AnthropicStreamDelta{
				Type:        AnthropicStreamDeltaTypeInputJSON,
				PartialJSON: deepintshieldResp.Delta,
			}
		}
	...
	case schemas.ResponsesStreamResponseTypeOutputTextAnnotationAdded:
		...
		if deepintshieldResp.Annotation != nil {
			...
			citation := convertAnnotationToAnthropicCitation(*deepintshieldResp.Annotation)
	...
	case schemas.ResponsesStreamResponseTypeOutputItemDone:
		...
		if ctx != nil {
			if IsClaudeCodeRequest(ctx) {
				...
				if deepintshieldResp.Item != nil &&
					deepintshieldResp.Item.Type != nil &&
					*deepintshieldResp.Item.Type == schemas.ResponsesMessageTypeFunctionCall &&
					deepintshieldResp.Item.ResponsesToolMessage != nil &&
					deepintshieldResp.Item.ResponsesToolMessage.Name != nil &&
					*deepintshieldResp.Item.ResponsesToolMessage.Name == "WebSearch" &&
					deepintshieldResp.Item.ResponsesToolMessage.Arguments != nil {

					argumentsJSON := sanitizeWebSearchArguments(*deepintshieldResp.Item.ResponsesToolMessage.Arguments)
					deepintshieldResp.Item.ResponsesToolMessage.Arguments = &argumentsJSON
					...
					if deepintshieldResp.Item.ID != nil {
						webSearchItemIDs.Delete(*deepintshieldResp.Item.ID)
					}
		...
		if deepintshieldResp.Item != nil &&
			...
			*deepintshieldResp.Item.Type == schemas.ResponsesMessageTypeComputerCall {
			...
			if deepintshieldResp.Item.ResponsesToolMessage != nil &&
				deepintshieldResp.Item.ResponsesToolMessage.Action != nil &&
				deepintshieldResp.Item.ResponsesToolMessage.Action.ResponsesComputerToolCallAction != nil {

				actionInput := convertResponsesToAnthropicComputerAction(
					deepintshieldResp.Item.ResponsesToolMessage.Action.ResponsesComputerToolCallAction,
				)
			...
			*deepintshieldResp.Item.Type == schemas.ResponsesMessageTypeWebSearchCall {
			...
			if deepintshieldResp.Item.ResponsesToolMessage != nil &&
				deepintshieldResp.Item.ResponsesToolMessage.Action != nil &&
				deepintshieldResp.Item.ResponsesToolMessage.Action.ResponsesWebSearchToolCallAction != nil &&
				deepintshieldResp.Item.ResponsesToolMessage.Action.ResponsesWebSearchToolCallAction.Query != nil {

				// Create input map with query
				inputMap := map[string]interface{}{
					"query": *deepintshieldResp.Item.ResponsesToolMessage.Action.ResponsesWebSearchToolCallAction.Query,
				}
			...
			if deepintshieldResp.ContentIndex != nil {
				stopEvent.Index = deepintshieldResp.ContentIndex
			} else if deepintshieldResp.OutputIndex != nil {
				stopEvent.Index = deepintshieldResp.OutputIndex
			}
			events = append(events, stopEvent)
			...
			if deepintshieldResp.Item.ResponsesToolMessage != nil &&
				deepintshieldResp.Item.ResponsesToolMessage.Action != nil &&
				deepintshieldResp.Item.ResponsesToolMessage.Action.ResponsesWebSearchToolCallAction != nil &&
				len(deepintshieldResp.Item.ResponsesToolMessage.Action.ResponsesWebSearchToolCallAction.Sources) > 0 {

				// Calculate next index for result block
				var resultIndex *int
				if deepintshieldResp.OutputIndex != nil {
					nextIdx := *deepintshieldResp.OutputIndex + 1
					resultIndex = &nextIdx
				} else if deepintshieldResp.ContentIndex != nil {
					nextIdx := *deepintshieldResp.ContentIndex + 1
					resultIndex = &nextIdx
				}

				// Create content blocks for each source
				var resultContentBlocks []AnthropicContentBlock
				for _, source := range deepintshieldResp.Item.ResponsesToolMessage.Action.ResponsesWebSearchToolCallAction.Sources {
				...
				resultStartEvent := &AnthropicStreamEvent{
					...
					ContentBlock: &AnthropicContentBlock{
						...
						ToolUseID: deepintshieldResp.Item.ID, // Link to the server_tool_use block
			...
			deepintshieldResp.Item.Type != nil &&
			(*deepintshieldResp.Item.Type == schemas.ResponsesMessageTypeFunctionCall ||
				*deepintshieldResp.Item.Type == schemas.ResponsesMessageTypeMCPCall) {
	...
	case schemas.ResponsesStreamResponseTypeCompleted:
		...
		// Convert usage from DeepIntShield to Anthropic
		if deepintshieldResp.Response != nil {
			anthropicContentDeltaEvent.Usage = ConvertDeepIntShieldUsageToAnthropicUsage(deepintshieldResp.Response.Usage)
			if deepintshieldResp.Response.StopReason != nil {
				anthropicContentDeltaEvent.Delta = &AnthropicStreamDelta{
					StopReason:   schemas.Ptr(ConvertDeepIntShieldFinishReasonToAnthropic(*deepintshieldResp.Response.StopReason)),
					StopSequence: nil,
				}
			}
		}
	...
	case schemas.ResponsesStreamResponseTypeMCPCallArgumentsDelta:
		...
		} else if deepintshieldResp.Arguments != nil {
	...
	case schemas.ResponsesStreamResponseTypeMCPCallFailed:
		...
		if deepintshieldResp.Message != nil {
			errorMsg = *deepintshieldResp.Message
		}
	...
	case "message_delta":
		...
		if ctx.Value(schemas.DeepIntShieldContextKeyIntegrationType) == "anthropic" {
			...
			// Convert usage from DeepIntShield format to Anthropic format using common converter
			if deepintshieldResp.Response != nil {
				streamResp.Usage = ConvertDeepIntShieldUsageToAnthropicUsage(deepintshieldResp.Response.Usage)
			}

			// Convert stop reason from DeepIntShield format to Anthropic format
			if deepintshieldResp.Response != nil && deepintshieldResp.Response.StopReason != nil {
				streamResp.Delta = &AnthropicStreamDelta{
					StopReason: schemas.Ptr(ConvertDeepIntShieldFinishReasonToAnthropic(*deepintshieldResp.Response.StopReason)),
				}
			} else if deepintshieldResp.Delta != nil {
	...
	case schemas.ResponsesStreamResponseTypeError:
		...
		if deepintshieldResp.Message != nil {
			streamResp.Error = &AnthropicStreamError{
				Type:    "error",
				Message: *deepintshieldResp.Message,
			}
		}
...
// ToDeepIntShieldResponsesRequest converts an Anthropic message request to DeepIntShield format
func (req *AnthropicMessageRequest) ToDeepIntShieldResponsesRequest(ctx *schemas.DeepIntShieldContext) *schemas.DeepIntShieldResponsesRequest {
	...
	if req.Thinking != nil {
		if req.Thinking.Type == "enabled" || req.Thinking.Type == "adaptive" {
			...
			if req.OutputConfig != nil && req.OutputConfig.Effort != nil {
				// Native effort present - map to DeepIntShield enum (e.g., "max" → "high")
				params.Reasoning = &schemas.ResponsesParametersReasoning{
					Effort:    schemas.Ptr(MapAnthropicEffortToDeepIntShield(*req.OutputConfig.Effort)),
	...
	deepintshieldReq.Params = params
	...
	convertedMessages := ConvertAnthropicMessagesToDeepIntShieldMessages(ctx, req.Messages, req.System, false, provider == schemas.Bedrock)
	deepintshieldMessages = append(deepintshieldMessages, convertedMessages...)

	// Convert tools if present
	if req.Tools != nil {
		var deepintshieldTools []schemas.ResponsesTool
		for _, tool := range req.Tools {
			deepintshieldTool := convertAnthropicToolToDeepIntShield(&tool)
			if deepintshieldTool != nil {
				deepintshieldTools = append(deepintshieldTools, *deepintshieldTool)
			}
		}
		if len(deepintshieldTools) > 0 {
			deepintshieldReq.Params.Tools = deepintshieldTools
		}
	}

	if req.MCPServers != nil {
		var deepintshieldMCPTools []schemas.ResponsesTool
		for _, mcpServer := range req.MCPServers {
			deepintshieldMCPTool := convertAnthropicMCPServerToDeepIntShieldTool(&mcpServer)
			if deepintshieldMCPTool != nil {
				deepintshieldMCPTools = append(deepintshieldMCPTools, *deepintshieldMCPTool)
			}
		}
		if len(deepintshieldMCPTools) > 0 {
			deepintshieldReq.Params.Tools = append(deepintshieldReq.Params.Tools, deepintshieldMCPTools...)
		}
	}

	// Convert tool choice if present
	if req.ToolChoice != nil {
		deepintshieldToolChoice := convertAnthropicToolChoiceToDeepIntShield(req.ToolChoice)
		if deepintshieldToolChoice != nil {
			deepintshieldReq.Params.ToolChoice = deepintshieldToolChoice
		}
	}
...
// ToAnthropicResponsesRequest converts a DeepIntShieldRequest with Responses structure back to AnthropicMessageRequest
func ToAnthropicResponsesRequest(ctx *schemas.DeepIntShieldContext, deepintshieldReq *schemas.DeepIntShieldResponsesRequest) (*AnthropicMessageRequest, error) {
	...
	if deepintshieldReq.Params != nil {
		if deepintshieldReq.Params.MaxOutputTokens != nil {
			anthropicReq.MaxTokens = *deepintshieldReq.Params.MaxOutputTokens
		}
		...
		} else if deepintshieldReq.Params.TopP != nil {
			anthropicReq.TopP = deepintshieldReq.Params.TopP
		}
		if deepintshieldReq.Params.User != nil {
			anthropicReq.Metadata = &AnthropicMetaData{
				UserID: deepintshieldReq.Params.User,
			}
		}
		if deepintshieldReq.Params.Text != nil {
			// Vertex doesn't support native structured outputs, so convert to tool
			if deepintshieldReq.Provider == schemas.Vertex {
				if deepintshieldReq.Params.Text.Format != nil {
					responseFormatTool := convertResponsesTextFormatToTool(ctx, deepintshieldReq.Params.Text)
				...
				for _, message := range deepintshieldReq.Input {
				...
				if !hasCitationsEnabled {
					...
					outputFormat := convertResponsesTextConfigToAnthropicOutputFormat(deepintshieldReq.Params.Text)
					if outputFormat != nil {
		...
		if deepintshieldReq.Params.Reasoning != nil {
			if deepintshieldReq.Params.Reasoning.MaxTokens != nil {
				budgetTokens := *deepintshieldReq.Params.Reasoning.MaxTokens
				...
				if deepintshieldReq.Params.Reasoning.Effort != nil {
					if *deepintshieldReq.Params.Reasoning.Effort != "none" {
						effort := MapDeepIntShieldEffortToAnthropic(*deepintshieldReq.Params.Reasoning.Effort)

						if SupportsAdaptiveThinking(deepintshieldReq.Model) {
						...
						} else if SupportsNativeEffort(deepintshieldReq.Model) {
		...
		anthropicReq.ServiceTier = deepintshieldReq.Params.ServiceTier

		if deepintshieldReq.Params.ExtraParams != nil {
			anthropicReq.ExtraParams = make(map[string]interface{}, len(deepintshieldReq.Params.ExtraParams))
			...
			topK, ok := schemas.SafeExtractIntPointer(deepintshieldReq.Params.ExtraParams["top_k"])
			if ok {
			...
			if inferenceGeo, ok := schemas.SafeExtractStringPointer(deepintshieldReq.Params.ExtraParams["inference_geo"]); ok {
				delete(anthropicReq.ExtraParams, "inference_geo")
				anthropicReq.InferenceGeo = inferenceGeo
			}
			if cmVal := deepintshieldReq.Params.ExtraParams["context_management"]; cmVal != nil {
		...
		if deepintshieldReq.Params.Tools != nil {
			...
			for _, tool := range deepintshieldReq.Params.Tools {
				...
				if tool.Type == schemas.ResponsesToolTypeMCP && tool.ResponsesToolMCP != nil {
					mcpServer := convertDeepIntShieldMCPToolToAnthropicServer(&tool)
					if mcpServer != nil {
						mcpServers = append(mcpServers, *mcpServer)
					}
					continue // Skip converting MCP tools to anthropicTools since they're handled separately
				}
				anthropicTool := convertDeepIntShieldToolToAnthropic(deepintshieldReq.Model, &tool, deepintshieldReq.Provider)
		...
		if deepintshieldReq.Params.ToolChoice != nil {
			anthropicToolChoice := convertResponsesToolChoiceToAnthropic(deepintshieldReq.Params.ToolChoice)
	...
	if deepintshieldReq.Input != nil {
		anthropicMessages, systemContent := ConvertDeepIntShieldMessagesToAnthropicMessages(ctx, deepintshieldReq.Input)

		// Set system message if present
		if systemContent != nil {
			anthropicReq.System = systemContent
		} else if deepintshieldReq.Params != nil && deepintshieldReq.Params.Instructions != nil && *deepintshieldReq.Params.Instructions != "" {
...
// ConvertAnthropicUsageToDeepIntShieldUsage converts Anthropic usage format to DeepIntShield usage format
// Handles iterations recursively
func ConvertAnthropicUsageToDeepIntShieldUsage(anthropicUsage *AnthropicUsage) *schemas.ResponsesResponseUsage {
	if anthropicUsage == nil {
		return nil
	}

	deepintshieldUsage := &schemas.ResponsesResponseUsage{
	...
	if anthropicUsage.CacheReadInputTokens > 0 {
		if deepintshieldUsage.InputTokensDetails == nil {
			deepintshieldUsage.InputTokensDetails = &schemas.ResponsesResponseInputTokens{}
		}
		deepintshieldUsage.InputTokensDetails.CachedReadTokens = anthropicUsage.CacheReadInputTokens
		deepintshieldUsage.InputTokens = deepintshieldUsage.InputTokens + anthropicUsage.CacheReadInputTokens
		deepintshieldUsage.TotalTokens = deepintshieldUsage.TotalTokens + anthropicUsage.CacheReadInputTokens
	}
	...
	if anthropicUsage.CacheCreationInputTokens > 0 {
		...
		deepintshieldUsage.InputTokensDetails.CachedWriteTokens = anthropicUsage.CacheCreationInputTokens
		deepintshieldUsage.InputTokens = deepintshieldUsage.InputTokens + anthropicUsage.CacheCreationInputTokens
		deepintshieldUsage.TotalTokens = deepintshieldUsage.TotalTokens + anthropicUsage.CacheCreationInputTokens
	}
	...
	if anthropicUsage.ServerToolUse != nil && anthropicUsage.ServerToolUse.WebSearchRequests > 0 {
		if deepintshieldUsage.OutputTokensDetails == nil {
			deepintshieldUsage.OutputTokensDetails = &schemas.ResponsesResponseOutputTokens{}
		}
		deepintshieldUsage.OutputTokensDetails.NumSearchQueries = schemas.Ptr(anthropicUsage.ServerToolUse.WebSearchRequests)
	}

	// Recursively convert iterations
	if len(anthropicUsage.Iterations) > 0 {
		deepintshieldUsage.Iterations = make([]schemas.ResponsesResponseUsage, len(anthropicUsage.Iterations))
		for i, iteration := range anthropicUsage.Iterations {
			if converted := ConvertAnthropicUsageToDeepIntShieldUsage(&iteration); converted != nil {
				deepintshieldUsage.Iterations[i] = *converted
			}
		}
	}

	return deepintshieldUsage
}

// ConvertDeepIntShieldUsageToAnthropicUsage converts DeepIntShield usage format to Anthropic usage format
// Handles iterations recursively
func ConvertDeepIntShieldUsageToAnthropicUsage(deepintshieldUsage *schemas.ResponsesResponseUsage) *AnthropicUsage {
	if deepintshieldUsage == nil {
		return nil
	}

	anthropicUsage := &AnthropicUsage{
		Type:         deepintshieldUsage.Type,
		InputTokens:  deepintshieldUsage.InputTokens,
		OutputTokens: deepintshieldUsage.OutputTokens,
	}

	// Handle cache read tokens
	if deepintshieldUsage.InputTokensDetails != nil {
		if deepintshieldUsage.InputTokensDetails.CachedReadTokens > 0 {
			anthropicUsage.CacheReadInputTokens = deepintshieldUsage.InputTokensDetails.CachedReadTokens
			anthropicUsage.InputTokens = anthropicUsage.InputTokens - deepintshieldUsage.InputTokensDetails.CachedReadTokens
		}
		if deepintshieldUsage.InputTokensDetails.CachedWriteTokens > 0 {
			anthropicUsage.CacheCreationInputTokens = deepintshieldUsage.InputTokensDetails.CachedWriteTokens
			anthropicUsage.InputTokens = anthropicUsage.InputTokens - deepintshieldUsage.InputTokensDetails.CachedWriteTokens
		}
	}

	// Handle server tool use statistics (e.g., web search)
	if deepintshieldUsage.OutputTokensDetails != nil && deepintshieldUsage.OutputTokensDetails.NumSearchQueries != nil && *deepintshieldUsage.OutputTokensDetails.NumSearchQueries > 0 {
		anthropicUsage.ServerToolUse = &AnthropicServerToolUseUsage{
			WebSearchRequests: *deepintshieldUsage.OutputTokensDetails.NumSearchQueries,
		}
	}

	// Recursively convert iterations
	if len(deepintshieldUsage.Iterations) > 0 {
		anthropicUsage.Iterations = make([]AnthropicUsage, len(deepintshieldUsage.Iterations))
		for i, iteration := range deepintshieldUsage.Iterations {
			if converted := ConvertDeepIntShieldUsageToAnthropicUsage(&iteration); converted != nil {
...
// ToDeepIntShieldResponsesResponse converts an Anthropic response to DeepIntShieldResponse with Responses structure
func (response *AnthropicMessageResponse) ToDeepIntShieldResponsesResponse(ctx *schemas.DeepIntShieldContext) *schemas.DeepIntShieldResponsesResponse {
	if response == nil {
		return nil
	}

	// Create the DeepIntShieldResponse with Responses structure
	...
	deepintshieldResp.Usage = ConvertAnthropicUsageToDeepIntShieldUsage(response.Usage)
	...
	if len(response.Content) > 0 {
		...
		outputMessages := ConvertAnthropicMessagesToDeepIntShieldMessages(ctx, []AnthropicMessage{tempMsg}, nil, true, false)
		if len(outputMessages) > 0 {
			deepintshieldResp.Output = outputMessages
		}
	}

	deepintshieldResp.Model = response.Model

	// Preserve stop reason from Anthropic response
	if response.StopReason != "" {
		deepintshieldResp.StopReason = schemas.Ptr(string(response.StopReason))
	}

	return deepintshieldResp
}

// ToAnthropicResponsesResponse converts a DeepIntShieldResponse with Responses structure back to AnthropicMessageResponse
func ToAnthropicResponsesResponse(ctx *schemas.DeepIntShieldContext, deepintshieldResp *schemas.DeepIntShieldResponsesResponse) *AnthropicMessageResponse {
	anthropicResp := &AnthropicMessageResponse{
		Type: "message",
		Role: "assistant",
	}
	if deepintshieldResp.ID != nil {
		anthropicResp.ID = *deepintshieldResp.ID
	}

	// Convert usage information using common converter (handles iterations recursively)
	anthropicResp.Usage = ConvertDeepIntShieldUsageToAnthropicUsage(deepintshieldResp.Usage)

	// Convert output messages to Anthropic content blocks using the new conversion method
	var contentBlocks []AnthropicContentBlock
	if deepintshieldResp.Output != nil {
		anthropicMessages, _ := ConvertDeepIntShieldMessagesToAnthropicMessages(ctx, deepintshieldResp.Output)
	...
	// Map stop reason from DeepIntShield response if available, otherwise infer from content
	if deepintshieldResp.StopReason != nil {
		anthropicResp.StopReason = ConvertDeepIntShieldFinishReasonToAnthropic(*deepintshieldResp.StopReason)
	} else {
	...
	anthropicResp.Model = deepintshieldResp.Model

	return anthropicResp
}

// ConvertAnthropicMessagesToDeepIntShieldMessages converts an array of Anthropic messages to DeepIntShield ResponsesMessage format
func ConvertAnthropicMessagesToDeepIntShieldMessages(ctx *schemas.DeepIntShieldContext, anthropicMessages []AnthropicMessage, systemContent *AnthropicContent, isOutputMessage bool, keepToolsGrouped bool) []schemas.ResponsesMessage {
	var deepintshieldMessages []schemas.ResponsesMessage
	...
	if systemContent != nil {
		systemMessages := convertAnthropicSystemToDeepIntShieldMessages(systemContent)
		deepintshieldMessages = append(deepintshieldMessages, systemMessages...)
	}
	...
	for _, msg := range anthropicMessages {
		...
		if keepToolsGrouped {
			convertedMessages = convertSingleAnthropicMessageToDeepIntShieldMessagesGrouped(&msg, isOutputMessage, structuredOutputToolName)
		} else {
			convertedMessages = convertSingleAnthropicMessageToDeepIntShieldMessages(ctx, &msg, isOutputMessage, structuredOutputToolName)
		}
		deepintshieldMessages = append(deepintshieldMessages, convertedMessages...)
	}
...
// ConvertDeepIntShieldMessagesToAnthropicMessages converts an array of DeepIntShield ResponsesMessage to Anthropic message format
// This is the main conversion method from DeepIntShield to Anthropic - handles all message types and returns messages + system content
func ConvertDeepIntShieldMessagesToAnthropicMessages(ctx *schemas.DeepIntShieldContext, deepintshieldMessages []schemas.ResponsesMessage) ([]AnthropicMessage, *AnthropicContent) {
	...
	for _, msg := range deepintshieldMessages {
		...
		case schemas.ResponsesMessageTypeMessage:
			...
			if msg.Role != nil && *msg.Role == schemas.ResponsesInputMessageRoleSystem {
				systemContent = convertDeepIntShieldMessageToAnthropicSystemContent(&msg)
				continue
			}
			...
			anthropicMsg := convertDeepIntShieldMessageToAnthropicMessage(&msg, &pendingReasoningContentBlocks)
			if anthropicMsg != nil {
		...
		case schemas.ResponsesMessageTypeReasoning:
			...
			reasoningBlocks := convertDeepIntShieldReasoningToAnthropicThinking(&msg)
		...
		case schemas.ResponsesMessageTypeFunctionCall:
			...
			toolUseBlock := convertDeepIntShieldFunctionCallToAnthropicToolUse(ctx, &msg)
			if toolUseBlock != nil {
		...
		case schemas.ResponsesMessageTypeFunctionCallOutput:
			...
			toolResultBlock := convertDeepIntShieldFunctionCallOutputToAnthropicToolResultBlock(&msg)
		...
		case schemas.ResponsesMessageTypeItemReference:
			...
			referenceMsg := convertDeepIntShieldItemReferenceToAnthropicMessage(&msg)
			if referenceMsg != nil {
		...
		case schemas.ResponsesMessageTypeComputerCall:
			...
			computerToolUseBlock := convertDeepIntShieldComputerCallToAnthropicToolUse(&msg)
		...
		case schemas.ResponsesMessageTypeMCPCall:
			...
			if msg.ResponsesToolMessage != nil {
				if msg.ResponsesToolMessage.Name != nil {
					...
					mcpToolUseBlock := convertDeepIntShieldMCPCallToAnthropicToolUse(&msg)
					...
					mcpToolResultBlock := convertDeepIntShieldMCPCallOutputToAnthropicToolResultBlock(&msg)
		...
		case schemas.ResponsesMessageTypeMCPApprovalRequest:
			...
			mcpApprovalBlock := convertDeepIntShieldMCPApprovalToAnthropicToolUse(&msg)
		...
		case schemas.ResponsesMessageTypeWebSearchCall:
			...
			webSearchBlocks := convertDeepIntShieldWebSearchCallToAnthropicBlocks(&msg)
		...
		case schemas.ResponsesMessageTypeFileSearchCall,
			...
			unsupportedToolMsg := convertDeepIntShieldUnsupportedToolCallToAnthropicMessage(&msg, msgType)
		...
		case schemas.ResponsesMessageTypeComputerCallOutput:
			...
			computerResultBlock := convertDeepIntShieldComputerCallOutputToAnthropicToolResultBlock(&msg)
		...
		case schemas.ResponsesMessageTypeLocalShellCallOutput,
			...
			toolOutputMsg := convertDeepIntShieldToolOutputToAnthropicMessage(&msg)
...
// Helper function to convert Anthropic system content to DeepIntShield messages
func convertAnthropicSystemToDeepIntShieldMessages(systemContent *AnthropicContent) []schemas.ResponsesMessage {
	var deepintshieldMessages []schemas.ResponsesMessage
...
// Helper function to convert a single Anthropic message to DeepIntShield messages
func convertSingleAnthropicMessageToDeepIntShieldMessages(ctx *schemas.DeepIntShieldContext, msg *AnthropicMessage, isOutputMessage bool, structuredOutputToolName string) []schemas.ResponsesMessage {
...
// Helper function to convert a single Anthropic message to DeepIntShield messages, grouping text and tool calls
// This keeps assistant messages with mixed text and tool_use blocks together
func convertSingleAnthropicMessageToDeepIntShieldMessagesGrouped(msg *AnthropicMessage, isOutputMessage bool, structuredOutputToolName string) []schemas.ResponsesMessage {
	// Determine if this message should use output types based on role
	// Assistant messages in conversation history should use output_text
	isOutput := isOutputMessage || msg.Role == AnthropicMessageRoleAssistant
...
// Helper function to convert Anthropic content blocks to DeepIntShield ResponsesMessages, grouping text and tool_use blocks
func convertAnthropicContentBlocksToResponsesMessagesGrouped(contentBlocks []AnthropicContentBlock, role *schemas.ResponsesMessageRoleType, isOutputMessage bool) []schemas.ResponsesMessage {
	...
	for _, block := range contentBlocks {
		...
		case AnthropicContentBlockTypeText:
			if block.Text != nil {
				if isOutputMessage {
					...
					deepintshieldMessages = append(deepintshieldMessages, deepintshieldMsg)
				}
			}
		...
		case AnthropicContentBlockTypeImage:
			...
			if block.Source != nil {
				deepintshieldMsg := schemas.ResponsesMessage{
					...
					Role: role,
					Content: &schemas.ResponsesMessageContent{
						ContentBlocks: []schemas.ResponsesMessageContentBlock{block.toDeepIntShieldResponsesImageBlock()},
					},
				}
				if isOutputMessage {
					deepintshieldMsg.ID = schemas.Ptr("msg_" + providerUtils.GetRandomString(50))
				}
				deepintshieldMessages = append(deepintshieldMessages, deepintshieldMsg)
			}
		...
		case AnthropicContentBlockTypeDocument:
			...
			if block.Source != nil {
				deepintshieldMsg := schemas.ResponsesMessage{
					...
					Role: role,
					Content: &schemas.ResponsesMessageContent{
						ContentBlocks: []schemas.ResponsesMessageContentBlock{block.toDeepIntShieldResponsesDocumentBlock()},
					},
				}
				if isOutputMessage {
					deepintshieldMsg.ID = schemas.Ptr("msg_" + providerUtils.GetRandomString(50))
		...
		case AnthropicContentBlockTypeToolResult:
			...
			if block.ToolUseID != nil {
				if block.Content != nil {
					...
					deepintshieldMsg.ResponsesToolMessage.Output = &schemas.ResponsesToolMessageOutputStruct{}

					if block.Content.ContentStr != nil {
						deepintshieldMsg.ResponsesToolMessage.Output.ResponsesToolCallOutputStr = block.Content.ContentStr
						...
						for _, contentBlock := range block.Content.ContentBlocks {
							...
							case AnthropicContentBlockTypeImage:
								if contentBlock.Source != nil {
									toolMsgContentBlocks = append(toolMsgContentBlocks, contentBlock.toDeepIntShieldResponsesImageBlock())
								}
							}
						}
						deepintshieldMsg.ResponsesToolMessage.Output.ResponsesFunctionToolCallOutputBlocks = toolMsgContentBlocks
					}

					// Handle is_error from Anthropic
					if block.IsError != nil && *block.IsError {
						deepintshieldMsg.Status = schemas.Ptr("incomplete")
					}
		...
		case AnthropicContentBlockTypeWebSearchResult:
			...
			if block.ToolUseID != nil {
				attachWebSearchSourcesToCall(deepintshieldMessages, *block.ToolUseID, block, true)
			}
		}
	}
	...
	if len(accumulatedTextContent) > 0 {
		...
		if isOutputMessage {
			deepintshieldMsg.ID = schemas.Ptr("msg_" + providerUtils.GetRandomString(50))
			deepintshieldMsg.Content = &schemas.ResponsesMessageContent{
	...
	if len(pendingToolUseBlocks) > 0 {
		for _, toolBlock := range pendingToolUseBlocks {
			...
			if isOutputMessage {
				deepintshieldMsg.ID = schemas.Ptr("fc_" + providerUtils.GetRandomString(50))
			}

			// Check for computer tool use
			if toolBlock.Name != nil && *toolBlock.Name == string(AnthropicToolNameComputer) {
				deepintshieldMsg.Type = schemas.Ptr(schemas.ResponsesMessageTypeComputerCall)
				deepintshieldMsg.ResponsesToolMessage.Name = nil
				var inputMap map[string]interface{}
				if err := sonic.Unmarshal(toolBlock.Input, &inputMap); err == nil {
					deepintshieldMsg.ResponsesToolMessage.Action = &schemas.ResponsesToolMessageActionStruct{
				...
				deepintshieldMsg.Type = schemas.Ptr(schemas.ResponsesMessageTypeWebSearchCall)
				deepintshieldMsg.ResponsesToolMessage.Name = nil
				...
				deepintshieldMsg.Type = schemas.Ptr(schemas.ResponsesMessageTypeWebFetchCall)
				deepintshieldMsg.ResponsesToolMessage.Name = nil
				...
				if len(toolBlock.Input) > 0 {
					deepintshieldMsg.ResponsesToolMessage.Arguments = schemas.Ptr(string(toolBlock.Input))
				}
			}
...
// Helper function to convert Anthropic content blocks to DeepIntShield ResponsesMessages
func convertAnthropicContentBlocksToResponsesMessages(ctx *schemas.DeepIntShieldContext, contentBlocks []AnthropicContentBlock, role *schemas.ResponsesMessageRoleType, isOutputMessage bool, structuredOutputToolName string) []schemas.ResponsesMessage {
	var deepintshieldMessages []schemas.ResponsesMessage
	...
	for _, block := range contentBlocks {
		...
		case AnthropicContentBlockTypeText:
			if block.Text != nil {
				var deepintshieldMsg schemas.ResponsesMessage
				if isOutputMessage {
			...
			if structuredOutputToolName != "" && block.Name != nil && *block.Name == structuredOutputToolName {
				...
				if block.ID != nil && block.Name != nil {
					...
					if block.Name != nil && *block.Name == string(AnthropicToolNameComputer) {
						...
						deepintshieldMsg.ResponsesToolMessage.Arguments = schemas.Ptr(string(block.Input))
					}
					deepintshieldMessages = append(deepintshieldMessages, deepintshieldMsg)
				}
			}
		...
		case AnthropicContentBlockTypeWebSearchToolResultError:
			...
			if block.ToolUseID != nil {
				for i := len(deepintshieldMessages) - 1; i >= 0; i-- {
					msg := &deepintshieldMessages[i]
		...
		case AnthropicContentBlockTypeMCPToolUse:
			...
			if block.ID != nil && block.Name != nil {
				...
				if block.ServerName != nil {
					deepintshieldMsg.ResponsesToolMessage.ResponsesMCPToolCall = &schemas.ResponsesMCPToolCall{
	...
	if len(reasoningContentBlocks) > 0 {
		...
		deepintshieldMessages = append([]schemas.ResponsesMessage{reasoningMessage}, deepintshieldMessages...)
	}

	return deepintshieldMessages
}

// Helper functions for converting individual DeepIntShield message types to Anthropic messages
// convertDeepIntShieldMessageToAnthropicSystemContent converts a DeepIntShield system message to Anthropic system content
func convertDeepIntShieldMessageToAnthropicSystemContent(msg *schemas.ResponsesMessage) *AnthropicContent {
	if msg.Content != nil {
		if msg.Content.ContentStr != nil {
			...
			contentBlocks := convertDeepIntShieldContentBlocksToAnthropic(msg.Content.ContentBlocks)
			if len(contentBlocks) > 0 {
				return &AnthropicContent{
					ContentBlocks: contentBlocks,
				}
...
// convertDeepIntShieldMessageToAnthropicMessage converts a regular DeepIntShield message to Anthropic message
func convertDeepIntShieldMessageToAnthropicMessage(msg *schemas.ResponsesMessage, pendingReasoningContentBlocks *[]AnthropicContentBlock) *AnthropicMessage {
		...
		*pendingReasoningContentBlocks = nil
		...
		if msg.Content != nil {
			if msg.Content.ContentStr != nil {
				...
				contentBlocks = append(contentBlocks, convertDeepIntShieldContentBlocksToAnthropic(msg.Content.ContentBlocks)...)
			}
		}
...
// convertDeepIntShieldReasoningToAnthropicThinking converts a DeepIntShield reasoning message to Anthropic thinking blocks
func convertDeepIntShieldReasoningToAnthropicThinking(msg *schemas.ResponsesMessage) []AnthropicContentBlock {
...
// convertDeepIntShieldFunctionCallToAnthropicToolUse converts a DeepIntShield function call to Anthropic tool use
func convertDeepIntShieldFunctionCallToAnthropicToolUse(ctx *schemas.DeepIntShieldContext, msg *schemas.ResponsesMessage) *AnthropicContentBlock {
	if msg.ResponsesToolMessage != nil {
...
// convertDeepIntShieldFunctionCallOutputToAnthropicToolResultBlock converts a DeepIntShield function call output to a single tool result block
// This is used to accumulate multiple tool results into a single user message
func convertDeepIntShieldFunctionCallOutputToAnthropicToolResultBlock(msg *schemas.ResponsesMessage) *AnthropicContentBlock {
	if msg.ResponsesToolMessage != nil {
...
// convertDeepIntShieldComputerCallOutputToAnthropicToolResultBlock converts a DeepIntShield computer call output to a single tool result block
// This is used to accumulate multiple tool results into a single user message
func convertDeepIntShieldComputerCallOutputToAnthropicToolResultBlock(msg *schemas.ResponsesMessage) *AnthropicContentBlock {
...
// convertDeepIntShieldMCPCallOutputToAnthropicToolResultBlock converts a DeepIntShield MCP call output to a single tool result block
// This is used to accumulate multiple tool results into a single user message
func convertDeepIntShieldMCPCallOutputToAnthropicToolResultBlock(msg *schemas.ResponsesMessage) *AnthropicContentBlock {
	if msg.ResponsesToolMessage != nil && msg.ResponsesToolMessage.CallID != nil {
		toolResultBlock := AnthropicContentBlock{
...
// convertDeepIntShieldItemReferenceToAnthropicMessage converts a DeepIntShield item reference to Anthropic message
func convertDeepIntShieldItemReferenceToAnthropicMessage(msg *schemas.ResponsesMessage) *AnthropicMessage {
...
// convertDeepIntShieldComputerCallToAnthropicToolUse converts a DeepIntShield computer call to Anthropic tool use
func convertDeepIntShieldComputerCallToAnthropicToolUse(msg *schemas.ResponsesMessage) *AnthropicContentBlock {
	if msg.ResponsesToolMessage != nil {
		toolUseBlock := AnthropicContentBlock{
			Type: AnthropicContentBlockTypeToolUse,
...
// convertDeepIntShieldMCPCallToAnthropicToolUse converts a DeepIntShield MCP call to Anthropic tool use
func convertDeepIntShieldMCPCallToAnthropicToolUse(msg *schemas.ResponsesMessage) *AnthropicContentBlock {
...
// convertDeepIntShieldMCPCallOutputToAnthropicMessage converts a DeepIntShield MCP call output to Anthropic message
func convertDeepIntShieldMCPCallOutputToAnthropicMessage(msg *schemas.ResponsesMessage) *AnthropicMessage {
	toolResultBlock := AnthropicContentBlock{
		Type: AnthropicContentBlockTypeMCPToolResult,
...
// convertDeepIntShieldMCPApprovalToAnthropicToolUse converts a DeepIntShield MCP approval request to Anthropic tool use
func convertDeepIntShieldMCPApprovalToAnthropicToolUse(msg *schemas.ResponsesMessage) *AnthropicContentBlock {
	if msg.ResponsesToolMessage != nil && msg.ResponsesToolMessage.Name != nil {
		toolUseBlock := AnthropicContentBlock{
			Type: AnthropicContentBlockTypeMCPToolUse,
		}
...
// convertDeepIntShieldWebSearchCallToAnthropicBlocks converts a DeepIntShield web_search_call to Anthropic server_tool_use and web_search_tool_result blocks
func convertDeepIntShieldWebSearchCallToAnthropicBlocks(msg *schemas.ResponsesMessage) []AnthropicContentBlock {
...
// convertDeepIntShieldUnsupportedToolCallToAnthropicMessage converts unsupported tool calls to text messages
func convertDeepIntShieldUnsupportedToolCallToAnthropicMessage(msg *schemas.ResponsesMessage, msgType schemas.ResponsesMessageType) *AnthropicMessage {
	if msg.ResponsesToolMessage != nil {
...
// convertDeepIntShieldComputerCallOutputToAnthropicMessage converts a DeepIntShield computer call output to Anthropic message
func convertDeepIntShieldComputerCallOutputToAnthropicMessage(msg *schemas.ResponsesMessage) *AnthropicMessage {
	if msg.ResponsesToolMessage != nil {
		toolResultBlock := AnthropicContentBlock{
			Type:      AnthropicContentBlockTypeToolResult,
			ToolUseID: msg.ResponsesToolMessage.CallID,
...
// convertDeepIntShieldToolOutputToAnthropicMessage converts tool outputs to user messages
func convertDeepIntShieldToolOutputToAnthropicMessage(msg *schemas.ResponsesMessage) *AnthropicMessage {
	if msg.ResponsesToolMessage != nil {
...
// convertAnthropicToolToDeepIntShield converts AnthropicTool to schemas.Tool
func convertAnthropicToolToDeepIntShield(tool *AnthropicTool) *schemas.ResponsesTool {
	if tool == nil {
		return nil
	}
	...
	if tool.Type != nil {
		...
		case AnthropicToolTypeComputer20250124, AnthropicToolTypeComputer20251124:
			...
			if tool.AnthropicToolComputerUse != nil {
				deepintshieldTool.ResponsesToolComputerUsePreview = &schemas.ResponsesToolComputerUsePreview{
					Environment: "browser", // Default environment
				}
				if tool.AnthropicToolComputerUse.DisplayWidthPx != nil {
					deepintshieldTool.ResponsesToolComputerUsePreview.DisplayWidth = *tool.AnthropicToolComputerUse.DisplayWidthPx
				}
				if tool.AnthropicToolComputerUse.DisplayHeightPx != nil {
					deepintshieldTool.ResponsesToolComputerUsePreview.DisplayHeight = *tool.AnthropicToolComputerUse.DisplayHeightPx
				}
				if tool.AnthropicToolComputerUse.EnableZoom != nil {
					deepintshieldTool.ResponsesToolComputerUsePreview.EnableZoom = tool.AnthropicToolComputerUse.EnableZoom
				}
			}
			return deepintshieldTool
		...
		case AnthropicToolTypeWebSearch20250305, AnthropicToolTypeWebSearch20260209:
			...
			if tool.AnthropicToolWebSearch != nil {
				deepintshieldTool.ResponsesToolWebSearch = &schemas.ResponsesToolWebSearch{
				...
				if tool.AnthropicToolWebSearch.MaxUses != nil {
					deepintshieldTool.ResponsesToolWebSearch.MaxUses = tool.AnthropicToolWebSearch.MaxUses
				}
				if tool.AnthropicToolWebSearch.UserLocation != nil {
					deepintshieldTool.ResponsesToolWebSearch.UserLocation = &schemas.ResponsesToolWebSearchUserLocation{
		...
		case AnthropicToolTypeWebFetch20250910, AnthropicToolTypeWebFetch20260209, AnthropicToolTypeWebFetch20260309:
			...
			if tool.AnthropicToolWebFetch != nil {
				deepintshieldTool.ResponsesToolWebFetch = &schemas.ResponsesToolWebFetch{
				...
				if len(tool.AnthropicToolWebFetch.AllowedDomains) > 0 || len(tool.AnthropicToolWebFetch.BlockedDomains) > 0 {
					deepintshieldTool.ResponsesToolWebFetch.Filters = &schemas.ResponsesToolWebSearchFilters{
	...
	if tool.InputSchema != nil || tool.Strict != nil {
		deepintshieldTool.ResponsesToolFunction = &schemas.ResponsesToolFunction{
	...
	if tool.CacheControl != nil {
		deepintshieldTool.CacheControl = tool.CacheControl
	}

	return deepintshieldTool
}

// convertAnthropicToolChoiceToDeepIntShield converts AnthropicToolChoice to schemas.ToolChoice
func convertAnthropicToolChoiceToDeepIntShield(toolChoice *AnthropicToolChoice) *schemas.ResponsesToolChoice {
	if toolChoice == nil {
		return nil
	}

	deepintshieldToolChoice := &schemas.ResponsesToolChoice{}

	// Handle string format
	if toolChoice.Type != "" {
		switch toolChoice.Type {
		case "auto":
			deepintshieldToolChoice.ResponsesToolChoiceStr = schemas.Ptr(string(schemas.ResponsesToolChoiceTypeAuto))
		case "any":
			deepintshieldToolChoice.ResponsesToolChoiceStr = schemas.Ptr(string(schemas.ResponsesToolChoiceTypeAny))
		case "none":
			deepintshieldToolChoice.ResponsesToolChoiceStr = schemas.Ptr(string(schemas.ResponsesToolChoiceTypeNone))
		case "tool":
			// Handle forced tool choice with specific function name
			deepintshieldToolChoice.ResponsesToolChoiceStruct = &schemas.ResponsesToolChoiceStruct{
				Type: schemas.ResponsesToolChoiceTypeFunction,
				Name: &toolChoice.Name,
			}
			return deepintshieldToolChoice
		default:
			deepintshieldToolChoice.ResponsesToolChoiceStr = schemas.Ptr(string(schemas.ResponsesToolChoiceTypeAuto))
		}
	}
...
func convertDeepIntShieldToolToAnthropic(model string, tool *schemas.ResponsesTool, provider schemas.ModelProvider) *AnthropicTool {
	if tool == nil {
		return nil
	}
...
// Helper to convert DeepIntShield content blocks slice to Anthropic content blocks
func convertDeepIntShieldContentBlocksToAnthropic(blocks []schemas.ResponsesMessageContentBlock) []AnthropicContentBlock {
	if len(blocks) == 0 {
		return nil
	}
...
func (block AnthropicContentBlock) toDeepIntShieldResponsesImageBlock() schemas.ResponsesMessageContentBlock {
...
func (block AnthropicContentBlock) toDeepIntShieldResponsesDocumentBlock() schemas.ResponsesMessageContentBlock {
...
// convertAnthropicMCPServerToDeepIntShieldTool converts a single Anthropic MCP server to a DeepIntShield ResponsesTool
func convertAnthropicMCPServerToDeepIntShieldTool(mcpServer *AnthropicMCPServer) *schemas.ResponsesTool {
	if mcpServer == nil {
		return nil
	}
	...
	if mcpServer.URL != "" {
		deepintshieldTool.ResponsesToolMCP.ServerURL = schemas.Ptr(mcpServer.URL)
	}

	// Set authorization token if present
	if mcpServer.AuthorizationToken != nil {
		deepintshieldTool.ResponsesToolMCP.Authorization = mcpServer.AuthorizationToken
	}
	...
	if mcpServer.ToolConfiguration != nil && len(mcpServer.ToolConfiguration.AllowedTools) > 0 {
		deepintshieldTool.ResponsesToolMCP.AllowedTools = &schemas.ResponsesToolMCPAllowedTools{
...
// convertDeepIntShieldMCPToolToAnthropicServer converts a DeepIntShield MCP tool back to an Anthropic MCP server
func convertDeepIntShieldMCPToolToAnthropicServer(tool *schemas.ResponsesTool) *AnthropicMCPServer {
