...
func TestToOpenAIChatRequest_ToolNormalization(t *testing.T) {
	...
	defer cancel()
	result := ToOpenAIChatRequest(ctx, deepintshieldReq)
	if result == nil {
	...
	// Verify original deepintshieldReq.Params.Tools was NOT mutated
	origKeys := deepintshieldReq.Params.Tools[0].Function.Parameters.Properties.Keys()
	...
	if result.ChatParameters.Tools[0].Function == deepintshieldReq.Params.Tools[0].Function {
