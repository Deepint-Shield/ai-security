...
// CustomResponseHandler is a function that produces a DeepIntShield response from a DeepIntShield request.
// T is the concrete DeepIntShield response type (e.g. DeepIntShieldEmbeddingResponse, DeepIntShieldTextCompletionResponse, DeepIntShieldChatResponse, DeepIntShieldResponsesResponse, DeepIntShieldImageGenerationResponse, DeepIntShieldTranscriptionResponse).
type responseHandler[T any] func(responseBody []byte, response *T, requestBody []byte, sendBackRawRequest bool, sendBackRawResponse bool) (rawRequest interface{}, rawResponse interface{}, deepintshieldErr *schemas.DeepIntShieldError)

func ConvertOpenAIMessagesToDeepIntShieldMessages(messages []OpenAIMessage) []schemas.ChatMessage {
	deepintshieldMessages := make([]schemas.ChatMessage, len(messages))
	for i, message := range messages {
		deepintshieldMessages[i] = schemas.ChatMessage{
			Name:            message.Name,
			Role:            message.Role,
		...
		if message.OpenAIChatAssistantMessage != nil {
			deepintshieldMessages[i].ChatAssistantMessage = &schemas.ChatAssistantMessage{
...
func ConvertDeepIntShieldMessagesToOpenAIMessages(messages []schemas.ChatMessage) []OpenAIMessage {
