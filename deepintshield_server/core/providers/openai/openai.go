// Package openai provides the OpenAI provider implementation for the DeepIntShield framework.
package openai
...
func NewOpenAIProvider(config *schemas.ProviderConfig, logger schemas.Logger) *OpenAIProvider {
	...
	// 	openAIResponsePool.Put(&schemas.DeepIntShieldResponse{})
	// }
...
func (provider *OpenAIProvider) buildRequestURL(ctx *schemas.DeepIntShieldContext, defaultPath string, requestType schemas.RequestType) string {
...
func (provider *OpenAIProvider) ListModels(ctx *schemas.DeepIntShieldContext, keys []schemas.Key, request *schemas.DeepIntShieldListModelsRequest) (*schemas.DeepIntShieldListModelsResponse, *schemas.DeepIntShieldError) {
	...
	if provider.customProviderConfig != nil && provider.customProviderConfig.IsKeyLess {
		return providerUtils.HandleKeylessListModelsRequest(providerName, func() (*schemas.DeepIntShieldListModelsResponse, *schemas.DeepIntShieldError) {
...
func ListModelsByKey(
	...
	if resp.StatusCode() != fasthttp.StatusOK {
		deepintshieldErr := ParseOpenAIError(resp, schemas.ListModelsRequest, providerName, "")
		return nil, deepintshieldErr
	}
	...
	rawRequest, rawResponse, deepintshieldErr := providerUtils.HandleProviderResponse(responseBody, openaiResponse, nil, sendBackRawRequest, sendBackRawResponse)
	if deepintshieldErr != nil {
		return nil, deepintshieldErr
	}

	response := openaiResponse.ToDeepIntShieldListModelsResponse(providerName, key.Models, unfiltered)
...
func HandleOpenAIListModelsRequest(
	...
	listModelsByKeyWrapper := func(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldListModelsRequest) (*schemas.DeepIntShieldListModelsResponse, *schemas.DeepIntShieldError) {
...
func (provider *OpenAIProvider) TextCompletion(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldTextCompletionRequest) (*schemas.DeepIntShieldTextCompletionResponse, *schemas.DeepIntShieldError) {
...
func HandleOpenAITextCompletionRequest(
	...
	sendBackRawRequest bool,
	sendBackRawResponse bool,
	customResponseHandler responseHandler[schemas.DeepIntShieldTextCompletionResponse],
	...
	if lpResult, lpErr, handled := handleOpenAILargePayloadPassthrough(ctx, client, url, key, extraHeaders, providerName, request.Model, schemas.TextCompletionRequest, logger); handled {
		...
		if len(lpResult.ResponseBody) > 0 {
			...
			response.ExtraFields = schemas.DeepIntShieldResponseExtraFields{Provider: providerName, ModelRequested: request.Model, RequestType: schemas.TextCompletionRequest, Latency: lpResult.Latency}
			return response, nil
		}
		return &schemas.DeepIntShieldTextCompletionResponse{
			Model:       request.Model,
			Usage:       lpResult.Usage,
			ExtraFields: schemas.DeepIntShieldResponseExtraFields{Provider: providerName, ModelRequested: request.Model, RequestType: schemas.TextCompletionRequest, Latency: lpResult.Latency},
		}, nil
	}
	...
	defer wait()
	if deepintshieldErr != nil {
		return nil, providerUtils.EnrichError(ctx, deepintshieldErr, jsonData, nil, sendBackRawRequest, sendBackRawResponse)
	}
	// Extract provider response headers early so they're available on error paths too
	providerResponseHeaders := providerUtils.ExtractProviderResponseHeaders(resp)
	ctx.SetValue(schemas.DeepIntShieldContextKeyProviderResponseHeaders, providerResponseHeaders)
	...
	if customResponseHandler != nil {
		rawRequest, rawResponse, deepintshieldErr = customResponseHandler(body, response, jsonData, sendBackRawRequest, sendBackRawResponse)
	} else {
		rawRequest, rawResponse, deepintshieldErr = providerUtils.HandleProviderResponse(body, response, jsonData, sendBackRawRequest, sendBackRawResponse)
	}

	if deepintshieldErr != nil {
		return nil, providerUtils.EnrichError(ctx, deepintshieldErr, jsonData, body, sendBackRawRequest, sendBackRawResponse)
	}
...
func (provider *OpenAIProvider) TextCompletionStream(ctx *schemas.DeepIntShieldContext, postHookRunner schemas.PostHookRunner, key schemas.Key, request *schemas.DeepIntShieldTextCompletionRequest) (chan *schemas.DeepIntShieldStreamChunk, *schemas.DeepIntShieldError) {
...
func HandleOpenAITextCompletionStreaming(
	...
	customResponseHandler responseHandler[schemas.DeepIntShieldTextCompletionResponse],
	postResponseConverter func(*schemas.DeepIntShieldTextCompletionResponse) *schemas.DeepIntShieldTextCompletionResponse,
	logger schemas.Logger,
	...
	go func() {
		...
		for {
			...
			var response schemas.DeepIntShieldTextCompletionResponse
			if customResponseHandler != nil {
				...
				if handlerErr != nil {
					// TODO fix this
					handlerErr.ExtraFields = schemas.DeepIntShieldErrorExtraFields{
					...
					ctx.SetValue(schemas.DeepIntShieldContextKeyStreamEndIndicator, true)
					providerUtils.ProcessAndSendDeepIntShieldError(ctx, postHookRunner, providerUtils.EnrichError(ctx, handlerErr, jsonBody, nil, sendBackRawRequest, sendBackRawResponse), responseChan, logger)
					return
				}
			} else {
				...
				if errorNode, _ := sonic.GetFromString(jsonData, "error"); errorNode.Exists() {
					...
					if err := sonic.UnmarshalString(jsonData, &deepintshieldErr); err == nil {
						if deepintshieldErr.Error != nil && deepintshieldErr.Error.Message != "" {
							...
							ctx.SetValue(schemas.DeepIntShieldContextKeyStreamEndIndicator, true)
							providerUtils.ProcessAndSendDeepIntShieldError(ctx, postHookRunner, providerUtils.EnrichError(ctx, &deepintshieldErr, jsonBody, nil, sendBackRawRequest, sendBackRawResponse), responseChan, logger)
							return
						}
					}
				}

				// Parse into deepintshield response
			...
			if response.Choices == nil {
				response.Choices = []schemas.DeepIntShieldResponseChoice{}
			}
			...
			if choice.TextCompletionResponseChoice != nil && choice.TextCompletionResponseChoice.Text != nil {
				...
				providerUtils.ProcessAndSendResponse(ctx, postHookRunner, providerUtils.GetDeepIntShieldResponseForStreamResponse(&response, nil, nil, nil, nil, nil), responseChan)
			}
		...
		response := providerUtils.CreateDeepIntShieldTextCompletionChunkResponse(messageID, usage, finishReason, chunkIndex, schemas.TextCompletionStreamRequest, providerName, request.Model)
		if postResponseConverter != nil {
...
func (provider *OpenAIProvider) ChatCompletion(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldChatRequest) (*schemas.DeepIntShieldChatResponse, *schemas.DeepIntShieldError) {
...
func HandleOpenAIChatCompletionRequest(
	...
	sendBackRawResponse bool,
	providerName schemas.ModelProvider,
	customResponseHandler responseHandler[schemas.DeepIntShieldChatResponse],
	customErrorConverter ErrorConverter,
	logger schemas.Logger,
	...
	if lpResult, lpErr, handled := handleOpenAILargePayloadPassthrough(ctx, client, url, key, extraHeaders, providerName, request.Model, schemas.ChatCompletionRequest, logger); handled {
		...
		if len(lpResult.ResponseBody) > 0 {
			...
			response.ExtraFields = schemas.DeepIntShieldResponseExtraFields{Provider: providerName, ModelRequested: request.Model, RequestType: schemas.ChatCompletionRequest, Latency: lpResult.Latency}
			return response, nil
		}
		return &schemas.DeepIntShieldChatResponse{
			Model:       request.Model,
			Usage:       lpResult.Usage,
			ExtraFields: schemas.DeepIntShieldResponseExtraFields{Provider: providerName, ModelRequested: request.Model, RequestType: schemas.ChatCompletionRequest, Latency: lpResult.Latency},
		}, nil
	}
...
func (provider *OpenAIProvider) ChatCompletionStream(ctx *schemas.DeepIntShieldContext, postHookRunner schemas.PostHookRunner, key schemas.Key, request *schemas.DeepIntShieldChatRequest) (chan *schemas.DeepIntShieldStreamChunk, *schemas.DeepIntShieldError) {
...
func HandleOpenAIChatCompletionStreaming(
	...
	providerName schemas.ModelProvider,
	postHookRunner schemas.PostHookRunner,
	customRequestConverter func(*schemas.DeepIntShieldChatRequest) (providerUtils.RequestBodyWithExtraParams, error),
	customResponseHandler responseHandler[schemas.DeepIntShieldChatResponse],
	customErrorConverter ErrorConverter,
	postRequestConverter func(*OpenAIChatRequest) *OpenAIChatRequest,
	postResponseConverter func(*schemas.DeepIntShieldChatResponse) *schemas.DeepIntShieldChatResponse,
	logger schemas.Logger,
) (chan *schemas.DeepIntShieldStreamChunk, *schemas.DeepIntShieldError) {
	...
	if ctx.Value(schemas.DeepIntShieldContextKeyIsResponsesToChatCompletionFallback) != nil {
		isResponsesToChatCompletionsFallbackValue, ok := ctx.Value(schemas.DeepIntShieldContextKeyIsResponsesToChatCompletionFallback).(bool)
	...
	go func() {
		...
		for {
			...
			// Parse into deepintshield response
			var response schemas.DeepIntShieldChatResponse
			// TODO fix this
			if customResponseHandler != nil {
				rawRequest, rawResponse, handlerErr := customResponseHandler([]byte(jsonData), &response, nil, sendBackRawRequest, sendBackRawResponse)
				if handlerErr != nil {
			...
			if isResponsesToChatCompletionsFallback {
				spreadResponses := response.ToDeepIntShieldResponsesStreamResponse(responsesStreamState)
				for _, response := range spreadResponses {
					if response.Type == schemas.ResponsesStreamResponseTypeError {
						...
						if response.Message != nil {
							deepintshieldErr.Error.Message = *response.Message
						}
						if response.Param != nil {
							deepintshieldErr.Error.Param = *response.Param
						}
						if response.Code != nil {
							deepintshieldErr.Error.Code = response.Code
						}
		...
		if !isResponsesToChatCompletionsFallback {
			...
			response := providerUtils.CreateDeepIntShieldChatCompletionChunkResponse(messageID, usage, finalFinishReason, chunkIndex, streamRequestType, providerName, request.Model)
			if postResponseConverter != nil {
				response = postResponseConverter(response)
			}
			// Set raw request if enabled
...
func (provider *OpenAIProvider) Responses(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldResponsesRequest) (*schemas.DeepIntShieldResponsesResponse, *schemas.DeepIntShieldError) {
	// Check if chat completion is allowed for this provider
...
func HandleOpenAIResponsesRequest(
	...
	sendBackRawResponse bool,
	providerName schemas.ModelProvider,
	customResponseHandler responseHandler[schemas.DeepIntShieldResponsesResponse],
	customErrorConverter ErrorConverter,
	logger schemas.Logger,
	...
	if lpResult, lpErr, handled := handleOpenAILargePayloadPassthrough(ctx, client, url, key, extraHeaders, providerName, request.Model, schemas.ResponsesRequest, logger); handled {
		...
		if len(lpResult.ResponseBody) > 0 {
			...
			response.ExtraFields = schemas.DeepIntShieldResponseExtraFields{Provider: providerName, ModelRequested: request.Model, RequestType: schemas.ResponsesRequest, Latency: lpResult.Latency}
			return response, nil
		}
		return &schemas.DeepIntShieldResponsesResponse{
			Model:       request.Model,
			ExtraFields: schemas.DeepIntShieldResponseExtraFields{Provider: providerName, ModelRequested: request.Model, RequestType: schemas.ResponsesRequest, Latency: lpResult.Latency},
		}, nil
	}
...
func (provider *OpenAIProvider) ResponsesStream(ctx *schemas.DeepIntShieldContext, postHookRunner schemas.PostHookRunner, key schemas.Key, request *schemas.DeepIntShieldResponsesRequest) (chan *schemas.DeepIntShieldStreamChunk, *schemas.DeepIntShieldError) {
	// Check if chat completion stream is allowed for this provider
...
func HandleOpenAIResponsesStreaming(
	...
	providerName schemas.ModelProvider,
	postHookRunner schemas.PostHookRunner,
	customResponseHandler responseHandler[schemas.DeepIntShieldResponsesStreamResponse],
	customErrorConverter ErrorConverter,
	postRequestConverter func(*OpenAIResponsesRequest) *OpenAIResponsesRequest,
	postResponseConverter func(*schemas.DeepIntShieldResponsesStreamResponse) *schemas.DeepIntShieldResponsesStreamResponse,
	logger schemas.Logger,
) (chan *schemas.DeepIntShieldStreamChunk, *schemas.DeepIntShieldError) {
	...
	go func() {
		...
		for {
			...
			// Parse into deepintshield response
			var response schemas.DeepIntShieldResponsesStreamResponse
			// TODO fix this
			if customResponseHandler != nil {
				rawRequest, rawResponse, deepintshieldErr := customResponseHandler([]byte(jsonData), &response, nil, false, false)
				if deepintshieldErr != nil {
					deepintshieldErr.ExtraFields = schemas.DeepIntShieldErrorExtraFields{
						Provider:       providerName,
						ModelRequested: request.Model,
					...
					if sendBackRawRequest {
						deepintshieldErr.ExtraFields.RawRequest = rawRequest
					}
					if sendBackRawResponse {
						deepintshieldErr.ExtraFields.RawResponse = rawResponse
					}
					ctx.SetValue(schemas.DeepIntShieldContextKeyStreamEndIndicator, true)
					providerUtils.ProcessAndSendDeepIntShieldError(ctx, postHookRunner, providerUtils.EnrichError(ctx, deepintshieldErr, jsonBody, nil, sendBackRawRequest, sendBackRawResponse), responseChan, logger)
					return
...
// Returns a DeepIntShieldResponse containing the embedding(s) and any error that occurred.
func (provider *OpenAIProvider) Embedding(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldEmbeddingRequest) (*schemas.DeepIntShieldEmbeddingResponse, *schemas.DeepIntShieldError) {
...
func HandleOpenAIEmbeddingRequest(
	...
	sendBackRawRequest bool,
	sendBackRawResponse bool,
	customResponseHandler responseHandler[schemas.DeepIntShieldEmbeddingResponse],
	logger schemas.Logger,
	...
	if lpResult, lpErr, handled := handleOpenAILargePayloadPassthrough(ctx, client, url, key, extraHeaders, providerName, request.Model, schemas.EmbeddingRequest, logger); handled {
		...
		if len(lpResult.ResponseBody) > 0 {
			response := &schemas.DeepIntShieldEmbeddingResponse{}
			if err := sonic.Unmarshal(lpResult.ResponseBody, response); err != nil {
				return nil, providerUtils.NewDeepIntShieldOperationError(schemas.ErrProviderResponseUnmarshal, err, providerName)
			}
			response.ExtraFields = schemas.DeepIntShieldResponseExtraFields{Provider: providerName, ModelRequested: request.Model, RequestType: schemas.EmbeddingRequest, Latency: lpResult.Latency}
			return response, nil
		}
		return &schemas.DeepIntShieldEmbeddingResponse{
			Model:       request.Model,
			Usage:       lpResult.Usage,
			ExtraFields: schemas.DeepIntShieldResponseExtraFields{Provider: providerName, ModelRequested: request.Model, RequestType: schemas.EmbeddingRequest, Latency: lpResult.Latency},
		}, nil
	}
...
func (provider *OpenAIProvider) Speech(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldSpeechRequest) (*schemas.DeepIntShieldSpeechResponse, *schemas.DeepIntShieldError) {
...
func HandleOpenAISpeechRequest(
	...
	sendBackRawRequest bool,
	sendBackRawResponse bool,
	customResponseHandler responseHandler[schemas.DeepIntShieldSpeechResponse],
	logger schemas.Logger,
	...
	if lpResult, lpErr, handled := handleOpenAILargePayloadPassthrough(ctx, client, url, key, extraHeaders, providerName, request.Model, schemas.SpeechRequest, logger); handled {
		...
		return &schemas.DeepIntShieldSpeechResponse{
			Audio:       lpResult.ResponseBody,
			ExtraFields: schemas.DeepIntShieldResponseExtraFields{Provider: providerName, ModelRequested: request.Model, RequestType: schemas.SpeechRequest, Latency: lpResult.Latency},
		}, nil
	}
...
// Returns a channel for streaming responses and any error that occurred.
func (provider *OpenAIProvider) SpeechStream(ctx *schemas.DeepIntShieldContext, postHookRunner schemas.PostHookRunner, key schemas.Key, request *schemas.DeepIntShieldSpeechRequest) (chan *schemas.DeepIntShieldStreamChunk, *schemas.DeepIntShieldError) {
	...
	for _, model := range providerUtils.UnsupportedSpeechStreamModels {
		if model == request.Model {
			return nil, providerUtils.NewDeepIntShieldOperationError(fmt.Sprintf("model %s is not supported for streaming speech synthesis", model), nil, provider.GetProviderKey())
		}
	}
...
func HandleOpenAISpeechStreamRequest(
	...
	postResponseConverter func(*schemas.DeepIntShieldSpeechStreamResponse) *schemas.DeepIntShieldSpeechStreamResponse,
	logger schemas.Logger,
) (chan *schemas.DeepIntShieldStreamChunk, *schemas.DeepIntShieldError) {
	// Create HTTP request for streaming
	req := fasthttp.AcquireRequest()
	...
	go func() {
		...
		for {
			...
			// Parse into deepintshield response
			var response schemas.DeepIntShieldSpeechStreamResponse
			if err := sonic.UnmarshalString(jsonData, &response); err != nil {
				logger.Warn("Failed to parse stream response: %v", err)
				continue
			}
...
// Returns the response and any error that occurred.
func (provider *OpenAIProvider) Transcription(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldTranscriptionRequest) (*schemas.DeepIntShieldTranscriptionResponse, *schemas.DeepIntShieldError) {
...
func HandleOpenAITranscriptionRequest(
	...
	providerName schemas.ModelProvider,
	sendBackRawResponse bool,
	customResponseHandler responseHandler[schemas.DeepIntShieldTranscriptionResponse],
	logger schemas.Logger,
	...
	if lpResult, lpErr, handled := handleOpenAILargePayloadPassthrough(ctx, client, url, key, extraHeaders, providerName, request.Model, schemas.TranscriptionRequest, logger); handled {
		...
		if len(lpResult.ResponseBody) > 0 {
			...
			response.ExtraFields = schemas.DeepIntShieldResponseExtraFields{Provider: providerName, ModelRequested: request.Model, RequestType: schemas.TranscriptionRequest, Latency: lpResult.Latency}
			return response, nil
		}
		return &schemas.DeepIntShieldTranscriptionResponse{
			ExtraFields: schemas.DeepIntShieldResponseExtraFields{Provider: providerName, ModelRequested: request.Model, RequestType: schemas.TranscriptionRequest, Latency: lpResult.Latency},
		}, nil
	}
	...
	// Parse OpenAI's transcription response directly into DeepIntShieldTranscribe
	response := &schemas.DeepIntShieldTranscriptionResponse{}
	var rawResponse interface{}
	if customResponseHandler != nil {
		_, rawResponse, deepintshieldErr = customResponseHandler(copiedResponseBody, response, nil, false, sendBackRawResponse)
	} else {
...
func (provider *OpenAIProvider) TranscriptionStream(ctx *schemas.DeepIntShieldContext, postHookRunner schemas.PostHookRunner, key schemas.Key, request *schemas.DeepIntShieldTranscriptionRequest) (chan *schemas.DeepIntShieldStreamChunk, *schemas.DeepIntShieldError) {
...
func HandleOpenAITranscriptionStreamRequest(
	...
	providerName schemas.ModelProvider,
	postHookRunner schemas.PostHookRunner,
	customResponseHandler responseHandler[schemas.DeepIntShieldTranscriptionStreamResponse],
	postRequestConverter func(*OpenAITranscriptionRequest) *OpenAITranscriptionRequest,
	postResponseConverter func(*schemas.DeepIntShieldTranscriptionStreamResponse) *schemas.DeepIntShieldTranscriptionStreamResponse,
	logger schemas.Logger,
) (chan *schemas.DeepIntShieldStreamChunk, *schemas.DeepIntShieldError) {
	// Use centralized converter
	reqBody := ToOpenAITranscriptionRequest(request)
	...
	if deepintshieldErr := ParseTranscriptionFormDataBodyFromRequest(writer, reqBody, providerName); deepintshieldErr != nil {
		return nil, deepintshieldErr
	}
	...
	// Make the request through the per-host circuit breaker so a
	// degraded OpenAI cluster fails fast instead of pinning every
	// goroutine on a slow socket. Non-fatal: if the breaker is open
	// we return ErrCircuitOpen and the caller surfaces a 503.
	err := providerUtils.DoWithCircuit(client, req, resp)
	if err != nil {
		defer providerUtils.ReleaseStreamingResponse(resp)
		if errors.Is(err, context.Canceled) {
			return nil, &schemas.DeepIntShieldError{
	...
	go func() {
		...
		for {
			...
			var deepintshieldErr *schemas.DeepIntShieldError
			if customResponseHandler != nil {
				_, _, deepintshieldErr = customResponseHandler([]byte(jsonData), response, nil, false, false)
				if deepintshieldErr != nil {
					deepintshieldErr.ExtraFields = schemas.DeepIntShieldErrorExtraFields{
						Provider:       providerName,
						ModelRequested: request.Model,
					...
					if sendBackRawResponse {
						deepintshieldErr.ExtraFields.RawResponse = jsonData
					}
					ctx.SetValue(schemas.DeepIntShieldContextKeyStreamEndIndicator, true)
					providerUtils.ProcessAndSendDeepIntShieldError(ctx, postHookRunner, providerUtils.EnrichError(ctx, deepintshieldErr, body.Bytes(), []byte(jsonData), false, sendBackRawResponse), responseChan, logger)
					return
				}
			} else {
				// Quick check for error field (allocation-free using sonic.GetFromString)
				if errorNode, _ := sonic.GetFromString(jsonData, "error"); errorNode.Exists() {
					// Only unmarshal when we know there's an error
					var deepintshieldErrVal schemas.DeepIntShieldError
					if err := sonic.UnmarshalString(jsonData, &deepintshieldErrVal); err == nil {
						if deepintshieldErrVal.Error != nil && deepintshieldErrVal.Error.Message != "" {
							deepintshieldErrVal.ExtraFields = schemas.DeepIntShieldErrorExtraFields{
								Provider:       providerName,
								ModelRequested: request.Model,
								RequestType:    schemas.TranscriptionStreamRequest,
							}
							ctx.SetValue(schemas.DeepIntShieldContextKeyStreamEndIndicator, true)
							providerUtils.ProcessAndSendDeepIntShieldError(ctx, postHookRunner, providerUtils.EnrichError(ctx, &deepintshieldErrVal, nil, nil, false, sendBackRawResponse), responseChan, logger)
							return
						}
					}
				}
...
// It formats the request, sends it to OpenAI, and processes the response.
// Returns a DeepIntShieldResponse containing the deepintshield response or an error if the request fails.
func (provider *OpenAIProvider) ImageGeneration(ctx *schemas.DeepIntShieldContext, key schemas.Key,
...
func HandleOpenAIImageGenerationRequest(
	...
	client *fasthttp.Client,
	url string,
	request *schemas.DeepIntShieldImageGenerationRequest,
	key schemas.Key,
	extraHeaders map[string]string,
	providerName schemas.ModelProvider,
	sendBackRawRequest bool,
	sendBackRawResponse bool,
	logger schemas.Logger,
) (*schemas.DeepIntShieldImageGenerationResponse, *schemas.DeepIntShieldError) {
	...
	if lpResult, lpErr, handled := handleOpenAILargePayloadPassthrough(ctx, client, url, key, extraHeaders, providerName, request.Model, schemas.ImageGenerationRequest, logger); handled {
		...
		if len(lpResult.ResponseBody) > 0 {
			response := &schemas.DeepIntShieldImageGenerationResponse{}
			if err := sonic.Unmarshal(lpResult.ResponseBody, response); err != nil {
				return nil, providerUtils.NewDeepIntShieldOperationError(schemas.ErrProviderResponseUnmarshal, err, providerName)
			}
			response.ExtraFields = schemas.DeepIntShieldResponseExtraFields{Provider: providerName, ModelRequested: request.Model, RequestType: schemas.ImageGenerationRequest, Latency: lpResult.Latency}
			return response, nil
		}
		return &schemas.DeepIntShieldImageGenerationResponse{
			ExtraFields: schemas.DeepIntShieldResponseExtraFields{Provider: providerName, ModelRequested: request.Model, RequestType: schemas.ImageGenerationRequest, Latency: lpResult.Latency},
		}, nil
	}
...
func (provider *OpenAIProvider) ImageGenerationStream(
	...
	if request == nil {
		return nil, providerUtils.NewDeepIntShieldOperationError("invalid request: nil", nil, provider.GetProviderKey())
	}
...
func HandleOpenAIImageGenerationStreaming(
	...
	providerName schemas.ModelProvider,
	postHookRunner schemas.PostHookRunner,
	customRequestConverter func(*schemas.DeepIntShieldImageGenerationRequest) (providerUtils.RequestBodyWithExtraParams, error),
	postRequestConverter func(*OpenAIImageGenerationRequest) *OpenAIImageGenerationRequest,
	postResponseConverter func(*schemas.DeepIntShieldImageGenerationStreamResponse) *schemas.DeepIntShieldImageGenerationStreamResponse,
	logger schemas.Logger,
) (chan *schemas.DeepIntShieldStreamChunk, *schemas.DeepIntShieldError) {
		...
		for {
			...
			if response.Type == "error" {
				...
				if response.Error != nil {
					deepintshieldErr.Error.Message = response.Error.Message
					if response.Error.Code != nil {
						deepintshieldErr.Error.Code = response.Error.Code
					}
					if response.Error.Param != nil {
						deepintshieldErr.Error.Param = response.Error.Param
					}
					if response.Error.Type != nil {
						deepintshieldErr.Error.Type = response.Error.Type
					}
				}
				ctx.SetValue(schemas.DeepIntShieldContextKeyStreamEndIndicator, true)
				providerUtils.ProcessAndSendDeepIntShieldError(ctx, postHookRunner, deepintshieldErr, responseChan, logger)
				return
			}
			...
			if isCompleted {
				...
				chunk.BackfillParams(&schemas.DeepIntShieldRequest{
...
func (provider *OpenAIProvider) Rerank(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldRerankRequest) (*schemas.DeepIntShieldRerankResponse, *schemas.DeepIntShieldError) {
	return nil, providerUtils.NewUnsupportedOperationError(schemas.RerankRequest, provider.GetProviderKey())
}

// VideoGeneration performs a video generation request via the OpenAI API.
func (provider *OpenAIProvider) VideoGeneration(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldVideoGenerationRequest) (*schemas.DeepIntShieldVideoGenerationResponse, *schemas.DeepIntShieldError) {
...
func (provider *OpenAIProvider) VideoRetrieve(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldVideoRetrieveRequest) (*schemas.DeepIntShieldVideoGenerationResponse, *schemas.DeepIntShieldError) {
...
func (provider *OpenAIProvider) VideoDownload(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldVideoDownloadRequest) (*schemas.DeepIntShieldVideoDownloadResponse, *schemas.DeepIntShieldError) {
	...
	return &schemas.DeepIntShieldVideoDownloadResponse{
...
func (provider *OpenAIProvider) VideoDelete(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldVideoDeleteRequest) (*schemas.DeepIntShieldVideoDeleteResponse, *schemas.DeepIntShieldError) {
...
func (provider *OpenAIProvider) VideoList(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldVideoListRequest) (*schemas.DeepIntShieldVideoListResponse, *schemas.DeepIntShieldError) {
...
func HandleOpenAIVideoGenerationRequest(
	...
	client *fasthttp.Client,
	url string,
	request *schemas.DeepIntShieldVideoGenerationRequest,
	key schemas.Key,
	extraHeaders map[string]string,
	providerName schemas.ModelProvider,
	sendBackRawRequest bool,
	sendBackRawResponse bool,
	logger schemas.Logger,
) (*schemas.DeepIntShieldVideoGenerationResponse, *schemas.DeepIntShieldError) {
	// Create request
	req := fasthttp.AcquireRequest()
	resp := fasthttp.AcquireResponse()
	defer fasthttp.ReleaseRequest(req)
	...
	if err != nil {
		return nil, providerUtils.NewDeepIntShieldOperationError("failed to convert video generation request to openai format", err, providerName)
	}
	if reqBody == nil {
		return nil, providerUtils.NewDeepIntShieldOperationError("video generation input is not provided", nil, providerName)
	}
	...
	rawRequest, rawResponse, deepintshieldErr := providerUtils.HandleProviderResponse(responseBody, response, nil, sendBackRawRequest, sendBackRawResponse)
	if deepintshieldErr != nil {
		return nil, deepintshieldErr
	}
...
type VideoDownloadHandler func(ctx *schemas.DeepIntShieldContext, key schemas.Key, req *schemas.DeepIntShieldVideoDownloadRequest) (*schemas.DeepIntShieldVideoDownloadResponse, *schemas.DeepIntShieldError)
...
func HandleOpenAIVideoRetrieveRequest(
	...
	client *fasthttp.Client,
	url string,
	request *schemas.DeepIntShieldVideoRetrieveRequest,
	key schemas.Key,
	extraHeaders map[string]string,
	...
	response := &schemas.DeepIntShieldVideoGenerationResponse{}
	_, rawResponse, deepintshieldErr := providerUtils.HandleProviderResponse(body, response, nil, sendBackRawRequest, sendBackRawResponse)
	if deepintshieldErr != nil {
		return nil, deepintshieldErr
	}
	if response.ID != "" {
	...
	if videoDownloaddHandler != nil {
		downloadVideo, ok := ctx.Value(schemas.DeepIntShieldContextKeyVideoOutputRequested).(bool)
		if ok && downloadVideo && response.Status == schemas.VideoStatusCompleted {
			videoDownloadRequest := &schemas.DeepIntShieldVideoDownloadRequest{
				Provider: providerName,
				ID:       response.ID,
			}
			videoDownloadResponse, deepintshieldErr := videoDownloaddHandler(ctx, key, videoDownloadRequest)
			if deepintshieldErr != nil {
				return nil, deepintshieldErr
			}
...
func HandleOpenAIVideoDeleteRequest(
	...
	sendBackRawResponse bool,
	logger schemas.Logger,
) (*schemas.DeepIntShieldVideoDeleteResponse, *schemas.DeepIntShieldError) {
	// Create request
	req := fasthttp.AcquireRequest()
	resp := fasthttp.AcquireResponse()
	defer fasthttp.ReleaseRequest(req)
	...
	rawRequest, rawResponse, deepintshieldErr := providerUtils.HandleProviderResponse(body, response, nil, sendBackRawRequest, sendBackRawResponse)
	if deepintshieldErr != nil {
		return nil, deepintshieldErr
	}
	if response.ID != "" {
...
func HandleOpenAIVideoListRequest(
	...
	client *fasthttp.Client,
	baseURL string,
	request *schemas.DeepIntShieldVideoListRequest,
	key schemas.Key,
	extraHeaders map[string]string,
	providerName schemas.ModelProvider,
	sendBackRawRequest bool,
	sendBackRawResponse bool,
	logger schemas.Logger,
) (*schemas.DeepIntShieldVideoListResponse, *schemas.DeepIntShieldError) {
	// Create request
	req := fasthttp.AcquireRequest()
	resp := fasthttp.AcquireResponse()
	defer fasthttp.ReleaseRequest(req)
	...
	response := &schemas.DeepIntShieldVideoListResponse{}
	_, rawResponse, deepintshieldErr := providerUtils.HandleProviderResponse(body, response, nil, sendBackRawRequest, sendBackRawResponse)
	if deepintshieldErr != nil {
		return nil, deepintshieldErr
	}
...
func (provider *OpenAIProvider) CountTokens(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldResponsesRequest) (*schemas.DeepIntShieldCountTokensResponse, *schemas.DeepIntShieldError) {
...
func HandleOpenAICountTokensRequest(
	...
	providerName schemas.ModelProvider,
	logger schemas.Logger,
) (*schemas.DeepIntShieldCountTokensResponse, *schemas.DeepIntShieldError) {
	// Create request
	req := fasthttp.AcquireRequest()
	resp := fasthttp.AcquireResponse()
	defer fasthttp.ReleaseRequest(req)
	...
	if lpResult, lpErr, handled := handleOpenAILargePayloadPassthrough(ctx, client, url, key, extraHeaders, providerName, request.Model, schemas.CountTokensRequest, logger); handled {
		...
		if len(lpResult.ResponseBody) > 0 {
			response := &schemas.DeepIntShieldCountTokensResponse{}
			if err := sonic.Unmarshal(lpResult.ResponseBody, response); err != nil {
				return nil, providerUtils.NewDeepIntShieldOperationError(schemas.ErrProviderResponseUnmarshal, err, providerName)
			}
			response.ExtraFields = schemas.DeepIntShieldResponseExtraFields{Provider: providerName, ModelRequested: request.Model, RequestType: schemas.CountTokensRequest, Latency: lpResult.Latency}
			return response, nil
		}
		return &schemas.DeepIntShieldCountTokensResponse{
			ExtraFields: schemas.DeepIntShieldResponseExtraFields{Provider: providerName, ModelRequested: request.Model, RequestType: schemas.CountTokensRequest, Latency: lpResult.Latency},
		}, nil
	}
...
func (provider *OpenAIProvider) ImageEdit(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldImageEditRequest) (*schemas.DeepIntShieldImageGenerationResponse, *schemas.DeepIntShieldError) {
...
func HandleOpenAIImageEditRequest(
	...
	client *fasthttp.Client,
	url string,
	request *schemas.DeepIntShieldImageEditRequest,
	key schemas.Key,
	extraHeaders map[string]string,
	sendBackRawRequest bool,
	sendBackRawResponse bool,
	...
	if lpResult, lpErr, handled := handleOpenAILargePayloadPassthrough(ctx, client, url, key, extraHeaders, providerName, request.Model, schemas.ImageEditRequest, logger); handled {
		...
		if len(lpResult.ResponseBody) > 0 {
			...
			response.ExtraFields = schemas.DeepIntShieldResponseExtraFields{Provider: providerName, ModelRequested: request.Model, RequestType: schemas.ImageEditRequest, Latency: lpResult.Latency}
			return response, nil
		}
		return &schemas.DeepIntShieldImageGenerationResponse{
			ExtraFields: schemas.DeepIntShieldResponseExtraFields{Provider: providerName, ModelRequested: request.Model, RequestType: schemas.ImageEditRequest, Latency: lpResult.Latency},
		}, nil
	}

	openaiReq := ToOpenAIImageEditRequest(request)
	if openaiReq == nil {
		return nil, providerUtils.NewDeepIntShieldOperationError("failed to convert request to OpenAI format", nil, providerName)
	}
	...
	defer wait()
	if deepintshieldErr != nil {
		return nil, providerUtils.EnrichError(ctx, deepintshieldErr, bodyData, nil, sendBackRawRequest, sendBackRawResponse)
	}
	// Extract provider response headers early so they're available on error paths too
	providerResponseHeaders := providerUtils.ExtractProviderResponseHeaders(resp)
	ctx.SetValue(schemas.DeepIntShieldContextKeyProviderResponseHeaders, providerResponseHeaders)
	...
	response := &schemas.DeepIntShieldImageGenerationResponse{}
	rawRequest, rawResponse, deepintshieldErr := providerUtils.HandleProviderResponse(bodyBytes, response, bodyData, false, sendBackRawResponse)
	if deepintshieldErr != nil {
		return nil, deepintshieldErr
	}
	response.ExtraFields.Provider = providerName
...
func (provider *OpenAIProvider) ImageEditStream(ctx *schemas.DeepIntShieldContext, postHookRunner schemas.PostHookRunner, key schemas.Key, request *schemas.DeepIntShieldImageEditRequest) (chan *schemas.DeepIntShieldStreamChunk, *schemas.DeepIntShieldError) {
	// Check if image generation stream is allowed for this provider
...
func HandleOpenAIImageEditStreamRequest(
	...
	providerName schemas.ModelProvider,
	postHookRunner schemas.PostHookRunner,
	customRequestConverter func(*schemas.DeepIntShieldImageEditRequest) (providerUtils.RequestBodyWithExtraParams, error),
	...
	if reqBody == nil {
		return nil, providerUtils.NewDeepIntShieldOperationError("image edit input is not provided", nil, providerName)
	}
	...
	if deepintshieldErr := parseImageEditFormDataBodyFromRequest(writer, reqBody, providerName); deepintshieldErr != nil {
		return nil, deepintshieldErr
	}
			...
			if errorNode, _ := sonic.GetFromString(jsonData, "error"); errorNode.Exists() {
				...
				if err := sonic.UnmarshalString(jsonData, &deepintshieldErr); err == nil {
					if deepintshieldErr.Error != nil && deepintshieldErr.Error.Message != "" {
						...
						ctx.SetValue(schemas.DeepIntShieldContextKeyStreamEndIndicator, true)
						providerUtils.ProcessAndSendDeepIntShieldError(ctx, postHookRunner, providerUtils.EnrichError(ctx, &deepintshieldErr, body.Bytes(), nil, sendBackRawRequest, sendBackRawResponse), responseChan, logger)
						return
					}
				}
			}
...
func (provider *OpenAIProvider) ImageVariation(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldImageVariationRequest) (*schemas.DeepIntShieldImageGenerationResponse, *schemas.DeepIntShieldError) {
...
func HandleOpenAIImageVariationRequest(
	...
	client *fasthttp.Client,
	url string,
	request *schemas.DeepIntShieldImageVariationRequest,
	key schemas.Key,
	extraHeaders map[string]string,
	sendBackRawRequest bool,
	sendBackRawResponse bool,
	...
	if lpResult, lpErr, handled := handleOpenAILargePayloadPassthrough(ctx, client, url, key, extraHeaders, providerName, request.Model, schemas.ImageVariationRequest, logger); handled {
		...
		if len(lpResult.ResponseBody) > 0 {
			...
			response.ExtraFields = schemas.DeepIntShieldResponseExtraFields{Provider: providerName, ModelRequested: request.Model, RequestType: schemas.ImageVariationRequest, Latency: lpResult.Latency}
			return response, nil
		}
		return &schemas.DeepIntShieldImageGenerationResponse{
			ExtraFields: schemas.DeepIntShieldResponseExtraFields{Provider: providerName, ModelRequested: request.Model, RequestType: schemas.ImageVariationRequest, Latency: lpResult.Latency},
		}, nil
	}
	...
	response := &schemas.DeepIntShieldImageGenerationResponse{}
	_, rawResponse, deepintshieldErr := providerUtils.HandleProviderResponse(bodyBytes, response, bodyData, sendBackRawRequest, sendBackRawResponse)
	if deepintshieldErr != nil {
		return nil, deepintshieldErr
	}
	response.ExtraFields.Provider = providerName
...
func (provider *OpenAIProvider) FileUpload(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldFileUploadRequest) (*schemas.DeepIntShieldFileUploadResponse, *schemas.DeepIntShieldError) {
	...
	if request.Purpose == "" {
		return nil, providerUtils.NewDeepIntShieldOperationError("purpose is required", nil, providerName)
	}
	...
	if err := writer.WriteField("purpose", string(request.Purpose)); err != nil {
		return nil, providerUtils.NewDeepIntShieldOperationError("failed to write purpose field", err, providerName)
	}
	...
	if request.ExpiresAfter != nil {
		if err := writer.WriteField("expires_after[anchor]", request.ExpiresAfter.Anchor); err != nil {
			return nil, providerUtils.NewDeepIntShieldOperationError("failed to write expires_after[anchor] field", err, providerName)
		}
		if err := writer.WriteField("expires_after[seconds]", fmt.Sprintf("%d", request.ExpiresAfter.Seconds)); err != nil {
			return nil, providerUtils.NewDeepIntShieldOperationError("failed to write expires_after[seconds] field", err, providerName)
		}
	}
	...
	rawRequest, rawResponse, deepintshieldErr := providerUtils.HandleProviderResponse(body, &openAIResp, nil, sendBackRawRequest, sendBackRawResponse)
	if deepintshieldErr != nil {
		return nil, deepintshieldErr
	}

	fileResponse := openAIResp.ToDeepIntShieldFileUploadResponse(providerName, latency, sendBackRawRequest, sendBackRawResponse, rawRequest, rawResponse)
...
func (provider *OpenAIProvider) FileList(ctx *schemas.DeepIntShieldContext, keys []schemas.Key, request *schemas.DeepIntShieldFileListRequest) (*schemas.DeepIntShieldFileListResponse, *schemas.DeepIntShieldError) {
	...
	_, _, deepintshieldErr = providerUtils.HandleProviderResponse(body, &openAIResp, nil, sendBackRawRequest, sendBackRawResponse)
	if deepintshieldErr != nil {
		return nil, deepintshieldErr
	}
	...
	for _, file := range openAIResp.Data {
		files = append(files, schemas.FileObject{
			...
			Status:        ToDeepIntShieldFileStatus(file.Status),
	...
	if nextCursor != "" {
		deepintshieldResp.After = &nextCursor
	}
...
func (provider *OpenAIProvider) FileRetrieve(ctx *schemas.DeepIntShieldContext, keys []schemas.Key, request *schemas.DeepIntShieldFileRetrieveRequest) (*schemas.DeepIntShieldFileRetrieveResponse, *schemas.DeepIntShieldError) {
	...
	for _, key := range keys {
		...
		return openAIResp.ToDeepIntShieldFileRetrieveResponse(providerName, latency, sendBackRawRequest, sendBackRawResponse, rawRequest, rawResponse), nil
	}
...
func (provider *OpenAIProvider) FileDelete(ctx *schemas.DeepIntShieldContext, keys []schemas.Key, request *schemas.DeepIntShieldFileDeleteRequest) (*schemas.DeepIntShieldFileDeleteResponse, *schemas.DeepIntShieldError) {
...
func (provider *OpenAIProvider) FileContent(ctx *schemas.DeepIntShieldContext, keys []schemas.Key, request *schemas.DeepIntShieldFileContentRequest) (*schemas.DeepIntShieldFileContentResponse, *schemas.DeepIntShieldError) {
	...
	for _, key := range keys {
		...
		return &schemas.DeepIntShieldFileContentResponse{
...
func (provider *OpenAIProvider) VideoRemix(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldVideoRemixRequest) (*schemas.DeepIntShieldVideoGenerationResponse, *schemas.DeepIntShieldError) {
	...
	if request.Input == nil || request.Input.Prompt == "" {
		return nil, providerUtils.NewDeepIntShieldOperationError("prompt is required", nil, providerName)
	}
...
func (provider *OpenAIProvider) BatchCreate(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldBatchCreateRequest) (*schemas.DeepIntShieldBatchCreateResponse, *schemas.DeepIntShieldError) {
	...
	if inputFileID == "" && len(request.Requests) > 0 {
		...
		if err != nil {
			return nil, providerUtils.NewDeepIntShieldOperationError("failed to convert requests to JSONL", err, providerName)
		}

		// Upload the file with purpose "batch"
		uploadResp, deepintshieldErr := provider.FileUpload(ctx, key, &schemas.DeepIntShieldFileUploadRequest{
	...
	if inputFileID == "" {
		return nil, providerUtils.NewDeepIntShieldOperationError("either input_file_id or requests array is required for OpenAI batch API", nil, providerName)
	}

	// Validate that we have an endpoint
	if request.Endpoint == "" {
		return nil, providerUtils.NewDeepIntShieldOperationError("endpoint is required for OpenAI batch API", nil, providerName)
	}
	...
	if err != nil {
		return nil, providerUtils.NewDeepIntShieldOperationError(schemas.ErrProviderRequestMarshal, err, providerName)
	}
	req.SetBody(jsonData)
	...
	body, err := providerUtils.CheckAndDecodeBody(resp)
	if err != nil {
		return nil, providerUtils.EnrichError(ctx, providerUtils.NewDeepIntShieldOperationError(schemas.ErrProviderResponseDecode, err, providerName), jsonData, nil, sendBackRawRequest, sendBackRawResponse)
	}

	var openAIResp OpenAIBatchResponse
	rawRequest, rawResponse, deepintshieldErr := providerUtils.HandleProviderResponse(body, &openAIResp, jsonData, sendBackRawRequest, sendBackRawResponse)
	if deepintshieldErr != nil {
		return nil, providerUtils.EnrichError(ctx, deepintshieldErr, jsonData, body, sendBackRawRequest, sendBackRawResponse)
	}

	return openAIResp.ToDeepIntShieldBatchCreateResponse(providerName, latency, sendBackRawRequest, sendBackRawResponse, rawRequest, rawResponse), nil
}

// BatchList lists batch jobs using serial pagination across keys.
// Exhausts all pages from one key before moving to the next.
func (provider *OpenAIProvider) BatchList(ctx *schemas.DeepIntShieldContext, keys []schemas.Key, request *schemas.DeepIntShieldBatchListRequest) (*schemas.DeepIntShieldBatchListResponse, *schemas.DeepIntShieldError) {
	...
	if !ok {
		// All keys exhausted
		return &schemas.DeepIntShieldBatchListResponse{
			Object:  "list",
			Data:    []schemas.DeepIntShieldBatchRetrieveResponse{},
			HasMore: false,
			ExtraFields: schemas.DeepIntShieldResponseExtraFields{
	...
	// Convert batches to DeepIntShield format
	batches := make([]schemas.DeepIntShieldBatchRetrieveResponse, 0, len(openAIResp.Data))
	var lastBatchID string
	for _, batch := range openAIResp.Data {
		batches = append(batches, *batch.ToDeepIntShieldBatchRetrieveResponse(providerName, latency, sendBackRawRequest, sendBackRawResponse, rawRequest, rawResponse))
	...
	// Convert to DeepIntShield response
	deepintshieldResp := &schemas.DeepIntShieldBatchListResponse{
		Object:  "list",
		Data:    batches,
		HasMore: hasMore,
		ExtraFields: schemas.DeepIntShieldResponseExtraFields{
	...
	if nextCursor != "" {
		deepintshieldResp.NextCursor = &nextCursor
	}
...
func (provider *OpenAIProvider) BatchRetrieve(ctx *schemas.DeepIntShieldContext, keys []schemas.Key, request *schemas.DeepIntShieldBatchRetrieveRequest) (*schemas.DeepIntShieldBatchRetrieveResponse, *schemas.DeepIntShieldError) {
	...
	if request.BatchID == "" {
		return nil, providerUtils.NewDeepIntShieldOperationError("batch_id is required", nil, request.Provider)
	}
	...
	for _, key := range keys {
		...
		result := openAIResp.ToDeepIntShieldBatchRetrieveResponse(providerName, latency, sendBackRawRequest, sendBackRawResponse, rawRequest, rawResponse)
...
func (provider *OpenAIProvider) BatchCancel(ctx *schemas.DeepIntShieldContext, keys []schemas.Key, request *schemas.DeepIntShieldBatchCancelRequest) (*schemas.DeepIntShieldBatchCancelResponse, *schemas.DeepIntShieldError) {
	...
	if request.BatchID == "" {
		return nil, providerUtils.NewDeepIntShieldOperationError("batch_id is required", nil, schemas.OpenAI)
	}
	...
	for _, key := range keys {
		...
		result := &schemas.DeepIntShieldBatchCancelResponse{
			ID:           openAIResp.ID,
			Object:       openAIResp.Object,
			Status:       ToDeepIntShieldBatchStatus(openAIResp.Status),
...
func (provider *OpenAIProvider) BatchDelete(ctx *schemas.DeepIntShieldContext, keys []schemas.Key, request *schemas.DeepIntShieldBatchDeleteRequest) (*schemas.DeepIntShieldBatchDeleteResponse, *schemas.DeepIntShieldError) {
...
func (provider *OpenAIProvider) BatchResults(ctx *schemas.DeepIntShieldContext, keys []schemas.Key, request *schemas.DeepIntShieldBatchResultsRequest) (*schemas.DeepIntShieldBatchResultsResponse, *schemas.DeepIntShieldError) {
	...
	batchResp, deepintshieldErr := provider.BatchRetrieve(ctx, keys, &schemas.DeepIntShieldBatchRetrieveRequest{
	...
	if batchResp.OutputFileID == nil || *batchResp.OutputFileID == "" {
		return nil, providerUtils.NewDeepIntShieldOperationError("batch results not available: output_file_id is empty (batch may not be completed)", nil, providerName)
	}
	...
	for _, key := range keys {
		...
		batchResultsResp := &schemas.DeepIntShieldBatchResultsResponse{
			BatchID: request.BatchID,
			Results: results,
			ExtraFields: schemas.DeepIntShieldResponseExtraFields{
...
func (provider *OpenAIProvider) ContainerCreate(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldContainerCreateRequest) (*schemas.DeepIntShieldContainerCreateResponse, *schemas.DeepIntShieldError) {
	...
	if request == nil {
		return nil, providerUtils.NewDeepIntShieldOperationError("invalid request: nil", nil, providerName)
	}

	if request.Name == "" {
		return nil, providerUtils.NewDeepIntShieldOperationError("invalid request: name is required", nil, providerName)
	}
	...
	rawRequest, rawResponse, deepintshieldErr := providerUtils.HandleProviderResponse(responseBody, &containerResp, jsonBody, providerUtils.ShouldSendBackRawRequest(ctx, provider.sendBackRawRequest), providerUtils.ShouldSendBackRawResponse(ctx, provider.sendBackRawResponse))
	if deepintshieldErr != nil {
		return nil, deepintshieldErr
	}

	response := &schemas.DeepIntShieldContainerCreateResponse{
		ID:           containerResp.ID,
...
func (provider *OpenAIProvider) ContainerList(ctx *schemas.DeepIntShieldContext, keys []schemas.Key, request *schemas.DeepIntShieldContainerListRequest) (*schemas.DeepIntShieldContainerListResponse, *schemas.DeepIntShieldError) {
	providerName := provider.GetProviderKey()
	...
	if len(keys) == 0 {
		if provider.customProviderConfig != nil && provider.customProviderConfig.IsKeyLess {
			...
			return nil, providerUtils.NewDeepIntShieldOperationError("provider config not found", nil, providerName)
		}
	}
	...
	if !ok {
		// All keys exhausted
		return &schemas.DeepIntShieldContainerListResponse{
			Object:  "list",
	...
	rawRequest, rawResponse, deepintshieldErr := providerUtils.HandleProviderResponse(responseBody, &listResp, nil, sendBackRawRequest, sendBackRawResponse)
	if deepintshieldErr != nil {
		return nil, deepintshieldErr
	}
	...
	response := &schemas.DeepIntShieldContainerListResponse{
...
func (provider *OpenAIProvider) ContainerRetrieve(ctx *schemas.DeepIntShieldContext, keys []schemas.Key, request *schemas.DeepIntShieldContainerRetrieveRequest) (*schemas.DeepIntShieldContainerRetrieveResponse, *schemas.DeepIntShieldError) {
	providerName := provider.GetProviderKey()
	...
	if request.ContainerID == "" {
		return nil, providerUtils.NewDeepIntShieldOperationError("container_id is required", nil, providerName)
	}
	...
	for _, key := range keys {
		...
		rawRequest, rawResponse, deepintshieldErr := providerUtils.HandleProviderResponse(responseBody, &containerResp, nil, providerUtils.ShouldSendBackRawRequest(ctx, provider.sendBackRawRequest), providerUtils.ShouldSendBackRawResponse(ctx, provider.sendBackRawResponse))
		if deepintshieldErr != nil {
			fasthttp.ReleaseRequest(req)
			fasthttp.ReleaseResponse(resp)
			lastErr = deepintshieldErr
		...
		response := &schemas.DeepIntShieldContainerRetrieveResponse{
			ID:           containerResp.ID,
			Object:       containerResp.Object,
			Name:         containerResp.Name,
			CreatedAt:    containerResp.CreatedAt,
...
func (provider *OpenAIProvider) ContainerDelete(ctx *schemas.DeepIntShieldContext, keys []schemas.Key, request *schemas.DeepIntShieldContainerDeleteRequest) (*schemas.DeepIntShieldContainerDeleteResponse, *schemas.DeepIntShieldError) {
	providerName := provider.GetProviderKey()
	...
	for _, key := range keys {
		...
		rawRequest, rawResponse, deepintshieldErr := providerUtils.HandleProviderResponse(responseBody, &deleteResp, nil, providerUtils.ShouldSendBackRawRequest(ctx, provider.sendBackRawRequest), providerUtils.ShouldSendBackRawResponse(ctx, provider.sendBackRawResponse))
		if deepintshieldErr != nil {
			fasthttp.ReleaseRequest(req)
			fasthttp.ReleaseResponse(resp)
			lastErr = deepintshieldErr
		...
		response := &schemas.DeepIntShieldContainerDeleteResponse{
			ID:      deleteResp.ID,
...
func (provider *OpenAIProvider) ContainerFileCreate(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldContainerFileCreateRequest) (*schemas.DeepIntShieldContainerFileCreateResponse, *schemas.DeepIntShieldError) {
	...
	if request.ContainerID == "" {
		return nil, providerUtils.NewDeepIntShieldOperationError("invalid request: container_id is required", nil, providerName)
	}
	...
	if len(request.File) == 0 {
		return nil, providerUtils.NewDeepIntShieldOperationError("invalid request: file is required", nil, providerName)
	}
	...
	if err != nil {
		return nil, providerUtils.NewDeepIntShieldOperationError("failed to create multipart form", err, providerName)
	}
	if _, err = part.Write(request.File); err != nil {
		return nil, providerUtils.NewDeepIntShieldOperationError("failed to write file to multipart form", err, providerName)
	}
	if err := writer.Close(); err != nil {
		return nil, providerUtils.NewDeepIntShieldOperationError("failed to close multipart form", err, providerName)
	}
	...
	_, rawResponse, deepintshieldErr := providerUtils.HandleProviderResponse(responseBody, &fileResp, nil, false, sendBackRawResponse)
	if deepintshieldErr != nil {
		return nil, deepintshieldErr
	}

	containerFileCreateResponse := &schemas.DeepIntShieldContainerFileCreateResponse{
		ID:          fileResp.ID,
...
// Uses SerialListHelper for multi-key pagination - exhausts all pages from one key before moving to next.
func (provider *OpenAIProvider) ContainerFileList(ctx *schemas.DeepIntShieldContext, keys []schemas.Key, request *schemas.DeepIntShieldContainerFileListRequest) (*schemas.DeepIntShieldContainerFileListResponse, *schemas.DeepIntShieldError) {
	providerName := provider.GetProviderKey()
	...
	if len(keys) == 0 {
		if provider.customProviderConfig != nil && provider.customProviderConfig.IsKeyLess {
			...
			return nil, providerUtils.NewDeepIntShieldOperationError("no keys provided", nil, providerName)
		}
	}
	...
	if !ok {
		// All keys exhausted
		return &schemas.DeepIntShieldContainerFileListResponse{
			Object:  "list",
	...
	containerFileListResponse := &schemas.DeepIntShieldContainerFileListResponse{
		Object:  listResp.Object,
		Data:    listResp.Data,
		FirstID: listResp.FirstID,
		LastID:  listResp.LastID,
...
func (provider *OpenAIProvider) ContainerFileRetrieve(ctx *schemas.DeepIntShieldContext, keys []schemas.Key, request *schemas.DeepIntShieldContainerFileRetrieveRequest) (*schemas.DeepIntShieldContainerFileRetrieveResponse, *schemas.DeepIntShieldError) {
	providerName := provider.GetProviderKey()
	...
	if request.FileID == "" {
		return nil, providerUtils.NewDeepIntShieldOperationError("invalid request: file_id is required", nil, providerName)
	}
	...
	for _, key := range keys {
		...
		rawRequest, rawResponse, deepintshieldErr := providerUtils.HandleProviderResponse(responseBody, &fileResp, nil, sendBackRawRequest, sendBackRawResponse)
		if deepintshieldErr != nil {
			lastErr = deepintshieldErr
			fasthttp.ReleaseRequest(req)
			fasthttp.ReleaseResponse(resp)
		...
		containerFileRetrieveResponse := &schemas.DeepIntShieldContainerFileRetrieveResponse{
			ID:          fileResp.ID,
			Object:      fileResp.Object,
			Bytes:       fileResp.Bytes,
			CreatedAt:   fileResp.CreatedAt,
...
func (provider *OpenAIProvider) ContainerFileContent(ctx *schemas.DeepIntShieldContext, keys []schemas.Key, request *schemas.DeepIntShieldContainerFileContentRequest) (*schemas.DeepIntShieldContainerFileContentResponse, *schemas.DeepIntShieldError) {
	providerName := provider.GetProviderKey()
	...
	for _, key := range keys {
		...
		containerFileContentResponse := &schemas.DeepIntShieldContainerFileContentResponse{
			Content:     content,
			ContentType: contentType,
			ExtraFields: schemas.DeepIntShieldResponseExtraFields{
				Provider:    providerName,
...
func (provider *OpenAIProvider) ContainerFileDelete(ctx *schemas.DeepIntShieldContext, keys []schemas.Key, request *schemas.DeepIntShieldContainerFileDeleteRequest) (*schemas.DeepIntShieldContainerFileDeleteResponse, *schemas.DeepIntShieldError) {
	providerName := provider.GetProviderKey()
	...
	for _, key := range keys {
		...
		rawRequest, rawResponse, deepintshieldErr := providerUtils.HandleProviderResponse(responseBody, &deleteResp, nil, sendBackRawRequest, sendBackRawResponse)
		if deepintshieldErr != nil {
			lastErr = deepintshieldErr
			fasthttp.ReleaseRequest(req)
			fasthttp.ReleaseResponse(resp)
		...
		containerFileDeleteResponse := &schemas.DeepIntShieldContainerFileDeleteResponse{
			ID:      deleteResp.ID,
			Object:  deleteResp.Object,
			Deleted: deleteResp.Deleted,
			ExtraFields: schemas.DeepIntShieldResponseExtraFields{
...
func (provider *OpenAIProvider) Passthrough(
	...
	latency, deepintshieldErr, wait := providerUtils.MakeRequestWithContext(ctx, provider.client, fasthttpReq, resp)
	defer wait()
	if deepintshieldErr != nil {
		return nil, deepintshieldErr
	}
	...
	body, err := providerUtils.CheckAndDecodeBody(resp)
	if err != nil {
		return nil, providerUtils.NewDeepIntShieldOperationError("failed to decode response body", err, provider.GetProviderKey())
	}
	...
	deepintshieldResponse := &schemas.DeepIntShieldPassthroughResponse{
	...
	deepintshieldResponse.ExtraFields.ModelRequested = req.Model
	deepintshieldResponse.ExtraFields.RequestType = schemas.PassthroughRequest
	deepintshieldResponse.ExtraFields.Latency = latency.Milliseconds()

	if providerUtils.ShouldSendBackRawRequest(ctx, provider.sendBackRawRequest) {
		providerUtils.ParseAndSetRawRequestIfJSON(fasthttpReq, &deepintshieldResponse.ExtraFields)
	}
...
func (provider *OpenAIProvider) PassthroughStream(
	...
	if err := activeClient.Do(fasthttpReq, resp); err != nil {
		...
		if errors.Is(err, fasthttp.ErrTimeout) || errors.Is(err, context.DeadlineExceeded) {
			return nil, providerUtils.NewDeepIntShieldOperationError(schemas.ErrProviderRequestTimedOut, err, provider.GetProviderKey())
		}
		return nil, providerUtils.NewDeepIntShieldOperationError(schemas.ErrProviderDoRequest, err, provider.GetProviderKey())
	}
	...
	ch := make(chan *schemas.DeepIntShieldStreamChunk, schemas.DefaultStreamBufferSize)
	go func() {
		defer func() {
			if ctx.Err() == context.Canceled {
		...
		for {
			...
			if n > 0 {
				...
				copy(chunk, buf[:n])
				select {
				case ch <- &schemas.DeepIntShieldStreamChunk{
					DeepIntShieldPassthroughResponse: &schemas.DeepIntShieldPassthroughResponse{
			...
			if readErr == io.EOF {
				...
				finalResp := &schemas.DeepIntShieldResponse{
					PassthroughResponse: &schemas.DeepIntShieldPassthroughResponse{
						StatusCode:  statusCode,
						Headers:     headers,
						ExtraFields: extraFields,
					},
				...
				if finalizer, ok := ctx.Value(schemas.DeepIntShieldContextKeyPostHookSpanFinalizer).(func(context.Context)); ok && finalizer != nil {
					finalizer(ctx)
				}
				return
			}
