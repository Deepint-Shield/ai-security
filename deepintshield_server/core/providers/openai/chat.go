...
// ToDeepIntShieldChatRequest converts an OpenAI chat request to DeepIntShield format
func (req *OpenAIChatRequest) ToDeepIntShieldChatRequest(ctx *schemas.DeepIntShieldContext) *schemas.DeepIntShieldChatRequest {
	...
	return &schemas.DeepIntShieldChatRequest{
		Provider:  provider,
		Model:     model,
		Input:     ConvertOpenAIMessagesToDeepIntShieldMessages(req.Messages),
		Params:    &req.ChatParameters,
		Fallbacks: schemas.ParseFallbacks(req.Fallbacks),
	}
}

// ToOpenAIChatRequest converts a DeepIntShield chat completion request to OpenAI format
func ToOpenAIChatRequest(ctx *schemas.DeepIntShieldContext, deepintshieldReq *schemas.DeepIntShieldChatRequest) *OpenAIChatRequest {
	...
	openaiReq := &OpenAIChatRequest{
		...
		Messages: ConvertDeepIntShieldMessagesToOpenAIMessages(deepintshieldReq.Input),
	}

	if deepintshieldReq.Params != nil {
		openaiReq.ChatParameters = *deepintshieldReq.Params
	...
	case schemas.XAI:
		...
		openaiReq.applyXAICompatibility(deepintshieldReq.Model)
		return openaiReq
	case schemas.Gemini:
		openaiReq.filterOpenAISpecificParameters()
	...
	case schemas.Vertex:
		...
		if schemas.IsMistralModel(deepintshieldReq.Model) {
			openaiReq.applyMistralCompatibility()
		}
		return openaiReq
	default:
		// Check if provider is a custom provider
		if isCustomProvider, ok := ctx.Value(schemas.DeepIntShieldContextKeyIsCustomProvider).(bool); ok && isCustomProvider {
			return openaiReq
		}
		openaiReq.filterOpenAISpecificParameters()
		return openaiReq
