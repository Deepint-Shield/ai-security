...
// ToDeepIntShieldResponsesRequest converts an OpenAI responses request to DeepIntShield format
func (resp *OpenAIResponsesRequest) ToDeepIntShieldResponsesRequest(ctx *schemas.DeepIntShieldContext) *schemas.DeepIntShieldResponsesRequest {
	if resp == nil {
		return nil
	}
...
// ToOpenAIResponsesRequest converts a DeepIntShield responses request to OpenAI format
func ToOpenAIResponsesRequest(deepintshieldReq *schemas.DeepIntShieldResponsesRequest) *OpenAIResponsesRequest {
	...
	messages = make([]schemas.ResponsesMessage, 0, len(deepintshieldReq.Input))
	for _, message := range deepintshieldReq.Input {
		...
		if message.ResponsesReasoning != nil {
			isGptOss := strings.Contains(deepintshieldReq.Model, "gpt-oss")
			isReasoning := isOpenAIReasoningModel(deepintshieldReq.Model)
	...
	if params != nil {
		...
		if req.ResponsesParameters.Reasoning != nil {
			...
			if deepintshieldReq.Provider == schemas.XAI &&
				schemas.IsGrokReasoningModel(deepintshieldReq.Model) &&
				!strings.Contains(deepintshieldReq.Model, "grok-3-mini") {
			...
			if deepintshieldReq.Provider == schemas.OpenAI && !isOpenAIReasoningModel(deepintshieldReq.Model) {
		...
		if isOpenAIReasoningModel(deepintshieldReq.Model) {
			stripTopP := true
			_, parsedModel := schemas.ParseModelString(deepintshieldReq.Model, schemas.OpenAI)
		...
		// We must copy the Tools slice since it shares the backing array with deepintshieldReq.Params.Tools.
		if len(req.Tools) > 0 {
