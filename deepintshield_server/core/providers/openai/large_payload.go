...
func setStreamingRequestBody(ctx *schemas.DeepIntShieldContext, req *fasthttp.Request, jsonBody []byte, providerName schemas.ModelProvider) {
...
) (*largePayloadResult, *schemas.DeepIntShieldError, bool) {
...
func finalizeOpenAIResponse(
	...
	providerName schemas.ModelProvider,
	logger schemas.Logger,
) ([]byte, *largePayloadResult, *schemas.DeepIntShieldError) {
	body, isLarge, deepintshieldErr := providerUtils.FinalizeResponseWithLargeDetection(ctx, resp, providerName, logger)
	if deepintshieldErr != nil {
		fasthttp.ReleaseResponse(resp)
...
func extractOpenAIUsageFromBytes(data []byte) *schemas.DeepIntShieldLLMUsage {
