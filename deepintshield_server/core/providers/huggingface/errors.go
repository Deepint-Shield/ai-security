...
func parseHuggingFaceImageError(resp *fasthttp.Response, meta *providerUtils.RequestMetadata) *schemas.DeepIntShieldError {
	...
	if strings.TrimSpace(errorResp.Type) != "" {
		...
		deepintshieldErr.Type = &typeCopy
	}
	...
	if len(errorResp.Detail) > 0 {
		...
		if len(errorMessages) > 0 {
			deepintshieldErr.Error.Message = strings.Join(errorMessages, "; ")
		}
