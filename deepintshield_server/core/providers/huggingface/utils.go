...
func (provider *HuggingFaceProvider) buildModelHubURL(request *schemas.DeepIntShieldListModelsRequest, inferenceProvider inferenceProvider) string {
	values := url.Values{}
...
func splitIntoModelProvider(deepintshieldModelName string) (inferenceProvider, string, error) {
	// Extract provider and model name
	t := strings.Count(deepintshieldModelName, "/")
	if t == 0 {
		return "", "", fmt.Errorf("invalid model name format: %s", deepintshieldModelName)
	}
	var prov inferenceProvider
	var model string
	if t > 1 {
		before, after, _ := strings.Cut(deepintshieldModelName, "/")
		prov = inferenceProvider(before)
		model = after
	} else if t == 1 {
		prov = ""
		model = deepintshieldModelName
	}
...
func (provider *HuggingFaceProvider) getInferenceProviderRouteURL(ctx *schemas.DeepIntShieldContext, inferenceProvider inferenceProvider, modelName string, requestType schemas.RequestType) (string, error) {
	defaultPath := ""
...
func (provider *HuggingFaceProvider) getModelInferenceProviderMapping(ctx context.Context, huggingfaceModelName string) (map[inferenceProvider]HuggingFaceInferenceProviderMapping, *schemas.DeepIntShieldError) {
...
// DeepIntShieldError indicating the operation isn't supported for the requested
// request type or provider.
func (provider *HuggingFaceProvider) getValidatedProviderModelID(ctx context.Context, inferenceProvider inferenceProvider, huggingfaceModelName string, requiredTask string, requestType schemas.RequestType) (string, *schemas.DeepIntShieldError) {
	providerName := provider.GetProviderKey()

	providerMapping, deepintshieldErr := provider.getModelInferenceProviderMapping(ctx, huggingfaceModelName)
	if deepintshieldErr != nil {
...
func (provider *HuggingFaceProvider) downloadAudioFromURL(ctx context.Context, audioURL string) ([]byte, error) {
	...
	defer wait()
	if deepintshieldErr != nil {
		return nil, fmt.Errorf("failed to download audio: %v", deepintshieldErr)
	}
