...
func ToHuggingFaceTranscriptionRequest(request *schemas.DeepIntShieldTranscriptionRequest) (*HuggingFaceTranscriptionRequest, error) {
	if request == nil {
		return nil, nil
	}
...
func (response *HuggingFaceTranscriptionResponse) ToDeepIntShieldTranscriptionResponse(requestedModel string) (*schemas.DeepIntShieldTranscriptionResponse, error) {
	if response == nil {
		return nil, nil
	}
	...
	// Create the base DeepIntShield response
	deepintshieldResponse := &schemas.DeepIntShieldTranscriptionResponse{
		Text: response.Text,
	...
	if len(response.Chunks) > 0 {
		...
		deepintshieldResponse.Segments = segments
	}
