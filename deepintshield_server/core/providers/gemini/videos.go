...
// ToGeminiVideoGenerationRequest converts a DeepIntShield video generation request to Gemini REST API format
// This creates the request body for POST /models/{model}:predictLongRunning
func ToGeminiVideoGenerationRequest(deepintshieldReq *schemas.DeepIntShieldVideoGenerationRequest) (*GeminiVideoGenerationRequest, error) {
	...
	if deepintshieldReq.Input.InputReference != nil && *deepintshieldReq.Input.InputReference != "" {
	...
	if deepintshieldReq.Params != nil && deepintshieldReq.Params.VideoURI != nil {
		instance.Video = &VideoGenerationVideoInput{
			URI: deepintshieldReq.Params.VideoURI,
		}
	}
	...
	if deepintshieldReq.Params != nil {
		...
		if deepintshieldReq.Params.NegativePrompt != nil {
			params.NegativePrompt = deepintshieldReq.Params.NegativePrompt
		}
		...
		if deepintshieldReq.Params.Seed != nil {
			params.Seed = deepintshieldReq.Params.Seed
		}

		if deepintshieldReq.Params.Audio != nil {
			params.GenerateAudio = deepintshieldReq.Params.Audio
		}

		if deepintshieldReq.Params.ExtraParams != nil {
			req.ExtraParams = deepintshieldReq.Params.ExtraParams
			if aspectRatio, ok := schemas.SafeExtractStringPointer(deepintshieldReq.Params.ExtraParams["aspectRatio"]); ok {
				params.AspectRatio = aspectRatio
			}
			if resolution, ok := schemas.SafeExtractStringPointer(deepintshieldReq.Params.ExtraParams["resolution"]); ok {
				params.Resolution = resolution
			}

			if sampleCount, ok := schemas.SafeExtractIntPointer(deepintshieldReq.Params.ExtraParams["sampleCount"]); ok {
				params.SampleCount = sampleCount
			}

			if personGeneration, ok := schemas.SafeExtractStringPointer(deepintshieldReq.Params.ExtraParams["personGeneration"]); ok {
				params.PersonGeneration = personGeneration
			}

			if numberOfVideos, ok := schemas.SafeExtractIntPointer(deepintshieldReq.Params.ExtraParams["numberOfVideos"]); ok {
				params.NumberOfVideos = numberOfVideos
			}
			if storageURI, ok := schemas.SafeExtractStringPointer(deepintshieldReq.Params.ExtraParams["storageURI"]); ok {
				params.StorageURI = storageURI
			}
			if compressionQuality, ok := schemas.SafeExtractStringPointer(deepintshieldReq.Params.ExtraParams["compressionQuality"]); ok {
			...
			if resizeMode, ok := schemas.SafeExtractStringPointer(deepintshieldReq.Params.ExtraParams["resizeMode"]); ok {
				params.ResizeMode = resizeMode
			}
			if referenceImages, ok := deepintshieldReq.Params.ExtraParams["referenceImages"]; ok {
			...
			if lastFrame, ok := deepintshieldReq.Params.ExtraParams["lastFrame"]; ok {
		...
		if params.AspectRatio == nil && deepintshieldReq.Params.Size != "" {
			aspectRatio := sizeToAspectRatio(deepintshieldReq.Params.Size)
			if aspectRatio != "" {
				params.AspectRatio = &aspectRatio
			}
		}
...
// ToDeepIntShieldVideoGenerationResponse converts Gemini operation response to DeepIntShield format
func ToDeepIntShieldVideoGenerationResponse(operation *GenerateVideosOperation, model string) (*schemas.DeepIntShieldVideoGenerationResponse, *schemas.DeepIntShieldError) {
	if operation == nil {
		return nil, providerUtils.NewDeepIntShieldOperationError("operation is nil", nil, schemas.Gemini)
	}
...
func (request *GeminiVideoGenerationRequest) ToDeepIntShieldVideoGenerationRequest(ctx *schemas.DeepIntShieldContext) (*schemas.DeepIntShieldVideoGenerationRequest, error) {
	...
	if instance.Image != nil && instance.Image.BytesBase64Encoded != nil && *instance.Image.BytesBase64Encoded != "" {
		...
		deepintshieldReq.Input.InputReference = schemas.Ptr(fmt.Sprintf("data:%s;base64,%s", mimeType, *instance.Image.BytesBase64Encoded))
	}

	// Helper to ensure params are initialized
	ensureParams := func() {
		if deepintshieldReq.Params == nil {
			deepintshieldReq.Params = &schemas.VideoGenerationParameters{
	...
	if len(instance.ReferenceImages) > 0 {
		ensureParams()
		deepintshieldReq.Params.ExtraParams["referenceImages"] = instance.ReferenceImages
	}

	// Handle video URI
	if instance.Video != nil && instance.Video.URI != nil {
		ensureParams()
		deepintshieldReq.Params.VideoURI = instance.Video.URI
	}

	// Handle last frame
	if instance.LastFrame != nil {
		ensureParams()
		deepintshieldReq.Params.ExtraParams["lastFrame"] = instance.LastFrame
	}
...
func ToGeminiVideoGenerationResponse(response *schemas.DeepIntShieldVideoGenerationResponse) *GenerateVideosOperation {
	if response == nil {
		return nil
	}
