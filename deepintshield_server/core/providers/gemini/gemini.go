...
const (
	DeepIntShieldContextKeyResponseFormat schemas.DeepIntShieldContextKey = "deepintshield_context_key_response_format"
)
...
// When large response streaming is activated (DeepIntShieldContextKeyLargeResponseMode set in ctx),
// returns (nil, nil, latency, nil) - callers must check the context flag.
func (provider *GeminiProvider) completeRequest(ctx *schemas.DeepIntShieldContext, model string, key schemas.Key, jsonBody []byte, endpoint string, meta *providerUtils.RequestMetadata) (*GenerateContentResponse, interface{}, time.Duration, map[string]string, *schemas.DeepIntShieldError) {
	...
	if err := sonic.Unmarshal(body, &geminiResponse); err != nil {
		return nil, nil, latency, providerResponseHeaders, providerUtils.NewDeepIntShieldOperationError(schemas.ErrProviderResponseUnmarshal, err, providerName)
	}
...
func (provider *GeminiProvider) listModelsByKey(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldListModelsRequest) (*schemas.DeepIntShieldListModelsResponse, *schemas.DeepIntShieldError) {
	providerName := provider.GetProviderKey()
	...
	rawRequest, rawResponse, deepintshieldErr := providerUtils.HandleProviderResponse(resp.Body(), &geminiResponse, nil, providerUtils.ShouldSendBackRawRequest(ctx, provider.sendBackRawRequest), providerUtils.ShouldSendBackRawResponse(ctx, provider.sendBackRawResponse))
	if deepintshieldErr != nil {
		return nil, deepintshieldErr
	}
	...
	response := geminiResponse.ToDeepIntShieldListModelsResponse(providerName, key.Models, request.Unfiltered)
...
func (provider *GeminiProvider) ListModels(ctx *schemas.DeepIntShieldContext, keys []schemas.Key, request *schemas.DeepIntShieldListModelsRequest) (*schemas.DeepIntShieldListModelsResponse, *schemas.DeepIntShieldError) {
...
func (provider *GeminiProvider) TextCompletion(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldTextCompletionRequest) (*schemas.DeepIntShieldTextCompletionResponse, *schemas.DeepIntShieldError) {
...
func (provider *GeminiProvider) TextCompletionStream(ctx *schemas.DeepIntShieldContext, postHookRunner schemas.PostHookRunner, key schemas.Key, request *schemas.DeepIntShieldTextCompletionRequest) (chan *schemas.DeepIntShieldStreamChunk, *schemas.DeepIntShieldError) {
	return nil, providerUtils.NewUnsupportedOperationError(schemas.TextCompletionStreamRequest, provider.GetProviderKey())
}

// ChatCompletion performs a chat completion request to the Gemini API.
func (provider *GeminiProvider) ChatCompletion(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldChatRequest) (*schemas.DeepIntShieldChatResponse, *schemas.DeepIntShieldError) {
	...
	geminiResponse, rawResponse, latency, providerResponseHeaders, deepintshieldErr := provider.completeRequest(ctx, request.Model, key, jsonData, ":generateContent", &providerUtils.RequestMetadata{
	...
	deepintshieldResponse := geminiResponse.ToDeepIntShieldChatResponse()
...
func (provider *GeminiProvider) ChatCompletionStream(ctx *schemas.DeepIntShieldContext, postHookRunner schemas.PostHookRunner, key schemas.Key, request *schemas.DeepIntShieldChatRequest) (chan *schemas.DeepIntShieldStreamChunk, *schemas.DeepIntShieldError) {
...
func HandleGeminiChatCompletionStream(
	...
	// Make the request
	streamingClient := buildStreamingResponseClient(providerUtils.ClientFromContext(ctx, client))
	...
	if doErr != nil {
		...
		if errors.Is(doErr, fasthttp.ErrTimeout) || errors.Is(doErr, context.DeadlineExceeded) {
			return nil, providerUtils.EnrichError(ctx, providerUtils.NewDeepIntShieldOperationError(schemas.ErrProviderRequestTimedOut, doErr, providerName), jsonBody, nil, sendBackRawRequest, sendBackRawResponse)
		}
		return nil, providerUtils.EnrichError(ctx, providerUtils.NewDeepIntShieldOperationError(schemas.ErrProviderDoRequest, doErr, providerName), jsonBody, nil, sendBackRawRequest, sendBackRawResponse)
	}
	...
	go func() {
		...
		for {
			...
			// Convert to DeepIntShield stream response
			response, deepintshieldErr, isLastChunk := geminiResponse.ToDeepIntShieldChatCompletionStream(streamState)
			if deepintshieldErr != nil {
...
func (provider *GeminiProvider) Responses(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldResponsesRequest) (*schemas.DeepIntShieldResponsesResponse, *schemas.DeepIntShieldError) {
	...
	if isLargePayload, ok := ctx.Value(schemas.DeepIntShieldContextKeyLargePayloadMode).(bool); ok && isLargePayload {
		if reader, readerOk := ctx.Value(schemas.DeepIntShieldContextKeyLargePayloadReader).(io.Reader); readerOk && reader != nil {
			bodyReader = reader
			if contentLength, lenOk := ctx.Value(schemas.DeepIntShieldContextKeyLargePayloadContentLength).(int); lenOk {
	...
	if responseThreshold, ok := ctx.Value(schemas.DeepIntShieldContextKeyLargeResponseThreshold).(int64); ok && responseThreshold > 0 {
	...
	deepintshieldResponse := geminiResponse.ToResponsesDeepIntShieldResponsesResponse()
...
func (provider *GeminiProvider) responsesWithLargeResponseDetection(
	...
	// Make request
	streamingClient := providerUtils.BuildLargeResponseClient(providerUtils.ClientFromContext(ctx, provider.client), responseThreshold)
	latency, deepintshieldErr, wait := providerUtils.MakeRequestWithContext(ctx, streamingClient, req, resp)
	if deepintshieldErr != nil {
		wait()
		fasthttp.ReleaseResponse(resp)
		return nil, deepintshieldErr
	...
	if resp.StatusCode() != fasthttp.StatusOK {
		providerUtils.MaterializeStreamErrorBody(ctx, resp)
		deepintshieldErr := parseGeminiError(resp, meta)
		wait()
		fasthttp.ReleaseResponse(resp)
		return nil, providerUtils.EnrichError(ctx, deepintshieldErr, jsonData, nil, provider.sendBackRawRequest, provider.sendBackRawResponse)
	}
	...
	if isLarge {
		...
		preview, _ := ctx.Value(schemas.DeepIntShieldContextKeyLargePayloadResponsePreview).(string)
	...
	if unmarshalErr := sonic.Unmarshal(responseBody, &geminiResponse); unmarshalErr != nil {
		return nil, providerUtils.NewDeepIntShieldOperationError(schemas.ErrProviderResponseUnmarshal, unmarshalErr, providerName)
	}
	deepintshieldResponse := geminiResponse.ToResponsesDeepIntShieldResponsesResponse()
	deepintshieldResponse.ExtraFields.Provider = providerName
	deepintshieldResponse.ExtraFields.ModelRequested = request.Model
...
func (provider *GeminiProvider) ResponsesStream(ctx *schemas.DeepIntShieldContext, postHookRunner schemas.PostHookRunner, key schemas.Key, request *schemas.DeepIntShieldResponsesRequest) (chan *schemas.DeepIntShieldStreamChunk, *schemas.DeepIntShieldError) {
...
func HandleGeminiResponsesStream(
	...
	go func() {
		...
		if resp.BodyStream() == nil {
			...
			ctx.SetValue(schemas.DeepIntShieldContextKeyStreamEndIndicator, true)
			providerUtils.ProcessAndSendDeepIntShieldError(
				ctx,
				postHookRunner,
				providerUtils.EnrichError(ctx, deepintshieldErr, jsonBody, nil, sendBackRawRequest, sendBackRawResponse),
				responseChan,
				logger,
			)
			return
		...
		for {
			...
			// Convert to DeepIntShield responses stream response
			responses, deepintshieldErr := geminiResponse.ToDeepIntShieldResponsesStream(sequenceNumber, streamState)
			if deepintshieldErr != nil {
				deepintshieldErr.ExtraFields = schemas.DeepIntShieldErrorExtraFields{
					RequestType:    schemas.ResponsesStreamRequest,
					Provider:       providerName,
...
func (provider *GeminiProvider) Embedding(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldEmbeddingRequest) (*schemas.DeepIntShieldEmbeddingResponse, *schemas.DeepIntShieldError) {
	...
	// Convert DeepIntShield request to Gemini batch embedding request format
	jsonData, err := providerUtils.CheckContextAndGetRequestBody(
		ctx,
		request,
		func() (providerUtils.RequestBodyWithExtraParams, error) {
	...
	rawRequest, rawResponse, deepintshieldErr := providerUtils.HandleProviderResponse(body, &geminiResponse, jsonData,
		providerUtils.ShouldSendBackRawRequest(ctx, provider.sendBackRawRequest),
		providerUtils.ShouldSendBackRawResponse(ctx, provider.sendBackRawResponse))
	if deepintshieldErr != nil {
		return nil, providerUtils.EnrichError(ctx, deepintshieldErr, jsonData, body, provider.sendBackRawRequest, provider.sendBackRawResponse)
	}

	// Convert to DeepIntShield format
	deepintshieldResponse := ToDeepIntShieldEmbeddingResponse(&geminiResponse, request.Model)
	if deepintshieldResponse == nil {
		return nil, providerUtils.NewDeepIntShieldOperationError(schemas.ErrProviderResponseUnmarshal,
			fmt.Errorf("failed to convert Gemini embedding response to DeepIntShield format"), providerName)
	}
...
func (provider *GeminiProvider) Speech(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldSpeechRequest) (*schemas.DeepIntShieldSpeechResponse, *schemas.DeepIntShieldError) {
	...
	if request.Params != nil {
		ctx.SetValue(DeepIntShieldContextKeyResponseFormat, request.Params.ResponseFormat)
	}
	response, convErr := geminiResponse.ToDeepIntShieldSpeechResponse(ctx)
	if convErr != nil {
...
func (provider *GeminiProvider) Rerank(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldRerankRequest) (*schemas.DeepIntShieldRerankResponse, *schemas.DeepIntShieldError) {
	return nil, providerUtils.NewUnsupportedOperationError(schemas.RerankRequest, provider.GetProviderKey())
}

// SpeechStream performs a streaming speech synthesis request to the Gemini API.
func (provider *GeminiProvider) SpeechStream(ctx *schemas.DeepIntShieldContext, postHookRunner schemas.PostHookRunner, key schemas.Key, request *schemas.DeepIntShieldSpeechRequest) (chan *schemas.DeepIntShieldStreamChunk, *schemas.DeepIntShieldError) {
	...
	// Make the request
	streamingClient := buildStreamingResponseClient(providerUtils.ClientFromContext(ctx, provider.client))
	...
	go func() {
		...
		for {
			...
			if len(audioChunk) > 0 {
				...
				// Create DeepIntShield speech response for streaming
...
func (provider *GeminiProvider) Transcription(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldTranscriptionRequest) (*schemas.DeepIntShieldTranscriptionResponse, *schemas.DeepIntShieldError) {
	...
	response := geminiResponse.ToDeepIntShieldTranscriptionResponse()
...
func (provider *GeminiProvider) TranscriptionStream(ctx *schemas.DeepIntShieldContext, postHookRunner schemas.PostHookRunner, key schemas.Key, request *schemas.DeepIntShieldTranscriptionRequest) (chan *schemas.DeepIntShieldStreamChunk, *schemas.DeepIntShieldError) {
	...
	if err != nil {
		...
		return nil, providerUtils.EnrichError(ctx, providerUtils.NewDeepIntShieldOperationError(schemas.ErrProviderDoRequest, err, provider.GetProviderKey()), jsonBody, nil, provider.sendBackRawRequest, provider.sendBackRawResponse)
	}
	...
	go func() {
		...
		for {
			...
			if deltaText != "" {
				...
				// Create DeepIntShield transcription response for streaming
...
func (provider *GeminiProvider) ImageGeneration(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldImageGenerationRequest) (*schemas.DeepIntShieldImageGenerationResponse, *schemas.DeepIntShieldError) {
	...
	response, deepintshieldErr := geminiResponse.ToDeepIntShieldImageGenerationResponse()
	if deepintshieldErr != nil {
		deepintshieldErr.ExtraFields = schemas.DeepIntShieldErrorExtraFields{
			Provider:       provider.GetProviderKey(),
			ModelRequested: request.Model,
	...
	if response == nil {
		return nil, providerUtils.NewDeepIntShieldOperationError(
			...
			fmt.Errorf("ToDeepIntShieldImageGenerationResponse returned nil response"),
			provider.GetProviderKey(),
		)
	}
...
func (provider *GeminiProvider) handleImagenImageGeneration(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldImageGenerationRequest) (*schemas.DeepIntShieldImageGenerationResponse, *schemas.DeepIntShieldError) {
	providerName := provider.GetProviderKey()
	...
	rawRequest, rawResponse, deepintshieldErr := providerUtils.HandleProviderResponse(body, &imagenResponse, jsonData, providerUtils.ShouldSendBackRawRequest(ctx, provider.sendBackRawRequest), providerUtils.ShouldSendBackRawResponse(ctx, provider.sendBackRawResponse))
	if deepintshieldErr != nil {
		return nil, deepintshieldErr
	}
	// Convert to DeepIntShield format
	response := imagenResponse.ToDeepIntShieldImageGenerationResponse()
...
func (provider *GeminiProvider) ImageGenerationStream(ctx *schemas.DeepIntShieldContext, postHookRunner schemas.PostHookRunner, key schemas.Key, request *schemas.DeepIntShieldImageGenerationRequest) (chan *schemas.DeepIntShieldStreamChunk, *schemas.DeepIntShieldError) {
...
func (provider *GeminiProvider) ImageEdit(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldImageEditRequest) (*schemas.DeepIntShieldImageGenerationResponse, *schemas.DeepIntShieldError) {
...
func (provider *GeminiProvider) ImageEditStream(ctx *schemas.DeepIntShieldContext, postHookRunner schemas.PostHookRunner, key schemas.Key, request *schemas.DeepIntShieldImageEditRequest) (chan *schemas.DeepIntShieldStreamChunk, *schemas.DeepIntShieldError) {
	return nil, providerUtils.NewUnsupportedOperationError(schemas.ImageEditStreamRequest, provider.GetProviderKey())
}

// ImageVariation is not supported by the Gemini provider.
func (provider *GeminiProvider) ImageVariation(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldImageVariationRequest) (*schemas.DeepIntShieldImageGenerationResponse, *schemas.DeepIntShieldError) {
...
func (provider *GeminiProvider) VideoGeneration(ctx *schemas.DeepIntShieldContext, key schemas.Key, deepintshieldReq *schemas.DeepIntShieldVideoGenerationRequest) (*schemas.DeepIntShieldVideoGenerationResponse, *schemas.DeepIntShieldError) {
	...
	jsonData, deepintshieldErr := providerUtils.CheckContextAndGetRequestBody(
		...
		deepintshieldReq,
		func() (providerUtils.RequestBodyWithExtraParams, error) {
			return ToGeminiVideoGenerationRequest(deepintshieldReq)
		},
		providerName,
	)
	if deepintshieldErr != nil {
	...
	rawRequest, rawResponse, deepintshieldErr := providerUtils.HandleProviderResponse(body, &operation, jsonData, providerUtils.ShouldSendBackRawRequest(ctx, provider.sendBackRawRequest), providerUtils.ShouldSendBackRawResponse(ctx, provider.sendBackRawResponse))
	if deepintshieldErr != nil {
		return nil, deepintshieldErr
	}

	// Convert to DeepIntShield response
	deepintshieldResp, deepintshieldErr := ToDeepIntShieldVideoGenerationResponse(&operation, model)
	if deepintshieldErr != nil {
		return nil, deepintshieldErr
	}
	...
	deepintshieldResp.ExtraFields.ModelRequested = model
	deepintshieldResp.ExtraFields.RequestType = schemas.VideoGenerationRequest
...
func (provider *GeminiProvider) VideoRetrieve(ctx *schemas.DeepIntShieldContext, key schemas.Key, deepintshieldReq *schemas.DeepIntShieldVideoRetrieveRequest) (*schemas.DeepIntShieldVideoGenerationResponse, *schemas.DeepIntShieldError) {
	...
	providerName := provider.GetProviderKey()
	operationID := deepintshieldReq.ID
	...
	// Parse response
	var operation GenerateVideosOperation
	_, rawResponse, deepintshieldErr := providerUtils.HandleProviderResponse(resp.Body(), &operation, nil, providerUtils.ShouldSendBackRawRequest(ctx, provider.sendBackRawRequest), providerUtils.ShouldSendBackRawResponse(ctx, provider.sendBackRawResponse))
	if deepintshieldErr != nil {
		return nil, deepintshieldErr
	}

	deepintshieldResp, deepintshieldErr := ToDeepIntShieldVideoGenerationResponse(&operation, "")
	if deepintshieldErr != nil {
		return nil, deepintshieldErr
	}
	deepintshieldResp.ID = providerUtils.AddVideoIDProviderSuffix(deepintshieldResp.ID, providerName)
...
func (provider *GeminiProvider) VideoDownload(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldVideoDownloadRequest) (*schemas.DeepIntShieldVideoDownloadResponse, *schemas.DeepIntShieldError) {
	...
	if videoResp.Videos[0].Type == schemas.VideoOutputTypeBase64 && videoResp.Videos[0].Base64Data != nil {
		...
		if err != nil {
			return nil, providerUtils.NewDeepIntShieldOperationError("failed to decode base64 video data", err, providerName)
		}
		content = decoded
...
func (provider *GeminiProvider) VideoDelete(_ *schemas.DeepIntShieldContext, _ schemas.Key, _ *schemas.DeepIntShieldVideoDeleteRequest) (*schemas.DeepIntShieldVideoDeleteResponse, *schemas.DeepIntShieldError) {
	return nil, providerUtils.NewUnsupportedOperationError(schemas.VideoDeleteRequest, provider.GetProviderKey())
}

// VideoList is not supported by the Gemini provider.
func (provider *GeminiProvider) VideoList(_ *schemas.DeepIntShieldContext, _ schemas.Key, _ *schemas.DeepIntShieldVideoListRequest) (*schemas.DeepIntShieldVideoListResponse, *schemas.DeepIntShieldError) {
	return nil, providerUtils.NewUnsupportedOperationError(schemas.VideoListRequest, provider.GetProviderKey())
}

// VideoRemix is not supported by the Gemini provider.
func (provider *GeminiProvider) VideoRemix(_ *schemas.DeepIntShieldContext, _ schemas.Key, _ *schemas.DeepIntShieldVideoRemixRequest) (*schemas.DeepIntShieldVideoGenerationResponse, *schemas.DeepIntShieldError) {
...
func (provider *GeminiProvider) BatchCreate(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldBatchCreateRequest) (*schemas.DeepIntShieldBatchCreateResponse, *schemas.DeepIntShieldError) {
	...
	if !hasFileInput && !hasInlineRequests {
		return nil, providerUtils.NewDeepIntShieldOperationError("either input_file_id or requests must be provided", nil, providerName)
	}

	if hasFileInput && hasInlineRequests {
		return nil, providerUtils.NewDeepIntShieldOperationError("cannot specify both input_file_id and requests", nil, providerName)
	}

	// Build the batch request with proper nested structure
	batchReq := &GeminiBatchCreateRequest{
		Batch: GeminiBatchConfig{
			DisplayName: fmt.Sprintf("deepintshield-batch-%d", time.Now().UnixNano()),
		},
	}
	...
	if hasFileInput {
		...
		// Inline requests: convert DeepIntShield requests to Gemini format
		geminiRequests := make([]GeminiBatchRequestItem, len(request.Requests))
		for i, deepintshieldItem := range request.Requests {
			body := deepintshieldItem.Body
			...
			if rawMessages, ok := body["messages"]; ok {
				...
				if err != nil {
					return nil, providerUtils.NewDeepIntShieldOperationError("failed to marshal messages", err, providerName)
				}
				var chatMessages []schemas.ChatMessage
				err = sonic.Unmarshal(messagesBytes, &chatMessages)
				if err != nil {
					return nil, providerUtils.NewDeepIntShieldOperationError("failed to unmarshal messages", err, providerName)
				}

				contents, systemInstruction := convertDeepIntShieldMessagesToGemini(chatMessages)
				...
				if err != nil {
					return nil, providerUtils.NewDeepIntShieldOperationError("failed to marshal gemini request", err, providerName)
				}
				err = sonic.Unmarshal(requestBytes, &geminiReq)
				if err != nil {
					return nil, providerUtils.NewDeepIntShieldOperationError("failed to unmarshal gemini request", err, providerName)
				}
			}
			...
			if deepintshieldItem.CustomID != "" {
				geminiRequests[i].Metadata = &GeminiBatchMetadata{
					Key: deepintshieldItem.CustomID,
				}
			}
		}
	...
	if err := sonic.Unmarshal(body, &geminiResp); err != nil {
		...
		return nil, providerUtils.EnrichError(ctx, providerUtils.NewDeepIntShieldOperationError(schemas.ErrProviderResponseUnmarshal, err, providerName), jsonData, body, provider.sendBackRawRequest, provider.sendBackRawResponse)
	}
	// Check for metadata
	if geminiResp.Metadata == nil {
		return nil, providerUtils.NewDeepIntShieldOperationError("gemini batch response missing metadata", nil, providerName)
	}
	// Check for batch stats
	if geminiResp.Metadata.BatchStats == nil {
		return nil, providerUtils.NewDeepIntShieldOperationError("gemini batch response missing batch stats", nil, providerName)
	}
	...
	// Determine status
	status := ToDeepIntShieldBatchStatus(geminiResp.Metadata.State)
...
func (provider *GeminiProvider) batchListByKey(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldBatchListRequest) (*schemas.DeepIntShieldBatchListResponse, time.Duration, *schemas.DeepIntShieldError) {
	providerName := provider.GetProviderKey()
	...
	defer wait()
	if deepintshieldErr != nil {
		return nil, latency, deepintshieldErr
	}
	...
	body, err := providerUtils.CheckAndDecodeBody(resp)
	if err != nil {
		return nil, latency, providerUtils.NewDeepIntShieldOperationError(schemas.ErrProviderResponseDecode, err, providerName)
	}

	var geminiResp GeminiBatchListResponse
	if err := sonic.Unmarshal(body, &geminiResp); err != nil {
		return nil, latency, providerUtils.NewDeepIntShieldOperationError(schemas.ErrProviderResponseUnmarshal, err, providerName)
	}

	// Convert to DeepIntShield format
	data := make([]schemas.DeepIntShieldBatchRetrieveResponse, 0, len(geminiResp.Operations))
	for _, batch := range geminiResp.Operations {
		data = append(data, schemas.DeepIntShieldBatchRetrieveResponse{
			ID:            extractBatchIDFromName(batch.Name),
			Object:        "batch",
			Status:        ToDeepIntShieldBatchStatus(batch.Metadata.State),
...
func (provider *GeminiProvider) BatchList(ctx *schemas.DeepIntShieldContext, keys []schemas.Key, request *schemas.DeepIntShieldBatchListRequest) (*schemas.DeepIntShieldBatchListResponse, *schemas.DeepIntShieldError) {
	...
	if len(keys) == 0 {
		return nil, providerUtils.NewDeepIntShieldOperationError("no keys provided for batch list", nil, providerName)
	}
	...
	resp, latency, deepintshieldErr := provider.batchListByKey(ctx, key, &modifiedRequest)
	if deepintshieldErr != nil {
		return nil, deepintshieldErr
	}
	...
	result := &schemas.DeepIntShieldBatchListResponse{
		Object:  "list",
		Data:    resp.Data,
		HasMore: hasMore,
		ExtraFields: schemas.DeepIntShieldResponseExtraFields{
...
func (provider *GeminiProvider) batchRetrieveByKey(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldBatchRetrieveRequest) (*schemas.DeepIntShieldBatchRetrieveResponse, *schemas.DeepIntShieldError) {
	providerName := provider.GetProviderKey()
	...
	return &schemas.DeepIntShieldBatchRetrieveResponse{
		ID:            geminiResp.Metadata.Name,
		Object:        "batch",
		Status:        ToDeepIntShieldBatchStatus(geminiResp.Metadata.State),
		CreatedAt:     parseGeminiTimestamp(geminiResp.Metadata.CreateTime),
...
func (provider *GeminiProvider) BatchRetrieve(ctx *schemas.DeepIntShieldContext, keys []schemas.Key, request *schemas.DeepIntShieldBatchRetrieveRequest) (*schemas.DeepIntShieldBatchRetrieveResponse, *schemas.DeepIntShieldError) {
	...
	if request.BatchID == "" {
		return nil, providerUtils.NewDeepIntShieldOperationError("batch_id is required", nil, providerName)
	}

	if len(keys) == 0 {
		return nil, providerUtils.NewDeepIntShieldOperationError("no keys provided for batch retrieve", nil, providerName)
	}

	// Try each key until we find the batch
	var lastError *schemas.DeepIntShieldError
...
func (provider *GeminiProvider) batchCancelByKey(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldBatchCancelRequest) (*schemas.DeepIntShieldBatchCancelResponse, *schemas.DeepIntShieldError) {
	providerName := provider.GetProviderKey()
...
func (provider *GeminiProvider) BatchCancel(ctx *schemas.DeepIntShieldContext, keys []schemas.Key, request *schemas.DeepIntShieldBatchCancelRequest) (*schemas.DeepIntShieldBatchCancelResponse, *schemas.DeepIntShieldError) {
	...
	if len(keys) == 0 {
		return nil, providerUtils.NewDeepIntShieldOperationError("no keys provided for batch cancel", nil, providerName)
	}
...
func (provider *GeminiProvider) batchDeleteByKey(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldBatchDeleteRequest) (*schemas.DeepIntShieldBatchDeleteResponse, *schemas.DeepIntShieldError) {
	providerName := provider.GetProviderKey()
	...
	return &schemas.DeepIntShieldBatchDeleteResponse{
		ID:     request.BatchID,
		Object: "batch",
...
// It does not cancel the operation. If the server doesn't support this method, it returns UNIMPLEMENTED.
func (provider *GeminiProvider) BatchDelete(ctx *schemas.DeepIntShieldContext, keys []schemas.Key, request *schemas.DeepIntShieldBatchDeleteRequest) (*schemas.DeepIntShieldBatchDeleteResponse, *schemas.DeepIntShieldError) {
	...
	if len(keys) == 0 {
		return nil, providerUtils.NewDeepIntShieldOperationError("no keys provided for batch delete", nil, providerName)
	}
...
func shouldSkipInlineDataForStreamingContext(ctx *schemas.DeepIntShieldContext) bool {
	if ctx == nil {
		return false
	}
	if isLargePayload, ok := ctx.Value(schemas.DeepIntShieldContextKeyLargePayloadMode).(bool); ok && isLargePayload {
...
func (provider *GeminiProvider) batchResultsByKey(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldBatchResultsRequest) (*schemas.DeepIntShieldBatchResultsResponse, *schemas.DeepIntShieldError) {
	providerName := provider.GetProviderKey()
	...
	if geminiResp.Dest != nil && geminiResp.Dest.FileName != "" {
		...
		fileResults, fileParseErrors, deepintshieldErr := provider.downloadBatchResultsFile(ctx, key, geminiResp.Dest.FileName)
		if deepintshieldErr != nil {
			return nil, deepintshieldErr
		}
		results = fileResults
...
func (provider *GeminiProvider) BatchResults(ctx *schemas.DeepIntShieldContext, keys []schemas.Key, request *schemas.DeepIntShieldBatchResultsRequest) (*schemas.DeepIntShieldBatchResultsResponse, *schemas.DeepIntShieldError) {
	...
	if len(keys) == 0 {
		return nil, providerUtils.NewDeepIntShieldOperationError("no keys provided for batch results", nil, providerName)
	}
...
func (provider *GeminiProvider) FileUpload(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldFileUploadRequest) (*schemas.DeepIntShieldFileUploadResponse, *schemas.DeepIntShieldError) {
	...
	if err != nil {
		return nil, providerUtils.NewDeepIntShieldOperationError("failed to create metadata field", err, providerName)
	}
	...
	return &schemas.DeepIntShieldFileUploadResponse{
		...
		Status:         ToDeepIntShieldFileStatus(geminiResp.State),
...
func (provider *GeminiProvider) fileListByKey(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldFileListRequest) (*schemas.DeepIntShieldFileListResponse, time.Duration, *schemas.DeepIntShieldError) {
	providerName := provider.GetProviderKey()
	...
	if geminiResp.NextPageToken != "" {
		deepintshieldResp.After = &geminiResp.NextPageToken
	}
	...
	for i, file := range geminiResp.Files {
		...
		deepintshieldResp.Data[i] = schemas.FileObject{
			ID:        file.Name,
			Object:    "file",
			Bytes:     sizeBytes,
			CreatedAt: createdAt,
			...
			Status:    ToDeepIntShieldFileStatus(file.State),
			ExpiresAt: expiresAt,
		}
	}

	return deepintshieldResp, latency, nil
}

// FileList lists files from Gemini across all provided keys.
// FileList lists files using serial pagination across keys.
// Exhausts all pages from one key before moving to the next.
func (provider *GeminiProvider) FileList(ctx *schemas.DeepIntShieldContext, keys []schemas.Key, request *schemas.DeepIntShieldFileListRequest) (*schemas.DeepIntShieldFileListResponse, *schemas.DeepIntShieldError) {
	...
	if len(keys) == 0 {
		return nil, providerUtils.NewDeepIntShieldOperationError("no keys provided for file list", nil, providerName)
	}
	...
	// Call the single-key helper
	resp, latency, deepintshieldErr := provider.fileListByKey(ctx, key, &modifiedRequest)
	if deepintshieldErr != nil {
		return nil, deepintshieldErr
	}
	...
	result := &schemas.DeepIntShieldFileListResponse{
		Object:  "list",
		Data:    resp.Data,
		HasMore: hasMore,
		ExtraFields: schemas.DeepIntShieldResponseExtraFields{
...
func (provider *GeminiProvider) fileRetrieveByKey(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldFileRetrieveRequest) (*schemas.DeepIntShieldFileRetrieveResponse, *schemas.DeepIntShieldError) {
	providerName := provider.GetProviderKey()
...
func (provider *GeminiProvider) FileRetrieve(ctx *schemas.DeepIntShieldContext, keys []schemas.Key, request *schemas.DeepIntShieldFileRetrieveRequest) (*schemas.DeepIntShieldFileRetrieveResponse, *schemas.DeepIntShieldError) {
	...
	if len(keys) == 0 {
		return nil, providerUtils.NewDeepIntShieldOperationError("no keys provided for file retrieve", nil, providerName)
	}
...
func (provider *GeminiProvider) fileDeleteByKey(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldFileDeleteRequest) (*schemas.DeepIntShieldFileDeleteResponse, *schemas.DeepIntShieldError) {
	providerName := provider.GetProviderKey()
...
func (provider *GeminiProvider) FileDelete(ctx *schemas.DeepIntShieldContext, keys []schemas.Key, request *schemas.DeepIntShieldFileDeleteRequest) (*schemas.DeepIntShieldFileDeleteResponse, *schemas.DeepIntShieldError) {
	...
	if len(keys) == 0 {
		return nil, providerUtils.NewDeepIntShieldOperationError("no keys provided for file delete", nil, providerName)
	}
...
func (provider *GeminiProvider) FileContent(ctx *schemas.DeepIntShieldContext, keys []schemas.Key, request *schemas.DeepIntShieldFileContentRequest) (*schemas.DeepIntShieldFileContentResponse, *schemas.DeepIntShieldError) {
...
func (provider *GeminiProvider) CountTokens(ctx *schemas.DeepIntShieldContext, key schemas.Key, request *schemas.DeepIntShieldResponsesRequest) (*schemas.DeepIntShieldCountTokensResponse, *schemas.DeepIntShieldError) {
	...
	if v, ok := ctx.Value(schemas.DeepIntShieldContextKeyLargePayloadMode).(bool); ok && v {
		isLargePayload = true
	}
	...
	if !isLargePayload {
		// Build JSON body from DeepIntShield request for normal path.
		jsonData, deepintshieldErr = providerUtils.CheckContextAndGetRequestBody(
			ctx,
			request,
			func() (providerUtils.RequestBodyWithExtraParams, error) {
		...
		if err := sonic.Unmarshal(jsonData, &payload); err == nil {
			...
			if err != nil {
				return nil, providerUtils.NewDeepIntShieldOperationError(schemas.ErrProviderRequestMarshal, err, provider.GetProviderKey())
			}
			jsonData = newData
		}
	}
	...
	if strings.TrimSpace(request.Model) == "" {
		return nil, providerUtils.NewDeepIntShieldOperationError("model is required for Gemini count tokens request", fmt.Errorf("missing model"), providerName)
	}
	...
	response := geminiResponse.ToDeepIntShieldCountTokensResponse(request.Model)
...
func (provider *GeminiProvider) ContainerCreate(_ *schemas.DeepIntShieldContext, _ schemas.Key, _ *schemas.DeepIntShieldContainerCreateRequest) (*schemas.DeepIntShieldContainerCreateResponse, *schemas.DeepIntShieldError) {
	return nil, providerUtils.NewUnsupportedOperationError(schemas.ContainerCreateRequest, provider.GetProviderKey())
}

// ContainerList is not supported by the Gemini provider.
func (provider *GeminiProvider) ContainerList(_ *schemas.DeepIntShieldContext, _ []schemas.Key, _ *schemas.DeepIntShieldContainerListRequest) (*schemas.DeepIntShieldContainerListResponse, *schemas.DeepIntShieldError) {
	return nil, providerUtils.NewUnsupportedOperationError(schemas.ContainerListRequest, provider.GetProviderKey())
}

// ContainerRetrieve is not supported by the Gemini provider.
func (provider *GeminiProvider) ContainerRetrieve(_ *schemas.DeepIntShieldContext, _ []schemas.Key, _ *schemas.DeepIntShieldContainerRetrieveRequest) (*schemas.DeepIntShieldContainerRetrieveResponse, *schemas.DeepIntShieldError) {
	return nil, providerUtils.NewUnsupportedOperationError(schemas.ContainerRetrieveRequest, provider.GetProviderKey())
}

// ContainerDelete is not supported by the Gemini provider.
func (provider *GeminiProvider) ContainerDelete(_ *schemas.DeepIntShieldContext, _ []schemas.Key, _ *schemas.DeepIntShieldContainerDeleteRequest) (*schemas.DeepIntShieldContainerDeleteResponse, *schemas.DeepIntShieldError) {
	return nil, providerUtils.NewUnsupportedOperationError(schemas.ContainerDeleteRequest, provider.GetProviderKey())
}

// ContainerFileCreate is not supported by the Gemini provider.
func (provider *GeminiProvider) ContainerFileCreate(_ *schemas.DeepIntShieldContext, _ schemas.Key, _ *schemas.DeepIntShieldContainerFileCreateRequest) (*schemas.DeepIntShieldContainerFileCreateResponse, *schemas.DeepIntShieldError) {
	return nil, providerUtils.NewUnsupportedOperationError(schemas.ContainerFileCreateRequest, provider.GetProviderKey())
}

// ContainerFileList is not supported by the Gemini provider.
func (provider *GeminiProvider) ContainerFileList(_ *schemas.DeepIntShieldContext, _ []schemas.Key, _ *schemas.DeepIntShieldContainerFileListRequest) (*schemas.DeepIntShieldContainerFileListResponse, *schemas.DeepIntShieldError) {
	return nil, providerUtils.NewUnsupportedOperationError(schemas.ContainerFileListRequest, provider.GetProviderKey())
}

// ContainerFileRetrieve is not supported by the Gemini provider.
func (provider *GeminiProvider) ContainerFileRetrieve(_ *schemas.DeepIntShieldContext, _ []schemas.Key, _ *schemas.DeepIntShieldContainerFileRetrieveRequest) (*schemas.DeepIntShieldContainerFileRetrieveResponse, *schemas.DeepIntShieldError) {
	return nil, providerUtils.NewUnsupportedOperationError(schemas.ContainerFileRetrieveRequest, provider.GetProviderKey())
}

// ContainerFileContent is not supported by the Gemini provider.
func (provider *GeminiProvider) ContainerFileContent(_ *schemas.DeepIntShieldContext, _ []schemas.Key, _ *schemas.DeepIntShieldContainerFileContentRequest) (*schemas.DeepIntShieldContainerFileContentResponse, *schemas.DeepIntShieldError) {
	return nil, providerUtils.NewUnsupportedOperationError(schemas.ContainerFileContentRequest, provider.GetProviderKey())
}

// ContainerFileDelete is not supported by the Gemini provider.
func (provider *GeminiProvider) ContainerFileDelete(_ *schemas.DeepIntShieldContext, _ []schemas.Key, _ *schemas.DeepIntShieldContainerFileDeleteRequest) (*schemas.DeepIntShieldContainerFileDeleteResponse, *schemas.DeepIntShieldError) {
