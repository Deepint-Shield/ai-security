...
func (r *GeminiGenerationRequest) convertGenerationConfigToResponsesParameters() *schemas.ResponsesParameters {
	...
	if config.ThinkingConfig != nil {
		...
		if config.ThinkingConfig.ThinkingBudget != nil {
			...
			// Map Gemini thinking level to DeepIntShield effort
			switch level {
			case "minimal":
				effort = "minimal"
			case "low":
...
var (
	// Maps Gemini finish reasons to DeepIntShield format
	geminiFinishReasonToDeepIntShield = map[FinishReason]string{
...
// ConvertGeminiFinishReasonToDeepIntShield converts Gemini finish reasons to DeepIntShield format
func ConvertGeminiFinishReasonToDeepIntShield(providerReason FinishReason) string {
	if deepintshieldReason, ok := geminiFinishReasonToDeepIntShield[providerReason]; ok {
		return deepintshieldReason
	}
	return string(providerReason)
}

// ConvertGeminiUsageMetadataToChatUsage converts Gemini usage metadata to DeepIntShield chat LLM usage
func ConvertGeminiUsageMetadataToChatUsage(metadata *GenerateContentResponseUsageMetadata) *schemas.DeepIntShieldLLMUsage {
	if metadata == nil {
		return nil
	}
...
// convertGeminiUsageMetadataToSpeechUsage converts Gemini usage metadata to DeepIntShield speech usage
...
// convertDeepIntShieldSpeechUsageToGeminiUsageMetadata converts DeepIntShield speech usage to Gemini usage metadata
func convertDeepIntShieldSpeechUsageToGeminiUsageMetadata(usage *schemas.SpeechUsage) *GenerateContentResponseUsageMetadata {
	if usage == nil {
		return nil
	}
...
// convertGeminiUsageMetadataToTranscriptionUsage converts Gemini usage metadata to DeepIntShield transcription usage
...
// convertDeepIntShieldTranscriptionUsageToGeminiUsageMetadata converts DeepIntShield transcription usage to Gemini usage metadata
func convertDeepIntShieldTranscriptionUsageToGeminiUsageMetadata(usage *schemas.TranscriptionUsage) *GenerateContentResponseUsageMetadata {
	if usage == nil {
		return nil
	}
...
// convertGeminiUsageMetadataToImageUsage converts Gemini usage metadata to DeepIntShield image usage
...
// convertDeepIntShieldImageUsageToGeminiUsageMetadata converts DeepIntShield image usage to Gemini usage metadata
func convertDeepIntShieldImageUsageToGeminiUsageMetadata(usage *schemas.ImageUsage) *GenerateContentResponseUsageMetadata {
	if usage == nil {
		return nil
	}
...
// ConvertGeminiUsageMetadataToResponsesUsage converts Gemini usage metadata to DeepIntShield responses usage
...
func ConvertDeepIntShieldResponsesUsageToGeminiUsageMetadata(usage *schemas.ResponsesResponseUsage) *GenerateContentResponseUsageMetadata {
	if usage == nil {
		return nil
	}
	metadata := &GenerateContentResponseUsageMetadata{
...
// convertParamsToGenerationConfig converts DeepIntShield parameters to Gemini GenerationConfig
...
// convertDeepIntShieldToolsToGemini converts DeepIntShield tools to Gemini format
func convertDeepIntShieldToolsToGemini(deepintshieldTools []schemas.ChatTool) []Tool {
	geminiTool := Tool{}

	for _, tool := range deepintshieldTools {
		if tool.Type == "" {
			continue
		}
...
// convertFunctionParametersToSchema converts DeepIntShield function parameters to Gemini Schema
...
// convertToolChoiceToToolConfig converts DeepIntShield tool choice to Gemini tool config
...
// convertDeepIntShieldMessagesToGemini converts DeepIntShield messages to Gemini format
func convertDeepIntShieldMessagesToGemini(messages []schemas.ChatMessage) ([]Content, *Content) {
					...
					if part.ThoughtSignature == nil && len(message.ChatAssistantMessage.ReasoningDetails) > 0 {
						...
						for _, reasoningDetail := range message.ChatAssistantMessage.ReasoningDetails {
							if reasoningDetail.ID != nil && *reasoningDetail.ID == lookupID &&
								reasoningDetail.Type == schemas.DeepIntShieldReasoningDetailsTypeEncrypted &&
...
func downloadImageFromURL(ctx context.Context, imageURL string) (string, error) {
	...
	if deepintshieldErr != nil {
		return "", fmt.Errorf("failed to download image: %v", deepintshieldErr)
	}
...
// ConvertGeminiLogprobsResultToDeepIntShield converts a Gemini LogprobsResult to DeepIntShield DeepIntShieldLogProbs
func ConvertGeminiLogprobsResultToDeepIntShield(result *LogprobsResult) *schemas.DeepIntShieldLogProbs {
	...
	return &schemas.DeepIntShieldLogProbs{Content: content}
}
