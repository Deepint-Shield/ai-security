...
func TestEmptyCandidatesRegression(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var deepintshieldResp *schemas.DeepIntShieldChatResponse

			if tt.isStream {
				deepintshieldResp, _, _ = tt.response.ToDeepIntShieldChatCompletionStream(gemini.NewGeminiStreamState())
			} else {
				deepintshieldResp = tt.response.ToDeepIntShieldChatResponse()
			}

			// Critical: Choices must NEVER be empty (this was the PR #1018 bug)
			require.NotNil(t, deepintshieldResp, "Response should not be nil")
			require.NotEmpty(t, deepintshieldResp.Choices, "Empty choices array")
			require.Len(t, deepintshieldResp.Choices, 1, "Should have exactly one error choice")
...
func TestThoughtSignatureInToolCalls(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NotNil(t, deepintshieldResp, "Response should not be nil")
			require.NotEmpty(t, deepintshieldResp.Choices, "Should have choices")
			...
			for _, detail := range reasoningDetails {
				if detail.Type == schemas.DeepIntShieldReasoningDetailsTypeEncrypted && detail.Signature != nil {
					foundEncrypted = true
					break
				}
			}
...
// TestDeepIntShieldToGeminiToolConversion tests the conversion of tools from DeepIntShield to Gemini format
func TestDeepIntShieldToGeminiToolConversion(t *testing.T) {
	tests := []struct {
		name     string
...
func TestDeepIntShieldToGeminiToolConversion_PropertyOrdering(t *testing.T) {
	input := &schemas.DeepIntShieldChatRequest{
		Model: "gemini-2.0-flash",
		Input: []schemas.ChatMessage{{
			Role:    schemas.ChatMessageRoleUser,
...
func TestDeepIntShieldToGeminiToolConversion_NestedPropertyOrdering(t *testing.T) {
	input := &schemas.DeepIntShieldChatRequest{
		Model: "gemini-2.0-flash",
		Input: []schemas.ChatMessage{{
			Role:    schemas.ChatMessageRoleUser,
...
func TestResponsesStructuredOutputConversion(t *testing.T) {
	tests := []struct {
		name     string
		input    *schemas.DeepIntShieldResponsesRequest
		validate func(t *testing.T, result *gemini.GeminiGenerationRequest)
	}{
		{
...
// TestDeepIntShieldResponsesToGeminiToolConversion tests the conversion of tools from DeepIntShield Responses API to Gemini format
func TestDeepIntShieldResponsesToGeminiToolConversion(t *testing.T) {
	tests := []struct {
		name     string
		input    *schemas.DeepIntShieldResponsesRequest
		validate func(t *testing.T, result *gemini.GeminiGenerationRequest)
...
func TestConvertGeminiUsageMetadataToChatUsage(t *testing.T) {
	tests := []struct {
		...
		expected *schemas.DeepIntShieldLLMUsage
	}{
		{
...
// TestConvertGeminiUsageMetadataToResponsesUsage tests the conversion of Gemini usage metadata to DeepIntShield responses usage
...
func TestGeminiToolInputKeyOrderPreservation(t *testing.T) {
	...
	result := gemini.ToGeminiChatCompletionRequest(deepintshieldReq)
	require.NotNil(t, result)
