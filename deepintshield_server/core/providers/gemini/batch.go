...
// ToDeepIntShieldBatchStatus converts Gemini batch job state to DeepIntShield status.
func ToDeepIntShieldBatchStatus(geminiState string) schemas.BatchStatus {
	switch geminiState {
...
// ToGeminiBatchStatus converts DeepIntShield batch status to Gemini batch job state.
...
// ToGeminiBatchJobResponse converts DeepIntShield batch create response to Gemini batch job response format.
func ToGeminiBatchJobResponse(resp *schemas.DeepIntShieldBatchCreateResponse) *GeminiBatchJobResponse {
	if resp == nil {
		return nil
	}
...
// ToGeminiBatchRetrieveResponse converts a DeepIntShield batch retrieve response to Gemini batch job response format.
func ToGeminiBatchRetrieveResponse(resp *schemas.DeepIntShieldBatchRetrieveResponse) *GeminiBatchJobResponse {
	if resp == nil {
		return nil
	}
...
// ToGeminiBatchListResponse converts a DeepIntShield batch list response to Gemini format.
func ToGeminiBatchListResponse(resp *schemas.DeepIntShieldBatchListResponse) *GeminiBatchListResponse {
	if resp == nil {
		return nil
	}
...
func (provider *GeminiProvider) downloadBatchResultsFile(ctx context.Context, key schemas.Key, fileName string) ([]schemas.BatchResultItem, []schemas.BatchError, *schemas.DeepIntShieldError) {
	...
	if deepintshieldErr != nil {
		return nil, nil, deepintshieldErr
	}
	...
	if err != nil {
		return nil, nil, providerUtils.NewDeepIntShieldOperationError(schemas.ErrProviderResponseDecode, err, providerName)
	}
