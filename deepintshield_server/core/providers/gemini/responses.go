...
func (request *GeminiGenerationRequest) ToDeepIntShieldResponsesRequest(ctx *schemas.DeepIntShieldContext) *schemas.DeepIntShieldResponsesRequest {
	if request == nil {
		return nil
	}
	...
	// Create the DeepIntShieldResponsesRequest
	...
	if len(inputMessages) > 0 {
		deepintshieldReq.Input = inputMessages
	}
...
func ToGeminiResponsesRequest(deepintshieldReq *schemas.DeepIntShieldResponsesRequest) *GeminiGenerationRequest {
	...
	if deepintshieldReq.Params != nil {
		geminiReq.GenerationConfig = geminiReq.convertParamsToGenerationConfigResponses(deepintshieldReq.Params)
		geminiReq.ExtraParams = deepintshieldReq.Params.ExtraParams
		// Handle tool-related parameters
		if len(deepintshieldReq.Params.Tools) > 0 {
			geminiReq.Tools = convertResponsesToolsToGemini(deepintshieldReq.Params.Tools)

			// Convert tool choice if present
			if deepintshieldReq.Params.ToolChoice != nil {
				geminiReq.ToolConfig = convertResponsesToolChoiceToGemini(deepintshieldReq.Params.ToolChoice)
			}
		}
	}
	...
	if deepintshieldReq.Input != nil {
		contents, systemInstruction, err := convertResponsesMessagesToGeminiContents(deepintshieldReq.Input)
		if err != nil {
			return nil
		}
...
// ToResponsesDeepIntShieldResponsesResponse converts a Gemini GenerateContentResponse to a DeepIntShieldResponsesResponse
func (response *GenerateContentResponse) ToResponsesDeepIntShieldResponsesResponse() *schemas.DeepIntShieldResponsesResponse {
	if response == nil {
		return nil
	}
	...
	deepintshieldResp.Usage = ConvertGeminiUsageMetadataToResponsesUsage(response.UsageMetadata)
...
func ToGeminiResponsesResponse(deepintshieldResp *schemas.DeepIntShieldResponsesResponse) *GenerateContentResponse {
	...
	if deepintshieldResp.ID != nil {
		geminiResp.ResponseID = *deepintshieldResp.ID
	}

	// Set creation time
	if deepintshieldResp.CreatedAt > 0 {
		geminiResp.CreateTime = time.Unix(int64(deepintshieldResp.CreatedAt), 0)
	}
	...
	if len(deepintshieldResp.Output) > 0 {
		...
		for i := range deepintshieldResp.Output {
			msg := &deepintshieldResp.Output[i]
		...
		for i, msg := range deepintshieldResp.Output {
			...
			if msg.Type != nil && *msg.Type == schemas.ResponsesMessageTypeFunctionCall && msg.ResponsesToolMessage != nil {
				...
				if len(thoughtSignature) > 0 {
					...
					if i+1 < len(deepintshieldResp.Output) {
						nextMsg := deepintshieldResp.Output[i+1]
		...
		if len(currentParts) > 0 {
			...
			if deepintshieldResp.IncompleteDetails != nil {
				switch deepintshieldResp.IncompleteDetails.Reason {
				case "max_tokens":
	...
	if deepintshieldResp.Usage != nil {
		geminiResp.UsageMetadata = ConvertDeepIntShieldResponsesUsageToGeminiUsageMetadata(deepintshieldResp.Usage)
	}

	return geminiResp
}

// DeepIntShieldToGeminiStreamState tracks state when converting DeepIntShield streams to Gemini format
type DeepIntShieldToGeminiStreamState struct {
...
// NewDeepIntShieldToGeminiStreamState creates a new state for DeepIntShield→Gemini streaming
func NewDeepIntShieldToGeminiStreamState() *DeepIntShieldToGeminiStreamState {
	return &DeepIntShieldToGeminiStreamState{
...
func ToGeminiResponsesStreamResponse(deepintshieldResp *schemas.DeepIntShieldResponsesStreamResponse, state *DeepIntShieldToGeminiStreamState) *GenerateContentResponse {
	if deepintshieldResp == nil {
		return nil
	}
	...
	if state == nil {
		state = NewDeepIntShieldToGeminiStreamState()
	}

	// Buffer web search call
	if deepintshieldResp.Type == schemas.ResponsesStreamResponseTypeOutputItemDone &&
		deepintshieldResp.Item != nil &&
		deepintshieldResp.Item.Type != nil &&
		*deepintshieldResp.Item.Type == schemas.ResponsesMessageTypeWebSearchCall {
		state.WebSearchCall = deepintshieldResp.Item
		state.HasWebSearch = true
		return nil // Don't emit yet, wait for completion
	}

	// Buffer annotations
	if deepintshieldResp.Type == schemas.ResponsesStreamResponseTypeOutputTextAnnotationAdded &&
		deepintshieldResp.Annotation != nil {
		state.Annotations = append(state.Annotations, *deepintshieldResp.Annotation)
		return nil // Don't emit yet, wait for completion
	}
	...
	if deepintshieldResp.Type == schemas.ResponsesStreamResponseTypeOutputItemDone &&
		deepintshieldResp.Item != nil &&
		deepintshieldResp.Item.Content != nil &&
		deepintshieldResp.Item.Content.ContentBlocks != nil {
		for _, block := range deepintshieldResp.Item.Content.ContentBlocks {
			if block.Type == schemas.ResponsesOutputMessageContentTypeRenderedContent &&
				block.ResponsesOutputMessageContentRenderedContent != nil &&
				block.ResponsesOutputMessageContentRenderedContent.RenderedContent != "" {
	...
	case schemas.ResponsesStreamResponseTypeFunctionCallArgumentsDone:
		...
		if deepintshieldResp.Item != nil && deepintshieldResp.Item.ResponsesToolMessage != nil {
			argsStr = deepintshieldResp.Item.ResponsesToolMessage.Arguments
			name = deepintshieldResp.Item.ResponsesToolMessage.Name
			callID = deepintshieldResp.Item.ResponsesToolMessage.CallID
		}
		if argsStr == nil && deepintshieldResp.Arguments != nil {
			// Some providers (e.g., Anthropic) send Arguments directly on the response
			argsStr = deepintshieldResp.Arguments
			// Try to get name and callID from state if available
			if state != nil {
				outputIndex := 0
				if deepintshieldResp.OutputIndex != nil {
					outputIndex = *deepintshieldResp.OutputIndex
				}
				if name == nil {
	...
	case schemas.ResponsesStreamResponseTypeOutputTextDone:
		if deepintshieldResp.Text != nil && *deepintshieldResp.Text != "" {
			candidate.Content.Parts = append(candidate.Content.Parts, &Part{
	...
	case schemas.ResponsesStreamResponseTypeOutputItemAdded:
		if deepintshieldResp.Item != nil && deepintshieldResp.Item.ResponsesReasoning != nil && deepintshieldResp.Item.EncryptedContent != nil {
			candidate.Content.Parts = append(candidate.Content.Parts, &Part{
				ThoughtSignature: []byte(*deepintshieldResp.Item.ResponsesReasoning.EncryptedContent),
			})
		}
		// Track function call metadata for later use in FunctionCallArgumentsDone
		if deepintshieldResp.Item != nil && deepintshieldResp.Item.Type != nil &&
			*deepintshieldResp.Item.Type == schemas.ResponsesMessageTypeFunctionCall &&
			...
			if deepintshieldResp.Item.ResponsesToolMessage.Name != nil {
				state.ToolCallNames[outputIndex] = *deepintshieldResp.Item.ResponsesToolMessage.Name
			}
			if deepintshieldResp.Item.ResponsesToolMessage.CallID != nil {
				state.ToolCallIDs[outputIndex] = *deepintshieldResp.Item.ResponsesToolMessage.CallID
			}
		}
		return nil
	...
	case schemas.ResponsesStreamResponseTypeContentPartAdded:
		...
		if deepintshieldResp.Part != nil {
			part, err := convertContentBlockToGeminiPart(*deepintshieldResp.Part)
			if err == nil && part != nil {
				candidate.Content.Parts = append(candidate.Content.Parts, part)
			}
		}
	...
	case schemas.ResponsesStreamResponseTypeCompleted:
		if deepintshieldResp.Response != nil {
			...
			if deepintshieldResp.Response.Model != "" {
				streamResp.ModelVersion = deepintshieldResp.Response.Model
			}

			// Convert usage metadata if available
			if deepintshieldResp.Response.Usage != nil {
				streamResp.UsageMetadata = ConvertDeepIntShieldResponsesUsageToGeminiUsageMetadata(deepintshieldResp.Response.Usage)
			}
	...
	case schemas.ResponsesStreamResponseTypeFailed:
		candidate.FinishReason = FinishReasonOther
		if deepintshieldResp.Response != nil && deepintshieldResp.Response.Error != nil {
			streamResp.PromptFeedback = &GenerateContentResponsePromptFeedback{
				BlockReason:        "ERROR",
				BlockReasonMessage: deepintshieldResp.Response.Error.Message,
			}
		}
	...
	case schemas.ResponsesStreamResponseTypeRefusalDone:
		if deepintshieldResp.Refusal != nil && *deepintshieldResp.Refusal != "" {
			candidate.FinishReason = FinishReasonSafety
		}
...
func (state *GeminiResponsesStreamState) closeTextItemIfOpen(sequenceNumber int) []*schemas.DeepIntShieldResponsesStreamResponse {
...
// ToDeepIntShieldResponsesStream converts a Gemini stream event to DeepIntShield Responses Stream responses
func (response *GenerateContentResponse) ToDeepIntShieldResponsesStream(sequenceNumber int, state *GeminiResponsesStreamState) ([]*schemas.DeepIntShieldResponsesStreamResponse, *schemas.DeepIntShieldError) {
	...
	if !state.HasEmittedCreated {
		...
		createdResp := &schemas.DeepIntShieldResponsesResponse{
			ID:        state.MessageID,
		...
		inProgressResp := &schemas.DeepIntShieldResponsesResponse{
			ID:        state.MessageID,
			CreatedAt: state.CreatedAt,
		}
		if state.Model != nil {
...
func processGeminiPart(part *Part, state *GeminiResponsesStreamState, sequenceNumber int) []*schemas.DeepIntShieldResponsesStreamResponse {
	var responses []*schemas.DeepIntShieldResponsesStreamResponse
...
func processGeminiTextPart(part *Part, state *GeminiResponsesStreamState, sequenceNumber int) []*schemas.DeepIntShieldResponsesStreamResponse {
	var responses []*schemas.DeepIntShieldResponsesStreamResponse
	...
	if part.Text != "" {
		...
		streamResponse := &schemas.DeepIntShieldResponsesStreamResponse{
...
func processGeminiThoughtPart(part *Part, state *GeminiResponsesStreamState, sequenceNumber int) []*schemas.DeepIntShieldResponsesStreamResponse {
	var responses []*schemas.DeepIntShieldResponsesStreamResponse
...
func processGeminiThoughtSignaturePart(part *Part, state *GeminiResponsesStreamState, sequenceNumber int) []*schemas.DeepIntShieldResponsesStreamResponse {
	var responses []*schemas.DeepIntShieldResponsesStreamResponse
...
func processGeminiFunctionCallPart(part *Part, state *GeminiResponsesStreamState, sequenceNumber int) []*schemas.DeepIntShieldResponsesStreamResponse {
	var responses []*schemas.DeepIntShieldResponsesStreamResponse
	...
	status := "in_progress"
	addedEvent := &schemas.DeepIntShieldResponsesStreamResponse{
		Type:           schemas.ResponsesStreamResponseTypeOutputItemAdded,
		SequenceNumber: sequenceNumber + len(responses),
		OutputIndex:    &outputIndex,
		ItemID:         &toolUseID,
	...
	doneEvent := &schemas.DeepIntShieldResponsesStreamResponse{
	...
	outputItemDone := &schemas.DeepIntShieldResponsesStreamResponse{
		Type:           schemas.ResponsesStreamResponseTypeOutputItemDone,
		SequenceNumber: sequenceNumber + len(responses),
		OutputIndex:    &outputIndex,
		ItemID:         &toolUseID,
...
func processGeminiFunctionResponsePart(part *Part, state *GeminiResponsesStreamState, sequenceNumber int) []*schemas.DeepIntShieldResponsesStreamResponse {
	var responses []*schemas.DeepIntShieldResponsesStreamResponse
...
func processGeminiInlineDataPart(part *Part, state *GeminiResponsesStreamState, sequenceNumber int) []*schemas.DeepIntShieldResponsesStreamResponse {
	var responses []*schemas.DeepIntShieldResponsesStreamResponse
...
func processGeminiFileDataPart(part *Part, state *GeminiResponsesStreamState, sequenceNumber int) []*schemas.DeepIntShieldResponsesStreamResponse {
	var responses []*schemas.DeepIntShieldResponsesStreamResponse
...
func closeGeminiTextItem(state *GeminiResponsesStreamState, sequenceNumber int) []*schemas.DeepIntShieldResponsesStreamResponse {
	var responses []*schemas.DeepIntShieldResponsesStreamResponse
...
func closeGeminiOpenItems(state *GeminiResponsesStreamState, groundingMetadata *GroundingMetadata, usage *GenerateContentResponseUsageMetadata, sequenceNumber int, finishReason FinishReason, finishMessage string) []*schemas.DeepIntShieldResponsesStreamResponse {
	...
	deepintshieldUsage := ConvertGeminiUsageMetadataToResponsesUsage(usage)

	completedResp := &schemas.DeepIntShieldResponsesResponse{
		ID:        state.MessageID,
		CreatedAt: state.CreatedAt,
		Usage:     deepintshieldUsage,
	}
	if state.Model != nil {
	...
	if finishReason != "" {
		stopReason := ConvertGeminiFinishReasonToDeepIntShield(finishReason)
...
func FinalizeGeminiResponsesStream(state *GeminiResponsesStreamState, usage *GenerateContentResponseUsageMetadata, sequenceNumber int) []*schemas.DeepIntShieldResponsesStreamResponse {
...
// buildGroundingMetadataFromWebSearch converts a DeepIntShield web_search_call message to Gemini GroundingMetadata
...
func emitWebSearchFromGroundingMetadata(
	...
	sequenceNumber int,
) []*schemas.DeepIntShieldResponsesStreamResponse {
	var responses []*schemas.DeepIntShieldResponsesStreamResponse
...
func generateSyntheticFunctionCallArgumentDeltas(argumentsJSON string, outputIndex *int, itemID *string, baseSequenceNumber int) []*schemas.DeepIntShieldResponsesStreamResponse {
	var events []*schemas.DeepIntShieldResponsesStreamResponse
	...
	for i := 0; i < len(runes); i += chunkSize {
		...
		deltaEvent := &schemas.DeepIntShieldResponsesStreamResponse{
