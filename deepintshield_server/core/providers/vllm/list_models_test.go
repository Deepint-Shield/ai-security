...
func TestListModels_QueriesAllBackends(t *testing.T) {
	...
	resp, deepintshieldErr := provider.ListModels(ctx, keys, request)
	if deepintshieldErr != nil {
		t.Fatalf("ListModels returned error: %v", deepintshieldErr.Error)
	}
	...
	// Model IDs are prefixed with "vllm/" by ToDeepIntShieldListModelsResponse.
...
func TestListModels_SingleBackendFailure(t *testing.T) {
	...
	resp, deepintshieldErr := provider.ListModels(ctx, keys, request)
	if deepintshieldErr != nil {
		t.Fatalf("ListModels should not return a top-level error when one backend succeeds, got: %v", deepintshieldErr.Error)
	}
...
func TestListModels_ErrorsWithoutPerKeyURL(t *testing.T) {
	...
	_, deepintshieldErr := provider.ListModels(ctx, keys, request)
	if deepintshieldErr == nil {
