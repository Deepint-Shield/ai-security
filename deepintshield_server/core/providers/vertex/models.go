...
// ToDeepIntShieldListModelsResponse converts a Vertex AI list models response to DeepIntShield's format.
...
func (response *VertexListModelsResponse) ToDeepIntShieldListModelsResponse(allowedModels []string, deployments map[string]string, unfiltered bool) *schemas.DeepIntShieldListModelsResponse {
	if response == nil {
		return nil
	}
	...
	for _, model := range response.Models {
		...
		for _, deployedModel := range model.DeployedModels {
			...
			deepintshieldResponse.Data = append(deepintshieldResponse.Data, modelEntry)
	...
	deepintshieldResponse.NextPageToken = response.NextPageToken

	return deepintshieldResponse
}

// ToDeepIntShieldListModelsResponse converts a Vertex AI publisher models response to DeepIntShield's format.
// This is for foundation models from the Model Garden (publishers.models.list endpoint).
func (response *VertexListPublisherModelsResponse) ToDeepIntShieldListModelsResponse(allowedModels []string, unfiltered bool) *schemas.DeepIntShieldListModelsResponse {
	if response == nil {
		return nil
	}
