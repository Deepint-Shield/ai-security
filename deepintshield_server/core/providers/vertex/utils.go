...
func getRequestBodyForAnthropicResponses(ctx *schemas.DeepIntShieldContext, request *schemas.DeepIntShieldResponsesRequest, deployment string, providerName schemas.ModelProvider, isStreaming bool, isCountTokens bool) ([]byte, *schemas.DeepIntShieldError) {
	...
	if useRawBody, ok := ctx.Value(schemas.DeepIntShieldContextKeyUseRawRequestBody).(bool); ok && useRawBody {
		...
		if request.Params != nil && request.Params.Tools != nil {
			if toolErr := anthropic.ValidateToolsForProvider(request.Params.Tools, schemas.Vertex); toolErr != nil {
				return nil, providerUtils.NewDeepIntShieldOperationError(toolErr.Error(), nil, providerName)
			}
		}
		...
		if extraHeaders, ok := ctx.Value(schemas.DeepIntShieldContextKeyExtraHeaders).(map[string][]string); ok {
			...
			if betaErr != nil {
				return nil, providerUtils.NewDeepIntShieldOperationError(schemas.ErrProviderRequestMarshal, betaErr, providerName)
			}
...
func buildResponseFromConfig(deployments map[string]string, allowedModels []string) *schemas.DeepIntShieldListModelsResponse {
