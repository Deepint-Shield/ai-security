...
// so we can reuse ToDeepIntShieldResponsesRequest() for messages-based requests.
...
// ToDeepIntShieldTextCompletionRequest handles ALL prompt-based families
// (Anthropic legacy, Mistral, Llama, Cohere Command, Cohere Command R).
func (r *BedrockInvokeRequest) ToDeepIntShieldTextCompletionRequest(ctx *schemas.DeepIntShieldContext) *schemas.DeepIntShieldTextCompletionRequest {
	...
	// Build a BedrockTextCompletionRequest and delegate to its ToDeepIntShieldTextCompletionRequest
	...
	return textReq.ToDeepIntShieldTextCompletionRequest(ctx)
}
...
// ToBedrockInvokeMessagesResponse converts a DeepIntShieldResponsesResponse to the model-family-specific
// InvokeModel response format. Switches on model family to produce the correct JSON structure.
func ToBedrockInvokeMessagesResponse(ctx *schemas.DeepIntShieldContext, resp *schemas.DeepIntShieldResponsesResponse) (interface{}, error) {
	if resp == nil {
...
// toBedrockInvokeAnthropicResponse converts DeepIntShieldResponsesResponse to Anthropic Messages API format.
func toBedrockInvokeAnthropicResponse(resp *schemas.DeepIntShieldResponsesResponse, model string) *BedrockInvokeMessagesResponse {
...
// toBedrockInvokeAI21Response converts DeepIntShieldResponsesResponse to AI21 Jamba format.
func toBedrockInvokeAI21Response(resp *schemas.DeepIntShieldResponsesResponse) *BedrockInvokeAI21Response {
...
// ToBedrockInvokeMessagesStreamResponse converts a DeepIntShield Responses stream event to
// a BedrockStreamEvent with InvokeModelRawChunk for the invoke-with-response-stream endpoint.
func ToBedrockInvokeMessagesStreamResponse(ctx *schemas.DeepIntShieldContext, resp *schemas.DeepIntShieldResponsesStreamResponse) (string, interface{}, error) {
	if resp == nil {
...
// toAnthropicInvokeStreamBytes converts a DeepIntShield stream event into raw bytes representing
// the Anthropic Messages API streaming event JSON, suitable for wrapping in InvokeModelRawChunk.
func toAnthropicInvokeStreamBytes(resp *schemas.DeepIntShieldResponsesStreamResponse) ([]byte, error) {
	var event interface{}
