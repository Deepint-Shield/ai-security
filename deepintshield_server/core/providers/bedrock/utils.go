...
var (
	...
	// bedrockFinishReasonToDeepIntShield maps Bedrock Converse API stop reasons to DeepIntShield format.
	// Bedrock has additional stop reasons beyond Anthropic (guardrail_intervened, content_filtered).
	bedrockFinishReasonToDeepIntShield = map[string]string{
		"end_turn":             "stop",
		"max_tokens":           "length",
		"stop_sequence":        "stop",
		"tool_use":             "tool_calls",
...
// convertBedrockStopReason converts a Bedrock stop reason to DeepIntShield format.
func convertBedrockStopReason(stopReason string) string {
	if reason, ok := bedrockFinishReasonToDeepIntShield[stopReason]; ok {
		return reason
	}
	return "stop"
}
...
func convertChatParameters(ctx *schemas.DeepIntShieldContext, deepintshieldReq *schemas.DeepIntShieldChatRequest, bedrockReq *BedrockConverseRequest) error {
	// Parameters are optional - if not provided, just skip conversion
	if deepintshieldReq.Params == nil {
		return nil
	}
	// Convert inference config
	if inferenceConfig := convertInferenceConfig(deepintshieldReq.Params); inferenceConfig != nil {
		bedrockReq.InferenceConfig = inferenceConfig
	}

	// Check for response_format and convert to tool
	responseFormatTool := convertResponseFormatToTool(ctx, deepintshieldReq.Params)

	// Convert tool config
	if toolConfig := convertToolConfig(deepintshieldReq.Model, deepintshieldReq.Params); toolConfig != nil {
		...
		if deepintshieldReq.Params.Reasoning.MaxTokens != nil {
			...
			if schemas.IsNovaModel(deepintshieldReq.Model) {
				...
				if anthropic.SupportsAdaptiveThinking(deepintshieldReq.Model) {
					...
					budgetTokens, err := providerUtils.GetBudgetTokensFromReasoningEffort(*deepintshieldReq.Params.Reasoning.Effort, anthropic.MinimumReasoningMaxTokens, maxTokens)
					if err != nil {
						return err
					}
					bedrockReq.AdditionalModelRequestFields.Set("thinking", map[string]any{
	...
	if len(deepintshieldReq.Params.ExtraParams) > 0 {
		bedrockReq.ExtraParams = deepintshieldReq.Params.ExtraParams
		// Handle guardrail configuration
		if guardrailConfig, exists := deepintshieldReq.Params.ExtraParams["guardrailConfig"]; exists {
		...
		if deepintshieldReq.Params != nil && deepintshieldReq.Params.ExtraParams != nil {
			...
			if perfConfig, exists := deepintshieldReq.Params.ExtraParams["performanceConfig"]; exists {
			...
			if promptVars, exists := deepintshieldReq.Params.ExtraParams["promptVariables"]; exists {
			...
			if reqMetadata, exists := deepintshieldReq.Params.ExtraParams["requestMetadata"]; exists {
...
func ensureChatToolConfigForConversation(deepintshieldReq *schemas.DeepIntShieldChatRequest, bedrockReq *BedrockConverseRequest) {
	if bedrockReq.ToolConfig != nil {
		return // Already has tool config
	}

	hasToolContent, tools := extractToolsFromConversationHistory(deepintshieldReq.Input)
	if hasToolContent && len(tools) > 0 {
		bedrockReq.ToolConfig = &BedrockToolConfig{Tools: tools}
	}
}

// convertMessages converts DeepIntShield messages to Bedrock format
// Returns regular messages and system messages separately
func convertMessages(deepintshieldMessages []schemas.ChatMessage) ([]BedrockMessage, []BedrockSystemMessage, error) {
	var messages []BedrockMessage
	var systemMessages []BedrockSystemMessage

	for i := 0; i < len(deepintshieldMessages); i++ {
		...
		case schemas.ChatMessageRoleTool:
			...
			for j := i + 1; j < len(deepintshieldMessages) && deepintshieldMessages[j].Role == schemas.ChatMessageRoleTool; j++ {
				toolMessages = append(toolMessages, deepintshieldMessages[j])
				i = j
			}
...
// convertSystemMessages converts a DeepIntShield system message to Bedrock format
...
// convertMessage converts a DeepIntShield message to Bedrock format
func convertMessage(msg schemas.ChatMessage) (BedrockMessage, error) {
	...
	if msg.ChatAssistantMessage != nil && len(msg.ChatAssistantMessage.ReasoningDetails) > 0 {
		for _, detail := range msg.ChatAssistantMessage.ReasoningDetails {
			if detail.Type == schemas.DeepIntShieldReasoningDetailsTypeText {
...
// convertToolMessages converts multiple consecutive DeepIntShield tool messages to a single Bedrock message
...
// convertContent converts DeepIntShield message content to Bedrock content blocks
...
// convertContentBlock converts a DeepIntShield content block to Bedrock format
func convertContentBlock(block schemas.ChatContentBlock) ([]BedrockContentBlock, error) {
	...
	case schemas.ChatContentBlockTypeText:
		...
		// Once the world onboards on DeepIntShield - we should remove these shitty patterns.
...
// convertImageToBedrockSource converts a DeepIntShield image URL to Bedrock image source
...
func convertResponseFormatToTool(ctx *schemas.DeepIntShieldContext, params *schemas.ChatParameters) *BedrockTool {
...
func convertTextFormatToTool(ctx *schemas.DeepIntShieldContext, textConfig *schemas.ResponsesTextConfig) *BedrockTool {
...
// convertInferenceConfig converts DeepIntShield parameters to Bedrock inference config
...
// convertToolConfig converts DeepIntShield tools to Bedrock tool config
...
// convertToolChoice converts DeepIntShield tool choice to Bedrock format
...
// convertToolCallToContentBlock converts a DeepIntShield tool call to a Bedrock content block
...
// ToBedrockError converts a DeepIntShieldError to BedrockError
// This is a standalone function similar to ToAnthropicChatCompletionError
func ToBedrockError(deepintshieldErr *schemas.DeepIntShieldError) *BedrockError {
	...
	// Map error type/code
	if deepintshieldErr.Error != nil && deepintshieldErr.Error.Code != nil {
		bedrockErr.Type = *deepintshieldErr.Error.Code
		bedrockErr.Code = deepintshieldErr.Error.Code
	} else if deepintshieldErr.Type != nil {
		bedrockErr.Type = *deepintshieldErr.Type
	} else {
...
// This handles the conversion from flexible parameter formats to DeepIntShield's structured format
