...
// ToBedrockChatCompletionRequest converts a DeepIntShield request to Bedrock Converse API format
func ToBedrockChatCompletionRequest(ctx *schemas.DeepIntShieldContext, deepintshieldReq *schemas.DeepIntShieldChatRequest) (*BedrockConverseRequest, error) {
	...
	messages, systemMessages, err := convertMessages(deepintshieldReq.Input)
	if err != nil {
	...
	if err := convertChatParameters(ctx, deepintshieldReq, bedrockReq); err != nil {
		return nil, fmt.Errorf("failed to convert chat parameters: %w", err)
	}

	// Ensure tool config is present when needed
	ensureChatToolConfigForConversation(deepintshieldReq, bedrockReq)

	return bedrockReq, nil
}

// ToDeepIntShieldChatResponse converts a Bedrock Converse API response to DeepIntShield format
func (response *BedrockConverseResponse) ToDeepIntShieldChatResponse(ctx context.Context, model string) (*schemas.DeepIntShieldChatResponse, error) {
	if response == nil {
	...
	if response.Output.Message != nil {
		for _, contentBlock := range response.Output.Message.Content {
			...
			if contentBlock.ToolUse != nil {
				...
				if structuredOutputToolName, ok := ctx.Value(schemas.DeepIntShieldContextKeyStructuredOutputToolName).(string); ok && contentBlock.ToolUse.Name == structuredOutputToolName {
	...
	// Create the final DeepIntShield response
	...
	if response.ServiceTier != nil && response.ServiceTier.Type != "" {
		deepintshieldResponse.ServiceTier = &response.ServiceTier.Type
	}
...
func (chunk *BedrockStreamEvent) ToDeepIntShieldChatCompletionStream(state *BedrockStreamState) (*schemas.DeepIntShieldChatResponse, *schemas.DeepIntShieldError, bool) {
	if state == nil {
	...
	case chunk.Delta != nil:
		...
		case chunk.Delta.ReasoningContent != nil:
			...
			var streamResponse *schemas.DeepIntShieldChatResponse
