...
// convertCohereContentBlockToDeepIntShield converts CohereContentBlock to schemas.ContentBlock for Responses
func convertCohereContentBlockToDeepIntShield(cohereBlock CohereContentBlock) schemas.ResponsesMessageContentBlock {
...
func (chunk *CohereStreamEvent) ToDeepIntShieldResponsesStream(sequenceNumber int, state *CohereResponsesStreamState) ([]*schemas.DeepIntShieldResponsesStreamResponse, *schemas.DeepIntShieldError, bool) {
	switch chunk.Type {
...
// ConvertResponsesTextFormatToCohere converts DeepIntShield Responses Text.Format to Cohere's typed format
...
// ToCohereResponsesRequest converts a DeepIntShieldRequest (Responses structure) to CohereChatRequest
func ToCohereResponsesRequest(deepintshieldReq *schemas.DeepIntShieldResponsesRequest) (*CohereChatRequest, error) {
	...
	if deepintshieldReq.Params != nil {
		if deepintshieldReq.Params.MaxOutputTokens != nil {
			cohereReq.MaxTokens = deepintshieldReq.Params.MaxOutputTokens
		}
		...
		if deepintshieldReq.Params.Reasoning != nil {
			if deepintshieldReq.Params.Reasoning.MaxTokens != nil {
				...
				if deepintshieldReq.Params.Reasoning.Effort != nil && *deepintshieldReq.Params.Reasoning.Effort != "none" {
					...
					if deepintshieldReq.Params.MaxOutputTokens != nil {
						maxOutputTokens = *deepintshieldReq.Params.MaxOutputTokens
					}
					budgetTokens, err := providerUtils.GetBudgetTokensFromReasoningEffort(*deepintshieldReq.Params.Reasoning.Effort, MinimumReasoningMaxTokens, maxOutputTokens)
					if err != nil {
						return nil, err
					}
		...
		if deepintshieldReq.Params.Text != nil && deepintshieldReq.Params.Text.Format != nil {
			cohereReq.ResponseFormat = convertResponsesTextFormatToCohere(deepintshieldReq.Params.Text.Format)
		}
		if deepintshieldReq.Params.ExtraParams != nil {
			...
			if thinkingParam, ok := schemas.SafeExtractFromMap(deepintshieldReq.Params.ExtraParams, "thinking"); ok {
				if thinkingMap, ok := thinkingParam.(map[string]interface{}); ok {
					...
					cohereReq.Thinking = thinking
					deepintshieldReq.Params.ExtraParams["thinking"] = thinkingMap
				}
			}
		}
	}
	...
	if deepintshieldReq.Params != nil && deepintshieldReq.Params.ToolChoice != nil {
		cohereReq.ToolChoice = convertDeepIntShieldToolChoiceToCohereToolChoice(*deepintshieldReq.Params.ToolChoice)
	}

	// Process ResponsesInput (which contains the Responses items)
	if deepintshieldReq.Input != nil {
		cohereReq.Messages = ConvertDeepIntShieldMessagesToCohereMessages(deepintshieldReq.Input, deepintshieldReq.Params)
	}

	return cohereReq, nil
}

// ToDeepIntShieldResponsesResponse converts CohereChatResponse to DeepIntShieldResponse (Responses structure)
func (response *CohereChatResponse) ToDeepIntShieldResponsesResponse() *schemas.DeepIntShieldResponsesResponse {
	if response == nil {
		return nil
	}
	...
	if response.Usage != nil {
		...
		deepintshieldResp.Usage = usage
	}

	// Convert output message to Responses format
	if response.Message != nil {
		outputMessages := ConvertCohereMessagesToDeepIntShieldMessages([]CohereMessage{*response.Message}, true)
...
// ConvertDeepIntShieldMessagesToCohereMessages converts an array of DeepIntShield ResponsesMessage to Cohere message format
// This is the main conversion method from DeepIntShield to Cohere - handles all message types and returns messages
func ConvertDeepIntShieldMessagesToCohereMessages(deepintshieldMessages []schemas.ResponsesMessage, params *schemas.ResponsesParameters) []CohereMessage {
	...
	for _, msg := range deepintshieldMessages {
		...
		case schemas.ResponsesMessageTypeMessage:
			...
			if role == "system" {
				...
				systemMsgs := convertDeepIntShieldMessageToCohereSystemContent(&msg)
				systemContent = append(systemContent, systemMsgs...)
			} else {
				// Convert regular message
				cohereMsg := convertDeepIntShieldMessageToCohereMessage(&msg)
				if cohereMsg != nil {
					if role == "assistant" {
		...
		case schemas.ResponsesMessageTypeReasoning:
			...
			reasoningBlocks := convertDeepIntShieldReasoningToCohereThinking(&msg)
		...
		case schemas.ResponsesMessageTypeFunctionCall:
			...
			assistantMsg := convertDeepIntShieldFunctionCallToCohereMessage(&msg)
			if assistantMsg != nil {
		...
		case schemas.ResponsesMessageTypeFunctionCallOutput:
			...
			toolMsg := convertDeepIntShieldFunctionCallOutputToCohereMessage(&msg)
			if toolMsg != nil {
...
// ConvertCohereMessagesToDeepIntShieldMessages converts an array of Cohere messages to DeepIntShield ResponsesMessage format
// This is the main conversion method from Cohere to DeepIntShield - handles all message types and content blocks
func ConvertCohereMessagesToDeepIntShieldMessages(cohereMessages []CohereMessage, isOutputMessage bool) []schemas.ResponsesMessage {
	var deepintshieldMessages []schemas.ResponsesMessage

	for _, msg := range cohereMessages {
		convertedMessages := convertSingleCohereMessageToDeepIntShieldMessages(&msg, isOutputMessage)
...
// convertDeepIntShieldToolChoiceToCohere converts schemas.ToolChoice to CohereToolChoice
func convertDeepIntShieldToolChoiceToCohereToolChoice(toolChoice schemas.ResponsesToolChoice) *CohereToolChoice {
...
// convertDeepIntShieldMessageToCohereSystemContent converts a DeepIntShield system message to Cohere system content
func convertDeepIntShieldMessageToCohereSystemContent(msg *schemas.ResponsesMessage) []string {
	var systemContent []string
...
// convertDeepIntShieldMessageToCohereMessage converts a regular DeepIntShield message to Cohere message
func convertDeepIntShieldMessageToCohereMessage(msg *schemas.ResponsesMessage) *CohereMessage {
	role := "user"
	if msg.Role != nil {
		role = string(*msg.Role)
	}
...
// convertDeepIntShieldReasoningToCohereThinking converts a DeepIntShield reasoning message to Cohere thinking blocks
func convertDeepIntShieldReasoningToCohereThinking(msg *schemas.ResponsesMessage) []CohereContentBlock {
...
// convertDeepIntShieldFunctionCallToCohereMessage converts a DeepIntShield function call to Cohere message
func convertDeepIntShieldFunctionCallToCohereMessage(msg *schemas.ResponsesMessage) *CohereMessage {
...
// convertDeepIntShieldFunctionCallOutputToCohereMessage converts a DeepIntShield function call output to Cohere message
func convertDeepIntShieldFunctionCallOutputToCohereMessage(msg *schemas.ResponsesMessage) *CohereMessage {
	if msg.ResponsesToolMessage != nil && msg.ResponsesToolMessage.CallID != nil {
...
// convertSingleCohereMessageToDeepIntShieldMessages converts a single Cohere message to DeepIntShield messages
func convertSingleCohereMessageToDeepIntShieldMessages(cohereMsg *CohereMessage, isOutputMessage bool) []schemas.ResponsesMessage {
		...
		if cohereMsg.Content.StringContent != nil {
			...
			for _, block := range cohereMsg.Content.BlocksContent {
				if block.Type == CohereContentBlockTypeThinking {
					...
					converted := convertCohereContentBlockToDeepIntShield(block)
					if converted.Type != "" {
...
// convertDeepIntShieldContentBlocksToCohere converts DeepIntShield content blocks to Cohere format
