...
// ToDeepIntShieldRealtimeEvent converts an ElevenLabs Conversational AI event to the unified DeepIntShield format.
func (provider *ElevenlabsProvider) ToDeepIntShieldRealtimeEvent(providerEvent json.RawMessage) (*schemas.DeepIntShieldRealtimeEvent, error) {
...
// ToProviderRealtimeEvent converts a unified DeepIntShield Realtime event to ElevenLabs' native JSON.
func (provider *ElevenlabsProvider) ToProviderRealtimeEvent(deepintshieldEvent *schemas.DeepIntShieldRealtimeEvent) (json.RawMessage, error) {
	if deepintshieldEvent.RawData != nil {
		return deepintshieldEvent.RawData, nil
	}

	switch deepintshieldEvent.Type {
	case schemas.RTEventInputAudioAppend:
		...
		out := map[string]interface{}{
			...
			"user_audio_chunk": deepintshieldEvent.Delta.Audio,
		}
