...
func ToElevenlabsTranscriptionRequest(deepintshieldReq *schemas.DeepIntShieldTranscriptionRequest) *ElevenlabsTranscriptionRequest {
	...
	if deepintshieldReq.Input != nil && len(deepintshieldReq.Input.File) > 0 {
		req.File = deepintshieldReq.Input.File
		req.Filename = deepintshieldReq.Input.Filename
	}
...
func ToDeepIntShieldTranscriptionResponse(chunks []ElevenlabsSpeechToTextChunkResponse) *schemas.DeepIntShieldTranscriptionResponse {
	if len(chunks) == 0 {
		return nil
	}
