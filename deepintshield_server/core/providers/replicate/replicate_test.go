...
// TestDeepIntShieldToReplicateChatRequestConversion tests the conversion from DeepIntShield chat request to Replicate format
// with special handling based on model names
func TestDeepIntShieldToReplicateChatRequestConversion(t *testing.T) {
	maxTokens := 100
	temp := 0.7
	topP := 0.9
	seed := 42
...
// TestDeepIntShieldToReplicateImageGenerationConversion tests the conversion from DeepIntShield image generation request
// to Replicate format with special handling for different model families
func TestDeepIntShieldToReplicateImageGenerationConversion(t *testing.T) {
	...
	tests := []struct {
		name     string
		input    *schemas.DeepIntShieldImageGenerationRequest
		validate func(t *testing.T, result *replicate.ReplicatePredictionRequest)
		wantErr  bool
	}{
		{
...
// TestDeepIntShieldToReplicateResponsesRequestConversion tests the conversion from DeepIntShield responses request to Replicate format
func TestDeepIntShieldToReplicateResponsesRequestConversion(t *testing.T) {
	maxOutputTokens := 100
	temp := 0.7
	topP := 0.9
...
// TestReplicateToDeepIntShieldResponsesResponse tests the conversion from Replicate prediction response to DeepIntShield responses format
func TestReplicateToDeepIntShieldResponsesResponse(t *testing.T) {
	...
	tests := []struct {
		...
		validate func(t *testing.T, result *schemas.DeepIntShieldResponsesResponse)
	}{
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			actual := tt.input.ToDeepIntShieldResponsesResponse()
			if tt.validate != nil {
				tt.validate(t, actual)
			}
		})
