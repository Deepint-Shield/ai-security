...
func checkForErrorStatus(prediction *ReplicatePredictionResponse) *schemas.DeepIntShieldError {
...
func listenToReplicateStreamURL(
	...
	streamURL string,
	key schemas.Key,
) (io.Reader, *fasthttp.Response, *schemas.DeepIntShieldError) {
	// Create request
	...
	if err != nil {
		...
		if errors.Is(err, fasthttp.ErrTimeout) || errors.Is(err, context.DeadlineExceeded) {
			return nil, nil, providerUtils.NewDeepIntShieldOperationError(schemas.ErrProviderRequestTimedOut, err, schemas.Replicate)
		}
		return nil, nil, providerUtils.NewDeepIntShieldOperationError(schemas.ErrProviderDoRequest, err, schemas.Replicate)
	}
...
func buildPredictionURL(ctx *schemas.DeepIntShieldContext, baseURL, model string, customProviderConfig *schemas.CustomProviderConfig, requestType schemas.RequestType, isDeployment bool) string {
