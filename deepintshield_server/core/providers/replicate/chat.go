...
func ToReplicateChatRequest(deepintshieldReq *schemas.DeepIntShieldChatRequest) (*ReplicatePredictionRequest, error) {
	...
	isGPT5Structured := strings.HasPrefix(deepintshieldReq.Model, string(schemas.OpenAI)) && strings.Contains(deepintshieldReq.Model, "gpt-5-structured")

	// openai models support messages
	if len(deepintshieldReq.Input) > 0 && strings.HasPrefix(deepintshieldReq.Model, string(schemas.OpenAI)) {
		if isGPT5Structured {
			...
			input.Messages = deepintshieldReq.Input
		}
	} else {
	...
	if deepintshieldReq.Params != nil {
		...
		if params.MaxCompletionTokens != nil {
			...
			} else if strings.HasPrefix(deepintshieldReq.Model, string(schemas.OpenAI)) {
...
// ToDeepIntShieldChatResponse converts a Replicate prediction response to DeepIntShield format
func (response *ReplicatePredictionResponse) ToDeepIntShieldChatResponse() *schemas.DeepIntShieldChatResponse {
	if response == nil {
		return nil
	}
