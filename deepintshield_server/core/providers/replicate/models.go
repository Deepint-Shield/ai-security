...
// ToDeepIntShieldListModelsResponse converts Replicate models and deployments to a DeepIntShield list models response
func ToDeepIntShieldListModelsResponse(
	...
	if deploymentsResponse != nil {
		for _, deployment := range deploymentsResponse.Results {
			...
			deepintshieldModel := schemas.Model{
			...
			deepintshieldResponse.Data = append(deepintshieldResponse.Data, deepintshieldModel)
			includedModels[deploymentID] = true
		}

		if deploymentsResponse.Next != nil {
			deepintshieldResponse.NextPageToken = *deploymentsResponse.Next
		}
	}
...
// ToReplicateListModelsResponse converts a DeepIntShield list models response to a Replicate list models response
// This is mainly used for testing and compatibility
func ToReplicateListModelsResponse(response *schemas.DeepIntShieldListModelsResponse) *ReplicateModelListResponse {
	if response == nil {
		return nil
	}
