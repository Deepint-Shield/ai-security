...
func TestToDeepIntShieldChatResponse_Citations(t *testing.T) {
	t.Parallel()

	t.Run("citations are mapped to deepintshield response", func(t *testing.T) {
		...
		deepintshieldResp := response.ToDeepIntShieldChatResponse("sonar-pro")

		if deepintshieldResp == nil {
			t.Fatal("expected non-nil deepintshield response")
		}

		// Verify citations are preserved
		if len(deepintshieldResp.Citations) != 3 {
			t.Fatalf("expected 3 citations, got %d", len(deepintshieldResp.Citations))
		}
		if deepintshieldResp.Citations[0] != "https://example.com/article1" {
			t.Errorf("expected first citation to be 'https://example.com/article1', got '%s'", deepintshieldResp.Citations[0])
		}
		if deepintshieldResp.Citations[1] != "https://example.com/article2" {
			t.Errorf("expected second citation to be 'https://example.com/article2', got '%s'", deepintshieldResp.Citations[1])
		}
		if deepintshieldResp.Citations[2] != "https://example.com/article3" {
			t.Errorf("expected third citation to be 'https://example.com/article3', got '%s'", deepintshieldResp.Citations[2])
		}

		// Verify search results are preserved
		if len(deepintshieldResp.SearchResults) != 1 {
			t.Fatalf("expected 1 search result, got %d", len(deepintshieldResp.SearchResults))
		}

		// Verify videos are preserved
		if len(deepintshieldResp.Videos) != 1 {
			t.Fatalf("expected 1 video, got %d", len(deepintshieldResp.Videos))
		}

		// Verify usage citation tokens are mapped
		if deepintshieldResp.Usage == nil || deepintshieldResp.Usage.CompletionTokensDetails == nil {
			t.Fatal("expected usage with completion token details")
		}
		if deepintshieldResp.Usage.CompletionTokensDetails.CitationTokens == nil || *deepintshieldResp.Usage.CompletionTokensDetails.CitationTokens != 5 {
			t.Error("expected citation_tokens to be 5")
		}
		if deepintshieldResp.Usage.CompletionTokensDetails.NumSearchQueries == nil || *deepintshieldResp.Usage.CompletionTokensDetails.NumSearchQueries != 2 {
	...
	t.Run("nil citations remain nil", func(t *testing.T) {
		...
		if deepintshieldResp.Citations != nil {
			t.Errorf("expected nil citations, got %v", deepintshieldResp.Citations)
		}
		if deepintshieldResp.SearchResults != nil {
			t.Errorf("expected nil search results, got %v", deepintshieldResp.SearchResults)
		}
	})
	...
	t.Run("empty citations slice is preserved", func(t *testing.T) {
		...
		if len(deepintshieldResp.Citations) != 0 {
			t.Errorf("expected 0 citations, got %d", len(deepintshieldResp.Citations))
		}
	})
}
...
func TestWebSearchOption_JSONSerialization(t *testing.T) {
	...
	t.Run("search_type extraction from ExtraParams", func(t *testing.T) {
		...
		result := perplexity.ToPerplexityChatCompletionRequest(deepintshieldReq)
...
func TestDeepIntShieldCost_DeserializationWithPerplexityFields(t *testing.T) {
	t.Parallel()
	...
	t.Run("deserializes new cost breakdown fields", func(t *testing.T) {
		...
		var cost schemas.DeepIntShieldCost
		if err := cost.UnmarshalJSON([]byte(jsonStr)); err != nil {
			t.Fatalf("failed to unmarshal DeepIntShieldCost: %v", err)
		}
