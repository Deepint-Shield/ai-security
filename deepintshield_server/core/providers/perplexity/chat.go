...
// ToPerplexityChatCompletionRequest converts a DeepIntShield request to Perplexity chat completion request
func ToPerplexityChatCompletionRequest(deepintshieldReq *schemas.DeepIntShieldChatRequest) *PerplexityChatRequest {
	...
	if deepintshieldReq.Params != nil {
		// Core parameters
		perplexityReq.MaxTokens = deepintshieldReq.Params.MaxCompletionTokens
		perplexityReq.Temperature = deepintshieldReq.Params.Temperature
		perplexityReq.TopP = deepintshieldReq.Params.TopP
		perplexityReq.PresencePenalty = deepintshieldReq.Params.PresencePenalty
		perplexityReq.FrequencyPenalty = deepintshieldReq.Params.FrequencyPenalty
		perplexityReq.ResponseFormat = deepintshieldReq.Params.ResponseFormat

		// Tool calling parameters
		perplexityReq.Tools = deepintshieldReq.Params.Tools
		perplexityReq.ToolChoice = deepintshieldReq.Params.ToolChoice
		perplexityReq.ParallelToolCalls = deepintshieldReq.Params.ParallelToolCalls

		// Standard parameters
		perplexityReq.Stop = deepintshieldReq.Params.Stop
		perplexityReq.LogProbs = deepintshieldReq.Params.LogProbs
		perplexityReq.TopLogProbs = deepintshieldReq.Params.TopLogProbs
		...
		if deepintshieldReq.Params.Reasoning != nil && deepintshieldReq.Params.Reasoning.Effort != nil {
			if *deepintshieldReq.Params.Reasoning.Effort == "minimal" {
				...
				perplexityReq.ReasoningEffort = deepintshieldReq.Params.Reasoning.Effort
			}
		}

		// Handle extra parameters for Perplexity-specific fields
		if deepintshieldReq.Params.ExtraParams != nil {
			perplexityReq.ExtraParams = deepintshieldReq.Params.ExtraParams
...
// ToDeepIntShieldChatResponse converts a Perplexity chat completion response to DeepIntShield format
func (response *PerplexityChatResponse) ToDeepIntShieldChatResponse(model string) *schemas.DeepIntShieldChatResponse {
	if response == nil {
		return nil
	}
	...
	if len(response.Choices) > 0 {
		deepintshieldResponse.Choices = response.Choices
	}
