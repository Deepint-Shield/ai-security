...
func TestAgent_Timeout(t *testing.T) {
	...
	_, deepintshieldErr := manager.CheckAndExecuteAgentForChatRequest(
		ctx,
		originalReq,
		initialResponse,
		mockLLM.MakeChatRequest,
	...
	require.NotNil(t, deepintshieldErr, "Expected timeout error but got success - timeout is not being enforced!")
	t.Logf("✅ Timeout correctly enforced: %v", deepintshieldErr.Error)
}
...
func TestAgent_TimeoutDuringExecution(t *testing.T) {
	...
	_, deepintshieldErr := manager.CheckAndExecuteAgentForChatRequest(
		ctx,
		&schemas.DeepIntShieldChatRequest{
			Provider: schemas.OpenAI,
			Model:    "gpt-4o",
	...
	require.NotNil(t, deepintshieldErr, "Expected timeout during tool execution but got success - timeout not enforced mid-execution!")
	t.Logf("✅ Timeout during execution correctly enforced: %v", deepintshieldErr.Error)
}
...
func TestAgent_Timeout_ChatFormat(t *testing.T) {
	...
	_, deepintshieldErr := manager.CheckAndExecuteAgentForChatRequest(ctx,
		&schemas.DeepIntShieldChatRequest{Provider: schemas.OpenAI, Model: "gpt-4o",
			Input: []schemas.ChatMessage{{Role: schemas.ChatMessageRoleUser, Content: &schemas.ChatMessageContent{ContentStr: schemas.Ptr("Test")}}}},
	...
	require.NotNil(t, deepintshieldErr, "Chat format timeout not enforced!")
	t.Logf("✅ Chat format timeout enforced: %v", deepintshieldErr.Error)
}
...
func TestAgent_Timeout_ResponsesFormat(t *testing.T) {
	...
	mockLLM := &MockLLMCaller{
		responsesResponses: []*schemas.DeepIntShieldResponsesResponse{
			CreateResponsesResponseWithToolCalls([]schemas.ResponsesToolMessage{
				{CallID: schemas.Ptr("call-1"), Name: schemas.Ptr("deepintshieldInternal-slow_responses_tool"), Arguments: schemas.Ptr(`{}`)},
			}),
		},
	}

	_, deepintshieldErr := manager.CheckAndExecuteAgentForResponsesRequest(ctx,
		&schemas.DeepIntShieldResponsesRequest{Provider: schemas.OpenAI, Model: "gpt-4o",
	...
	require.NotNil(t, deepintshieldErr, "Responses format timeout not enforced!")
	t.Logf("✅ Responses format timeout enforced: %v", deepintshieldErr.Error)
}
			...
			CreateChatResponseWithToolCalls([]schemas.ChatAssistantMessageToolCall{
				{
					...
					Type: schemas.Ptr("function"),
					Function: schemas.ChatAssistantMessageToolCallFunction{
						Name: schemas.Ptr("deepintshieldInternal-nonexistent_tool"),
						Arguments: `{}`,
					},
				},
			}),
			...
			CreateChatResponseWithToolCalls([]schemas.ChatAssistantMessageToolCall{
				{
					...
					Type: schemas.Ptr("function"),
					Function: schemas.ChatAssistantMessageToolCallFunction{
						Name: schemas.Ptr("deepintshieldInternal-nonexistent"),
						Arguments: `{}`,
					},
				},
			}),
