...
func setupMCPManagerWithRequestIDFunc(t *testing.T, fetchNewRequestIDFunc func(ctx *schemas.DeepIntShieldContext) string, clientConfigs ...schemas.MCPClientConfig) *mcp.MCPManager {
	t.Helper()
...
//
// Key concepts:
// - DeepIntShieldContextKeyRequestID: Current request ID (updated each iteration)
// - DeepIntShieldMCPAgentOriginalRequestID: Original request ID (preserved)
...
func TestAgent_RequestID_Propagation(t *testing.T) {
	...
	// Assertions
	require.Nil(t, deepintshieldErr, "Should complete without error")
	...
	storedOriginalID, ok := ctx.Value(schemas.DeepIntShieldMCPAgentOriginalRequestID).(string)
	...
	currentRequestID, ok := ctx.Value(schemas.DeepIntShieldContextKeyRequestID).(string)
...
func TestAgent_RequestID_MixedAutoAndNonAuto(t *testing.T) {
	...
	// Setup context and mocker
	ctx := createTestContext()
	ctx.SetValue(schemas.DeepIntShieldContextKeyRequestID, "req-mixed-001")
