...
import (
	...
	core "github.com/deepint-shield/ai-security/core"
...
func TestPlugin_PreMCPHook(t *testing.T) {
	...
	deepintshield, err := core.Init(context.Background(), schemas.DeepIntShieldConfig{
		Account:    &testAccount{},
	...
	result, deepintshieldErr := deepintshield.ExecuteChatMCPTool(ctx, &echoCall)
...
func TestPlugin_ShortCircuit(t *testing.T) {
	...
	governancePlugin.BlockTool("deepintshieldInternal-echo")
	...
	// Tool name might be "echo" or "deepintshieldInternal-echo" in the response
	hasToolName := strings.Contains(*result.Content.ContentStr, "echo") || strings.Contains(*result.Content.ContentStr, "deepintshieldInternal-echo")
...
func TestPlugin_ShortCircuit_CustomMessage(t *testing.T) {
	...
	governancePlugin.BlockTool("deepintshieldInternal-calculator")
	...
	result, deepintshieldErr := deepintshield.ExecuteChatMCPTool(ctx, &calcCall)
	...
	require.NotNil(t, result.Content)
	require.NotNil(t, result.Content.ContentStr)
	// Tool name might be "calculator" or "deepintshieldInternal-calculator" in the response
	hasToolName := strings.Contains(*result.Content.ContentStr, "calculator") || strings.Contains(*result.Content.ContentStr, "deepintshieldInternal-calculator")
...
func TestPlugin_MultiplePlugins(t *testing.T) {
	...
	// Setup DeepIntShield with multiple plugins in pipeline
	deepintshield, err := core.Init(context.Background(), schemas.DeepIntShieldConfig{
		Account:    &testAccount{},
...
func TestPlugin_ResponsesFormat(t *testing.T) {
	...
	responsesToolCall := GetSampleResponsesToolCallMessage("test-responses-format", "deepintshieldInternal-echo", map[string]interface{}{
		"message": "test message",
	})
	result, deepintshieldErr := deepintshield.ExecuteResponsesMCPTool(ctx, responsesToolCall.ResponsesToolMessage)
...
func TestPlugin_LoggingPlugin(t *testing.T) {
	...
	_, err1 := deepintshield.ExecuteChatMCPTool(ctx, &echoCall)
	require.Nil(t, err1)

	calcCall := GetSampleCalculatorToolCall("log-test-2", "add", 1.0, 2.0)
	_, err2 := deepintshield.ExecuteChatMCPTool(ctx, &calcCall)
	require.Nil(t, err2)

	weatherCall := GetSampleWeatherToolCall("log-test-3", "Tokyo", "celsius")
	_, err3 := deepintshield.ExecuteChatMCPTool(ctx, &weatherCall)
	require.Nil(t, err3)
