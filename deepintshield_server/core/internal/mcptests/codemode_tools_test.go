...
result = main()`
	...
	require.Nil(t, deepintshieldErr)
...
func TestCodeMode_CallingMCPTool(t *testing.T) {
	...
	code := `echoResult = deepintshieldInternal.echo(message="Testing MCP call")
...
func TestCodeMode_CallingMultipleServers(t *testing.T) {
	...
	code := `result1 = deepintshieldInternal.echo(message="from HTTP")
result2 = deepintshieldInternal.echo(message="from SSE")
result = {
    "http": result1,
    "sse": result2
}`
...
func TestCodeMode_NestedToolCalls(t *testing.T) {
	...
	code := `# First call
echo1 = deepintshieldInternal.echo(message="step 1")

# Process result
processed = "Processed: " + str(echo1)

# Second call using processed result
echo2 = deepintshieldInternal.echo(message=processed)

# Third call
calc = deepintshieldInternal.calculator(operation="add", x=5, y=3)
...
func TestCodeMode_ToolNotInExecuteList(t *testing.T) {
	...
	code := `def main():
    if hasattr(deepintshieldInternal, "echo"):
        r = deepintshieldInternal.echo(message="blocked")
...
func TestCodeMode_NonAllowedToolExecution(t *testing.T) {
	...
	// Execute code that tries to access deepintshieldInternal
	// When ToolsToExecute = [], the server won't be bound in the environment at all
	// This should cause a runtime error: "undefined: deepintshieldInternal"
	code := `result = deepintshieldInternal.echo(message="should fail")`
	...
	// Verify we get an error because deepintshieldInternal is not defined
	// When ToolsToExecute = [], the server is not bound in the Starlark environment
	require.True(t, hasError, "should have execution error because deepintshieldInternal is not bound when ToolsToExecute is empty")
	assert.Contains(t, strings.ToLower(errorMsg), "undefined", "error should indicate deepintshieldInternal is undefined")
	t.Logf("Expected error (deepintshieldInternal unbound due to empty ToolsToExecute): %s", errorMsg)
}
...
func TestCodeMode_ToolExecutionTimeout(t *testing.T) {
	...
	code := `if hasattr(deepintshieldInternal, "echo"):
    r = deepintshieldInternal.echo(message="test")
	...
	if deepintshieldErr == nil && result != nil {
		returnValue, hasError, errorMsg := ParseCodeModeResponse(t, *result.Content.ContentStr)
		if hasError {
			t.Logf("Error: %s", errorMsg)
		} else {
...
func TestCodeMode_ToolCallWithAwait(t *testing.T) {
	...
	code := `result = deepintshieldInternal.calculator(operation="multiply", x=6, y=7)`
...
func TestCodeMode_ToolCallWithoutAwait(t *testing.T) {
	...
	code := `result = deepintshieldInternal.echo(message="promise test")`
...
results.append(deepintshieldInternal.echo(message="first"))
results.append(deepintshieldInternal.echo(message="second"))
results.append(deepintshieldInternal.echo(message="third"))
...
func TestCodeMode_MultipleParallelToolCalls(t *testing.T) {
	...
	code := `r1 = deepintshieldInternal.echo(message="parallel1")
r2 = deepintshieldInternal.echo(message="parallel2")
r3 = deepintshieldInternal.calculator(operation="add", x=10, y=20)
...
func TestCodeMode_ToolReturnsError(t *testing.T) {
	...
	code := `def main():
    r = deepintshieldInternal.throw_error(error_message="intentional error")
...
func TestCodeMode_ToolNotFound(t *testing.T) {
	...
	// Execute code that tries to use deepintshieldInternal which won't be bound
	// When ToolsToExecute = [], the server won't be bound in the environment at all
	// This should cause a runtime error: "undefined: deepintshieldInternal"
	code := `result = deepintshieldInternal.nonexistent_tool(param="value")`
	...
	// Verify we get an error because deepintshieldInternal is not defined
	// When ToolsToExecute = [], the server is not bound in the Starlark environment
	require.True(t, hasError, "should have execution error because deepintshieldInternal is not bound")
...
func TestCodeMode_ChatFormat(t *testing.T) {
	...
	code := `r = deepintshieldInternal.calculator(operation="divide", x=100, y=4)
