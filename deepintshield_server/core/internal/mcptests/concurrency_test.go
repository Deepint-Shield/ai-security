	...
	for i := 0; i < concurrency; i++ {
		...
		go func(id int) {
			...
			if deepintshieldErr != nil {
				errors <- fmt.Errorf("execution %d failed: %v", id, deepintshieldErr)
				return
			}
...
func TestConcurrent_DifferentTools(t *testing.T) {
	...
	for i := 0; i < concurrency; i++ {
		...
		go func(id, tType int) {
			...
			if deepintshieldErr != nil {
				errors <- fmt.Errorf("tool type %d execution %d failed: %v", tType, id, deepintshieldErr)
				return
			}
...
func TestConcurrent_AddClientDuringExecution(t *testing.T) {
	...
	go func() {
		...
		for i := 0; i < 20; i++ {
			...
			if deepintshieldErr != nil {
				errors <- fmt.Errorf("execution %d failed: %v", i, deepintshieldErr)
			}
...
func TestConcurrent_RemoveClientDuringExecution(t *testing.T) {
	...
	go func() {
		...
		for i := 0; i < 10; i++ {
			...
			if deepintshieldErr != nil {
				t.Logf("Execution %d failed (expected after removal): %v", i, deepintshieldErr)
			}
			executions <- true
	...
	go func() {
		...
		for i := 0; i < 10; i++ {
			...
			if len(clients) > 0 && len(clients[0].ToolMap) > 0 {
				...
				_, deepintshieldErr := deepintshield.ExecuteChatMCPTool(ctx, &toolCall)
				if deepintshieldErr != nil {
					t.Logf("Execution %d: %v", i, deepintshieldErr)
				}
			}
			executions <- true
...
func TestConcurrent_HealthCheckDuringExecution(t *testing.T) {
	...
	go func() {
		...
		if deepintshieldErr != nil {
			errors <- fmt.Errorf("long-running tool failed: %v", deepintshieldErr)
		} else {
	...
	for i := 0; i < 5; i++ {
		...
		go func(id int) {
			...
			if deepintshieldErr != nil {
				errors <- fmt.Errorf("health check %d failed: %v", id, deepintshieldErr)
			}
		}(i)
	}
	...
	for i := 0; i < 10; i++ {
		...
		go func(id int) {
			...
			case 1:
				...
				_, deepintshieldErr := deepintshield.ExecuteChatMCPTool(ctx, &toolCall)
				if deepintshieldErr != nil {
					errors <- fmt.Errorf("calc %d failed: %v", id, deepintshieldErr)
				}
			case 2:
				toolCall := GetSampleWeatherToolCall(fmt.Sprintf("weather-%d", id), "Tokyo", "celsius")
				_, deepintshieldErr := deepintshield.ExecuteChatMCPTool(ctx, &toolCall)
				if deepintshieldErr != nil {
					errors <- fmt.Errorf("weather %d failed: %v", id, deepintshieldErr)
				}
			}
			...
			for j := 0; j < 5; j++ {
				...
				default:
					...
					if deepintshieldErr != nil {
						...
						t.Logf("Execution %d-%d: %v", id, j, deepintshieldErr)
					}
					time.Sleep(10 * time.Millisecond)
				}
			}
...
func TestConcurrent_PluginHooks(t *testing.T) {
	...
	// Setup DeepIntShield with plugin
	account := &testAccount{}
	deepintshieldInstance, err := core.Init(context.Background(), schemas.DeepIntShieldConfig{
		Account:    account,
		MCPPlugins: []schemas.MCPPlugin{loggingPlugin},
		Logger:     core.NewDefaultLogger(schemas.LogLevelInfo),
	})
	require.NoError(t, err)
	deepintshieldInstance.SetMCPManager(manager)
	...
	for i := 0; i < concurrency; i++ {
		...
		go func(id, tType int) {
			...
			case 0: // Echo
				...
				result, deepintshieldErr = deepintshieldInstance.ExecuteChatMCPTool(ctx, &toolCall)
			case 1: // Calculator
				toolCall := GetSampleCalculatorToolCall(fmt.Sprintf("calc-%d", id), "add", float64(id), float64(id+1))
				result, deepintshieldErr = deepintshieldInstance.ExecuteChatMCPTool(ctx, &toolCall)
			}
...
func TestConcurrent_MultiplePlugins(t *testing.T) {
	...
	// Setup DeepIntShield with multiple plugins
	account := &testAccount{}
	deepintshieldInstance, err := core.Init(context.Background(), schemas.DeepIntShieldConfig{
		Account: account,
	...
	for i := 0; i < concurrency; i++ {
		...
		go func(id, opType int) {
			...
			case 0: // Execute echo tool
				...
				_, deepintshieldErr := deepintshield.ExecuteChatMCPTool(ctx, &toolCall)
				if deepintshieldErr != nil {
					errors <- fmt.Errorf("echo %d failed: %v", id, deepintshieldErr)
				}
