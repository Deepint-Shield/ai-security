...
func TestAgent_ErrorHandling_AllToolsFail(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		toolCalls = append(toolCalls, schemas.ChatAssistantMessageToolCall{
			...
			Function: schemas.ChatAssistantMessageToolCallFunction{
				Name:      schemas.Ptr(fmt.Sprintf("deepintshieldInternal-failing_tool_%d", i)),
				Arguments: "{}",
			},
		})
	}
	...
	require.Nil(t, deepintshieldErr, "agent should not crash when all tools fail")
...
func TestAgent_ErrorHandling_TimeoutInLoop(t *testing.T) {
	...
	toolCalls := []schemas.ChatAssistantMessageToolCall{
		{
			...
			Type: schemas.Ptr("function"),
			Function: schemas.ChatAssistantMessageToolCallFunction{
				Name: schemas.Ptr("deepintshieldInternal-slow_tool"),
				Arguments: "{}",
			},
		},
	}
	...
	if deepintshieldErr != nil {
		t.Logf("Timeout resulted in error (expected): %v", deepintshieldErr.Error)
	} else if result != nil {
...
func TestAgent_ErrorHandling_MalformedResponse(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if deepintshieldErr != nil {
				t.Logf("Malformed response (%s) handled with error: %v", tc.name, deepintshieldErr.Error)
			} else if result != nil {
...
func TestAgent_ErrorHandling_PartialBatchFailure(t *testing.T) {
	...
	for i := range outcomes {
		toolCalls = append(toolCalls, schemas.ChatAssistantMessageToolCall{
			...
			Type: schemas.Ptr("function"),
			Function: schemas.ChatAssistantMessageToolCallFunction{
				Name:      schemas.Ptr(fmt.Sprintf("deepintshieldInternal-tool_%d", i)),
				Arguments: "{}",
			},
		})
	}
	...
	require.Nil(t, deepintshieldErr, "partial failures should not crash agent")
	require.NotNil(t, result)
...
func TestAgent_ErrorHandling_RecoveryAndContinuation(t *testing.T) {
			...
			CreateChatResponseWithToolCalls([]schemas.ChatAssistantMessageToolCall{
				{
					...
					Type: schemas.Ptr("function"),
					Function: schemas.ChatAssistantMessageToolCallFunction{
						Name: schemas.Ptr("deepintshieldInternal-fail_tool"),
						Arguments: "{}",
					},
				},
			}),
	...
	require.Nil(t, deepintshieldErr, "agent should recover from error")
	require.NotNil(t, result)
...
func TestAgent_ErrorHandling_ErrorInToolArguments(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if deepintshieldErr != nil {
				t.Logf("Argument error (%s) handled with error: %v", tc.name, deepintshieldErr.Error)
			} else if result != nil {
...
func TestAgent_ErrorHandling_MultipleErrorsInSequence(t *testing.T) {
	...
	require.Nil(t, deepintshieldErr, "agent should handle multiple sequential errors")
	require.NotNil(t, result)
			...
			CreateChatResponseWithToolCalls([]schemas.ChatAssistantMessageToolCall{
				{
					...
					Type: schemas.Ptr("function"),
					Function: schemas.ChatAssistantMessageToolCallFunction{
						Name: schemas.Ptr("deepintshieldInternal-error_tool"),
						Arguments: "{}",
					},
				},
			}),
