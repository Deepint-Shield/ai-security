		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectError {
				...
				if deepintshieldErr != nil {
					assert.Contains(t, deepintshieldErr.Error.Message, tc.errorMessage)
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.shouldFail {
				...
				if deepintshieldErr != nil {
					...
					failureReason = fmt.Sprintf("deepintshieldErr: %v", deepintshieldErr)
				} else if result != nil && result.Content != nil && result.Content.ContentStr != nil {
				...
				if deepintshieldErr != nil {
					t.Errorf("%s (%s): unexpected deepintshieldErr: %v", tc.name, tc.description, deepintshieldErr)
					return
				}
...
func TestListToolFiles_InputValidation(t *testing.T) {
	...
	result, deepintshieldErr := deepintshield.ExecuteChatMCPTool(ctx, &toolCall)
	require.Nil(t, deepintshieldErr, "listToolFiles should succeed")
...
func TestReadToolFile_LineNumberBoundaries(t *testing.T) {
	...
	listResult, _ := deepintshield.ExecuteChatMCPTool(ctx, &listCall)
	...
	for _, tc := range boundaryTests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if deepintshieldErr != nil {
				t.Logf("%s: Got deepintshield error (acceptable): %v", tc.description, deepintshieldErr)
			} else {
