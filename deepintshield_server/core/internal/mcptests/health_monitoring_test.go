...
func TestHealthCheckSTDIOServerDropAndRecoverIn20Seconds(t *testing.T) {
	...
	deepintshieldRoot := "/Users/deepintshield/Desktop/deepintshield"
	clientConfig := GetTemperatureMCPClientConfig(deepintshieldRoot)
	clientConfig.ID = "stdio-health-recovery-test"

	// 1. Create STDIO client with deepintshield manager
...
func TestHealthCheckDuringToolExecution(t *testing.T) {
	...
	clients := manager.GetClients()
	require.Len(t, clients, 1, "should have deepintshieldInternal client")
	...
	require.Nil(t, deepintshieldErr, "tool execution should succeed during health monitoring")
