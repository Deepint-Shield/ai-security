...
func InitMCPServerPaths(t *testing.T) {
	if mcpServerPaths.DeepIntShieldRoot != "" {
		return // Already initialized
	}

	deepintshieldRoot := GetDeepIntShieldRoot(t)
	examplesRoot := filepath.Join(deepintshieldRoot, "..", "examples")

	mcpServerPaths.DeepIntShieldRoot = deepintshieldRoot
	...
	t.Logf("  - DeepIntShield Root: %s", mcpServerPaths.DeepIntShieldRoot)
...
// SetInternalClientAutoExecute configures which tools should be auto-executed for the internal DeepIntShield client
func SetInternalClientAutoExecute(manager *mcp.MCPManager, toolNames []string) error {
	...
	if internalClient == nil {
		return fmt.Errorf("internal deepintshield client not found")
	}
...
// SetInternalClientAsCodeMode configures the internal DeepIntShield client as a CodeMode client
...
// The path is relative to the deepintshield root directory.
func GetTemperatureMCPClientConfig(deepintshieldRoot string) schemas.MCPClientConfig {
	// Use global path if available, otherwise fall back to parameter
	serverPath := mcpServerPaths.TemperatureServer
	if serverPath == "" {
		serverPath = deepintshieldRoot + "/examples/mcps/temperature-server/dist/index.js"
	}
...
func GetGoTestServerConfig(deepintshieldRoot string) schemas.MCPClientConfig {
	// Use global path if available, otherwise fall back to parameter
	serverPath := mcpServerPaths.GoTestServer
	if serverPath == "" {
		serverPath = deepintshieldRoot + "/../examples/mcps/go-test-server/bin/go-test-server"
	}
...
func GetEdgeCaseServerConfig(deepintshieldRoot string) schemas.MCPClientConfig {
	// Use global path if available, otherwise fall back to parameter
	serverPath := mcpServerPaths.EdgeCaseServer
	if serverPath == "" {
		serverPath = deepintshieldRoot + "/../examples/mcps/edge-case-server/bin/edge-case-server"
	}
...
func GetErrorTestServerConfig(deepintshieldRoot string) schemas.MCPClientConfig {
	// Use global path if available, otherwise fall back to parameter
	serverPath := mcpServerPaths.ErrorTestServer
	if serverPath == "" {
		serverPath = deepintshieldRoot + "/../examples/mcps/error-test-server/bin/error-test-server"
	}
...
func GetParallelTestServerConfig(deepintshieldRoot string) schemas.MCPClientConfig {
	// Use global path if available, otherwise fall back to parameter
	serverPath := mcpServerPaths.ParallelTestServer
	if serverPath == "" {
		serverPath = deepintshieldRoot + "/../examples/mcps/parallel-test-server/bin/parallel-test-server"
	}
...
// GetDeepIntShieldRoot returns the deepintshield root directory by walking up from the current directory
func GetDeepIntShieldRoot(t *testing.T) string {
	// Start from current working directory
	cwd, err := os.Getwd()
	require.NoError(t, err, "should get current working directory")

	// Walk up the directory tree to find the deepintshield root (contains go.mod with module github.com/deepint-shield/ai-security)
	dir := cwd
	for {
		goModPath := filepath.Join(dir, "go.mod")
		if _, err := os.Stat(goModPath); err == nil {
			// Found go.mod, this is likely the deepintshield root
			return dir
		}
		...
		if parent == dir {
			...
			t.Fatal("could not find deepintshield root (go.mod not found)")
		}
		dir = parent
	}
}
...
func GetFilteringScenarios() []FilteringScenario {
	return []FilteringScenario{
		// Nil config scenarios
		{
			...
			ConfigTools:      nil,
			ContextTools:     nil,
			RequestedTool:    "deepintshieldInternal-echo",
			ShouldExecute:    false,
		...
		},
		{
			...
			ConfigTools:      nil,
			ContextTools:     []string{"deepintshieldInternal-echo"},
			RequestedTool:    "deepintshieldInternal-echo",
			ShouldExecute:    true,
		...
		},
		{
			...
			ConfigTools:      []string{"*"},
			ContextTools:     []string{"deepintshieldInternal-echo"},
			RequestedTool:    "deepintshieldInternal-calculator",
			ShouldExecute:    false,
		...
		},
		{
			...
			ContextTools:     []string{"deepintshieldInternal-calculator"},
			RequestedTool:    "deepintshieldInternal-calculator",
			ShouldExecute:    true,
...
const (
	...
	// DeepIntShield API configuration
	EnvDeepIntShieldAPIKey       = "OPENAI_API_KEY"
	EnvDeepIntShieldTestProvider = "DEEPINTSHIELD_TEST_PROVIDER"
	EnvDeepIntShieldTestModel    = "DEEPINTSHIELD_TEST_MODEL"
...
func (a *testAccount) GetKeysForProvider(ctx context.Context, providerKey schemas.ModelProvider) ([]schemas.Key, error) {
	...
	apiKey := os.Getenv(EnvDeepIntShieldAPIKey)
	if apiKey == "" {
...
// setupDeepIntShield creates a DeepIntShield instance for testing
func setupDeepIntShield(t *testing.T) *deepintshield.DeepIntShield {
	t.Helper()

	account := &testAccount{}

	// Create deepintshield instance
	deepintshieldInstance, err := deepintshield.Init(context.Background(), schemas.DeepIntShieldConfig{
		Account: account,
		Logger:  deepintshield.NewDefaultLogger(schemas.LogLevelError),
	})
	require.NoError(t, err, "failed to create deepintshield instance")

	// Cleanup
	t.Cleanup(func() {
		deepintshieldInstance.Shutdown()
	})

	return deepintshieldInstance
}
...
func loadTestConfig() *TestConfig {
	...
	testConfig := &TestConfig{
		...
		APIKey:        os.Getenv(EnvDeepIntShieldAPIKey),
		Provider:      schemas.ModelProvider(getEnvOrDefault(EnvDeepIntShieldTestProvider, DefaultTestProvider)),
		Model:         getEnvOrDefault(EnvDeepIntShieldTestModel, DefaultTestModel),
		UseRealLLM:    os.Getenv(EnvDeepIntShieldAPIKey) != "",
		MaxRetries:    3,
...
func AssertToolResponse(t *testing.T, resp *schemas.DeepIntShieldMCPResponse, expectedContent string) {
	t.Helper()
...
func AssertToolExecuted(t *testing.T, resp *schemas.DeepIntShieldMCPResponse, err error) {
	t.Helper()
...
func AssertToolResponseContains(t *testing.T, resp *schemas.DeepIntShieldMCPResponse, expectedText string) {
	t.Helper()
	require.NotNil(t, resp, "response should not be nil")
...
// AssertDeepIntShieldErrorContains asserts that deepintshield error contains expected message
func AssertDeepIntShieldErrorContains(t *testing.T, deepintshieldErr *schemas.DeepIntShieldError, expectedMessage string) {
	t.Helper()
	require.NotNil(t, deepintshieldErr, "deepintshield error should not be nil")
	require.NotNil(t, deepintshieldErr.Error, "deepintshield error.Error should not be nil")
	assert.Contains(t, deepintshieldErr.Error.Message, expectedMessage, "error message should contain expected text")
}
...
func AssertResponseHasToolCalls(t *testing.T, resp *schemas.DeepIntShieldChatResponse, expectedCount int) {
	t.Helper()
	require.NotNil(t, resp, "response should not be nil")
...
func AssertAgentCompletedSuccessfully(t *testing.T, resp *schemas.DeepIntShieldChatResponse, deepintshieldErr *schemas.DeepIntShieldError) {
	t.Helper()
	if deepintshieldErr != nil && deepintshieldErr.Error != nil {
		fmt.Println("deepintshieldErr", deepintshieldErr.Error.Message)
	}
	assert.Nil(t, deepintshieldErr, "agent should complete without error")
...
func CreateTestContextWithMCPFilter(includeClients []string, includeTools []string) *schemas.DeepIntShieldContext {
	...
	return schemas.NewDeepIntShieldContext(baseCtx, schemas.NoDeadline)
}

// CreateTestContextWithTimeout creates a test context with custom timeout
func CreateTestContextWithCustomTimeout(timeout time.Duration) (*schemas.DeepIntShieldContext, context.CancelFunc) {
	baseCtx, cancel := context.WithTimeout(context.Background(), timeout)
	return schemas.NewDeepIntShieldContext(baseCtx, schemas.NoDeadline), cancel
}
...
func GetTestDataPath(t *testing.T, filename string) string {
	t.Helper()
	deepintshieldRoot := GetDeepIntShieldRoot(t)
	return filepath.Join(deepintshieldRoot, "core", "internal", "mcptests", "testdata", filename)
}

// CreateTempTestFile creates a temporary test file
func CreateTempTestFile(t *testing.T, content string) string {
	t.Helper()
	tmpFile, err := os.CreateTemp("", "deepintshield-test-*")
	require.NoError(t, err)
...
type ChatResponseFunc func(history []schemas.ChatMessage) (*schemas.DeepIntShieldChatResponse, *schemas.DeepIntShieldError)

// ResponsesResponseFunc is a function that generates a Responses response based on message history
type ResponsesResponseFunc func(history []schemas.ResponsesMessage) (*schemas.DeepIntShieldResponsesResponse, *schemas.DeepIntShieldError)
...
func (m *DynamicLLMMocker) AddStaticChatResponse(response *schemas.DeepIntShieldChatResponse) {
	m.AddChatResponse(func(history []schemas.ChatMessage) (*schemas.DeepIntShieldChatResponse, *schemas.DeepIntShieldError) {
		return response, nil
	})
}

// AddStaticResponsesResponse adds a static Responses response (backwards compatible)
func (m *DynamicLLMMocker) AddStaticResponsesResponse(response *schemas.DeepIntShieldResponsesResponse) {
	m.AddResponsesResponse(func(history []schemas.ResponsesMessage) (*schemas.DeepIntShieldResponsesResponse, *schemas.DeepIntShieldError) {
		return response, nil
	})
}
...
func (m *DynamicLLMMocker) SetDefaultStaticChatResponse(response *schemas.DeepIntShieldChatResponse) {
	m.SetDefaultChatResponse(func(history []schemas.ChatMessage) (*schemas.DeepIntShieldChatResponse, *schemas.DeepIntShieldError) {
		return response, nil
	})
}

// SetDefaultStaticResponsesResponse sets a static default Responses response
func (m *DynamicLLMMocker) SetDefaultStaticResponsesResponse(response *schemas.DeepIntShieldResponsesResponse) {
	m.SetDefaultResponsesResponse(func(history []schemas.ResponsesMessage) (*schemas.DeepIntShieldResponsesResponse, *schemas.DeepIntShieldError) {
		return response, nil
	})
}

// MakeChatRequest implements the LLM caller interface for Chat API
func (m *DynamicLLMMocker) MakeChatRequest(ctx *schemas.DeepIntShieldContext, req *schemas.DeepIntShieldChatRequest) (*schemas.DeepIntShieldChatResponse, *schemas.DeepIntShieldError) {
...
func (m *DynamicLLMMocker) MakeResponsesRequest(ctx *schemas.DeepIntShieldContext, req *schemas.DeepIntShieldResponsesRequest) (*schemas.DeepIntShieldResponsesResponse, *schemas.DeepIntShieldError) {
	// Store the message history
				...
				for _, tc := range msg.ChatAssistantMessage.ToolCalls {
					if tc.Function.Name != nil {
						...
						if fullName == toolName ||
							fullName == "deepintshieldInternal-"+toolName ||
...
// Supports both prefixed (deepintshieldInternal-toolName) and unprefixed tool names
	...
	for _, msg := range history {
		if msg.Type != nil && *msg.Type == schemas.ResponsesMessageTypeFunctionCall {
			if msg.ResponsesToolMessage != nil && msg.ResponsesToolMessage.Name != nil {
				...
				// Check with deepintshieldInternal- prefix
				if fullName == "deepintshieldInternal-"+toolName {
					return true
				}
...
func CreateChatResponseWithToolCalls(toolCalls []schemas.ChatAssistantMessageToolCall) *schemas.DeepIntShieldChatResponse {
...
func CreateChatResponseWithText(text string) *schemas.DeepIntShieldChatResponse {
	return &schemas.DeepIntShieldChatResponse{
		Choices: []schemas.DeepIntShieldResponseChoice{
			{
...
func CreateResponsesResponseWithToolCalls(toolCalls []schemas.ResponsesToolMessage) *schemas.DeepIntShieldResponsesResponse {
...
func CreateResponsesResponseWithText(text string) *schemas.DeepIntShieldResponsesResponse {
...
func CreateDynamicChatResponse(fn func(history []schemas.ChatMessage) *schemas.DeepIntShieldChatResponse) ChatResponseFunc {
	return func(history []schemas.ChatMessage) (*schemas.DeepIntShieldChatResponse, *schemas.DeepIntShieldError) {
		return fn(history), nil
	}
}

// CreateDynamicResponsesResponse is a convenience function for creating a dynamic Responses response
func CreateDynamicResponsesResponse(fn func(history []schemas.ResponsesMessage) *schemas.DeepIntShieldResponsesResponse) ResponsesResponseFunc {
	return func(history []schemas.ResponsesMessage) (*schemas.DeepIntShieldResponsesResponse, *schemas.DeepIntShieldError) {
		return fn(history), nil
	}
}
...
func CreateValidatingChatResponse(callID string, mustContain []string, successText string, failureText string) ChatResponseFunc {
	return CreateDynamicChatResponse(func(history []schemas.ChatMessage) *schemas.DeepIntShieldChatResponse {
...
func CreateConditionalChatResponse(condition func(history []schemas.ChatMessage) bool, trueResponse, falseResponse *schemas.DeepIntShieldChatResponse) ChatResponseFunc {
	return CreateDynamicChatResponse(func(history []schemas.ChatMessage) *schemas.DeepIntShieldChatResponse {
...
func CreateSequentialChatResponses(responses []*schemas.DeepIntShieldChatResponse) []ChatResponseFunc {
	funcs := make([]ChatResponseFunc, len(responses))
	for i, resp := range responses {
		r := resp // Capture for closure
		funcs[i] = CreateDynamicChatResponse(func(history []schemas.ChatMessage) *schemas.DeepIntShieldChatResponse {
			return r
		})
	}
	return funcs
...
func CreateToolCallSequence(sequences []struct {
	...
	for i, seq := range sequences {
		...
		if isLast {
			...
			funcs = append(funcs, CreateDynamicChatResponse(func(history []schemas.ChatMessage) *schemas.DeepIntShieldChatResponse {
				if toolCall.ID != nil {
...
mocker.AddChatResponse(CreateDynamicChatResponse(func(history []schemas.ChatMessage) *schemas.DeepIntShieldChatResponse {
...
// The tool name is automatically prefixed with "deepintshieldInternal-" to match
...
func CreateToolCallForExecution(callID string, toolName string, args map[string]interface{}) schemas.ChatAssistantMessageToolCall {
	argsJSON, _ := json.Marshal(args)
	prefixedToolName := "deepintshieldInternal-" + toolName
