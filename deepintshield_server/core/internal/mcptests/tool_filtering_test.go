...
func getActualToolsFromGoTestServer(t *testing.T) (tool1, tool2 string) {
	...
	clientConfig := GetGoTestServerConfig(deepintshieldRoot)
...
func executeToolViaManager(t *testing.T, manager interface{ ExecuteToolCall(*schemas.DeepIntShieldContext, *schemas.DeepIntShieldMCPRequest) (*schemas.DeepIntShieldMCPResponse, error) }, toolName string) error {
	t.Helper()
...
func TestFilteringMultipleClients_DifferentRules(t *testing.T) {
	...
	client1 := GetGoTestServerConfig(deepintshieldRoot)
	...
	client2 := GetEdgeCaseServerConfig(deepintshieldRoot)
