...
func (m *MockLLMCaller) MakeChatRequest(ctx *schemas.DeepIntShieldContext, req *schemas.DeepIntShieldChatRequest) (*schemas.DeepIntShieldChatResponse, *schemas.DeepIntShieldError) {
...
func (m *MockLLMCaller) MakeResponsesRequest(ctx *schemas.DeepIntShieldContext, req *schemas.DeepIntShieldResponsesRequest) (*schemas.DeepIntShieldResponsesResponse, *schemas.DeepIntShieldError) {
...
func TestAgent_BasicLoop(t *testing.T) {
	...
	// Execute agent mode
	result, deepintshieldErr := manager.CheckAndExecuteAgentForChatRequest(
		ctx,
		originalReq,
		initialResponse,
	...
	require.Nil(t, deepintshieldErr, "agent loop should complete successfully")
...
func TestAgent_BasicLoop_ResponsesFormat(t *testing.T) {
	...
	result, deepintshieldErr := manager.CheckAndExecuteAgentForResponsesRequest(
		ctx,
		originalReq,
		initialResponse,
...
func TestAgent_MixedAutoAndNonAutoTools(t *testing.T) {
	...
	for _, tc := range finalMessage.ChatAssistantMessage.ToolCalls {
		if *tc.Function.Name == "deepintshieldInternal-calculator" {
			found = true
			break
		}
	}
...
func TestAgent_OnlyNonAutoTools(t *testing.T) {
	...
	require.NotNil(t, finalMessage.ChatAssistantMessage)
	require.NotEmpty(t, finalMessage.ChatAssistantMessage.ToolCalls)
	assert.Equal(t, "deepintshieldInternal-echo", *finalMessage.ChatAssistantMessage.ToolCalls[0].Function.Name)
}
...
func TestAgent_WithRealLLM_Simple(t *testing.T) {
	...
	// Setup deepintshield with real LLM
	...
	result, deepintshieldErr := deepintshield.ChatCompletionRequest(ctx, req)
	if deepintshieldErr != nil {
		// Skip if there's an API error (likely missing/invalid API key or config issue)
		t.Skipf("Skipping real LLM test due to API error: %v", deepintshieldErr.Error)
	}
	require.NotNil(t, result)
