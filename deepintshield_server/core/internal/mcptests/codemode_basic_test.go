...
func TestCodeMode_MultiServer_BasicCalls(t *testing.T) {
	...
	temperatureClient := GetTemperatureMCPClientConfig(deepintshieldRoot)
	temperatureClient.IsCodeModeClient = true
	temperatureClient.ToolsToExecute = []string{"*"}

	goTestClient := GetGoTestServerConfig(deepintshieldRoot)
...
echo = deepintshieldInternal.echo(message="multi-server")
...
`
	...
	require.Nil(t, deepintshieldErr, "should execute without deepintshield error")
...
func TestCodeMode_MultiServer_ParallelExecution(t *testing.T) {
	...
	parallelClient := GetParallelTestServerConfig(deepintshieldRoot)
	parallelClient.ToolsToExecute = []string{"*"}

	edgeClient := GetEdgeCaseServerConfig(deepintshieldRoot)
