...
func SetupAgentTest(t *testing.T, config AgentTestConfig) (*mcp.MCPManager, *DynamicLLMMocker, *schemas.DeepIntShieldContext) {
	t.Helper()
...
func SetupAgentTestWithClients(t *testing.T, config AgentTestConfig, customClients []schemas.MCPClientConfig) (*mcp.MCPManager, *DynamicLLMMocker, *schemas.DeepIntShieldContext) {
	t.Helper()
...
func SetupMultiClientAgentTest(t *testing.T, inProcessTools []string, stdioClients []string, autoExecute []string, maxDepth int) (*mcp.MCPManager, *DynamicLLMMocker, *schemas.DeepIntShieldContext) {
	t.Helper()
...
func SetupContextFilteredAgentTest(t *testing.T, inProcessTools []string, stdioClients []string, autoExecute []string, toolFilter []string, clientFilter []string) (*mcp.MCPManager, *DynamicLLMMocker, *schemas.DeepIntShieldContext) {
	t.Helper()
...
func AssertAgentFinalResponse(t *testing.T, response *schemas.DeepIntShieldChatResponse, expectedFinishReason string, shouldContainText string) {
	t.Helper()
...
func AssertToolExecutedInTurn(t *testing.T, mocker *DynamicLLMMocker, toolName string, turn int) {
	...
	// Check for exact match or with "deepintshieldInternal-" prefix
	if assistantMsg.ChatAssistantMessage != nil {
		for _, tc := range assistantMsg.ChatAssistantMessage.ToolCalls {
			if tc.Function.Name != nil {
				...
				if fullName == toolName || fullName == "deepintshieldInternal-"+toolName ||
...
func AssertNoToolCalls(t *testing.T, response *schemas.DeepIntShieldChatResponse) {
	t.Helper()
...
func AssertAgentError(t *testing.T, deepintshieldErr *schemas.DeepIntShieldError, shouldContain string) {
	t.Helper()

	require.NotNil(t, deepintshieldErr, "Should return error")
	require.NotNil(t, deepintshieldErr.Error, "Error field should not be nil")

	if shouldContain != "" {
		assert.Contains(t, deepintshieldErr.Error.Message, shouldContain, "Error message should contain expected text")
	}
}

// AssertAgentSuccess verifies the agent completed without errors
func AssertAgentSuccess(t *testing.T, response *schemas.DeepIntShieldChatResponse, deepintshieldErr *schemas.DeepIntShieldError) {
	t.Helper()

	if deepintshieldErr != nil && deepintshieldErr.Error != nil {
		t.Logf("deepintshieldErr: %s", deepintshieldErr.Error.Message)
	}
	assert.Nil(t, deepintshieldErr, "Should not return error")
	require.NotNil(t, response, "Response should not be nil")
	require.NotEmpty(t, response.Choices, "Response should have choices")
}
...
func AssertRequestIDChanged(t *testing.T, ctx1 *schemas.DeepIntShieldContext, ctx2 *schemas.DeepIntShieldContext) {
	t.Helper()
...
func AssertRequestIDPropagated(t *testing.T, ctx *schemas.DeepIntShieldContext) {
	t.Helper()
...
// In-process tools are registered with "deepintshieldInternal-" prefix
// The tool name format is: deepintshieldInternal-{tool_name} (e.g., "deepintshieldInternal-echo")
func CreateInProcessToolCall(id, toolName string, args map[string]interface{}) schemas.ChatAssistantMessageToolCall {
	fullToolName := fmt.Sprintf("deepintshieldInternal-%s", toolName)
	return CreateToolCall(id, fullToolName, args)
}
...
func RunAgentScenario(t *testing.T, scenario AgentScenario) {
	...
	t.Run(scenario.Name, func(t *testing.T) {
		...
		response, deepintshieldErr := manager.CheckAndExecuteAgentForChatRequest(
			ctx,
			req,
			initialResponse,
			mocker.MakeChatRequest,
			func(ctx *schemas.DeepIntShieldContext, request *schemas.DeepIntShieldMCPRequest) (*schemas.DeepIntShieldMCPResponse, error) {
		...
		for _, assertion := range scenario.Assertions {
			...
			case "success":
				AssertAgentSuccess(t, response, deepintshieldErr)
			case "final_reason":
...
func SimpleAgentTest(t *testing.T, name string, config AgentTestConfig, responses []ChatResponseFunc, assertions func(*testing.T, *schemas.DeepIntShieldChatResponse, *schemas.DeepIntShieldError, *DynamicLLMMocker)) {
	t.Helper()
	t.Run(name, func(t *testing.T) {
		...
		assertions(t, response, deepintshieldErr, mocker)
	})
}
...
func CreateAgentTurnWithToolCallsResponses(toolCalls ...schemas.ChatAssistantMessageToolCall) ResponsesResponseFunc {
	return CreateDynamicResponsesResponse(func(history []schemas.ResponsesMessage) *schemas.DeepIntShieldResponsesResponse {
...
func AssertAgentFinalResponseResponses(t *testing.T, result *schemas.DeepIntShieldResponsesResponse, mustContainInContent string) {
	t.Helper()
