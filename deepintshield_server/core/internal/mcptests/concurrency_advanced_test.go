	...
	for i := 0; i < numConcurrent; i++ {
		...
		go func(id int) {
			...
			if deepintshieldErr != nil {
				errors <- fmt.Errorf("execution %d failed: %v", id, deepintshieldErr.Error.Message)
				return
			}
			if result == nil {
...
func TestConcurrent_CodeModeExecutionWithToolCalls(t *testing.T) {
	...
	for i := 0; i < numConcurrent; i++ {
		...
		go func(id int) {
			...
			switch id % 3 {
			case 0:
				code = fmt.Sprintf(`await deepintshieldInternal.echo({message: "test-%d"})`, id)
			case 1:
				code = fmt.Sprintf(`await deepintshieldInternal.calculator({operation: "add", x: %d, y: 10})`, id)
			case 2:
				code = `await deepintshieldInternal.get_time({timezone: "UTC"})`
			}
	...
	for i := 0; i < 10; i++ {
		...
		go func(id int) {
			...
			for _, client := range clients {
				if client.ExecutionConfig.ID == "deepintshieldInternal" {
		...
		go func(id int) {
			...
			case 2:
				...
				toolCall = schemas.ChatAssistantMessageToolCall{
					...
					Type: schemas.Ptr("function"),
					Function: schemas.ChatAssistantMessageToolCallFunction{
						Name:      schemas.Ptr("deepintshieldInternal-get_time"),
						Arguments: toJSON(argsMap),
					},
				}
			case 3:
				...
				toolCall = schemas.ChatAssistantMessageToolCall{
					...
					Type: schemas.Ptr("function"),
					Function: schemas.ChatAssistantMessageToolCallFunction{
						Name:      schemas.Ptr("deepintshieldInternal-search"),
						Arguments: toJSON(argsMap),
					},
				}
			}
...
func TestConcurrent_FilteringChanges(t *testing.T) {
	...
	for i := 0; i < 50; i++ {
		...
		go func(id int) {
			...
			var ctx *schemas.DeepIntShieldContext

			if id%2 == 0 {
				// Even: allow all tools
				baseCtx := context.Background()
				baseCtx = context.WithValue(baseCtx, mcp.MCPContextKeyIncludeClients, []string{"*"})
				baseCtx = context.WithValue(baseCtx, mcp.MCPContextKeyIncludeTools, []string{"deepintshieldInternal-*"})
				ctx = schemas.NewDeepIntShieldContext(baseCtx, schemas.NoDeadline)
			} else {
				// Odd: allow only echo
				baseCtx := context.Background()
				baseCtx = context.WithValue(baseCtx, mcp.MCPContextKeyIncludeClients, []string{"*"})
				baseCtx = context.WithValue(baseCtx, mcp.MCPContextKeyIncludeTools, []string{"deepintshieldInternal-echo"})
				ctx = schemas.NewDeepIntShieldContext(baseCtx, schemas.NoDeadline)
			}
	...
	for i := 0; i < numConcurrent; i++ {
		...
		go func(id int) {
			...
			for time.Now().Before(stopTime) {
				...
				result, deepintshieldErr := deepintshield.ExecuteChatMCPTool(ctx, &toolCall)
				if deepintshieldErr != nil {
					errors <- fmt.Errorf("execution %d-%d failed: %v", id, counter, deepintshieldErr.Error.Message)
					return
				}
				if result != nil {
					successCount.Add(1)
