...
func TestAgent_ContextToolFilter_BlockedToolError(t *testing.T) {
	...
	require.NotNil(t, deepintshieldErr, "Should return error when tool is filtered")
	require.Nil(t, result, "Result should be nil when there's an error")

	// Verify error contains expected message
	require.NotNil(t, deepintshieldErr.Error)
	// The error will be propagated from the tool execution failure
	t.Logf("Error message: %s", deepintshieldErr.Error.Message)
}
...
func TestAgent_ContextClientFilter_Whitelist(t *testing.T) {
	...
	// Context only allows InProcess client (deepintshieldInternal)
	...
	manager, mocker, ctx := SetupAgentTestWithClients(t, AgentTestConfig{
		...
		AutoExecuteTools: []string{"*"},
		ClientFiltering:  []string{"deepintshieldInternal"}, // Only allow InProcess client
		MaxDepth:         5,
...
func TestAgent_ContextToolFilter_EmptyList(t *testing.T) {
	...
	require.NotNil(t, deepintshieldErr, "Should return error when tool is filtered by empty context")
	require.Nil(t, result)

	// Verify error indicates tool is not permitted
	require.NotNil(t, deepintshieldErr.Error)
	t.Logf("Error (expected): %s", deepintshieldErr.Error.Message)
}
...
func TestAgent_ContextClientFilter_MultipleClients(t *testing.T) {
	...
	manager, mocker, ctx := SetupAgentTestWithClients(t, AgentTestConfig{
		InProcessTools:   []string{"echo"},
		AutoExecuteTools: []string{"*"},
		ClientFiltering:  []string{"deepintshieldInternal", "temperature-mcp-client"}, // Allow 2 clients
		MaxDepth:         10,
...
func TestAgent_ContextToolFilter_ParallelMixed(t *testing.T) {
	...
	if deepintshieldErr != nil {
		...
		require.NotNil(t, deepintshieldErr.Error)
		t.Logf("Error returned: %s", deepintshieldErr.Error.Message)
		require.Nil(t, result)
	} else {
