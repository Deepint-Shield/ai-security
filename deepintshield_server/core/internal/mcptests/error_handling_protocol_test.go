...
func TestErrorHandling_InProcess_ToolReturnsError(t *testing.T) {
	...
	if deepintshieldErr != nil {
		...
		assert.Contains(t, deepintshieldErr.Error.Message, errorMessage, "error message should contain original error")
		t.Logf("✅ Tool error returned as execution error: %s", deepintshieldErr.Error.Message)
	} else {
...
func TestErrorHandling_InProcess_ToolReturnsError_ResponsesFormat(t *testing.T) {
	...
	result, deepintshieldErr := deepintshield.ExecuteResponsesMCPTool(ctx, &responsesToolMsg)
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.shouldError {
				...
				if deepintshieldErr != nil {
					...
					if tc.errorMatch != "" {
						assert.Contains(t, deepintshieldErr.Error.Message, tc.errorMatch,
							"error message should contain expected text for test case '%s'", tc.name)
					}
					t.Logf("✅ %s: Error handled as execution error: %s", tc.name, deepintshieldErr.Error.Message)
					errorHandled = true
				} else if result != nil {
				...
				assert.Nil(t, deepintshieldErr, "test case '%s' should not return error", tc.name)
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectErr {
				...
				errorHandled := (deepintshieldErr != nil) || (result != nil)
				...
				assert.Nil(t, deepintshieldErr, "test '%s' should not return error", tc.name)
				assert.NotNil(t, result, "result should not be nil")
...
func TestErrorHandling_STDIO_MCPErrorResponse(t *testing.T) {
	...
	errorServerConfig := GetErrorTestServerConfig(deepintshieldRoot)
...
func TestErrorHandling_STDIO_TimeoutScenario(t *testing.T) {
	...
	assert.NotNil(t, deepintshieldErr, "should return timeout error")
	assert.Nil(t, result, "result should be nil on timeout")
	assert.Contains(t, deepintshieldErr.Error.Message, "timed out", "error message should indicate timeout")
	...
	t.Logf("✅ Timeout handled correctly after %v: %s", elapsed, deepintshieldErr.Error.Message)
}
...
func TestErrorHandling_STDIO_MalformedJSON(t *testing.T) {
	...
	if deepintshieldErr != nil {
		...
		t.Logf("✅ Malformed JSON handled as error: %s", deepintshieldErr.Error.Message)
	} else {
...
func TestErrorHandling_ErrorMessageFormat(t *testing.T) {
	...
	if deepintshieldErr != nil {
		...
		assert.NotEmpty(t, deepintshieldErr.Error.Message, "error message should not be empty")
		assert.Contains(t, deepintshieldErr.Error.Message, errorMessage, "error should contain original message")
		t.Logf("✅ Error message format (as error): %s", deepintshieldErr.Error.Message)
	} else {
...
func TestErrorHandling_MultipleConsecutiveErrors(t *testing.T) {
	...
	for i := 0; i < numErrors; i++ {
		...
		if deepintshieldErr != nil {
			assert.Contains(t, deepintshieldErr.Error.Message, errorMessage, "error %d should contain correct message", i+1)
		} else {
...
func TestErrorHandling_ErrorWithSpecialCharacters(t *testing.T) {
	...
	for i, msg := range specialMessages {
		...
		if deepintshieldErr != nil {
			...
			assert.True(t,
				strings.Contains(deepintshieldErr.Error.Message, msg) ||
					strings.Contains(deepintshieldErr.Error.Message, strings.ReplaceAll(msg, "\n", "\\n")) ||
					strings.Contains(deepintshieldErr.Error.Message, strings.ReplaceAll(msg, "\t", "\\t")),
