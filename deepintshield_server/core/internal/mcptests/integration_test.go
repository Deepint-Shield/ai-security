...
func TestIntegration_FullChatWorkflow(t *testing.T) {
	...
	// End-to-end test: Setup deepintshield with MCP, add multiple clients,
	// execute tools, verify workflow, check health, remove client

	// 1. Setup deepintshield with MCP manager
	...
	echoResult, echoErr := deepintshield.ExecuteChatMCPTool(ctx, &echoCall)
	...
	calcResult, calcErr := deepintshield.ExecuteChatMCPTool(ctx, &calcCall)
...
func TestIntegration_FullResponsesWorkflow(t *testing.T) {
	...
	echoToolCall := GetSampleResponsesToolCallMessage("call-1", "deepintshieldInternal-echo", map[string]interface{}{
	...
	echoResult, echoErr := deepintshield.ExecuteResponsesMCPTool(ctx, echoToolCall.ResponsesToolMessage)
	require.Nil(t, echoErr, "echo tool should execute successfully")
	require.NotNil(t, echoResult, "echo tool should return result")
	assert.Equal(t, schemas.ResponsesMessageTypeFunctionCallOutput, *echoResult.Type)

	// Execute calculator tool (Responses format)
	calcToolCall := GetSampleResponsesToolCallMessage("call-2", "deepintshieldInternal-calculator", map[string]interface{}{
		"operation": "multiply",
		"x":         float64(5),
		"y":         float64(7),
	})
	calcResult, calcErr := deepintshield.ExecuteResponsesMCPTool(ctx, calcToolCall.ResponsesToolMessage)
	require.Nil(t, calcErr, "calculator tool should execute successfully")
	require.NotNil(t, calcResult, "calculator tool should return result")
...
func TestIntegration_MultiClientMultiTool(t *testing.T) {
	...
	weatherResult, weatherErr := deepintshield.ExecuteChatMCPTool(ctx, &weatherCall)
	...
	// Tools are registered without prefix but accessed with deepintshieldInternal- prefix
	// Check that we have at least the registered tools (they might be named either way depending on how they're exposed)
	hasEcho := allTools["echo"] || allTools["deepintshieldInternal-echo"]
	hasCalculator := allTools["calculator"] || allTools["deepintshieldInternal-calculator"]
	hasWeather := allTools["get_weather"] || allTools["deepintshieldInternal-get_weather"]
...
func TestIntegration_ReconnectDuringExecution(t *testing.T) {
	...
	if deepintshieldErr != nil {
		t.Logf("Initial execution: %v", deepintshieldErr)
	}
...
func TestIntegration_ErrorRecovery(t *testing.T) {
	...
	result1, err1 := deepintshield.ExecuteChatMCPTool(ctx, &echoCall)
	...
	_, err2 := deepintshield.ExecuteChatMCPTool(ctx, &errorCall)
	...
	result3, err3 := deepintshield.ExecuteChatMCPTool(ctx, &calcCall)
...
func TestIntegration_PartialFailure(t *testing.T) {
	...
	errors := make([]*schemas.DeepIntShieldError, 3)

	// Tool 1: Echo (should succeed)
	echoCall := GetSampleEchoToolCall("call-1", "first tool")
	results[0], errors[0] = deepintshield.ExecuteChatMCPTool(ctx, &echoCall)
	...
	results[1], errors[1] = deepintshield.ExecuteChatMCPTool(ctx, &errorCall)

	// Tool 3: Calculator (should succeed)
	calcCall := GetSampleCalculatorToolCall("call-3", "multiply", 6.0, 7.0)
	results[2], errors[2] = deepintshield.ExecuteChatMCPTool(ctx, &calcCall)
