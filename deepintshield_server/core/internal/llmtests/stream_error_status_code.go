...
// the correct HTTP status code in DeepIntShieldError.StatusCode. This is critical because
...
// and DeepIntShield must preserve those codes through the error chain.
func RunStreamErrorStatusCodeTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("StreamErrorStatusCode", func(t *testing.T) {
		...
		invalidModel := "deepintshield-nonexistent-model-for-testing-12345"
		...
		t.Run("ChatCompletionStream", func(t *testing.T) {
			...
			request := &schemas.DeepIntShieldChatRequest{
				...
				Params: &schemas.ChatParameters{
					MaxCompletionTokens: deepintshield.Ptr(10),
				},
			}

			bfCtx := schemas.NewDeepIntShieldContext(ctx, schemas.NoDeadline)
			stream, deepintshieldErr := client.ChatCompletionStreamRequest(bfCtx, request)
			...
			if deepintshieldErr.StatusCode == nil {
				t.Fatalf("❌ DeepIntShieldError.StatusCode is nil for provider %s - provider status code was not propagated. Error: %s",
					testConfig.Provider, GetErrorMessage(deepintshieldErr))
			}
			...
			if statusCode < 400 || statusCode >= 600 {
				t.Fatalf("❌ Expected 4xx/5xx status code for invalid model, got %d. Error: %s",
					statusCode, GetErrorMessage(deepintshieldErr))
			}

			// Should not be a DeepIntShield-generated error - it should come from the provider
			if deepintshieldErr.IsDeepIntShieldError {
				// Some providers may have deepintshield-level validation that catches invalid models
				// before reaching the provider. Log but don't fail.
				t.Logf("⚠️  Error is a DeepIntShield error (not provider error) with status %d - this may indicate model validation happened before the provider call", statusCode)
			}

			t.Logf("✅ Stream error for invalid model returned status code %d (provider: %s)", statusCode, testConfig.Provider)
			t.Logf("   Error message: %s", GetErrorMessage(deepintshieldErr))
		})
		...
		t.Run("ResponsesStream", func(t *testing.T) {
			...
			request := &schemas.DeepIntShieldResponsesRequest{
				...
				Params: &schemas.ResponsesParameters{
					MaxOutputTokens: deepintshield.Ptr(10),
				},
			}

			bfCtx := schemas.NewDeepIntShieldContext(ctx, schemas.NoDeadline)
			stream, deepintshieldErr := client.ResponsesStreamRequest(bfCtx, request)
			...
			if deepintshieldErr.StatusCode == nil {
				t.Fatalf("❌ DeepIntShieldError.StatusCode is nil for provider %s responses stream - provider status code was not propagated. Error: %s",
					testConfig.Provider, GetErrorMessage(deepintshieldErr))
			}
