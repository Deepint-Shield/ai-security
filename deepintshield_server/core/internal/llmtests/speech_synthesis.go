...
func RunSpeechSynthesisTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("SpeechSynthesis", func(t *testing.T) {
		...
		for _, tc := range testCases {
			t.Run(tc.name, func(t *testing.T) {
				...
				speechResponse, deepintshieldErr := WithSpeechTestRetry(t, speechRetryConfig, retryContext, expectations, "SpeechSynthesis_"+tc.name, func() (*schemas.DeepIntShieldSpeechResponse, *schemas.DeepIntShieldError) {
				...
				if deepintshieldErr != nil {
					t.Fatalf("❌ SpeechSynthesis_"+tc.name+" request failed after retries: %v", GetErrorMessage(deepintshieldErr))
				}
...
func RunSpeechSynthesisAdvancedTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.SpeechSynthesis {
		t.Logf("Speech synthesis not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("SpeechSynthesisAdvanced", func(t *testing.T) {
		...
		t.Run("LongText_HDModel", func(t *testing.T) {
			...
			speechResponse, deepintshieldErr := WithSpeechTestRetry(t, speechRetryConfig, retryContext, expectations, "SpeechSynthesis_HD", func() (*schemas.DeepIntShieldSpeechResponse, *schemas.DeepIntShieldError) {
				requestCtx := schemas.NewDeepIntShieldContext(ctx, schemas.NoDeadline)
				return client.SpeechRequest(requestCtx, request)
			})
			if deepintshieldErr != nil {
				t.Fatalf("❌ SpeechSynthesis_HD request failed after retries: %v", GetErrorMessage(deepintshieldErr))
			}
		...
		t.Run("AllVoiceOptions", func(t *testing.T) {
			...
			for _, voiceType := range voiceTypes {
				t.Run("VoiceType_"+voiceType, func(t *testing.T) {
					...
					speechResponse, deepintshieldErr := WithSpeechTestRetry(t, voiceSpeechRetryConfig, voiceRetryContext, expectations, "SpeechSynthesis_VoiceType_"+voiceType, func() (*schemas.DeepIntShieldSpeechResponse, *schemas.DeepIntShieldError) {
						requestCtx := schemas.NewDeepIntShieldContext(ctx, schemas.NoDeadline)
						return client.SpeechRequest(requestCtx, request)
					})

					if deepintshieldErr != nil {
						t.Fatalf("❌ SpeechSynthesis_Voice_"+voiceType+" request failed after retries: %v", GetErrorMessage(deepintshieldErr))
					}
...
func validateSpeechSynthesisSpecific(t *testing.T, response *schemas.DeepIntShieldSpeechResponse, expectMinBytes int, expectedModel string) {
	if response == nil {
