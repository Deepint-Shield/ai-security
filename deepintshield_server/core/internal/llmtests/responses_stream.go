...
func RunResponsesStreamTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("ResponsesStream", func(t *testing.T) {
		...
		request := &schemas.DeepIntShieldResponsesRequest{
			...
			Params: &schemas.ResponsesParameters{
				MaxOutputTokens: deepintshield.Ptr(150),
			},
			...
			func(responseChannel chan *schemas.DeepIntShieldStreamChunk) ResponsesStreamValidationResult {
				...
				for {
					...
					case response, ok := <-responseChannel:
						...
						if response.DeepIntShieldResponsesStreamResponse != nil {
							if response.DeepIntShieldResponsesStreamResponse.ExtraFields.Provider != testConfig.Provider {
								t.Logf("⚠️ Warning: Provider mismatch - expected %s, got %s", testConfig.Provider, response.DeepIntShieldResponsesStreamResponse.ExtraFields.Provider)
							}

							// Log latency for each chunk (can be 0 for inter-chunks)
							t.Logf("📊 Chunk %d latency: %d ms", responseCount+1, response.DeepIntShieldResponsesStreamResponse.ExtraFields.Latency)

							// Process the streaming response
							streamResp := response.DeepIntShieldResponsesStreamResponse
	...
	if testConfig.Scenarios.Reasoning && testConfig.ReasoningModel != "" {
		t.Run("ResponsesStreamWithReasoning", func(t *testing.T) {
			...
			request := &schemas.DeepIntShieldResponsesRequest{
				...
				Input:    messages,
				Params: &schemas.ResponsesParameters{
					MaxOutputTokens: deepintshield.Ptr(1800),
					Reasoning: &schemas.ResponsesParametersReasoning{
						Effort: deepintshield.Ptr("high"),
						// Summary: deepintshield.Ptr("detailed"),
					},
	...
	t.Run("ResponsesStreamLifecycle", func(t *testing.T) {
		...
		request := &schemas.DeepIntShieldResponsesRequest{
			...
			Input:    messages,
			Params: &schemas.ResponsesParameters{
				MaxOutputTokens: deepintshield.Ptr(50),
			},
			Fallbacks: testConfig.Fallbacks,
		}
...
func validateResponsesStreamingResponse(t *testing.T, eventTypes map[schemas.ResponsesStreamResponseType]int, sequenceNumbers []int, finalContent string, lastResponse *schemas.DeepIntShieldStreamChunk, testConfig ComprehensiveTestConfig) StreamingValidationResult {
	var errors []string
	...
	if lastResponse == nil {
		...
		if lastResponse.DeepIntShieldResponsesStreamResponse == nil {
			errors = append(errors, "Last streaming response should have DeepIntShieldResponsesStreamResponse")
		} else {
			if lastResponse.DeepIntShieldResponsesStreamResponse.ExtraFields.Provider != testConfig.Provider {
				errors = append(errors, fmt.Sprintf("Provider mismatch: expected %s, got %s", testConfig.Provider, lastResponse.DeepIntShieldResponsesStreamResponse.ExtraFields.Provider))
			}
		}
	}
	...
	if lastResponse != nil && lastResponse.DeepIntShieldResponsesStreamResponse != nil {
		if lastResponse.DeepIntShieldResponsesStreamResponse.ExtraFields.Latency <= 0 {
			errors = append(errors, fmt.Sprintf("Last streaming chunk missing latency information (got %d ms)", lastResponse.DeepIntShieldResponsesStreamResponse.ExtraFields.Latency))
		} else {
			t.Logf("✅ Total streaming latency: %d ms", lastResponse.DeepIntShieldResponsesStreamResponse.ExtraFields.Latency)
		}
	}
