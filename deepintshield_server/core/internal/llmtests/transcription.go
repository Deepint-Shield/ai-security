...
func RunTranscriptionTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("Transcription", func(t *testing.T) {
		...
		for _, tc := range roundTripCases {
			t.Run(tc.name, func(t *testing.T) {
				...
				var transcriptionRequest *schemas.DeepIntShieldTranscriptionRequest
				if testConfig.Provider == schemas.HuggingFace && strings.HasPrefix(testConfig.TranscriptionModel, "fal-ai/") {
					...
					ttsResponse, err := WithSpeechTestRetry(t, speechRetryConfig, ttsRetryContext, ttsExpectations, "Transcription_RoundTrip_TTS_"+tc.name, func() (*schemas.DeepIntShieldSpeechResponse, *schemas.DeepIntShieldError) {
				...
				transcriptionResponse, deepintshieldErr := WithTranscriptionTestRetry(t, transcriptionRetryConfig, retryContext, expectations, "Transcription_RoundTrip_"+tc.name, func() (*schemas.DeepIntShieldTranscriptionResponse, *schemas.DeepIntShieldError) {
					bfCtx := schemas.NewDeepIntShieldContext(ctx, schemas.NoDeadline)
					return client.TranscriptionRequest(bfCtx, transcriptionRequest)
				})

				if deepintshieldErr != nil {
					t.Fatalf("❌ Transcription_RoundTrip_"+tc.name+" request failed after retries: %v", GetErrorMessage(deepintshieldErr))
				}
		...
		t.Run("AdditionalAudioTests", func(t *testing.T) {
			...
			for _, tc := range customCases {
				t.Run(tc.name, func(t *testing.T) {
					...
					response, err := WithTranscriptionTestRetry(t, customTranscriptionRetryConfig, customRetryContext, customExpectations, "Transcription_Custom_"+tc.name, func() (*schemas.DeepIntShieldTranscriptionResponse, *schemas.DeepIntShieldError) {
						bfCtx := schemas.NewDeepIntShieldContext(ctx, schemas.NoDeadline)
...
func RunTranscriptionAdvancedTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.Transcription {
		t.Logf("Transcription not supported for provider %s", testConfig.Provider)
		return
	}
		...
		t.Run("AllResponseFormats", func(t *testing.T) {
			...
			for _, format := range formats {
				t.Run("Format_"+format, func(t *testing.T) {
					...
					response, err := WithTranscriptionTestRetry(t, formatTranscriptionRetryConfig, formatRetryContext, formatExpectations, "Transcription_Format_"+format, func() (*schemas.DeepIntShieldTranscriptionResponse, *schemas.DeepIntShieldError) {
						bfCtx := schemas.NewDeepIntShieldContext(ctx, schemas.NoDeadline)
						return client.TranscriptionRequest(bfCtx, request)
					})
					if err != nil {
		...
		t.Run("WithCustomParameters", func(t *testing.T) {
			...
			request := &schemas.DeepIntShieldTranscriptionRequest{
				...
				Params: &schemas.TranscriptionParameters{
					Language:       deepintshield.Ptr("en"),
					Format:         &audioFormat,
					Prompt:         deepintshield.Ptr("This audio contains technical terminology and proper nouns."),
					ResponseFormat: deepintshield.Ptr("json"), // Use json instead of verbose_json for whisper-1
				},
				Fallbacks: testConfig.TranscriptionFallbacks,
			}
			...
			response, err := WithTranscriptionTestRetry(t, advancedTranscriptionRetryConfig, advancedRetryContext, advancedExpectations, "Transcription_Advanced_CustomParams", func() (*schemas.DeepIntShieldTranscriptionResponse, *schemas.DeepIntShieldError) {
				bfCtx := schemas.NewDeepIntShieldContext(ctx, schemas.NoDeadline)
				return client.TranscriptionRequest(bfCtx, request)
			})
			if err != nil {
		...
		t.Run("MultipleLanguages", func(t *testing.T) {
			...
			for _, lang := range languages {
				t.Run("Language_"+lang, func(t *testing.T) {
					...
					response, err := WithTranscriptionTestRetry(t, langTranscriptionRetryConfig, langRetryContext, langExpectations, "Transcription_Language_"+lang, func() (*schemas.DeepIntShieldTranscriptionResponse, *schemas.DeepIntShieldError) {
						bfCtx := schemas.NewDeepIntShieldContext(ctx, schemas.NoDeadline)
						return client.TranscriptionRequest(bfCtx, request)
					})
					if err != nil {
...
func validateTranscriptionRoundTrip(t *testing.T, response *schemas.DeepIntShieldTranscriptionResponse, originalText string, testName string, testConfig ComprehensiveTestConfig) {
