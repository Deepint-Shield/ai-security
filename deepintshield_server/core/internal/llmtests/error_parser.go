...
var (
	...
	CategoryDeepIntShield    = ErrorCategory{"DeepIntShield", "DeepIntShield Internal Error", "🌉"}
	CategoryUnknown    = ErrorCategory{"Unknown", "Unknown Error", "❓"}
)

// ParseDeepIntShieldError converts a DeepIntShieldError into a human-readable ParsedError
func ParseDeepIntShieldError(err *schemas.DeepIntShieldError) ParsedError {
	if err == nil {
		return ParsedError{
	...
	parsed.Technical["is_deepintshield_error"] = err.IsDeepIntShieldError
...
func categorizeError(err *schemas.DeepIntShieldError) (category, title string) {
	...
	case err.IsDeepIntShieldError:
		return CategoryDeepIntShield.Name, "DeepIntShield Internal Error"
	}
...
func parseHTTPError(err *schemas.DeepIntShieldError, parsed *ParsedError) {
	if err.StatusCode != nil {
...
func parseAuthError(err *schemas.DeepIntShieldError, parsed *ParsedError) {
	message := strings.ToLower(err.Error.Message)
...
func parseRateLimitError(err *schemas.DeepIntShieldError, parsed *ParsedError) {
...
func parseProviderError(err *schemas.DeepIntShieldError, parsed *ParsedError) {
...
func parseValidationError(err *schemas.DeepIntShieldError, parsed *ParsedError) {
...
func parseTimeoutError(err *schemas.DeepIntShieldError, parsed *ParsedError) {
...
func parseQuotaError(err *schemas.DeepIntShieldError, parsed *ParsedError) {
...
func parseModelError(err *schemas.DeepIntShieldError, parsed *ParsedError) {
	message := strings.ToLower(err.Error.Message)
...
func parseGenericError(err *schemas.DeepIntShieldError, parsed *ParsedError) {
...
// LogError logs a DeepIntShieldError in a readable format
func LogError(t *testing.T, err *schemas.DeepIntShieldError, context string) {
	if err == nil {
		return
	}
...
// LogErrorConcise logs a DeepIntShieldError in a concise format
func LogErrorConcise(t *testing.T, err *schemas.DeepIntShieldError, context string) {
	if err == nil {
		return
	}
...
func RequireNoError(t *testing.T, err *schemas.DeepIntShieldError, msgAndArgs ...interface{}) {
	if err != nil {
		parsed := ParseDeepIntShieldError(err)
...
func AssertNoError(t *testing.T, err *schemas.DeepIntShieldError, msgAndArgs ...interface{}) bool {
	if err != nil {
		parsed := ParseDeepIntShieldError(err)
		message := "Expected no error"
		if len(msgAndArgs) > 0 {
...
func getCategory(name string) ErrorCategory {
	...
	case CategoryDeepIntShield.Name:
		return CategoryDeepIntShield
	default:
...
func IsRetryableError(err *schemas.DeepIntShieldError) bool {
	if err == nil {
		return false
	}
...
func GetRetryDelay(err *schemas.DeepIntShieldError, attempt int) int {
	if err == nil {
		return 0
	}
