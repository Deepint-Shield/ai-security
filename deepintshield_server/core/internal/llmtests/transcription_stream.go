...
func RunTranscriptionStreamTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("TranscriptionStream", func(t *testing.T) {
		...
		streamRoundTripCases := []struct {
			...
			},
			{
				...
				voiceType:      "secondary",
				format:         "mp3",
				responseFormat: deepintshield.Ptr("json"),
			},
			{
		...
		for _, tc := range streamRoundTripCases {
			t.Run(tc.name, func(t *testing.T) {
				...
				ttsRequest := &schemas.DeepIntShieldSpeechRequest{
				...
				ttsResponse, err := WithSpeechTestRetry(t, ttsSpeechRetryConfig, ttsRetryContext, ttsExpectations, "TranscriptionStream_TTS", func() (*schemas.DeepIntShieldSpeechResponse, *schemas.DeepIntShieldError) {
				...
				streamRequest := &schemas.DeepIntShieldTranscriptionRequest{
					...
					Params: &schemas.TranscriptionParameters{
						Language:       deepintshield.Ptr("en"),
						Format:         deepintshield.Ptr(tc.format),
				...
				lastResponse := &schemas.DeepIntShieldStreamChunk{}
				streamErrors := []string{}
				...
				for {
					...
					case response, ok := <-responseChannel:
						...
						if response.DeepIntShieldError != nil {
							streamErrors = append(streamErrors, FormatErrorConcise(ParseDeepIntShieldError(response.DeepIntShieldError)))
							continue
						}

						if response.DeepIntShieldTranscriptionStreamResponse == nil {
							streamErrors = append(streamErrors, "Stream response missing transcription stream payload")
							continue
						}

						if response.DeepIntShieldTranscriptionStreamResponse != nil {
							lastTokenLatency = response.DeepIntShieldTranscriptionStreamResponse.ExtraFields.Latency
						}

						if response.DeepIntShieldTranscriptionStreamResponse.Text == "" && response.DeepIntShieldTranscriptionStreamResponse.Delta == nil {
							streamErrors = append(streamErrors, "Stream response missing transcription data")
							continue
						}

						chunkIndex := response.DeepIntShieldTranscriptionStreamResponse.ExtraFields.ChunkIndex

						// Log latency for each chunk (can be 0 for inter-chunks)
						t.Logf("📊 Transcription chunk %d latency: %d ms", chunkIndex, response.DeepIntShieldTranscriptionStreamResponse.ExtraFields.Latency)

						// Collect transcription chunks
						transcribeData := response.DeepIntShieldTranscriptionStreamResponse
						if transcribeData.Text != "" {
							t.Logf("✅ Received transcription text chunk %d with latency %d ms: '%s'", chunkIndex, response.DeepIntShieldTranscriptionStreamResponse.ExtraFields.Latency, transcribeData.Text)
						}
						...
						if transcribeData.Delta != nil {
							...
							t.Logf("✅ Received transcription delta chunk %d with latency %d ms: '%s'", chunkIndex, response.DeepIntShieldTranscriptionStreamResponse.ExtraFields.Latency, deltaText)
						}

						// Validate chunk structure
						if response.DeepIntShieldTranscriptionStreamResponse.Type != schemas.TranscriptionStreamResponseTypeDelta {
							t.Logf("⚠️ Unexpected object type in stream: %s", response.DeepIntShieldTranscriptionStreamResponse.Type)
						}
						if response.DeepIntShieldTranscriptionStreamResponse.ExtraFields.ModelRequested != "" && response.DeepIntShieldTranscriptionStreamResponse.ExtraFields.ModelRequested != testConfig.TranscriptionModel {
							t.Logf("⚠️ Unexpected model in stream: %s", response.DeepIntShieldTranscriptionStreamResponse.ExtraFields.ModelRequested)
						}
...
func RunTranscriptionStreamAdvancedTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.TranscriptionStream {
		t.Logf("Transcription streaming not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("TranscriptionStreamAdvanced", func(t *testing.T) {
			...
			for _, lang := range languages {
				t.Run("StreamLang_"+lang, func(t *testing.T) {
					...
					for response := range responseChannel {
						...
						if response.DeepIntShieldError != nil {
							streamErrors = append(streamErrors, fmt.Sprintf("Error in stream for language %s: %s", lang, FormatErrorConcise(ParseDeepIntShieldError(response.DeepIntShieldError))))
							continue
						}
		...
		t.Run("WithCustomPrompt_Streaming", func(t *testing.T) {
			...
			request := &schemas.DeepIntShieldTranscriptionRequest{
				...
				Params: &schemas.TranscriptionParameters{
					Language: deepintshield.Ptr("en"),
					Prompt:   deepintshield.Ptr("This audio contains technical terms, proper nouns, and streaming-related vocabulary."),
				},
				Fallbacks: testConfig.TranscriptionFallbacks,
			}
			...
			for response := range responseChannel {
				...
				if response.DeepIntShieldTranscriptionStreamResponse != nil && response.DeepIntShieldTranscriptionStreamResponse.Text != "" {
					chunkCount++
					chunkText := response.DeepIntShieldTranscriptionStreamResponse.Text
