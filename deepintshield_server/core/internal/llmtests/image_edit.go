...
func RunImageEditTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("ImageEdit", func(t *testing.T) {
		...
		imageEditOperation := func() (*schemas.DeepIntShieldImageGenerationResponse, *schemas.DeepIntShieldError) {
			request := &schemas.DeepIntShieldImageEditRequest{
				...
				Params: &schemas.ImageEditParameters{
					...
					Type: deepintshield.Ptr("inpainting"),
					Mask: maskBytes,
				},
				Fallbacks: testConfig.ImageEditFallbacks,
			}

			response, err := client.ImageEditRequest(schemas.NewDeepIntShieldContext(ctx, schemas.NoDeadline), request)
			if err != nil {
				return nil, err
			}
			if response != nil {
...
func RunImageEditStreamTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("ImageEditStream", func(t *testing.T) {
		...
		validationResult := WithImageGenerationStreamRetry(
			...
			func() (chan *schemas.DeepIntShieldStreamChunk, *schemas.DeepIntShieldError) {
				return client.ImageEditStreamRequest(schemas.NewDeepIntShieldContext(streamCtx, schemas.NoDeadline), request)
			},
