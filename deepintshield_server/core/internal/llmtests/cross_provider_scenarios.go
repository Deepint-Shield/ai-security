...
func NewOpenAIJudge(client *deepintshield.DeepIntShield, judgeModel string, t *testing.T) *OpenAIJudge {
	return &OpenAIJudge{
		client:     client,
...
func (judge *OpenAIJudge) EvaluateResponse(ctx *schemas.DeepIntShieldContext, evaluation EvaluationRequest) (*EvaluationResult, error) {
  ...
  "fatal_issues": ["serious problems"]
	...
	request := &schemas.DeepIntShieldChatRequest{
		...
		Params: &schemas.ChatParameters{
			MaxCompletionTokens: deepintshield.Ptr(600),
...
func NewOpenAIConversationDriver(client *deepintshield.DeepIntShield, driverModel string, t *testing.T) *OpenAIConversationDriver {
...
func (driver *OpenAIConversationDriver) GenerateNextMessage(ctx *schemas.DeepIntShieldContext, request NextMessageRequest) (*GeneratedFollowup, error) {
...
JSON response:
{
  ...
  "test_focus": "what capability this tests"
	...
	llmRequest := &schemas.DeepIntShieldChatRequest{
		Provider: schemas.OpenAI,
		...
		Params: &schemas.ChatParameters{
			MaxCompletionTokens: deepintshield.Ptr(300),
...
func RunCrossProviderScenarioTest(t *testing.T, client *deepintshield.DeepIntShield, ctx *schemas.DeepIntShieldContext, config CrossProviderTestConfig, scenario CrossProviderScenario, useResponsesAPI bool) {
...
func RunCrossProviderConsistencyTest(t *testing.T, client *deepintshield.DeepIntShield, ctx *schemas.DeepIntShieldContext, config CrossProviderTestConfig, useResponsesAPI bool) {
	apiType := "Chat Completions"
	if useResponsesAPI {
		apiType = "Responses API"
	}
		...
		if useResponsesAPI {
			...
			responsesReq := &schemas.DeepIntShieldResponsesRequest{
				...
				Params: &schemas.ResponsesParameters{
					...
					Temperature:     deepintshield.Ptr(0.3),
				},
			}
...
func executeStepWithProvider(t *testing.T, client *deepintshield.DeepIntShield, ctx *schemas.DeepIntShieldContext,
	provider ProviderConfig, history []schemas.ChatMessage, step ScenarioStep, useResponsesAPI bool) (*schemas.DeepIntShieldResponse, *schemas.DeepIntShieldError) {
	...
	if useResponsesAPI {
		...
		request := &schemas.DeepIntShieldResponsesRequest{
			...
			Params: &schemas.ResponsesParameters{
				MaxOutputTokens: deepintshield.Ptr(300),
				Temperature:     deepintshield.Ptr(0.7),
			},
		}
		...
		return &schemas.DeepIntShieldResponse{ResponsesResponse: responsesResponse}, nil
	} else {
		// Use Chat Completions API
		request := &schemas.DeepIntShieldChatRequest{
			Provider: provider.Provider,
		...
		return &schemas.DeepIntShieldResponse{ChatResponse: chatResponse}, nil
	}
}
