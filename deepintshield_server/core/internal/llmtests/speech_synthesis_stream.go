...
func RunSpeechSynthesisStreamTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
		...
		for _, tc := range testCases {
			t.Run(tc.name, func(t *testing.T) {
				...
				responseChannel, err := WithStreamRetry(t, retryConfig, retryContext, func() (chan *schemas.DeepIntShieldStreamChunk, *schemas.DeepIntShieldError) {
					requestCtx := schemas.NewDeepIntShieldContext(ctx, schemas.NoDeadline)
				...
				for response := range responseChannel {
					...
					if response.DeepIntShieldSpeechStreamResponse != nil {
						lastTokenLatency = response.DeepIntShieldSpeechStreamResponse.ExtraFields.Latency
					}
					...
					if response.DeepIntShieldSpeechStreamResponse.Audio == nil {
						streamErrors = append(streamErrors, "Stream response missing audio data")
						continue
					}

					// Log latency for each chunk (can be 0 for inter-chunks)
					t.Logf("📊 Speech chunk %d latency: %d ms", chunkCount+1, response.DeepIntShieldSpeechStreamResponse.ExtraFields.Latency)

					// Collect audio chunks
					if response.DeepIntShieldSpeechStreamResponse.Audio != nil {
						chunkSize := len(response.DeepIntShieldSpeechStreamResponse.Audio)
						if chunkSize == 0 {
							t.Logf("⚠️ Skipping zero-length audio chunk")
							continue
						}
						// Accumulate audio data for codec validation
						audioBuffer.Write(response.DeepIntShieldSpeechStreamResponse.Audio)
						totalBytes += chunkSize
						chunkCount++
						t.Logf("✅ Received audio chunk %d: %d bytes", chunkCount, chunkSize)

						// Validate chunk structure
						if response.DeepIntShieldSpeechStreamResponse.Type != "" && (response.DeepIntShieldSpeechStreamResponse.Type != schemas.SpeechStreamResponseTypeDelta && response.DeepIntShieldSpeechStreamResponse.Type != schemas.SpeechStreamResponseTypeDone) {
							t.Logf("⚠️ Unexpected object type in stream: %s", response.DeepIntShieldSpeechStreamResponse.Type)
						}
						if response.DeepIntShieldSpeechStreamResponse.ExtraFields.ModelRequested != "" && response.DeepIntShieldSpeechStreamResponse.ExtraFields.ModelRequested != testConfig.SpeechSynthesisModel {
							t.Logf("⚠️ Unexpected model in stream: %s", response.DeepIntShieldSpeechStreamResponse.ExtraFields.ModelRequested)
						}
					}
...
func RunSpeechSynthesisStreamAdvancedTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.SpeechSynthesisStream {
		t.Logf("Speech synthesis streaming not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("SpeechSynthesisStreamAdvanced", func(t *testing.T) {
		t.Run("LongText_HDModel_Streaming", func(t *testing.T) {
			...
			for response := range responseChannel {
				...
				if response.DeepIntShieldSpeechStreamResponse != nil && response.DeepIntShieldSpeechStreamResponse.Audio != nil {
					chunkSize := len(response.DeepIntShieldSpeechStreamResponse.Audio)
					if chunkSize == 0 {
				...
				if response.DeepIntShieldSpeechStreamResponse != nil && response.DeepIntShieldSpeechStreamResponse.ExtraFields.ModelRequested != "" && response.DeepIntShieldSpeechStreamResponse.ExtraFields.ModelRequested != testConfig.SpeechSynthesisModel {
					t.Logf("⚠️ Unexpected HD model: %s", response.DeepIntShieldSpeechStreamResponse.ExtraFields.ModelRequested)
				}
			}
			...
			for _, voice := range voices {
				...
				t.Run("StreamingVoice_"+voiceCopy, func(t *testing.T) {
					...
					validationResult := WithSpeechStreamValidationRetry(
						...
						retryConfig,
						retryContext,
						func() (chan *schemas.DeepIntShieldStreamChunk, *schemas.DeepIntShieldError) {							
							accumulatedAudio.Reset() // Reset buffer on retry
							requestCtx := schemas.NewDeepIntShieldContext(ctx, schemas.NoDeadline)
							return client.SpeechStreamRequest(requestCtx, request)
						},
						func(responseChannel chan *schemas.DeepIntShieldStreamChunk) SpeechStreamValidationResult {
							...
							for response := range responseChannel {
								...
								if response.DeepIntShieldError != nil {
									streamErrors = append(streamErrors, fmt.Sprintf("Error in stream for voice %s: %s", voiceCopy, FormatErrorConcise(ParseDeepIntShieldError(response.DeepIntShieldError))))
									continue
								}
								...
								if response.DeepIntShieldSpeechStreamResponse != nil && response.DeepIntShieldSpeechStreamResponse.Audio != nil && len(response.DeepIntShieldSpeechStreamResponse.Audio) > 0 {
									receivedData = true
									// Accumulate audio data for codec validation
									accumulatedAudio.Write(response.DeepIntShieldSpeechStreamResponse.Audio)
									t.Logf("✅ Received data for voice %s: %d bytes", voiceCopy, len(response.DeepIntShieldSpeechStreamResponse.Audio))
								}
							}
