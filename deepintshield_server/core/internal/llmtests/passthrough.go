...
func RunPassthroughExtraParamsTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("PassthroughExtraParams", func(t *testing.T) {
		...
		// Create a DeepIntShield context with passthrough extraParams enabled
		bfCtx := schemas.NewDeepIntShieldContext(ctx, schemas.NoDeadline)
		bfCtx.SetValue(schemas.DeepIntShieldContextKeyPassthroughExtraParams, true)
		bfCtx.SetValue(schemas.DeepIntShieldContextKeySendBackRawRequest, true)
		...
		if response.ExtraFields.RawRequest == nil {
			...
			t.Logf("   Check DeepIntShield logs for the raw request body sent to provider")
