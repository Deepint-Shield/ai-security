			...
			ContentBlocks: []schemas.ChatContentBlock{
				{Type: schemas.ChatContentBlockTypeText, Text: deepintshield.Ptr(text)},
				{
					...
					File: &schemas.ChatInputFile{
						FileURL: deepintshield.Ptr(fileURL),
					},
				},
			},
		},
...
func RunFileURLTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
...
func RunFileURLChatCompletionsTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.FileURL {
		t.Logf("File URL not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("FileURL-ChatCompletions", func(t *testing.T) {
		...
		response, chatError := WithChatTestRetry(t, chatRetryConfig, retryContext, expectations, "FileURL", func() (*schemas.DeepIntShieldChatResponse, *schemas.DeepIntShieldError) {
...
func RunFileURLResponsesTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.FileURL {
		t.Logf("File URL not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("FileURL-Responses", func(t *testing.T) {
		...
		response, responsesError := WithResponsesTestRetry(t, responsesRetryConfig, retryContext, expectations, "FileURL", func() (*schemas.DeepIntShieldResponsesResponse, *schemas.DeepIntShieldError) {
			bfCtx := schemas.NewDeepIntShieldContext(ctx, schemas.NoDeadline)
