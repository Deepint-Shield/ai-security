...
// forwarded through DeepIntShield via the Responses API.
//
...
//  1. The context_management field survives the DeepIntShield request round-trip
//  2. The compact-2026-01-12 beta header is properly sent
//  3. The API accepts the request without error (non-streaming + streaming)
func RunCompactionTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
		...
		t.Run("NonStreaming", func(t *testing.T) {
			...
			request := &schemas.DeepIntShieldResponsesRequest{
				...
				Params: &schemas.ResponsesParameters{
					MaxOutputTokens: deepintshield.Ptr(100),
			...
			for {
				...
				case chunk, ok := <-responseChan:
					...
					if chunk.DeepIntShieldResponsesStreamResponse != nil {
						if chunk.DeepIntShieldResponsesStreamResponse.Type == schemas.ResponsesStreamResponseTypeCreated {
							hasCreated = true
						}
						if chunk.DeepIntShieldResponsesStreamResponse.Type == schemas.ResponsesStreamResponseTypeCompleted {
							hasCompleted = true
						}
						if chunk.DeepIntShieldResponsesStreamResponse.Delta != nil {
							fullContent.WriteString(*chunk.DeepIntShieldResponsesStreamResponse.Delta)
						}
					}
				case <-streamCtx.Done():
