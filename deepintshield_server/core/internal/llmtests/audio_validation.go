...
func SaveAndValidateAudio(t *testing.T, audioData []byte) (string, error) {
	...
	// Create temp file with unique name in deepintshield subdirectory
	tempDir := os.TempDir()
	deepintshieldDir := filepath.Join(tempDir, "deepintshield")
	fileName := fmt.Sprintf("deepintshield_test_speech_%s.%s", uuid.New().String(), detectedFormat)
	filePath := filepath.Join(deepintshieldDir, fileName)

	// Ensure deepintshield subdirectory exists
	if err := os.MkdirAll(deepintshieldDir, 0755); err != nil {
