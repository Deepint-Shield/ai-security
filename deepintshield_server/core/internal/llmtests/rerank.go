...
func BasicRerankExpectations(t *testing.T, rerankResponse *schemas.DeepIntShieldRerankResponse, documents []schemas.RerankDocument) {
	t.Helper()
...
func RunRerankTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("Rerank", func(t *testing.T) {
		...
		rerankResponse, deepintshieldErr := client.RerankRequest(bfCtx, request)

		if deepintshieldErr != nil {
			t.Fatalf("❌ Rerank request failed: %v", GetErrorMessage(deepintshieldErr))
		}
