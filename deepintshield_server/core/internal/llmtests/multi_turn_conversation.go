...
func RunMultiTurnConversationTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("MultiTurnConversation", func(t *testing.T) {
		...
		firstRequest := &schemas.DeepIntShieldChatRequest{
		...
		response1, deepintshieldErr := WithChatTestRetry(t, chatRetryConfig1, retryContext1, expectations1, "MultiTurnConversation_Step1", func() (*schemas.DeepIntShieldChatResponse, *schemas.DeepIntShieldError) {
		...
		if deepintshieldErr != nil {
			t.Fatalf("❌ MultiTurnConversation_Step1 request failed after retries: %v", GetErrorMessage(deepintshieldErr))
		}
		...
		secondRequest := &schemas.DeepIntShieldChatRequest{
			Provider: testConfig.Provider,
			Model:    testConfig.ChatModel,
			Input:    messages2,
			Params: &schemas.ChatParameters{
	...
	response2, deepintshieldErr := WithChatTestRetry(t, chatRetryConfig2, retryContext2, expectations2, "MultiTurnConversation_Step2", func() (*schemas.DeepIntShieldChatResponse, *schemas.DeepIntShieldError) {
		bfCtx := schemas.NewDeepIntShieldContext(ctx, schemas.NoDeadline)
		return client.ChatCompletionRequest(bfCtx, secondRequest)
	})

	if deepintshieldErr != nil {
		t.Fatalf("❌ MultiTurnConversation_Step2 request failed after retries: %v", GetErrorMessage(deepintshieldErr))
	}
