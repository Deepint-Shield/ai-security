	...
	return []schemas.ChatTool{
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: deepintshield.Ptr("Retrieve account information for a given account ID"),
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: deepintshield.Ptr("Calculate the cost for cloud resource usage based on service type and quantity"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: deepintshield.Ptr("Check the current status and health of the API service"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: deepintshield.Ptr("Create a new support ticket for a customer issue"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: deepintshield.Ptr("Retrieve documentation for a specific service or feature"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: deepintshield.Ptr("Get a list of available cloud regions for deployment"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
						schemas.KV("service_type", map[string]interface{}{
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: deepintshield.Ptr("Estimate the monthly cost for a cloud deployment configuration"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: deepintshield.Ptr("Validate an API key and return its permissions and status"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: deepintshield.Ptr("Retrieve usage analytics and metrics for an account"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
						schemas.KV("account_id", map[string]interface{}{
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: deepintshield.Ptr("Check compliance status and certifications for security and regulatory requirements"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
...
// content blocks survives the DeepIntShield round-trip (Anthropic format -> DeepIntShield ResponsesMessage -> Provider format).
...
func RunPromptCachingToolBlocksTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("PromptCachingToolBlocks", func(t *testing.T) {
		...
		systemMsg := schemas.ResponsesMessage{
			Type: deepintshield.Ptr(schemas.ResponsesMessageTypeMessage),
			Role: deepintshield.Ptr(schemas.ResponsesInputMessageRoleSystem),
			Content: &schemas.ResponsesMessageContent{
				ContentBlocks: []schemas.ResponsesMessageContentBlock{
					{
						Type:         schemas.ResponsesInputMessageContentBlockTypeText,
						Text:         deepintshield.Ptr(longSharedPrefix),
		...
		userMsg1 := schemas.ResponsesMessage{
			...
			Content: &schemas.ResponsesMessageContent{
				ContentStr: deepintshield.Ptr("What's the weather in San Francisco?"),
			},
		}

		// Assistant responds with a tool call (function_call with cache_control)
		toolCallMsg := schemas.ResponsesMessage{
			Type:         deepintshield.Ptr(schemas.ResponsesMessageTypeFunctionCall),
			Status:       deepintshield.Ptr("completed"),
			CacheControl: cacheControl,
			ResponsesToolMessage: &schemas.ResponsesToolMessage{
				CallID:    deepintshield.Ptr("call_weather_001"),
				Name:      deepintshield.Ptr("weather"),
				Arguments: deepintshield.Ptr(`{"location":"San Francisco"}`),
			},
		}

		// Tool result (function_call_output with cache_control)
		toolResultMsg := schemas.ResponsesMessage{
			Type:         deepintshield.Ptr(schemas.ResponsesMessageTypeFunctionCallOutput),
			CacheControl: cacheControl,
			ResponsesToolMessage: &schemas.ResponsesToolMessage{
				CallID: deepintshield.Ptr("call_weather_001"),
				Output: &schemas.ResponsesToolMessageOutputStruct{
					ResponsesToolCallOutputStr: deepintshield.Ptr(`{"temperature": 18, "unit": "celsius", "condition": "partly cloudy", "humidity": 72}`),
				},
			},
		}
		...
		userMsg2 := schemas.ResponsesMessage{
			...
			Role: deepintshield.Ptr(schemas.ResponsesInputMessageRoleUser),
			Content: &schemas.ResponsesMessageContent{
				ContentStr: deepintshield.Ptr("Summarize the weather information you received."),
			},
		}
		...
		responsesReq := &schemas.DeepIntShieldResponsesRequest{
			...
			Params: &schemas.ResponsesParameters{
				...
				MaxOutputTokens: deepintshield.Ptr(200),
			},
		}

		// Enable raw request capture so we can inspect the outgoing provider request
		rawCtx := context.WithValue(ctx, schemas.DeepIntShieldContextKeySendBackRawRequest, true)
		...
		operation := func() (*schemas.DeepIntShieldResponsesResponse, *schemas.DeepIntShieldError) {
			bfCtx := schemas.NewDeepIntShieldContext(rawCtx, schemas.NoDeadline)
		...
		require.NotNil(t, rawReq, "Raw request should be present (DeepIntShieldContextKeySendBackRawRequest was set)")
...
// Vertex caching bug where DeepIntShield's key reordering in tool_use input fields caused
...
func RunPromptCachingMultipleToolCallsTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.PromptCaching {
	...
	t.Run("PromptCachingMultipleToolCalls", func(t *testing.T) {
		...
		makeUserMsg := func(text string) schemas.ResponsesMessage {
			return schemas.ResponsesMessage{
				...
				Role: deepintshield.Ptr(schemas.ResponsesInputMessageRoleUser),
				Content: &schemas.ResponsesMessageContent{
					ContentStr: deepintshield.Ptr(text),
				},
			}
		}
		...
		makeAssistantMsg := func(text string) schemas.ResponsesMessage {
			return schemas.ResponsesMessage{
				Type: deepintshield.Ptr(schemas.ResponsesMessageTypeMessage),
				Role: deepintshield.Ptr(schemas.ResponsesInputMessageRoleAssistant),
				Content: &schemas.ResponsesMessageContent{
					ContentStr: deepintshield.Ptr(text),
				},
			}
		...
		makeToolCall := func(callID, name, args string, withCacheControl bool) schemas.ResponsesMessage {
			msg := schemas.ResponsesMessage{
				...
				Status: deepintshield.Ptr("completed"),
				ResponsesToolMessage: &schemas.ResponsesToolMessage{
					CallID:    deepintshield.Ptr(callID),
					Name:      deepintshield.Ptr(name),
					Arguments: deepintshield.Ptr(args),
				},
			}
			if withCacheControl {
		...
		makeToolResult := func(callID, output string, withCacheControl bool) schemas.ResponsesMessage {
			msg := schemas.ResponsesMessage{
				...
				ResponsesToolMessage: &schemas.ResponsesToolMessage{
					CallID: deepintshield.Ptr(callID),
					Output: &schemas.ResponsesToolMessageOutputStruct{
						ResponsesToolCallOutputStr: deepintshield.Ptr(output),
					},
				},
			}
			if withCacheControl {
		...
		// If DeepIntShield re-orders these keys, the cache prefix diverges at this point.
...
func RunPromptCachingTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
		...
		for i, query := range testQueries {
			t.Run(query.name, func(t *testing.T) {
				userMessage := schemas.ChatMessage{
					...
					Content: &schemas.ChatMessageContent{
						ContentStr: deepintshield.Ptr(query.message),
					},
				}
				...
				chatReq := &schemas.DeepIntShieldChatRequest{
					...
					Params: &schemas.ChatParameters{
						...
						ToolChoice: &schemas.ChatToolChoice{
							ChatToolChoiceStr: deepintshield.Ptr("auto"),
						},
					},
...
func RunPromptCachingMultiTurnTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.SimpleChat {
		...
		for i, query := range queries {
			...
			t.Run(turnName, func(t *testing.T) {
				userMessage := schemas.ChatMessage{
					Role: schemas.ChatMessageRoleUser,
					Content: &schemas.ChatMessageContent{
						ContentStr: deepintshield.Ptr(query),
					},
				}
				...
				chatReq := &schemas.DeepIntShieldChatRequest{
					...
					Params: &schemas.ChatParameters{
						Tools: tools,
						ToolChoice: &schemas.ChatToolChoice{
							ChatToolChoiceStr: deepintshield.Ptr("none"),
						},
					},
					Fallbacks: testConfig.Fallbacks,
				}
