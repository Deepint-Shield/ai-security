...
func RunToolCallsTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("ToolCalls", func(t *testing.T) {
		...
		validateLocationInChatToolCalls := func(response *schemas.DeepIntShieldChatResponse, apiName string) {
			toolCalls := ExtractChatToolCalls(response)
			validateLocationInToolCalls(t, toolCalls, apiName)
		}

		validateLocationInResponsesToolCalls := func(response *schemas.DeepIntShieldResponsesResponse, apiName string) {
...
func RunToolCallsWithEmptyPropertiesTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.ToolCalls {
		t.Logf("Tool calls not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("ToolCallsWithEmptyProperties", func(t *testing.T) {
			...
			chatReq := &schemas.DeepIntShieldChatRequest{
				...
				Params: &schemas.ChatParameters{
					...
					ToolChoice: &schemas.ChatToolChoice{
						ChatToolChoiceStr: deepintshield.Ptr("required"),
					},
				},
				Fallbacks: testConfig.Fallbacks,
			}
			...
			responsesReq := &schemas.DeepIntShieldResponsesRequest{
				...
				Params: &schemas.ResponsesParameters{
					...
					ToolChoice: &schemas.ResponsesToolChoice{
						ResponsesToolChoiceStr: deepintshield.Ptr("required"),
					},
				},
			}
			return client.ResponsesRequest(bfCtx, responsesReq)
		...
		validatePingToolCall := func(response *schemas.DeepIntShieldChatResponse, apiName string) {
			toolCalls := ExtractChatToolCalls(response)
		...
		validatePingResponsesToolCall := func(response *schemas.DeepIntShieldResponsesResponse, apiName string) {
			toolCalls := ExtractResponsesToolCalls(response)
			require.True(t, len(toolCalls) > 0, "%s API should have tool calls", apiName)
			pingFound := false
			for _, toolCall := range toolCalls {
...
func RunToolCallsWithNilPropertiesTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.ToolCalls {
		t.Logf("Tool calls not supported for provider %s", testConfig.Provider)
		return
	}
