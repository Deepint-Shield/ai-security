...
func RunMultipleImagesTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("MultipleImages", func(t *testing.T) {
		...
		response, deepintshieldError := WithChatTestRetry(t, chatRetryConfig, retryContext, expectations, "MultipleImages", func() (*schemas.DeepIntShieldChatResponse, *schemas.DeepIntShieldError) {
		...
		if deepintshieldError != nil {
			t.Fatalf("❌ Multiple images request failed after retries: %v", GetErrorMessage(deepintshieldError))
		}
