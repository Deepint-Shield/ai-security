...
func RunResponsesReasoningTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("ResponsesReasoning", func(t *testing.T) {
		...
		responsesReq := &schemas.DeepIntShieldResponsesRequest{
			...
			Params: &schemas.ResponsesParameters{
				...
				MaxOutputTokens: deepintshield.Ptr(25000),
				// Configure reasoning-specific parameters
				Reasoning: &schemas.ResponsesParametersReasoning{
					Effort: deepintshield.Ptr("high"), // High effort for complex reasoning
					// Summary: deepintshield.Ptr("detailed"), // Detailed summary of reasoning process
				},
		...
		response, responsesError := WithResponsesTestRetry(t, responsesRetryConfig, retryContext, expectations, "Reasoning", func() (*schemas.DeepIntShieldResponsesResponse, *schemas.DeepIntShieldError) {
...
func validateResponsesAPIReasoning(t *testing.T, response *schemas.DeepIntShieldResponsesResponse) bool {
...
func RunChatCompletionReasoningTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.Reasoning {
		t.Logf("⏭️ Reasoning not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("ChatCompletionReasoning", func(t *testing.T) {
		...
		chatReq := &schemas.DeepIntShieldChatRequest{
			...
			Params: &schemas.ChatParameters{
				...
				Reasoning: &schemas.ChatReasoning{
					Effort:    deepintshield.Ptr("high"), // High effort for complex reasoning
					MaxTokens: deepintshield.Ptr(1500),   // Maximum tokens for reasoning output
				},
			},
			Fallbacks: testConfig.Fallbacks,
		}
		...
		response, chatError := WithChatTestRetry(t, chatRetryConfig, retryContext, expectations, "Reasoning", func() (*schemas.DeepIntShieldChatResponse, *schemas.DeepIntShieldError) {
			bfCtx := schemas.NewDeepIntShieldContext(ctx, schemas.NoDeadline)
...
// Returns true if reasoning indicators are found
func validateChatCompletionReasoning(t *testing.T, response *schemas.DeepIntShieldChatResponse) bool {
...
func RunMultiTurnReasoningTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.Reasoning {
		t.Logf("⏭️ Reasoning not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("MultiTurnReasoning", func(t *testing.T) {
		...
		firstResponse, chatError := WithChatTestRetry(t, chatRetryConfig, retryContext, expectations, "MultiTurnReasoning_Step1", func() (*schemas.DeepIntShieldChatResponse, *schemas.DeepIntShieldError) {
			bfCtx := schemas.NewDeepIntShieldContext(ctx, schemas.NoDeadline)
			return client.ChatCompletionRequest(bfCtx, chatReq)
		})
		...
		secondResponse, chatError2 := WithChatTestRetry(t, chatRetryConfig, retryContext2, expectations, "MultiTurnReasoning_Step2", func() (*schemas.DeepIntShieldChatResponse, *schemas.DeepIntShieldError) {
			bfCtx := schemas.NewDeepIntShieldContext(ctx, schemas.NoDeadline)
