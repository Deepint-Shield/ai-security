...
// basePassthroughChatRequest returns a minimal DeepIntShieldChatRequest suitable for
// conversion into a provider-native passthrough body.
func basePassthroughChatRequest(model string) *schemas.DeepIntShieldChatRequest {
...
// buildPassthroughChatReq converts a minimal DeepIntShieldChatRequest into the
...
func buildPassthroughChatReq(provider schemas.ModelProvider, model string, stream bool) (passthroughChatReq, bool) {
	...
	case schemas.OpenAI:
		...
		if stream {
			nativeReq.Stream = deepintshield.Ptr(true)
		}
...
// RunPassthroughAPITest exercises DeepIntShield's raw HTTP passthrough API for the
...
func RunPassthroughAPITest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("PassthroughAPI", func(t *testing.T) {
		t.Run("NonStream", func(t *testing.T) {
			...
			resp, deepintshieldErr := client.Passthrough(bfCtx, testConfig.Provider, &schemas.DeepIntShieldPassthroughRequest{
				Method: "POST",
				Path:   req.path,
				Body:   req.body,
			...
			if deepintshieldErr != nil {
				t.Fatalf("❌ Passthrough request failed: %s", GetErrorMessage(deepintshieldErr))
			}
			if resp == nil {
		...
		t.Run("Stream", func(t *testing.T) {
			...
			ch, deepintshieldErr := client.PassthroughStream(bfCtx, testConfig.Provider, &schemas.DeepIntShieldPassthroughRequest{
				Method:   "POST",
				Path:     req.path,
				Body:     req.body,
				RawQuery: req.query,
			...
			if deepintshieldErr != nil {
				t.Fatalf("❌ PassthroughStream failed: %s", GetErrorMessage(deepintshieldErr))
			}
			if ch == nil {
			...
			for chunk := range ch {
				...
				if chunk.DeepIntShieldError != nil {
					t.Fatalf("❌ Stream chunk contained error: %s", GetErrorMessage(chunk.DeepIntShieldError))
				}
				if chunk.DeepIntShieldPassthroughResponse != nil {
					totalBytes += len(chunk.DeepIntShieldPassthroughResponse.Body)
					if len(chunk.DeepIntShieldPassthroughResponse.Body) > 0 {
						chunkCount++
					}
				}
			}
