...
func RunImageVariationTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("ImageVariation", func(t *testing.T) {
		...
		imageVariationOperation := func() (*schemas.DeepIntShieldImageGenerationResponse, *schemas.DeepIntShieldError) {
			request := &schemas.DeepIntShieldImageVariationRequest{
				...
				Params: &schemas.ImageVariationParameters{
					...
					N:    deepintshield.Ptr(2), // Generate 2 variations
				},
				Fallbacks: testConfig.ImageVariationFallbacks,
			}

			response, err := client.ImageVariationRequest(schemas.NewDeepIntShieldContext(ctx, schemas.NoDeadline), request)
			if err != nil {
				return nil, err
			}
			if response != nil {
...
func RunImageVariationStreamTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
