...
func RunWebSearchToolTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
		...
		webSearchTool := &schemas.ResponsesTool{
			...
			ResponsesToolWebSearch: &schemas.ResponsesToolWebSearch{
				UserLocation: &schemas.ResponsesToolWebSearchUserLocation{
					Type:    deepintshield.Ptr("approximate"),
					Country: deepintshield.Ptr("US"),
					City:    deepintshield.Ptr("New York"),
				},
			},
		}
...
func RunWebSearchToolStreamTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.WebSearchTool {
		t.Logf("Web search tool not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("WebSearchToolStream", func(t *testing.T) {
		...
		webSearchTool := &schemas.ResponsesTool{
			...
			ResponsesToolWebSearch: &schemas.ResponsesToolWebSearch{
				UserLocation: &schemas.ResponsesToolWebSearchUserLocation{
					Type:     deepintshield.Ptr("approximate"),
					Country:  deepintshield.Ptr("US"),
					City:     deepintshield.Ptr("San Francisco"),
					Region:   deepintshield.Ptr("California"),
					Timezone: deepintshield.Ptr("America/Los_Angeles"),
				},
			},
		}
		...
		request := &schemas.DeepIntShieldResponsesRequest{
			...
			Params: &schemas.ResponsesParameters{
				Tools:           []schemas.ResponsesTool{*webSearchTool},
				MaxOutputTokens: deepintshield.Ptr(1500),
			},
			Fallbacks: testConfig.Fallbacks,
		}
		...
		validationResult := WithResponsesStreamValidationRetry(t, retryConfig, retryContext,
			...
			func(responseChannel chan *schemas.DeepIntShieldStreamChunk) ResponsesStreamValidationResult {
				...
				for {
					...
					case stream, ok := <-responseChannel:
						...
						if stream.DeepIntShieldResponsesStreamResponse != nil {
							streamType := stream.DeepIntShieldResponsesStreamResponse.Type

							// Check for output_item.added with web_search_call
							if streamType == schemas.ResponsesStreamResponseTypeOutputItemAdded {
								if stream.DeepIntShieldResponsesStreamResponse.Item != nil {
									if stream.DeepIntShieldResponsesStreamResponse.Item.Type != nil &&
										*stream.DeepIntShieldResponsesStreamResponse.Item.Type == schemas.ResponsesMessageTypeWebSearchCall {
										hasWebSearchCall = true
										t.Logf("✅ Found web_search_call in streaming event: %s", streamType)

										// Extract query and sources if available
										if stream.DeepIntShieldResponsesStreamResponse.Item.ResponsesToolMessage != nil &&
											stream.DeepIntShieldResponsesStreamResponse.Item.ResponsesToolMessage.Action != nil {
											action := stream.DeepIntShieldResponsesStreamResponse.Item.ResponsesToolMessage.Action
											if action.ResponsesWebSearchToolCallAction != nil {
							...
							if streamType == schemas.ResponsesStreamResponseTypeOutputTextDelta {
								if stream.DeepIntShieldResponsesStreamResponse.Delta != nil && *stream.DeepIntShieldResponsesStreamResponse.Delta != "" {
									hasMessageContent = true
									t.Logf("✅ Found message text delta: %s", *stream.DeepIntShieldResponsesStreamResponse.Delta)
								}
							}
						}
...
func RunWebSearchToolWithDomainsTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.WebSearchTool {
		t.Logf("Web search tool not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("WebSearchToolWithDomains", func(t *testing.T) {
		...
		responsesOperation := func() (*schemas.DeepIntShieldResponsesResponse, *schemas.DeepIntShieldError) {
			...
			responsesReq := &schemas.DeepIntShieldResponsesRequest{
				...
				Params: &schemas.ResponsesParameters{
					Tools:           []schemas.ResponsesTool{*webSearchTool},
					MaxOutputTokens: deepintshield.Ptr(1200),
				},
				Fallbacks: testConfig.Fallbacks,
			}
...
func RunWebSearchToolContextSizesTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.WebSearchTool {
		t.Logf("Web search tool not supported for provider %s", testConfig.Provider)
		return
	}
...
func RunWebSearchToolMultiTurnTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.WebSearchTool {
		t.Logf("Web search tool not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("WebSearchToolMultiTurn", func(t *testing.T) {
		...
		firstOperation := func() (*schemas.DeepIntShieldResponsesResponse, *schemas.DeepIntShieldError) {
			bfCtx := schemas.NewDeepIntShieldContext(ctx, schemas.NoDeadline)
			responsesReq := &schemas.DeepIntShieldResponsesRequest{
				Provider: testConfig.Provider,
				Model:    testConfig.ChatModel,
		...
		secondOperation := func() (*schemas.DeepIntShieldResponsesResponse, *schemas.DeepIntShieldError) {
			bfCtx := schemas.NewDeepIntShieldContext(ctx, schemas.NoDeadline)
			responsesReq := &schemas.DeepIntShieldResponsesRequest{
				Provider: testConfig.Provider,
				Model:    testConfig.ChatModel,
...
func RunWebSearchToolMaxUsesTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.WebSearchTool {
		t.Logf("Web search tool not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("WebSearchToolMaxUses", func(t *testing.T) {
		...
		responsesOperation := func() (*schemas.DeepIntShieldResponsesResponse, *schemas.DeepIntShieldError) {
			...
			responsesReq := &schemas.DeepIntShieldResponsesRequest{
				...
				Params: &schemas.ResponsesParameters{
					Tools:           []schemas.ResponsesTool{*webSearchTool},
					MaxOutputTokens: deepintshield.Ptr(2000),
				},
				Fallbacks: testConfig.Fallbacks,
			}
