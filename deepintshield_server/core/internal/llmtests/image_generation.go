...
func RunImageGenerationTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("ImageGeneration", func(t *testing.T) {
		...
		imageGenerationOperation := func() (*schemas.DeepIntShieldImageGenerationResponse, *schemas.DeepIntShieldError) {
			request := &schemas.DeepIntShieldImageGenerationRequest{
				...
				Params: &schemas.ImageGenerationParameters{
					Size: deepintshield.Ptr("1024x1024"),
					N:    deepintshield.Ptr(1),
				},
				Fallbacks: testConfig.ImageGenerationFallbacks,
			}

			response, err := client.ImageGenerationRequest(schemas.NewDeepIntShieldContext(ctx, schemas.NoDeadline), request)
			if err != nil {
				return nil, err
			}
			if response != nil {
...
func RunImageGenerationStreamTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("ImageGenerationStream", func(t *testing.T) {
		...
		validationResult := WithImageGenerationStreamRetry(
			...
			func() (chan *schemas.DeepIntShieldStreamChunk, *schemas.DeepIntShieldError) {
				return client.ImageGenerationStreamRequest(schemas.NewDeepIntShieldContext(streamCtx, schemas.NoDeadline), request)
			},
			func(responseChannel chan *schemas.DeepIntShieldStreamChunk) ImageGenerationStreamValidationResult {
				...
				for {
					...
					case response, ok := <-responseChannel:
						...
						if response.DeepIntShieldError != nil {
							streamErrors = append(streamErrors, fmt.Sprintf("Error in stream: %s", GetErrorMessage(response.DeepIntShieldError)))
							continue
						}

						if response.DeepIntShieldImageGenerationStreamResponse != nil {
							receivedData = true
							imgResp := response.DeepIntShieldImageGenerationStreamResponse
