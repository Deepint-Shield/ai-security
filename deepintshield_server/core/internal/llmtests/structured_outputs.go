...
func RunStructuredOutputChatTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
...
func testStructuredOutputChatWithValue(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig, expectValue bool) {
	...
	chatOperation := func() (*schemas.DeepIntShieldChatResponse, *schemas.DeepIntShieldError) {
		...
		reqCtx := schemas.NewDeepIntShieldContext(ctx, schemas.NoDeadline)
		if strings.Contains(strings.ToLower(testConfig.ChatModel), "claude") && testConfig.Provider != schemas.Vertex {
			extraHeaders := map[string][]string{
				"anthropic-beta": {"structured-outputs-2025-11-13"},
			}
			reqCtx.SetValue(schemas.DeepIntShieldContextKeyExtraHeaders, extraHeaders)
		}
		...
		chatReq := &schemas.DeepIntShieldChatRequest{
			...
			Params: &schemas.ChatParameters{
				MaxCompletionTokens: deepintshield.Ptr(5000),
...
func RunStructuredOutputChatStreamTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
		...
		for {
			...
			case response, ok := <-responseChannel:
				...
				if response.DeepIntShieldChatResponse != nil {
					if len(response.DeepIntShieldChatResponse.Choices) > 0 {
						choice := response.DeepIntShieldChatResponse.Choices[0]
...
func RunStructuredOutputResponsesTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.StructuredOutputs {
		t.Logf("Structured outputs not supported for provider %s", testConfig.Provider)
		return
	}
		...
		responsesOperation := func() (*schemas.DeepIntShieldResponsesResponse, *schemas.DeepIntShieldError) {
			...
			responsesReq := &schemas.DeepIntShieldResponsesRequest{
				...
				Params: &schemas.ResponsesParameters{
					MaxOutputTokens: deepintshield.Ptr(5000),
					Text: &schemas.ResponsesTextConfig{
						Format: &schemas.ResponsesTextConfigFormat{
							Type: "json_schema",
							Name: deepintshield.Ptr("decision_schema"),
...
func RunStructuredOutputResponsesStreamTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.StructuredOutputs || !testConfig.Scenarios.CompletionStream {
		t.Logf("Structured outputs streaming not supported for provider %s", testConfig.Provider)
		return
	}
