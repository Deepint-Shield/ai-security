...
func RunListModelsTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("ListModels", func(t *testing.T) {
		...
		response, deepintshieldErr := WithListModelsTestRetry(t, listModelsRetryConfig, retryContext, expectations, "ListModels", func() (*schemas.DeepIntShieldListModelsResponse, *schemas.DeepIntShieldError) {
		...
		if deepintshieldErr != nil {
			t.Fatalf("❌ List models request failed after retries: %v", GetErrorMessage(deepintshieldErr))
		}
...
func RunListModelsResponseMarshalTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.ListModels {
		t.Logf("List models not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("ListModelsResponseMarshal", func(t *testing.T) {
		...
		response, deepintshieldErr := WithListModelsTestRetry(t, listModelsRetryConfig, retryContext, expectations, "ListModelsResponseMarshal", func() (*schemas.DeepIntShieldListModelsResponse, *schemas.DeepIntShieldError) {
			bfCtx := schemas.NewDeepIntShieldContext(ctx, schemas.NoDeadline)
			return client.ListModelsRequest(bfCtx, request)
		})
...
// RunListModelsErrorMarshalTest verifies that the KeyStatus ↔ DeepIntShieldError circular
...
func RunListModelsErrorMarshalTest(t *testing.T, _ *deepintshield.DeepIntShield, _ context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.ListModels {
		t.Logf("List models not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("ListModelsErrorMarshal", func(t *testing.T) {
		...
		// Create the cycle: DeepIntShieldError → ExtraFields.KeyStatuses → KeyStatus → Error → DeepIntShieldError
		deepintshieldErr.ExtraFields.KeyStatuses = []schemas.KeyStatus{keyStatus}

		// Marshal the DeepIntShieldError (top-level, contains the cycle via KeyStatuses)
		errData, err := schemas.Marshal(deepintshieldErr)
		if err != nil {
			t.Fatalf("❌ Failed to marshal DeepIntShieldError with circular KeyStatuses: %v", err)
		}
		t.Logf("✅ DeepIntShieldError with circular KeyStatuses marshaled successfully (%d bytes)", len(errData))
...
func RunListModelsPaginationTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.ListModels {
		t.Logf("List models not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("ListModelsPagination", func(t *testing.T) {
		...
		response, deepintshieldErr := WithListModelsTestRetry(t, listModelsRetryConfig, retryContext, expectations, "ListModelsPagination", func() (*schemas.DeepIntShieldListModelsResponse, *schemas.DeepIntShieldError) {
			bfCtx := schemas.NewDeepIntShieldContext(ctx, schemas.NoDeadline)
			return client.ListModelsRequest(bfCtx, request)
		})

		if deepintshieldErr != nil {
			t.Fatalf("❌ List models pagination request failed after retries: %v", GetErrorMessage(deepintshieldErr))
		}
		...
		if response.NextPageToken != "" {
			...
			nextPageRequest := &schemas.DeepIntShieldListModelsRequest{
				Provider:  testConfig.Provider,
				PageSize:  pageSize,
			...
			nextPageResponse, nextPageErr := WithListModelsTestRetry(t, listModelsRetryConfig, nextPageRetryContext, expectations, "ListModelsPagination_NextPage", func() (*schemas.DeepIntShieldListModelsResponse, *schemas.DeepIntShieldError) {
				bfCtx := schemas.NewDeepIntShieldContext(ctx, schemas.NoDeadline)
