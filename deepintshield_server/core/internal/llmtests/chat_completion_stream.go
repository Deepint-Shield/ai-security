...
func RunChatCompletionStreamTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("ChatCompletionStream", func(t *testing.T) {
		...
		request := &schemas.DeepIntShieldChatRequest{
			...
			Params: &schemas.ChatParameters{
				MaxCompletionTokens: deepintshield.Ptr(150),
			},
		...
		for {
			...
			case response, ok := <-responseChannel:
				...
				if response.DeepIntShieldChatResponse != nil {
					if response.DeepIntShieldChatResponse.ExtraFields.Provider != testConfig.Provider {
						t.Logf("⚠️ Warning: Provider mismatch - expected %s, got %s", testConfig.Provider, response.DeepIntShieldChatResponse.ExtraFields.Provider)
					}
					if response.DeepIntShieldChatResponse.ID == "" {
						t.Logf("⚠️ Warning: Response ID is empty")
					}

					// Log latency for each chunk (can be 0 for inter-chunks)
					t.Logf("📊 Chunk %d latency: %d ms", responseCount+1, response.DeepIntShieldChatResponse.ExtraFields.Latency)

					// Process each choice in the response
					for _, choice := range response.DeepIntShieldChatResponse.Choices {
	...
	streamComplete:
		...
		consolidatedResponse := &schemas.DeepIntShieldChatResponse{
		...
		if lastResponse != nil && lastResponse.DeepIntShieldChatResponse != nil {
			consolidatedResponse.Usage = lastResponse.DeepIntShieldChatResponse.Usage
			consolidatedResponse.Model = lastResponse.DeepIntShieldChatResponse.Model
			consolidatedResponse.ID = lastResponse.DeepIntShieldChatResponse.ID
			consolidatedResponse.Created = lastResponse.DeepIntShieldChatResponse.Created

			// Copy finish reason from last choice if available
			if len(lastResponse.DeepIntShieldChatResponse.Choices) > 0 && lastResponse.DeepIntShieldChatResponse.Choices[0].FinishReason != nil {
				consolidatedResponse.Choices[0].FinishReason = lastResponse.DeepIntShieldChatResponse.Choices[0].FinishReason
			}
			consolidatedResponse.ExtraFields = lastResponse.DeepIntShieldChatResponse.ExtraFields
		}
	...
	if testConfig.Scenarios.ToolCalls {
		t.Run("ChatCompletionStreamWithTools", func(t *testing.T) {
			...
			validationResult := WithChatStreamValidationRetry(
				...
				func(responseChannel chan *schemas.DeepIntShieldStreamChunk) ChatStreamValidationResult {
					...
					for {
						...
						case response, ok := <-responseChannel:
							...
							if response == nil || response.DeepIntShieldChatResponse == nil {
							...
							if response.DeepIntShieldChatResponse.Choices != nil {
								for _, choice := range response.DeepIntShieldChatResponse.Choices {
									if choice.ChatStreamResponseChoice != nil && choice.ChatStreamResponseChoice.Delta != nil {
										delta := choice.ChatStreamResponseChoice.Delta
	...
	if testConfig.Scenarios.Reasoning && testConfig.ReasoningModel != "" {
		t.Run("ChatCompletionStreamWithReasoning", func(t *testing.T) {
			...
			request := &schemas.DeepIntShieldChatRequest{
				...
				Input:    messages,
				Params: &schemas.ChatParameters{
					MaxCompletionTokens: deepintshield.Ptr(1800),
					Reasoning: &schemas.ChatReasoning{
						Effort:    deepintshield.Ptr("high"),
						MaxTokens: deepintshield.Ptr(1500),
					},
				},
				Fallbacks: testConfig.Fallbacks,
			}
			...
			for {
				...
				case response, ok := <-responseChannel:
					...
					if response.DeepIntShieldChatResponse != nil {
						chatResp := response.DeepIntShieldChatResponse
								...
								if choice.ChatStreamResponseChoice != nil && choice.ChatStreamResponseChoice.Delta != nil {
									...
									if len(delta.ReasoningDetails) > 0 {
										...
										for _, detail := range delta.ReasoningDetails {
											...
											switch detail.Type {
											case schemas.DeepIntShieldReasoningDetailsTypeText:
											...
											case schemas.DeepIntShieldReasoningDetailsTypeSummary:
												if detail.Summary != nil {
													t.Logf("    Summary length: %d", len(*detail.Summary))
												}
											case schemas.DeepIntShieldReasoningDetailsTypeEncrypted:
												if detail.Data != nil {
		...
		t.Run("ChatCompletionStreamWithReasoningValidated", func(t *testing.T) {
			...
			if testConfig.Provider == schemas.Cerebras {
				problemPrompt = "Hello how are you, can you search hackernews news regarding open-source AI for me? use your tools for this"
			}
