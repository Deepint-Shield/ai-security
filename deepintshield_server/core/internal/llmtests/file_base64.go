			...
			ContentBlocks: []schemas.ChatContentBlock{
				{Type: schemas.ChatContentBlockTypeText, Text: deepintshield.Ptr(text)},
				{
					...
					File: &schemas.ChatInputFile{
						FileData: deepintshield.Ptr(documentBase64),
						Filename: deepintshield.Ptr("test_document.pdf"),
					},
				},
			},
		},
...
func RunFileBase64Test(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
...
func RunFileBase64ChatCompletionsTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.FileBase64 {
		t.Logf("File base64 not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("FileBase64-ChatCompletions", func(t *testing.T) {
		...
		response, chatError := WithChatTestRetry(t, chatRetryConfig, retryContext, expectations, "FileBase64", func() (*schemas.DeepIntShieldChatResponse, *schemas.DeepIntShieldError) {
			...
			chatReq := &schemas.DeepIntShieldChatRequest{
				...
				Params: &schemas.ChatParameters{
					MaxCompletionTokens: deepintshield.Ptr(500),
				},
...
func RunFileBase64ResponsesTest(t *testing.T, client *deepintshield.DeepIntShield, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.FileBase64 {
		t.Logf("File base64 not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("FileBase64-Responses", func(t *testing.T) {
		...
		response, responsesError := WithResponsesTestRetry(t, responsesRetryConfig, retryContext, expectations, "FileBase64", func() (*schemas.DeepIntShieldResponsesResponse, *schemas.DeepIntShieldError) {
			bfCtx := schemas.NewDeepIntShieldContext(ctx, schemas.NoDeadline)
			responsesReq := &schemas.DeepIntShieldResponsesRequest{
				...
				Params: &schemas.ResponsesParameters{
					MaxOutputTokens: deepintshield.Ptr(500),
				},
				Fallbacks: testConfig.Fallbacks,
			}
