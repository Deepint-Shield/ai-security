...
// Helper function to create a DeepIntShieldError
func createDeepIntShieldError(message string, statusCode *int, errorType *string, isDeepIntShieldError bool) *schemas.DeepIntShieldError {
	return &schemas.DeepIntShieldError{
		IsDeepIntShieldError: isDeepIntShieldError,
...
func TestExecuteRequestWithRetries_SuccessScenarios(t *testing.T) {
	...
	ctx.SetValue(schemas.DeepIntShieldContextKeyTracer, &schemas.NoOpTracer{})
	// Test immediate success
	t.Run("ImmediateSuccess", func(t *testing.T) {
		callCount := 0
		handler := func() (string, *schemas.DeepIntShieldError) {
			callCount++
			return "success", nil
		}
	...
	t.Run("SuccessAfterRetries", func(t *testing.T) {
		...
		handler := func() (string, *schemas.DeepIntShieldError) {
			...
			if callCount <= 2 {
				...
				return "", createDeepIntShieldError("rate limit exceeded", Ptr(429), nil, false)
			}
			// Third call succeeds
			return "success", nil
		}
...
func TestExecuteRequestWithRetries_NonRetryableErrors(t *testing.T) {
	...
	testCases := []struct {
		name  string
		error *schemas.DeepIntShieldError
	}{
		{
			name:  "DeepIntShieldError",
			error: createDeepIntShieldError("validation error", nil, nil, true),
		},
		{
			name:  "RequestCancelled",
			error: createDeepIntShieldError("request cancelled", nil, Ptr(schemas.ErrRequestCancelled), false),
		},
		{
			name:  "Non-retryable status code",
			error: createDeepIntShieldError("bad request", Ptr(400), nil, false),
		},
		{
			name:  "Non-retryable error message",
			error: createDeepIntShieldError("invalid model", nil, nil, false),
		},
	}
	logger := NewDefaultLogger(schemas.LogLevelError)
...
func TestExecuteRequestWithRetries_RetryableConditions(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:  "StatusCode_500",
			error: createDeepIntShieldError("internal server error", Ptr(500), nil, false),
		},
		{
			name:  "StatusCode_502",
			error: createDeepIntShieldError("bad gateway", Ptr(502), nil, false),
		},
		{
			name:  "StatusCode_503",
			error: createDeepIntShieldError("service unavailable", Ptr(503), nil, false),
		},
		{
			name:  "StatusCode_504",
			error: createDeepIntShieldError("gateway timeout", Ptr(504), nil, false),
		},
		{
			name:  "StatusCode_429",
			error: createDeepIntShieldError("too many requests", Ptr(429), nil, false),
		},
		{
			name:  "ErrProviderDoRequest",
			error: createDeepIntShieldError(schemas.ErrProviderDoRequest, nil, nil, false),
		},
		{
			name:  "RateLimitMessage",
			error: createDeepIntShieldError("rate limit exceeded", nil, nil, false),
		},
		{
			name:  "RateLimitType",
			error: createDeepIntShieldError("some error", nil, Ptr("rate_limit"), false),
		},
	}
	logger := NewDefaultLogger(schemas.LogLevelError)
...
func TestKeySupportsRequestedModel_NormalizesProviderPrefixes(t *testing.T) {
	testCases := []struct {
		name           string
		...
		{
			name:           "prefixed allowed model matches plain request",
			allowedModels:  []string{"openai/gpt-4.1-mini"},
			requestedModel: "gpt-4.1-mini",
			want:           true,
		},
		{
			name:           "plain allowed model matches prefixed request",
			allowedModels:  []string{"gpt-4.1-mini"},
			requestedModel: "openai/gpt-4.1-mini",
			want:           true,
		},
		{
			name:           "nested provider prefixes normalize recursively",
			allowedModels:  []string{"openrouter/openai/gpt-4.1-mini"},
			requestedModel: "gpt-4.1-mini",
			want:           true,
		},
		{
			name:           "different models remain blocked",
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			if got := keySupportsRequestedModel(tc.allowedModels, tc.requestedModel); got != tc.want {
				t.Fatalf("keySupportsRequestedModel(%v, %q) = %v, want %v", tc.allowedModels, tc.requestedModel, got, tc.want)
			}
		})
	}
}
...
func TestSelectKeyFromProviderForModel_SessionStickiness(t *testing.T) {
	...
	deterministicSelector := func(ctx *schemas.DeepIntShieldContext, keys []schemas.Key, _ schemas.ModelProvider, _ string) (schemas.Key, error) {
		keySelectorCalls++
		return keys[0], nil // always return first key
	}

	ctx := context.Background()
	deepintshield, err := Init(ctx, schemas.DeepIntShieldConfig{
		Account:     account,
		Logger:      NewDefaultLogger(schemas.LogLevelError),
		KVStore:     kvStore,
	...
	bfCtx := schemas.NewDeepIntShieldContext(context.Background(), schemas.NoDeadline)
	bfCtx.SetValue(schemas.DeepIntShieldContextKeySessionID, "sess-123")

	// First call: cache miss, keySelector runs, key stored
	key1, err := deepintshield.selectKeyFromProviderForModel(bfCtx, schemas.ChatCompletionRequest, schemas.OpenAI, "gpt-4", schemas.OpenAI)
	if err != nil {
	...
	key2, err := deepintshield.selectKeyFromProviderForModel(bfCtx, schemas.ChatCompletionRequest, schemas.OpenAI, "gpt-4", schemas.OpenAI)
	if err != nil {
...
func TestSelectKeyFromProviderForModel_NoStickinessWithoutSessionID(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		key, err := deepintshield.selectKeyFromProviderForModel(bfCtx, schemas.ChatCompletionRequest, schemas.OpenAI, "gpt-4", schemas.OpenAI)
		if err != nil {
...
func TestSelectKeyFromProviderForModel_NormalizesKeyModelRestrictions(t *testing.T) {
	account := NewMockAccount()
	account.AddProvider(schemas.OpenAI, 5, 1000)
	account.SetKeysForProvider(schemas.OpenAI, []schemas.Key{
		{
			ID:     "key-openai-mini",
			Name:   "OpenAI Mini",
			Value:  *schemas.NewEnvVar("sk-mini"),
			Models: []string{"openai/gpt-4.1-mini"},
			Weight: 1,
		},
	})

	deepintshield, err := Init(context.Background(), schemas.DeepIntShieldConfig{
		Account: account,
		Logger:  NewDefaultLogger(schemas.LogLevelError),
	})
	if err != nil {
	...
	key, err := deepintshield.selectKeyFromProviderForModel(
		schemas.NewDeepIntShieldContext(context.Background(), schemas.NoDeadline),
		schemas.ChatCompletionRequest,
		schemas.OpenAI,
		"gpt-4.1-mini",
		schemas.OpenAI,
	...
	if err != nil {
		t.Fatalf("selectKeyFromProviderForModel returned error: %v", err)
	}
	if key.ID != "key-openai-mini" {
		t.Fatalf("expected key-openai-mini, got %s", key.ID)
	}
}

func TestGetKeysForBatchAndFileOps_NormalizesKeyModelRestrictions(t *testing.T) {
	account := NewMockAccount()
	account.AddProvider(schemas.OpenAI, 5, 1000)
	account.SetKeysForProvider(schemas.OpenAI, []schemas.Key{
		{
			ID:             "key-openai-batch",
			Name:           "OpenAI Batch",
			Value:          *schemas.NewEnvVar("sk-batch"),
			Models:         []string{"openai/gpt-4.1-mini"},
			Weight:         1,
			UseForBatchAPI: Ptr(true),
		},
	})
	...
	model := "gpt-4.1-mini"
	keys, err := deepintshield.getKeysForBatchAndFileOps(
		schemas.NewDeepIntShieldContext(context.Background(), schemas.NoDeadline),
		schemas.OpenAI,
		schemas.OpenAI,
		&model,
	...
	if err != nil {
		t.Fatalf("getKeysForBatchAndFileOps returned error: %v", err)
	}
	if len(keys) != 1 {
		t.Fatalf("expected 1 key, got %d", len(keys))
	}
	if keys[0].ID != "key-openai-batch" {
		t.Fatalf("expected key-openai-batch, got %s", keys[0].ID)
	}
}
...
func TestUpdateProvider(t *testing.T) {
	t.Run("SuccessfulUpdate", func(t *testing.T) {
		...
		if err != nil {
			t.Fatalf("Failed to initialize DeepIntShield: %v", err)
		}

		// Verify initial provider exists
		initialProvider := deepintshield.getProviderByKey(schemas.OpenAI)
		...
		// Perform update
		err = deepintshield.UpdateProvider(schemas.OpenAI)
		if err != nil {
			t.Fatalf("UpdateProvider failed: %v", err)
		}

		// Verify provider was replaced
		updatedProvider := deepintshield.getProviderByKey(schemas.OpenAI)
	...
	t.Run("UpdateNonExistentProvider", func(t *testing.T) {
		...
		err = deepintshield.UpdateProvider(schemas.Anthropic)
		if err == nil {
	...
	t.Run("UpdateInactiveProvider", func(t *testing.T) {
		// Setup account with provider but don't initialize it in DeepIntShield
		account := NewMockAccount()
		...
		if deepintshield.getProviderByKey(schemas.Ollama) != nil {
			t.Fatal("Provider should not exist initially")
		}

		// Add provider to account after deepintshield initialization
		...
		err = deepintshield.UpdateProvider(schemas.Ollama)
		if err != nil {
			t.Fatalf("UpdateProvider should succeed for inactive provider: %v", err)
		}

		// Verify provider now exists
		provider := deepintshield.getProviderByKey(schemas.Ollama)
		if provider == nil {
	...
	t.Run("MultipleProviderUpdates", func(t *testing.T) {
		...
		initialOpenAI := deepintshield.getProviderByKey(schemas.OpenAI)
		initialAnthropic := deepintshield.getProviderByKey(schemas.Anthropic)
		initialCohere := deepintshield.getProviderByKey(schemas.Cohere)
		...
		for _, provider := range providers {
			err = deepintshield.UpdateProvider(provider)
			if err != nil {
		...
		newOpenAI := deepintshield.getProviderByKey(schemas.OpenAI)
		newAnthropic := deepintshield.getProviderByKey(schemas.Anthropic)
		newCohere := deepintshield.getProviderByKey(schemas.Cohere)
	...
	t.Run("ConcurrentProviderUpdates", func(t *testing.T) {
		...
		provider := deepintshield.getProviderByKey(schemas.OpenAI)
		if provider == nil {
...
func TestUpdateProvider_ProviderSliceIntegrity(t *testing.T) {
	t.Run("ProviderSliceConsistency", func(t *testing.T) {
		...
		initialProviders := deepintshield.providers.Load()
		...
		updatedProviders := deepintshield.providers.Load()
