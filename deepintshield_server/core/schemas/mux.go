...
func (cu *DeepIntShieldLLMUsage) ToResponsesResponseUsage() *ResponsesResponseUsage {
	if cu == nil {
		return nil
	}
...
func (ru *ResponsesResponseUsage) ToDeepIntShieldLLMUsage() *DeepIntShieldLLMUsage {
	if ru == nil {
		return nil
	}

	usage := &DeepIntShieldLLMUsage{
...
// ToResponsesRequest converts a DeepIntShieldChatRequest to DeepIntShieldResponsesRequest format
func (cr *DeepIntShieldChatRequest) ToResponsesRequest() *DeepIntShieldResponsesRequest {
	if cr == nil {
		return &DeepIntShieldResponsesRequest{}
	}

	brr := &DeepIntShieldResponsesRequest{
...
// ToChatRequest converts a DeepIntShieldResponsesRequest to DeepIntShieldChatRequest format
func (brr *DeepIntShieldResponsesRequest) ToChatRequest() *DeepIntShieldChatRequest {
	if brr == nil {
		return &DeepIntShieldChatRequest{}
	}

	bcr := &DeepIntShieldChatRequest{
...
func responsesStatusFromChatFinishReason(finishReason string) (status string, incompleteDetails *ResponsesResponseIncompleteDetails, mapped bool) {
	switch finishReason {
	case string(DeepIntShieldFinishReasonLength):
		return "incomplete", &ResponsesResponseIncompleteDetails{Reason: "max_output_tokens"}, true
	case string(DeepIntShieldFinishReasonStop), string(DeepIntShieldFinishReasonToolCalls):
...
// ToDeepIntShieldResponsesResponse converts the DeepIntShieldChatResponse to DeepIntShieldResponsesResponse format
// This converts Chat-style fields (Choices) to Responses API format
func (cr *DeepIntShieldChatResponse) ToDeepIntShieldResponsesResponse() *DeepIntShieldResponsesResponse {
	if cr == nil {
		return nil
	}

	// Create new DeepIntShieldResponsesResponse from Chat fields
	responsesResp := &DeepIntShieldResponsesResponse{
		ID:            Ptr(cr.ID),
		Object:        "response",
...
// ToDeepIntShieldChatResponse converts a DeepIntShieldResponsesResponse to DeepIntShieldChatResponse format
// This converts Responses API format to Chat-style fields (Choices)
func (responsesResp *DeepIntShieldResponsesResponse) ToDeepIntShieldChatResponse() *DeepIntShieldChatResponse {
	if responsesResp == nil {
		return nil
	}

	// Create new DeepIntShieldChatResponse from Responses fields
	chatResp := &DeepIntShieldChatResponse{
	...
	if len(responsesResp.Output) > 0 {
		...
		choices := make([]DeepIntShieldResponseChoice, 0, len(chatMessages))
		for i, chatMsg := range chatMessages {
			choice := DeepIntShieldResponseChoice{
				Index: i,
	...
	if responsesResp.Usage != nil {
		...
		chatResp.Usage = responsesResp.Usage.ToDeepIntShieldLLMUsage()
	}
...
// ToDeepIntShieldResponsesStreamResponse converts the DeepIntShieldChatResponse from Chat streaming format to Responses streaming format
// This converts Chat stream chunks (Choices with Deltas) to DeepIntShieldResponsesStreamResponse format
// Returns a slice of responses to support cases where a single event produces multiple responses
func (cr *DeepIntShieldChatResponse) ToDeepIntShieldResponsesStreamResponse(state *ChatToResponsesStreamState) []*DeepIntShieldResponsesStreamResponse {
	if cr == nil || state == nil {
		return nil
	}
	...
	// Convert first streaming choice to DeepIntShieldResponsesStreamResponse
	...
	var responses []*DeepIntShieldResponsesStreamResponse
	...
	if delta.Role != nil && !state.HasEmittedCreated {
		...
		response := &DeepIntShieldResponsesResponse{
			ID:        state.MessageID,
			CreatedAt: state.CreatedAt,
		}
		responses = append(responses, &DeepIntShieldResponsesStreamResponse{
	...
	if hasContent || (hasReasoning && !state.TextItemAdded) {
		...
		if hasContent || (!state.TextItemHasContent && (hasReasoning || hasContent)) {
			...
			response := &DeepIntShieldResponsesStreamResponse{
