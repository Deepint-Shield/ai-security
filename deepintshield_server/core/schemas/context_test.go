...
func TestNewDeepIntShieldContext_NoGoroutineLeakWithBackgroundAndNoDeadline(t *testing.T) {
	...
	contexts := make([]*DeepIntShieldContext, 100)
	for i := 0; i < 100; i++ {
		contexts[i] = NewDeepIntShieldContext(context.Background(), NoDeadline)
	}
...
func TestNewDeepIntShieldContext_GoroutineStartsWithDeadline(t *testing.T) {
	// Get baseline goroutine count
	runtime.GC()
	time.Sleep(10 * time.Millisecond)
	baseline := runtime.NumGoroutine()
	...
	ctx := NewDeepIntShieldContext(context.Background(), deadline)
...
func TestNewDeepIntShieldContext_GoroutineStartsWithCancellableParent(t *testing.T) {
	// Get baseline goroutine count
	runtime.GC()
	time.Sleep(10 * time.Millisecond)
	baseline := runtime.NumGoroutine()
	...
	// Create DeepIntShieldContext with cancellable parent but no deadline
...
func TestNewDeepIntShieldContext_DeadlineExpires(t *testing.T) {
...
func TestNewDeepIntShieldContext_SetAndGetValue(t *testing.T) {
	ctx := NewDeepIntShieldContext(context.Background(), NoDeadline)
...
func TestNewDeepIntShieldContext_NilParent(t *testing.T) {
	// Should not panic with nil parent
	// Note: passing nil is allowed by NewDeepIntShieldContext which converts it to context.Background()
	var nilCtx context.Context //nolint:staticcheck // testing nil parent handling
	ctx := NewDeepIntShieldContext(nilCtx, NoDeadline)
