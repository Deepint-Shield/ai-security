...
// MCPConfig represents the configuration for MCP integration in DeepIntShield.
...
type MCPConfig struct {
	...
	FetchNewRequestIDFunc func(ctx *DeepIntShieldContext) string `json:"-"`
...
type MCPToolManagerConfig struct {
	...
	CacheEnabled         *bool                `json:"cache_enabled,omitempty"`           // Direct (non-semantic) MCP result cache. Defaults to true when nil - toggle from MCP Settings.
	CacheTTLSeconds      int                  `json:"cache_ttl_seconds,omitempty"`       // TTL for cached MCP tool results in seconds. 0 = use plugin default (300).
}
...
type MCPClientConfig struct {
	...
	IsCacheEnabled   bool               `json:"is_cache_enabled,omitempty"`   // Whether the direct MCP result cache (mcpcache plugin) is enabled for this client. Default: false.
	CacheableTools   []string           `json:"cacheable_tools,omitempty"`    // Whitelist of tools eligible for caching (bare names, no `<server>-` prefix). ["*"] = all, empty/nil = none. Ignored when IsCacheEnabled is false.
	CacheTTLSeconds  int                `json:"cache_ttl_seconds,omitempty"`  // Per-client TTL override for cached results in seconds. 0 = use plugin default.
...
// It is returned by GetMCPClients() method in deepintshield.
