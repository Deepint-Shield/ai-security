...
type StreamAccumulatorResult struct {
	...
	TokenUsage            *DeepIntShieldLLMUsage                // Token usage
	Cost                  *float64                              // Cost in dollars
	ErrorDetails          *DeepIntShieldError                   // Error details if any
	AudioOutput           *DeepIntShieldSpeechResponse          // For speech streaming
	TranscriptionOutput   *DeepIntShieldTranscriptionResponse   // For transcription streaming
	ImageGenerationOutput *DeepIntShieldImageGenerationResponse // For image generation streaming
	FinishReason          *string                               // Finish reason
	RawResponse           *string                               // Raw response
	RawRequest            interface{}                           // Raw request
}

// Tracer defines the interface for distributed tracing in DeepIntShield.
// Implementations can be injected via DeepIntShieldConfig to enable automatic instrumentation.
...
type Tracer interface {
	...
	PopulateLLMRequestAttributes(handle SpanHandle, req *DeepIntShieldRequest)
	...
	PopulateLLMResponseAttributes(handle SpanHandle, resp *DeepIntShieldResponse, err *DeepIntShieldError)
	...
	// Pass the full DeepIntShieldResponse to capture content, tool calls, reasoning, etc.
	// This is called for each streaming chunk to build up the complete response.
	AddStreamingChunk(traceID string, response *DeepIntShieldResponse)
	...
	GetAccumulatedChunks(traceID string) (response *DeepIntShieldResponse, ttftNs int64, chunkCount int)
	...
	ProcessStreamingChunk(traceID string, isFinalChunk bool, result *DeepIntShieldResponse, err *DeepIntShieldError) *StreamAccumulatorResult
...
func (n *NoOpTracer) PopulateLLMRequestAttributes(_ SpanHandle, _ *DeepIntShieldRequest) {}

// PopulateLLMResponseAttributes does nothing.
func (n *NoOpTracer) PopulateLLMResponseAttributes(_ SpanHandle, _ *DeepIntShieldResponse, _ *DeepIntShieldError) {
}
...
func (n *NoOpTracer) AddStreamingChunk(_ string, _ *DeepIntShieldResponse) {}

// GetAccumulatedChunks returns nil, 0, 0.
func (n *NoOpTracer) GetAccumulatedChunks(_ string) (*DeepIntShieldResponse, int64, int) {
	return nil, 0, 0
}
...
func (n *NoOpTracer) ProcessStreamingChunk(_ string, _ bool, _ *DeepIntShieldResponse, _ *DeepIntShieldError) *StreamAccumulatorResult {
	return nil
}
