...
func TestToChatRequest_NormalizesDeveloperRoleToSystemForFallback(t *testing.T) {
	req := &DeepIntShieldResponsesRequest{
...
func TestToDeepIntShieldResponsesStreamResponse_PopulatesFinalDoneTextAndCompletedOutput(t *testing.T) {
	state := AcquireChatToResponsesStreamState()
	defer ReleaseChatToResponsesStreamState(state)

	makeChunk := func(role *string, content *string, finishReason *string) *DeepIntShieldChatResponse {
		return &DeepIntShieldChatResponse{
			ID:    "chatcmpl-test",
			Model: "test-model",
	...
	part1 := "Hello"
	part2 := " world"
	stop := string(DeepIntShieldFinishReasonStop)

	var all []*DeepIntShieldResponsesStreamResponse
	all = append(all, makeChunk(&role, nil, nil).ToDeepIntShieldResponsesStreamResponse(state)...)
	all = append(all, makeChunk(nil, &part1, nil).ToDeepIntShieldResponsesStreamResponse(state)...)
	all = append(all, makeChunk(nil, &part2, nil).ToDeepIntShieldResponsesStreamResponse(state)...)
	all = append(all, makeChunk(nil, nil, &stop).ToDeepIntShieldResponsesStreamResponse(state)...)

	var outputTextDone *DeepIntShieldResponsesStreamResponse
	var completed *DeepIntShieldResponsesStreamResponse
	for _, evt := range all {
		if evt == nil {
			continue
		}
...
func TestToDeepIntShieldResponsesResponse_MapsLengthToIncomplete(t *testing.T) {
	length := string(DeepIntShieldFinishReasonLength)
	resp := (&DeepIntShieldChatResponse{
		Choices: []DeepIntShieldResponseChoice{
			{FinishReason: &length},
		},
...
func TestToDeepIntShieldResponsesResponse_MapsToolCallsToCompleted(t *testing.T) {
	toolCalls := string(DeepIntShieldFinishReasonToolCalls)
	resp := (&DeepIntShieldChatResponse{
		Choices: []DeepIntShieldResponseChoice{
...
func TestToDeepIntShieldResponsesResponse_PrioritizesLengthAcrossChoices(t *testing.T) {
	stop := string(DeepIntShieldFinishReasonStop)
	length := string(DeepIntShieldFinishReasonLength)
	resp := (&DeepIntShieldChatResponse{
		Choices: []DeepIntShieldResponseChoice{
...
func TestToDeepIntShieldResponsesResponse_UnknownFinishReasonLeavesStatusUnset(t *testing.T) {
...
func TestToDeepIntShieldResponsesStreamResponse_IncludesFunctionCallsInCompletedOutput(t *testing.T) {
	state := AcquireChatToResponsesStreamState()
	defer ReleaseChatToResponsesStreamState(state)

	role := string(ChatMessageRoleAssistant)
	part1 := "Let me help"
	toolCallsFinish := string(DeepIntShieldFinishReasonToolCalls)
	...
	// Role chunk
	all = append(all, (&DeepIntShieldChatResponse{
		ID:    "chatcmpl-test",
		Model: "test-model",
		Choices: []DeepIntShieldResponseChoice{
			{
	...
	}).ToDeepIntShieldResponsesStreamResponse(state)...)
...
func TestToDeepIntShieldResponsesStreamResponse_ToolCallsOnlyInCompletedOutput(t *testing.T) {
	state := AcquireChatToResponsesStreamState()
	defer ReleaseChatToResponsesStreamState(state)
...
func TestToDeepIntShieldResponsesStreamResponse_MapsLengthToIncompleteEvent(t *testing.T) {
	state := AcquireChatToResponsesStreamState()
	defer ReleaseChatToResponsesStreamState(state)
	...
	var all []*DeepIntShieldResponsesStreamResponse
	all = append(all, makeChunk(&role, nil, nil).ToDeepIntShieldResponsesStreamResponse(state)...)
	all = append(all, makeChunk(nil, &part, nil).ToDeepIntShieldResponsesStreamResponse(state)...)
	all = append(all, makeChunk(nil, nil, &length).ToDeepIntShieldResponsesStreamResponse(state)...)

	var completed *DeepIntShieldResponsesStreamResponse
	var incomplete *DeepIntShieldResponsesStreamResponse
	for _, evt := range all {
		if evt == nil {
			continue
		}
