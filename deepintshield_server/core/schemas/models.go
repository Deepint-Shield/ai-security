...
type ListModelsByKeyResult struct {
	Response *DeepIntShieldListModelsResponse
	Err      *DeepIntShieldError
	KeyID    string
}
...
// circular reference: KeyStatus.Error → DeepIntShieldError.ExtraFields.KeyStatuses → KeyStatus.
...
type DeepIntShieldListModelsRequest struct {
	...
	// Unfiltered: If true, the response will include all models for the provider, regardless of the allowed models (internal deepintshield use only, not sent to the provider)
...
type DeepIntShieldListModelsResponse struct {
...
// ApplyPagination applies offset-based pagination to a DeepIntShieldListModelsResponse.
...
func (response *DeepIntShieldListModelsResponse) ApplyPagination(pageSize int, pageToken string) *DeepIntShieldListModelsResponse {
	if response == nil {
		return nil
	}
	...
	if offset >= totalItems {
		...
		return &DeepIntShieldListModelsResponse{
			Data:          []Model{},
	...
	paginatedResponse := &DeepIntShieldListModelsResponse{
		Data:        paginatedData,
		ExtraFields: response.ExtraFields,
		KeyStatuses: response.KeyStatuses,
	}
