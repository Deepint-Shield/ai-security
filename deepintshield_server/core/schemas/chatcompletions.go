...
// DeepIntShieldChatRequest is the request struct for chat completion requests
type DeepIntShieldChatRequest struct {
	Provider       ModelProvider   `json:"provider"`
	Model          string          `json:"model"`
	Input          []ChatMessage   `json:"input,omitempty"`
	Params         *ChatParameters `json:"params,omitempty"`
	Fallbacks      []Fallback      `json:"fallbacks,omitempty"`
	RawRequestBody []byte          `json:"-"` // set deepintshield-use-raw-request-body to true in ctx to use the raw request body. DeepIntShield will directly send this to the downstream provider.
}

// GetRawRequestBody returns the raw request body
func (cr *DeepIntShieldChatRequest) GetRawRequestBody() []byte {
	return cr.RawRequestBody
}

func (cr *DeepIntShieldChatRequest) GetExtraParams() map[string]interface{} {
	if cr.Params == nil {
...
// DeepIntShieldChatResponse represents the complete result from a chat completion request.
type DeepIntShieldChatResponse struct {
	ID                      string                     `json:"id"`
	Choices                 []DeepIntShieldResponseChoice    `json:"choices"`
	Created                 int                        `json:"created"` // The Unix timestamp (in seconds).
	Model                   string                     `json:"model"`
	Object                  string                     `json:"object"` // "chat.completion" or "chat.completion.chunk"
	ServiceTier             *string                    `json:"service_tier,omitempty"`
	SystemFingerprint       string                     `json:"system_fingerprint"`
	Usage                   *DeepIntShieldLLMUsage           `json:"usage"`
...
// ToTextCompletionResponse converts a DeepIntShieldChatResponse to a DeepIntShieldTextCompletionResponse
func (cr *DeepIntShieldChatResponse) ToTextCompletionResponse() *DeepIntShieldTextCompletionResponse {
	if cr == nil {
		return nil
	}

	if len(cr.Choices) == 0 {
		return &DeepIntShieldTextCompletionResponse{
			ID:                cr.ID,
			Model:             cr.Model,
			Object:            "text_completion",
			SystemFingerprint: cr.SystemFingerprint,
			Usage:             cr.Usage,
			ExtraFields: DeepIntShieldResponseExtraFields{
	...
	if choice.ChatStreamResponseChoice != nil && choice.ChatStreamResponseChoice.Delta != nil {
		return &DeepIntShieldTextCompletionResponse{
			...
			Object:            "text_completion",
			SystemFingerprint: cr.SystemFingerprint,
			Choices: []DeepIntShieldResponseChoice{
				{
					Index: 0,
...
type ChatStreamOptions struct {
	...
	IncludeUsage       *bool `json:"include_usage,omitempty"` // DeepIntShield marks this as true by default
}
...
type CacheControl struct {
	...
	TTL   *string          `json:"ttl,omitempty"`   // "5m" (default) | "1h" (Anthropic)
...
func (cm *ChatAssistantMessage) UnmarshalJSON(data []byte) error {
	...
	*cm = ChatAssistantMessage(aux.Alias)
	...
	// Map xAI's reasoning_content to DeepIntShield's Reasoning field
	...
	if cm.Reasoning != nil && len(cm.ReasoningDetails) == 0 {
		...
		cm.ReasoningDetails = []ChatReasoningDetails{
			{
				Index: 0,
				Type:  DeepIntShieldReasoningDetailsTypeText,
				Text:  &text,
			},
		}
	}
...
// DeepIntShieldResponseChoice represents a choice in the completion result.
...
type DeepIntShieldResponseChoice struct {
	Index        int              `json:"index"`
	FinishReason *string          `json:"finish_reason,omitempty"`
	LogProbs     *DeepIntShieldLogProbs `json:"logprobs,omitempty"`
...
// DeepIntShieldFinishReason represents the reason why the model stopped generating.
type DeepIntShieldFinishReason string

// DeepIntShieldFinishReason values
const (
	DeepIntShieldFinishReasonStop      DeepIntShieldFinishReason = "stop"
	DeepIntShieldFinishReasonLength    DeepIntShieldFinishReason = "length"
	DeepIntShieldFinishReasonToolCalls DeepIntShieldFinishReason = "tool_calls"
)

type DeepIntShieldReasoningDetailsType string

const (
	DeepIntShieldReasoningDetailsTypeSummary       DeepIntShieldReasoningDetailsType = "reasoning.summary"
	DeepIntShieldReasoningDetailsTypeEncrypted     DeepIntShieldReasoningDetailsType = "reasoning.encrypted"
	DeepIntShieldReasoningDetailsTypeText          DeepIntShieldReasoningDetailsType = "reasoning.text"
	DeepIntShieldReasoningDetailsTypeContentBlocks DeepIntShieldReasoningDetailsType = "deepintshield.content_blocks"
)
...
type ChatReasoningDetails struct {
	ID        *string                     `json:"id,omitempty"`
	Index     int                         `json:"index"`
	Type      DeepIntShieldReasoningDetailsType `json:"type"`
...
// DeepIntShieldLogProbs represents the log probabilities for different aspects of a response.
type DeepIntShieldLogProbs struct {
...
// DeepIntShieldLLMUsage represents token usage information
type DeepIntShieldLLMUsage struct {
	...
	Cost                    *DeepIntShieldCost                 `json:"cost,omitempty"` //Only for the providers which support cost calculation
}
...
// MarshalJSON always emits reasoning_tokens for OpenAI chat completion compatibility.
func (d ChatCompletionTokensDetails) MarshalJSON() ([]byte, error) {
	type raw struct {
		TextTokens               int  `json:"text_tokens,omitempty"`
		AcceptedPredictionTokens int  `json:"accepted_prediction_tokens,omitempty"`
		AudioTokens              int  `json:"audio_tokens,omitempty"`
	...
	return Marshal(raw{
		...
		AudioTokens:              d.AudioTokens,
		CitationTokens:           d.CitationTokens,
		NumSearchQueries:         d.NumSearchQueries,
...
// UnmarshalJSON implements custom JSON unmarshalling for DeepIntShieldCost.
func (bc *DeepIntShieldCost) UnmarshalJSON(data []byte) error {
	...
	// Try to unmarshal as a full DeepIntShieldCost struct
	// Use a type alias to avoid infinite recursion
	type Alias DeepIntShieldCost
	var costStruct Alias
	if err := Unmarshal(data, &costStruct); err == nil {
		*bc = DeepIntShieldCost(costStruct)
		return nil
	}
