...
func TestSonic_ChatCompletionTokensDetails_AlwaysEmitsReasoningTokens(t *testing.T) {
	output, err := Marshal(ChatCompletionTokensDetails{})
	require.NoError(t, err)

	assert.Contains(t, string(output), `"reasoning_tokens":0`)
}

func TestSonic_DeepIntShieldChatResponse_EmitsTierSensitiveUsageFields(t *testing.T) {
