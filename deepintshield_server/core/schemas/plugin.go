...
// Plugin defines the interface for DeepIntShield plugins.
// Plugins can intercept and modify requests and responses at different stages
// of the processing pipeline.
// User can provide multiple plugins in the DeepIntShieldConfig.
...
// - No Plugin errors are returned to the caller; they are logged as warnings by the DeepIntShield instance.
...
//
// IMPORTANT: When returning DeepIntShieldError from PreLLMHook or PostLLMHook:
// - You can set the AllowFallbacks field to control fallback behavior
// - AllowFallbacks = &true: Allow DeepIntShield to try fallback providers
...
type BasePlugin interface {
	...
	// Cleanup is called on deepintshield shutdown.
	// It allows plugins to clean up any resources they have allocated.
	// Returns any error that occurred during cleanup, which will be logged as a warning by the DeepIntShield instance.
	Cleanup() error
}

// WorkspaceScoped is a marker interface for plugin instances that should only
// apply to requests targeting a specific workspace. The PluginPipeline uses it
// at dispatch time: for each (plugin name, request workspace) pair, a workspace-
// scoped instance whose WorkspaceID matches the request wins over a non-scoped
// "global" instance of the same name; scoped instances whose WorkspaceID does
// not match are skipped entirely. Returning the empty string here is equivalent
// to not implementing the interface (treated as global).
//
// Plugins do not normally implement this themselves - the framework wraps a
// BasePlugin in a per-workspace decorator at registration time (see
// framework/plugins.WrapWithWorkspace).
...
type HTTPTransportPlugin interface {
	...
	// HTTPTransportPreHook is called at the HTTP transport layer before requests enter DeepIntShield core.
	// It receives a serializable HTTPRequest and allows plugins to modify it in-place.
	// Only invoked when using HTTP transport (deepintshield-http), not when using DeepIntShield as a Go SDK directly.
	...
	HTTPTransportPreHook(ctx *DeepIntShieldContext, req *HTTPRequest) (*HTTPResponse, error)

	// HTTPTransportPostHook is called at the HTTP transport layer after requests exit DeepIntShield core.
	...
	HTTPTransportPostHook(ctx *DeepIntShieldContext, req *HTTPRequest, resp *HTTPResponse) error

	// HTTPTransportStreamChunkHook is called for each chunk during streaming responses.
	// It receives the DeepIntShieldStreamChunk BEFORE they are written to the client.
	// Only invoked for streaming responses when using HTTP transport (deepintshield-http).
	// Works with both native .so plugins and WASM plugins due to serializable types.
	//
	// Plugins can modify the chunk by returning a different DeepIntShieldStreamChunk.
	// Return the original chunk unchanged if no modification is needed.
	//
	// Return values:
	// - (*DeepIntShieldStreamChunk, nil): Continue with the (potentially modified) DeepIntShieldStreamChunk
	// - (nil, nil): Skip this DeepIntShieldStreamChunk entirely (don't send to client)
	// - (*DeepIntShieldStreamChunk, error): Log warning and continue with the DeepIntShieldStreamChunk
	// - (nil, error): Send back error to the client and stop the streaming
	//
	// Return (*DeepIntShieldStreamChunk, nil) unchanged if the plugin doesn't need streaming chunk interception.
	HTTPTransportStreamChunkHook(ctx *DeepIntShieldContext, req *HTTPRequest, chunk *DeepIntShieldStreamChunk) (*DeepIntShieldStreamChunk, error)
}

type LLMPlugin interface {
	BasePlugin

	PreLLMHook(ctx *DeepIntShieldContext, req *DeepIntShieldRequest) (*DeepIntShieldRequest, *LLMPluginShortCircuit, error)
	PostLLMHook(ctx *DeepIntShieldContext, resp *DeepIntShieldResponse, deepintshieldErr *DeepIntShieldError) (*DeepIntShieldResponse, *DeepIntShieldError, error)
}

type MCPPlugin interface {
	BasePlugin

	PreMCPHook(ctx *DeepIntShieldContext, req *DeepIntShieldMCPRequest) (*DeepIntShieldMCPRequest, *MCPPluginShortCircuit, error)
	PostMCPHook(ctx *DeepIntShieldContext, resp *DeepIntShieldMCPResponse, deepintshieldErr *DeepIntShieldError) (*DeepIntShieldMCPResponse, *DeepIntShieldError, error)
}
...
type PluginConfig struct {
	...
	// WorkspaceID, when non-nil, restricts the instantiated plugin to
	// requests targeting that workspace. nil means a global instance that
	// applies to every workspace lacking an override - config.json plugins
	// and historical tenant-wide defaults land here. See WorkspaceScoped
	// for the dispatch rule the pipeline enforces.
...
//
// Example backends: OpenTelemetry collectors, Datadog, Jaeger, Langfuse, etc.
//
