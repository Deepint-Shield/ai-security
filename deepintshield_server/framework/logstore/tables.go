...
type SearchFilters struct {
	...
	// IncludeAgenticObjects opts a query INTO seeing agentic activity rows
	// (object_type "agentic.*" - agent/tool PDP decisions written to the logs
	// table so they appear in the AI Logs list). Only SearchLogs sets it; every
	// aggregate (stats / histograms) leaves it false so KPI tiles stay LLM-only.
	IncludeAgenticObjects bool              `json:"include_agentic_objects,omitempty"`
	...
	GuardrailStatuses     []string          `json:"guardrail_statuses,omitempty"`  // For filtering by guardrail decision (allow, deny, warn, etc.)
	GuardrailSources      []string          `json:"guardrail_sources,omitempty"`   // For filtering by guardrail engine kind (ai_model, policy, mixed)
	...
	// WorkspaceID narrows results to logs scoped to this workspace plus
	// pre-workspace logs (workspace_id IS NULL within the same tenant).
	// Empty = no workspace filter.
	WorkspaceID string `json:"workspace_id,omitempty"`
	// TenantID is the hard tenant boundary. When set, only rows with this
	// tenant_id are returned - this is what stops the dashboard / metrics
	// from leaking another tenant's traffic into the active tenant's
	// histograms. Populated by the HTTP layer from request context.
	TenantID string `json:"tenant_id,omitempty"`
	// AllowedWorkspaceIDs constrains results to the listed workspace IDs.
	// Used to enforce active-org isolation: the email-keyed tenant_id
	// column is the same across every UI tenant the user owns, so without
	// this allowlist, switching tenants in the sidebar leaks data across
	// orgs. nil = no constraint; empty (non-nil) = no workspaces exist for
	// the active org, return zero rows; non-empty = WHERE workspace_id IN
	// (...). Set by the HTTP layer from the workspace-by-org lookup.
	AllowedWorkspaceIDs *[]string `json:"allowed_workspace_ids,omitempty"`
	// IncludeBodies loads the full request/response JSON columns
	// (input_history, output_message, responses_input_history,
	// responses_output, embedding_output, tools, params, etc.) instead of
	// the trimmed list-view projection. The dashboard list views leave
	// this false to keep pagination fast; the export pipeline sets it
	// true so the CSV/JSON output carries actual prompt + response text.
...
type SearchStats struct {
	...
	TotalCacheSavings       float64 `json:"total_cache_savings"`        // Total cost avoided by semantic cache short-circuit
	TotalPromptCacheSavings float64 `json:"total_prompt_cache_savings"` // Total cost avoided by provider-native prompt caching (cached_read discount net of cached_write premium)
}
...
type Log struct {
	...
	WorkspaceID           *string   `gorm:"column:workspace_id;type:varchar(255);index" json:"workspace_id,omitempty"` // Stamped at write-time from the inference request's workspace context (typically derived from the virtual key). NULL = legacy / pre-workspace log.
	...
	SpeechOutput          string    `gorm:"type:text" json:"-"` // JSON serialized *schemas.DeepIntShieldSpeech
	TranscriptionOutput   string    `gorm:"type:text" json:"-"` // JSON serialized *schemas.DeepIntShieldTranscribe
	ImageGenerationOutput string    `gorm:"type:text" json:"-"` // JSON serialized *schemas.DeepIntShieldImageGenerationResponse
	ListModelsOutput      string    `gorm:"type:text" json:"-"` // JSON serialized []schemas.Model
	VideoGenerationOutput string    `gorm:"type:text" json:"-"` // JSON serialized *schemas.DeepIntShieldVideoGenerationResponse
	VideoRetrieveOutput   string    `gorm:"type:text" json:"-"` // JSON serialized *schemas.DeepIntShieldVideoRetrieveResponse
	VideoDownloadOutput   string    `gorm:"type:text" json:"-"` // JSON serialized *schemas.DeepIntShieldVideoDownloadResponse
	VideoListOutput       string    `gorm:"type:text" json:"-"` // JSON serialized *schemas.DeepIntShieldVideoListResponse
	VideoDeleteOutput     string    `gorm:"type:text" json:"-"` // JSON serialized *schemas.DeepIntShieldVideoDeleteResponse
	CacheDebug            string    `gorm:"type:text" json:"-"` // JSON serialized *schemas.DeepIntShieldCacheDebug
	...
	CacheSavings          *float64  `gorm:"index" json:"cache_savings,omitempty"`         // Cost avoided by serving from cache instead of the full upstream request - sum of every cost-optimization technique (semantic + prompt + reasoning + compression). Per-source breakdown lives in the columns below.
	PromptCacheSavings    *float64  `gorm:"index" json:"prompt_cache_savings,omitempty"`  // Cost avoided by provider-native prompt caching (cached_read discount net of cached_write premium)
	ReasoningSavings      *float64  `gorm:"index" json:"reasoning_savings,omitempty"`     // Cost avoided by reasoning-effort throttling (estimated)
	CompressionSavings    *float64  `gorm:"index" json:"compression_savings,omitempty"`   // Cost avoided by LLMLingua-2 prompt compression (measured: tokens dropped × input rate)
	RagSavings            *float64  `gorm:"index" json:"rag_savings,omitempty"`           // Cost avoided by RAG context trimming (measured: chunk tokens dropped × input rate)
	SummarizationSavings  *float64  `gorm:"index" json:"summarization_savings,omitempty"` // Cost avoided by conversation summarization (measured: tokens replaced − summary tokens, × input rate)
	ConsistencySavings    *float64  `gorm:"index" json:"consistency_savings,omitempty"`   // Cost avoided by the Response Consistency engine serving a cached answer (estimated: avoided prompt+completion tokens × base rate)

	// Per-optimization debug fields stamped at log-write time so the analytics
	// histograms can aggregate categorical / distributive metrics without
	// re-parsing the response JSON on every read. Indexes are only on the
	// fields used as group-by keys; everything else stays untouched on hot reads.
	CompressionApplied             *bool    `gorm:"index" json:"compression_applied,omitempty"`
	CompressionCacheHit            *bool    `json:"compression_cache_hit,omitempty"`
	CompressionOriginalTokens      *int     `json:"compression_original_tokens,omitempty"`
	CompressionCompressedTokens    *int     `json:"compression_compressed_tokens,omitempty"`
	CompressionLatencyMs           *int     `json:"compression_latency_ms,omitempty"`
	ReasoningApplied               *bool    `gorm:"index" json:"reasoning_applied,omitempty"`
	ReasoningAppliedEffort         *string  `gorm:"index;type:varchar(20)" json:"reasoning_applied_effort,omitempty"`
	ReasoningOriginalEffort        *string  `gorm:"type:varchar(20)" json:"reasoning_original_effort,omitempty"`
	ReasoningSampled               *bool    `json:"reasoning_sampled,omitempty"`
	CascadeScore                   *float64 `json:"cascade_score,omitempty"`
	CascadeNeedsEscalation         *bool    `gorm:"index" json:"cascade_needs_escalation,omitempty"`
	CascadeSource                  *string  `gorm:"type:varchar(30)" json:"cascade_source,omitempty"`
	BatchEligible                  *bool    `gorm:"index" json:"batch_eligible,omitempty"`
	BatchProvider                  *string  `gorm:"type:varchar(40)" json:"batch_provider,omitempty"`
	RagApplied                     *bool    `gorm:"index" json:"rag_applied,omitempty"`
	RagChunksDetected              *int     `json:"rag_chunks_detected,omitempty"`
	RagChunksKept                  *int     `json:"rag_chunks_kept,omitempty"`
	RagTrimmedTokens               *int     `json:"rag_trimmed_tokens,omitempty"`
	RagOriginalTokens              *int     `json:"rag_original_tokens,omitempty"`
	RagTopScore                    *float64 `json:"rag_top_score,omitempty"`
	RagRerankLatencyMs             *int     `json:"rag_rerank_latency_ms,omitempty"`
	RagCacheHit                    *bool    `json:"rag_cache_hit,omitempty"`
	SummarizationApplied           *bool    `gorm:"index" json:"summarization_applied,omitempty"`
	SummarizationTurnsSummarized   *int     `json:"summarization_turns_summarized,omitempty"`
	SummarizationOriginalTokens    *int     `json:"summarization_original_tokens,omitempty"`
	SummarizationSummaryTokens     *int     `json:"summarization_summary_tokens,omitempty"`
	SummarizationSavedTokens       *int     `json:"summarization_saved_tokens,omitempty"`
	SummarizationCacheHit          *bool    `json:"summarization_cache_hit,omitempty"`
	SummarizationAsyncKickoff      *bool    `json:"summarization_async_kickoff,omitempty"`
	TTFTApplied                    *bool    `gorm:"index" json:"ttft_applied,omitempty"`
	TTFTMessagesReordered          *int     `json:"ttft_messages_reordered,omitempty"`
	TTFTStablePrefixTokens         *int     `json:"ttft_stable_prefix_tokens,omitempty"`
	ParallelToolsApplied           *bool    `gorm:"index" json:"parallel_tools_applied,omitempty"`
	ParallelToolsTotal             *int     `json:"parallel_tools_total,omitempty"`
	ParallelToolsParallelCount     *int     `json:"parallel_tools_parallel_count,omitempty"`
	ParallelToolsSequentialCount   *int     `json:"parallel_tools_sequential_count,omitempty"`
	ParallelToolsWallClockMs       *int     `json:"parallel_tools_wall_clock_ms,omitempty"`
	ParallelToolsSerialEstimateMs  *int     `json:"parallel_tools_serial_estimate_ms,omitempty"`
	ParallelToolsLatencySavedMs    *int     `json:"parallel_tools_latency_saved_ms,omitempty"`
	HallucinationApplied           *bool    `gorm:"index" json:"hallucination_applied,omitempty"`
	HallucinationFaithfulness      *float64 `gorm:"index" json:"hallucination_faithfulness,omitempty"`
	HallucinationAnswerRelevance   *float64 `gorm:"index" json:"hallucination_answer_relevance,omitempty"`
	HallucinationCoherence         *float64 `gorm:"index" json:"hallucination_coherence,omitempty"`
	HallucinationHelpfulness       *float64 `json:"hallucination_helpfulness,omitempty"`
	HallucinationCitationPrecision *float64 `json:"hallucination_citation_precision,omitempty"`
	HallucinationScore             *float64 `gorm:"index" json:"hallucination_score,omitempty"`
	// Hallucination Control (proactive mitigation). Populated in
	// PostLLMHook by the semantic_cache plugin when at least one
	// technique was applied to the request.
	HallucinationControlApplied     *bool    `gorm:"index" json:"hallucination_control_applied,omitempty"`
	HallucinationControlTechniques  *string  `gorm:"type:text" json:"hallucination_control_techniques,omitempty"` // comma-separated technique ids
	HallucinationControlStrictness  *string  `gorm:"type:varchar(16)" json:"hallucination_control_strictness,omitempty"`
	HallucinationControlImprovement *float64 `gorm:"index" json:"hallucination_control_improvement,omitempty"`
	...
	ErrorDetails                    string   `gorm:"type:text" json:"-"`                                                                         // JSON serialized *schemas.DeepIntShieldError
	...
	TokenUsageParsed            *schemas.DeepIntShieldLLMUsage                `gorm:"-" json:"token_usage,omitempty"`
	ErrorDetailsParsed          *schemas.DeepIntShieldError                   `gorm:"-" json:"error_details,omitempty"`
	...
	SpeechOutputParsed          *schemas.DeepIntShieldSpeechResponse          `gorm:"-" json:"speech_output,omitempty"`
	TranscriptionOutputParsed   *schemas.DeepIntShieldTranscriptionResponse   `gorm:"-" json:"transcription_output,omitempty"`
	ImageGenerationOutputParsed *schemas.DeepIntShieldImageGenerationResponse `gorm:"-" json:"image_generation_output,omitempty"`
	CacheDebugParsed            *schemas.DeepIntShieldCacheDebug              `gorm:"-" json:"cache_debug,omitempty"`
	GuardrailStatus             string                                        `gorm:"-" json:"guardrail_status,omitempty"`
	// GuardrailSource classifies which kind of guardrail produced this log's
	// findings - "ai_model", "policy", "mixed", or "" (no findings). Joined
	// at read time from guardrail_findings the same way GuardrailStatus is
	// joined from guardrail_decisions. Surfaces as the "Engine" column on
	// the AI Logs page.
	GuardrailSource             string                                        `gorm:"-" json:"guardrail_source,omitempty"`
	...
	VideoGenerationOutputParsed *schemas.DeepIntShieldVideoGenerationResponse `gorm:"-" json:"video_generation_output,omitempty"`
	VideoRetrieveOutputParsed   *schemas.DeepIntShieldVideoGenerationResponse `gorm:"-" json:"video_retrieve_output,omitempty"`
	VideoDownloadOutputParsed   *schemas.DeepIntShieldVideoDownloadResponse   `gorm:"-" json:"video_download_output,omitempty"`
	VideoListOutputParsed       *schemas.DeepIntShieldVideoListResponse       `gorm:"-" json:"video_list_output,omitempty"`
	VideoDeleteOutputParsed     *schemas.DeepIntShieldVideoDeleteResponse     `gorm:"-" json:"video_delete_output,omitempty"`
...
type MCPToolLog struct {
	ID             string    `gorm:"primaryKey;type:varchar(255)" json:"id"`
	TenantID       string    `gorm:"column:tenant_id;type:varchar(255);index" json:"-"`
	WorkspaceID    *string   `gorm:"column:workspace_id;type:varchar(255);index" json:"workspace_id,omitempty"`                                 // Stamped at write-time from the inference request's workspace context. NULL = legacy / pre-workspace log.
	...
	CacheHit       *bool     `gorm:"column:cache_hit;index:idx_mcp_logs_cache_hit" json:"cache_hit,omitempty"` // true when served from the mcpcache plugin without an upstream call
	Metadata       string    `gorm:"type:text" json:"-"`                                                       // JSON serialized map[string]interface{}
	CreatedAt      time.Time `gorm:"index;not null" json:"created_at"`
	...
	GuardrailStatus    string                      `gorm:"-" json:"guardrail_status,omitempty"` // Latest guardrail decision joined at read time
	GuardrailSource    string                      `gorm:"-" json:"guardrail_source,omitempty"` // ai_model | policy | mixed | "" - joined at read time
}
...
func (j *AsyncJob) ToResponse() *schemas.AsyncJobResponse {
	...
	if j.Response != "" {
		...
		case schemas.ResponsesRequest, schemas.ResponsesStreamRequest:
			var result schemas.DeepIntShieldResponsesResponse
			if err := sonic.Unmarshal([]byte(j.Response), &result); err == nil {
				resp.Result = &result
			}
		case schemas.ChatCompletionRequest, schemas.ChatCompletionStreamRequest:
			var result schemas.DeepIntShieldChatResponse
			if err := sonic.Unmarshal([]byte(j.Response), &result); err == nil {
				resp.Result = &result
			}
		case schemas.TextCompletionRequest, schemas.TextCompletionStreamRequest:
			var result schemas.DeepIntShieldTextCompletionResponse
			if err := sonic.Unmarshal([]byte(j.Response), &result); err == nil {
				resp.Result = &result
			}
		case schemas.EmbeddingRequest:
			var result schemas.DeepIntShieldEmbeddingResponse
			if err := sonic.Unmarshal([]byte(j.Response), &result); err == nil {
				resp.Result = &result
			}
		case schemas.SpeechRequest, schemas.SpeechStreamRequest:
			var result schemas.DeepIntShieldSpeechResponse
			if err := sonic.Unmarshal([]byte(j.Response), &result); err == nil {
				resp.Result = &result
			}
		case schemas.TranscriptionRequest, schemas.TranscriptionStreamRequest:
			var result schemas.DeepIntShieldTranscriptionResponse
			if err := sonic.Unmarshal([]byte(j.Response), &result); err == nil {
				resp.Result = &result
			}
		case schemas.ImageGenerationRequest, schemas.ImageGenerationStreamRequest:
			var result schemas.DeepIntShieldImageGenerationResponse
			if err := sonic.Unmarshal([]byte(j.Response), &result); err == nil {
				resp.Result = &result
			}
		...
		case schemas.CountTokensRequest:
			var result schemas.DeepIntShieldCountTokensResponse
			if err := sonic.Unmarshal([]byte(j.Response), &result); err == nil {
				resp.Result = &result
			}
		default:
	...
	if j.Error != "" {
		...
		if err := sonic.Unmarshal([]byte(j.Error), &deepintshieldErr); err == nil {
			resp.Error = &deepintshieldErr
		}
	}
...
type MCPToolLogSearchFilters struct {
	...
	MaxLatency    *float64   `json:"max_latency,omitempty"`
	ContentSearch string     `json:"content_search,omitempty"`
	// WorkspaceID narrows results to MCP tool logs scoped to this
	// workspace plus pre-workspace logs (workspace_id IS NULL) within the
	// same tenant.
	WorkspaceID string `json:"workspace_id,omitempty"`
	// TenantID is the hard tenant boundary. Required when serving
	// dashboard-style listings; without it, the NULL-workspace fallback
	// branch leaks cross-tenant entries.
	TenantID string `json:"tenant_id,omitempty"`
	// AllowedWorkspaceIDs - same semantics as on SearchFilters. Empty
	// (non-nil) = active org has no workspaces, return zero rows.
	AllowedWorkspaceIDs *[]string `json:"allowed_workspace_ids,omitempty"`
}
...
type TokenHistogramBucket struct {
	...
	EffectiveCachedPromptTokens int64     `json:"effective_cached_prompt_tokens"`
}
...
// CacheHistogramBucket represents a single time bucket for semantic cache outcomes.
type CacheHistogramBucket struct {
	Timestamp               time.Time `json:"timestamp"`
	...
	UserScopeHits           int64     `json:"user_scope_hits"`
	UseCaseScopeHits        int64     `json:"use_case_scope_hits"`
	SessionScopeHits        int64     `json:"session_scope_hits"`
	VirtualKeyScopeHits     int64     `json:"virtual_key_scope_hits"`
	CustomMetadataScopeHits int64     `json:"custom_metadata_scope_hits"`
	TenantScopeHits         int64     `json:"tenant_scope_hits"`
	AutoScopedHits          int64     `json:"auto_scoped_hits"`
	ExplicitOverrideHits    int64     `json:"explicit_override_hits"`
	DefaultScopeHits        int64     `json:"default_scope_hits"`
	RequestFallbackHits     int64     `json:"request_fallback_hits"`
	SemanticSuppressions    int64     `json:"semantic_suppressions"`
}

// CacheHistogramResult represents the semantic cache histogram query result.
type CacheHistogramResult struct {
	Buckets           []CacheHistogramBucket `json:"buckets"`
	BucketSizeSeconds int64                  `json:"bucket_size_seconds"`
}
...
type CostHistogramBucket struct {
	...
	ByModelCacheSavings map[string]float64 `json:"by_model_cache_savings"`
}
...
// OptimizationHistogramBucket is the per-time-slice rollup for the Cost
// Optimization analytics tab. Carries the per-source savings breakdown
// (semantic/prompt/reasoning/compression) plus the categorical aggregates
// each technique exposes (effort distribution, escalation rate, compression
// cache-hit ratio, etc.). All sums are workspace-scoped via the same filters
// the rest of the dashboard uses.
type OptimizationHistogramBucket struct {
	Timestamp time.Time `json:"timestamp"`

	// Per-source savings totals. CacheSavings is preserved on the cost
	// histogram for backwards compat; here we expose the breakdown so the
	// UI can render a stacked-bar attribution chart.
	SemanticSavings      float64 `json:"semantic_savings"` // derived: cache_savings − (prompt+reasoning+compression+rag+summarization+consistency)
	PromptCacheSavings   float64 `json:"prompt_cache_savings"`
	ReasoningSavings     float64 `json:"reasoning_savings"`
	CompressionSavings   float64 `json:"compression_savings"`
	RagSavings           float64 `json:"rag_savings"`
	SummarizationSavings float64 `json:"summarization_savings"`
	ConsistencySavings   float64 `json:"consistency_savings"` // Response Consistency engine (estimated)
	TotalSavings         float64 `json:"total_savings"`       // sum of the seven above; equal to cache_savings

	// Compression aggregates
	CompressionApplied          int64   `json:"compression_applied"`           // count of rows where compression fired
	CompressionCacheHits        int64   `json:"compression_cache_hits"`        // count of compressed rows served from the prefix LRU
	CompressionOriginalTokens   int64   `json:"compression_original_tokens"`   // sum of pre-compression tokens
	CompressionCompressedTokens int64   `json:"compression_compressed_tokens"` // sum of post-compression tokens
	CompressionAvgLatencyMs     float64 `json:"compression_avg_latency_ms"`    // avg over non-zero rows
	CompressionAvgRatio         float64 `json:"compression_avg_ratio"`         // avg of compressed/original

	// Reasoning aggregates
	ReasoningApplied   int64            `json:"reasoning_applied"`    // count of rows where throttle fired
	ReasoningSampled   int64            `json:"reasoning_sampled"`    // count of pass-through (drift) rows
	ReasoningEffortMix map[string]int64 `json:"reasoning_effort_mix"` // applied_effort → count
	ReasoningTokens    int64            `json:"reasoning_tokens"`     // sum of completion_tokens_details.reasoning_tokens (best-effort)

	// Cascade aggregates
	CascadeAvgScore    float64          `json:"cascade_avg_score"`   // avg score across stamped rows
	CascadeEscalations int64            `json:"cascade_escalations"` // count of needs_escalation=true
	CascadeSourceMix   map[string]int64 `json:"cascade_source_mix"`  // source → count

	// Batch aggregates
	BatchEligible    int64            `json:"batch_eligible"`     // count of batch-eligible rows
	BatchProviderMix map[string]int64 `json:"batch_provider_mix"` // provider → count

	// RAG aggregates - measured, not projected
	RagApplied            int64   `json:"rag_applied"`         // count of rows where trimming fired
	RagChunksDetected     int64   `json:"rag_chunks_detected"` // sum of chunks detected pre-trim
	RagChunksKept         int64   `json:"rag_chunks_kept"`     // sum of chunks forwarded post-trim
	RagTrimmedTokens      int64   `json:"rag_trimmed_tokens"`  // sum of input tokens dropped
	RagOriginalTokens     int64   `json:"rag_original_tokens"` // sum of pre-trim chunk tokens
	RagAvgRerankLatencyMs float64 `json:"rag_avg_rerank_latency_ms"`
	RagCacheHits          int64   `json:"rag_cache_hits"` // count of rows served from the (query, chunks) LRU

	// Summarization aggregates - measured, not projected
	SummarizationApplied         int64 `json:"summarization_applied"`
	SummarizationCacheHits       int64 `json:"summarization_cache_hits"`
	SummarizationAsyncKickoffs   int64 `json:"summarization_async_kickoffs"`
	SummarizationTurnsSummarized int64 `json:"summarization_turns_summarized"`
	SummarizationOriginalTokens  int64 `json:"summarization_original_tokens"`
	SummarizationSummaryTokens   int64 `json:"summarization_summary_tokens"`
	SummarizationSavedTokens     int64 `json:"summarization_saved_tokens"`

	// TTFT aggregates - count of rows reordered + tokens placed in the
	// now-stable prefix. Dollar impact lands in PromptCacheSavings above.
	TTFTApplied            int64 `json:"ttft_applied"`
	TTFTMessagesReordered  int64 `json:"ttft_messages_reordered"`
	TTFTStablePrefixTokens int64 `json:"ttft_stable_prefix_tokens"`

	// Parallel-tool-execution aggregates - agent latency savings, not $.
	ParallelToolsApplied          int64 `json:"parallel_tools_applied"`
	ParallelToolsTotal            int64 `json:"parallel_tools_total"`
	ParallelToolsParallelCount    int64 `json:"parallel_tools_parallel_count"`
	ParallelToolsSequentialCount  int64 `json:"parallel_tools_sequential_count"`
	ParallelToolsWallClockMs      int64 `json:"parallel_tools_wall_clock_ms"`
	ParallelToolsSerialEstimateMs int64 `json:"parallel_tools_serial_estimate_ms"`
	ParallelToolsLatencySavedMs   int64 `json:"parallel_tools_latency_saved_ms"`

	// Hallucination eval aggregates - count of rows scored + AVG of each
	// metric. AVG is preferred over SUM here because the metrics are 0..1
	// quality scores, not additive savings.
	HallucinationScored               int64   `json:"hallucination_scored"`
	HallucinationAvgFaithfulness      float64 `json:"hallucination_avg_faithfulness"`
	HallucinationAvgAnswerRelevance   float64 `json:"hallucination_avg_answer_relevance"`
	HallucinationAvgCoherence         float64 `json:"hallucination_avg_coherence"`
	HallucinationAvgHelpfulness       float64 `json:"hallucination_avg_helpfulness"`
	HallucinationAvgCitationPrecision float64 `json:"hallucination_avg_citation_precision"`
	HallucinationAvgScore             float64 `json:"hallucination_avg_score"`

	// Hallucination Control aggregates - count of rows where any
	// technique was applied + AVG improvement (heuristic 0..1).
	HallucinationControlApplied        int64   `json:"hallucination_control_applied"`
	HallucinationControlAvgImprovement float64 `json:"hallucination_control_avg_improvement"`
}

// OptimizationHistogramResult is the wire shape returned by
// GetOptimizationHistogram.
type OptimizationHistogramResult struct {
	Buckets           []OptimizationHistogramBucket `json:"buckets"`
	BucketSizeSeconds int64                         `json:"bucket_size_seconds"`
}
...
type ProviderCostHistogramBucket struct {
	...
	ByProviderCacheSavings map[string]float64 `json:"by_provider_cache_savings"`
}
