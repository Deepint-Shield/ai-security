...
func (s *RDBLogStore) cacheHitSQL() string {
	switch s.db.Dialector.Name() {
	case "sqlite":
		return "cache_debug IS NOT NULL AND cache_debug != '' AND json_valid(cache_debug) AND COALESCE(CAST(json_extract(cache_debug, '$.cache_hit') AS INTEGER), 0) = 1"
	case "mysql":
		return "JSON_VALID(cache_debug) AND JSON_UNQUOTE(JSON_EXTRACT(cache_debug, '$.cache_hit')) = 'true'"
	default:
		return "cache_debug IS NOT NULL AND cache_debug != '' AND COALESCE((cache_debug::jsonb->>'cache_hit')::boolean, false)"
	}
}

func (s *RDBLogStore) actualPromptTokensSQL() string {
	return fmt.Sprintf("CASE WHEN %s THEN 0 ELSE prompt_tokens END", s.cacheHitSQL())
}

func (s *RDBLogStore) actualCompletionTokensSQL() string {
	return fmt.Sprintf("CASE WHEN %s THEN 0 ELSE completion_tokens END", s.cacheHitSQL())
}

func (s *RDBLogStore) actualTotalTokensSQL() string {
	return fmt.Sprintf("CASE WHEN %s THEN 0 ELSE total_tokens END", s.cacheHitSQL())
}

func (s *RDBLogStore) actualCostSQL() string {
	return "COALESCE(cost, 0)"
}

func (s *RDBLogStore) effectiveCachedPromptTokensSQL() string {
	return fmt.Sprintf("CASE WHEN %s THEN COALESCE(NULLIF(cached_read_tokens, 0), prompt_tokens) ELSE cached_read_tokens END", s.cacheHitSQL())
}
...
// objectsIncludeAgentic reports whether the explicit object_type filter
// requests any agentic activity type (object_type "agentic.*"). When it does,
// applyFilters skips the default agentic-exclusion so the Agentic Logs tab can
// scope to those rows.
func objectsIncludeAgentic(objects []string) bool {
	for _, o := range objects {
		if strings.HasPrefix(o, "agentic.") {
			return true
		}
	}
	return false
...
func (s *RDBLogStore) applyFilters(baseQuery *gorm.DB, filters SearchFilters) *gorm.DB {
	// Tenant boundary FIRST. Without this, a workspace_id IS NULL row (the
	// legacy pre-workspace fallback) from a different tenant leaks into the
	// active tenant's histogram. Applies before any other filter so the
	// downstream WHERE clauses can't bypass it.
	if tenant := strings.TrimSpace(filters.TenantID); tenant != "" {
		baseQuery = baseQuery.Where("tenant_id = ?", tenant)
	}
	// Active-org workspace allowlist. The tenant_id column is the email-
	// keyed partition (one value per user), so the tenant filter alone
	// can't distinguish DEV / Prod / TEST orgs that share an owner. Pin
	// the result set to workspaces that belong to the active org. An
	// explicitly-empty allowlist means the org has zero workspaces - return
	// no rows (the correct "fresh tenant" empty state).
	if filters.AllowedWorkspaceIDs != nil {
		if len(*filters.AllowedWorkspaceIDs) == 0 {
			baseQuery = baseQuery.Where("1 = 0")
		} else {
			baseQuery = baseQuery.Where("workspace_id IN ?", *filters.AllowedWorkspaceIDs)
		}
	}
	if ws := strings.TrimSpace(filters.WorkspaceID); ws != "" {
		// Workspace narrowing: strictly equal. The earlier "IS NULL OR" arm
		// caused legacy logs (pre-workspace-stamping) to appear under every
		// workspace tab, which surfaced as cross-workspace row flashes on
		// the AI Logs page. NULL-workspace rows are unmigrated and stay
		// hidden until a backfill assigns them a workspace.
	...
	// Agentic activity rows (object_type "agentic.*") live in their own
	// "Agentic Logs" tab. Exclude them from the default AI Logs list AND every
	// aggregate (stats / histograms) so AI Logs stays LLM-only - UNLESS the
	// caller explicitly asked for agentic object types (the Agentic Logs tab
	// passes objects=agentic.tool/agentic.agent), in which case the IN filter
	// above already scopes the query. (On legacy LLM-only data this matches
	// nothing, so existing behaviour is unchanged.)
	if !filters.IncludeAgenticObjects && !objectsIncludeAgentic(filters.Objects) {
		baseQuery = baseQuery.Where("object_type NOT LIKE ?", "agentic.%")
	}
	...
	if len(filters.GuardrailStatuses) > 0 {
		baseQuery = baseQuery.Where("id IN (SELECT DISTINCT request_id FROM guardrail_decisions WHERE decision IN ?)", filters.GuardrailStatuses)
	}
	if len(filters.GuardrailSources) > 0 {
		// Filter by which engine EVALUATED the request, joining decisions →
		// provider bindings. Mirrors the classification in
		// GetGuardrailSourceByRequestIDs so the filter matches the column.
		// Each source label is its own subquery (mixed needs HAVING with
		// both predicates); ORed together for multi-select.
		var conds []string
		var args []interface{}
		const aiOnlySubquery = `id IN (
			SELECT gd.request_id
			...
			HAVING BOOL_OR(gp.provider_type = ?) = true
			   AND BOOL_OR(gp.provider_type IS DISTINCT FROM ? OR gp.provider_type IS NULL) = false
		)`
		const policyOnlySubquery = `id IN (
			SELECT gd.request_id
			FROM guardrail_decisions gd
			LEFT JOIN guardrail_policy_provider_bindings gpb ON gd.policy_id = gpb.policy_id
			LEFT JOIN guardrail_providers gp ON gpb.provider_id = gp.id
			GROUP BY gd.request_id
			HAVING BOOL_OR(gp.provider_type = ?) = false
			   AND BOOL_OR(gp.provider_type IS DISTINCT FROM ? OR gp.provider_type IS NULL) = true
		)`
		const mixedSubquery = `id IN (
			SELECT gd.request_id
			FROM guardrail_decisions gd
			LEFT JOIN guardrail_policy_provider_bindings gpb ON gd.policy_id = gpb.policy_id
			LEFT JOIN guardrail_providers gp ON gpb.provider_id = gp.id
		...
		for _, src := range filters.GuardrailSources {
			switch strings.ToLower(strings.TrimSpace(src)) {
			case "ai_model":
				conds = append(conds, aiOnlySubquery)
				args = append(args, "deepintshield_models", "deepintshield_models")
			case "policy":
				conds = append(conds, policyOnlySubquery)
				args = append(args, "deepintshield_models", "deepintshield_models")
			case "mixed":
				conds = append(conds, mixedSubquery)
				args = append(args, "deepintshield_models", "deepintshield_models")
			}
		}
		if len(conds) > 0 {
			baseQuery = baseQuery.Where(strings.Join(conds, " OR "), args...)
		}
	}
	...
	if filters.MinTokens != nil {
		baseQuery = baseQuery.Where(s.actualTotalTokensSQL()+" >= ?", *filters.MinTokens)
	}
	if filters.MaxTokens != nil {
		baseQuery = baseQuery.Where(s.actualTotalTokensSQL()+" <= ?", *filters.MaxTokens)
	}
	if filters.MinCost != nil {
		baseQuery = baseQuery.Where(s.actualCostSQL()+" >= ?", *filters.MinCost)
	}
	if filters.MaxCost != nil {
		baseQuery = baseQuery.Where(s.actualCostSQL()+" <= ?", *filters.MaxCost)
	}
...
func (s *RDBLogStore) SearchLogs(ctx context.Context, filters SearchFilters, pagination PaginationOptions) (*SearchResult, error) {
	// AI Logs is LLM-only; agentic activity rows (object_type "agentic.*") have
	// their own "Agentic Logs" view, which scopes via objects=agentic.tool/
	// agentic.agent. So the list uses the same default agentic-exclusion as the
	// aggregates (see applyFilters) - unless the caller explicitly requests
	// agentic objects.
	...
	case "tokens":
		orderClause = s.actualTotalTokensSQL() + " " + direction
	case "cost":
		orderClause = s.actualCostSQL() + " " + direction
	default:
		orderClause = "timestamp " + direction
	}
	...
	g.Go(func() error {
		...
		dataQuery = dataQuery.Order(orderClause).Limit(limit)
		// Export pipeline wants the full request/response JSON; dashboard
		// list views stay on the trimmed projection (output_message and
		// friends are heavy TEXT and would balloon the response).
		if !filters.IncludeBodies {
			dataQuery = dataQuery.Select(s.listSelectColumns())
		}
...
func (s *RDBLogStore) listSelectColumns() string {
	baseCols := strings.Join([]string{
		...
		"latency", "token_usage", "cost", "cache_savings", "cache_debug", "status", "error_details", "stream",
...
func (s *RDBLogStore) GetStats(ctx context.Context, filters SearchFilters) (*SearchStats, error) {
	...
	if totalCount > 0 {
		// Single query for all completed-request stats: counts, latency, tokens, cost, savings
		var result struct {
			...
			TotalCacheSavings       sql.NullFloat64 `gorm:"column:total_cache_savings"`
			TotalPromptCacheSavings sql.NullFloat64 `gorm:"column:total_prompt_cache_savings"`
		}
		...
		if err := statsQuery.Select(`
			...
			COALESCE(SUM(` + s.actualTotalTokensSQL() + `), 0) as total_tokens,
			COALESCE(SUM(` + s.actualCostSQL() + `), 0) as total_cost,
			COALESCE(SUM(cache_savings), 0) as total_cache_savings,
			COALESCE(SUM(prompt_cache_savings), 0) as total_prompt_cache_savings
		...
		if completedCount > 0 {
			...
			if result.TotalCacheSavings.Valid {
				stats.TotalCacheSavings = result.TotalCacheSavings.Float64
			}
			if result.TotalPromptCacheSavings.Valid {
				stats.TotalPromptCacheSavings = result.TotalPromptCacheSavings.Float64
			}
		}
	}
...
func (s *RDBLogStore) GetTokenHistogram(ctx context.Context, filters SearchFilters, bucketSizeSeconds int64) (*TokenHistogramResult, error) {
	...
	case "sqlite":
		selectClause = fmt.Sprintf(`
			...
			COALESCE(SUM(%s), 0) as completion_tokens,
			COALESCE(SUM(%s), 0) as total_tokens,
			COALESCE(SUM(cached_read_tokens), 0) as cached_read_tokens,
			COALESCE(SUM(%s), 0) as effective_cached_prompt_tokens
		`, bucketSizeSeconds, bucketSizeSeconds, s.actualPromptTokensSQL(), s.actualCompletionTokensSQL(), s.actualTotalTokensSQL(), s.effectiveCachedPromptTokensSQL())
	case "mysql":
		selectClause = fmt.Sprintf(`
			(FLOOR(UNIX_TIMESTAMP(timestamp) / %d) * %d) as bucket_timestamp,
			COALESCE(SUM(%s), 0) as prompt_tokens,
	...
	resultMap := make(map[int64]struct {
		...
		EffectiveCachedPromptTokens int64
	})
	for _, r := range results {
		resultMap[r.BucketTimestamp] = struct {
			PromptTokens                int64
			...
			EffectiveCachedPromptTokens: r.EffectiveCachedPromptTokens,
		}
	}
	...
	for i, ts := range allTimestamps {
		if data, exists := resultMap[ts]; exists {
			buckets[i] = TokenHistogramBucket{
				...
				EffectiveCachedPromptTokens: data.EffectiveCachedPromptTokens,
			}
		} else {
			buckets[i] = TokenHistogramBucket{
				Timestamp: time.Unix(ts, 0).UTC(),
...
func (s *RDBLogStore) GetCacheHistogram(ctx context.Context, filters SearchFilters, bucketSizeSeconds int64) (*CacheHistogramResult, error) {
	if bucketSizeSeconds <= 0 {
	...
	baseQuery = s.applyFilters(baseQuery, filters)
	baseQuery = baseQuery.Where("status IN ?", []string{"success", "error"})
	baseQuery = baseQuery.Where("cache_debug IS NOT NULL AND cache_debug != ''")

	var results []struct {
		BucketTimestamp         int64 `gorm:"column:bucket_timestamp"`
		CacheRequests           int64 `gorm:"column:cache_requests"`
		CacheHits               int64 `gorm:"column:cache_hits"`
		CacheMisses             int64 `gorm:"column:cache_misses"`
		DirectHits              int64 `gorm:"column:direct_hits"`
		SemanticHits            int64 `gorm:"column:semantic_hits"`
		UserScopeHits           int64 `gorm:"column:user_scope_hits"`
		UseCaseScopeHits        int64 `gorm:"column:use_case_scope_hits"`
		SessionScopeHits        int64 `gorm:"column:session_scope_hits"`
		VirtualKeyScopeHits     int64 `gorm:"column:virtual_key_scope_hits"`
		CustomMetadataScopeHits int64 `gorm:"column:custom_metadata_scope_hits"`
		TenantScopeHits         int64 `gorm:"column:tenant_scope_hits"`
		AutoScopedHits          int64 `gorm:"column:auto_scoped_hits"`
		ExplicitOverrideHits    int64 `gorm:"column:explicit_override_hits"`
		DefaultScopeHits        int64 `gorm:"column:default_scope_hits"`
		RequestFallbackHits     int64 `gorm:"column:request_fallback_hits"`
		SemanticSuppressions    int64 `gorm:"column:semantic_suppressions"`
	}
	...
	case "sqlite":
		selectClause = fmt.Sprintf(`
			...
			COALESCE(SUM(CASE WHEN COALESCE(CAST(json_extract(cache_debug, '$.cache_hit') AS INTEGER), 0) = 1 THEN 1 ELSE 0 END), 0) as cache_hits,
			COALESCE(SUM(CASE WHEN COALESCE(CAST(json_extract(cache_debug, '$.cache_hit') AS INTEGER), 0) = 0 THEN 1 ELSE 0 END), 0) as cache_misses,
			COALESCE(SUM(CASE WHEN COALESCE(CAST(json_extract(cache_debug, '$.cache_hit') AS INTEGER), 0) = 1 AND json_extract(cache_debug, '$.hit_type') = 'direct' THEN 1 ELSE 0 END), 0) as direct_hits,
			COALESCE(SUM(CASE WHEN COALESCE(CAST(json_extract(cache_debug, '$.cache_hit') AS INTEGER), 0) = 1 AND json_extract(cache_debug, '$.hit_type') = 'semantic' THEN 1 ELSE 0 END), 0) as semantic_hits,
			COALESCE(SUM(CASE WHEN COALESCE(CAST(json_extract(cache_debug, '$.cache_hit') AS INTEGER), 0) = 1 AND json_extract(cache_debug, '$.scope_type') = 'user' THEN 1 ELSE 0 END), 0) as user_scope_hits,
			COALESCE(SUM(CASE WHEN COALESCE(CAST(json_extract(cache_debug, '$.cache_hit') AS INTEGER), 0) = 1 AND json_extract(cache_debug, '$.scope_type') = 'use_case' THEN 1 ELSE 0 END), 0) as use_case_scope_hits,
			COALESCE(SUM(CASE WHEN COALESCE(CAST(json_extract(cache_debug, '$.cache_hit') AS INTEGER), 0) = 1 AND json_extract(cache_debug, '$.scope_type') = 'session' THEN 1 ELSE 0 END), 0) as session_scope_hits,
			COALESCE(SUM(CASE WHEN COALESCE(CAST(json_extract(cache_debug, '$.cache_hit') AS INTEGER), 0) = 1 AND json_extract(cache_debug, '$.scope_type') = 'virtual_key' THEN 1 ELSE 0 END), 0) as virtual_key_scope_hits,
			COALESCE(SUM(CASE WHEN COALESCE(CAST(json_extract(cache_debug, '$.cache_hit') AS INTEGER), 0) = 1 AND json_extract(cache_debug, '$.scope_type') = 'custom_metadata' THEN 1 ELSE 0 END), 0) as custom_metadata_scope_hits,
			COALESCE(SUM(CASE WHEN COALESCE(CAST(json_extract(cache_debug, '$.cache_hit') AS INTEGER), 0) = 1 AND json_extract(cache_debug, '$.scope_type') = 'tenant' THEN 1 ELSE 0 END), 0) as tenant_scope_hits,
			COALESCE(SUM(CASE WHEN COALESCE(CAST(json_extract(cache_debug, '$.cache_hit') AS INTEGER), 0) = 1 AND json_extract(cache_debug, '$.scope_source') NOT IN ('explicit_cache_key', 'default_cache_key', 'request_hash_fallback') AND json_extract(cache_debug, '$.scope_source') IS NOT NULL THEN 1 ELSE 0 END), 0) as auto_scoped_hits,
			COALESCE(SUM(CASE WHEN COALESCE(CAST(json_extract(cache_debug, '$.cache_hit') AS INTEGER), 0) = 1 AND json_extract(cache_debug, '$.scope_source') = 'explicit_cache_key' THEN 1 ELSE 0 END), 0) as explicit_override_hits,
			COALESCE(SUM(CASE WHEN COALESCE(CAST(json_extract(cache_debug, '$.cache_hit') AS INTEGER), 0) = 1 AND json_extract(cache_debug, '$.scope_source') = 'default_cache_key' THEN 1 ELSE 0 END), 0) as default_scope_hits,
			COALESCE(SUM(CASE WHEN COALESCE(CAST(json_extract(cache_debug, '$.cache_hit') AS INTEGER), 0) = 1 AND json_extract(cache_debug, '$.scope_source') = 'request_hash_fallback' THEN 1 ELSE 0 END), 0) as request_fallback_hits,
			COALESCE(SUM(CASE WHEN COALESCE(json_extract(cache_debug, '$.semantic_suppressed_reason'), '') != '' THEN 1 ELSE 0 END), 0) as semantic_suppressions
		`, bucketSizeSeconds, bucketSizeSeconds)
	case "mysql":
		selectClause = fmt.Sprintf(`
			(FLOOR(UNIX_TIMESTAMP(timestamp) / %d) * %d) as bucket_timestamp,
			COUNT(*) as cache_requests,
			COALESCE(SUM(CASE WHEN JSON_UNQUOTE(JSON_EXTRACT(cache_debug, '$.cache_hit')) = 'true' THEN 1 ELSE 0 END), 0) as cache_hits,
			COALESCE(SUM(CASE WHEN COALESCE(JSON_UNQUOTE(JSON_EXTRACT(cache_debug, '$.cache_hit')), 'false') != 'true' THEN 1 ELSE 0 END), 0) as cache_misses,
			COALESCE(SUM(CASE WHEN JSON_UNQUOTE(JSON_EXTRACT(cache_debug, '$.cache_hit')) = 'true' AND JSON_UNQUOTE(JSON_EXTRACT(cache_debug, '$.hit_type')) = 'direct' THEN 1 ELSE 0 END), 0) as direct_hits,
			COALESCE(SUM(CASE WHEN JSON_UNQUOTE(JSON_EXTRACT(cache_debug, '$.cache_hit')) = 'true' AND JSON_UNQUOTE(JSON_EXTRACT(cache_debug, '$.hit_type')) = 'semantic' THEN 1 ELSE 0 END), 0) as semantic_hits,
			COALESCE(SUM(CASE WHEN JSON_UNQUOTE(JSON_EXTRACT(cache_debug, '$.cache_hit')) = 'true' AND JSON_UNQUOTE(JSON_EXTRACT(cache_debug, '$.scope_type')) = 'user' THEN 1 ELSE 0 END), 0) as user_scope_hits,
			COALESCE(SUM(CASE WHEN JSON_UNQUOTE(JSON_EXTRACT(cache_debug, '$.cache_hit')) = 'true' AND JSON_UNQUOTE(JSON_EXTRACT(cache_debug, '$.scope_type')) = 'use_case' THEN 1 ELSE 0 END), 0) as use_case_scope_hits,
			COALESCE(SUM(CASE WHEN JSON_UNQUOTE(JSON_EXTRACT(cache_debug, '$.cache_hit')) = 'true' AND JSON_UNQUOTE(JSON_EXTRACT(cache_debug, '$.scope_type')) = 'session' THEN 1 ELSE 0 END), 0) as session_scope_hits,
			COALESCE(SUM(CASE WHEN JSON_UNQUOTE(JSON_EXTRACT(cache_debug, '$.cache_hit')) = 'true' AND JSON_UNQUOTE(JSON_EXTRACT(cache_debug, '$.scope_type')) = 'virtual_key' THEN 1 ELSE 0 END), 0) as virtual_key_scope_hits,
			COALESCE(SUM(CASE WHEN JSON_UNQUOTE(JSON_EXTRACT(cache_debug, '$.cache_hit')) = 'true' AND JSON_UNQUOTE(JSON_EXTRACT(cache_debug, '$.scope_type')) = 'custom_metadata' THEN 1 ELSE 0 END), 0) as custom_metadata_scope_hits,
			COALESCE(SUM(CASE WHEN JSON_UNQUOTE(JSON_EXTRACT(cache_debug, '$.cache_hit')) = 'true' AND JSON_UNQUOTE(JSON_EXTRACT(cache_debug, '$.scope_type')) = 'tenant' THEN 1 ELSE 0 END), 0) as tenant_scope_hits,
			COALESCE(SUM(CASE WHEN JSON_UNQUOTE(JSON_EXTRACT(cache_debug, '$.cache_hit')) = 'true' AND JSON_UNQUOTE(JSON_EXTRACT(cache_debug, '$.scope_source')) NOT IN ('explicit_cache_key', 'default_cache_key', 'request_hash_fallback') AND JSON_UNQUOTE(JSON_EXTRACT(cache_debug, '$.scope_source')) IS NOT NULL THEN 1 ELSE 0 END), 0) as auto_scoped_hits,
			COALESCE(SUM(CASE WHEN JSON_UNQUOTE(JSON_EXTRACT(cache_debug, '$.cache_hit')) = 'true' AND JSON_UNQUOTE(JSON_EXTRACT(cache_debug, '$.scope_source')) = 'explicit_cache_key' THEN 1 ELSE 0 END), 0) as explicit_override_hits,
			COALESCE(SUM(CASE WHEN JSON_UNQUOTE(JSON_EXTRACT(cache_debug, '$.cache_hit')) = 'true' AND JSON_UNQUOTE(JSON_EXTRACT(cache_debug, '$.scope_source')) = 'default_cache_key' THEN 1 ELSE 0 END), 0) as default_scope_hits,
			COALESCE(SUM(CASE WHEN JSON_UNQUOTE(JSON_EXTRACT(cache_debug, '$.cache_hit')) = 'true' AND JSON_UNQUOTE(JSON_EXTRACT(cache_debug, '$.scope_source')) = 'request_hash_fallback' THEN 1 ELSE 0 END), 0) as request_fallback_hits,
			COALESCE(SUM(CASE WHEN COALESCE(JSON_UNQUOTE(JSON_EXTRACT(cache_debug, '$.semantic_suppressed_reason')), '') != '' THEN 1 ELSE 0 END), 0) as semantic_suppressions
		`, bucketSizeSeconds, bucketSizeSeconds)
	default:
		selectClause = fmt.Sprintf(`
			CAST(FLOOR(EXTRACT(EPOCH FROM timestamp) / %d) * %d AS BIGINT) as bucket_timestamp,
			COUNT(*) as cache_requests,
			COALESCE(SUM(CASE WHEN COALESCE((cache_debug::jsonb->>'cache_hit')::boolean, false) THEN 1 ELSE 0 END), 0) as cache_hits,
			COALESCE(SUM(CASE WHEN NOT COALESCE((cache_debug::jsonb->>'cache_hit')::boolean, false) THEN 1 ELSE 0 END), 0) as cache_misses,
			COALESCE(SUM(CASE WHEN COALESCE((cache_debug::jsonb->>'cache_hit')::boolean, false) AND cache_debug::jsonb->>'hit_type' = 'direct' THEN 1 ELSE 0 END), 0) as direct_hits,
			COALESCE(SUM(CASE WHEN COALESCE((cache_debug::jsonb->>'cache_hit')::boolean, false) AND cache_debug::jsonb->>'hit_type' = 'semantic' THEN 1 ELSE 0 END), 0) as semantic_hits,
			COALESCE(SUM(CASE WHEN COALESCE((cache_debug::jsonb->>'cache_hit')::boolean, false) AND cache_debug::jsonb->>'scope_type' = 'user' THEN 1 ELSE 0 END), 0) as user_scope_hits,
			COALESCE(SUM(CASE WHEN COALESCE((cache_debug::jsonb->>'cache_hit')::boolean, false) AND cache_debug::jsonb->>'scope_type' = 'use_case' THEN 1 ELSE 0 END), 0) as use_case_scope_hits,
			COALESCE(SUM(CASE WHEN COALESCE((cache_debug::jsonb->>'cache_hit')::boolean, false) AND cache_debug::jsonb->>'scope_type' = 'session' THEN 1 ELSE 0 END), 0) as session_scope_hits,
			COALESCE(SUM(CASE WHEN COALESCE((cache_debug::jsonb->>'cache_hit')::boolean, false) AND cache_debug::jsonb->>'scope_type' = 'virtual_key' THEN 1 ELSE 0 END), 0) as virtual_key_scope_hits,
			COALESCE(SUM(CASE WHEN COALESCE((cache_debug::jsonb->>'cache_hit')::boolean, false) AND cache_debug::jsonb->>'scope_type' = 'custom_metadata' THEN 1 ELSE 0 END), 0) as custom_metadata_scope_hits,
			COALESCE(SUM(CASE WHEN COALESCE((cache_debug::jsonb->>'cache_hit')::boolean, false) AND cache_debug::jsonb->>'scope_type' = 'tenant' THEN 1 ELSE 0 END), 0) as tenant_scope_hits,
			COALESCE(SUM(CASE WHEN COALESCE((cache_debug::jsonb->>'cache_hit')::boolean, false) AND cache_debug::jsonb->>'scope_source' NOT IN ('explicit_cache_key', 'default_cache_key', 'request_hash_fallback') AND cache_debug::jsonb->>'scope_source' IS NOT NULL THEN 1 ELSE 0 END), 0) as auto_scoped_hits,
			COALESCE(SUM(CASE WHEN COALESCE((cache_debug::jsonb->>'cache_hit')::boolean, false) AND cache_debug::jsonb->>'scope_source' = 'explicit_cache_key' THEN 1 ELSE 0 END), 0) as explicit_override_hits,
			COALESCE(SUM(CASE WHEN COALESCE((cache_debug::jsonb->>'cache_hit')::boolean, false) AND cache_debug::jsonb->>'scope_source' = 'default_cache_key' THEN 1 ELSE 0 END), 0) as default_scope_hits,
			COALESCE(SUM(CASE WHEN COALESCE((cache_debug::jsonb->>'cache_hit')::boolean, false) AND cache_debug::jsonb->>'scope_source' = 'request_hash_fallback' THEN 1 ELSE 0 END), 0) as request_fallback_hits,
			COALESCE(SUM(CASE WHEN COALESCE(cache_debug::jsonb->>'semantic_suppressed_reason', '') != '' THEN 1 ELSE 0 END), 0) as semantic_suppressions
		`, bucketSizeSeconds, bucketSizeSeconds)
	}
	...
	if err := baseQuery.
		...
		Order("bucket_timestamp ASC").
		Find(&results).Error; err != nil {
		return nil, fmt.Errorf("failed to get cache histogram: %w", err)
	}
	...
	resultMap := make(map[int64]struct {
		...
		SemanticHits            int64
		UserScopeHits           int64
		UseCaseScopeHits        int64
		SessionScopeHits        int64
		VirtualKeyScopeHits     int64
		CustomMetadataScopeHits int64
		TenantScopeHits         int64
		AutoScopedHits          int64
		ExplicitOverrideHits    int64
		DefaultScopeHits        int64
		RequestFallbackHits     int64
		SemanticSuppressions    int64
	}, len(results))
	...
	for _, r := range results {
		resultMap[r.BucketTimestamp] = struct {
			...
			CacheRequests:           r.CacheRequests,
			CacheHits:               r.CacheHits,
			CacheMisses:             r.CacheMisses,
			DirectHits:              r.DirectHits,
			SemanticHits:            r.SemanticHits,
			UserScopeHits:           r.UserScopeHits,
			UseCaseScopeHits:        r.UseCaseScopeHits,
			SessionScopeHits:        r.SessionScopeHits,
			VirtualKeyScopeHits:     r.VirtualKeyScopeHits,
			CustomMetadataScopeHits: r.CustomMetadataScopeHits,
			TenantScopeHits:         r.TenantScopeHits,
			AutoScopedHits:          r.AutoScopedHits,
			ExplicitOverrideHits:    r.ExplicitOverrideHits,
			DefaultScopeHits:        r.DefaultScopeHits,
			RequestFallbackHits:     r.RequestFallbackHits,
			SemanticSuppressions:    r.SemanticSuppressions,
		}
	}

	allTimestamps := generateBucketTimestamps(filters.StartTime, filters.EndTime, bucketSizeSeconds)
	if len(allTimestamps) == 0 {
		buckets := make([]CacheHistogramBucket, len(results))
		for i, r := range results {
			buckets[i] = CacheHistogramBucket{
				Timestamp:               time.Unix(r.BucketTimestamp, 0).UTC(),
				CacheRequests:           r.CacheRequests,
				CacheHits:               r.CacheHits,
				CacheMisses:             r.CacheMisses,
		...
		return &CacheHistogramResult{
			Buckets:           buckets,
			BucketSizeSeconds: bucketSizeSeconds,
		}, nil
	}

	buckets := make([]CacheHistogramBucket, len(allTimestamps))
	for i, ts := range allTimestamps {
		if data, exists := resultMap[ts]; exists {
			buckets[i] = CacheHistogramBucket{
				Timestamp:               time.Unix(ts, 0).UTC(),
				CacheRequests:           data.CacheRequests,
				CacheHits:               data.CacheHits,
				CacheMisses:             data.CacheMisses,
				DirectHits:              data.DirectHits,
				SemanticHits:            data.SemanticHits,
				UserScopeHits:           data.UserScopeHits,
				UseCaseScopeHits:        data.UseCaseScopeHits,
				SessionScopeHits:        data.SessionScopeHits,
				VirtualKeyScopeHits:     data.VirtualKeyScopeHits,
				CustomMetadataScopeHits: data.CustomMetadataScopeHits,
				TenantScopeHits:         data.TenantScopeHits,
				AutoScopedHits:          data.AutoScopedHits,
				ExplicitOverrideHits:    data.ExplicitOverrideHits,
				DefaultScopeHits:        data.DefaultScopeHits,
				RequestFallbackHits:     data.RequestFallbackHits,
				SemanticSuppressions:    data.SemanticSuppressions,
			}
		} else {
			buckets[i] = CacheHistogramBucket{
				Timestamp: time.Unix(ts, 0).UTC(),
...
func (s *RDBLogStore) RefreshDashboardAggregates(ctx context.Context) error {
	if s.db == nil || s.db.Dialector.Name() != "postgres" {
		return nil
	}
	return refreshMatViews(ctx, s.db)
}
...
func (s *RDBLogStore) GetCostHistogram(ctx context.Context, filters SearchFilters, bucketSizeSeconds int64) (*CostHistogramResult, error) {
	...
	baseQuery = baseQuery.Where("status IN ?", []string{"success", "error"})
	baseQuery = baseQuery.Where("(cost IS NOT NULL AND cost > 0) OR (cache_savings IS NOT NULL AND cache_savings > 0)")
	...
	for _, r := range results {
		...
		if bucket, exists := bucketMap[r.BucketTimestamp]; exists {
			...
			bucket.CacheSavings += r.CacheSavings
			bucket.ByModel[r.Model] = r.TotalCost
			bucket.ByModelCacheSavings[r.Model] = r.CacheSavings
		} else {
			bucketMap[r.BucketTimestamp] = &CostHistogramBucket{
				Timestamp:           time.Unix(r.BucketTimestamp, 0).UTC(),
				TotalCost:           r.TotalCost,
				CacheSavings:        r.CacheSavings,
				ByModel:             map[string]float64{r.Model: r.TotalCost},
				ByModelCacheSavings: map[string]float64{r.Model: r.CacheSavings},
			}
		}
	}
...
// GetOptimizationHistogram returns the per-source savings + categorical
// aggregates that power the Cost Optimization analytics tab. Single query
// per dialect - every aggregate is a SUM / COUNT / AVG over the indexed
// optimization columns stamped at log-write time, so this is cheap to run
// next to the existing histogram fetches.
//
// Categorical mixes (effort distribution, cascade source mix, batch provider
// mix) come back as separate result-sets keyed by (bucket, label) and merged
// into the OptimizationHistogramBucket here in Go - simpler than nested
// jsonb aggregation, and the row count is bounded by `time-buckets ×
// distinct-labels`, which is tiny.
func (s *RDBLogStore) GetOptimizationHistogram(ctx context.Context, filters SearchFilters, bucketSizeSeconds int64) (*OptimizationHistogramResult, error) {
	if bucketSizeSeconds <= 0 {
		bucketSizeSeconds = 3600
	}
	dialect := s.db.Dialector.Name()

	// Bucket-timestamp expression - same shape used by every histogram in
	// this file so all charts align on the same x-axis points.
	var bucketExpr string
	switch dialect {
	case "sqlite":
		bucketExpr = fmt.Sprintf("(CAST(strftime('%%s', timestamp) AS INTEGER) / %d) * %d", bucketSizeSeconds, bucketSizeSeconds)
	case "mysql":
		bucketExpr = fmt.Sprintf("(FLOOR(UNIX_TIMESTAMP(timestamp) / %d) * %d)", bucketSizeSeconds, bucketSizeSeconds)
	default:
		bucketExpr = fmt.Sprintf("CAST(FLOOR(EXTRACT(EPOCH FROM timestamp) / %d) * %d AS BIGINT)", bucketSizeSeconds, bucketSizeSeconds)
	}

	// 1. Numeric aggregates - one row per bucket.
	type numericRow struct {
		BucketTimestamp                    int64   `gorm:"column:bucket_timestamp"`
		TotalSavings                       float64 `gorm:"column:total_savings"`
		PromptCacheSavings                 float64 `gorm:"column:prompt_cache_savings"`
		ReasoningSavings                   float64 `gorm:"column:reasoning_savings"`
		CompressionSavings                 float64 `gorm:"column:compression_savings"`
		RagSavings                         float64 `gorm:"column:rag_savings"`
		CompressionApplied                 int64   `gorm:"column:compression_applied_count"`
		CompressionCacheHits               int64   `gorm:"column:compression_cache_hits"`
		CompressionOriginalTokens          int64   `gorm:"column:compression_original_tokens"`
		CompressionCompressedTokens        int64   `gorm:"column:compression_compressed_tokens"`
		CompressionAvgLatencyMs            float64 `gorm:"column:compression_avg_latency_ms"`
		ReasoningApplied                   int64   `gorm:"column:reasoning_applied_count"`
		ReasoningSampled                   int64   `gorm:"column:reasoning_sampled_count"`
		CascadeAvgScore                    float64 `gorm:"column:cascade_avg_score"`
		CascadeEscalations                 int64   `gorm:"column:cascade_escalations"`
		BatchEligible                      int64   `gorm:"column:batch_eligible_count"`
		RagApplied                         int64   `gorm:"column:rag_applied_count"`
		RagChunksDetected                  int64   `gorm:"column:rag_chunks_detected"`
		RagChunksKept                      int64   `gorm:"column:rag_chunks_kept"`
		RagTrimmedTokens                   int64   `gorm:"column:rag_trimmed_tokens"`
		RagOriginalTokens                  int64   `gorm:"column:rag_original_tokens"`
		RagAvgRerankLatencyMs              float64 `gorm:"column:rag_avg_rerank_latency_ms"`
		RagCacheHits                       int64   `gorm:"column:rag_cache_hits"`
		SummarizationSavings               float64 `gorm:"column:summarization_savings"`
		ConsistencySavings                 float64 `gorm:"column:consistency_savings"`
		SummarizationApplied               int64   `gorm:"column:summarization_applied_count"`
		SummarizationCacheHits             int64   `gorm:"column:summarization_cache_hits"`
		SummarizationAsyncKickoffs         int64   `gorm:"column:summarization_async_kickoffs"`
		SummarizationTurnsSummarized       int64   `gorm:"column:summarization_turns_summarized"`
		SummarizationOriginalTokens        int64   `gorm:"column:summarization_original_tokens"`
		SummarizationSummaryTokens         int64   `gorm:"column:summarization_summary_tokens"`
		SummarizationSavedTokens           int64   `gorm:"column:summarization_saved_tokens"`
		TTFTApplied                        int64   `gorm:"column:ttft_applied_count"`
		TTFTMessagesReordered              int64   `gorm:"column:ttft_messages_reordered"`
		TTFTStablePrefixTokens             int64   `gorm:"column:ttft_stable_prefix_tokens"`
		ParallelToolsApplied               int64   `gorm:"column:parallel_tools_applied_count"`
		ParallelToolsTotal                 int64   `gorm:"column:parallel_tools_total"`
		ParallelToolsParallelCount         int64   `gorm:"column:parallel_tools_parallel_count"`
		ParallelToolsSequentialCount       int64   `gorm:"column:parallel_tools_sequential_count"`
		ParallelToolsWallClockMs           int64   `gorm:"column:parallel_tools_wall_clock_ms"`
		ParallelToolsSerialEstimateMs      int64   `gorm:"column:parallel_tools_serial_estimate_ms"`
		ParallelToolsLatencySavedMs        int64   `gorm:"column:parallel_tools_latency_saved_ms"`
		HallucinationScored                int64   `gorm:"column:hallucination_scored"`
		HallucinationAvgFaithfulness       float64 `gorm:"column:hallucination_avg_faithfulness"`
		HallucinationAvgAnswerRelevance    float64 `gorm:"column:hallucination_avg_answer_relevance"`
		HallucinationAvgCoherence          float64 `gorm:"column:hallucination_avg_coherence"`
		HallucinationAvgHelpfulness        float64 `gorm:"column:hallucination_avg_helpfulness"`
		HallucinationAvgCitationPrecision  float64 `gorm:"column:hallucination_avg_citation_precision"`
		HallucinationAvgScore              float64 `gorm:"column:hallucination_avg_score"`
		HallucinationControlApplied        int64   `gorm:"column:hallucination_control_applied"`
		HallucinationControlAvgImprovement float64 `gorm:"column:hallucination_control_avg_improvement"`
	}
	var rows []numericRow
	baseSelect := fmt.Sprintf(`
		%s as bucket_timestamp,
		COALESCE(SUM(cache_savings), 0) as total_savings,
		COALESCE(SUM(prompt_cache_savings), 0) as prompt_cache_savings,
		COALESCE(SUM(reasoning_savings), 0) as reasoning_savings,
		COALESCE(SUM(compression_savings), 0) as compression_savings,
		COALESCE(SUM(rag_savings), 0) as rag_savings,
		COALESCE(SUM(CASE WHEN compression_applied = true THEN 1 ELSE 0 END), 0) as compression_applied_count,
		COALESCE(SUM(CASE WHEN compression_cache_hit = true THEN 1 ELSE 0 END), 0) as compression_cache_hits,
		COALESCE(SUM(compression_original_tokens), 0) as compression_original_tokens,
		COALESCE(SUM(compression_compressed_tokens), 0) as compression_compressed_tokens,
		COALESCE(AVG(NULLIF(compression_latency_ms, 0)), 0) as compression_avg_latency_ms,
		COALESCE(SUM(CASE WHEN reasoning_applied = true THEN 1 ELSE 0 END), 0) as reasoning_applied_count,
		COALESCE(SUM(CASE WHEN reasoning_sampled = true THEN 1 ELSE 0 END), 0) as reasoning_sampled_count,
		COALESCE(AVG(cascade_score), 0) as cascade_avg_score,
		COALESCE(SUM(CASE WHEN cascade_needs_escalation = true THEN 1 ELSE 0 END), 0) as cascade_escalations,
		COALESCE(SUM(CASE WHEN batch_eligible = true THEN 1 ELSE 0 END), 0) as batch_eligible_count,
		COALESCE(SUM(CASE WHEN rag_applied = true THEN 1 ELSE 0 END), 0) as rag_applied_count,
		COALESCE(SUM(rag_chunks_detected), 0) as rag_chunks_detected,
		COALESCE(SUM(rag_chunks_kept), 0) as rag_chunks_kept,
		COALESCE(SUM(rag_trimmed_tokens), 0) as rag_trimmed_tokens,
		COALESCE(SUM(rag_original_tokens), 0) as rag_original_tokens,
		COALESCE(AVG(NULLIF(rag_rerank_latency_ms, 0)), 0) as rag_avg_rerank_latency_ms,
		COALESCE(SUM(CASE WHEN rag_cache_hit = true THEN 1 ELSE 0 END), 0) as rag_cache_hits,
		COALESCE(SUM(summarization_savings), 0) as summarization_savings,
		COALESCE(SUM(consistency_savings), 0) as consistency_savings,
		COALESCE(SUM(CASE WHEN summarization_applied = true THEN 1 ELSE 0 END), 0) as summarization_applied_count,
		COALESCE(SUM(CASE WHEN summarization_cache_hit = true THEN 1 ELSE 0 END), 0) as summarization_cache_hits,
		COALESCE(SUM(CASE WHEN summarization_async_kickoff = true THEN 1 ELSE 0 END), 0) as summarization_async_kickoffs,
		COALESCE(SUM(summarization_turns_summarized), 0) as summarization_turns_summarized,
		COALESCE(SUM(summarization_original_tokens), 0) as summarization_original_tokens,
		COALESCE(SUM(summarization_summary_tokens), 0) as summarization_summary_tokens,
		COALESCE(SUM(summarization_saved_tokens), 0) as summarization_saved_tokens,
		COALESCE(SUM(CASE WHEN ttft_applied = true THEN 1 ELSE 0 END), 0) as ttft_applied_count,
		COALESCE(SUM(ttft_messages_reordered), 0) as ttft_messages_reordered,
		COALESCE(SUM(ttft_stable_prefix_tokens), 0) as ttft_stable_prefix_tokens,
		COALESCE(SUM(CASE WHEN parallel_tools_applied = true THEN 1 ELSE 0 END), 0) as parallel_tools_applied_count,
		COALESCE(SUM(parallel_tools_total), 0) as parallel_tools_total,
		COALESCE(SUM(parallel_tools_parallel_count), 0) as parallel_tools_parallel_count,
		COALESCE(SUM(parallel_tools_sequential_count), 0) as parallel_tools_sequential_count,
		COALESCE(SUM(parallel_tools_wall_clock_ms), 0) as parallel_tools_wall_clock_ms,
		COALESCE(SUM(parallel_tools_serial_estimate_ms), 0) as parallel_tools_serial_estimate_ms,
		COALESCE(SUM(parallel_tools_latency_saved_ms), 0) as parallel_tools_latency_saved_ms,
		COALESCE(SUM(CASE WHEN hallucination_applied = true THEN 1 ELSE 0 END), 0) as hallucination_scored,
		COALESCE(AVG(hallucination_faithfulness), 0) as hallucination_avg_faithfulness,
		COALESCE(AVG(hallucination_answer_relevance), 0) as hallucination_avg_answer_relevance,
		COALESCE(AVG(hallucination_coherence), 0) as hallucination_avg_coherence,
		COALESCE(AVG(hallucination_helpfulness), 0) as hallucination_avg_helpfulness,
		COALESCE(AVG(hallucination_citation_precision), 0) as hallucination_avg_citation_precision,
		COALESCE(AVG(hallucination_score), 0) as hallucination_avg_score,
		COALESCE(SUM(CASE WHEN hallucination_control_applied = true THEN 1 ELSE 0 END), 0) as hallucination_control_applied,
		COALESCE(AVG(hallucination_control_improvement), 0) as hallucination_control_avg_improvement
	`, bucketExpr)
	q := s.db.WithContext(ctx).Model(&Log{})
	q = s.applyFilters(q, filters)
	if err := q.Select(baseSelect).Group("bucket_timestamp").Order("bucket_timestamp ASC").Find(&rows).Error; err != nil {
		return nil, fmt.Errorf("failed to get optimization histogram numeric aggregates: %w", err)
	}

	bucketMap := make(map[int64]*OptimizationHistogramBucket, len(rows))
	for _, r := range rows {
		semantic := r.TotalSavings - r.PromptCacheSavings - r.ReasoningSavings - r.CompressionSavings - r.RagSavings - r.SummarizationSavings - r.ConsistencySavings
		if semantic < 0 {
			semantic = 0
		}
		ratio := 0.0
		if r.CompressionOriginalTokens > 0 {
			ratio = float64(r.CompressionCompressedTokens) / float64(r.CompressionOriginalTokens)
		}
		bucketMap[r.BucketTimestamp] = &OptimizationHistogramBucket{
			Timestamp:                          time.Unix(r.BucketTimestamp, 0).UTC(),
			SemanticSavings:                    semantic,
			PromptCacheSavings:                 r.PromptCacheSavings,
			ReasoningSavings:                   r.ReasoningSavings,
			CompressionSavings:                 r.CompressionSavings,
			RagSavings:                         r.RagSavings,
			SummarizationSavings:               r.SummarizationSavings,
			ConsistencySavings:                 r.ConsistencySavings,
			TotalSavings:                       r.TotalSavings,
			CompressionApplied:                 r.CompressionApplied,
			CompressionCacheHits:               r.CompressionCacheHits,
			CompressionOriginalTokens:          r.CompressionOriginalTokens,
			CompressionCompressedTokens:        r.CompressionCompressedTokens,
			CompressionAvgLatencyMs:            r.CompressionAvgLatencyMs,
			CompressionAvgRatio:                ratio,
			ReasoningApplied:                   r.ReasoningApplied,
			ReasoningSampled:                   r.ReasoningSampled,
			CascadeAvgScore:                    r.CascadeAvgScore,
			CascadeEscalations:                 r.CascadeEscalations,
			BatchEligible:                      r.BatchEligible,
			RagApplied:                         r.RagApplied,
			RagChunksDetected:                  r.RagChunksDetected,
			RagChunksKept:                      r.RagChunksKept,
			RagTrimmedTokens:                   r.RagTrimmedTokens,
			RagOriginalTokens:                  r.RagOriginalTokens,
			RagAvgRerankLatencyMs:              r.RagAvgRerankLatencyMs,
			RagCacheHits:                       r.RagCacheHits,
			SummarizationApplied:               r.SummarizationApplied,
			SummarizationCacheHits:             r.SummarizationCacheHits,
			SummarizationAsyncKickoffs:         r.SummarizationAsyncKickoffs,
			SummarizationTurnsSummarized:       r.SummarizationTurnsSummarized,
			SummarizationOriginalTokens:        r.SummarizationOriginalTokens,
			SummarizationSummaryTokens:         r.SummarizationSummaryTokens,
			SummarizationSavedTokens:           r.SummarizationSavedTokens,
			TTFTApplied:                        r.TTFTApplied,
			TTFTMessagesReordered:              r.TTFTMessagesReordered,
			TTFTStablePrefixTokens:             r.TTFTStablePrefixTokens,
			ParallelToolsApplied:               r.ParallelToolsApplied,
			ParallelToolsTotal:                 r.ParallelToolsTotal,
			ParallelToolsParallelCount:         r.ParallelToolsParallelCount,
			ParallelToolsSequentialCount:       r.ParallelToolsSequentialCount,
			ParallelToolsWallClockMs:           r.ParallelToolsWallClockMs,
			ParallelToolsSerialEstimateMs:      r.ParallelToolsSerialEstimateMs,
			ParallelToolsLatencySavedMs:        r.ParallelToolsLatencySavedMs,
			HallucinationScored:                r.HallucinationScored,
			HallucinationAvgFaithfulness:       r.HallucinationAvgFaithfulness,
			HallucinationAvgAnswerRelevance:    r.HallucinationAvgAnswerRelevance,
			HallucinationAvgCoherence:          r.HallucinationAvgCoherence,
			HallucinationAvgHelpfulness:        r.HallucinationAvgHelpfulness,
			HallucinationAvgCitationPrecision:  r.HallucinationAvgCitationPrecision,
			HallucinationAvgScore:              r.HallucinationAvgScore,
			HallucinationControlApplied:        r.HallucinationControlApplied,
			HallucinationControlAvgImprovement: r.HallucinationControlAvgImprovement,
			ReasoningEffortMix:                 map[string]int64{},
			CascadeSourceMix:                   map[string]int64{},
			BatchProviderMix:                   map[string]int64{},
		}
	}

	// 2. Categorical mixes - one query each, grouped by (bucket, label).
	type mixRow struct {
		BucketTimestamp int64  `gorm:"column:bucket_timestamp"`
		Label           string `gorm:"column:label"`
		Count           int64  `gorm:"column:cnt"`
	}
	fillMix := func(column string, sink func(b *OptimizationHistogramBucket) map[string]int64) error {
		var mr []mixRow
		mq := s.db.WithContext(ctx).Model(&Log{})
		mq = s.applyFilters(mq, filters)
		mq = mq.Where(column + " IS NOT NULL AND " + column + " <> ''")
		if err := mq.Select(fmt.Sprintf("%s as bucket_timestamp, %s as label, COUNT(*) as cnt", bucketExpr, column)).
			Group("bucket_timestamp, " + column).
			Find(&mr).Error; err != nil {
			return err
		}
		for _, r := range mr {
			b, ok := bucketMap[r.BucketTimestamp]
			if !ok {
				continue
			}
			sink(b)[r.Label] = r.Count
		}
		return nil
	}
	if err := fillMix("reasoning_applied_effort", func(b *OptimizationHistogramBucket) map[string]int64 { return b.ReasoningEffortMix }); err != nil {
		return nil, fmt.Errorf("failed to get reasoning effort mix: %w", err)
	}
	if err := fillMix("cascade_source", func(b *OptimizationHistogramBucket) map[string]int64 { return b.CascadeSourceMix }); err != nil {
		return nil, fmt.Errorf("failed to get cascade source mix: %w", err)
	}
	if err := fillMix("batch_provider", func(b *OptimizationHistogramBucket) map[string]int64 { return b.BatchProviderMix }); err != nil {
		return nil, fmt.Errorf("failed to get batch provider mix: %w", err)
	}

	// 3. Emit one bucket per time-slice in the configured range so the UI
	// sees a continuous x-axis (empty buckets render as zero bars).
	allTimestamps := generateBucketTimestamps(filters.StartTime, filters.EndTime, bucketSizeSeconds)
	buckets := make([]OptimizationHistogramBucket, 0, len(allTimestamps))
	if len(allTimestamps) > 0 {
		for _, ts := range allTimestamps {
			...
			buckets = append(buckets, OptimizationHistogramBucket{
				Timestamp:          time.Unix(ts, 0).UTC(),
				ReasoningEffortMix: map[string]int64{},
				CascadeSourceMix:   map[string]int64{},
				BatchProviderMix:   map[string]int64{},
	...
	return &OptimizationHistogramResult{
		Buckets:           buckets,
		BucketSizeSeconds: bucketSizeSeconds,
	}, nil
}
...
func (s *RDBLogStore) GetProviderCostHistogram(ctx context.Context, filters SearchFilters, bucketSizeSeconds int64) (*ProviderCostHistogramResult, error) {
	...
	for _, r := range results {
		...
		if bucket, exists := bucketMap[r.BucketTimestamp]; exists {
			...
			bucket.ByProviderCacheSavings[r.Provider] = r.CacheSavings
		} else {
			bucketMap[r.BucketTimestamp] = &ProviderCostHistogramBucket{
				...
				ByProviderCacheSavings: map[string]float64{r.Provider: r.CacheSavings},
			}
		}
	}
...
func (s *RDBLogStore) GetProviderTokenHistogram(ctx context.Context, filters SearchFilters, bucketSizeSeconds int64) (*ProviderTokenHistogramResult, error) {
	...
	case "sqlite":
		selectClause = fmt.Sprintf(`
			...
			COALESCE(SUM(%s), 0) as completion_tokens,
			COALESCE(SUM(%s), 0) as total_tokens
		`, bucketSizeSeconds, bucketSizeSeconds, s.actualPromptTokensSQL(), s.actualCompletionTokensSQL(), s.actualTotalTokensSQL())
	case "mysql":
		selectClause = fmt.Sprintf(`
			(FLOOR(UNIX_TIMESTAMP(timestamp) / %d) * %d) as bucket_timestamp,
			provider,
...
// GetDistinctGuardrailStatuses returns all unique guardrail decision values from the guardrail_decisions table.
// Scoped to recent data to avoid full table scans.
func (s *RDBLogStore) GetDistinctGuardrailStatuses(ctx context.Context) ([]string, error) {
	cutoff := time.Now().UTC().AddDate(0, 0, -defaultFilterDataCutoffDays)
	var statuses []string
	err := s.db.WithContext(ctx).Model(&GuardrailDecision{}).
		Where("created_at >= ?", cutoff).
		Distinct("decision").
		Limit(defaultFilterDataLimit).
		Pluck("decision", &statuses).Error
	if err != nil {
		return nil, fmt.Errorf("failed to get distinct guardrail statuses: %w", err)
	}
...
func (s *RDBLogStore) GetDistinctMetadataKeys(ctx context.Context) (map[string][]string, error) {
	return s.GetDistinctMetadataKeysScoped(ctx, MetadataScopeAll)
}

// GetDistinctMetadataKeysScoped is GetDistinctMetadataKeys with an
// AI-Logs / Agentic-Logs split. The same recent-rows pluck strategy
// applies; the only difference is the extra object_type filter that keeps
// each audience's metadata column set disjoint. See the LogStore interface
// comment for the why.
func (s *RDBLogStore) GetDistinctMetadataKeysScoped(ctx context.Context, scope MetadataScope) (map[string][]string, error) {
	cutoff := time.Now().UTC().AddDate(0, 0, -defaultFilterDataCutoffDays)
	q := s.db.WithContext(ctx).Model(&Log{}).
		Where("metadata IS NOT NULL AND metadata != '' AND metadata != '{}' AND timestamp >= ?", cutoff)
	switch scope {
	case MetadataScopeLLM:
		q = q.Where("object_type NOT LIKE ?", "agentic.%")
	case MetadataScopeAgentic:
		q = q.Where("object_type LIKE ?", "agentic.%")
	}
...
func (s *RDBLogStore) applyMCPFilters(baseQuery *gorm.DB, filters MCPToolLogSearchFilters) *gorm.DB {
	// Same tenant-first ordering as applyFilters - keeps the NULL-workspace
	// fallback branch from leaking cross-tenant MCP tool calls.
	if tenant := strings.TrimSpace(filters.TenantID); tenant != "" {
		baseQuery = baseQuery.Where("tenant_id = ?", tenant)
	}
	if filters.AllowedWorkspaceIDs != nil {
	...
	if ws := strings.TrimSpace(filters.WorkspaceID); ws != "" {
		// Strict equality - see comment on applyFilters above.
		baseQuery = baseQuery.Where("workspace_id = ?", ws)
	}
