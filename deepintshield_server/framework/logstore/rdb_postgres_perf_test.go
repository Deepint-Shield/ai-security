...
func insertPerfLog(t *testing.T, db *gorm.DB, opts logOpts) {
	...
	id := uuid.New().String()
	err := db.Exec(`
		INSERT INTO logs (id, tenant_id, timestamp, object_type, provider, model, status,
	...
	`, id, opts.TenantID, opts.Timestamp, opts.Provider, opts.Model, opts.Status,
...
func TestMatViewQueries_AreTenantScoped(t *testing.T) {
	store, db := setupPerfTestDB(t)
	now := time.Now().UTC()
	start := now.Add(-1 * time.Hour)
	...
	aliceCtx := withLogTenant(context.Background(), "alice@example.com")
	bobCtx := withLogTenant(context.Background(), "bob@example.com")

	aliceStats, err := store.GetStats(aliceCtx, SearchFilters{StartTime: &start, EndTime: &end})
	require.NoError(t, err)
	assert.Equal(t, int64(1), aliceStats.TotalRequests)

	bobStats, err := store.GetStats(bobCtx, SearchFilters{StartTime: &start, EndTime: &end})
	require.NoError(t, err)
	assert.Equal(t, int64(1), bobStats.TotalRequests)

	aliceModels, err := store.GetDistinctModels(aliceCtx)
	require.NoError(t, err)
	assert.Equal(t, []string{"alice-model"}, aliceModels)

	bobModels, err := store.GetDistinctModels(bobCtx)
	require.NoError(t, err)
	assert.Equal(t, []string{"bob-model"}, bobModels)
}
