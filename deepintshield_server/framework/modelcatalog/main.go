...
type ModelCatalog struct {
	...
	pricingStaleFallbackThreshold time.Duration
	...
	// pricingAlias maps canonicalized (provider, model, mode) keys to the
	// fully-qualified key that lives in pricingData. Built alongside
	// pricingData so that requests using an unversioned model name
	// (e.g. "claude-opus-4-5", "gpt-4o-mini") still resolve to the dated
	// pricing row ("claude-opus-4-5-20251101"). The alias is a fallback,
	// not the primary lookup, so exact matches stay O(1) on the hot path.
	pricingAlias map[string]string
	mu           sync.RWMutex
...
// It handles two cross-source quirks:
//
//  1. search_context_cost_per_query may arrive as either a plain float64 or
//     a tiered object {"search_context_size_low":…, …_medium:…, …_high:…}.
//  2. The DeepintShield datasheet uses `"provider"`; the LiteLLM datasheet uses
//     `"litellm_provider"`. When DeepintShield is unreachable we fall back to
//     LiteLLM (see DefaultPricingFallbackURL), and accepting both spellings
//     here keeps the rest of the catalog code provider-source-agnostic.
func (p *PricingEntry) UnmarshalJSON(data []byte) error {
	...
	*p = PricingEntry(raw.PricingEntryAlias)
	...
	// LiteLLM-shape fallback: their JSON spells the provider field
	// `litellm_provider`; copy it through so downstream lookups (which key
	// off Provider) keep working uniformly across both sources.
	if p.Provider == "" && raw.LiteLLMProvider != "" {
...
func Init(ctx context.Context, config *Config, configStore configstore.ConfigStore, shouldSyncPricingFunc ShouldSyncPricingFunc, logger schemas.Logger) (*ModelCatalog, error) {
	...
	pricingFallbackURL := DefaultPricingFallbackURL
	if config.PricingFallbackURL != nil {
		pricingFallbackURL = *config.PricingFallbackURL
	}
	pricingStaleFallbackThreshold := DefaultPricingStaleFallbackThreshold
	if config.PricingStaleFallbackThreshold != nil {
		pricingStaleFallbackThreshold = *config.PricingStaleFallbackThreshold
	}

	mc := &ModelCatalog{
		pricingURL:                    pricingURL,
		pricingFallbackURL:            pricingFallbackURL,
		pricingStaleFallbackThreshold: pricingStaleFallbackThreshold,
	...
	if configStore != nil {
		if mc.distributedLockManager == nil {
			...
			// Try to win the sync lock; whichever pod wins owns the
			// remote pricing pull for this rollout. Losers must not fail
			// - they still need a functional in-memory pricing manager so
			// /api/billing/*, model catalog lookups, and cost attribution
			// work on them. Without this fallback, every losing replica
			// returned `pricing manager not found` for the rest of its
			// lifetime and the relevant requests 500'd on whichever pod
			// the GCLB happened to route them to.
			lockAcquired := false
			if lockErr := lock.Lock(ctx); lockErr != nil {
				// Another replica holds the lock - that's fine, log and
				// proceed with DB-only init. The lock winner will have
				// (or will shortly) populate the pricing tables we read
				// from below; the background sync worker started at the
				// end of New() will pick up any updates after the winner
				// finishes.
				mc.logger.Warn("could not acquire model_catalog_pricing_sync lock (another pod likely holds it); skipping remote sync and loading from DB: %v", lockErr)
			} else {
				lockAcquired = true
				defer lock.Unlock(ctx)
			}

			// Load initial pricing data (always - both winner and loser
			// rely on the DB-resident catalog to bootstrap their in-memory
			// pricing index). loadPricingFromDatabase is read-only.
			if err := mc.loadPricingFromDatabase(ctx); err != nil {
				return nil, fmt.Errorf("failed to load initial pricing data: %w", err)
			}
			// Only the lock-holder performs the remote sync. Losers skip
			// it (the winner does it for them) and rely on the periodic
			// startSyncWorker below to pick up any newer pricing.
			if lockAcquired {
				if err := mc.syncPricing(ctx); err != nil {
					return nil, fmt.Errorf("failed to sync pricing data: %w", err)
				}
...
func (mc *ModelCatalog) ReloadPricing(ctx context.Context, config *Config) error {
	...
	mc.pricingFallbackURL = DefaultPricingFallbackURL
	if config.PricingFallbackURL != nil {
		mc.pricingFallbackURL = *config.PricingFallbackURL
	}
	mc.pricingStaleFallbackThreshold = DefaultPricingStaleFallbackThreshold
	if config.PricingStaleFallbackThreshold != nil {
		mc.pricingStaleFallbackThreshold = *config.PricingStaleFallbackThreshold
	}
...
// getPricingFallbackURL returns the URL used after the primary URL has been
// stale beyond pricingStaleFallbackThreshold (defaults to LiteLLM's
// model_prices_and_context_window.json).
func (mc *ModelCatalog) getPricingFallbackURL() string {
	mc.pricingMu.RLock()
	defer mc.pricingMu.RUnlock()
	return mc.pricingFallbackURL
}

// getPricingStaleFallbackThreshold returns the cutoff age past which the
// worker stops serving the DB-cached catalog and tries the fallback URL.
// Defaults to 48h per the §3.1 cost/billing-freshness requirement.
func (mc *ModelCatalog) getPricingStaleFallbackThreshold() time.Duration {
	mc.pricingMu.RLock()
	defer mc.pricingMu.RUnlock()
	return mc.pricingStaleFallbackThreshold
}
...
func (mc *ModelCatalog) UpsertModelDataForProvider(provider schemas.ModelProvider, modelData *schemas.DeepIntShieldListModelsResponse, allowedModels []schemas.Model) {
	if modelData == nil {
		return
	}
	mc.mu.Lock()
...
func (mc *ModelCatalog) UpsertUnfilteredModelDataForProvider(provider schemas.ModelProvider, modelData *schemas.DeepIntShieldListModelsResponse) {
	if modelData == nil {
		return
	}
	mc.mu.Lock()
