...
// costInput holds the extracted usage data from a DeepIntShieldResponse,
...
// CalculateCost calculates the cost of a DeepIntShield response.
// It handles all request types, cache debug billing, and tiered pricing.
func (mc *ModelCatalog) CalculateCost(result *schemas.DeepIntShieldResponse) float64 {
	if result == nil {
		return 0
	}
...
// CalculateCostBreakdown returns the actual billed cost plus the net savings
// realized by serving a response from cache instead of forwarding the full
// request upstream.
func (mc *ModelCatalog) CalculateCostBreakdown(result *schemas.DeepIntShieldResponse) (actualCost float64, cacheSavings float64) {
	if result == nil {
		return 0, 0
	}

	actualCost = mc.CalculateCost(result)
	...
	uncachedCost := mc.calculateBaseCost(result)
	if uncachedCost <= 0 || actualCost >= uncachedCost {
		return actualCost, 0
	}

	return actualCost, uncachedCost - actualCost
}

// CalculatePromptCacheSavings returns the input-token cost saved (or lost) by
// provider-native prompt caching on this request. This is distinct from
// CalculateCostBreakdown's semantic-cache savings - semantic cache short-
// circuits the entire LLM call, while provider prompt cache only discounts the
// prefix prefill on a real call.
//
// Formula:
//
//	savings = cached_read_tokens  × (input_rate − cache_read_rate)
//	        − cached_write_tokens × (cache_creation_rate − input_rate)
//
// A negative result (write premium paid with no offsetting reads, e.g. cold
// prefix on Anthropic) is returned honestly so dashboards can show the leak
// rather than silently hide it.
//
// Returns 0 when:
//   - The response came from semantic cache (no provider call happened)
//   - The request type isn't subject to provider prompt caching (embeddings,
//     image, video, etc.)
//   - The provider returned no cached_read/cached_write token counts
//   - We can't resolve a pricing entry for the model
func (mc *ModelCatalog) CalculatePromptCacheSavings(result *schemas.DeepIntShieldResponse) float64 {
	if result == nil {
		return 0
	}
	...
	// Semantic cache short-circuit: no upstream call, no provider prompt cache savings.
	if extraFields.CacheDebug != nil && extraFields.CacheDebug.CacheHit {
		return 0
	}

	requestType := normalizeStreamRequestType(extraFields.RequestType)
	switch requestType {
	...
	if input.usage == nil || input.usage.PromptTokensDetails == nil {
		return 0
	}
	cachedRead := input.usage.PromptTokensDetails.CachedReadTokens
	cachedWrite := input.usage.PromptTokensDetails.CachedWriteTokens
	if cachedRead <= 0 && cachedWrite <= 0 {
		return 0
	}

	pricing := mc.resolvePricing(string(extraFields.Provider), extraFields.ModelRequested, extraFields.ModelDeployment, requestType)
	if pricing == nil {
		return 0
	}
	...
	cacheReadRate := tieredCacheReadInputTokenRate(pricing, totalTokens)
	cacheCreationRate := tieredCacheCreationInputTokenRate(pricing, totalTokens)

	// Clamp to defend against malformed provider payloads (mirrors computeTextCost).
	promptTokens := input.usage.PromptTokens
	if cachedRead > promptTokens {
		cachedRead = promptTokens
	}
	if cachedWrite > promptTokens-cachedRead {
		cachedWrite = promptTokens - cachedRead
	}

	savings := float64(cachedRead) * (inputRate - cacheReadRate)
	savings -= float64(cachedWrite) * (cacheCreationRate - inputRate)
	return savings
}

// effortBudgetRatio maps a reasoning-effort level to a relative "reasoning
// budget" multiplier. These are industry-known approximations published by
// the providers (OpenAI o-series ratios, Anthropic extended-thinking budgets,
// Gemini Deep Think tiers) - they're directionally accurate, not measured
// per-tenant. Operators who want measured ratios should enable the drift
// sampler and compare sampled vs throttled outcomes downstream.
var effortBudgetRatio = map[string]float64{
	"none":    0.0,
	"minimal": 0.5,
	"low":     1.0,
	"medium":  3.0,
	"high":    12.0,
	"auto":    1.0, // sentinel: caller's value left in place
}

// CalculateReasoningSavings returns the *estimated* cost the workspace
// avoided by throttling reasoning effort below the caller's intent.
//
// Formula:
//
//	would_have_been = reasoning_tokens × (orig_ratio / applied_ratio) × output_rate
//	saved           = would_have_been − (reasoning_tokens × output_rate)
//
// Returns 0 when:
//   - The response carries no ReasoningDebug (throttling didn't run)
//   - Throttling didn't actually fire (applied=false) - sampled traffic or no-op
//   - The throttle didn't downgrade effort (applied_ratio >= orig_ratio)
//   - The provider didn't report reasoning_tokens
//   - We can't resolve a pricing entry for the model
//
// Estimate, not measurement - see effortBudgetRatio comments. Dashboards
// should label this honestly (e.g. "estimated savings").
func (mc *ModelCatalog) CalculateReasoningSavings(result *schemas.DeepIntShieldResponse) float64 {
	if result == nil {
		return 0
	}
	extra := result.GetExtraFields()
	if extra == nil || extra.ReasoningDebug == nil || !extra.ReasoningDebug.Applied {
		return 0
	}
	dbg := extra.ReasoningDebug
	origRatio, ok1 := effortBudgetRatio[strings.ToLower(strings.TrimSpace(dbg.OriginalEffort))]
	appliedRatio, ok2 := effortBudgetRatio[strings.ToLower(strings.TrimSpace(dbg.AppliedEffort))]
	if !ok1 || !ok2 || appliedRatio <= 0 || origRatio <= appliedRatio {
		return 0
	}

	requestType := normalizeStreamRequestType(extra.RequestType)
	switch requestType {
	...
	input := extractCostInput(result)
	if input.usage == nil || input.usage.CompletionTokensDetails == nil {
		return 0
	}
	reasoningTokens := input.usage.CompletionTokensDetails.ReasoningTokens
	if reasoningTokens <= 0 {
		return 0
	}

	pricing := mc.resolvePricing(string(extra.Provider), extra.ModelRequested, extra.ModelDeployment, requestType)
	if pricing == nil {
		return 0
	}
	outputRate := tieredOutputRate(pricing, input.usage.TotalTokens)
	if outputRate <= 0 {
		return 0
	}

	actual := float64(reasoningTokens) * outputRate
	wouldHaveBeen := actual * (origRatio / appliedRatio)
	saved := wouldHaveBeen - actual
	if saved < 0 {
		return 0
	}
	return saved
...
// CalculateCompressionSavings returns the input-token cost avoided by prompt
// compression. Unlike the reasoning estimate, this one is exact: we know the
// pre- and post-compression token counts directly from the compressor and
// multiply by the model's input rate.
//
// Returns 0 when:
//   - The response carries no CompressionDebug (compression didn't run)
//   - Compression didn't actually apply (sampled traffic, no-op result)
//   - The compressed token count is >= original (no real savings)
//   - We can't resolve a pricing entry for the model
func (mc *ModelCatalog) CalculateCompressionSavings(result *schemas.DeepIntShieldResponse) float64 {
	if result == nil {
		return 0
	}
	extra := result.GetExtraFields()
	if extra == nil || extra.CompressionDebug == nil || !extra.CompressionDebug.Applied {
		return 0
	}
	dbg := extra.CompressionDebug
	saved := dbg.OriginalTokens - dbg.CompressedTokens
	if saved <= 0 {
		return 0
	}
	...
	rate := tieredInputRate(pricing, totalTokens)
	if rate <= 0 {
		return 0
	}
	return float64(saved) * rate
}

// CalculateRagSavings returns the input-token cost avoided by RAG context
// trimming. Exact (not estimated): the dropped chunks' token count is
// measured by the detector + reranker, and we multiply by the model's
// input rate.
func (mc *ModelCatalog) CalculateRagSavings(result *schemas.DeepIntShieldResponse) float64 {
	if result == nil {
		return 0
	}
	extra := result.GetExtraFields()
	if extra == nil || extra.RagDebug == nil || !extra.RagDebug.Applied {
		return 0
	}
	saved := extra.RagDebug.TrimmedTokens
	if saved <= 0 {
		return 0
	}
	requestType := normalizeStreamRequestType(extra.RequestType)
...
// CalculateSummarizationSavings returns the input-token cost avoided when
// the gateway replaced older conversation turns with a structured summary.
// Measured (not projected): saved_tokens = original_tokens − summary_tokens,
// stamped on the response by the summarization module after the cache hit.
func (mc *ModelCatalog) CalculateSummarizationSavings(result *schemas.DeepIntShieldResponse) float64 {
	if result == nil {
		return 0
	}
	extra := result.GetExtraFields()
	if extra == nil || extra.SummarizationDebug == nil || !extra.SummarizationDebug.Applied {
		return 0
	}
	saved := extra.SummarizationDebug.SavedTokens
	if saved <= 0 {
		return 0
	}
	requestType := normalizeStreamRequestType(extra.RequestType)
...
// CalculateConsistencySavings returns the cost avoided when the Response
// Consistency engine served a cached answer and short-circuited the whole LLM
// call. This is the 7th cost-optimization source folded into cache_savings.
//
// The engine ships in-process with no pricing catalog, so it stamps the
// *avoided token counts* (ConsistencyDebug) rather than dollars; we price them
// here at the model's base input (prompt) + output (completion) rates.
//
// Estimate, not measurement: a cache hit has no provider usage, so the token
// counts come from the ~4-chars/token heuristic in the plugin and the canonical
// question understates the full prompt - the result is deliberately
// conservative. Dashboards label it honestly alongside the reasoning estimate.
//
// Returns 0 when:
//   - The response carries no ConsistencyDebug (engine didn't serve a hit)
//   - The hit didn't actually apply
//   - There are no avoided tokens to price
//   - We can't resolve a pricing entry for the model
//
// No double-count with semantic-cache savings: the materialized hit response
// carries CacheDebug.CacheHit=true but no token usage, so
// CalculateCostBreakdown's base cost (and thus its semantic savings) is 0.
func (mc *ModelCatalog) CalculateConsistencySavings(result *schemas.DeepIntShieldResponse) float64 {
	if result == nil {
		return 0
	}
	extra := result.GetExtraFields()
	if extra == nil || extra.ConsistencyDebug == nil || !extra.ConsistencyDebug.Applied {
		return 0
	}
	dbg := extra.ConsistencyDebug
	if dbg.AvoidedPromptTokens <= 0 && dbg.AvoidedCompletionTokens <= 0 {
		return 0
	}
	...
	// The engine stamps the model on ConsistencyDebug; fall back to the
	// response's ModelRequested (materializeResponse sets both).
	model := dbg.Model
	if strings.TrimSpace(model) == "" {
		model = extra.ModelRequested
	}
	pricing := mc.resolvePricing(string(extra.Provider), model, extra.ModelDeployment, requestType)
	if pricing == nil {
		return 0
	}

	totalTokens := dbg.AvoidedPromptTokens + dbg.AvoidedCompletionTokens
	inputRate := tieredInputRate(pricing, totalTokens)
	outputRate := tieredOutputRate(pricing, totalTokens)

	saved := float64(dbg.AvoidedPromptTokens)*inputRate + float64(dbg.AvoidedCompletionTokens)*outputRate
	if saved < 0 {
		return 0
	}
	return saved
...
func (mc *ModelCatalog) calculateCostWithCache(result *schemas.DeepIntShieldResponse, cacheDebug *schemas.DeepIntShieldCacheDebug) float64 {
	if cacheDebug.CacheHit {
		// Direct cache hit - no LLM call, no cost
		if cacheDebug.HitType != nil && *cacheDebug.HitType == "direct" {
			return 0
		}
		// Coalesced subscriber - leader paid the full cost, subscriber pays
		// nothing. CalculateCostBreakdown then attributes the leader's
		// uncached cost as savings on the subscriber's log row, so the
		// dashboard's existing cache_savings total picks it up automatically.
		if cacheDebug.HitType != nil && *cacheDebug.HitType == "coalesced" {
			return 0
		}
...
func (mc *ModelCatalog) computeCacheEmbeddingCost(cacheDebug *schemas.DeepIntShieldCacheDebug) float64 {
...
func (mc *ModelCatalog) calculateBaseCost(result *schemas.DeepIntShieldResponse) float64 {
	extraFields := result.GetExtraFields()
	if extraFields == nil {
		return 0
	}
...
func extractCostInput(result *schemas.DeepIntShieldResponse) costInput {
	var input costInput
	...
	case result.ResponsesResponse != nil && result.ResponsesResponse.Usage != nil:
		input.usage = responsesUsageToDeepIntShieldUsage(result.ResponsesResponse.Usage)

	case result.ResponsesStreamResponse != nil && result.ResponsesStreamResponse.Response != nil && result.ResponsesStreamResponse.Response.Usage != nil:
		input.usage = responsesUsageToDeepIntShieldUsage(result.ResponsesStreamResponse.Response.Usage)
	...
	case result.SpeechResponse != nil && result.SpeechResponse.Usage != nil:
		input.usage = speechUsageToDeepIntShieldUsage(result.SpeechResponse.Usage)
	...
	case result.SpeechStreamResponse != nil && result.SpeechStreamResponse.Usage != nil:
		input.usage = speechUsageToDeepIntShieldUsage(result.SpeechStreamResponse.Usage)
...
func responsesUsageToDeepIntShieldUsage(u *schemas.ResponsesResponseUsage) *schemas.DeepIntShieldLLMUsage {
	usage := &schemas.DeepIntShieldLLMUsage{
...
func speechUsageToDeepIntShieldUsage(u *schemas.SpeechUsage) *schemas.DeepIntShieldLLMUsage {
...
func extractTranscriptionUsage(u *schemas.TranscriptionUsage) (*schemas.DeepIntShieldLLMUsage, *int, *schemas.TranscriptionUsageInputTokenDetails) {
	usage := &schemas.DeepIntShieldLLMUsage{}
...
func computeTextCost(pricing *configstoreTables.TableModelPricing, usage *schemas.DeepIntShieldLLMUsage) float64 {
	if usage == nil {
		return 0
	}
...
func computeEmbeddingCost(pricing *configstoreTables.TableModelPricing, usage *schemas.DeepIntShieldLLMUsage) float64 {
	if usage == nil {
		return 0
	}
...
func computeRerankCost(pricing *configstoreTables.TableModelPricing, usage *schemas.DeepIntShieldLLMUsage) float64 {
	if usage == nil {
		return 0
	}
...
func computeSpeechCost(pricing *configstoreTables.TableModelPricing, usage *schemas.DeepIntShieldLLMUsage, audioSeconds *int, audioTextInputChars int) float64 {
...
func computeTranscriptionCost(pricing *configstoreTables.TableModelPricing, usage *schemas.DeepIntShieldLLMUsage, audioSeconds *int, audioTokenDetails *schemas.TranscriptionUsageInputTokenDetails) float64 {
	totalTokens := safeTotalTokens(usage)
...
func computeAudioInputCost(pricing *configstoreTables.TableModelPricing, usage *schemas.DeepIntShieldLLMUsage, audioSeconds *int, audioTokenDetails *schemas.TranscriptionUsageInputTokenDetails, totalTokens int) float64 {
...
func computeAudioOutputCost(pricing *configstoreTables.TableModelPricing, usage *schemas.DeepIntShieldLLMUsage, audioSeconds *int, totalTokens int) float64 {
...
// Input and output are calculated independently - tokens first, then per-second fallback.
func computeVideoCost(pricing *configstoreTables.TableModelPricing, usage *schemas.DeepIntShieldLLMUsage, videoSeconds *int) float64 {
	totalTokens := safeTotalTokens(usage)
...
func safeTotalTokens(usage *schemas.DeepIntShieldLLMUsage) int {
	if usage == nil {
		return 0
	}
...
func (mc *ModelCatalog) getPricing(model, provider string, requestType schemas.RequestType) (*configstoreTables.TableModelPricing, bool) {
	...
	// Generic alias fallback. Hot path unchanged: this branch only runs
	// when none of the exact-match lookups above matched, so common
	// in-catalog requests pay zero added latency. On a miss we strip
	// provider prefixes, dated snapshots, "@version" tags, and
	// release-channel suffixes from the requested model name and probe
	// the pre-built alias index - one extra map lookup, ~100 ns, no
	// allocation beyond what the regex replace produces. Lets every
	// provider's unversioned model name (claude-opus-4-5,
	// gpt-4o-mini, gemini-2.5-flash, …) resolve to the dated pricing
	// row the catalog actually ships, so cost / savings columns are
	// populated end-to-end without operators having to seed aliases
	// by hand.
	if fullKey, ok := mc.pricingAlias[canonicalAliasKey(model, provider, mode)]; ok {
		if pricing, hit := mc.pricingData[fullKey]; hit {
			return &pricing, true
		}
	}
	// Same alias probe with the alternate request-type when ResponsesRequest
	// can serve from a chat-mode pricing row (matches the existing fallback
	// behavior a few branches up).
	if requestType == schemas.ResponsesRequest || requestType == schemas.ResponsesStreamRequest {
		if fullKey, ok := mc.pricingAlias[canonicalAliasKey(model, provider, normalizeRequestType(schemas.ChatCompletionRequest))]; ok {
			if pricing, hit := mc.pricingData[fullKey]; hit {
				return &pricing, true
			}
		}
