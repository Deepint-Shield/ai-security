...
// makeChatResponse builds a minimal DeepIntShieldResponse for a chat completion.
func makeChatResponse(provider schemas.ModelProvider, model string, usage *schemas.DeepIntShieldLLMUsage) *schemas.DeepIntShieldResponse {
...
// makeEmbeddingResponse builds a minimal DeepIntShieldResponse for an embedding request.
func makeEmbeddingResponse(provider schemas.ModelProvider, model string, usage *schemas.DeepIntShieldLLMUsage) *schemas.DeepIntShieldResponse {
	return &schemas.DeepIntShieldResponse{
...
// makeRerankResponse builds a minimal DeepIntShieldResponse for a rerank request.
func makeRerankResponse(provider schemas.ModelProvider, model string, usage *schemas.DeepIntShieldLLMUsage) *schemas.DeepIntShieldResponse {
	return &schemas.DeepIntShieldResponse{
...
// makeImageResponse builds a minimal DeepIntShieldResponse for an image generation request.
func makeImageResponse(provider schemas.ModelProvider, model string, usage *schemas.ImageUsage) *schemas.DeepIntShieldResponse {
	return &schemas.DeepIntShieldResponse{
...
func TestComputeSpeechCost_OutputFallsBackToPerSecond(t *testing.T) {
	...
	seconds := 120
	usage := &schemas.DeepIntShieldLLMUsage{PromptTokens: 500}
	cost := computeSpeechCost(&p, usage, &seconds, 0)
...
func TestComputeVideoCost_DurationBased(t *testing.T) {
	...
	seconds := 30
	usage := &schemas.DeepIntShieldLLMUsage{PromptTokens: 500, TotalTokens: 500}
...
func TestComputeVideoCost_NilSeconds(t *testing.T) {
	...
	usage := &schemas.DeepIntShieldLLMUsage{PromptTokens: 1000}
...
func TestExtractCostInput_ChatResponse(t *testing.T) {
	usage := &schemas.DeepIntShieldLLMUsage{PromptTokens: 100, CompletionTokens: 50, TotalTokens: 150}
	resp := &schemas.DeepIntShieldResponse{
		ChatResponse: &schemas.DeepIntShieldChatResponse{Usage: usage},
	}
...
func TestExtractCostInput_EmbeddingResponse(t *testing.T) {
	usage := &schemas.DeepIntShieldLLMUsage{PromptTokens: 200, TotalTokens: 200}
	resp := &schemas.DeepIntShieldResponse{
		EmbeddingResponse: &schemas.DeepIntShieldEmbeddingResponse{Usage: usage},
	}
	input := extractCostInput(resp)
	require.NotNil(t, input.usage)
...
func TestExtractCostInput_ImageResponse(t *testing.T) {
	...
	resp := &schemas.DeepIntShieldResponse{
		ImageGenerationResponse: &schemas.DeepIntShieldImageGenerationResponse{Usage: imgUsage},
	}
	input := extractCostInput(resp)
...
func TestCalculateCost_SemanticCacheMiss(t *testing.T) {
	...
	resp := &schemas.DeepIntShieldResponse{
		ChatResponse: &schemas.DeepIntShieldChatResponse{
			Usage: &schemas.DeepIntShieldLLMUsage{PromptTokens: 1000, CompletionTokens: 500, TotalTokens: 1500},
			ExtraFields: schemas.DeepIntShieldResponseExtraFields{
				RequestType:    schemas.ChatCompletionRequest,
				Provider:       schemas.OpenAI,
				ModelRequested: "gpt-4o",
...
func TestCalculateCostBreakdown_DirectCacheHit(t *testing.T) {
	mc := testCatalogWithPricing(map[string]configstoreTables.TableModelPricing{
		makeKey("gpt-4o", "openai", "chat"): {
			Model: "gpt-4o", Provider: "openai", Mode: "chat",
			InputCostPerToken: 0.000005, OutputCostPerToken: 0.000015,
	...
	actualCost, cacheSavings := mc.CalculateCostBreakdown(resp)
	assert.Equal(t, 0.0, actualCost)
	assert.InDelta(t, 0.0125, cacheSavings, 1e-12)
}

func TestCalculateCostBreakdown_SemanticCacheHit(t *testing.T) {
	embProvider := "openai"
	embModel := "text-embedding-3-small"
	embTokens := 500
	...
	actualCost, cacheSavings := mc.CalculateCostBreakdown(resp)
	assert.InDelta(t, 0.00001, actualCost, 1e-12)
	assert.InDelta(t, 0.01249, cacheSavings, 1e-12)
}

func TestCalculateConsistencySavings(t *testing.T) {
	mc := testCatalogWithPricing(map[string]configstoreTables.TableModelPricing{
		makeKey("gpt-4o", "openai", "chat"): {
			Model: "gpt-4o", Provider: "openai", Mode: "chat",
			InputCostPerToken: 0.000005, OutputCostPerToken: 0.000015,
	...
	makeResp := func(dbg *schemas.DeepIntShieldConsistencyDebug) *schemas.DeepIntShieldResponse {
		return &schemas.DeepIntShieldResponse{
			ChatResponse: &schemas.DeepIntShieldChatResponse{
				ExtraFields: schemas.DeepIntShieldResponseExtraFields{
					RequestType:      schemas.ChatCompletionRequest,
					Provider:         schemas.OpenAI,
					ModelRequested:   "gpt-4o",
					ConsistencyDebug: dbg,
				},
			},
		}
	}

	t.Run("hit prices avoided prompt+completion tokens at base rate", func(t *testing.T) {
		resp := makeResp(&schemas.DeepIntShieldConsistencyDebug{
			Applied:                 true,
			Source:                  "semantic",
			Model:                   "gpt-4o",
			AvoidedPromptTokens:     1000,
			AvoidedCompletionTokens: 500,
		})
		// 1000×0.000005 + 500×0.000015 = 0.005 + 0.0075 = 0.0125
		assert.InDelta(t, 0.0125, mc.CalculateConsistencySavings(resp), 1e-12)
	})

	t.Run("no debug → zero", func(t *testing.T) {
		assert.Equal(t, 0.0, mc.CalculateConsistencySavings(makeResp(nil)))
	})

	t.Run("not applied → zero", func(t *testing.T) {
		assert.Equal(t, 0.0, mc.CalculateConsistencySavings(makeResp(&schemas.DeepIntShieldConsistencyDebug{Applied: false, AvoidedPromptTokens: 1000})))
	})

	t.Run("no avoided tokens → zero", func(t *testing.T) {
		assert.Equal(t, 0.0, mc.CalculateConsistencySavings(makeResp(&schemas.DeepIntShieldConsistencyDebug{Applied: true, Model: "gpt-4o"})))
	})

	t.Run("nil response → zero", func(t *testing.T) {
		assert.Equal(t, 0.0, mc.CalculateConsistencySavings(nil))
	})
}
...
func TestCalculateCost_ProviderComputedCostPassthrough(t *testing.T) {
	...
	resp := makeChatResponse(schemas.OpenAI, "gpt-4o", &schemas.DeepIntShieldLLMUsage{
		PromptTokens:     1000,
		CompletionTokens: 500,
		TotalTokens:      1500,
		Cost: &schemas.DeepIntShieldCost{
...
func TestCalculateCost_ChatCompletion_Claude35Sonnet_WithCache(t *testing.T) {
	...
	resp := makeChatResponse(schemas.Bedrock, "anthropic.claude-3-5-sonnet-20241022-v2:0", &schemas.DeepIntShieldLLMUsage{
		PromptTokens:     5000,
...
func TestCalculateCost_Embedding(t *testing.T) {
	...
	resp := makeEmbeddingResponse(schemas.Bedrock, "amazon.titan-embed-text-v1", &schemas.DeepIntShieldLLMUsage{
		PromptTokens: 10000,
		TotalTokens:  10000,
	})
...
func TestCalculateCost_Rerank(t *testing.T) {
	...
	resp := makeRerankResponse(schemas.Bedrock, "amazon.rerank-v1:0", &schemas.DeepIntShieldLLMUsage{
		PromptTokens: 500,
		TotalTokens:  500,
	})
...
func TestCalculateCost_NoPricingData(t *testing.T) {
	mc := testCatalogWithPricing(nil)
	resp := makeChatResponse(schemas.OpenAI, "unknown-model", &schemas.DeepIntShieldLLMUsage{
...
// =========================================================================
// 15. responsesUsageToDeepIntShieldUsage
// =========================================================================

func TestResponsesUsageToDeepIntShieldUsage_Basic(t *testing.T) {
	u := &schemas.ResponsesResponseUsage{
		InputTokens:  100,
		OutputTokens: 50,
		TotalTokens:  150,
	}
	result := responsesUsageToDeepIntShieldUsage(u)
...
func TestResponsesUsageToDeepIntShieldUsage_WithTokenDetails(t *testing.T) {
	numQueries := 2
	u := &schemas.ResponsesResponseUsage{
		InputTokens:  1000,
		OutputTokens: 500,
...
func TestCalculateCost_200kTier_EndToEnd(t *testing.T) {
	...
	resp := makeChatResponse(schemas.Bedrock, "anthropic.claude-3-5-sonnet-20240620-v1:0", &schemas.DeepIntShieldLLMUsage{
		PromptTokens:     190000,
