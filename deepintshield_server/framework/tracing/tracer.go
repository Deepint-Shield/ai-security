// Package tracing provides distributed tracing infrastructure for DeepIntShield
package tracing
...
// 1. DeepIntShieldContextKeySpanID - existing span in this service (for child spans)
// 2. DeepIntShieldContextKeyParentSpanID - incoming parent from W3C traceparent (for root spans)
...
func (t *Tracer) StartSpan(ctx context.Context, name string, kind schemas.SpanKind) (context.Context, schemas.SpanHandle) {
	...
	parentSpanID, _ := ctx.Value(schemas.DeepIntShieldContextKeySpanID).(string)
	...
	if parentSpanID == "" {
		parentSpanID, _ = ctx.Value(schemas.DeepIntShieldContextKeyParentSpanID).(string)
	}
	...
	newCtx := context.WithValue(ctx, schemas.DeepIntShieldContextKeySpanID, span.SpanID)
...
func (t *Tracer) PopulateLLMRequestAttributes(handle schemas.SpanHandle, req *schemas.DeepIntShieldRequest) {
	h, ok := handle.(*spanHandle)
	if !ok || h == nil || req == nil {
		return
	}
...
func (t *Tracer) PopulateLLMResponseAttributes(handle schemas.SpanHandle, resp *schemas.DeepIntShieldResponse, err *schemas.DeepIntShieldError) {
	h, ok := handle.(*spanHandle)
	if !ok || h == nil {
		return
	}
...
func (t *Tracer) AddStreamingChunk(traceID string, response *schemas.DeepIntShieldResponse) {
...
func (t *Tracer) GetAccumulatedChunks(traceID string) (*schemas.DeepIntShieldResponse, int64, int) {
...
func (t *Tracer) ProcessStreamingChunk(traceID string, isFinalChunk bool, result *schemas.DeepIntShieldResponse, err *schemas.DeepIntShieldError) *schemas.StreamAccumulatorResult {
	if traceID == "" || t.accumulator == nil {
		return nil
	}

	// Create a new context for accumulator that sets the traceID as the accumulator lookup ID.
	accumCtx := schemas.NewDeepIntShieldContext(context.Background(), time.Time{})
	accumCtx.SetValue(schemas.DeepIntShieldContextKeyAccumulatorID, traceID)
	accumCtx.SetValue(schemas.DeepIntShieldContextKeyStreamEndIndicator, isFinalChunk)
