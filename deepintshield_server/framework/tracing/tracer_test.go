...
func TestTracer_StartSpan_RootSpanWithW3CParent(t *testing.T) {
	...
	// the root span in DeepIntShield correctly links to the upstream service's span.
	...
	ctx := context.WithValue(context.Background(), schemas.DeepIntShieldContextKeyTraceID, traceID)
	ctx = context.WithValue(ctx, schemas.DeepIntShieldContextKeyParentSpanID, externalParentSpanID)

	// Create root span - this should link to the external parent
	newCtx, handle := tracer.StartSpan(ctx, "deepintshield-http-request", schemas.SpanKindHTTPRequest)
	if handle == nil {
	...
	spanID, ok := newCtx.Value(schemas.DeepIntShieldContextKeySpanID).(string)
	if !ok || spanID == "" {
...
func TestIntegration_FullDistributedTraceFlow(t *testing.T) {
	...
	// Step 4: DeepIntShield creates LLM call span
