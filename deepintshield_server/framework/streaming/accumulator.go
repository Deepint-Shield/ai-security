...
// Returns the value of DeepIntShieldContextKeyAccumulatorID.
func getAccumulatorID(ctx *schemas.DeepIntShieldContext) (string, bool) {
	if id, ok := ctx.Value(schemas.DeepIntShieldContextKeyAccumulatorID).(string); ok && id != "" {
		return id, true
	}
	return "", false
}
...
func (a *Accumulator) ProcessStreamingResponse(ctx *schemas.DeepIntShieldContext, result *schemas.DeepIntShieldResponse, deepintshieldErr *schemas.DeepIntShieldError) (*ProcessedStreamResponse, error) {
	...
	if isChatStreaming {
		...
		return a.processChatStreamingResponse(ctx, result, deepintshieldErr)
		...
		if requestType == schemas.TranscriptionStreamRequest {
			return a.processTranscriptionStreamingResponse(ctx, result, deepintshieldErr)
		}
		if requestType == schemas.SpeechStreamRequest {
			return a.processAudioStreamingResponse(ctx, result, deepintshieldErr)
		}
	} else if isResponsesStreaming {
		// Handle responses streaming with responses accumulation
		return a.processResponsesStreamingResponse(ctx, result, deepintshieldErr)
	} else if isImageStreaming {
		// Handle image streaming
		return a.processImageStreamingResponse(ctx, result, deepintshieldErr)
	}
