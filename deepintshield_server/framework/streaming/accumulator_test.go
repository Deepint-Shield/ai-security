...
func TestChatStreamingFinalChunkNoDeadlock(t *testing.T) {
	...
	ctx.SetValue(schemas.DeepIntShieldContextKeyAccumulatorID, requestID)
	...
	for i := 0; i < 10; i++ {
		chunk := &ChatStreamChunk{
			...
			Delta: &schemas.ChatStreamResponseChoiceDelta{
				Content: deepintshield.Ptr(fmt.Sprintf("chunk %d", i)),
			},
		}
		if i == 9 {
			...
			Choices: []schemas.DeepIntShieldResponseChoice{
				{
					...
					FinishReason: deepintshield.Ptr("stop"),
				},
			},
		...
		go func(goroutineID int) {
			...
			for i := 0; i < chunksPerGoroutine; i++ {
				chunk := &ChatStreamChunk{
					...
					Timestamp:  time.Now(),
					Delta: &schemas.ChatStreamResponseChoiceDelta{
						Content: deepintshield.Ptr(fmt.Sprintf("g%d-c%d", goroutineID, i)),
					},
				}
...
func TestBuildCompleteMessageFromResponsesStreamChunksParallelToolCalls(t *testing.T) {
	...
	makeChunk := func(idx int, resp *schemas.DeepIntShieldResponsesStreamResponse) *ResponsesStreamChunk {
	...
	chunks := []*ResponsesStreamChunk{
		...
		makeChunk(0, &schemas.DeepIntShieldResponsesStreamResponse{
		...
		makeChunk(1, &schemas.DeepIntShieldResponsesStreamResponse{
			Type: schemas.ResponsesStreamResponseTypeOutputItemAdded,
			Item: &schemas.ResponsesMessage{
				ID:   ptr(itemID1),
				Type: schemas.Ptr(schemas.ResponsesMessageTypeFunctionCall),
		...
		makeChunk(2, &schemas.DeepIntShieldResponsesStreamResponse{
			Type:   schemas.ResponsesStreamResponseTypeFunctionCallArgumentsDelta,
			ItemID: ptr(itemID0),
			Delta:  ptr(`{"a": 1`),
		}),
		// Argument delta for call_1
		makeChunk(3, &schemas.DeepIntShieldResponsesStreamResponse{
			Type:   schemas.ResponsesStreamResponseTypeFunctionCallArgumentsDelta,
			ItemID: ptr(itemID1),
			Delta:  ptr(`{"a": 2`),
		}),
		// Argument delta continuation for call_0
		makeChunk(4, &schemas.DeepIntShieldResponsesStreamResponse{
			Type:   schemas.ResponsesStreamResponseTypeFunctionCallArgumentsDelta,
			ItemID: ptr(itemID0),
			Delta:  ptr(`, "b": 3}`),
		}),
		// Argument delta continuation for call_1
		makeChunk(5, &schemas.DeepIntShieldResponsesStreamResponse{
			Type:   schemas.ResponsesStreamResponseTypeFunctionCallArgumentsDelta,
			ItemID: ptr(itemID1),
			Delta:  ptr(`, "b": 4}`),
		}),
...
func TestAudioStreamingFinalChunkNoDeadlock(t *testing.T) {
	...
	for i := 0; i < 8; i++ {
		chunk := &AudioStreamChunk{
			ChunkIndex: i,
			Timestamp:  time.Now(),
			Delta: &schemas.DeepIntShieldSpeechStreamResponse{
...
func TestTranscriptionStreamingFinalChunkNoDeadlock(t *testing.T) {
	...
	for i := 0; i < 6; i++ {
		...
		chunk := &TranscriptionStreamChunk{
			ChunkIndex: i,
			Timestamp:  time.Now(),
			Delta: &schemas.DeepIntShieldTranscriptionStreamResponse{
