...
func (a *Accumulator) buildCompleteMessageFromTranscriptionStreamChunks(chunks []*TranscriptionStreamChunk) *schemas.DeepIntShieldTranscriptionResponse {
	completeMessage := &schemas.DeepIntShieldTranscriptionResponse{}
	finalContent := ""
...
func (a *Accumulator) processAccumulatedTranscriptionStreamingChunks(requestID string, deepintshieldErr *schemas.DeepIntShieldError, isFinalChunk bool) (*AccumulatedData, error) {
...
func (a *Accumulator) processTranscriptionStreamingResponse(ctx *schemas.DeepIntShieldContext, result *schemas.DeepIntShieldResponse, deepintshieldErr *schemas.DeepIntShieldError) (*ProcessedStreamResponse, error) {
	...
	if deepintshieldErr != nil {
		...
		newDelta := &schemas.DeepIntShieldTranscriptionStreamResponse{
		...
		if result.TranscriptionStreamResponse.ExtraFields.RawResponse != nil {
			chunk.RawResponse = deepintshield.Ptr(fmt.Sprintf("%v", result.TranscriptionStreamResponse.ExtraFields.RawResponse))
		}
		if isFinalChunk {
	...
	if isFinalChunk {
		...
		data, processErr := a.processAccumulatedTranscriptionStreamingChunks(requestID, deepintshieldErr, isFinalChunk)
		if processErr != nil {
