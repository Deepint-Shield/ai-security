...
func (a *Accumulator) buildCompleteMessageFromChatStreamChunks(chunks []*ChatStreamChunk) *schemas.ChatMessage {
	...
	type rdAccum struct {
		...
		typ                          schemas.DeepIntShieldReasoningDetailsType
		id, signature                *string
	}
...
func (a *Accumulator) processAccumulatedChatStreamingChunks(requestID string, respErr *schemas.DeepIntShieldError, isFinalChunk bool) (*AccumulatedData, error) {
	...
	if len(accumulator.ChatStreamChunks) > 0 {
		var mergedLogProbs *schemas.DeepIntShieldLogProbs
		for _, chunk := range accumulator.ChatStreamChunks {
			if chunk.LogProbs != nil {
				if mergedLogProbs == nil {
					mergedLogProbs = &schemas.DeepIntShieldLogProbs{}
				}
...
func (a *Accumulator) processChatStreamingResponse(ctx *schemas.DeepIntShieldContext, result *schemas.DeepIntShieldResponse, deepintshieldErr *schemas.DeepIntShieldError) (*ProcessedStreamResponse, error) {
	...
	if deepintshieldErr != nil {
		...
		if result.ChatResponse.ExtraFields.RawResponse != nil {
			chunk.RawResponse = deepintshield.Ptr(fmt.Sprintf("%v", result.ChatResponse.ExtraFields.RawResponse))
		}
		if isFinalChunk {
			if a.pricingManager != nil {
				cost := a.pricingManager.CalculateCost(result)
	...
	if isFinalChunk {
		...
		data, processErr := a.processAccumulatedChatStreamingChunks(requestID, deepintshieldErr, isFinalChunk)
		if processErr != nil {
