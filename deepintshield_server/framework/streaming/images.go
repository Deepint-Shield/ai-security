...
func (a *Accumulator) buildCompleteImageFromImageStreamChunks(chunks []*ImageStreamChunk) *schemas.DeepIntShieldImageGenerationResponse {
	...
	for i := range len(chunks) {
		if chunks[i].Delta != nil && (chunks[i].FinishReason != nil || chunks[i].Delta.Type == schemas.ImageGenerationEventTypeCompleted || chunks[i].Delta.Type == schemas.ImageEditEventTypeCompleted) {
			finalResponse := &schemas.DeepIntShieldImageGenerationResponse{
...
func (a *Accumulator) processAccumulatedImageStreamingChunks(requestID string, deepintshieldErr *schemas.DeepIntShieldError, isFinalChunk bool) (*AccumulatedData, error) {
...
func (a *Accumulator) processImageStreamingResponse(ctx *schemas.DeepIntShieldContext, result *schemas.DeepIntShieldResponse, deepintshieldErr *schemas.DeepIntShieldError) (*ProcessedStreamResponse, error) {
	...
	if deepintshieldErr != nil {
		...
		newDelta := &schemas.DeepIntShieldImageGenerationStreamResponse{
		...
		if result.ImageGenerationStreamResponse.ExtraFields.RawResponse != nil {
			chunk.RawResponse = deepintshield.Ptr(fmt.Sprintf("%v", result.ImageGenerationStreamResponse.ExtraFields.RawResponse))
		}
		...
		if isFinalChunk {
			...
			chunk.FinishReason = deepintshield.Ptr("completed")
		}
	}
	...
	if isFinalChunk {
		...
		if shouldProcess {
			data, processErr := a.processAccumulatedImageStreamingChunks(requestID, deepintshieldErr, isFinalChunk)
			if processErr != nil {
