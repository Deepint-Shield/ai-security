...
// deepCopyResponsesStreamResponse creates a deep copy of DeepIntShieldResponsesStreamResponse
// to prevent shared data mutation between different plugin accumulators
func deepCopyResponsesStreamResponse(original *schemas.DeepIntShieldResponsesStreamResponse) *schemas.DeepIntShieldResponsesStreamResponse {
	if original == nil {
		return nil
	}

	copy := &schemas.DeepIntShieldResponsesStreamResponse{
		Type:           original.Type,
	...
	if original.Response != nil {
		copy.Response = &schemas.DeepIntShieldResponsesResponse{}
...
func (a *Accumulator) processAccumulatedResponsesStreamingChunks(requestID string, respErr *schemas.DeepIntShieldError, isFinalChunk bool) (*AccumulatedData, error) {
...
func (a *Accumulator) processResponsesStreamingResponse(ctx *schemas.DeepIntShieldContext, result *schemas.DeepIntShieldResponse, deepintshieldErr *schemas.DeepIntShieldError) (*ProcessedStreamResponse, error) {
	...
	if deepintshieldErr != nil {
		...
		if result.ResponsesStreamResponse.ExtraFields.RawResponse != nil {
			chunk.RawResponse = deepintshield.Ptr(fmt.Sprintf("%v", result.ResponsesStreamResponse.ExtraFields.RawResponse))
		}
		...
		if result.ResponsesStreamResponse.Response != nil &&
			...
			chunk.TokenUsage = &schemas.DeepIntShieldLLMUsage{
	...
	if isFinalChunk {
		...
		data, processErr := a.processAccumulatedResponsesStreamingChunks(requestID, deepintshieldErr, isFinalChunk)
		if processErr != nil {
