...
const (
	...
	PineconeTestNamespace        = "deepintshield-test-namespace"
...
func NewPineconeTestSetup(t *testing.T) *PineconeTestSetup {
	...
	if err != nil {
		cancel()
		// Skip rather than fail when the Pinecone Local container
		// (default localhost:5081) isn't running. Operator-run
		// integration test; production Pinecone use is set via
		// PINECONE_API_KEY / PINECONE_INDEX_HOST.
		t.Skipf("Pinecone integration unavailable: %v (start Pinecone Local or set PINECONE_INDEX_HOST)", err)
	}
