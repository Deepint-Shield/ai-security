...
func TestExtractFTInfoIndexedFields(t *testing.T) {
	info := []interface{}{
		"index_name", "DeepIntShieldSemanticCachePlugin",
		"attributes", []interface{}{
			[]interface{}{"identifier", "embedding", "attribute", "embedding", "type", "VECTOR"},
			[]interface{}{"identifier", "request_hash", "attribute", "request_hash", "type", "TAG"},
			[]interface{}{"identifier", "tenant_id", "attribute", "tenant_id", "type", "TAG"},
		},
		"field statistics", []interface{}{
			[]interface{}{"identifier", "request_hash", "attribute", "request_hash"},
		},
	}

	fields := extractFTInfoIndexedFields(info)
	assert.Contains(t, fields, "embedding")
	assert.Contains(t, fields, "request_hash")
	assert.Contains(t, fields, "tenant_id")
}
...
func TestBuildRedisQueryCondition_NumericEquality(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "tag values escape scoped cache key separators",
			query: Query{
				Field:    "cache_key",
				Operator: QueryOperatorEqual,
				Value:    "tenant:global|vk:vk-shared|scope:virtual_key:abcdef",
			},
			expected: "@cache_key:{tenant\\:global\\|vk\\:vk\\-shared\\|scope\\:virtual_key\\:abcdef}",
		},
	}
...
func TestIsQuerySyntaxError(t *testing.T) {
	assert.True(t, isQuerySyntaxError("syntax error at offset 51 near tenant"))
	assert.True(t, isQuerySyntaxError("unknown field at offset 136 near tenant_id"))
	assert.False(t, isQuerySyntaxError("connection refused"))
}

func ptr(v string) *string {
	return deepintshield.Ptr(v)
}
...
func TestRedisStore_CompleteUseCases(t *testing.T) {
	...
	t.Run("Semantic Cache-like Workflow", func(t *testing.T) {
		...
		cacheEntries := []struct {
		...
		}{
			{
				...
				map[string]interface{}{
					...
					"lang":                               "en",
					"response":                           "answer1",
					"from_deepintshield_semantic_cache_plugin": true,
				},
			},
			{
				generateUUID(),
		...
		filterFields := []string{"request_hash", "user", "lang", "response", "from_deepintshield_semantic_cache_plugin"}
...
func TestRedisStore_CreateNamespace_ReconcilesMissingFields(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration tests in short mode")
	}
	...
	namespace := "TestSchemaReconcile"
	require.NoError(t, setup.Store.DeleteNamespace(setup.ctx, namespace))

	initialProperties := map[string]VectorStoreProperties{
		"request_hash": {DataType: VectorStorePropertyTypeString},
		"cache_key":    {DataType: VectorStorePropertyTypeString},
	}

	require.NoError(t, setup.Store.CreateNamespace(setup.ctx, namespace, RedisTestDimension, initialProperties))

	upgradedProperties := map[string]VectorStoreProperties{
		"request_hash": {DataType: VectorStorePropertyTypeString},
		"cache_key":    {DataType: VectorStorePropertyTypeString},
		"tenant_id":    {DataType: VectorStorePropertyTypeString},
		"expires_at":   {DataType: VectorStorePropertyTypeInteger},
	}

	require.NoError(t, setup.Store.CreateNamespace(setup.ctx, namespace, RedisTestDimension, upgradedProperties))

	infoResult := setup.Store.client.Do(setup.ctx, "FT.INFO", namespace)
	require.NoError(t, infoResult.Err())
	fields := extractFTInfoIndexedFields(infoResult.Val())
	assert.Contains(t, fields, "tenant_id")
	assert.Contains(t, fields, "expires_at")
	...
	require.NoError(t, setup.Store.Add(setup.ctx, namespace, "doc-1", embedding, metadata))

	results, _, err := setup.Store.GetAll(setup.ctx, namespace, []Query{
		{Field: "tenant_id", Operator: QueryOperatorEqual, Value: "tenant-a"},
	}, []string{"tenant_id", "request_hash"}, nil, 10)
	require.NoError(t, err)
	require.Len(t, results, 1)
	assert.Equal(t, "tenant-a", results[0].Properties["tenant_id"])
