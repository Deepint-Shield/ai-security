...
const (
	...
	QdrantTestCollection  = "deepintshield-test-collection"
...
func NewQdrantTestSetup(t *testing.T) *QdrantTestSetup {
	...
	if err != nil {
		cancel()
		// Skip rather than fail when the Qdrant integration service
		// isn't reachable - these tests are operator-run integration
		// checks, not unit tests. Set QDRANT_HOST/QDRANT_PORT (or
		// boot the docker-compose-test profile) to exercise them.
		t.Skipf("Qdrant integration unavailable: %v (set QDRANT_HOST/QDRANT_PORT or skip)", err)
	}
