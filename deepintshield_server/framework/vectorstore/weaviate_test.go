...
func NewTestSetup(t *testing.T) *TestSetup {
	...
	if err != nil {
		cancel()
		// Skip rather than fail when Weaviate isn't reachable.
		// Operator-run integration test - set WEAVIATE_HOST/PORT to
		// exercise; default localhost:9000 isn't running in CI.
		t.Skipf("Weaviate integration unavailable: %v (set WEAVIATE_HOST/WEAVIATE_PORT or skip)", err)
	}
