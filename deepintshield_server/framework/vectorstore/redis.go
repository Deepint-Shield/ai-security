...
func (s *RedisStore) CreateNamespace(ctx context.Context, namespace string, dimension int, properties map[string]VectorStoreProperties) error {
	...
	if infoResult.Err() == nil {
		if err := s.reconcileNamespaceSchema(ctx, namespace, properties, infoResult.Val()); err != nil {
			return err
		}
	...
	if err := infoResult.Err(); err != nil && strings.Contains(strings.ToLower(err.Error()), "unknown command") {
		return fmt.Errorf("search module not available: please use Redis 8+, Redis Stack, or a Valkey bundle with search support (FT.* commands required). original error: %w", err)
	}
...
func (s *RedisStore) reconcileNamespaceSchema(ctx context.Context, namespace string, properties map[string]VectorStoreProperties, info interface{}) error {
	existingFields := extractFTInfoIndexedFields(info)
	if len(existingFields) == 0 {
		s.logger.Warn(fmt.Sprintf("could not determine indexed fields for Redis namespace %s from FT.INFO; leaving schema unchanged", namespace))
		return nil
	}

	missingFields := make([]string, 0)
	for fieldName := range properties {
		if _, ok := existingFields[fieldName]; !ok {
	...
	for _, fieldName := range missingFields {
		args := []interface{}{"FT.ALTER", namespace, "SCHEMA", "ADD"}
		args = append(args, buildRedisSchemaFieldArgs(fieldName, properties[fieldName])...)
		if err := s.client.Do(ctx, args...).Err(); err != nil {
			errMsg := strings.ToLower(err.Error())
			if strings.Contains(errMsg, "already contains field") || strings.Contains(errMsg, "field already exists") {
				continue
			}
			return fmt.Errorf("failed to reconcile Redis namespace %s with field %s: %w", namespace, fieldName, err)
		}
		s.logger.Info(fmt.Sprintf("reconciled Redis namespace %s by adding missing field %s", namespace, fieldName))
	}

	return nil
}

func buildRedisSchemaFieldArgs(fieldName string, property VectorStoreProperties) []interface{} {
	switch property.DataType {
	case VectorStorePropertyTypeInteger:
		return []interface{}{fieldName, "NUMERIC"}
	default:
		return []interface{}{fieldName, "TAG"}
	}
}
...
func extractFTInfoIndexedFields(info interface{}) map[string]struct{} {
	fields := make(map[string]struct{})
	collectFTInfoIndexedFields(info, fields)
	return fields
}

func collectFTInfoIndexedFields(raw interface{}, fields map[string]struct{}) {
	switch typed := raw.(type) {
	case map[string]interface{}:
		for key, value := range typed {
			lowerKey := strings.ToLower(strings.TrimSpace(key))
			if (lowerKey == "identifier" || lowerKey == "attribute") && addIndexedField(fields, value) {
				continue
			}
			collectFTInfoIndexedFields(value, fields)
		}
	case map[interface{}]interface{}:
		for key, value := range typed {
			keyStr, ok := toString(key)
			if ok {
				lowerKey := strings.ToLower(strings.TrimSpace(keyStr))
				if (lowerKey == "identifier" || lowerKey == "attribute") && addIndexedField(fields, value) {
					continue
				}
			}
	...
	case []interface{}:
		for i := 0; i < len(typed); i++ {
			key, ok := toString(typed[i])
			if !ok || i+1 >= len(typed) {
				collectFTInfoIndexedFields(typed[i], fields)
				continue
			}

			lowerKey := strings.ToLower(strings.TrimSpace(key))
			if lowerKey == "identifier" || lowerKey == "attribute" {
				if addIndexedField(fields, typed[i+1]) {
					i++
					continue
				}
			}
...
func addIndexedField(fields map[string]struct{}, value interface{}) bool {
	fieldName, ok := toString(value)
	if !ok {
		return false
	}
...
func (s *RedisStore) DeleteNamespace(ctx context.Context, namespace string) error {
	...
	if err := s.client.Do(ctx, "FT.DROPINDEX", namespace).Err(); err != nil {
		...
		errMsg := strings.ToLower(err.Error())
		if strings.Contains(errMsg, "unknown index name") || strings.Contains(errMsg, "no such index") {
