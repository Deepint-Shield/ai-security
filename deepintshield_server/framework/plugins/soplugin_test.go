...
func TestDynamicPluginLifecycle(t *testing.T) {
	...
	t.Run("HTTPTransportPreHook", func(t *testing.T) {
		...
		pluginCtx, cancel := schemas.NewDeepIntShieldContextWithTimeout(ctx, 10*time.Second)
		defer cancel()
	...
	t.Run("PostLLMHook", func(t *testing.T) {
		...
		deepintshieldErr := (*schemas.DeepIntShieldError)(nil)
		pluginCtx, cancel := schemas.NewDeepIntShieldContextWithTimeout(ctx, 10*time.Second)
		defer cancel()
		modifiedResp, modifiedErr, err := plugin.PostLLMHook(pluginCtx, resp, deepintshieldErr)
		require.NoError(t, err, "PostLLMHook should not return error")
		assert.Equal(t, resp, modifiedResp, "Response should be unchanged")
		assert.Equal(t, deepintshieldErr, modifiedErr, "Error should be unchanged")
	})
	...
	t.Run("PostHook_WithError", func(t *testing.T) {
		...
		pluginCtx, cancel := schemas.NewDeepIntShieldContextWithTimeout(ctx, 10*time.Second)
		defer cancel()
		modifiedResp, modifiedErr, err := plugin.PostLLMHook(pluginCtx, nil, deepintshieldErr)
		require.NoError(t, err, "PostLLMHook should not return error")
...
func TestLoadPluginWithOptionalHooks(t *testing.T) {
	...
	t.Run("UnimplementedMCPHooks", func(t *testing.T) {
		...
		mcpReq := &schemas.DeepIntShieldMCPRequest{}
		...
		deepintshieldErr := &schemas.DeepIntShieldError{}
		returnedResp, returnedErr, hookErr := dynamicPlugin.PostMCPHook(pluginCtx, mcpResp, deepintshieldErr)
		...
		assert.Equal(t, deepintshieldErr, returnedErr, "PostMCPHook should return original error")
	})
