...
// GetPrompts gets all prompts, optionally filtered by folder ID. Note: this
// does not apply workspace scoping - use GetPromptsScoped for workspace-
// aware reads. The caller (governance plugin / inference path) should rely
// on tenant scoping enforced at the connection or request-context layer.
...
// GetPromptsScoped is GetPrompts with strict per-workspace scoping. Empty
// workspaceID degrades to GetPrompts (no workspace filter). The earlier
// "workspace_id = ? OR workspace_id IS NULL" fallback was dropped in the
// per-workspace rollout - historical NULL rows leaked across every
// workspace under the tenant.
func (s *RDBConfigStore) GetPromptsScoped(ctx context.Context, folderID *string, workspaceID string) ([]tables.TablePrompt, error) {
	if workspaceID == "" {
		return s.GetPrompts(ctx, folderID)
	}
	var prompts []tables.TablePrompt
	query := s.db.WithContext(ctx).
		Preload("Folder").
...
// CreatePrompt creates a new prompt. Defensive: stamp WorkspaceID from the
// request context when the caller didn't already set it so newly-created
// prompts are never tenant-wide by accident - the read path is strict
// per-workspace, so a NULL workspace_id row would be invisible everywhere.
func (s *RDBConfigStore) CreatePrompt(ctx context.Context, prompt *tables.TablePrompt) error {
	if prompt.WorkspaceID == nil {
