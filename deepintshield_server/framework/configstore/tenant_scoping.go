...
const (
	configstoreTenantCreateCallback = "deepintshield:configstore_assign_tenant"
	configstoreTenantQueryCallback  = "deepintshield:configstore_scope_tenant_query"
	configstoreTenantUpdateCallback = "deepintshield:configstore_scope_tenant_update"
	configstoreTenantDeleteCallback = "deepintshield:configstore_scope_tenant_delete"
)

func registerTenantCallbacks(db *gorm.DB) error {
	if err := db.Callback().Create().Before("gorm:create").Register(configstoreTenantCreateCallback, assignTenantOnCreate); err != nil {
		return err
	}
	if err := db.Callback().Query().Before("gorm:query").Register(configstoreTenantQueryCallback, scopeTenantStatement); err != nil {
		return err
	}
	if err := db.Callback().Update().Before("gorm:update").Register(configstoreTenantUpdateCallback, scopeTenantStatement); err != nil {
		return err
	}
	if err := db.Callback().Delete().Before("gorm:delete").Register(configstoreTenantDeleteCallback, scopeTenantStatement); err != nil {
		return err
	}
	return nil
}
