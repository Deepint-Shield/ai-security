...
// ClientConfig represents the core configuration for DeepIntShield HTTP transport and the DeepIntShield Client.
...
type ClientConfig struct {
	...
	InitialPoolSize                 int                              `json:"initial_pool_size"`       // The initial pool size for the deepintshield client
	...
	MCPCacheEnabled                 *bool                            `json:"mcp_cache_enabled,omitempty"`          // Direct (non-semantic) MCP tool result cache. Defaults to true when nil; toggle from MCP Settings.
	MCPCacheTTLSeconds              int                              `json:"mcp_cache_ttl_seconds,omitempty"`      // TTL for cached MCP tool results in seconds. 0 = use plugin default (300).
	...
	LoadBalancerEnabled            bool                             `json:"load_balancer_enabled"`                // Enable LLM Key Load Balancer feature (strategies, circuit breaker, health tracking)
...
func (p *ProviderConfig) Redacted() *ProviderConfig {
	...
	for i, key := range p.Keys {
		...
		if key.UseForBatchAPI != nil {
			...
			redactedConfig.Keys[i].UseForBatchAPI = deepintshield.Ptr(false)
		}
		if key.UseForCache != nil {
			redactedConfig.Keys[i].UseForCache = key.UseForCache
		} else {
			redactedConfig.Keys[i].UseForCache = deepintshield.Ptr(true)
		}
...
func GenerateKeyHash(key schemas.Key) (string, error) {
	...
	// Hash UseForCache (nil = default true)
	useForCache := true
	if key.UseForCache != nil {
		useForCache = *key.UseForCache
	}
	if !useForCache {
		hash.Write([]byte("useForCache:false"))
	}
	return hex.EncodeToString(hash.Sum(nil)), nil
}
...
type VirtualKeyHashInput struct {
	...
	CacheMetadataScopeKeys         []string
	CacheAllowSemanticWhenUnscoped *bool
	TeamID                         *string
	CustomerID                     *string
	BudgetID                       *string
	RateLimitID                    *string
...
func GenerateVirtualKeyHash(vk tables.TableVirtualKey) (string, error) {
	...
	if strings.TrimSpace(vk.CacheKey) != "" {
		hash.Write([]byte("cacheKey:" + strings.TrimSpace(vk.CacheKey)))
	}
	// Hash CacheEnabled (nil = default true)
	cacheEnabled := true
	if vk.CacheEnabled != nil {
		cacheEnabled = *vk.CacheEnabled
	}
	if !cacheEnabled {
		hash.Write([]byte("cacheEnabled:false"))
	}
	// Hash SemanticCacheEnabled (nil = default true)
	semanticCacheEnabled := true
	if vk.SemanticCacheEnabled != nil {
		semanticCacheEnabled = *vk.SemanticCacheEnabled
	}
	if !semanticCacheEnabled {
		hash.Write([]byte("semanticCacheEnabled:false"))
	}
	// Hash CacheScopeMode (nil = inherit)
	cacheScopeMode := "inherit"
	if vk.CacheScopeMode != nil && *vk.CacheScopeMode != "" {
		cacheScopeMode = *vk.CacheScopeMode
	}
	hash.Write([]byte("cacheScopeMode:" + cacheScopeMode))
	// Hash CacheMetadataScopeKeys (sorted for deterministic hashing)
	if len(vk.CacheMetadataScopeKeys) > 0 {
		sortedMetadataKeys := make([]string, len(vk.CacheMetadataScopeKeys))
		copy(sortedMetadataKeys, vk.CacheMetadataScopeKeys)
		sort.Strings(sortedMetadataKeys)
		data, err := sonic.Marshal(sortedMetadataKeys)
		if err != nil {
			return "", err
		}
		hash.Write([]byte("cacheMetadataScopeKeys:"))
		hash.Write(data)
	}
	// Hash CacheAllowSemanticWhenUnscoped (nil = false)
	cacheAllowSemanticWhenUnscoped := false
	if vk.CacheAllowSemanticWhenUnscoped != nil {
		cacheAllowSemanticWhenUnscoped = *vk.CacheAllowSemanticWhenUnscoped
	}
	if cacheAllowSemanticWhenUnscoped {
		hash.Write([]byte("cacheAllowSemanticWhenUnscoped:true"))
	}
	// Hash TeamID
	if vk.TeamID != nil {
...
// AuthConfig represents configured auth config for DeepIntShield dashboard
