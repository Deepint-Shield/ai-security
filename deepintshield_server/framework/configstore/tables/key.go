...
type TableKey struct {
	...
	// Unique constraints include workspace_id so the same key name (or
	// the same UUID) can co-exist in sibling workspaces of the same
	// tenant - same rationale as TableProvider's
	// idx_provider_tenant_workspace_name. Without this the user gets
	// "Failed to update provider: a record with this name already
	// exists" when they try to add an `openai_key` in workspace B
	// after one already exists in workspace A.
	TenantID    string         `gorm:"column:tenant_id;type:varchar(255);index;uniqueIndex:idx_key_tenant_workspace_name;uniqueIndex:idx_key_tenant_workspace_key_id" json:"-"`
	WorkspaceID *string        `gorm:"column:workspace_id;type:varchar(255);index;uniqueIndex:idx_key_tenant_workspace_name;uniqueIndex:idx_key_tenant_workspace_key_id" json:"workspace_id,omitempty"` // Empty/NULL = tenant-wide (legacy); non-NULL = scoped to that workspace.
	Name        string         `gorm:"type:varchar(255);not null;uniqueIndex:idx_key_tenant_workspace_name" json:"name"`
	ProviderID  uint           `gorm:"index;not null" json:"provider_id"`
	Provider    string         `gorm:"index;type:varchar(50)" json:"provider"`                                               // ModelProvider as string
	KeyID       string         `gorm:"type:varchar(255);not null;uniqueIndex:idx_key_tenant_workspace_key_id" json:"key_id"` // UUID from schemas.Key
	...
	UseForCache    *bool `gorm:"default:true" json:"use_for_cache,omitempty"`      // Whether this key can be used for semantic cache reads/writes
...
func (k *TableKey) BeforeSave(tx *gorm.DB) error {
	...
	if k.UseForCache == nil {
		useForCache := true // DB default
		k.UseForCache = &useForCache
	}
