...
type TableRoutingRule struct {
	...
	// Unique key includes workspace_id so the same routing rule name can
	// exist in sibling workspaces of the same tenant - same rationale
	// as TableProvider's idx_provider_tenant_workspace_name.
	TenantID      string  `gorm:"column:tenant_id;type:varchar(255);index;uniqueIndex:idx_routing_rule_tenant_workspace_scope_name" json:"-"`
	WorkspaceID   *string `gorm:"column:workspace_id;type:varchar(255);index;uniqueIndex:idx_routing_rule_tenant_workspace_scope_name" json:"workspace_id,omitempty"` // Empty/NULL = tenant-wide (legacy); non-NULL = scoped to that workspace.
	ConfigHash    string  `gorm:"type:varchar(255)" json:"config_hash"`                                                                                               // Hash of config.json version, used for change detection
	Name          string  `gorm:"type:varchar(255);not null;uniqueIndex:idx_routing_rule_tenant_workspace_scope_name" json:"name"`
	...
	Scope   string  `gorm:"type:varchar(50);not null;uniqueIndex:idx_routing_rule_tenant_workspace_scope_name" json:"scope"` // "global" | "team" | "customer" | "virtual_key"
	ScopeID *string `gorm:"type:varchar(255);uniqueIndex:idx_routing_rule_tenant_workspace_scope_name" json:"scope_id"`      // nil for global, otherwise entity ID
...
func (r *TableRoutingRule) BeforeSave(tx *gorm.DB) error {
	if len(r.ParsedFallbacks) > 0 {
		...
		r.Fallbacks = deepintshield.Ptr(string(data))
	} else {
		r.Fallbacks = nil
	}
	if r.ParsedQuery != nil {
		...
		r.Query = deepintshield.Ptr(string(data))
	} else {
		r.Query = nil
	}
	return nil
