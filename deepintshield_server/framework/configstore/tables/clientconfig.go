...
type TableClientConfig struct {
	...
	TenantID                        string `gorm:"column:tenant_id;type:varchar(255);index:idx_config_client_tenant,unique" json:"-"`
	...
	MCPCacheEnabled                 *bool  `gorm:"default:true" json:"mcp_cache_enabled,omitempty"`           // Direct (non-semantic) MCP tool result cache. nil/true = enabled.
	MCPCacheTTLSeconds              int    `gorm:"default:300" json:"mcp_cache_ttl_seconds,omitempty"`        // TTL for cached MCP tool results in seconds (default: 300)
	...
	// LLM Load Balancer feature switch. When true the gateway uses the
	// strategy-aware key selector (weighted random / round robin / least
	// load + circuit breaker + health tracking) instead of the default
	// stateless weighted-random selector. Default true so the feature
	// works out-of-the-box once a virtual key declares a strategy; admins
	// can flip it off via Client Settings if they want the cheaper
	// stateless path.
	LoadBalancerEnabled bool `gorm:"column:load_balancer_enabled;default:true" json:"load_balancer_enabled"`
