...
func init() {
	encrypt.Init(testEncryptionKey, deepintshield.NewDefaultLogger(schemas.LogLevelInfo))
}
...
func disableEncryption(t *testing.T) {
	t.Helper()
	encrypt.Init("", deepintshield.NewDefaultLogger(schemas.LogLevelInfo))
	t.Cleanup(func() {
		encrypt.Init(testEncryptionKey, deepintshield.NewDefaultLogger(schemas.LogLevelInfo))
	})
}
...
const postgresDSN = "host=localhost user=deepintshield password=deepintshield_password dbname=deepintshield port=5432 sslmode=disable"
