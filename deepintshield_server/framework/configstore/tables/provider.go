...
type TableProvider struct {
	...
	// Unique constraint key is (tenant_id, workspace_id, name) - i.e. a
	// provider name like "openai" can exist in workspace A AND workspace
	// B under the same tenant without colliding. Without workspace_id in
	// the key, adding the same provider in a sibling workspace fails
	// with a 409 unique-violation, even though strict workspace scoping
	// (see GetProvidersConfig) treats them as independent rows.
	TenantID                 string    `gorm:"column:tenant_id;type:varchar(255);index;uniqueIndex:idx_provider_tenant_workspace_name" json:"-"`
	WorkspaceID              *string   `gorm:"column:workspace_id;type:varchar(255);index;uniqueIndex:idx_provider_tenant_workspace_name" json:"workspace_id,omitempty"` // Empty/NULL = tenant-wide (legacy); non-NULL = scoped to that workspace.
	Name                     string    `gorm:"type:varchar(50);not null;uniqueIndex:idx_provider_tenant_workspace_name" json:"name"`                                     // ModelProvider as string
