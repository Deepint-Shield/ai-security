...
type TablePlugin struct {
	...
	TenantID    string    `gorm:"column:tenant_id;type:varchar(255);index;uniqueIndex:idx_plugin_tenant_workspace_name" json:"-"`
	WorkspaceID *string   `gorm:"column:workspace_id;type:varchar(255);index;uniqueIndex:idx_plugin_tenant_workspace_name" json:"workspace_id,omitempty"` // NULL = tenant-wide; non-NULL = scoped to that workspace.
	Name        string    `gorm:"type:varchar(255);not null;uniqueIndex:idx_plugin_tenant_workspace_name" json:"name"`
