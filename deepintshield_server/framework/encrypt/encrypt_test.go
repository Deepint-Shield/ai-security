...
func TestEncryptDecrypt(t *testing.T) {
	...
	Init(testKey, deepintshield.NewDefaultLogger(schemas.LogLevelInfo))
...
func TestKDFWithVariousKeyLengths(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			Init(tc.key, deepintshield.NewDefaultLogger(schemas.LogLevelInfo))
...
func TestKDFDeterministic(t *testing.T) {
	...
	Init(passphrase, deepintshield.NewDefaultLogger(schemas.LogLevelInfo))
	encrypted1, err := Encrypt(plaintext)
	if err != nil {
		t.Fatalf("Failed to encrypt: %v", err)
	}
