...
func Init(key string, _logger schemas.Logger) {
	...
	if key == "" {
		encryptionKey = nil
		logger.Warn("encryption key is not set, encryption will be disabled. To set encryption key: use the encryption_key field in the configuration file or set the DEEPINTSHIELD_ENCRYPTION_KEY environment variable. Note that - once encryption key is set, it cannot be changed later unless you clean up the database.")
		return
	}
	...
	salt := []byte("deepintshield-encryption-v1-salt-2024")
