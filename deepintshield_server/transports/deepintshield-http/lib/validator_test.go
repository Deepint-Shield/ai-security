...
func TestSchemaCandidatePaths_PrefersExecutableDirectory(t *testing.T) {
	paths := schemaCandidatePaths("/tmp/deepintshield/bin/main")
	if len(paths) < 3 {
		t.Fatalf("expected multiple schema candidates, got %v", paths)
	}
	if paths[0] != "/tmp/deepintshield/bin/config.schema.json" {
		t.Fatalf("expected executable directory candidate first, got %q", paths[0])
	}
	if paths[1] != "/tmp/deepintshield/config.schema.json" {
		t.Fatalf("expected executable parent candidate second, got %q", paths[1])
	}
	foundBundledPath := false
	for _, path := range paths {
		if path == "/app/config.schema.json" {
			foundBundledPath = true
			break
		}
	}
	if !foundBundledPath {
		t.Fatalf("expected bundled container schema path in candidates, got %v", paths)
	}
}
...
func TestValidateConfigSchema_ValidRedisVectorStoreEnvDB(t *testing.T) {
	validConfig := `{
		"vector_store": {
			"enabled": true,
			"type": "redis",
	...
	err := ValidateConfigSchema([]byte(validConfig), loadLocalSchema(t))
	if err != nil {
		t.Errorf("expected redis vector store config with env db to pass validation, got error: %v", err)
	}
}

func TestValidateConfigSchema_PostgresRedisExampleConfig(t *testing.T) {
	_, filename, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("failed to get current file path")
	}
	configPath := filepath.Join(filepath.Dir(filename), "..", "..", "..", "examples", "dockers", "postgres-redis", "config.json")
	data, err := os.ReadFile(configPath)
	if err != nil {
		t.Fatalf("failed to read postgres-redis example config: %v", err)
	}

	err = ValidateConfigSchema(data, loadLocalSchema(t))
	if err != nil {
		t.Fatalf("expected postgres-redis example config to pass validation, got error: %v", err)
	}
}
		...
		"plugins": [
			{
				...
				"config": {
					"provider": "openai",
					"keys": [{
						"name": "Semantic Cache Embedding Key",
...
func TestValidateConfigSchema_SemanticCachePlugin_KeysOptional(t *testing.T) {
	// 'keys' is optional: with embedding-via-VK the embeddings are produced
	// using the virtual key's provider, so a semantic_cache config with only
	// the required provider + dimension is valid.
	validConfig := `{
		"plugins": [
			{
				"enabled": true,
	...
	err := ValidateConfigSchema([]byte(validConfig), loadLocalSchema(t))
	if err != nil {
		t.Errorf("expected semantic cache config without 'keys' to be valid (embedding-via-VK), got: %v", err)
	}
}
