...
import (
	...
	"github.com/deepint-shield/ai-security/framework/envutils"
	...
	plugins "github.com/deepint-shield/ai-security/framework/plugins"
...
// Plugin-name constants for premium plugins that are not part of the
// open-source build. The names are retained so any persisted/config-file
// plugin entries with these names are still recognised (provider-key
// injection, vector-store auto-config) - the gateway simply never loads the
// plugin instances themselves.
const (
	semanticCachePluginName           = "semantic_cache"
	semanticCacheDirectGatePluginName = "semantic_cache_direct_gate"
	responseConsistencyPluginName     = "response_consistency"
	mcpCachePluginName                = "mcpcache"
)
...
type StreamChunkInterceptor interface {
	...
	InterceptChunk(ctx *schemas.DeepIntShieldContext, req *schemas.HTTPRequest, chunk *schemas.DeepIntShieldStreamChunk) (*schemas.DeepIntShieldStreamChunk, error)
}
...
func IsBuiltinPlugin(name string) bool {
	return name == telemetry.PluginName ||
		...
		name == guardrails.PluginName ||
		name == litellmcompat.PluginName ||
		name == otel.PluginName ||
		name == semanticCachePluginName
}
...
// ConfigData represents the configuration data for the DeepIntShield HTTP transport.
					...
					for _, tableKey := range providerConfig.Keys {
						if tableKey.Value.GetValue() != "" {
							...
							keysToAddToProvider = append(keysToAddToProvider, schemas.Key{
								...
								UseForCache:      tableKey.UseForCache,
...
// Config represents a high-performance in-memory configuration store for DeepIntShield.
...
func LoadConfig(ctx context.Context, configDirPath string) (*Config, error) {
	...
	if err != nil {
		...
		if schema["$schema"] != "https://deepintshield.com/schema" {
			yellowColor := "\033[33m"
			resetColor := "\033[0m"
			message := fmt.Sprintf("config file %s does not include \"$schema\":\"https://deepintshield.com/schema\". Use our official schema file to avoid unexpected behavior.", absConfigFilePath)
			boxWidth := 100
			...
			fmt.Printf("%s╚%s╝%s\n", yellowColor, strings.Repeat("═", boxWidth-2), resetColor)
			fmt.Println("")
			logger.Warn("config file %s does not include \"$schema\":\"https://deepintshield.com/schema\". Use our official schema file to avoid unexpected behavior.", absConfigFilePath)
		}
		// Validate config file against the schema
		if err := ValidateConfigSchema(data); err != nil {
			logger.Error("config validation failed: %v. You can find the official schema at https://deepintshield.com/schema. Some features may not work as expected unless you fix the config file.", err)
		}
		// Parse config data
	...
	// 2.5. Normalize persisted tenancy to email-scoped partitions before any
	// tenant-aware config is loaded into memory.
	if err := alignEmailScopedTenancy(ctx, config); err != nil {
		return nil, err
	}
	// 3. KV store
...
func initStores(ctx context.Context, config *Config, configData *ConfigData, configDBPath, logsDBPath string) error {
	...
	// Register the config store with the workspace-logging cache so the
	// logging plugin's hot path can resolve per-workspace overrides via the
	// atomic snapshot (zero-DB-RTT on cache hits). Safe to call before the
	// first read because the cache initializes an empty snapshot on
	// register; first lookup misses fall through to a single coalesced DB
	// load via sync.Once-per-workspace.
	if config.ConfigStore != nil {
		configstore.RegisterWorkspaceLoggingStore(config.ConfigStore)
		configstore.RegisterWorkspaceMCPStore(config.ConfigStore)
	}
	...
	vectorStoreConfig, persistResolvedVectorStore, err := resolveVectorStoreConfig(ctx, config, configData)
	if err != nil {
		return err
	}

	// Initialize vector store when configured explicitly, loaded from the config store,
	// or auto-provisioned for semantic cache.
	if vectorStoreConfig != nil && vectorStoreConfig.Enabled {
		logger.Info("connecting to vectorstore")
		config.VectorStore, err = vectorstore.NewVectorStore(ctx, vectorStoreConfig, logger)
		if err != nil {
			// An unreachable vector store must not take the gateway down. The
			// semantic cache plugin falls back to a no-op store, so prompt
			// caching, hallucination control, routing, and guardrails all keep
			// running - only the semantic/direct cache layers go quiet.
			logger.Warn("vector store unavailable, semantic cache disabled: %v", err)
			config.VectorStore = nil
		} else if config.ConfigStore != nil && persistResolvedVectorStore {
...
func resolveVectorStoreConfig(ctx context.Context, config *Config, configData *ConfigData) (*vectorstore.Config, bool, error) {
	if configData.VectorStoreConfig != nil {
		return configData.VectorStoreConfig, true, nil
	}

	if config.ConfigStore != nil {
		vectorStoreConfig, err := config.ConfigStore.GetVectorStoreConfig(ctx)
		if err != nil {
			return nil, false, fmt.Errorf("failed to get vector store config: %w", err)
		}
		if vectorStoreConfig != nil {
			return vectorStoreConfig, false, nil
		}
	}

	// Auto-provision a Redis vector store when either the semantic cache plugin
	// is enabled OR an operator/bundled Redis endpoint is explicitly supplied via
	// DEEPINTSHIELD_REDIS_ADDR (e.g. the all-in-one Docker image or the npx
	// launcher, which both start a redis-stack and export the addr). This makes
	// semantic caching available out of the box instead of showing "requires
	// vector store" while a perfectly good Redis sits unused. If the endpoint is
	// unreachable, the caller degrades gracefully (warn + no semantic cache).
	if !isSemanticCacheEnabled(ctx, config, configData) && strings.TrimSpace(os.Getenv("DEEPINTSHIELD_REDIS_ADDR")) == "" {
		return nil, false, nil
	}

	autoConfig := defaultRedisVectorStoreConfig()
	logger.Info("auto-configuring Redis vector store for the semantic cache")
	return autoConfig, true, nil
}

func isSemanticCacheEnabled(ctx context.Context, config *Config, configData *ConfigData) bool {
	if hasEnabledPluginConfig(configData.Plugins, semanticCachePluginName) {
		return true
	}
	if config.ConfigStore == nil {
		return false
	...
	if err != nil {
		logger.Warn("failed to inspect stored plugins while resolving vector store: %v", err)
		return false
	}
	for _, plugin := range plugins {
		if plugin != nil && plugin.Name == semanticCachePluginName && plugin.Enabled {
			return true
		}
	}
	return false
...
func hasEnabledPluginConfig(plugins []*schemas.PluginConfig, name string) bool {
	for _, plugin := range plugins {
		if plugin != nil && plugin.Name == name && plugin.Enabled {
			return true
		}
	}
	return false
...
func defaultRedisVectorStoreConfig() *vectorstore.Config {
	addrValue := "127.0.0.1:6379"
	if strings.TrimSpace(os.Getenv("DEEPINTSHIELD_REDIS_ADDR")) != "" {
		addrValue = "env.DEEPINTSHIELD_REDIS_ADDR"
	}

	dbValue := "0"
	if strings.TrimSpace(os.Getenv("DEEPINTSHIELD_REDIS_DB")) != "" {
		dbValue = "env.DEEPINTSHIELD_REDIS_DB"
	}

	redisConfig := vectorstore.RedisConfig{
		Addr:           schemas.NewEnvVar(addrValue),
		DB:             schemas.NewEnvVar(dbValue),
		ContextTimeout: 5 * time.Second,
	}

	if strings.TrimSpace(os.Getenv("DEEPINTSHIELD_REDIS_USERNAME")) != "" {
		redisConfig.Username = schemas.NewEnvVar("env.DEEPINTSHIELD_REDIS_USERNAME")
	}
	if strings.TrimSpace(os.Getenv("DEEPINTSHIELD_REDIS_PASSWORD")) != "" {
		redisConfig.Password = schemas.NewEnvVar("env.DEEPINTSHIELD_REDIS_PASSWORD")
	}
...
func processProvider(
	...
	activeKeys, skippedKeys := splitActiveAndSkippedFileKeys(provider, providerCfgInFile.Keys)
	providerCfgInFile.Keys = activeKeys
	...
	mergeProviderWithHash(provider, providerCfgInFile, skippedKeys, providersInConfigStore)
	return nil
}
...
func mergeProviderWithHash(
	...
	if existingCfg.ConfigHash != providerCfgInFile.ConfigHash {
		...
		mergedKeys = pruneSkippedFileKeys(provider, mergedKeys, skippedFileKeys)
		mergedKeys = pruneUnresolvedEnvBackedProviderKeys(provider, mergedKeys)
...
func splitActiveAndSkippedFileKeys(provider schemas.ModelProvider, fileKeys []schemas.Key) ([]schemas.Key, []schemas.Key) {
	if len(fileKeys) == 0 {
		return nil, nil
	}

	activeKeys := make([]schemas.Key, 0, len(fileKeys))
	skippedKeys := make([]schemas.Key, 0)
	for _, key := range fileKeys {
		if shouldSkipUnresolvedEnvFileKey(provider, key) {
			logger.Debug("skipping unresolved env-backed key %s for provider %s", key.Name, provider)
	...
	return activeKeys, skippedKeys
}

func shouldSkipUnresolvedEnvFileKey(provider schemas.ModelProvider, key schemas.Key) bool {
	...
	return !deepintshield.CanProviderKeyValueBeEmpty(provider)
}

func ensureProviderConfigHashes(provider schemas.ModelProvider, config *configstore.ProviderConfig) error {
	if config == nil {
		return nil
	}

	if config.ConfigHash == "" {
		configHash, err := config.GenerateConfigHash(string(provider))
		if err != nil {
			return err
		}
	...
	for i := range config.Keys {
		if config.Keys[i].ConfigHash != "" {
			continue
		}
		keyHash, err := configstore.GenerateKeyHash(config.Keys[i])
		if err != nil {
			return err
		}
		config.Keys[i].ConfigHash = keyHash
	}

	return nil
}

// mergeProviderKeys syncs keys when provider hash has changed. File-defined keys are updated
// from config.json, while keys that exist only in DB are preserved so dashboard-added keys
// survive rebuilds and config reloads.
func mergeProviderKeys(provider schemas.ModelProvider, fileKeys, dbKeys []schemas.Key) []schemas.Key {
	mergedKeys := make([]schemas.Key, 0, len(fileKeys)+len(dbKeys))
	for _, dbKey := range dbKeys {
		...
		// Keep keys that only exist in DB so dashboard-added keys survive file-driven reloads.
		logger.Debug("key %s exists only in DB for provider %s, preserving", dbKey.Name, provider)
...
func pruneSkippedFileKeys(provider schemas.ModelProvider, mergedKeys, skippedFileKeys []schemas.Key) []schemas.Key {
	if len(skippedFileKeys) == 0 || len(mergedKeys) == 0 {
		return mergedKeys
	}

	skippedHashes := make(map[string]struct{}, len(skippedFileKeys))
	skippedEnvRefsByName := make(map[string]string, len(skippedFileKeys))
	for _, skippedFileKey := range skippedFileKeys {
		keyHash, err := configstore.GenerateKeyHash(skippedFileKey)
		if err != nil {
			logger.Warn("failed to generate key hash for skipped file key %s (%s): %v", skippedFileKey.Name, provider, err)
		} else {
			skippedHashes[keyHash] = struct{}{}
		}
		if skippedFileKey.Value.IsFromEnv() {
			skippedEnvRefsByName[skippedFileKey.Name] = skippedFileKey.Value.EnvVar
		}
	}
	if len(skippedHashes) == 0 && len(skippedEnvRefsByName) == 0 {
		return mergedKeys
	}

	prunedKeys := make([]schemas.Key, 0, len(mergedKeys))
	for _, mergedKey := range mergedKeys {
		keyHash := mergedKey.ConfigHash
		if keyHash == "" {
			var err error
			keyHash, err = configstore.GenerateKeyHash(mergedKey)
			if err != nil {
				logger.Warn("failed to generate key hash for merged key %s (%s): %v", mergedKey.Name, provider, err)
				prunedKeys = append(prunedKeys, mergedKey)
				continue
			}
		}

		if _, shouldPrune := skippedHashes[keyHash]; shouldPrune {
			logger.Debug("pruning unresolved env-backed key %s for provider %s", mergedKey.Name, provider)
			continue
		}
		if envRef, exists := skippedEnvRefsByName[mergedKey.Name]; exists &&
			mergedKey.Value.IsFromEnv() &&
			mergedKey.Value.EnvVar == envRef &&
			strings.TrimSpace(mergedKey.Value.GetValue()) == "" {
			logger.Debug("pruning unresolved env-backed key %s for provider %s via env reference fallback", mergedKey.Name, provider)
			continue
		}
		prunedKeys = append(prunedKeys, mergedKey)
	}
...
func pruneUnresolvedEnvBackedProviderKeys(provider schemas.ModelProvider, keys []schemas.Key) []schemas.Key {
	if len(keys) == 0 {
		return keys
	}

	prunedKeys := make([]schemas.Key, 0, len(keys))
	for _, key := range keys {
		if shouldSkipUnresolvedEnvFileKey(provider, key) {
			logger.Debug("dropping unresolved env-backed provider key %s for provider %s", key.Name, provider)
			continue
		}
...
//
// Bootstrap order (low → high precedence):
//  1. Global rows (workspace_id IS NULL) - historical config.json defaults
//     and the bootstrap upserts; these become the "applies to every
//     workspace that hasn't overridden" instances at runtime.
//  2. Workspace-scoped rows - every (name, workspace_id) tuple gets its own
//     PluginConfig with WorkspaceID set so the framework can wrap the
//     instance with WrapWithWorkspace and the pipeline can dispatch the
//     right one per request (see core/deepintshield.go
//     effectivePluginsForWorkspace).
//  3. config.json plugins layered last via mergePlugins, treated as global.
//
// Without step 2, gateway restarts would silently revert every workspace's
// Cost-Optimization settings to whichever workspace happened to save most
// recently (the pre-refactor behavior).
func loadPlugins(ctx context.Context, config *Config, configData *ConfigData) {
	if config.ConfigStore != nil {
		logger.Debug("getting plugins from store (per-workspace bootstrap)")
		plugins, err := config.ConfigStore.GetAllPluginsForRuntimeBootstrap(ctx)
		if err != nil {
			logger.Warn("failed to get plugins from store: %v", err)
		}
		if plugins != nil {
			config.PluginConfigs = make([]*schemas.PluginConfig, 0, len(plugins))
			for _, plugin := range plugins {
				...
				if plugin.WorkspaceID != nil {
					pluginConfig.WorkspaceID = plugin.WorkspaceID
				}
				if plugin.Name == semanticCachePluginName {
	...
	// Merge with config file plugins. These are always global (workspace_id nil).
	...
	if len(config.PluginConfigs) == 0 {
		...
		for _, plugin := range configData.Plugins {
			if plugin.Version == nil {
				plugin.Version = deepintshield.Ptr(int16(1))
			}
...
func initEncryption(configData *ConfigData) error {
	if configData.EncryptionKey == nil || configData.EncryptionKey.GetValue() == "" {
		// Checking if DEEPINTSHIELD_ENCRYPTION_KEY environment variable is set
		if GetenvCompat("DEEPINTSHIELD_ENCRYPTION_KEY", "DEEPINTSHIELD_ENCRYPTION_KEY") != "" {
			configData.EncryptionKey = schemas.NewEnvVar("env.DEEPINTSHIELD_ENCRYPTION_KEY")
		}
	}
...
func (c *Config) GetProviderConfigRaw(provider schemas.ModelProvider) (*configstore.ProviderConfig, error) {
	...
	// Return direct reference for maximum performance - this is used by DeepIntShield core
...
func (i *pluginChunkInterceptor) InterceptChunk(ctx *schemas.DeepIntShieldContext, req *schemas.HTTPRequest, stream *schemas.DeepIntShieldStreamChunk) (*schemas.DeepIntShieldStreamChunk, error) {
...
// ReloadPlugin adds or updates a plugin in the registry, keyed by
// (name, workspace_id). A workspace-scoped instance replaces only the entry
// with the same workspace tag; a global (untagged) instance replaces only
// other global entries with the same name. This is what lets two workspaces
// hold independent semantic_cache instances side-by-side after the runtime
// per-workspace dispatch landed - see schemas.WorkspaceScoped for the rule
// the pipeline applies at request time.
func (c *Config) ReloadPlugin(plugin schemas.BasePlugin) error {
	c.pluginsMu.Lock()
	defer c.pluginsMu.Unlock()

	name := plugin.GetName()
	incomingWS := plugins.WorkspaceIDOf(plugin)
	...
	for {
		...
		if oldPlugins == nil {
			...
			for _, p := range *oldPlugins {
				if p.GetName() == name && plugins.WorkspaceIDOf(p) == incomingWS {
					newPlugins = append(newPlugins, plugin) // Replace same (name, workspace) slot
					replaced = true
				} else {
					newPlugins = append(newPlugins, p) // Keep - different name OR different workspace
				}
			}

			if !replaced {
				newPlugins = append(newPlugins, plugin) // First instance for this (name, workspace)
			}
		}
...
// UnregisterPlugin removes the global (untagged) instance of a plugin from
// the registry. Use UnregisterWorkspacePlugin to target a specific
// workspace-scoped instance instead.
func (c *Config) UnregisterPlugin(name string) error {
	return c.unregisterPluginByWorkspace(name, "")
}

// UnregisterWorkspacePlugin removes the (name, workspaceID)-keyed instance
// from the registry. Used when a workspace deletes a plugin so the other
// workspaces' instances and any global default stay in place.
func (c *Config) UnregisterWorkspacePlugin(name, workspaceID string) error {
	return c.unregisterPluginByWorkspace(name, workspaceID)
}

func (c *Config) unregisterPluginByWorkspace(name, workspaceID string) error {
	c.pluginsMu.Lock()
	defer c.pluginsMu.Unlock()
	...
	for {
		...
		found := false
		for _, p := range *oldPlugins {
			if p.GetName() == name && plugins.WorkspaceIDOf(p) == workspaceID {
				found = true
				continue
			}
		...
		if c.BasePlugins.CompareAndSwap(oldPlugins, &newPlugins) {
			// Order metadata is per-name (placement/order applies to all
			// workspace copies of the same plugin); only drop it once no
			// instances remain.
			anyLeftForName := false
...
func (c *Config) AddProvider(ctx context.Context, provider schemas.ModelProvider, config configstore.ProviderConfig) error {
	...
	tenantScopedRequest := c.ConfigStore != nil && tenantctx.TenantIDFromContext(ctx) != ""
	if tenantScopedRequest {
		existingConfig, err := c.ConfigStore.GetProviderConfig(ctx, provider)
		if err == nil && existingConfig != nil {
			return fmt.Errorf("provider %s: %w", provider, ErrAlreadyExists)
		}
		if err != nil && !errors.Is(err, configstore.ErrNotFound) {
			return fmt.Errorf("failed to check provider config in store: %w", err)
		}
	} else {
	...
	if err := ensureProviderConfigHashes(provider, &config); err != nil {
		return fmt.Errorf("failed to generate provider hashes: %w", err)
	}
	// First add the provider to the store
	skipDBUpdate := false
	if ctx.Value(schemas.DeepIntShieldContextKeySkipDBUpdate) != nil {
		if skip, ok := ctx.Value(schemas.DeepIntShieldContextKeySkipDBUpdate).(bool); ok {
			skipDBUpdate = skip
		}
	}
...
// SetDeepIntShieldClient sets the DeepIntShield client in the store.
// This is used to allow the store to access the DeepIntShield client.
// This is useful for the MCP handler to access the DeepIntShield client.
func (c *Config) SetDeepIntShieldClient(client *deepintshield.DeepIntShield) {
	c.muMCP.Lock()
...
func (c *Config) GetMCPClient(id string) (*schemas.MCPClientConfig, error) {
	...
	if c.client == nil {
		return nil, fmt.Errorf("deepintshield client not set")
	}
...
func (c *Config) AddMCPClient(ctx context.Context, clientConfig *schemas.MCPClientConfig) error {
	if c.client == nil {
		return fmt.Errorf("deepintshield client not set")
	}
	c.muMCP.Lock()
	defer c.muMCP.Unlock()
	if c.MCPConfig == nil {
...
//   - Removes the MCP client from the DeepIntShield client
...
func (c *Config) GetVectorStoreConfigRedacted(ctx context.Context) (*vectorstore.Config, error) {
	...
	case vectorstore.VectorStoreTypeWeaviate:
		...
		if err := decodeVectorStoreConfig(vectorStoreConfig.Config, &weaviateConfig); err != nil {
			return nil, err
		}
		if weaviateConfig.APIKey != nil {
			weaviateConfig.APIKey = weaviateConfig.APIKey.Redacted()
		}
		if weaviateConfig.Host != nil {
			host := *weaviateConfig.Host
			weaviateConfig.Host = &host
		}
		if weaviateConfig.GrpcConfig != nil && weaviateConfig.GrpcConfig.Host != nil {
			host := *weaviateConfig.GrpcConfig.Host
			grpcConfig := *weaviateConfig.GrpcConfig
			grpcConfig.Host = &host
			weaviateConfig.GrpcConfig = &grpcConfig
		}
		redactedVectorStoreConfig.Config = weaviateConfig
	case vectorstore.VectorStoreTypeRedis:
		var redisConfig vectorstore.RedisConfig
		if err := decodeVectorStoreConfig(vectorStoreConfig.Config, &redisConfig); err != nil {
			return nil, err
		}
		if redisConfig.Password != nil {
			redisConfig.Password = redisConfig.Password.Redacted()
		}
		redactedVectorStoreConfig.Config = redisConfig
	case vectorstore.VectorStoreTypeQdrant:
		var qdrantConfig vectorstore.QdrantConfig
		if err := decodeVectorStoreConfig(vectorStoreConfig.Config, &qdrantConfig); err != nil {
			return nil, err
		}
		if qdrantConfig.APIKey.GetValue() != "" {
			qdrantConfig.APIKey = *qdrantConfig.APIKey.Redacted()
		}
		redactedVectorStoreConfig.Config = qdrantConfig
	case vectorstore.VectorStoreTypePinecone:
		var pineconeConfig vectorstore.PineconeConfig
		if err := decodeVectorStoreConfig(vectorStoreConfig.Config, &pineconeConfig); err != nil {
			return nil, err
		}
		if pineconeConfig.APIKey.GetValue() != "" {
			pineconeConfig.APIKey = *pineconeConfig.APIKey.Redacted()
		}
		redactedVectorStoreConfig.Config = pineconeConfig
	default:
		return nil, nil
	}
...
func decodeVectorStoreConfig(raw any, out any) error {
	...
	case *string:
		if typed == nil {
			return fmt.Errorf("vector store config is nil")
		}
		if err := json.Unmarshal([]byte(*typed), out); err != nil {
			return fmt.Errorf("failed to unmarshal vector store config: %w", err)
		}
	default:
		...
		if err != nil {
			return fmt.Errorf("failed to marshal vector store config: %w", err)
		}
...
func ValidateCustomProvider(config configstore.ProviderConfig, provider schemas.ModelProvider) error {
	...
	if deepintshield.IsStandardProvider(provider) {
	...
	if !deepintshield.IsSupportedBaseProvider(cpc.BaseProviderType) {
...
func (c *Config) AddProviderKeysToSemanticCacheConfig(config *schemas.PluginConfig) error {
	if config.Name != semanticCachePluginName {
		return nil
	}
	...
	// Check if provider key exists and is a string. If missing, resolve from configured providers.
	...
	if strings.TrimSpace(provider) == "" {
		availableProviders := c.GetAvailableProviders()
		sort.Slice(availableProviders, func(i, j int) bool {
			return string(availableProviders[i]) < string(availableProviders[j])
		})

		if len(availableProviders) == 0 {
			return fmt.Errorf("semantic_cache plugin missing required provider key and no providers configured")
		}

		if len(availableProviders) > 1 {
			return fmt.Errorf("semantic_cache plugin missing provider and multiple providers are configured: %v", availableProviders)
		}
	...
	// Local-sidecar embedding providers (huggingface/local) are served by the
	// in-cluster deepintshield-models service and need NO external provider keys
	// (the sidecar authenticates via networkConfig). Skip the key lookup so it
	// doesn't fail "provider config not found" — that lookup only made sense for
	// cloud embedding providers (openai/cohere/…).
	...
	if err != nil {
		if existing, ok := configMap["keys"].([]interface{}); ok && len(existing) > 0 {
			return nil
		}
...
func (c *Config) GetAvailableProviders() []schemas.ModelProvider {
	...
	for provider, config := range c.Providers {
		...
		for _, key := range config.Keys {
			if key.Value.GetValue() != "" || deepintshield.CanProviderKeyValueBeEmpty(provider) {
