...
// SendJSONCached emits the same 200 + JSON body as SendJSON but adds an
// HTTP `Cache-Control: private, max-age=<ttlSeconds>, stale-while-revalidate=600`
// header so the BROWSER serves the cached response synchronously on revisits
// inside the TTL - no spinner, no waterfall, no SWR-style flash. The
// `stale-while-revalidate=600` window lets the browser show stale data
// immediately while a background fetch refreshes it within 10 min of expiry.
//
// `private` because dashboard data is user/workspace-scoped and shouldn't
// land in shared proxies. Combined with the RTK Query global config in
// ui/lib/store/apis/baseApi.ts, this gives a two-layer cache: RTK Query for
// in-session navigation; HTTP Cache-Control for cross-session navigation.
//
// Pick ttlSeconds to match how often the underlying data actually changes:
//   - config / settings rows that the operator edits manually → 300
//   - histograms / matview-backed reads that update every minute → 60
//   - "last 1h" rolling windows that always include recent data → 30
func SendJSONCached(ctx *fasthttp.RequestCtx, data interface{}, ttlSeconds int) {
	if ttlSeconds <= 0 {
		ttlSeconds = 60
	}
	ctx.Response.Header.Set("Cache-Control",
		fmt.Sprintf("private, max-age=%d, stale-while-revalidate=600", ttlSeconds))
	SendJSON(ctx, data)
}
...
// SendError sends a DeepIntShieldError response
...
// SendDeepIntShieldError sends a DeepIntShieldError response
func SendDeepIntShieldError(ctx *fasthttp.RequestCtx, deepintshieldErr *schemas.DeepIntShieldError) {
	if deepintshieldErr.StatusCode != nil {
		ctx.SetStatusCode(*deepintshieldErr.StatusCode)
	} else if !deepintshieldErr.IsDeepIntShieldError {
	...
	ctx.SetContentType("application/json")
	if encodeErr := json.NewEncoder(ctx).Encode(deepintshieldErr); encodeErr != nil {
...
func streamLargeResponseIfActive(ctx *fasthttp.RequestCtx, deepintshieldCtx *schemas.DeepIntShieldContext) bool {
...
func SendSSEError(ctx *fasthttp.RequestCtx, deepintshieldErr *schemas.DeepIntShieldError) {
