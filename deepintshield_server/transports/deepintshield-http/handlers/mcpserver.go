...
type MCPToolManager interface {
	...
	ExecuteChatMCPTool(ctx context.Context, toolCall *schemas.ChatAssistantMessageToolCall) (*schemas.ChatMessage, *schemas.DeepIntShieldError)
	ExecuteResponsesMCPTool(ctx context.Context, toolCall *schemas.ResponsesToolMessage) (*schemas.ResponsesMessage, *schemas.DeepIntShieldError)
}
...
func (h *MCPServerHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.DeepIntShieldHTTPMiddleware) {
...
func (h *MCPServerHandler) handleMCPServer(ctx *fasthttp.RequestCtx) {
	...
	// Convert context
	deepintshieldCtx, cancel := lib.ConvertToDeepIntShieldContext(ctx, false, h.config.GetHeaderMatcher())
	defer cancel()

	// Use mcp-go server to handle the request
	// HandleMessage processes JSON-RPC messages and returns appropriate responses
	response := mcpServer.HandleMessage(deepintshieldCtx, ctx.PostBody())
...
func (h *MCPServerHandler) syncServer(server *server.MCPServer, availableTools []schemas.ChatTool, toolFilter []string) {
	...
	for _, tool := range availableTools {
		...
		handler := func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			if toolFilter != nil {
				ctx = context.WithValue(ctx, schemas.DeepIntShieldContextKey("mcp-include-tools"), toolFilter)
			}
			// Convert to DeepIntShield tool call format
			...
			if err != nil {
				return mcp.NewToolResultError(fmt.Sprintf("Tool execution failed: %v", deepintshield.GetErrorMessage(err))), nil
			}
...
func (h *MCPServerHandler) fetchToolsForVK(vk *tables.TableVirtualKey) ([]schemas.ChatTool, []string) {
	...
	if len(vk.MCPConfigs) > 0 {
		...
		ctx = context.WithValue(ctx, schemas.DeepIntShieldContextKey("mcp-include-tools"), executeOnlyTools)
...
func getVKFromRequest(ctx *fasthttp.RequestCtx) string {
	if value := strings.TrimSpace(string(ctx.Request.Header.Peek(string(schemas.DeepIntShieldContextKeyVirtualKey)))); value != "" {
		return value
	}
