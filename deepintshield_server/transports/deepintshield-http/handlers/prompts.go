...
// requireWorkspaceWrite gates destructive prompt-repo writes against the
// caller's workspace permission. Mirrors the helper in GovernanceHandler.
func (h *PromptsHandler) requireWorkspaceWrite(ctx *fasthttp.RequestCtx) bool {
	if h.store == nil {
		return true
	}
	...
	if ws := tenantctx.WorkspaceIDFromContext(ctx); ws != "" {
		allowed = CanManageWorkspaceByID(ctx, h.store, user, ws)
	} else {
		allowed = CanManageTenant(ctx, h.store, user, user.TenantID)
	}
	if !allowed {
		SendError(ctx, fasthttp.StatusForbidden, "Only workspace admins, tenant owners/admins, or system admins can modify prompts")
		return false
	}
	return true
}
...
func (h *PromptsHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.DeepIntShieldHTTPMiddleware) {
	// Folders
...
// getPrompts handles GET /api/prompt-repo/prompts. Supports an optional
// ?workspace_id= filter that narrows to rows scoped to the workspace plus
// org-wide rows (workspace_id IS NULL). Empty / missing returns the full
// tenant view (legacy behaviour).
func (h *PromptsHandler) getPrompts(ctx *fasthttp.RequestCtx) {
	...
	// Fall back to the sidebar's active workspace from the request context
	// (stamped by validateSession from X-Active-Workspace-Id) when the
	// caller didn't pass an explicit query param.
	if workspaceID == "" {
	...
	var err error
	if workspaceID != "" {
		prompts, err = h.store.GetPromptsScoped(ctx, folderID, workspaceID)
	} else {
...
func (h *PromptsHandler) createPrompt(ctx *fasthttp.RequestCtx) {
	...
	// Stamp the caller's workspace on the new prompt so it shows up under
	// the active workspace's Playground tab. Without this stamp the
	// strict workspace-scoped read path (GetPromptsScoped) would render
	// the just-created prompt invisible.
	if ws := tenantctx.WorkspaceIDFromContext(ctx); ws != "" {
		prompt.WorkspaceID = &ws
	}
