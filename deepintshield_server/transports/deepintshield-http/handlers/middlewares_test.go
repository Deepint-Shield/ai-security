...
type mockSessionConfigStore struct {
...
func (m *mockSessionConfigStore) GetSession(_ context.Context, token string) (*tables.SessionsTable, error) {
	if m == nil {
		return nil, nil
	}
...
func (m *mockSessionConfigStore) GetSingleTenantID(_ context.Context) (string, error) {
	if m == nil {
		return "", nil
	}
	return m.singleTenantID, nil
}

func (m *mockSessionConfigStore) GetVirtualKeyByValue(_ context.Context, value string) (*tables.TableVirtualKey, error) {
	if m == nil {
...
func TestChainMiddlewares_SingleMiddleware(t *testing.T) {
	...
	middleware := schemas.DeepIntShieldHTTPMiddleware(func(next fasthttp.RequestHandler) fasthttp.RequestHandler {
...
func TestChainMiddlewares_MultipleMiddlewares(t *testing.T) {
	...
	middleware1 := schemas.DeepIntShieldHTTPMiddleware(func(next fasthttp.RequestHandler) fasthttp.RequestHandler {
		return func(ctx *fasthttp.RequestCtx) {
	...
	middleware2 := schemas.DeepIntShieldHTTPMiddleware(func(next fasthttp.RequestHandler) fasthttp.RequestHandler {
		return func(ctx *fasthttp.RequestCtx) {
	...
	middleware3 := schemas.DeepIntShieldHTTPMiddleware(func(next fasthttp.RequestHandler) fasthttp.RequestHandler {
		return func(ctx *fasthttp.RequestCtx) {
...
func TestAuthMiddleware_DisabledAuthConfig_AttachesSessionTenant(t *testing.T) {
	SetLogger(&mockLogger{})

	userID := "user-1"
	userEmail := "alice@example.com"
	store := &mockSessionConfigStore{
		sessions: map[string]*tables.SessionsTable{
			"session-token": {
				TenantID:  userEmail,
				UserID:    &userID,
	...
	ctx := &fasthttp.RequestCtx{}
	ctx.Request.SetRequestURI("/api/prompt-repo/prompts")
	ctx.Request.Header.SetCookie("token", "session-token")

	nextCalled := false
	next := func(ctx *fasthttp.RequestCtx) {
		nextCalled = true
		if got := ctx.UserValue(schemas.DeepIntShieldContextKeyTenantID); got != userEmail {
			t.Fatalf("expected tenant %q, got %#v", userEmail, got)
		}
		if got := ctx.UserValue(schemas.DeepIntShieldContextKeyUserID); got != userID {
			t.Fatalf("expected user id %q, got %#v", userID, got)
		}
		if got := ctx.UserValue(schemas.DeepIntShieldContextKeySessionToken); got != "session-token" {
			t.Fatalf("expected session token to be attached, got %#v", got)
		}
	}
	...
	if !nextCalled {
		t.Fatal("Next handler should be called when auth is disabled")
	}
}

func TestAuthMiddleware_DisabledAuthConfig_AttachesSingleTenantWithoutSession(t *testing.T) {
	SetLogger(&mockLogger{})

	store := &mockSessionConfigStore{
		singleTenantID: "alice@example.com",
	}
	...
	next := func(ctx *fasthttp.RequestCtx) {
		nextCalled = true
		if got := ctx.UserValue(schemas.DeepIntShieldContextKeyTenantID); got != "alice@example.com" {
			t.Fatalf("expected tenant %q, got %#v", "alice@example.com", got)
		}
	}
...
func TestAuthMiddleware_EnabledAuthConfig_AllowsVirtualKeyForRAGEvaluate(t *testing.T) {
	SetLogger(&mockLogger{})
	...
	ctx := &fasthttp.RequestCtx{}
	ctx.Request.SetRequestURI("/api/rag-security/evaluate")
	ctx.Request.Header.SetMethod(fasthttp.MethodPost)
	ctx.Request.Header.Set("x-bf-vk", "sk-bf-valid")
	...
	handler := am.APIMiddleware()(func(ctx *fasthttp.RequestCtx) {
		...
		if got := ctx.UserValue(schemas.DeepIntShieldContextKeyTenantID); got != "tenant-a" {
			t.Fatalf("expected tenant %q, got %#v", "tenant-a", got)
		}
		if got := ctx.UserValue(schemas.DeepIntShieldContextKeyVirtualKey); got != "sk-bf-valid" {
			t.Fatalf("expected virtual key %q, got %#v", "sk-bf-valid", got)
		}
	})
	handler(ctx)

	if !nextCalled {
		t.Fatal("expected RAG evaluate route to allow valid virtual key auth")
	}
}

func TestAuthMiddleware_EnabledAuthConfig_AllowsVirtualKeyForGuardrailEvaluate(t *testing.T) {
	SetLogger(&mockLogger{})
	...
	ctx := &fasthttp.RequestCtx{}
	ctx.Request.SetRequestURI("/api/guardrails/evaluate")
	ctx.Request.Header.SetMethod(fasthttp.MethodPost)
	ctx.Request.Header.Set("x-bf-vk", "sk-bf-valid")
	...
	if !nextCalled {
		t.Fatal("expected guardrail evaluate route to allow valid virtual key auth")
	}
}

func TestAuthMiddleware_EnabledAuthConfig_RejectsInvalidVirtualKeyForRAGEvaluate(t *testing.T) {
	SetLogger(&mockLogger{})
	...
	ctx.Request.SetRequestURI("/api/rag-security/evaluate")
	ctx.Request.Header.SetMethod(fasthttp.MethodPost)
	ctx.Request.Header.Set("x-bf-vk", "sk-bf-missing")
	...
	if nextCalled {
		t.Fatal("expected invalid virtual key to block RAG evaluate route")
	}
	if got := ctx.Response.StatusCode(); got != fasthttp.StatusForbidden {
		t.Fatalf("expected status %d, got %d", fasthttp.StatusForbidden, got)
	}
}

func TestAuthMiddleware_EnabledAuthConfig_RejectsInvalidVirtualKeyForGuardrailEvaluate(t *testing.T) {
	SetLogger(&mockLogger{})
	...
	if nextCalled {
		t.Fatal("expected invalid virtual key to block guardrail evaluate route")
	}
	if got := ctx.Response.StatusCode(); got != fasthttp.StatusForbidden {
		t.Fatalf("expected status %d, got %d", fasthttp.StatusForbidden, got)
	}
...
func TestInferenceMiddleware_DisableAuthOnInference_AttachesSessionTenant(t *testing.T) {
	SetLogger(&mockLogger{})
	...
	ctx.Request.Header.SetCookie("token", "inference-session")
	...
	next := func(ctx *fasthttp.RequestCtx) {
		...
		if got := ctx.UserValue(schemas.DeepIntShieldContextKeySessionToken); got != "inference-session" {
			t.Fatalf("expected session token to be attached, got %#v", got)
		}
	}
	...
	if !nextCalled {
		t.Fatal("Next handler should be called when inference auth is disabled")
	}
}
...
func TestFasthttpToHTTPRequest_PathParams(t *testing.T) {
	...
	ctx.SetUserValue("DeepIntShieldContextKeyRequestID", "req-123")
	...
	systemKeys := []string{"DeepIntShieldContextKeyRequestID", "trace_id", "span_id"}
...
func TestRequestDecompressionMiddleware_InvalidCompressedBody(t *testing.T) {
	...
	if err := json.Unmarshal(ctx.Response.Body(), &deepintshieldErr); err != nil {
		t.Fatalf("failed to decode error response: %v", err)
	}
	if deepintshieldErr.Error == nil || !strings.Contains(deepintshieldErr.Error.Message, "invalid compressed request body") {
		t.Fatalf("unexpected error message: %#v", deepintshieldErr.Error)
	}
}
...
func TestRequestDecompressionMiddleware_UnsupportedEncoding(t *testing.T) {
	...
	if deepintshieldErr.Error == nil || !strings.Contains(deepintshieldErr.Error.Message, "unsupported Content-Encoding") {
		t.Fatalf("unexpected error message: %#v", deepintshieldErr.Error)
	}
}
...
func TestRequestDecompressionMiddleware_DecompressedSizeLimit(t *testing.T) {
	...
	if deepintshieldErr.Error == nil || !strings.Contains(deepintshieldErr.Error.Message, "decompressed request body exceeds max allowed size") {
		t.Fatalf("unexpected error message: %#v", deepintshieldErr.Error)
	}
}
