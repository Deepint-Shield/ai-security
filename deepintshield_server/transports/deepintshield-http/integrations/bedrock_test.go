...
func Test_createBedrockRerankRouteRequestConverter(t *testing.T) {
	...
	assert.Equal(t, schemas.Bedrock, deepintshieldReq.RerankRequest.Provider)
	assert.Equal(t, "capital of france", deepintshieldReq.RerankRequest.Query)
	require.Len(t, deepintshieldReq.RerankRequest.Documents, 1)
...
func Test_s3PutObjectPostCallback(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			response: &schemas.DeepIntShieldFileUploadResponse{
				ID: "file-123",
			},
			wantStatus: 200,
			wantETag:   "\"file-123\"",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, "deepintshield", string(ctx.Response.Header.Peek("x-amz-request-id")))
...
func Test_s3GetObjectPostCallback(t *testing.T) {
	tests := []struct {
		...
		{
			name: "valid response",
			response: &schemas.DeepIntShieldFileContentResponse{
...
func Test_s3HeadObjectPostCallback(t *testing.T) {
	tests := []struct {
		...
		{
			name: "valid response",
			response: &schemas.DeepIntShieldFileRetrieveResponse{
				ID:    "file-789",
				Bytes: 1024,
			},
			wantStatus: 200,
...
func Test_extractBedrockBatchListQueryParams(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			deepintshieldCtx := createTestDeepIntShieldContext()
			err := callback(ctx, deepintshieldCtx, req)
...
func Test_extractBedrockJobArnFromPath(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			deepintshieldCtx := createTestDeepIntShieldContextWithProvider(tt.provider)
			err := callback(ctx, deepintshieldCtx, req)
...
func Test_extractS3ListObjectsV2Params(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, tt.wantBucket, deepintshieldCtx.Value(s3ContextKeyBucket))
			assert.Equal(t, tt.wantPrefix, deepintshieldCtx.Value(s3ContextKeyPrefix))
			assert.Equal(t, tt.wantMaxKeys, deepintshieldCtx.Value(s3ContextKeyMaxKeys))
		})
	}
}
...
func createTestDeepIntShieldContext() *schemas.DeepIntShieldContext {
	deepintshieldCtx := schemas.NewDeepIntShieldContext(context.Background(), schemas.NoDeadline)
	deepintshieldCtx.SetValue(deepintshieldContextKeyProvider, schemas.Bedrock)
	return deepintshieldCtx
}

func createTestDeepIntShieldContextWithProvider(provider schemas.ModelProvider) *schemas.DeepIntShieldContext {
	deepintshieldCtx := schemas.NewDeepIntShieldContext(context.Background(), schemas.NoDeadline)
