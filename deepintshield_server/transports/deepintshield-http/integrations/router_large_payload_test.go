...
func TestCreateHandler_SkipsRequestParserInLargePayloadMode(t *testing.T) {
	...
	router.SetLargePayloadHook(func(ctx *fasthttp.RequestCtx, deepintshieldCtx *schemas.DeepIntShieldContext, routeType RouteConfigType) (bool, error) {
		return true, nil
	})
	...
	ctx.SetUserValue(schemas.DeepIntShieldContextKeyHTTPRequestType, schemas.ChatCompletionRequest)
...
func TestResolveLargePayloadMetadata_SyncPath(t *testing.T) {
	ctx := schemas.NewDeepIntShieldContext(nil, time.Time{})
	meta := &schemas.LargePayloadMetadata{Model: "gpt-4o"}
	ctx.SetValue(schemas.DeepIntShieldContextKeyLargePayloadMetadata, meta)
...
func TestResolveLargePayloadMetadata_DeferredReady(t *testing.T) {
	...
	ctx.SetValue(schemas.DeepIntShieldContextKeyDeferredLargePayloadMetadata, (<-chan *schemas.LargePayloadMetadata)(ch))
	...
	cached, ok := ctx.Value(schemas.DeepIntShieldContextKeyLargePayloadMetadata).(*schemas.LargePayloadMetadata)
	require.True(t, ok)
...
func TestResolveLargePayloadMetadata_SyncTakesPrecedence(t *testing.T) {
	...
	ctx.SetValue(schemas.DeepIntShieldContextKeyLargePayloadMetadata, syncMeta)
