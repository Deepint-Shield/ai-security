...
// extra_fields that DeepIntShieldChatResponse would include.
...
func cursorChunkID(extras *schemas.DeepIntShieldResponseExtraFields) string {
	return "chatcmpl-deepintshield-" + strconv.Itoa(extras.ChunkIndex)
}

// cursorModel returns the best model name available from extra fields.
func cursorModel(extras *schemas.DeepIntShieldResponseExtraFields) string {
...
func convertResponsesStreamToChatChunk(resp *schemas.DeepIntShieldResponsesStreamResponse) (string, interface{}, error) {
	switch resp.Type {
...
func convertResponsesResponseToChatCompletion(resp *schemas.DeepIntShieldResponsesResponse) *cursorChatCompletion {
...
// normalizeDeepIntShieldInputContentBlocks ensures all input messages in a DeepIntShieldResponsesRequest
// have ContentBlocks instead of ContentStr. This is a defense-in-depth normalization that runs
// AFTER ToDeepIntShieldResponsesRequest, which can re-introduce ContentStr when the input is a string.
func normalizeDeepIntShieldInputContentBlocks(req *schemas.DeepIntShieldResponsesRequest) {
	if req == nil {
		return
	}
		...
		routes = append(routes, RouteConfig{
			...
			RequestConverter: func(ctx *schemas.DeepIntShieldContext, req interface{}) (*schemas.DeepIntShieldRequest, error) {
				if openaiReq, ok := req.(*openai.OpenAIResponsesRequest); ok {
					deepintshieldReq := openaiReq.ToDeepIntShieldResponsesRequest(ctx)
					if deepintshieldReq == nil {
						return nil, errors.New("deepintshield responses request conversion returned nil")
					}
...
// NewCursorRouter creates a new CursorRouter with the given deepintshield client.
func NewCursorRouter(client *deepintshield.DeepIntShield, handlerStore lib.HandlerStore, logger schemas.Logger) *CursorRouter {
	routes := []RouteConfig{}
