...
func TestRequestWithSettableExtraParams_OpenAIChatRequest(t *testing.T) {
	...
	t.Run("extra params propagate through ToDeepIntShieldChatRequest", func(t *testing.T) {
		req := &openai.OpenAIChatRequest{
		...
		ctx := schemas.NewDeepIntShieldContext(nil, schemas.NoDeadline)
		deepintshieldReq := req.ToDeepIntShieldChatRequest(ctx)

		require.NotNil(t, deepintshieldReq)
		require.NotNil(t, deepintshieldReq.Params)
		assert.Contains(t, deepintshieldReq.Params.ExtraParams, "guardrailConfig")
	})
}
...
func TestExtraParamsRequiresPassthroughHeader(t *testing.T) {
	...
	t.Run("extra_params NOT extracted without passthrough header", func(t *testing.T) {
		...
		deepintshieldCtx := schemas.NewDeepIntShieldContext(nil, schemas.NoDeadline)
		// Header not set -- simulate router logic
		if deepintshieldCtx.Value(schemas.DeepIntShieldContextKeyPassthroughExtraParams) == true {
...
func TestExtraParamsPassthrough_EndToEnd(t *testing.T) {
	...
	deepintshieldReq := req.ToDeepIntShieldChatRequest(deepintshieldCtx)

	require.NotNil(t, deepintshieldReq)
	require.NotNil(t, deepintshieldReq.Params)
	require.Contains(t, deepintshieldReq.Params.ExtraParams, "guardrailConfig")

	gc, ok := deepintshieldReq.Params.ExtraParams["guardrailConfig"].(map[string]interface{})
	require.True(t, ok)
	assert.Equal(t, "my-guardrail", gc["guardrailIdentifier"])
	assert.Equal(t, "1", gc["guardrailVersion"])
	assert.Equal(t, "disabled", gc["trace"])

	assert.NotContains(t, deepintshieldReq.Params.ExtraParams, "model")
	assert.NotContains(t, deepintshieldReq.Params.ExtraParams, "messages")
	assert.NotContains(t, deepintshieldReq.Params.ExtraParams, "stream")
	assert.NotContains(t, deepintshieldReq.Params.ExtraParams, "temperature")
}
...
func TestExtraParamsSetViaInterfaceMutatesOriginalReq(t *testing.T) {
	...
	deepintshieldCtx := schemas.NewDeepIntShieldContext(nil, schemas.NoDeadline)
	deepintshieldReq, err := chatRoute.RequestConverter(deepintshieldCtx, req)
	require.NoError(t, err)
	require.NotNil(t, deepintshieldReq)
	require.NotNil(t, deepintshieldReq.ChatRequest)
	require.NotNil(t, deepintshieldReq.ChatRequest.Params)
	assert.Contains(t, deepintshieldReq.ChatRequest.Params.ExtraParams, "guardrailConfig",
		"extra params should propagate through RequestConverter to DeepIntShieldChatRequest")
}
