...
func newTestDeepIntShieldContext() *schemas.DeepIntShieldContext {
	return schemas.NewDeepIntShieldContext(context.Background(), schemas.NoDeadline)
}

// TestSendStreamError_PropagatesProviderStatusCode verifies that sendStreamError
// sets the HTTP status code from the provider's DeepIntShieldError.StatusCode field.
// All three providers (OpenAI, Anthropic, Bedrock) return actual HTTP error codes
// for pre-stream errors, so DeepIntShield must propagate them faithfully.
func TestSendStreamError_PropagatesProviderStatusCode(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			deepintshieldCtx := newTestDeepIntShieldContext()
			...
			router.sendStreamError(ctx, deepintshieldCtx, config, deepintshieldErr)
...
// OpenAI error format. OpenAI's ErrorConverter returns *schemas.DeepIntShieldError directly,
// which serializes to {"is_deepintshield_error":false,"status_code":400,"error":{...}}.
func TestSendStreamError_OpenAIErrorFormat(t *testing.T) {
	...
	assert.Contains(t, result, "is_deepintshield_error")
	assert.Contains(t, result, "status_code")
	assert.Contains(t, result, "error")
	assert.Equal(t, false, result["is_deepintshield_error"])
...
// stored in the DeepIntShieldContext are forwarded to the HTTP response. This ensures
...
func TestSendStreamError_ForwardsProviderHeaders(t *testing.T) {
	...
	deepintshieldCtx.SetValue(schemas.DeepIntShieldContextKeyProviderResponseHeaders, map[string]string{
