		...
		RequestConverter: func(ctx *schemas.DeepIntShieldContext, req interface{}) (*schemas.DeepIntShieldRequest, error) {
			if anthropicReq, ok := req.(*anthropic.AnthropicTextRequest); ok {
				return &schemas.DeepIntShieldRequest{
					TextCompletionRequest: anthropicReq.ToDeepIntShieldTextCompletionRequest(ctx),
				}, nil
			}
		...
		routes = append(routes, RouteConfig{
			...
			RequestConverter: func(ctx *schemas.DeepIntShieldContext, req interface{}) (*schemas.DeepIntShieldRequest, error) {
				if anthropicReq, ok := req.(*anthropic.AnthropicMessageRequest); ok {
					deepintshieldReq := anthropicReq.ToDeepIntShieldResponsesRequest(ctx)
					normalizeDeepIntShieldInputContentBlocks(deepintshieldReq)
					return &schemas.DeepIntShieldRequest{
						ResponsesRequest: deepintshieldReq,
					}, nil
				}
				return nil, errors.New("invalid request type")
			},
			...
			StreamConfig: &StreamConfig{
				ResponsesStreamResponseConverter: func(ctx *schemas.DeepIntShieldContext, resp *schemas.DeepIntShieldResponsesStreamResponse) (string, interface{}, error) {
					if shouldUsePassthrough(ctx, resp.ExtraFields.Provider, resp.ExtraFields.ModelRequested, resp.ExtraFields.ModelDeployment) {
						...
						// Fallback: if RawResponse is not available, use deepintshield-to-anthropic conversion
...
func hydrateAnthropicRequestFromLargePayloadMetadata(deepintshieldCtx *schemas.DeepIntShieldContext, req interface{}) {
...
// It also checks for anthropic oauth headers and sets the deepintshield context.
func checkAnthropicPassthrough(ctx *fasthttp.RequestCtx, deepintshieldCtx *schemas.DeepIntShieldContext, req interface{}) error {
	hydrateAnthropicRequestFromLargePayloadMetadata(deepintshieldCtx, req)
	...
	if len(headers) > 0 {
		...
		if len(userAgent) > 0 {
			...
			if strings.Contains(userAgent[0], "claude-cli") {
				deepintshieldCtx.SetValue(schemas.DeepIntShieldContextKeyUserAgent, "claude-cli")
			}
		}
	}

	// Check if anthropic oauth headers are present
	if shouldUsePassthrough(deepintshieldCtx, provider, model, "") {
		deepintshieldCtx.SetValue(schemas.DeepIntShieldContextKeyUseRawRequestBody, true)
		...
		if !isAnthropicAPIKeyAuth(ctx) && (provider == schemas.Anthropic || provider == "") {
			...
			deepintshieldCtx.SetValue(schemas.DeepIntShieldContextKeyExtraHeaders, headers)
			deepintshieldCtx.SetValue(schemas.DeepIntShieldContextKeyURLPath, url)
			...
			deepintshieldCtx.SetValue(schemas.DeepIntShieldContextKeySkipKeySelection, true)
		} else {
			...
			if len(passthroughHeaders) > 0 {
				deepintshieldCtx.SetValue(schemas.DeepIntShieldContextKeyExtraHeaders, passthroughHeaders)
			}
		}
		if provider == schemas.Vertex && hasPromptCachingScopeBetaHeader(headers) {
			deepintshieldCtx.SetValue(schemas.DeepIntShieldContextKeyUseRawRequestBody, false)
			return nil
		}
	}
	return nil
...
func shouldUsePassthrough(ctx *schemas.DeepIntShieldContext, provider schemas.ModelProvider, model string, deployment string) bool {
...
func extractAnthropicListModelsParams(ctx *fasthttp.RequestCtx, deepintshieldCtx *schemas.DeepIntShieldContext, req interface{}) error {
	if listModelsReq, ok := req.(*schemas.DeepIntShieldListModelsRequest); ok {
	...
	return []RouteConfig{
		{
			...
			RequestConverter: func(ctx *schemas.DeepIntShieldContext, req interface{}) (*schemas.DeepIntShieldRequest, error) {
				if anthropicReq, ok := req.(*anthropic.AnthropicMessageRequest); ok {
					...
					normalizeDeepIntShieldInputContentBlocks(deepintshieldReq)
					return &schemas.DeepIntShieldRequest{
						CountTokensRequest: deepintshieldReq,
					}, nil
				}
...
func CreateAnthropicBatchRouteConfigs(pathPrefix string, handlerStore lib.HandlerStore) []RouteConfig {
	...
	routes = append(routes, RouteConfig{
		...
		BatchRequestConverter: func(ctx *schemas.DeepIntShieldContext, req any) (*BatchRequest, error) {
			if anthropicReq, ok := req.(*anthropic.AnthropicBatchCreateRequest); ok {
				// Convert Anthropic batch request items to DeepIntShield format
				isNonAnthropicProvider := false
				var provider schemas.ModelProvider
				var ok bool
				if provider, ok = ctx.Value(deepintshieldContextKeyProvider).(schemas.ModelProvider); ok && provider != schemas.Anthropic {
	...
	routes = append(routes, RouteConfig{
		...
		BatchRequestConverter: func(ctx *schemas.DeepIntShieldContext, req interface{}) (*BatchRequest, error) {
			if retrieveReq, ok := req.(*anthropic.AnthropicBatchRetrieveRequest); ok {
				provider := ctx.Value(deepintshieldContextKeyProvider).(schemas.ModelProvider)
	...
	routes = append(routes, RouteConfig{
		...
		BatchResultsResponseConverter: func(ctx *schemas.DeepIntShieldContext, resp *schemas.DeepIntShieldBatchResultsResponse) (interface{}, error) {
			if resp.ExtraFields.RawResponse != nil {
				return resp.ExtraFields.RawResponse, nil
			}
			return resp, nil
...
func extractAnthropicBatchCreateParams(ctx *fasthttp.RequestCtx, deepintshieldCtx *schemas.DeepIntShieldContext, req interface{}) error {
	...
	deepintshieldCtx.SetValue(deepintshieldContextKeyProvider, schemas.ModelProvider(provider))
	return nil
}
...
func extractAnthropicBatchListQueryParams(ctx *fasthttp.RequestCtx, deepintshieldCtx *schemas.DeepIntShieldContext, req interface{}) error {
	if listReq, ok := req.(*anthropic.AnthropicBatchListRequest); ok {
		// Extract provider from header, default to Anthropic
		provider := string(ctx.Request.Header.Peek("x-model-provider"))
		if provider == "" {
...
func extractAnthropicBatchIDFromPath(ctx *fasthttp.RequestCtx, deepintshieldCtx *schemas.DeepIntShieldContext, req interface{}) error {
	// Extract provider from header, default to Anthropic
	provider := string(ctx.Request.Header.Peek("x-model-provider"))
	if provider == "" {
		provider = string(schemas.Anthropic)
...
func extractAnthropicFileUploadParams(ctx *fasthttp.RequestCtx, deepintshieldCtx *schemas.DeepIntShieldContext, req interface{}) error {
	provider := string(ctx.Request.Header.Peek("x-model-provider"))
	if provider == "" {
		provider = string(schemas.Anthropic)
	}
...
func extractAnthropicFileListQueryParams(ctx *fasthttp.RequestCtx, deepintshieldCtx *schemas.DeepIntShieldContext, req interface{}) error {
...
func extractAnthropicFileIDFromPath(ctx *fasthttp.RequestCtx, deepintshieldCtx *schemas.DeepIntShieldContext, req interface{}) error {
	// Extract provider from header, default to Anthropic
	provider := string(ctx.Request.Header.Peek("x-model-provider"))
	if provider == "" {
		provider = string(schemas.Anthropic)
...
func CreateAnthropicFilesRouteConfigs(pathPrefix string, handlerStore lib.HandlerStore) []RouteConfig {
	...
	routes = append(routes, RouteConfig{
		...
		FileRequestConverter: func(ctx *schemas.DeepIntShieldContext, req any) (*FileRequest, error) {
...
// NewAnthropicRouter creates a new AnthropicRouter with the given deepintshield client.
func NewAnthropicRouter(client *deepintshield.DeepIntShield, handlerStore lib.HandlerStore, logger schemas.Logger) *AnthropicRouter {
