...
// GetDefaultConfigDir returns the OS-specific default configuration directory for DeepIntShield.
// This follows standard conventions:
// - Linux/macOS: ~/.config/deepintshield
// - Windows: %APPDATA%\deepintshield
...
func GetDefaultConfigDir(appDir string) string {
	...
	case "windows":
		...
		if appData := os.Getenv("APPDATA"); appData != "" {
			configDir = filepath.Join(appData, "deepintshield")
		} else {
			// Fallback to user home directory
			if homeDir, err := os.UserHomeDir(); err == nil {
				configDir = filepath.Join(homeDir, "AppData", "Roaming", "deepintshield")
			}
		}
	default:
		// Linux, macOS and other Unix-like systems: ~/.config/deepintshield
		if homeDir, err := os.UserHomeDir(); err == nil {
			configDir = filepath.Join(homeDir, ".config", "deepintshield")
		}
	}

	// If we couldn't determine the config directory, fall back to current directory
	if configDir == "" {
		configDir = "./deepintshield-data"
	}
...
func (s *DeepIntShieldHTTPServer) registerPluginWithStatus(ctx context.Context, name string, path *string, config any, failOnError bool) error {
...
func (s *DeepIntShieldHTTPServer) registerInstantiatedPluginWithStatus(name string, plugin schemas.BasePlugin, failOnError bool) error {
	if plugin == nil {
		err := fmt.Errorf("plugin %s instantiated but returned nil", name)
		s.Config.UpdatePluginOverallStatus(name, name, schemas.PluginStatusError,
			[]string{err.Error()}, []schemas.PluginType{})
		if failOnError {
			return err
		}
		return nil
...
func (s *DeepIntShieldHTTPServer) CollectObservabilityPlugins() []schemas.ObservabilityPlugin {
...
func (s *DeepIntShieldHTTPServer) updateKeyStatus(
	ctx context.Context,
