...
//
//	To bind to all interfaces for container usage, set DEEPINTSHIELD_HOST=0.0.0.0 or use -host 0.0.0.0
//
// Integration Support:
// DeepIntShield supports multiple AI provider integrations through dedicated HTTP endpoints.
// Each integration exposes API-compatible endpoints that accept the provider's native request format,
// automatically convert it to DeepIntShield's unified format, process it, and return the expected response format.
//
...
// from DeepIntShield's unified model routing, fallbacks, monitoring capabilities, and high-performance configuration management.
//
...
import (
	...
	deepintshieldServer "github.com/deepint-shield/ai-security/transports/deepintshield-http/server"
)
...
var logger = deepintshield.NewDefaultLogger(schemas.LogLevelInfo)
var server *deepintshieldServer.DeepIntShieldHTTPServer

func centeredBannerLine(content string) string {
	const innerWidth = 59
	if len(content) >= innerWidth {
		return fmt.Sprintf("║ %s ║", content)
	}

	leftPadding := (innerWidth - len(content)) / 2
	rightPadding := innerWidth - len(content) - leftPadding
...
//   - host: Host to bind the server to (default: localhost, can be overridden with DEEPINTSHIELD_HOST env var)
...
func init() {
	...
	defaultHost := lib.GetenvCompat("DEEPINTSHIELD_HOST", "DEEPINTSHIELD_HOST")
	if defaultHost == "" {
		defaultHost = deepintshieldServer.DefaultHost
	}
	defaultLogLevel := strings.ToLower(os.Getenv("LOG_LEVEL"))
	if defaultLogLevel == "" {
		defaultLogLevel = deepintshieldServer.DefaultLogLevel
	}
	// Initializing server
	server = deepintshieldServer.NewDeepIntShieldHTTPServer(Version, uiContent)
	// Updating server properties from flags
	flag.StringVar(&server.Port, "port", deepintshieldServer.DefaultPort, "Port to run the server on")
	flag.StringVar(&server.Host, "host", defaultHost, "Host to bind the server to (default: localhost, override with DEEPINTSHIELD_HOST env var)")
	flag.StringVar(&server.AppDir, "app-dir", deepintshieldServer.DefaultAppDir, "Application data directory (contains config.json and logs)")
	flag.StringVar(&server.LogLevel, "log-level", defaultLogLevel, "Logger level (debug, info, warn, error). Default is info.")
	flag.StringVar(&server.LogOutputStyle, "log-style", deepintshieldServer.DefaultLogOutputStyle, "Logger output type (json or pretty). Default is JSON.")
}
...
func main() {
	...
	// Printing version
	versionLine := centeredBannerLine(Version)
	titleLine := centeredBannerLine("DeepIntShield")
	taglineLine := centeredBannerLine("Secure AI Gateway")
	footerLine := centeredBannerLine("deepintshield")
	fmt.Printf(`
╔═══════════════════════════════════════════════════════════╗
║                                                           ║
%s
...
`, titleLine, versionLine, taglineLine, footerLine)
	...
	deepintshieldServer.SetLogger(logger)
